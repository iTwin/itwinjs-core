...
import { AccessToken, Briefcase, ChangeSet, IModel as HubIModel, MultiCode, CodeState, ContainsSchemaChanges, IModelQuery } from "@bentley/imodeljs-clients";
...
describe.skip("BriefcaseManager", () => {
  ...
  const dumpTestCase = async (projectName: string, iModelName: string, openMode: OpenMode) => {
    ...
    if (openMode === OpenMode.Readonly) {
      await IModelTestUtils.hubClient.IModels().download(accessToken, iModelId, downloadToPathname);
    } else {
      const hubBriefcase: Briefcase = await IModelTestUtils.hubClient.Briefcases().create(accessToken, iModelId);
      await IModelTestUtils.hubClient.Briefcases().download(hubBriefcase, downloadToPathname);
    }

    // Get all change set information including the download link
    const changeSets: ChangeSet[] = await IModelTestUtils.hubClient.ChangeSets().get(accessToken, iModelId);

    // Download change sets
    await IModelTestUtils.hubClient.ChangeSets().download(changeSets, imodelPath);
  };
  ...
  it("should download seed files and change sets for all test cases", async () => {
    await dumpTestCase("NodeJsTestProject", "TestModel", OpenMode.Readonly);
    await dumpTestCase("NodeJsTestProject", "NoVersionsTest", OpenMode.Readonly);
  });
