...
import { OpenMode/*, Id64, DbResult, ChangeSetProcessOption*/ } from "@bentley/bentleyjs-core";
import { /*Appearance, ColorDef,*/ IModelVersion/*, IModel, IModelError, CreateIModelProps*/} from "@bentley/imodeljs-common";
import { IModelTestUtils } from "../IModelTestUtils";
import { BriefcaseManager, /*DictionaryModel, ChangeSetToken,*/ IModelDb, IModelHost } from "../../backend";
...
import {
  ...
  ContainsSchemaChanges,*/ AccessToken, IModelQuery,
...
describe("BriefcaseManager", () => {
  ...
  it.only("should be able to open a first version IModel in Readonly mode", async () => {
    ...
    // Act
    const iModel: IModelDb = await IModelDb.open(accessToken as any, testProjectId, testIModels[0].id, OpenMode.Readonly, iModelVersionMock.object);
  ...
  it.skip("should create a new iModel", async () => {
    // Act
    const iModel: IModelDb = await IModelDb.create(accessToken as any, testProjectId, testIModels[1].name, { rootSubject: { name: "TestSubject" } });
  ...
  it("should reuse open briefcases in Readonly mode", async () => {
    ...
    const iModel0: IModelDb = await IModelDb.open(accessToken as any, testProjectId, testIModels[0].id, OpenMode.Readonly, iModelVersionMock.object);
  ...
  it("should open a briefcase of an iModel with no versions", async () => {
    // Act
    const queryRes = await iModelHubClientMock.object.IModels().get(accessToken as any, testProjectId, new IModelQuery().byName(testIModels[2].name));
    assert(queryRes.length === 1 && queryRes[0].wsgId, "Correct iModel not found");
    const iModelNoVerId = queryRes[0].wsgId;
    const iModelNoVer: IModelDb = await IModelDb.open(accessToken as any, testProjectId, iModelNoVerId, OpenMode.Readonly);
  ...
  //   const iModelFirstVersion: IModelDb = await IModelDb.open(accessToken as any, testProjectId, testIModels[0].id, OpenMode.Readonly, IModelVersion.first());
  //   assert.exists(iModelFirstVersion);

  //   for (const [arrayIndex, versionName] of testVersionNames.entries()) {
  //     const iModelFromVersion: IModelDb = await IModelDb.open(accessToken as any, testProjectId, testIModels[0].id, OpenMode.Readonly, IModelVersion.asOfChangeSet(testIModels[0].changeSets[arrayIndex].wsgId));
  //     assert.exists(iModelFromVersion);

  //     const iModelFromChangeSet: IModelDb = await IModelDb.open(accessToken as any, testProjectId, testIModels[0].id, OpenMode.Readonly, IModelVersion.named(versionName));
  ...
  //   const iModels: HubIModel[] = await iModelHubClientMock.object.IModels().get(accessToken as any, testProjectId, new IModelQuery().byName(testIModels[2].name));
  //   for (const iModel of iModels) {
  //     await iModelHubClientMock.object.IModels().delete(accessToken, testProjectId, iModel.wsgId);
  //   }
  //   timer.end();

  //   // Create a new iModel on the Hub (by uploading a seed file)
  //   timer = new Timer("create iModel");
  //   const rwIModel: IModelDb = await IModelDb.create(accessToken as any, testProjectId, testIModels[2].name, "TestSubject");
  ...
  //   assert.isTrue(await rwIModel.concurrencyControl.areAvailable(accessToken as any));
  ...
  //   // Reserve all of the codes that are required by the new model and category.
  //   try {
  //     await rwIModel.concurrencyControl.request(accessToken as any);
  //   } catch (err) {
  //     if (err instanceof ConcurrencyControl.RequestError) {
  //       assert.fail(JSON.stringify(err.unavailableCodes) + ", " + JSON.stringify(err.unavailableLocks));
  //     }
  ...
  //   const codeStates: MultiCode[] = await rwIModel.concurrencyControl.codes.query(accessToken as any, category.code.spec, category.code.scope);
  ...
  //   await rwIModel.pushChanges(accessToken as any);
