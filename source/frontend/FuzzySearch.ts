...
export class FuzzySearch<T> {
   ...
   * @param key The name of the member to search in the searchedObjects.
  ...
  public search(searchedObjects: T[], keys: string[], pattern: string): FuzzySearchResults<T> {
    ...
    // but that results in matches that don't make sense when there are "good" matches. So we throw out  matches where the match score
    // does a sudden increase.
...
export class FuzzySearchResults<T> implements Iterable<T> {
  ...
  public [Symbol.iterator]() {
    const nextFunc = () => {
      ...
      return {
        ...
        value: thisFSR.results[thisFSR.counter++].item as T,
      };
    };
    return {
  ...
  public getResult(resultIndex: number): T | undefined {
    if ((resultIndex < 0) || (resultIndex > this.results.length))
      return undefined;

    return this.results[resultIndex].item as T;
  }

  public getMatchedValue(resultIndex: number): string | undefined {
    if ((resultIndex < 0) || (resultIndex > this.results.length))
      return undefined;
    return this.results[resultIndex].matches[0].value;
  }

  public getMatchedKey(resultIndex: number): string | undefined {
    if ((resultIndex < 0) || (resultIndex > this.results.length))
      return undefined;
    return this.results[resultIndex].matches[0].key;
  }

  public getBoldMask(resultIndex: number): boolean[] | undefined {
    if ((resultIndex < 0) || (resultIndex > this.results.length))
      return undefined;

    const thisResult = this.results[resultIndex];
    if (thisResult.boldMask)
      return thisResult.boldMask;

    const thisMatchedString: string = thisResult.matches[0].value;
    const keyinLength = thisMatchedString.length;
    const boldMask: boolean[] = new Array<boolean>(keyinLength);
    boldMask.fill(false);
    const indicesArray: number[][] = this.results[resultIndex].matches[0].indices;
