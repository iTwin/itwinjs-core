...
((config: CertaConfig) => {
  ...
  if (mochaOpts.fgrep) mocha.fgrep(mochaOpts.fgrep);
  if (mochaOpts.grep) mocha.grep(mochaOpts.grep);
  if (mochaOpts.invert) mocha.invert();
  if (mochaOpts.forbidOnly) mocha.forbidOnly();

  // Disable timeouts when debugging.
  if (config.debug) mocha.timeout(0);
})(_CERTA_CONFIG);
