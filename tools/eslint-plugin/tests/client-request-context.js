...
function makeTest(strings) {
  ...
  if (codeLines.length <= 1) return prelude + strings;
  const leftPadding = codeLines[1].match(/\s+/)[0];
  return (
    prelude + codeLines.map((l) => l.substr(leftPadding.length)).join("\n")
  );
}
  ...
  supportSkippedAndOnlyInTests({
    ...
    invalid: [
      ...
      },
      {
        ...
        `,
        errors: [{ messageId: "noEnterOnFirstLine" }],
        output: makeTest`
          async function missingFirstEnterCall(reqCtx: ClientRequestContext) {
      ...
      },
      {
        ...
        code: makeTest`
          async function awaitInBlocklessArrowFunc(reqCtx: ClientRequestContext) {
            reqCtx.enter();
            const subAsync = async (reqCtx2: ClientRequestContext) => await Promise.resolve(5);
