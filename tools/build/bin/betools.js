...
yargs.strict(true)
  ...
  .command("test-tsnode", false, {}, () => {
    testCommand();
  })
  .command(
    "docs",
    "Generate TypeDoc documentation by using the provided parameters to pass to TypeDoc.  Supports generating html TypeScript documentation as well as a json representation of the documentation.",
    function(yargs) {
  ...
  .command("extract", "Extract sample code from test files in a specific directory", function(yargs) {
    return yargs.options({
      "extractFrom": {
  ...
  .command(
    "extract-api",
    "Extracts the API of the Typescript library starting from an entry file with a default presets. Powered by @microsoft/api-extractor (https://api-extractor.com)",
    function(yargs) {
      return yargs.options({
        "entry": {
        ...
        "includeUnexportedApis": {
          boolean: true,
          describe:
            "If this flag is set, then APIs that are unexported, but still indirectly accessible via exported APIs, will also be included to the API report. Defaults to `false`",
        },
      });
    },
    (argv) => {
      extractApiCommand(argv);
    },
  )
  .command("pseudolocalize", "Pseudo-localizes an english localization JSON file.", function(yargs) {
    return yargs.options({
      "englishDir": {
    ...
    pseudolocalizeCommand(argv);
  })
  .command("copy-assets", "copy assets from @itwin or @bentley dependencies into a destination directory", function(yargs) {
    return yargs.options({
      "packageJsonDir": {
...
function extractApiCommand(options) {
  ...
  exec("node", [
    getScriptPath("extract-api.js"),
    ...entryOpt,
    ...ignoreTagsOpt,
    ...apiReportFolderOpt,
    ...apiReportTempFolderOpt,
    ...apiSummaryFolderOpt,
    ...includeUnexportedApisOpt,
  ]);
}
