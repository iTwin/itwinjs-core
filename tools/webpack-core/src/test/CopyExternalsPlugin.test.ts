...
describe("CopyExternalsPlugin", () => {
  ...
  before(() => {
    setApplicationDir(
      path.join(__dirname, "assets/copy-externals-plugin-test")
    );
  });
  ...
  it("skips electron", async () => {
    ...
    testConfig = getTestConfig(
      "assets/copy-externals-plugin-test/test.js",
    ...
    expect(fs.existsSync(path.join(__dirname, "dist/node_modules"))).to.be
      .false;
  });
  ...
  it("should copy direct external dependencies", async () => {
    ...
    expect(
      fs.readFileSync(
        path.join(__dirname, "dist/node_modules/foo/index.js"),
        "utf8"
      )
    ).to.equal(`console.log("This is foo");`);
    expect(
      fs.readFileSync(
        path.join(__dirname, "dist/node_modules/a/index.js"),
        "utf8"
      )
    ).to.equal(`console.log("This is a");`);
    expect(
      fs.readFileSync(
        path.join(__dirname, "dist/node_modules/b/index.js"),
        "utf8"
      )
    ).to.equal(`console.log("This is b");`);
    expect(
      fs.readFileSync(
        path.join(__dirname, "dist/node_modules/c/index.js"),
        "utf8"
      )
    ).to.equal(`console.log("This is c");`);
  });
  ...
  it("should copy indirect dependencies with symlink", async () => {
    ...
    fs.symlinkSync(
      "../bar/node_modules/b",
      "lib/test/assets/copy-externals-plugin-test/node_modules/b",
      "junction"
    );
    testConfig = getTestConfig(
      "assets/copy-externals-plugin-test/test.js",
    ...
    const { logging } = await runWebpack(testConfig);
    expect(
      logging?.CopyExternalsPlugin.entries.length,
      "Length of entries should be 3"
    ).to.be.equal(3);
    expect(fs.existsSync(path.join(__dirname, "dist/node_modules/a"))).to.be
      .true;
    expect(fs.existsSync(path.join(__dirname, "dist/node_modules/b"))).to.be
      .true;
    expect(fs.existsSync(path.join(__dirname, "dist/node_modules/c"))).to.be
      .true;
  });
  ...
  it("should copy hoisted indirect dependencies", async () => {
    ...
    await runWebpack(testConfig);
    expect(fs.existsSync(path.join(__dirname, "dist/node_modules/bar"))).to.be
      .true;
    expect(
      fs.readFileSync(
        path.join(__dirname, "dist/node_modules/bar/index.js"),
        "utf8"
      )
    ).to.equal(`console.log("This is bar");`);
  });
  ...
  it("should copy nested indirect dependencies", async () => {
    ...
    ).to.equal(`console.log("This is bar inside foo");`);
  });
  ...
  it("should log when optional dependency is not installed ", async () => {
    ...
    const { logging } = await runWebpack(testConfig);
    expect(fs.existsSync(path.join(__dirname, "dist/node_modules/foo"))).to.be
      .false;
  ...
  it("should warn when required dependency is not installed ", async () => {
    ...
    expect(warnings![0].message).to.match(
      /Can't copy external package "foo" - it is not installed./
    );
  });
