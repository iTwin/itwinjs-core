...
describe("Convert from ECSchema xml file to typescript file", () => {
  it("Success 3.2 Schema File Tests", async () => {
    ...
    await testFileConverterSuccess(new SchemaContext(), schemaFilePath, "ComprehensiveSchema", outDir, [
      ec32ReferenceDir,
    ]);
  ...
  it("Test failure when bad schema path", async () => {
    await testFileConverterFailure(new SchemaContext(), path.join("bad", "path"), utils.getOutDir(), [
      ec32ReferenceDir,
    ]);
  });
