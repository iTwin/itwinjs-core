...
async function testFileConverterFailure(
...
async function testFileConverterSuccess(
  ...
  assert.isTrue(
    fs.existsSync(path.join(outputDir, `${schemaName}Elements.ts`))
  );
  assert.isTrue(
    fs.existsSync(path.join(outputDir, `${schemaName}ElementProps.ts`))
  );
}
...
describe("Convert from ECSchema xml file to typescript string", () => {
  it("Expect failure with bad schema path", async () => {
    ...
    await testFileConverterFailure(
      context,
      badSchemaPath,
      badSchemaOutputPath,
      [ec32ReferenceDir]
...
describe("Convert from ECSchema xml file to typescript file", () => {
  it("Success 3.2 Schema File Tests", async () => {
    ...
    await testFileConverterSuccess(
      new SchemaContext(),
      schemaFilePath,
      "BasicTest",
      outDir,
    ...
    schemaFilePath = path.join(
      assetDir,
      "schema3.2",
      "ComprehensiveSchema.01.00.00.ecschema.xml"
    );
  ...
  it("Test failure when empty schema path", async () => {
    await testFileConverterFailure(new SchemaContext(), "", utils.getOutDir(), [
      ec32ReferenceDir,
    ]);
  });
  ...
  it("Failure with non-existent out directory", async () => {
    const schemaFilePath = path.join(
      assetDir,
      "BasicTest.01.00.00.ecschema.xml"
    );
    await testFileConverterFailure(
      new SchemaContext(),
      schemaFilePath,
