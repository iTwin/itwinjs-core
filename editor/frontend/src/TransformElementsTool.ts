...
export class TransformGraphicsProvider {
  ...
  private async createRequest(id: Id64String): Promise<TransformGraphicsData | undefined> {
    ...
    if (0 === elementProps.length) return;

    const placementProps = elementProps[0].placement;
    if (undefined === placementProps) return;

    const placement = isPlacement2dProps(placementProps)
      ? Placement2d.fromJSON(placementProps)
      : Placement3d.fromJSON(placementProps);
    if (!placement.isValid) return; // Ignore assembly parents w/o geometry, etc...
  ...
  private async cancelPendingRequests(): Promise<void> {
    ...
    for (const [_key, id] of this.pending) requests.push(id);
  ...
  public async createSingleGraphic(id: Id64String): Promise<boolean> {
    try {
      ...
      if (undefined !== info?.id) this.pending.delete(info.id);

      if (undefined === info?.graphic) return false;
  ...
  public createGraphics(elements: Id64Arg): void {
    if (0 === Id64.sizeOf(elements)) return;
    ...
    try {
      for (const id of Id64.iterable(elements)) {
        ...
        promise.then((info) => {
          ...
          if (undefined !== info?.graphic) this.data.push(info);
        });
      }
    } catch {}
  }
  ...
  public addGraphics(transform: Transform, context: DynamicsContext): void {
    ...
    const branch = new GraphicBranch(false);
    for (const data of this.data) branch.add(data.graphic);
...
export abstract class TransformElementsTool extends ElementSetTool {
  ...
  protected async createAgendaGraphics(changed: boolean): Promise<void> {
    if (changed) {
      if (undefined === this._graphicsProvider) return; // Not yet needed...
    } else {
      if (undefined !== this._graphicsProvider) return; // Use existing graphics...
    }

    if (undefined === this._graphicsProvider)
      this._graphicsProvider = new TransformGraphicsProvider(this.iModel, this.toolId);
    else await this._graphicsProvider.cleanupGraphics();
  ...
  protected updateAnchorLocation(transform: Transform): void {
    ...
    if (undefined === this.anchorPoint) return;
  ...
  protected async replaceAgenda(newIds: Id64Arg | undefined): Promise<void> {
    ...
    if (undefined !== newIds) this.agenda.add(newIds);

    if (this.isSelectionSetModify) {
      if (this.agenda.isEmpty) this.iModel.selectionSet.emptyAll();
      else this.iModel.selectionSet.replace(this.agenda.elements);
  ...
  protected async transformAgenda(transform: Transform): Promise<void> {
    try {
      ...
      if (
        ...
        (await basicManipulationIpc.transformPlacement(this.agenda.compressIds(), transform.toJSON()))
      )
  ...
  public override async processAgenda(ev: BeButtonEvent): Promise<void> {
    ...
    if (this.wantMakeCopy) await this.replaceAgenda(await this.transformAndCopyAgenda(transform));
    else await this.transformAgenda(transform);

    this.updateAnchorLocation(transform);
  }

  public override async onProcessComplete(): Promise<void> {
    if (this.wantRepeatOperation) return; // Continue with current agenda instead of restarting (ex. create additional copies)
...
export class MoveElementsTool extends TransformElementsTool {
  ...
  protected calculateTransform(ev: BeButtonEvent): Transform | undefined {
    if (undefined === this.anchorPoint) return undefined;
  ...
  protected override provideToolAssistance(
    ...
    if (!this.isSelectByPoints && !this.wantAdditionalElements)
      mainMsg = CoreTools.translate(
        this.wantAdditionalInput ? "ElementSet.Prompts.StartPoint" : "ElementSet.Prompts.EndPoint"
      );
...
export class CopyElementsTool extends MoveElementsTool {
    ...
    if (!this._numCopiesProperty)
      this._numCopiesProperty = new DialogProperty<number>(
        PropertyDescriptionHelper.buildNumberEditorDescription(
          "numCopies",
          EditTools.translate("CopyElements.Label.NumCopies"),
          {
  ...
  public get numCopies(): number {
    return this.numCopiesProperty.value;
  }
  public set numCopies(value: number) {
    this.numCopiesProperty.value = value;
  }

  protected override get wantMakeCopy(): boolean {
  ...
  protected override updateAnchorLocation(transform: Transform): void {
    ...
    for (let iCopy = 0; iCopy < this.numCopies; ++iCopy) super.updateAnchorLocation(transform);
  }

  protected async doTransformedCopy(
    ids: Id64Array,
    transform: Transform,
    numCopies: number
  ): Promise<Id64Arg | undefined> {
    if (numCopies < 1 || 0 === ids.length) return undefined;
    ...
    for (const id of ids) {
      ...
      if (undefined === props.placement) continue;
      ...
      if (!placement.isValid) continue; // Ignore assembly parents w/o geometry, etc...
      ...
      if (undefined !== newId) newIds.push(newId); // When numCopies > 1 ids are return for just the final copy...
    }
  ...
  protected override async transformAndCopyAgenda(transform: Transform): Promise<Id64Arg | undefined> {
    try {
      ...
      if (undefined !== newIds) await this.saveChanges();
      return newIds;
    } catch (err) {
      IModelApp.notifications.outputMessage(
        new NotifyMessageDetails(
  ...
  public override async parseAndRun(...inputArgs: string[]): Promise<boolean> {
    for (const arg of inputArgs) {
      ...
      if (parts[0].toLowerCase().startsWith("num")) {
        ...
        if (copies >= 1) this.numCopies = copies; // NOTE: Don't call saveToolSettingPropertyValue, always default to single copy...
      }
    }
...
export class RotateElementsTool extends TransformElementsTool {
  ...
  private static methodMessage(str: string) {
    return EditTools.translate(`RotateElements.Method.${str}`);
  }
  private static getMethodChoices = (): EnumerationChoice[] => {
    return [
      {
        label: RotateElementsTool.methodMessage("3Points"),
        value: RotateMethod.By3Points,
      },
      {
        label: RotateElementsTool.methodMessage("Angle"),
        value: RotateMethod.ByAngle,
      },
    ];
  };
    ...
    if (!this._methodProperty)
      this._methodProperty = new DialogProperty<number>(
        PropertyDescriptionHelper.buildEnumPicklistEditorDescription(
          "rotateMethod",
          EditTools.translate("RotateElements.Label.Method"),
          RotateElementsTool.getMethodChoices()
        ),
        RotateMethod.By3Points as number
      );
    return this._methodProperty;
  }

  public get rotateMethod(): RotateMethod {
    return this.methodProperty.value as RotateMethod;
  }
  public set rotateMethod(method: RotateMethod) {
    this.methodProperty.value = method;
  }

  private static aboutMessage(str: string) {
    return EditTools.translate(`RotateElements.About.${str}`);
  }
  private static getAboutChoices = (): EnumerationChoice[] => {
    return [
      {
        label: RotateElementsTool.aboutMessage("Point"),
        value: RotateAbout.Point,
      },
      {
        label: RotateElementsTool.aboutMessage("Origin"),
        value: RotateAbout.Origin,
      },
      {
        label: RotateElementsTool.aboutMessage("Center"),
        value: RotateAbout.Center,
      },
    ];
  };
  ...
  public get aboutProperty() {
    if (!this._aboutProperty)
      this._aboutProperty = new DialogProperty<number>(
        PropertyDescriptionHelper.buildEnumPicklistEditorDescription(
          "rotateAbout",
          EditTools.translate("RotateElements.Label.About"),
          RotateElementsTool.getAboutChoices()
        ),
        RotateAbout.Point as number
      );
    return this._aboutProperty;
  }

  public get rotateAbout(): RotateAbout {
    return this.aboutProperty.value as RotateAbout;
  }
  public set rotateAbout(method: RotateAbout) {
    this.aboutProperty.value = method;
  }
  ...
  public get angleProperty() {
    if (!this._angleProperty)
      this._angleProperty = new DialogProperty<number>(
        new AngleDescription("rotateAngle", EditTools.translate("RotateElements.Label.Angle")),
        0.0
      );
  ...
  public get rotateAngle(): number {
    return this.angleProperty.value;
  }
  public set rotateAngle(value: number) {
    this.angleProperty.value = value;
  }

  protected override get requireAcceptForSelectionSetDynamics(): boolean {
    return RotateMethod.ByAngle !== this.rotateMethod;
  }
  ...
  protected calculateTransform(ev: BeButtonEvent): Transform | undefined {
    ...
    if (RotateMethod.ByAngle === this.rotateMethod) {
      ...
      if (undefined === rotMatrix) return undefined;

      const invMatrix = rotMatrix.inverse();
      if (undefined === invMatrix) return undefined;

      const angMatrix = YawPitchRollAngles.createRadians(this.rotateAngle, 0, 0).toMatrix3d();
      if (undefined === angMatrix) return undefined;
    ...
    if (undefined === this.anchorPoint || undefined === this.xAxisPoint) return undefined;
    ...
    if (!vec1.normalizeInPlace() || !vec2.normalizeInPlace()) return undefined;

    const dot = vec1.dotProduct(vec2);
    if (dot > 1.0 - Geometry.smallAngleRadians) return undefined;
    ...
    if (undefined === zVec) return undefined;

    const yVec = zVec.unitCrossProduct(vec1);
    if (undefined === yVec) return undefined;
  ...
  protected override transformAgendaDynamics(transform: Transform, context: DynamicsContext): void {
    if (RotateAbout.Point === this.rotateAbout) return super.transformAgendaDynamics(transform, context);
    ...
    for (const data of this._graphicsProvider.data) {
      if (RotateAbout.Origin === this.rotateAbout) rotatePoint.setFrom(data.placement.origin);
      else rotatePoint.setFrom(data.placement.calculateRange().center);
  ...
  protected override async transformAgenda(transform: Transform): Promise<void> {
    if (RotateAbout.Point === this.rotateAbout) return super.transformAgenda(transform);

    try {
      this._startedCmd = await this.startCommand();
      if (
        IModelStatus.Success ===
        (await basicManipulationIpc.rotatePlacement(
          this.agenda.compressIds(),
          transform.matrix.toJSON(),
          RotateAbout.Center === this.rotateAbout
        ))
      )
        await this.saveChanges();
    } catch (err) {
  ...
  public override onDynamicFrame(ev: BeButtonEvent, context: DynamicsContext): void {
    const transform = this.calculateTransform(ev);
    if (undefined !== transform) return this.transformAgendaDynamics(transform, context);
  ...
  protected override get wantAdditionalInput(): boolean {
    if (RotateMethod.ByAngle === this.rotateMethod) return super.wantAdditionalInput;

    return !this.haveFinalPoint;
  }

  protected override wantProcessAgenda(ev: BeButtonEvent): boolean {
    if (RotateMethod.ByAngle === this.rotateMethod) return super.wantProcessAgenda(ev);

    if (!this.havePivotPoint) this.havePivotPoint = true; // Uses anchorPoint...
    else if (undefined === this.xAxisPoint) this.xAxisPoint = ev.point.clone();
    else if (!this.haveFinalPoint) this.haveFinalPoint = true; // Uses button event...
  ...
  protected override setupAndPromptForNextAction(): void {
    ...
    if (RotateMethod.ByAngle === this.rotateMethod) return;

    if (undefined === this.anchorPoint || undefined === this.xAxisPoint) return;
  ...
  protected override provideToolAssistance(
    ...
    if (RotateMethod.ByAngle === this.rotateMethod) {
      ...
      if (!this.isSelectByPoints && !this.wantAdditionalElements) {
        if (!this.havePivotPoint) mainMsg = EditTools.translate("RotateElements.Prompts.IdentifyPoint");
        else if (undefined === this.xAxisPoint) mainMsg = EditTools.translate("RotateElements.Prompts.DefineStart");
        else mainMsg = EditTools.translate("RotateElements.Prompts.DefineAmount");
      }
    }
    super.provideToolAssistance(mainMsg);
  }
  ...
  public override async applyToolSettingPropertyChange(updatedValue: DialogPropertySyncItem): Promise<boolean> {
    ...
    if (this.methodProperty.name === updatedValue.propertyName) await this.onRestartTool(); // calling restart, not reinitialize to not exit tool for selection set...
  ...
  public override async parseAndRun(...inputArgs: string[]): Promise<boolean> {
    ...
    if (undefined !== rotateAbout)
      this.saveToolSettingPropertyValue(this.aboutProperty, {
        value: rotateAbout,
      });

    if (undefined !== rotateAngle)
      this.saveToolSettingPropertyValue(this.angleProperty, {
        value: rotateAngle,
      });
