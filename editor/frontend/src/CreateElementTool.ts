...
export class DynamicGraphicsProvider {
  ...
  private async createRequest(
    ...
    const graphic = await readElementGraphics(
      graphicData,
      this.iModel,
      this.modelId ? this.modelId : Id64.invalid,
      is3d,
  ...
  public createGraphicAndUpdateDynamics(
    ...
    promise
      .then(() => {
        if (promise !== this._graphicPromise) return; // abandoned this request...
...
export abstract class CreateElementTool extends PrimitiveTool {
  ...
  public override get targetModelId(): Id64String {
    ...
    if (undefined === model) throw new IModelError(IModelStatus.BadModel, "");
  ...
  protected async processDataButton(ev: BeButtonEvent): Promise<EventHandled> {
    ...
    if (!this.isDynamicsStarted && this.wantDynamics) this.beginDynamics();
...
export abstract class CreateElementWithDynamicsTool extends CreateElementTool {
  ...
  protected async createGraphics(ev: BeButtonEvent): Promise<void> {
    if (!(await this.updateDynamicData(ev))) return;

    const placement = this.getPlacementProps();
    if (undefined === placement) return;
