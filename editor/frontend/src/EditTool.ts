...
import { EditorAppEnum } from "@bentley/imodeljs-editor-common";
...
export class EditTool extends PrimitiveTool {
  ...
  public static async startCommand<T>(commandId: string, iModelKey: string, ...args: any[]): Promise<T> {
    return FrontendIpc.callBackend(EditorAppEnum.Channel, "startCommand", commandId, iModelKey, ...args) as Promise<T>;
  }

  public static async callCommand(methodName: string, ...args: any[]): Promise<any> {
    return FrontendIpc.callBackend(EditorAppEnum.Channel, "callMethod", methodName, ...args);
  }
}
