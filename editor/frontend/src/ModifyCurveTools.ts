...
export abstract class ModifyCurveTool extends ModifyElementWithDynamicsTool {
  ...
  public static isInPlane(
    ...
    for (const child of curve.children) {
      if (child instanceof CurvePrimitive) {
        if (!child.isInPlane(plane)) return false;
      } else if (child instanceof CurveCollection) {
        if (!this.isInPlane(child, plane)) return false;
      }
    }
  ...
  protected async getCurveData(id: Id64String): Promise<CurveData | undefined> {
    try {
      ...
      const reject: ElementGeometryOpcode[] = [
        ElementGeometryOpcode.Polyface,
        ElementGeometryOpcode.SolidPrimitive,
        ElementGeometryOpcode.BsplineSurface,
        ElementGeometryOpcode.BRep,
      ];
      const info = await basicManipulationIpc.requestElementGeometry(id, {
        maxDisplayable: 1,
        reject,
        geometry: { curves: true, surfaces: true, solids: false },
      });
  ...
  protected getGeometryProps(
    ...
    if (!builder.appendGeometryParamsChange(this.curveData.params)) return;

    if (!builder.appendGeometryQuery(geom)) return;
  ...
  protected getElementProps(
    ...
    if (!this.wantModifyOriginal) {
      ...
      return {
        ...
        category: this.curveData.props.category,
        code: Code.createEmpty(),
        placement: this.curveData.props.placement,
      };
    }
...
export class OffsetCurveTool extends ModifyCurveTool {
    ...
    if (!this._makeCopyProperty)
      this._makeCopyProperty = new DialogProperty<boolean>(
        PropertyDescriptionHelper.buildToggleDescription(
          "offsetCopy",
          EditTools.translate("OffsetCurve.Label.MakeCopy")
        ),
        false
      );
  ...
  public get makeCopy(): boolean {
    return this.makeCopyProperty.value;
  }
  public set makeCopy(value: boolean) {
  ...
  protected override modifyCurve(
    ...
    const spacePoint = AccuDrawHintBuilder.projectPointToPlaneInView(
      ...
      localToWorld.matrix.getColumn(2),
      ev.viewport
    );
    ...
    if (refDir.dotProduct(unitY) < 0.0) distance = -distance;
    ...
    if (Math.abs(offset) < Geometry.smallMetricDistance) return undefined;
  ...
  protected override setupAccuDraw(): void {
    ...
    if (this.agenda.isEmpty) {
      ...
      const closeDetail =
        geom instanceof CurvePrimitive
          ? geom.closestPoint(this.anchorPoint, false)
...
export class BreakCurveTool extends ModifyCurveTool {
  ...
  protected doBreakCurve(ev: BeButtonEvent): void {
    ...
    if (geom instanceof CurvePrimitive) {
      if (selectedStart || selectedEnd) return; // split is no-op...
      ...
      for (const curve of geom.children) {
        if (curve === closeDetail.curve) {
          if (selectedStart) {
            ...
            if (undefined !== curveB) resultB.children.push(curveB);
          }
          beforeCurve = false;
        } else {
          if (beforeCurve) resultA.children.push(curve.clone());
          else resultB.children.push(curve.clone());
        }
      }
      ...
      for (let index = closeIndex; index < endIndex; ++index) {
        ...
        if (undefined === curve || curve === closeDetail.curve) continue;
...
export class ExtendCurveTool extends ModifyCurveTool {
  ...
  protected extendPathEnd(
    ...
    const curve = closeDetail.curve.clonePartialCurve(
      isStart ? closeDetail.fraction : 0.0,
      isStart ? 1.0 : closeDetail.fraction
    );
    if (undefined === curve) return undefined;

    if (
      curve instanceof Arc3d &&
      closeDetail.curve instanceof Arc3d &&
      curve.sweep.isCCW !== closeDetail.curve.sweep.isCCW
    )
      ...
      if (curve instanceof Arc3d) {
        ...
        hints.setXAxis(
          Vector3d.createStartEnd(
            pickDetail.fraction > 0.5 ? curve.point0Ref : curve.point1Ref,
            pickDetail.fraction > 0.5 ? curve.point1Ref : curve.point0Ref
          )
        );
...
export class RegionBooleanTool extends ModifyCurveTool {
  ...
  public get makeCopyProperty() {
    if (!this._makeCopyProperty)
      this._makeCopyProperty = new DialogProperty<boolean>(
        PropertyDescriptionHelper.buildToggleDescription(
          "regionBooleanKeep",
          EditTools.translate("RegionBoolean.Label.KeepOriginal")
        ),
        false
      );
    return this._makeCopyProperty;
  ...
  private static modeMessage(str: string) {
    return EditTools.translate(`RegionBoolean.Mode.${str}`);
  }
  private static getModeChoices = (): EnumerationChoice[] => {
    return [
      {
        label: RegionBooleanTool.modeMessage("Unite"),
        value: RegionBooleanMode.Unite,
      },
      {
        label: RegionBooleanTool.modeMessage("Subtract"),
        value: RegionBooleanMode.Subtract,
      },
      {
        label: RegionBooleanTool.modeMessage("Intersect"),
        value: RegionBooleanMode.Intersect,
      },
    ];
  };
  ...
  public get modeProperty() {
    if (!this._modeProperty)
      this._modeProperty = new DialogProperty<number>(
        PropertyDescriptionHelper.buildEnumPicklistEditorDescription(
          "regionBooleanMode",
          EditTools.translate("RegionBoolean.Label.Mode"),
          RegionBooleanTool.getModeChoices()
        ),
        RegionBooleanMode.Unite as number
      );
    return this._modeProperty;
  }

  public get mode(): RegionBooleanMode {
    return this.modeProperty.value as RegionBooleanMode;
  }
  public set mode(mode: RegionBooleanMode) {
    this.modeProperty.value = mode;
  }

  protected override get clearSelectionSet(): boolean {
    return false;
  } // Don't clear for subtract so that mode can be changed...
  protected override get allowSelectionSet(): boolean {
    return RegionBooleanMode.Subtract !== this.mode;
  }
  ...
  private regionBooleanXY(
    ...
    if (undefined === areas) return undefined;
    ...
    if (1 === loops.length) return this.regionFromSignedLoops(loops[0]);
    ...
    if (loops.length > 1) {
      ...
      if (unionRegion.children.length > 1) return unionRegion;
    }
  ...
  protected async doRegionBoolean(_ev: BeButtonEvent): Promise<void> {
    ...
    if (this.agenda.length < this.requiredElementCount) return;
    ...
    if (undefined === targetWorldToLocal) return;

    const targetPlane = Plane3dByOriginAndUnitNormal.create(
      targetLocalToWorld.getOrigin(),
      targetLocalToWorld.matrix.getColumn(2)
    );
    if (undefined === targetPlane) return;

    if (!targetData.geom.tryTransformInPlace(targetWorldToLocal)) return;

    const tools: AnyRegion[] = [targetData.geom as AnyRegion];

    for (const id of this.agenda.elements) {
      if (id === targetData.props.id) continue;

      const curveData = await this.getCurveData(id);
      if (undefined === curveData?.geom) return;
      ...
      if (!curveData.geom.tryTransformInPlace(targetWorldToLocal)) return;
