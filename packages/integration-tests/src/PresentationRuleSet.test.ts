...
describe("PresentationRuleSet", async () => {
  ...
  it("creates ruleset from Json and gets rootNode from frontend", async () => {
    ...
    const rootNodes = await ECPresentation.presentation.getRootNodes(imodel.iModelToken, undefined, { RulesetId: presentationRuleSet.ruleSetId });
  ...
  it("removes ruleset from frontend", async () => {
    await ECPresentation.presentation.addRuleSet(presentationRuleSet);
    let rootNodes = await ECPresentation.presentation.getRootNodes(imodel.iModelToken, undefined, { RulesetId: presentationRuleSet.ruleSetId });
    expect(rootNodes.length).to.be.equal(1);

    await ECPresentation.presentation.removeRuleSet(presentationRuleSet.ruleSetId);
    rootNodes = await ECPresentation.presentation.getRootNodes(imodel.iModelToken, undefined, { RulesetId: presentationRuleSet.ruleSetId });
