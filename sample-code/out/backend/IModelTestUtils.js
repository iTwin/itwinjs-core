...
imodeljs_common_1.Gateway.initialize(imodeljs_common_1.IModelReadGateway);
...
class IModelTestUtils {
    ...
    static startupIModelHost() {
        ...
        if (briefcaseCacheDir === undefined) {
            const tempDir = process.env.MY_SERVICE_TMP_DIR || imodeljs_backend_1.KnownLocations.tmpdir;
            briefcaseCacheDir = path.join(tempDir, "iModelJs_cache");
        }
        const imHostConfig = new imodeljs_backend_1.IModelHostConfiguration();
        imHostConfig.briefcaseCacheDir = briefcaseCacheDir;
        // Start up IModelHost, supplying the configuration.
        imodeljs_backend_1.IModelHost.startup(imHostConfig);
    }
}
