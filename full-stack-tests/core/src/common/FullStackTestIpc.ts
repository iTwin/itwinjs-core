...
export interface FullStackTestIpc {
  ...
  throwLegacyError(errorNumber: number, message: string, metaData: LoggingMetaData, logFn: boolean): Promise<void>;
  throwChannelError(errorKey: ChannelError.Key, message: string, channelKey: string, metadata: LoggingMetaData, logFn: boolean): Promise<void>;
}
