...
function areAllChildTilesLoaded(parent?: Tile): boolean {
  ...
  else if (TileTreeLoadStatus.Loading === (parent as any)._childrenLoadStatus) return false;
...
export async function createOnScreenTestViewport(
  ...
  if (undefined !== devicePixelRatio) {
    ...
    if (undefined !== debugControl) debugControl.devicePixelRatioOverride = devicePixelRatio;
  }
...
export async function testViewportsWithDpr(
  ...
  for (const dpr of devicePixelRatios) await testViewports("0x24", imodel, rect.width, rect.height, test, dpr);
}
