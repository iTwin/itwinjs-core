...
describe("ECSql Query", () => {
  ...
  skipIfWeb("Restart query frontend", async () => {
    ...
    const cb = async () => {
      return new Promise<void>(async (resolve, reject) => {
        try {
          for await (const _row of imodel1.createQueryReader("SELECT * FROM BisCore.element", undefined, {
            restartToken: "tag",
          })) {
            rowCount++;
          }
  ...
  it("Paging Results", async () => {
    ...
    for (const db of dbs) {
      ...
      if (await reader.step()) pendingRowCount.push(reader.current[0] as number);
    }
  ...
  it("Query with Abbreviated Blobs", async function () {
    ...
    for await (const row of imodel2.createQueryReader(query2, QueryBinder.from([row1.id]), {
      rowFormat: QueryRowFormat.UseJsPropertyNames,
