...
describe("Section-cut tile tree", () => {
  ...
  before(async () => {
    ...
    const imodels = await Promise.all([
      SnapshotConnection.openFile("mirukuru.ibim"),
      SnapshotConnection.openFile("planprojection.bim"),
    ]);
  ...
  async function test(
    setup: (view: SpatialViewState) => void,
  ...
  const defaultClip = ClipVector.createCapture([
  ...
  it("creates an additional tile tree reference for section-cut graphics", async () => {
    ...
    await test(
      (view) => enableClip(view, false, defaultClip),
      (view) => {
        expectNumTreesPerModel(1, view);
      }
    );
    ...
    await test(
      (view) => enableClip(view, true, defaultClip),
      (view) => {
  ...
  it("applies current section cut to newly-added tile tree references", async () => {
    ...
    await test(
      (view) => {
        ...
        for (const model of view.modelSelector.models) modelIds.push(model);
  ...
  it("does not apply section cut for empty clip vector", async () => {
    await test(
      (view) => enableClip(view, true, ClipVector.createEmpty()),
      (view) => {
        expectNumTreesPerModel(1, view);
      }
    );
