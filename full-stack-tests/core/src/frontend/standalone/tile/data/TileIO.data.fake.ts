...
function changeHeaderLengthInPlace(bytes: Uint8Array, data: TileTestData, numPaddingBytes: number): void {
  ...
  const tileLength =
    ((bytes[80] | (bytes[81] << 8) | (bytes[82] << 0x10) | (bytes[83] << 0x18)) >>> 0) + numPaddingBytes;
...
function padTestCaseHeader(
  ...
  for (let i = 0; i < numPaddingBytes; i++) bytes[data.headerLength + i] = Math.floor(Math.random() * 0xff);

  for (let i = data.headerLength; i < src.bytes.length; i++) bytes[i + numPaddingBytes] = src.bytes[i];
