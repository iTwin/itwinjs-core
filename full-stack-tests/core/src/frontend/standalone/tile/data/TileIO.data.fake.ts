...
function changeTestCaseVersion(
  src: TileTestCase,
  majorVersion?: number,
  minorVersion?: number
): TileTestCase {
...
function changeVersion(
  ...
  let dst: TileTestData = {
    ...
    lineString: changeTestCaseVersion(
      src.lineString,
      versionMajor,
      versionMinor
    ),
    lineStrings: changeTestCaseVersion(
      src.lineStrings,
      versionMajor,
      versionMinor
    ),
  ...
  dst.unreadable =
    dst.versionMajor > CurrentImdlVersion.Major ? true : undefined;

  if (
    undefined !== versionMajor &&
    versionMajor > 1 &&
    1 === src.versionMajor
  ) {
...
export function changeMinorVersion(
  src: TileTestData,
  versionMinor: number
): TileTestData {
...
export function changeMajorVersion(
  src: TileTestData,
  versionMajor: number
): TileTestData {
...
function changeHeaderLengthInPlace(
  bytes: Uint8Array,
  data: TileTestData,
  numPaddingBytes: number
): void {
...
function padTestCaseHeader(
  ...
  for (let i = 0; i < data.headerLength; i++) bytes[i] = src.bytes[i];
...
export function changeHeaderLength(
  src: TileTestData,
  versionMinor: number,
  numPaddingBytes: number
): TileTestData {
  return {
    ...
    rectangle: padTestCaseHeader(
      src,
      "rectangle",
      versionMinor,
      numPaddingBytes
    ),
    triangles: padTestCaseHeader(
      src,
      "triangles",
      versionMinor,
      numPaddingBytes
    ),
    lineString: padTestCaseHeader(
      src,
      "lineString",
      versionMinor,
      numPaddingBytes
    ),
    lineStrings: padTestCaseHeader(
      src,
      "lineStrings",
      versionMinor,
      numPaddingBytes
