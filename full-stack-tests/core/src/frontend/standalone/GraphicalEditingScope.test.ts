...
describe("GraphicalEditingScope", () => {
  if (!ProcessDetector.isMobileAppFrontend) {
    ...
    it("throws if enter is called repeatedly", async () => {
      ...
      const scope = await imodel.enterEditingScope();
      await expect(imodel.enterEditingScope()).to.be.rejectedWith(
    ...
    it("throws if exit is called repeatedly", async () => {
      ...
      const scope = await imodel.enterEditingScope();
      await scope.exit();
      await expect(scope.exit()).to.be.rejectedWith(
    ...
    it("updates state of tile trees", async () => {
      ...
      const elem1 = await insertLineElement(
        ...
        modelId,
        category,
        makeLineSegment(new Point3d(0, 0, 0), new Point3d(10, 0, 0))
      );
      await imodel.saveChanges();
      ...
      const elem2 = await insertLineElement(
        imodel,
        modelId,
        category,
        makeLineSegment(new Point3d(0, 0, 0), new Point3d(-10, 0, 0))
