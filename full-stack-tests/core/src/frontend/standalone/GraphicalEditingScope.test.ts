...
describe("GraphicalEditingScope", () => {
  if (!ProcessDetector.isMobileAppFrontend) {
    ...
    const oldFilePath = path.join(process.env.IMODELJS_CORE_DIRNAME!, "core/backend-tests/lib/cjs/test/assets/test.bim");
    // Editable; BisCore version == 1.0.11
    const newFilePath = path.join(process.env.IMODELJS_CORE_DIRNAME!, "core/backend-tests/lib/cjs/test/assets/planprojection.bim");
