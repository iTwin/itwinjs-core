...
describe("Elements", () => {
  ...
  it("loads element properties by identifier with options", async () => {
    for (let i = 0; i < 3; i++) {
      ...
      byCode = (await imodel.elements.loadProps(code, {
        wantGeometry: false,
      })) as GeometryPartProps;
      expect(byCode).to.deep.equal(byId);

      byId = (await imodel.elements.loadProps(id, {
        wantGeometry: true,
      })) as GeometryPartProps;
  ...
  it("queries placements", async () => {
    const ids2d = await imodel.elements.queryIds({
      from: "bis.GeometricElement2d",
    });
    expect(ids2d.size).to.equal(18);
    const ids3d = await imodel.elements.queryIds({
  ...
  it("queries individual placements", async () => {
    async function test(dim: "2d" | "3d"): Promise<void> {
      ...
      if ("2d" === dim) expect(placements[0]).instanceof(Placement2d);
      else expect(placements[0]).instanceof(Placement3d);
    }
