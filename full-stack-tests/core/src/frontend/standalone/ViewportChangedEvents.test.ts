...
describe("Viewport changed events", async () => {
  ...
  it("should be dispatched when display style is modified directly", async () => {
    ...
    ViewportChangedHandler.test(vp, (mon) => {
      ...
      mon.expect(ChangeFlag.DisplayStyle, ViewportState.AnalysisFraction | ViewportState.RenderPlan, () => settings.analysisStyle = AnalysisStyle.fromJSON({ inputName: "source" }));
