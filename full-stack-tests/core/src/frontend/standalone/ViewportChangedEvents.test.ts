...
describe("Viewport changed events", async () => {
  ...
  it("should be dispatched when display style is modified using Viewport APIs", async () => {
    ...
    ViewportChangedHandler.test(vp, (mon) => {
      ...
      vp.saveViewUndo();
      mon.expect(
        ChangeFlag.DisplayStyle | ChangeFlag.ViewedModels | ChangeFlag.FeatureOverrideProvider,
        ViewportState.RenderPlan,
        () => {
  ...
  it("should be dispatched when display style is modified directly", async () => {
    ...
    ViewportChangedHandler.test(vp, (mon) => {
      ...
      expectOverrideChange(() =>
        settings.overrideSubCategory("0x123", SubCategoryOverride.fromJSON({ color: ColorDef.blue.tbgr }))
      );
      ...
      expectChange(
        () => (settings.ambientOcclusionSettings = AmbientOcclusion.Settings.fromJSON({ bias: 42, maxDistance: 24 }))
      );
      expectChange(
        () =>
      ...
      expectChange(() =>
        settings.setPlanProjectionSettings("0xfedcba", PlanProjectionSettings.fromJSON({ elevation: 42 }))
      );
    });
  });
  ...
  it("should be dispatched when category selector is modified using Viewport APIs", async () => {
    ...
    await ViewportChangedHandler.testAsync(vp, async (mon) => {
      ...
      mon.expect(ChangeFlag.ViewState | ChangeFlag.DisplayStyle, ViewportState.Controller, () =>
  ...
  it("should be dispatched when per-model category visibility changes", async () => {
    ...
    ViewportChangedHandler.test(vp, (mon) => {
      ...
      mon.expect(ChangeFlag.None, undefined, () =>
        vis.setOverride("0x1c", "0x1234", PerModelCategoryVisibility.Override.None)
      );
      expect(vis.getOverride("0x1c", "0x1234")).to.equal(PerModelCategoryVisibility.Override.None);

      mon.expect(ChangeFlag.ViewedCategoriesPerModel, undefined, () =>
        vis.setOverride("0x1c", "0x1234", PerModelCategoryVisibility.Override.Show)
      );
      expect(vis.getOverride("0x1c", "0x1234")).to.equal(PerModelCategoryVisibility.Override.Show);

      mon.expect(ChangeFlag.ViewedCategoriesPerModel, undefined, () =>
        vis.setOverride("0x1c", "0x1234", PerModelCategoryVisibility.Override.Hide)
      );
      ...
      idSet.add("0x1234567");
      mon.expect(ChangeFlag.None, undefined, () =>
        vis.setOverride(new Set<string>(), new Set<string>(), PerModelCategoryVisibility.Override.Show)
      );
      mon.expect(ChangeFlag.None, undefined, () =>
        vis.setOverride(idSet, new Set<string>(), PerModelCategoryVisibility.Override.Show)
      );
      mon.expect(ChangeFlag.None, undefined, () =>
        vis.setOverride(new Set<string>(), idSet, PerModelCategoryVisibility.Override.Show)
      );
  ...
  it("should be dispatched when changing ViewState", async () => {
    ...
    ViewportChangedHandler.test(vp, (mon) => {
      ...
      mon.expect(ChangeFlag.ViewState | ChangeFlag.ViewedCategories, ViewportState.Controller, () =>
  ...
  it("should be dispatched when modifying ViewState directly", async () => {
    ...
    ViewportChangedHandler.test(vp, (mon) => {
      ...
      expectChange(
        ViewportState.Scene,
        () => (view.details.modelClipGroups = new ModelClipGroups([ModelClipGroup.fromJSON({ models: ["0x123"] })]))
      );
      expectChange(
        ViewportState.Scene,
        () =>
  ...
  it("should load subcategories for all displayed categories", async () => {
    ...
    const waitForSubCats = async (catIds: Id64Arg): Promise<void> => {
      for (const catId of Id64.iterable(catIds)) expect(subcats.getSubCategories(catId)).to.be.undefined;
      ...
      for (const catId of Id64.iterable(catIds)) expect(subcats.getSubCategories(catId)).not.to.be.undefined;
    };
