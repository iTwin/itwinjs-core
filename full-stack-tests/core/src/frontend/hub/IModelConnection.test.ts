...
async function executeQuery(iModel: IModelConnection, ecsql: string, bindings?: any[] | object): Promise<any[]> {
  const rows: any[] = [];
  for await (const row of iModel.createQueryReader(ecsql, QueryBinder.from(bindings), {
...
describe("IModelConnection (#integration)", () => {
  ...
  it("should be able to open the same IModel many times", async () => {
    ...
    let n = 0;
    while (++n < 25) {
      const promise = CheckpointConnection.openRemote(iTwinId, iModelId).then((readOnlyTest2: IModelConnection) => {
