...
describe("Schedule script (#integration)", () => {
  ...
  it("excludes element Ids if specified", async () => {
    ...
    const styleHasNonEmptyElementIds = (styleProps: DisplayStyleProps) => {
      ...
      const script = styleProps.jsonProperties!.styles!.scheduleScript!; // eslint-disable-line @typescript-eslint/no-deprecated
  ...
  it("creates an additional tile tree per animation transform node", async () => {
    ...
    view.displayStyle.settings.scheduleScriptProps = undefined; // eslint-disable-line @typescript-eslint/no-deprecated
    ...
    view.displayStyle.settings.scheduleScriptProps = json; // eslint-disable-line @typescript-eslint/no-deprecated
    expect(view.displayStyle.scheduleScript).not.to.be.undefined;
  ...
  it("updates tile tree references when script changes", async () => {
    ...
    view.displayStyle.settings.scheduleScriptProps = script.toJSON(); // eslint-disable-line @typescript-eslint/no-deprecated
    expect(view.displayStyle.scheduleScript).not.to.be.undefined;
    expect(countTileTrees(view)).to.equal(2);

    const style = view.displayStyle.clone();
    style.settings.scheduleScriptProps = undefined; // eslint-disable-line @typescript-eslint/no-deprecated
