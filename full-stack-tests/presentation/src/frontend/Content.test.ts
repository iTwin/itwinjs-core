...
describe("Content", () => {
  ...
  const closeIModel = async () => {
    if (imodel && imodel.isOpen) await imodel.close();
  };
    ...
    it("associates content items with given input keys", async () => {
      ...
      expect(
        content!.contentSet.map((item) => ({
          itemId: item.primaryKeys[0].id,
          inputIds: item.inputKeys!.map((ik) => ik.id),
        }))
      ).to.containSubset([
        {
          itemId: "0xe",
      ...
      const content = await Presentation.presentation.getContent({
        ...
        descriptor: {
          instanceFilter: {
            ...
            expression:
              "related.Btu__x002F__lb__x0020____x005B__Btu__x0020__per__x0020__pound__x0020__mass__x005D__ = 1475.699",
            relatedInstances: [
              {
                pathFromSelectToPropertyClass: [
                  {
                    ...
                    targetClassName:
                      "DgnCustomItemTypes_MyProp:area__x0020__per__x0020__time__x0020__squaredElementAspect",
  ...
  describe("Content sources", () => {
    it("retrieves content sources for given class", async function () {
      ...
      let sources = await Presentation.presentation.getContentSources({
        imodel,
        classes: ["PCJ_TestSchema.TestClass"],
      });
      expect(sources).to.matchSnapshot(snapshotFilePath, snapshotName);

      sources = await Presentation.presentation.getContentSources({
        imodel,
        classes: ["PCJ_TestSchema:TestClass"],
      });
      expect(sources).to.matchSnapshot(snapshotFilePath, snapshotName);
  ...
  describe("Content instance keys", () => {
    it("retrieves content instance keys for given input", async () => {
      ...
      const resultKeys = [];
      for await (const key of result.items()) resultKeys.push(key);
  ...
  describe("waits for frontend timeout when request exceeds the backend timeout time", () => {
    ...
    it("should throw PresentationError", async () => {
      ...
      await expect(
        Presentation.presentation.getContentDescriptor({
          imodel,
          rulesetOrId: ruleset,
          keys,
          displayType: "Grid",
      ...
      expect(BeTimePoint.now().milliseconds).to.be.greaterThanOrEqual(
        start.plus(BeDuration.fromMilliseconds(frontendTimeout)).milliseconds
      );
    });
  });
});
...
function getFieldLabels(fields: Descriptor | Field[]): FieldLabels {
  if (fields instanceof Descriptor) fields = fields.fields;
...
function filterNestedContentFieldsByClass(
  ...
  fields.forEach((f) => {
    if (
      ...
      if (clone.nestedFields.length > 0) filteredFields.push(clone);
    } else {
...
function filterFieldsByClass(fields: Field[], classInfo: ECClassHierarchyInfo) {
  ...
  fields.forEach((f) => {
    if (f.isNestedContentField()) {
      ...
      if (appliesForGivenClass) filteredFields.push(f);
    } else {
      filteredFields.push(f);
    }
  });
