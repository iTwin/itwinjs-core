...
describe("Selection Scopes", () => {
  ...
  it("sets correct selection with 'element' selection scope", async () => {
    ...
    expect(
      selection.has({
        className: elementProps[0].classFullName,
        id: elementProps[0].id!,
      })
    );
  });
  ...
  it("sets correct selection with 'category' selection scope", async () => {
    ...
    expect(
      selection.has({
        className: "BisCore:Category",
        id: Id64.fromUint32Pair(23, 0),
      })
    );
  });
