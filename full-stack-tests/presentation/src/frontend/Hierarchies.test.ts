...
describe("Hierarchies", () => {
                ...
                relationshipPaths: [
                  {
                    relationship: {
                      schemaName: "BisCore",
                      className: "SubjectOwnsPartitionElements",
                    },
                    direction: RelationshipDirection.Forward,
                    targetClass: {
                      schemaName: "BisCore",
                      className: "DocumentPartition",
                    },
                  },
                ],
                groupByClass: false,
            ...
            children: [
              NodeValidators.createForPropertyGroupingNode({
                ...
                children: [
                  ...
                  NodeValidators.createForInstanceNode({
                    instanceKeys: [imodelElementKeys[2]],
                    label: "b",
                  }),
                ],
                supportsFiltering: true,
  ...
  describe("Limiting hierarchy level size", () => {
    describe("root instance nodes", () => {
      ...
      it("succeeds without limiting", async () => {
        await validateHierarchy({
          ...
          expectedHierarchy: [
            ...
            NodeValidators.createForInstanceNode({
              instanceKeys: [expectedInstanceKeys[1]],
              label: "b",
            }),
          ],
        });
      ...
      it("throws when result set size exceeds given limit", async () => {
        await expect(
          Presentation.presentation.getNodes({
            imodel,
            rulesetOrId: ruleset,
            sizeLimit: 1,
          })
    ...
    describe("grouping nodes", () => {
              ...
              children: [
                NodeValidators.createForPropertyGroupingNode({
                  ...
                  children: [
                    ...
                    NodeValidators.createForInstanceNode({
                      instanceKeys: [expectedInstanceKeys[2]],
                      label: "b",
                    }),
                  ],
                }),
      ...
      it("throws when result set size exceeds given limit", async () => {
        const classGroupingNodes = await Presentation.presentation.getNodes({
          imodel,
          rulesetOrId: ruleset,
        });
        ...
        const propertyGroupingNodes = await Presentation.presentation.getNodes({
          imodel,
          rulesetOrId: ruleset,
          parentKey: classGroupingNode.key,
        });
        ...
        await expect(
          Presentation.presentation.getNodes({
            imodel,
            rulesetOrId: ruleset,
            parentKey: propertyGroupingNode.key,
            sizeLimit: 2,
          })
        ).to.eventually.be.rejectedWith(PresentationError);

        const labelGroupingNodes = await Presentation.presentation.getNodes({
          imodel,
          rulesetOrId: ruleset,
          parentKey: propertyGroupingNode.key,
        });
        ...
        await expect(
          Presentation.presentation.getNodes({
            imodel,
            rulesetOrId: ruleset,
            parentKey: labelGroupingNode.key,
            sizeLimit: 1,
          })
        ).to.eventually.be.rejectedWith(PresentationError);
      });
  ...
  describe("Getting hierarchy level descriptors", () => {
    it("creates descriptor for root hierarchy level", async function () {
      ...
      const result = await Presentation.presentation.getNodesDescriptor({
        imodel,
        rulesetOrId: ruleset,
      });
    ...
    it("throws when attempting to get descriptor non-filterable hierarchy level", async function () {
      ...
      await expect(
        Presentation.presentation.getNodesDescriptor({
          imodel,
          rulesetOrId: ruleset,
        })
      ).to.eventually.be.rejectedWith(PresentationError);
  ...
  describe("Getting node paths", () => {
    ...
    it("gets node paths based on instance key paths", async () => {
      ...
      */
      ...
      const key3: InstanceKey = {
        id: Id64.fromString("0x10"),
        className: "BisCore:DefinitionPartition",
      };
      const key4: InstanceKey = {
        id: Id64.fromString("0xe"),
  ...
  export function createForPropertyGroupingNode(props: {
    ...
    return {
      node: (node) => {
        ...
        if (node.key.className !== props.className) {
          throw new Error(
            `Expected node to represent a property from class "${props.className}", got "${node.key.className}"`
          );
        }
        if (node.key.propertyName !== props.propertyName) {
          throw new Error(
            `Expected node to represent a property "${props.propertyName}", got "${node.key.propertyName}"`
          );
        }
        if (
...
async function validateHierarchy(props: {
  ...
  if (props.configureParams) props.configureParams(requestParams);
