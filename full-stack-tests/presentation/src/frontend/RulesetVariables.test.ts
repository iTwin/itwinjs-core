...
describe("Ruleset Variables", async () => {
  ...
  describe("Using variables in rules", () => {
    ...
    it("can specify lots of ids with Id64[] overload", async () => {
      ...
      const ids = [...Array(maxNumberOfSupportedBindParams).keys()].map((key) =>
        Id64.fromUint32Pair(key + largestECInstanceIdInTestDataset + 1, 0)
      );
      ids.push("0x61");
      ...
      expect(content!.contentSet[0].primaryKeys[0]).to.deep.eq({
