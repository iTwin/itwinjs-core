...
describe("Learning Snippets", () => {
  ...
  describe("Hierarchy Specifications", () => {
    it("uses `hideNodesInHierarchy` attribute", async () => {
      ...
      const customNodes = await Presentation.presentation.getNodes({
        imodel,
        rulesetOrId: ruleset,
        parentKey: classGroupingNodes[0].key,
      });
      expect(customNodes)
        .to.have.lengthOf(1)
        .and.to.containSubset([
    ...
    it("uses `hideIfNoChildren` attribute", async () => {
      ...
      const element3dNodes = await Presentation.presentation.getNodes({
        imodel,
        rulesetOrId: ruleset,
    ...
    it("uses `suppressSimilarAncestorsCheck` attribute", async () => {
      ...
      const rootSubjectNodes = await Presentation.presentation.getNodes({
        imodel,
        rulesetOrId: ruleset,
      });
      ...
      const rootSubjectChildNodes = await Presentation.presentation.getNodes({
        imodel,
        rulesetOrId: ruleset,
        parentKey: rootSubjectNodes[0].key,
      });
    ...
    it("uses `hasChildren` attribute", async () => {
      ...
      const modelClassGroupingNodes = await Presentation.presentation.getNodes({
        imodel,
        rulesetOrId: ruleset,
        parentKey: rootNodes[0].key,
      });
    ...
    it("uses `nestedRules` attribute", async () => {
      ...
      const modelChildren = await Presentation.presentation.getNodes({
        imodel,
        rulesetOrId: ruleset,
