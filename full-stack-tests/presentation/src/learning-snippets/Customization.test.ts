...
describe("Learning Snippets", () => {
  ...
  describe("Rules", () => {
    ...
    describe("Sorting", () => {
      ...
      it("uses `condition` attribute", async () => {
        // __PUBLISH_EXTRACT_START__ SortingRule.Condition.Ruleset
        ...
        // instances by `Pitch` property if "SORT_INSTANCES" ruleset variable set to "TRUE".
        const ruleset: Ruleset = {
          id: "example",
          rules: [{
            ruleType: RuleTypes.RootNodes,
            ...
            ruleType: RuleTypes.PropertySorting,
            condition: "GetVariableBoolValue(\"SORT_INSTANCES\")",
            propertyName: "Pitch",
          }],
        };
        // __PUBLISH_EXTRACT_END__
      ...
      it("uses `class` attribute", async () => {
        // __PUBLISH_EXTRACT_START__ SortingRule.Class.Ruleset
        // The ruleset has root node rule that returns `bis.SpatialViewDefinition` instances with labels
        // consisting of `Roll` and `Pitch` property values. Also there are customization rule to sort
