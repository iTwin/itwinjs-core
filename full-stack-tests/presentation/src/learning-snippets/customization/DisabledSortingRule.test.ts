  ...
  describe("Customization Rules", () => {
    describe("DisabledSortingRule", () => {
      ...
      it("uses `class` attribute", async () => {
        ...
        expect(nodes).to.be.lengthOf(4);
        expect(nodes[0]).to.containSubset({
          label: { displayValue: "SpatialViewDefinition - Default - View 1" },
        });
        expect(nodes[1]).to.containSubset({
          label: { displayValue: "SpatialViewDefinition - Default - View 2" },
        });
        expect(nodes[2]).to.containSubset({
          label: { displayValue: "SpatialViewDefinition - Default - View 3" },
        });
        expect(nodes[3]).to.containSubset({
          label: { displayValue: "SpatialViewDefinition - Default - View 4" },
        });
      });
