...
export class ECClassHierarchy {
  ...
  public static async create(imodel: IModelConnection) {
    ...
    const classesQuery = `SELECT
      ...
      `;
    for await (const row of imodel.createQueryReader(classesQuery, undefined, {
    ...
    for await (const row of imodel.createQueryReader(
      ...
      if (baseClasses) baseClasses.push(baseClassId);
      else baseClassHierarchy.set(classId, [baseClassId]);

      const derivedClasses = derivedClassHierarchy.get(baseClassId);
      if (derivedClasses) derivedClasses.push(classId);
