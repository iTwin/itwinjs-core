  ...
  describe("Property value formatting", () => {
    ...
    it("formats property using default format when it doesn't have format for requested unit system", async () => {
      ...
      const defaultFormats = {
        area: {
          unitSystems: ["imperial" as UnitSystemKey],
          format: formatProps,
        },
      };

      expect(await getAreaDisplayValue("imperial", defaultFormats)).to.eq(
        "0.1616 ft²"
      );
    });
    ...
    it("formats property using provided format when it has provided format and default format for requested unit system", async () => {
      ...
      expect(await getAreaDisplayValue("metric", defaultFormats)).to.eq(
        "150.1235 cm²"
      );
    });

    async function getAreaDisplayValue(
      unitSystem: UnitSystemKey,
      defaultFormats?: { [phenomenon: string]: UnitSystemFormat }
    ): Promise<DisplayValue> {
      return using(
        new PresentationManager({ defaultFormats, defaultLocale: "en-PSEUDO" }),
        async (manager) => {
          ...
          const displayValues = content!.contentSet[0].displayValues
            .rc_generic_PhysicalObject_ncc_MyProp_areaElementAspect as DisplayValuesArray;
          expect(displayValues.length).is.eq(1);
          return (
            (displayValues[0] as DisplayValuesMap)
              .displayValues as DisplayValuesMap
          )[field.name]!;
        }
      );
    }
  ...
  describe("getElementProperties", () => {
    it("returns properties for some elements of class 'PhysicalObject", async () => {
      await using(new PresentationManager(), async (manager) => {
        ...
        const { iterator } = await manager.getElementProperties({
          imodel,
    ...
    it("cancels 'getNodes' request", async () => {
      await using(new PresentationManager(), async (manager) => {
        ...
        await expect(promise)
          ...
          .and.have.property("errorNumber", PresentationStatus.Canceled);
      });
    });
  });
});
