      ...
      while (stmt.step() === DbResult.BE_SQLITE_ROW) {
        ...
        switch (instanceChange.opCode) {
          ...
          case ChangeOpCode.Update: {
            let rows: any[] = IModelTestUtils.executeQuery(
              iModel,
              ChangeSummaryManager.buildPropertyValueChangesECSql(
                iModel,
                instanceChange,
                ChangedValueState.BeforeUpdate
              )
            );
            assert.equal(rows.length, 1);
          ...
          case ChangeOpCode.Delete: {
            const rows: any[] = IModelTestUtils.executeQuery(
              ...
              ChangeSummaryManager.buildPropertyValueChangesECSql(
                iModel,
                instanceChange,
                ChangedValueState.BeforeDelete
              )
            );
            assert.equal(rows.length, 1);
            instanceChange.before = rows[0];
...
describe("ChangeSummary", () => {
  ...
  it("Extract ChangeSummaries", async () => {
    ...
    const summaryIds = await ChangeSummaryManager.createChangeSummaries({
      accessToken,
      iTwinId,
      iModelId,
      range: { first: 0 },
    ...
    try {
      ...
      iModel.withPreparedStatement(
        "SELECT ECInstanceId,ECClassId,ExtendedProperties FROM change.ChangeSummary ORDER BY ECInstanceId",
        (myStmt) => {
          ...
          while (myStmt.step() === DbResult.BE_SQLITE_ROW) {
            ...
            assert.isUndefined(
              ...
              "ChangeSummary.ExtendedProperties is not expected to be populated when change summaries are extracted."
            );
          }
      ...
      iModel.withPreparedStatement(
        "SELECT ECClassId,Summary,WsgId,ParentWsgId,Description,PushDate,UserCreated FROM imodelchange.ChangeSet ORDER BY Summary.Id",
        (myStmt) => {
          let rowCount: number = 0;
          while (myStmt.step() === DbResult.BE_SQLITE_ROW) {
            rowCount++;
            ...
            assert.isDefined(
              row.userCreated,
              "IModelChange.ChangeSet.UserCreated is expected to be set for the changesets used in this test."
            );
  ...
  it("Extract ChangeSummary for a model with no changesets", async () => {
    ...
    try {
      ...
      iModel.withPreparedStatement(
        "SELECT WsgId, Summary, ParentWsgId, Description, PushDate, UserCreated FROM imodelchange.ChangeSet",
        (myStmt) => {
  ...
  it("Extracting ChangeSummaries for a range of changesets", async () => {
    ...
    const iModel = await HubWrappers.downloadAndOpenBriefcase({
      ...
      iTwinId,
      iModelId,
      asOf: IModelVersion.asOfChangeSet(lastChangeSet.id).toJSON(),
    });
    try {
      assert.exists(iModel);
      ChangeSummaryManager.attachChangeCache(iModel);
      ...
      iModel.withPreparedStatement(
        "SELECT WsgId, Summary, ParentWsgId, Description, PushDate, UserCreated FROM imodelchange.ChangeSet ORDER BY Summary.Id",
        (myStmt) => {
          assert.equal(myStmt.step(), DbResult.BE_SQLITE_ROW);
  ...
  it("Subsequent ChangeSummary extractions", async () => {
    ...
    let iModel = await HubWrappers.downloadAndOpenBriefcase({
      accessToken,
      iTwinId,
      iModelId,
    });
    try {
      ...
      await HubWrappers.closeAndDeleteBriefcaseDb(accessToken, iModel);
      iModel = await HubWrappers.downloadAndOpenBriefcase({
        accessToken,
        iTwinId,
        iModelId,
        asOf: IModelVersion.asOfChangeSet(lastChangesetId).toJSON(),
      });
      ...
      iModel.withPreparedStatement(
        "SELECT cset.WsgId changesetId FROM change.ChangeSummary csum JOIN imodelchange.ChangeSet cset ON csum.ECInstanceId=cset.Summary.Id ORDER BY csum.ECInstanceId",
        (myStmt) => {
          let rowCount: number = 0;
          while (myStmt.step() === DbResult.BE_SQLITE_ROW) {
            rowCount++;
  ...
  it("Query ChangeSummary content", async () => {
    ...
    await ChangeSummaryManager.createChangeSummaries({
      accessToken,
      iTwinId,
      iModelId,
      range: { first: 0 },
    ...
    try {
      ...
      for (const changeSummary of changeSummaries) {
        const filePath = path.join(
          outDir,
          `imodelid_${iModelId}_changesummaryid_${changeSummary.id}.changesummary.json`
        );
        ...
        iModel.withPreparedStatement(
          ...
          (stmt) => {
            stmt.bindId(1, changeSummary.id);
            perfLogger = new PerfLogger(
              `ChangeSummaryManager.queryInstanceChange for all instances in ChangeSummary ${changeSummary.id}`
            );
            while (stmt.step() === DbResult.BE_SQLITE_ROW) {
              const row = stmt.getRow();
  ...
  it.skip("Create ChangeSummaries for changes to parent elements", async () => {
    ...
    const elementId1: Id64String = iModel.elements.insertElement(
      IModelTestUtils.createPhysicalObject(iModel, modelId, categoryId).toJSON()
    );
    const elementId2: Id64String = iModel.elements.insertElement(
      IModelTestUtils.createPhysicalObject(iModel, modelId, categoryId).toJSON()
    );
    const elementId3: Id64String = iModel.elements.insertElement(
      IModelTestUtils.createPhysicalObject(iModel, modelId, categoryId).toJSON()
    );
    ...
    await iModel.pushChanges({
      accessToken: managerRequestContext,
      description: "Updated parent element",
    });
  ...
  it("Detaching and reattaching change cache", async () => {
    ...
    try {
      for (const changeset of changesets) {
        ...
        iModel.withPreparedStatement(
          "SELECT WsgId, Summary FROM imodelchange.ChangeSet WHERE Summary.Id=?",
          (myStmt) => {
        ...
        for await (const row of iModel.createQueryReader(
          "SELECT WsgId, Summary FROM imodelchange.ChangeSet WHERE Summary.Id=?",
          QueryBinder.from([changeSummaryId]),
