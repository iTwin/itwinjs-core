...
describe("Cloud workspace containers", () => {
  ...
  it.only("create additional workspace db in workspace container after retrieving container", async () => {
    ...
    AzuriteTest.userToken = AzuriteTest.service.userToken.admin;
    const testContainer: EditableWorkspaceContainer = await editor.createNewCloudContainer({ metadata: { label: "test container", description: "test container" }, scope: { iTwinId: iTwin1Id }, manifest: { workspaceName: "test container" }, dbName: "wsdb-all-settings" });
    AzuriteTest.userToken = AzuriteTest.service.userToken.readWrite;
    const testContainerProps: WorkspaceContainerProps = testContainer.cloudProps!;
    ...
    // Query for the newly created default workspacedb v1.0.0 (named "wsdb-all-settings")
    const defaultWorkspaceDb = await IModelHost.appWorkspace.getWorkspaceDb({
      dbName: "wsdb-all-settings",
      storageType: "azure",
    ...
    // const wsDbEditPre = container.getEditableDb({ dbName: "new workspacedb test" });
    const wsDbEditPre = await testContainer.createDb({ ...workspaceDbProps, manifest }); // dbName: newWorkspaceDbName
    wsDbEditPre.open();
