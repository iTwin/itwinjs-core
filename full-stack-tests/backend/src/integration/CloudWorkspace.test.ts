...
describe("Cloud workspace containers", () => {
  async function initializeContainer(containerId: string) {
    const cloudCont1 = CloudSqliteTest.makeCloudSqliteContainer(
      containerId,
      false
    );
  ...
  it("cloud workspace", async () => {
    ...
    const workspace1 = new ITwinWorkspace(new BaseSettings(), {
      containerDir: join(IModelHost.cacheDir, "TestWorkspace1"),
      testCloudCache: makeCloudCache("test1"),
    });
    const workspace2 = new ITwinWorkspace(new BaseSettings(), {
      containerDir: join(IModelHost.cacheDir, "TestWorkspace2"),
      testCloudCache: makeCloudCache("test2"),
    });
    const settings = workspace1.settings;
    settings.addDictionary(
      "containers",
      SettingsPriority.application,
      containerDict
    );

    await initializeContainer(containerId);
    const wsCont1 = workspace1.getContainer(
      {
        containerId,
        writeable: true,
        accessToken: CloudSqliteTest.makeSasToken(containerId, "rwadl"),
      },
      CloudSqliteTest.storage
    );
    ...
    const makeVersion = async (version?: string) => {
      ...
      await CloudSqlite.withWriteLock(
        ...
        wsCont1.cloudContainer!,
        async () => {
          const wsDbEdit = new EditableWorkspaceDb(
            { dbName: testDbName },
            wsCont1
          );
          try {
            await wsDbEdit.createDb(version);
            const account1 = settings.getObject<CloudSqlite.AccountAccessProps>(
              "cloudSqlite/accountProps"
            )!;
    ...
    const wsCont2 = workspace2.getContainer(
      {
        containerId,
        accessToken: CloudSqliteTest.makeSasToken(containerId, "rl"),
      },
      CloudSqliteTest.storage
    );
    ...
    expect(() => ws2Cloud.acquireWriteLock("other session")).to.throw(
    ...
    expect(() =>
      wsCont2.getWorkspaceDb({ dbName: testDbName, version: "^2.0.0" })
    ).throws("No version of");
    ...
    await CloudSqlite.withWriteLock(
      ...
      wsCont1.cloudContainer,
      async () => {
        const ws3 = new EditableWorkspaceDb(
          { dbName: testDbName, version: "1.1.4-beta" },
          wsCont1
        );
        ws3.open();
    ...
    expect(
      wsCont2.resolveDbFileName({ dbName: testDbName, version: "~1.1.0" })
    ).contains("1.1.3");
    expect(
      wsCont2.resolveDbFileName({ dbName: testDbName, version: "1.2.0" })
    ).contains("1.2.0");
    expect(
      wsCont2.resolveDbFileName({
        dbName: testDbName,
        version: ">1.0.0 <3.0.0",
      })
    ).contains("1.2.4");
    expect(
      wsCont2.resolveDbFileName({ dbName: testDbName, version: "1.0.0" })
    ).contains("1.0.0");

    ws2 = wsCont2.getWorkspaceDb({
      dbName: testDbName,
      version: "~1.1.0",
    ...
    const workspace3 = new ITwinWorkspace(new BaseSettings(), {
      containerDir: join(IModelHost.cacheDir, "TestWorkspace3"),
      testCloudCache: makeCloudCache("test3"),
    });
    workspace3.settings.addDictionary(
      "testDict",
      SettingsPriority.application,
      dict
    );
    const db = await workspace3.getWorkspaceDb(
      "test/test1",
      async (props, account) => {
