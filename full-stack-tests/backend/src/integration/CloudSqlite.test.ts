...
export namespace CloudSqliteTest {
  ...
  const credential = new azureBlob.StorageSharedKeyCredential(
    storage.accessName,
  ...
  export function makeCaches(names: string[]) {
    const caches = [];
    for (const name of names) caches.push(makeCache(name));
    return caches;
  }
  ...
  export async function uploadFile(
    ...
    await CloudSqlite.withWriteLock("upload", container, async () =>
      CloudSqlite.uploadDb(container, { dbName, localFileName })
    );
    expect(container.isConnected);
...
describe("CloudSqlite", () => {
  ...
  before(async () => {
    testContainers = CloudSqliteTest.makeCloudSqliteContainers([
      ["test1", false],
      ["test2", false],
      ["test3", true],
    ]);
  ...
  it("cloud containers", async () => {
    ...
    await CloudSqlite.withWriteLock(user, contain1, async () => {
      await expect(contain1.cleanDeletedBlocks())
        .eventually.rejectedWith("not authorized")
        .property("errorNumber", 403);
    });
    ...
    await CloudSqlite.withWriteLock(user, contain1, async () => {
      ...
      expect(() => cont2.acquireWriteLock(user))
        .throws("is currently locked")
        .property("errorNumber", DbResult.BE_SQLITE_BUSY);
    });
    ...
    await CloudSqlite.withWriteLock(user2, cont2, async () => {
      await expect(
        CloudSqlite.withWriteLock(
          user,
          contain1,
          async () => {},
          async (lockedBy: string, expires: string) => {
    ...
    expect(() => contain1.connect(caches[0]))
      .throws("403")
      .property("errorNumber", 403);
    ...
    expect(contain1.isConnected);
    // eslint-disable-next-line @typescript-eslint/promise-function-async
    expect(() => contain1.acquireWriteLock(user))
      .throws("not authorized")
      .property("errorNumber", DbResult.BE_SQLITE_AUTH);
    ...
    expect(dbs.length).equals(2);
    // eslint-disable-next-line @typescript-eslint/promise-function-async
    expect(() => anonContainer.acquireWriteLock(user))
      .throws("not authorized")
      .property("errorNumber", DbResult.BE_SQLITE_AUTH);
    ...
    const wasCache1 = {
      ...
      cacheDir: caches[0].rootDir,
      guid: caches[0].guid,
    };
    const wasCache2 = {
      cacheName: caches[1].name,
      cacheDir: caches[1].rootDir,
      guid: caches[1].guid,
    };
