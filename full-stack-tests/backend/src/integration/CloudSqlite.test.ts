...
// spell:ignore imodelid itwinid mkdirs localstore itwindb devstoreaccount racwdl

useFromChai(chaiAsPromised);

describe.only("CloudSqlite", () => {
  let caches: CloudSqlite.CloudCache[];
  let testContainers: AzuriteContainerService.TestContainer[];
  let testBimGuid: GuidString;
  const user1 = "CloudSqlite test1";
  const user2 = "CloudSqlite test2";

  before(async () => {
    testContainers = AzuriteContainerService.makeCloudSqliteContainers([["test1", false], ["test2", false], ["test3", true]]);
    caches = AzuriteContainerService.makeCaches(["cache1", "cache2"]);
    await AzuriteContainerService.initializeContainers(testContainers);
    ...
    await AzuriteContainerService.uploadFile(testContainers[0], caches[0], "c0-db1:0", tempDbFile);
    await AzuriteContainerService.uploadFile(testContainers[0], caches[0], "testBim", testBimFileName);
    await AzuriteContainerService.uploadFile(testContainers[1], caches[0], "c1-db1:2.1", tempDbFile);
    await AzuriteContainerService.uploadFile(testContainers[2], caches[0], "c2-db1", tempDbFile);
    await AzuriteContainerService.uploadFile(testContainers[2], caches[0], "testBim", testBimFileName);
  });
  ...
  it("cloud containers", async () => {
    ...
    contain1.disconnect();
    await AzuriteContainerService.setSasToken(contain1, false); // don't ask for delete permission
    ...
    contain1.disconnect();
    await AzuriteContainerService.setSasToken(contain1, true); // now ask for delete permission
    contain1.connect(caches[1]);
    ...
    const cont2 = AzuriteContainerService.makeCloudSqliteContainer(contain1.containerId, false);

    await AzuriteContainerService.setSasToken(cont2, true);
    ...
    await AzuriteContainerService.setSasToken(contain1, false); // get a read-only token
