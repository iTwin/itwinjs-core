...
import { _nativeDb, BriefcaseDb, BriefcaseManager, CloudSqlite, HubMock, IModelDb, IModelHost, PullMergeMethod, SchemaSync, SnapshotDb, SqliteStatement } from "@itwin/core-backend";
...
for (const pullMergeMethod of ["Rebase"] as PullMergeMethod[]) {
  describe.only(`Schema synchronization (PullMergeMethod: ${pullMergeMethod})`, function (this: Suite) {
    this.timeout(0);
