...
describe("SchemaDesignPerf Impact of Properties", () => {
  ...
  function createSchema(propCount: number): string {
    const schemaPath = path.join(
      outDir,
      `TestPropsSchema-${propCount}.01.00.00.ecschema.xml`
    );
  ...
  before(async () => {
    ...
    for (const pCount of propCounts) {
      ...
      if (!IModelJsFs.existsSync(seedName)) {
        ...
        assert.isDefined(
          seedIModel.getMetaData("TestPropsSchema:PropElement"),
          "PropsClass is present in iModel."
        );
        const [, newModelId] =
        ...
        assert.equal(
          getCount(seedIModel, "TestPropsSchema:PropElement"),
          seedCount
        );
        seedIModel.close();
      }
  ...
  it("Insert", async () => {
    for (const propCount of propCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "PropPerformance",
        `PropsPerf_Insert_${propCount}.bim`
      );
      ...
      assert.equal(
        getCount(perfimodel, "TestPropsSchema:PropElement"),
        opCount + seedCount
      );
      perfimodel.close();
      ...
      reporter.addEntry(
        ...
        "Execution time(s)",
        totalTime,
        { count: opCount, properties: propCount }
      );
    }
  });
  it("Delete", async () => {
    for (const propCount of propCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "PropPerformance",
        `PropsPerf_Delete_${propCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
  ...
  it("Read", async () => {
    for (const propCount of propCounts) {
      ...
      for (let j = 0; j < opCount; ++j) {
        ...
        const elemFound: any = perfimodel.elements.getElement(
          Id64.fromLocalAndBriefcaseIds(elId, 0)
        );
  ...
  it("Update", async () => {
    for (const propCount of propCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "PropPerformance",
        `PropsPerf_Update_${propCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
      ...
      for (let i = 0; i < opCount; ++i) {
        const elId = minId + elementIdIncrement * i;
        const editElem: any = perfimodel.elements.getElement(
          Id64.fromLocalAndBriefcaseIds(elId, 0)
        );
...
describe("SchemaDesignPerf Number of Indices", () => {
  ...
  function createSchema(indexCount: number, perClass: boolean = false): string {
    ...
    if (perClass)
      schemaPath = path.join(
        outDir,
        `TestIndexSchema-PerClass${indexCount}.01.00.00.ecschema.xml`
      );
    else
      schemaPath = path.join(
        outDir,
        `TestIndexSchema-${indexCount}.01.00.00.ecschema.xml`
      );
    if (!IModelJsFs.existsSync(schemaPath)) {
      let sxml = `<?xml version="1.0" encoding="UTF-8"?>
  ...
  before(async () => {
    ...
    for (const iCount of indexCounts) {
      ...
      if (!IModelJsFs.existsSync(seedName)) {
        ...
        seedIModel.nativeDb.resetBriefcaseId(BriefcaseIdValue.Unassigned);
        assert.isDefined(
          seedIModel.getMetaData("TestIndexSchema:PropElement"),
          "PropsClass is present in iModel."
        );
        const [, newModelId] =
          IModelTestUtils.createAndInsertPhysicalPartitionAndModel(
        ...
        seedIModel.saveChanges();
        assert.equal(
          getCount(seedIModel, "TestIndexSchema:PropElement"),
          seedCount
        );
        seedIModel.close();
      }
    ...
    for (const iCount of indexCounts) {
      ...
      if (!IModelJsFs.existsSync(seedName)) {
        ...
        seedIModel.nativeDb.resetBriefcaseId(BriefcaseIdValue.Unassigned);
        assert.isDefined(
          seedIModel.getMetaData("TestIndexSchema:PropElement0"),
          "PropsClass is present in iModel."
        );
        const [, newModelId] =
          IModelTestUtils.createAndInsertPhysicalPartitionAndModel(
        ...
        seedIModel.saveChanges();
        assert.equal(
          getCount(seedIModel, "TestIndexSchema:PropElement0"),
          seedCount
        );
  ...
  it("Insert", async () => {
    for (const indexCount of indexCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "IndexPerformance",
        `IndexPerf_Insert_${indexCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
      ...
      perfimodel.saveChanges();
      assert.equal(
        getCount(perfimodel, "TestIndexSchema:PropElement"),
        opCount + seedCount
      );
      perfimodel.close();
      ...
      reporter.addEntry(
        ...
        "Execution time(s)",
        totalTime,
        { count: opCount, indices: indexCount, perClass: "No" }
      );
    }
    ...
    for (const indexCount of indexCounts) {
      ...
      const seedFileName = path.join(
        outDir,
        `index_perclass_${indexCount}.bim`
      );
      const testFileName = IModelTestUtils.prepareOutputFile(
        "IndexPerformance",
        `IndexPerf_PerClass_Insert_${indexCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
      ...
      perfimodel.saveChanges();
      assert.equal(
        getCount(perfimodel, "TestIndexSchema:PropElement0"),
        opCount + seedCount
      );
      perfimodel.close();
      ...
      reporter.addEntry(
        ...
        "Execution time(s)",
        totalTime,
        { count: opCount, indices: indexCount, perClass: "Yes" }
      );
    }
  });
  it("Delete", async () => {
    for (const indexCount of indexCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "IndexPerformance",
        `IndexPerf_Delete_${indexCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
    ...
    for (const indexCount of indexCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "IndexPerformance",
        `IndexPerf_PerClass_Delete_${indexCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
  ...
  it("Read", async () => {
    for (const indexCount of indexCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "IndexPerformance",
        `IndexPerf_Read_${indexCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
    ...
    for (const indexCount of indexCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "IndexPerformance",
        `IndexPerf_PerClass_Read_${indexCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
  ...
  it("Update", async () => {
    for (const indexCount of indexCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "IndexPerformance",
        `PropsPerf_Update_${indexCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
    ...
    for (const indexCount of indexCounts) {
      ...
      const testFileName = IModelTestUtils.prepareOutputFile(
        "IndexPerformance",
        `PropsPerf_PerClassUpdate_${indexCount}.bim`
      );
      const perfimodel = IModelTestUtils.createSnapshotFromSeed(
        testFileName,
        seedFileName
