...
describe("SchemaDesignPerf Impact of Properties", () => {
  ...
  before(async () => {
    ...
    for (let i = pConfig.start; i <= pConfig.end; i = i + pConfig.increment) propCounts.push(i);
    ...
    for (const pCount of propCounts) {
      ...
      if (!IModelJsFs.existsSync(seedName)) {
        const seedIModel = SnapshotDb.createEmpty(
          IModelTestUtils.prepareOutputFile("PropPerformance", `props_${pCount}.bim`),
  ...
  it("Insert", async () => {
    for (const propCount of propCounts) {
      ...
      reporter.addEntry("PropPerfTest", "ElementsInsert", "Execution time(s)", totalTime, {
        count: opCount,
  ...
  it("Delete", async () => {
    for (const propCount of propCounts) {
      ...
      reporter.addEntry("PropPerfTest", "ElementsDelete", "Execution time(s)", elapsedTime, {
        count: opCount,
        properties: propCount,
      });
    }
  ...
  it("Read", async () => {
    for (const propCount of propCounts) {
      ...
      perfimodel.close();
      reporter.addEntry("PropPerfTest", "ElementsRead", "Execution time(s)", elapsedTime, {
        count: opCount,
        properties: propCount,
      });
    }
  ...
  it("Update", async () => {
    for (const propCount of propCounts) {
      ...
      const elapsedTime = (endTime - startTime) / 1000.0;
      perfimodel.close();
      reporter.addEntry("PropPerfTest", "ElementsUpdate", "Execution time(s)", elapsedTime, {
        count: opCount,
        properties: propCount,
      });
    }
...
describe("SchemaDesignPerf Number of Indices", () => {
  ...
  function createSchema(indexCount: number, perClass: boolean = false): string {
    let schemaPath = "";
    if (perClass) schemaPath = path.join(outDir, `TestIndexSchema-PerClass${indexCount}.01.00.00.ecschema.xml`);
    else schemaPath = path.join(outDir, `TestIndexSchema-${indexCount}.01.00.00.ecschema.xml`);
    if (!IModelJsFs.existsSync(schemaPath)) {
      let sxml = `<?xml version="1.0" encoding="UTF-8"?>
  ...
  before(async () => {
    ...
    for (const iCount of indexCounts) {
      ...
      if (!IModelJsFs.existsSync(seedName)) {
        const seedIModel = SnapshotDb.createEmpty(
          IModelTestUtils.prepareOutputFile("IndexPerformance", `index_${iCount}.bim`),
          { rootSubject: { name: "PerfTest" } }
        );
        await seedIModel.importSchemas([st]);
        seedIModel.nativeDb.resetBriefcaseId(BriefcaseIdValue.Unassigned);
    ...
    for (const iCount of indexCounts) {
      ...
      if (!IModelJsFs.existsSync(seedName)) {
        const seedIModel = SnapshotDb.createEmpty(
          IModelTestUtils.prepareOutputFile("IndexPerformance", `index_perclass_${iCount}.bim`),
          { rootSubject: { name: "PerfTest" } }
        );
        await seedIModel.importSchemas([st]);
        seedIModel.nativeDb.resetBriefcaseId(BriefcaseIdValue.Unassigned);
  ...
  it("Insert", async () => {
    for (const indexCount of indexCounts) {
      ...
      reporter.addEntry("IndexPerfTest", "ElementsInsert", "Execution time(s)", totalTime, {
        count: opCount,
        indices: indexCount,
        perClass: "No",
      });
  ...
  it("Read", async () => {
    for (const indexCount of indexCounts) {
      ...
      perfimodel.close();
      reporter.addEntry("IndexPerfTest", "ElementsRead", "Execution time(s)", elapsedTime, {
        count: opCount,
        indices: indexCount,
        perClass: "No",
      });
