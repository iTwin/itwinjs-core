...
import { ECSqlStatement, IModelDb, IModelHost, IModelJsFs, SnapshotDb, SpatialCategory } from "@itwin/core-backend";
...
const crudConfig = require(path.join("../../../src/perftest/", "CRUDConfig.json")).test3d; // eslint-disable-line @typescript-eslint/no-var-requires
    ...
    for (const size of crudConfig.dbSizes) {
      ...
      for (const opCount of crudConfig.opSizes) {
        ...
        for (let m = 0; m < opCount; ++m) {
          ...
          geomElements.push(geomElement);
        }
        geomElements.forEach((geomElement) => {
          perfimodel.elements.insertElement(geomElement.toJSON());
          // if(index %100 === 0){
          //   // eslint-disable-next-line no-console
