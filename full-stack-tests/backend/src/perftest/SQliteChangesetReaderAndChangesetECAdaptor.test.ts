...
describe("SqliteChangesetReaderAndChangesetECAdaptorAPI", async () => {
  ...
  after(async () => {
    const csvPath = path.join(KnownTestLocations.outputDir, "PerformanceResultsSqliteChangesetReaderAndChangesetECAdaptor.csv");
  ...
  it("Large Changeset Performance - InMemoryCache", async () => {
    ...
    for (const testCase of testCases) {
      ...
      reporter.addEntry("SqliteChangesetReaderAndChangesetECAdaptorAPI", "Unifier-InMemoryCache", "Execution time (seconds)", watch.elapsedSeconds, { iModelId: rwIModelId, changesetId: changesets[testCase.testCaseNum].id, changesetInserts: testCase.numElements });
    }
    rwIModel.close();
  });
  ...
  it("Large Changeset Performance - SqliteBackedCache", async () => {
    ...
    for (const testCase of testCases) {
      ...
      assert.equal(unifier.getInstanceCount(), testCase.numElements, "Number of instances should match the number of inserted elements til now");
      reporter.addEntry("SqliteChangesetReaderAndChangesetECAdaptorAPI", "Unifier-SqliteBackedCache", "Execution time (seconds)", watch.elapsedSeconds, { iModelId: rwIModelId, changesetId: changesets[testCase.testCaseNum].id, changesetInserts: testCase.numElements });
    }
    rwIModel.close();
  });
  ...
  it("Track changeset health stats", async () => {
    ...
    reporter.addEntry("SqliteChangesetReaderAndChangesetECAdaptorAPI", "ChangesetHealthStats", "Execution time (ms)", secondBriefcaseChangeset2.totalElapsedMs, { changesetId: secondBriefcaseChangeset2.changesetId, statementsExecuted: secondBriefcaseChangeset2.perStatementStats.length });
