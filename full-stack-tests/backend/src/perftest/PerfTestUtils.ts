...
export class PerfTestDataMgr {
  ...
  public constructor(imodelPath: string, createNew: boolean = false) {
    if (createNew) {
      if (IModelJsFs.existsSync(imodelPath)) IModelJsFs.removeSync(imodelPath);
    }
...
function genPropValue(prop: string): any {
  if (prop.toLowerCase().includes("string")) return "Test Value";
  if (prop.toLowerCase().includes("double")) return 4.25658;
  if (prop.toLowerCase().includes("long")) return 30486025;
  if (prop.toLowerCase().includes("int")) return 12;
  if (prop.toLowerCase().includes("datetime")) return "2020-01-01T00:00:00.000";
...
export class PerfTestUtility {
  ...
  public static getMinId(imodel: IModelDb, className: string): number {
    const stat = IModelTestUtils.executeQuery(
      imodel,
      `SELECT MAX(ECInstanceId) maxId, MIN(ECInstanceId) minId FROM ${className}`
    )[0];
    const minId: number = parseInt(stat.minId.toString().substring(2), 16);
    return minId;
  }

  public static createElemProps(
    className: string,
    _iModelName: IModelDb,
    modId: Id64String,
    catId: Id64String
