...
describe("PerformanceElementsTests", () => {
  ...
  it("ElementsInsert", async () => {
    for (const name of crudConfig.classNames) {
      for (const size of crudConfig.dbSizes) {
        const seedFileName = path.join(
          KnownTestLocations.outputDir,
          "ElementCRUDPerformance",
          `Performance_seed_${name}_${size}.bim`
        );
        for (const opCount of crudConfig.opSizes) {
          // eslint-disable-next-line no-console
          console.log(
            `Executing Element Insert for the class ${name} on an iModel with ${size} elements ${opCount} times`
          );
    ...
    for (const name of crudConfig.classNames) {
      for (const size of crudConfig.dbSizes) {
        ...
        for (const opCount of crudConfig.opSizes) {
          // eslint-disable-next-line no-console
          console.log(
            `Executing Element Delete for the class ${name} on an iModel with ${size} elements ${opCount} times`
          );
    ...
    for (const name of crudConfig.classNames) {
      for (const size of crudConfig.dbSizes) {
        ...
        for (const opCount of crudConfig.opSizes) {
          // eslint-disable-next-line no-console
          console.log(
            `Executing Element Read for the class ${name} on an iModel with ${size} elements ${opCount} times`
          );
    ...
    for (const name of crudConfig.classNames) {
      for (const size of crudConfig.dbSizes) {
        ...
        for (const opCount of crudConfig.opSizes) {
          // eslint-disable-next-line no-console
          console.log(
            `Executing Element Update for the class ${name} on an iModel with ${size} elements ${opCount} times`
          );
    ...
    for (const name of crudConfig.classNames) {
      for (const size of crudConfig.dbSizes) {
        const seedFileName = path.join(
          KnownTestLocations.outputDir,
          "ElementCRUDPerformance2d",
          `Performance2d_seed_${name}_${size}.bim`
        );
        for (const opCount of crudConfig.opSizes) {
          // eslint-disable-next-line no-console
          console.log(
