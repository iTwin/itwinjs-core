...
describe("Schema RPC Interface", () => {
  ...
  it("should get schema keys", async () => {
    ...
    const props: SchemaKeyProps[] =
      await ECSchemaRpcInterface.getClient().getSchemaKeys(
        iModel.getRpcProps()
      );
    props.forEach((prop: SchemaKeyProps) =>
      schemaKeys.push(SchemaKey.fromJSON(prop))
    );
  ...
  it("should get schema JSON", async () => {
    ...
    const schemaJSON: SchemaProps =
      await ECSchemaRpcInterface.getClient().getSchemaJSON(
        iModel.getRpcProps(),
        schemaKeys[0].name
      );
