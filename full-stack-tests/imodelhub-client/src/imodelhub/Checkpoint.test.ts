...
describe("iModelHub CheckpointHandler", () => {
  ...
  before(async function () {
    ...
    changeSets = await utils.createChangeSets(accessToken, imodelId, briefcase, 0, 3);
  ...
  it("should query and download Checkpoint", async () => {
    ...
    const checkpoints = await iModelClient.checkpoints.get(accessToken, imodelId, new CheckpointQuery().selectDownloadUrl());
    ...
    await iModelClient.checkpoints.download(accessToken, checkpoints[0], path.join(workDir, checkpoints[0].fileName!), progressTracker.track());
  ...
  it("should query Checkpoint by id", async () => {
    ...
    const checkpoints = await iModelClient.checkpoints.get(accessToken, imodelId, new CheckpointQuery().byChangeSetId(changeSets[2].id!));
    chai.assert(checkpoints);
    chai.expect(checkpoints.length).to.be.equal(1);
  });
  ...
  it("should query nearest Checkpoint", async () => {
    ...
    const checkpoints1 = await iModelClient.checkpoints.get(accessToken, imodelId, new CheckpointQuery().nearestCheckpoint(changeSets[0].id!));
    ...
    const checkpoints2 = await iModelClient.checkpoints.get(accessToken, imodelId, new CheckpointQuery().nearestCheckpoint(changeSets[1].id!));
  ...
  it("should query preceding Checkpoint", async () => {
    ...
    const checkpoints1 = await iModelClient.checkpoints.get(accessToken, imodelId, new CheckpointQuery().precedingCheckpoint(changeSets[1].id!));
    chai.assert(checkpoints1);
    chai.expect(checkpoints1.length).to.be.equal(1);
    chai.expect(checkpoints1[0].mergedChangeSetId).to.be.equal("");
    ...
    const checkpoints2 = await iModelClient.checkpoints.get(accessToken, imodelId, new CheckpointQuery().precedingCheckpoint(changeSets[2].id!));
    chai.assert(checkpoints2);
    chai.expect(checkpoints2.length).to.be.equal(1);
    chai.expect(checkpoints2[0].mergedChangeSetId).to.be.equal(changeSets[2].id!);
  });
