...
import { AccessToken, Guid, GuidString, IModelHubStatus } from "@bentley/bentleyjs-core";
import {
...
describe("iModelHub iModelsHandler", () => {
  ...
  before(async function () {
    ...
    assetId = await utils.getAssetId(accessToken);
  ...
  beforeEach(async () => {
    await utils.deleteIModelByName(accessToken, projectId, createimodelName, false);
  });
  ...
  after(async () => {
    ...
    await utils.deleteIModelByName(accessToken, assetId, createimodelName, false);
    await utils.deleteIModelByName(accessToken, assetId, updatedimodelName, false);
    await utils.deleteIModelByName(accessToken, assetId, imodelNameWithSpecialChars, false);
  });
  ...
  it("should get list of IModels (#iModelBank)", async () => {
    ...
    const imodels: HubIModel[] = await imodelClient.iModels.get(accessToken, projectId, undefined);
  ...
  it("should get a specific IModel (#iModelBank)", async () => {
    ...
    const iModel: HubIModel = (await imodelClient.iModels.get(accessToken, projectId, new IModelQuery().byName(imodelName)))[0];
  ...
  it("should be able to delete iModels (#unit)", async () => {
    ...
    for (const name of names) {
      ...
      const iModel: HubIModel = (await imodelClient.iModels.get(accessToken, projectId, new IModelQuery().byName(name)))[0];
      chai.expect(iModel.name).to.be.equal(name);
      mockDeleteiModel(projectId, iModel.id!);
      await imodelClient.iModels.delete(accessToken, projectId, iModel.id!);
    }
  });
  ...
  it("should retrieve an iModel by its id (#iModelBank)", async () => {
    ...
    const iModel: HubIModel = (await imodelClient.iModels.get(accessToken, projectId, new IModelQuery().byId(imodelId)))[0];
  ...
  it("should fail getting an invalid iModel (#iModelBank)", async () => {
    ...
    try {
      await imodelClient.iModels.get(accessToken, projectId, new IModelQuery().byId(mockGuid));
    } catch (err) {
  ...
  it("should fail creating existing and initialized iModel (#iModelBank)", async () => {
    ...
    try {
      await imodelClient.iModels.create(accessToken, projectId, imodelName, { path: utils.getMockSeedFilePath(), description: "" });
    } catch (err) {
  ...
  it("should create iModel and upload SeedFile", async () => {
    ...
    const iModel: HubIModel = await imodelClient.iModels.create(accessToken, projectId, createimodelName,
  ...
  it("should continue creating not initialized iModel (#unit)", async () => {
    ...
    const iModel = await imodelClient.iModels.create(accessToken, projectId, imodelName, { path: filePath, description: "" });
  ...
  it("should fail creating an iModel with no file handler", async () => {
    ...
    const invalidClient = new IModelHubClient();
    try {
      await invalidClient.iModels.create(accessToken, projectId, createimodelName, { path: workDir });
    } catch (err) {
      if (err instanceof IModelHubClientError)
        error = err;
    }
  ...
  it("should fail creating an iModel with no file (#iModelBank)", async () => {
    let error: IModelHubClientError | undefined;
    try {
      await iModelClient.iModels.create(accessToken, projectId, createimodelName, { path: `${workDir}InvalidiModel.bim` });
    } catch (err) {
      if (err instanceof IModelHubClientError)
        error = err;
    }
  ...
  it("should fail creating an iModel with directory path (#iModelBank)", async () => {
    let error: IModelHubClientError | undefined;
    try {
      await iModelClient.iModels.create(accessToken, projectId, createimodelName, { path: workDir });
    } catch (err) {
      if (err instanceof IModelHubClientError)
        error = err;
    }
  ...
  it("should fail creating an iModel with invalid size of extent (#iModelBank)", async () => {
    let error: IModelHubClientError | undefined;
    try {
      await iModelClient.iModels.create(accessToken, projectId, createimodelName, { extent: [1] });
    } catch (err) {
      if (err instanceof IModelHubClientError)
        error = err;
    }
  ...
  it("should fail creating an iModel with invalid coordinate of extent (#iModelBank)", async () => {
    let error: IModelHubClientError | undefined;
    try {
      await iModelClient.iModels.create(accessToken, projectId, createimodelName, { extent: [1, -200, 3, 4] });
    } catch (err) {
      if (err instanceof IModelHubClientError)
        error = err;
    }
  ...
  it("should create iModel from another iModel and ChangeSet (#unit)", async () => {
    ...
    const imodel = await iModelClient.iModels.create(accessToken, projectId, createimodelName, { template: { imodelId, changeSetId: changeSet.id } });
    chai.assert(imodel);
    chai.assert(imodel.initialized);
  ...
  it("should update iModel name and description (#iModelBank)", async () => {
    ...
    await utils.deleteIModelByName(accessToken, projectId, updatedimodelName);
    imodel.name = updatedimodelName;
    imodel.description = newDescription;
    mockUpdateiModel(projectId, imodel);
    let updatediModel = await iModelClient.iModels.update(accessToken, projectId, imodel);
    ...
    updatediModel = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byName(updatedimodelName)))[0];
    ...
    mockUpdateiModel(projectId, imodel);
    await iModelClient.iModels.update(accessToken, projectId, imodel);
  });
  ...
  it("should set and update iModel type correctly", async () => {
    ...
    mockUpdateiModel(projectId, imodel);
    const updatediModel = await iModelClient.iModels.update(accessToken, projectId, imodel);
    ...
    await iModelClient.iModels.update(accessToken, projectId, updatediModel);

    mockGetIModelByName(projectId, imodelName, updatediModel.description, updatediModel.id, true, IModelType.Library);
    const queriediModel = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byName(imodelName)))[0];
  ...
  it("should get oldest IModel (#iModelBank)", async () => {
    ...
    const imodel = await imodelClient.iModel.get(accessToken, projectId);
    chai.assert(imodel);
  });
  ...
  it("should throw if no IModels returned (#iModelBank)", async () => {
    ...
    let error: IModelHubError | undefined;
    try {
      await imodelClient.iModel.get(accessToken, projectId);
    } catch (err) {
      if (err instanceof IModelHubError)
        error = err;
    }
  ...
  it("should be able to delete iModel (#unit)", async () => {
    ...
    await imodelClient.iModel.delete(accessToken, projectId);
  });
  ...
  it("should return initialization status (#unit)", async () => {
    ...
    const initializationState = await imodelClient.iModel.getInitializationState(accessToken, projectId);
  ...
  it("should create iModel if iModel does not exist (#unit)", async () => {
    ...
    const progressTracker = new utils.ProgressTracker();
    const iModel = await imodelClient.iModel.create(accessToken, projectId, createimodelName, { path: filePath, description, progressCallback: progressTracker.track() });
  ...
  it("should throw iModelAlreadyExists if iModel already exist (#iModelBank)", async () => {
    ...
    let error: IModelHubError | undefined;
    try {
      await imodelClient.iModel.create(accessToken, projectId, createimodelName, { path: filePath, description, progressCallback: progressTracker.track() });
    } catch (err) {
      if (err instanceof IModelHubError)
        error = err;
    }
  ...
  it("should create iModel from empty seed file (#iModelBank)", async () => {
    ...
    const progressTracker = new utils.ProgressTracker();
    const imodel = await imodelClient.iModels.create(accessToken, projectId, createimodelName, { description, progressCallback: progressTracker.track() });
    ...
    const getiModel = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byName(createimodelName)))[0];
  ...
  it("should create single iModel from empty seed file (#unit)", async () => {
    ...
    mockCreateEmptyiModel(projectId, Guid.createValue(), createimodelName, description);
    const progressTracker = new utils.ProgressTracker();
    const imodel = await imodelClient.iModel.create(accessToken, projectId, createimodelName, { description, progressCallback: progressTracker.track() });
  ...
  it("should update iModel extents (#iModelBank)", async () => {
    ...
    updatediModel = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byName(newName)))[0];
  ...
  it("should download a Seed File if iModel exist (#iModelBank #unit)", async () => {
    ...
    const progressTracker = new utils.ProgressTracker();
    await iModelClient.iModel.download(accessToken, projectId, downloadToPathname, progressTracker.track());
    progressTracker.check();
  ...
  it("should create an iModel in the Asset (#iModelBank)", async () => {
    ...
    const progressTracker = new utils.ProgressTracker();
    const imodel = await imodelClient.iModels.create(accessToken, assetId, createimodelName, { description, progressCallback: progressTracker.track() });
    ...
    const getiModel = (await iModelClient.iModels.get(accessToken, assetId, new IModelQuery().byName(createimodelName)))[0];
    chai.assert(!!getiModel);
    chai.expect(getiModel.wsgId).to.be.equal(imodel.id!);
  });
  ...
  it("should create iModel with an extent from empty seed file (#iModelBank)", async () => {
    ...
    const progressTracker = new utils.ProgressTracker();
    const imodel = await imodelClient.iModels.create(accessToken, projectId, createimodelName, { description, progressCallback: progressTracker.track(), extent });
  ...
  it("should filter iModels by type", async () => {
    mockGetIModelByName(projectId, imodelName);
    let imodel: HubIModel = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byName(imodelName)))[0];

    imodel.iModelType = IModelType.Undefined;
    mockUpdateiModel(projectId, imodel);
    imodel = await iModelClient.iModels.update(accessToken, projectId, imodel);
    chai.expect(imodel.iModelType).to.be.equal(IModelType.Undefined);

    mockGetIModelByType(projectId, imodelName, imodelId, IModelType.Undefined);
    const iModelsWithUndefinedType = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byiModelType(IModelType.Undefined)));
    ...
    let iModelsWithLibraryType = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byiModelType(IModelType.Library)));
    ...
    iModelsWithLibraryType = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byiModelType(IModelType.Library)));
  ...
  it("should filter iModels by template", async () => {
    ...
    const iModelsWithEmptyTemplate = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byiModelTemplate(emptyTemplate)));
    ...
    mockGetSeedFile(imodelId);
    const clonediModel: HubIModel = await iModelClient.iModels.create(accessToken, projectId, createimodelName,
      { template: { imodelId }, timeOutInMilliseconds: TestConfig.initializeiModelTimeout });

    mockGetIModelByTemplate(projectId, createimodelName, imodelId, customTemplate);
    const iModelsWithCustomTemplate = (await iModelClient.iModels.get(accessToken, projectId, new IModelQuery().byiModelTemplate(customTemplate)));
  ...
  it("should handle special characters in get by name query", async () => {
    ...
    const iModels = await imodelClient.iModels.get(accessToken, projectId, new IModelQuery().byName(imodelNameWithSpecialChars));
