...
function orderTest(socket: {
  handle(
    channel: string,
    listener: (event: any, ...args: any[]) => Promise<any>
  ): void;
}) {
  socket.handle(
    "a",
    async (_event: Event, methodName: string, ..._args: any[]) => {
      return [methodName, "a"];
    }
  );
...
export async function setupIpcTest(
  ...
  registerBackendCallback(BackendTestCallbacks.startIpcTest, () => {
    setTimeout(async () => {
      ...
      socket.handle(
        "testinvoke",
        async (_event: Event, methodName: string, ...args: any[]) => {
      ...
      socket.handle(
        iTwinChannel("ipc-app"),
        async (_event: Event, _methodName: string, ..._args: any[]) => {
          return { result: undefined };
        }
      );

      socket.handle(
        iTwinChannel("nativeApp"),
        async (_event: Event, methodName: string, ..._args: any[]) => {
