    ...
    it("should return 400 response for requests with invalid URL parameters", async () => {
      ...
      const testParamsWithSuffix = async (suffix: string) => {
        ...
        const response = await fetch(
          `${protocol.pathPrefix}/${protocol.info.title}/${protocol.info.version}/mode/1/context/undefined/imodel/undefined/changeset/0/${TestRpcInterface.interfaceName}-${TestRpcInterface.interfaceVersion}-op1?parameters=${
            encodeURIComponent(realParams)
          }${suffix}`,
          {
            method: "GET",
            headers: {
