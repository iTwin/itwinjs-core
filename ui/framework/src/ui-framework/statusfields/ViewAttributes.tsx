...
export class ViewAttributesStatusField extends React.Component<StatusFieldProps, ViewAttributesStatusFieldState> {
  ...
  private getViewFlagItem(flagName: string, value: boolean, labelKey?: string) {
    return <Checkbox key={flagName} label={labelKey ? IModelApp.localizationClient.getLocalizedString(labelKey) : /* istanbul ignore next */ this.stylizeName(flagName)} onClick={() => this._handleViewFlagClick(flagName)} defaultChecked={value} />;
  }
  ...
  private getToggleCameraItem() {
    return <Checkbox key={"toggleCamera"} label={IModelApp.localizationClient.getLocalizedString("UiFramework:listTools.camera")} onClick={this._handleToggleCamera} defaultChecked={this.state.cameraOn} />;
  }
