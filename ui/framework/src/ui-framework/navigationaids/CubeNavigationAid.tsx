...
export class CubeNavigationAid extends React.Component<CubeNavigationAidProps, CubeNavigationAidState> {
  ...
  public componentDidMount() {
    ...
    if (activeContentControl && activeContentControl.viewport) {
      const newMatrix = activeContentControl.viewport.view.getRotation();
  ...
  private _handleViewRotationChangeEvent = (args: ViewRotationChangeEventArgs) => {
    ...
    if (activeContentControl && activeContentControl.isViewport && activeContentControl.viewport === args.viewport && animation >= 1 && !dragging) {
      ...
      const newMatrix = activeContentControl.viewport.view.getRotation();
      if (!matrix.isAlmostEqual(newMatrix)) {
        this.setState({ startRotMatrix: matrix, endRotMatrix: newMatrix, animation: 1, face: Face.None });
      }
    }
  }
  ...
  private _onArrowClick = (arrow: Pointer) => {
    ...
    this._animateRotation(this.state.endRotMatrix, newRotation, face);
  ...
  private _handleBoxClick = (event: any) => {
    ...
    // if (this.state.animation !== 1) {
    //   endRotMatrix = CubeNavigationAid._interpolateRotMatrix(this.state.startRotMatrix, this.state.animation, this.state.endRotMatrix);
    // }

    this._start = Vector2d.create(event.clientX, event.clientY);

    this.setState({ startRotMatrix: endRotMatrix, endRotMatrix });
  }

  private _onMouseDrag = (event: any) => {
    const mousePos = Point2d.create(event.clientX, event.clientY);
    if (!this._start.isAlmostEqual(mousePos)) {
      const diff = this._start.minus(mousePos).scale(0.05);
      ...
      this._setRotation(mat, CubeNavigationAid._getMatrixFace(mat), this.state.startRotMatrix);
  ...
  private _handleFaceCellClick = (pos: Vector3d, face: Face) => {
    ...
    if (rotMatrix) {
      ...
      this._animateRotation(endRotMatrix, rotMatrix, face);
    }
    window.removeEventListener("mousemove", this._onMouseDrag);
  }

  private _animateRotation = (startRotMatrix: Matrix3d, endRotMatrix: Matrix3d, face: Face) => {
    if (startRotMatrix.isAlmostEqual(endRotMatrix))
      return;
    // set animation variables, let css transitions animate it.
    ViewportComponentEvents.setCubeMatrix(startRotMatrix, Face.None, 0);
    this._animationFrame = setTimeout(this._animation, 16.667);
    this.setState({
  ...
  private _setRotation = (endRotMatrix: Matrix3d, face: Face, startRotMatrix?: Matrix3d) => {
    ViewportComponentEvents.setCubeMatrix(endRotMatrix, face, 0);
    // set variables, with animTime at 0 to prevent animation.
    this.setState({
      startRotMatrix: startRotMatrix || /* istanbul ignore next */ endRotMatrix,
      endRotMatrix,
      animation: 1,
      face,
    });
...
export class FaceCell extends React.Component<FaceCellProps> {
  ...
  private _handleMouseUp = (event: React.MouseEvent) => {
    ...
    if (this._startMouse && this._startMouse.isAlmostEqual(mouse)) {
      const isFace = Math.abs(vector.x + vector.y + vector.z) === 1;
