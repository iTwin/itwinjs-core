...
describe("StagePanel", () => {
  ...
  it("should render StagePanelTarget", () => {
    ...
    const draggedWidget = moq.Mock.ofType<NonNullable<typeof zones.object["draggedWidget"]>>();
    runtimeProps.setup((x) => x.panel).returns(() => panel.object);
    runtimeProps.setup((x) => x.panelDef).returns(() => panelDef.object);
    ...
    zones.setup((x) => x.draggedWidget).returns(() => draggedWidget.object);
    draggedWidget.setup((x) => x.id).returns(() => 6);
    shallow(<StagePanel
      runtimeProps={runtimeProps.object}
      allowedZones={[6]}
