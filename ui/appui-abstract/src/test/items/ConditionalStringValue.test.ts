...
import { ConditionalStringValue } from "../../appui-abstract";
...
describe("ConditionalStringValue", () => {
  ...
  it("should construct with initial value", () => {
    const sut = new ConditionalStringValue(
      helloFunc,
      syncEventIds,
      defaultValue
    );
  ...
  it("test static getValue method", () => {
    ...
    expect(ConditionalStringValue.getValue(goodByeValue)).to.be.equal(
      goodByeValue
    );
    expect(
      ConditionalStringValue.getValue(
        new ConditionalStringValue(helloFunc, syncEventIds, defaultValue)
      )
    ).to.be.equal(defaultValue);
    expect(
      ConditionalStringValue.getValue(
        new ConditionalStringValue(goodbyeFunc, syncEventIds, defaultValue)
      )
    ).to.be.equal(defaultValue);
    expect(
      ConditionalStringValue.getValue(
        new ConditionalStringValue(helloFunc, syncEventIds)
      )
    ).to.be.equal(helloValue);
    expect(
      ConditionalStringValue.getValue(
        new ConditionalStringValue(goodbyeFunc, syncEventIds)
      )
    ).to.be.equal(goodByeValue);
  });
  ...
  it("test static refreshValue method", () => {
    ...
    expect(sut.value).to.be.equal(defaultValue);
    expect(ConditionalStringValue.refreshValue(sut, new Set<string>(["cat"])))
      .to.be.false;
    expect(sut.value).to.be.equal(defaultValue);
    expect(
      ConditionalStringValue.refreshValue(sut, new Set<string>(["sync-id-two"]))
    ).to.be.true;
    expect(sut.value).to.be.equal(helloValue);
    expect(
      ConditionalStringValue.refreshValue(undefined, new Set<string>(["cat"]))
    ).to.be.false;
  });
  ...
  it("test static refreshValue method with capitalized ids", () => {
    ...
    expect(sut.value).to.be.equal(defaultValue);
    expect(
      ConditionalStringValue.refreshValue(
        sut,
        new Set<string>(["sync-id-three"])
      )
    ).to.be.true;
    expect(sut.value).to.be.equal(helloValue);
    expect(
