...
describe("ConditionalStringValue", () => {
  ...
  it("test static getValue method", () => {
    ...
    expect(
      ConditionalStringValue.getValue(new ConditionalStringValue(helloFunc, syncEventIds, defaultValue))
    ).to.be.equal(defaultValue);
    expect(
      ConditionalStringValue.getValue(new ConditionalStringValue(goodbyeFunc, syncEventIds, defaultValue))
    ).to.be.equal(defaultValue);
    expect(ConditionalStringValue.getValue(new ConditionalStringValue(helloFunc, syncEventIds))).to.be.equal(
      helloValue
    );
    expect(ConditionalStringValue.getValue(new ConditionalStringValue(goodbyeFunc, syncEventIds))).to.be.equal(
      goodByeValue
    );
  });
