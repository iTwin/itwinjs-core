...
import { ConditionalBooleanValue } from "../../appui-abstract";
...
describe("ConditionalBooleanValue", () => {
  ...
  it("test static getValue method", () => {
    ...
    expect(
      ConditionalBooleanValue.getValue(
        new ConditionalBooleanValue(trueFunc, syncEventIds, false)
      )
    ).to.be.false;
    expect(
      ConditionalBooleanValue.getValue(
        new ConditionalBooleanValue(trueFunc, syncEventIds, true)
      )
    ).to.be.true;
    expect(
      ConditionalBooleanValue.getValue(
        new ConditionalBooleanValue(trueFunc, syncEventIds)
      )
    ).to.be.true;
    expect(
      ConditionalBooleanValue.getValue(
        new ConditionalBooleanValue(falseFunc, syncEventIds)
      )
    ).to.be.false;
  });
  ...
  it("test static refreshValue method", () => {
    const sut = new ConditionalBooleanValue(trueFunc, syncEventIds, false);
    expect(sut.value).to.be.false;
    expect(ConditionalBooleanValue.refreshValue(sut, new Set<string>(["cat"])))
      .to.be.false;
    expect(sut.value).to.be.false;
    expect(
      ConditionalBooleanValue.refreshValue(
        sut,
        new Set<string>(["sync-id-two"])
      )
    ).to.be.true;
    expect(sut.value).to.be.true;
    expect(
      ConditionalBooleanValue.refreshValue(undefined, new Set<string>(["cat"]))
    ).to.be.false;
  });
