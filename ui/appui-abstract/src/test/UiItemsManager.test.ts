...
class TestUiItemsProvider implements UiItemsProvider {
  public get id(): string {
    return "unitTest:TestUiItemsProvider";
  }
...
describe("UiItemsManager", () => {
  ...
  it("if no registered providers no tools are available", () => {
    const toolSpecs = UiItemsManager.getToolbarButtonItems(
      "stage",
      testStageUsage,
  ...
  it("Should return provider specific items per stage ", () => {
    ...
    UiItemsManager.register(testUiProvider, { stageIds: ["stage"] });
    UiItemsManager.register(testUiProvider, {
      providerId: "stage2:testProvider",
      stageIds: ["stage2"],
    });
    const toolSpecs = UiItemsManager.getToolbarButtonItems(
      "stage",
    ...
    const toolSpecs2 = UiItemsManager.getToolbarButtonItems(
      "stage2",
      testStageUsage,
      ToolbarUsage.ContentManipulation,
      ToolbarOrientation.Horizontal
    ...
    const statusbarItems2 = UiItemsManager.getStatusBarItems(
      "stage2",
      testStageUsage
    );
    ...
    const widgets2 = UiItemsManager.getWidgets(
      "stage2",
      testStageUsage,
      StagePanelLocation.Right
    );
  ...
  it("Should return provider specific items per stage and avoid duplicate", () => {
    ...
    UiItemsManager.register(testUiProvider, { stageIds: ["stage"] });
    UiItemsManager.register(testUiProvider, {
      providerId: "stage:testProvider",
      stageIds: ["stage"],
    }); // alternate provider but wrongly targets same stage
    const toolSpecs = UiItemsManager.getToolbarButtonItems(
      "stage",
      testStageUsage,
      ToolbarUsage.ContentManipulation,
