...
describe("BackstageItemsManager", () => {
  ...
  describe("uisync", () => {
    it("uisync", () => {
      ...
      const subTitleConditional = new ConditionalStringValue(
        () => (isVisible ? "default subtitle" : "new subtitle"),
        [syncId]
      );
      ...
      expect(ConditionalBooleanValue.getValue(actionItem!.isHidden)).to.be
        .false;
      expect(ConditionalStringValue.getValue(actionItem!.label)).to.be.equal(
        "Hello"
      );
      expect(ConditionalStringValue.getValue(actionItem!.icon)).to.be.equal(
        "icon-hand-2"
      );
      expect(ConditionalStringValue.getValue(actionItem!.subtitle)).to.be.equal(
        "default subtitle"
      );

      let stageItem = sut.items.find((i) => i.id === "stageId-C");
      expect(ConditionalBooleanValue.getValue(actionItem!.isDisabled)).to.be
        .false;
      expect(ConditionalBooleanValue.getValue(actionItem!.isHidden)).to.be
        .false;
      expect(ConditionalStringValue.getValue(stageItem!.label)).to.be.equal(
        "Hello"
      );
      expect(ConditionalStringValue.getValue(stageItem!.icon)).to.be.equal(
        "icon-hand-2"
      );
      expect(ConditionalStringValue.getValue(stageItem!.subtitle)).to.be.equal(
        "default subtitle"
      );

      let ncActionItem = sut.items.find((i) => i.id === "Action-NC");
      expect(ConditionalBooleanValue.getValue(ncActionItem!.isHidden)).to.be
        .false;
      let ncStageItem = sut.items.find((i) => i.id === "stageId-NC");
      expect(ConditionalBooleanValue.getValue(ncStageItem!.isDisabled)).to.be
        .false;
      ...
      expect(ConditionalBooleanValue.getValue(stageItem!.isDisabled)).to.be
        .true;
    ...
    it("should convert SyncEventIds to lowercase", () => {
      ...
      const action = BackstageItemUtilities.createActionItem(
        "TestAction",
        100,
        50,
        () => {},
