...
import { IModelApp, Plugin, Tool, Marker, ScreenViewport, Cluster, MarkerSet, DecorateContext, imageElementFromUrl, ViewState3d, BeButton, BeButtonEvent, InputSource, NotifyMessageDetails, OutputMessagePriority } from "@bentley/imodeljs-frontend";
...
class GeoNameMarker extends Marker {
  ...
  constructor(location: XYAndZ, public props: GeoNameProps, icon: HTMLImageElement) {
    ...
    if (props.population)
      this.title = this.title + " (" + GeoNamePlugin.plugin!.i18n.translate("geoNames:misc.Population") + ": " + props.population + ")";
...
export class GeoNameMarkerManager {
  ...
  private outputInfoMessage(messageKey: string) {
    const message: string = GeoNamePlugin.plugin!.i18n.translate("geoNames:messages." + messageKey);
  ...
  public static async show(vp: ScreenViewport): Promise<void> {
    if (undefined === GeoNameMarkerManager.decorator) {
      const cityMarkerImage = await this.loadImage(GeoNamePlugin.plugin!.resolveResourceUrl("city.ico"));
...
IModelApp.pluginAdmin.register(new GeoNamePlugin(PLUGIN_NAME));
