...
import { IModelApp, Plugin, PrimitiveTool, CoordinateLockOverrides, BeButtonEvent, HitDetail, EventHandled, LocateResponse, ManipulatorToolEvent, BeTouchEvent, LocateFilterStatus, Tool } from "@bentley/imodeljs-frontend";
 ...
 * @beta
 */
export class ProjectLocationPlugin extends Plugin {
  private _i18NNamespace?: I18NNamespace;
  public static plugin: ProjectLocationPlugin | undefined;
  ...
  public onLoad(_args: string[]): void {
    ProjectLocationPlugin.plugin = this; // store the plugin.
  ...
  public onExecute(args: string[]): void {
    ...
    // NOTE: For easier testing. If this plugin starts including a frontstage, that should specify the default tool to use instead of being done here...
  ...
  private establishDefaultTool(enable: boolean): void {
    if (!ProjectLocationPlugin._changeDefaultTool)
      return;
    ...
    if (enable) {
      ...
      ProjectLocationPlugin._saveDefaultToolId = IModelApp.toolAdmin.defaultToolId;
      ...
      IModelApp.toolAdmin.defaultToolId = ProjectLocationPlugin._saveDefaultToolId;
      ProjectLocationPlugin._saveDefaultToolId = "";
    }
  }
}
...
IModelApp.pluginAdmin.register(new ProjectLocationPlugin(PLUGIN_NAME));
