...
async function processWorkspace<W extends ITwinWorkspaceDb, T extends WorkspaceDbOpt>(
  args: T,
  ws: W,
...
function getCloudContainer(args: EditorOpts): CloudSqlite.CloudContainer {
  ...
  if (!cloudContainer || !cloudContainer.isConnected) throw new Error("no cloud container");
...
async function listWorkspaceDb(args: ListOptions) {
  await readWorkspace(args, async (file, args) => {
    ...
    if (!args.strings && !args.blobs && !args.files) args.blobs = args.files = args.strings = true;

    const nameAndSize = (stmt: SqliteStatement, size?: number, info?: string) =>
      showMessage(
        `  name=${stmt.getValueString(0)}, size=${friendlyFileSize(size ?? stmt.getValueInteger(1))}${info ?? ""}`
      );
...
async function addResource(args: AddFileOptions) {
  return editWorkspace(args, async (wsFile, args) => {
    glob.sync(args.files, { cwd: args.root ?? process.cwd(), nodir: true }).forEach((filePath) => {
      ...
      if (!IModelJsFs.existsSync(file)) throw new Error(`file [${file}] does not exist`);
...
async function extractResource(args: ExtractResourceOpts) {
  return readWorkspace(args, async (file, args) => {
    const verify = <T>(val: T | undefined): T => {
      if (val === undefined) throw new Error(` ${args.type} resource "${args.rscName}" does not exist`);
      return val;
    };
...
async function performTransfer(
  ...
  const info = `${direction === "download" ? "export" : "import"} ${localFileName}, container=${
    args.containerId
  }, dbName=${args.dbFileName} : `;
...
async function importWorkspaceDb(args: UploadOptions) {
  ...
  if ("" === extname(args.localFileName)) args.localFileName = `${args.localFileName}.${ITwinWorkspaceDb.fileExt}`;
...
async function initializeWorkspace(args: InitializeOpts) {
  if (undefined === args.storageType || !args.accessName) throw new Error("No cloud container supplied");
  if (!args.noPrompt) {
...
async function preFetchWorkspaceDb(args: WorkspaceDbOpt) {
  ...
  showMessage(
    `preFetched WorkspaceDb [${args.dbFileName}] in ${sayContainer(args)}, time=${timer.elapsedSeconds.toString()}`
  );
}
...
async function acquireLock(args: WorkspaceDbOpt) {
  const container = getCloudContainer(args);
  if (container.hasWriteLock) throw new Error(`write lock is already held for ${sayContainer(args)}`);
...
async function releaseLock(args: WorkspaceDbOpt) {
  const container = getCloudContainer(args);
  if (!container.hasWriteLock) throw new Error(`write lock is not held for ${sayContainer(args)}`);
...
async function queryWorkspaceDbs(args: WorkspaceDbOpt) {
  ...
  for (const dbName of dbs) {
    ...
    if (db) {
      ...
      showMessage(
        ` "${dbName}", size=${friendlyFileSize(db.totalBlocks * blockSize)}, ${friendlyFileSize(
          db.localBlocks * blockSize
        )} downloaded (${((100 * db.localBlocks) / db.totalBlocks).toFixed(0)}%)${editable}${dirty}`
      );
    }
  }
}
...
const addOrReplace = {
  ...
  root: {
    alias: "r",
    describe: "Root directory. Path parts after this will be saved in resource name",
    string: true,
  },
  type,
};
...
Yargs.options({
  ...
  accessToken: {
    describe: "Token that grants access to the container (either SAS or account key)",
    string: true,
    default: "",
  },
  storageType: {
...
async function main() {
  if (process.argv.length > 1 && process.argv[2]?.[0] === "@") {
    ...
    if (parsed.config) process.env.WORKSPACE_EDITOR_CONFIG = parsed.config as string;
