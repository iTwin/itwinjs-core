...
type TransferOptions = WorkspaceDbOpt &
  CloudSqlite.TransferDbProps & {
    noVacuum?: boolean;
  };
...
function doVacuum(dbName: string, container?: CloudSqlite.CloudContainer) {
  ...
  db.withOpenDb({ dbName, openMode: OpenMode.ReadWrite, container }, () =>
    db.vacuum()
  );
...
function friendlyFileSize(size: number) {
  if (size < 1024) return `${size}`; // less than 1K, just show number of bytes with no units

  const units = Math.floor(Math.log(size) / Math.log(1024));
  return `${Math.round(size / Math.pow(1024, units))}${
    ["", "K", "M", "G", "T"][units]
  }`;
}
...
async function createWorkspaceDb(args: WorkspaceDbOpt) {
  args.writeable = true;
  const wsFile = new EditableWorkspaceDb(
    args,
    IModelHost.appWorkspace.getContainer(args, args)
  );
...
async function processWorkspace<
  W extends ITwinWorkspaceDb,
  T extends WorkspaceDbOpt
>(args: T, ws: W, fn: (ws: W, args: T) => Promise<void>) {
  ws.open();
...
function fixVersionArg(args: WorkspaceDbOpt) {
  ...
  args.dbFileName = ITwinWorkspaceContainer.makeDbFileName(
    dbParts.dbName,
    dbParts.version
  );
}
...
async function editWorkspace<T extends WorkspaceDbOpt>(
  args: T,
  fn: (ws: EditableWorkspaceDb, args: T) => Promise<void>
) {
  fixVersionArg(args);
  ...
  if (
    cloudContainer &&
    cloudContainer.queryDatabase(ws.dbFileName)?.state !== "copied"
  )
    throw new Error(
      `${args.dbFileName} is not editable. Create a new version first`
    );
...
async function readWorkspace<T extends WorkspaceDbOpt>(
  args: T,
  fn: (ws: ITwinWorkspaceDb, args: T) => Promise<void>
) {
  fixVersionArg(args);
  return processWorkspace(
    args,
    new ITwinWorkspaceDb(args, getContainer(args)),
    fn
  );
}
...
async function listWorkspaceDb(args: ListOptions) {
  await readWorkspace(args, async (file, args) => {
    ...
    const nameAndSize = (stmt: SqliteStatement, size?: number, info?: string) =>
      showMessage(
        `  name=${stmt.getValueString(0)}, size=${friendlyFileSize(
          size ?? stmt.getValueInteger(1)
        )}${info ?? ""}`
      );

    if (args.strings) {
      showMessage(" strings:");
      file.sqliteDb.withSqliteStatement(
        "SELECT id,LENGTH(value) FROM strings ORDER BY id COLLATE NOCASE",
        (stmt) => {
          while (DbResult.BE_SQLITE_ROW === stmt.step()) nameAndSize(stmt);
        }
      );
    }
    ...
    if (args.blobs) {
      showMessage(" blobs:");
      file.sqliteDb.withSqliteStatement(
        "SELECT id,LENGTH(value) FROM blobs ORDER BY id COLLATE NOCASE",
        (stmt) => {
          while (DbResult.BE_SQLITE_ROW === stmt.step()) nameAndSize(stmt);
        }
      );
    ...
    if (args.files) {
      showMessage(" files:");
      file.sqliteDb.withSqliteStatement(
        "SELECT name FROM be_EmbedFile ORDER BY name COLLATE NOCASE",
        (stmt) => {
          while (DbResult.BE_SQLITE_ROW === stmt.step()) {
            ...
            if (embed) {
              ...
              nameAndSize(
                stmt,
                info.size,
                `, ext="${info.fileExt}", date=${date.toString()}`
              );
            }
          }
        }
    ...
    if (prefetch) {
      ...
      showMessage(
        `prefetch time = ${timer.elapsedSeconds.toString()}, done=${done}`
      );
    }
  });
}
...
async function addResource(args: AddFileOptions) {
  return editWorkspace(args, async (wsFile, args) => {
    glob
      .sync(args.files, { cwd: args.root ?? process.cwd(), nodir: true })
      .forEach((filePath) => {
...
async function extractResource(args: ExtractResourceOpts) {
  return readWorkspace(args, async (file, args) => {
    const verify = <T>(val: T | undefined): T => {
      if (val === undefined)
        throw new Error(
          ` ${args.type} resource "${args.rscName}" does not exist`
        );
      return val;
    };
    ...
    showMessage(
      ` ${args.type} resource [${args.rscName}] extracted to "${args.fileName}"`
    );
  });
}
...
async function removeResource(args: RemoveResourceOpts) {
  return editWorkspace(args, async (wsFile, args) => {
    if (args.type === "string") wsFile.removeString(args.rscName);
...
async function performTransfer(
  ...
  direction: CloudSqlite.TransferDirection,
  args: TransferOptions
) {
  fixVersionArg(args);
  const localFileName = args.localFileName;

  if (direction === "upload" && !args.noVacuum) doVacuum(localFileName);

  const info = `${
    direction === "download" ? "export" : "import"
  } ${localFileName}, container=${args.containerId}, dbName=${
    args.dbFileName
  } : `;
  ...
  await CloudSqlite.transferDb(direction, container, {
    ...args,
    localFileName,
    onProgress,
  });
...
async function importWorkspaceDb(args: UploadOptions) {
  ...
  if (!IModelJsFs.existsSync(args.localFileName))
    args.localFileName = join(
      args.directory ?? IModelHost.appWorkspace.containerDir,
      args.localFileName
    );
...
async function exportWorkspaceDb(args: TransferOptions) {
  ...
  const dbParts = ITwinWorkspaceContainer.parseDbFileName(args.dbName);
  if (!dbParts.version) throw new Error("exportDb requires a version");
...
async function deleteWorkspaceDb(args: WorkspaceDbOpt) {
  ...
  showMessage(
    `deleted WorkspaceDb [${args.dbName}] from ${sayContainer(args)}`
  );
}
...
async function initializeWorkspace(args: InitializeOpts) {
  ...
  if (!args.noPrompt) {
    const yesNo = await askQuestion(
      `Are you sure you want to initialize ${sayContainer(args)}"? [y/n]: `
    );
    if (yesNo[0].toUpperCase() !== "Y") return;
  }
  const container = CloudSqlite.createCloudContainer(
    args as CloudSqlite.ContainerAccessProps
  );
...
async function purgeWorkspace(args: EditorOpts) {
  ...
  await CloudSqlite.withWriteLock(args.user, container, async () =>
    container.cleanDeletedBlocks()
  );
  container.checkForChanges(); // re-read manifest to get current garbage count
  showMessage(
    `purged ${sayContainer(args)}. ${
      nGarbage - container.garbageBlocks
    } garbage blocks cleaned`
  );
}
...
async function copyWorkspaceDb(args: CopyWorkspaceDbOpt) {
  ...
  const newName = ITwinWorkspaceContainer.makeDbFileName(
    newVersion.dbName,
    ITwinWorkspaceContainer.validateVersion(newVersion.version)
  );

  await CloudSqlite.withWriteLock(args.user, container, async () =>
    container.copyDatabase(oldName, newName)
  );
  showMessage(
    `copied WorkspaceDb [${oldName}] to [${newName}] in ${sayContainer(args)}`
  );
}
...
async function versionWorkspaceDb(args: MakeVersionOpt) {
  fixVersionArg(args);
  const container = getCloudContainer(args);
  const result = await ITwinWorkspaceContainer.makeNewVersion(
    container,
    args,
    args.versionType
  );
  showMessage(
    `created new version: [${result.newName}] from [${
      result.oldName
    }] in ${sayContainer(args)}`
  );
}
...
async function preFetchWorkspaceDb(args: WorkspaceDbOpt) {
  ...
  showMessage(
    `preFetched WorkspaceDb [${args.dbFileName}] in ${sayContainer(
      args
    )}, time=${timer.elapsedSeconds.toString()}`
  );
}
...
async function queryWorkspaceDbs(args: WorkspaceDbOpt) {
  ...
  const nGarbage = container.garbageBlocks;
  const garbageMsg = nGarbage
    ? `, ${nGarbage} garbage block${nGarbage > 1 ? "s" : ""}`
    : "";
  const blockSize = container.blockSize;
  showMessage(
    `WorkspaceDbs in ${sayContainer(
      args
    )}${writeLockMsg}${hasLocalMsg}${garbageMsg}`
  );
  ...
  for (const dbName of dbs) {
    ...
    if (db) {
      ...
      showMessage(
        ` "${dbName}", size=${friendlyFileSize(
          db.totalBlocks * blockSize
        )}, ${friendlyFileSize(db.localBlocks * blockSize)} downloaded (${(
          (100 * db.localBlocks) /
          db.totalBlocks
        ).toFixed(0)}%)${editable}${dirty}`
      );
    }
  }
}
    ...
    try {
      ...
      if (true === args.logging) {
        ...
        IModelHost.appWorkspace
          .getCloudCache()
          .setLogMask(CloudSqlite.LoggingMask.All);
        logTimer = setInterval(() => flushLog(), 250); // logging from other threads is buffered. This causes it to appear every 1/4 second.
      }

      await cmd(args);
    } catch (e: any) {
      if (typeof e.message === "string") console.error(e.message);
      else console.log(BentleyError.getErrorMessage(e));
    } finally {
      if (logTimer) {
        flushLog();
...
const addOrReplace = {
  ...
  root: {
    alias: "r",
    describe:
      "Root directory. Path parts after this will be saved in resource name",
    string: true,
  },
  type,
};
...
Yargs.options({
  directory: {
    alias: "d",
    describe: "Directory to use for WorkspaceContainers",
    string: true,
  },
  nRequests: {
    describe: "Number of simultaneous http requests for cloud operations",
    number: true,
    hidden: true,
  },
  containerId: {
    alias: "c",
    describe: "ContainerId for WorkspaceDb",
    string: true,
    demandOption: true,
  },
  user: {
    describe: "String shown in cloud container locked message",
    string: true,
    default: "workspace-editor",
  },
  accessName: {
    alias: "a",
    describe: "Cloud storage account name for container",
    string: true,
  },
  accessToken: {
    describe:
      "Token that grants access to the container (either SAS or account key)",
    string: true,
    default: "",
  },
  storageType: {
    describe: "Cloud storage module type",
    string: true,
    default: "azure?sas=1",
  },
  logging: {
    describe: "enable log messages",
    boolean: true,
    default: false,
    hidden: true,
  },
...
Yargs.command<ListOptions>({
  ...
  builder: {
    strings: {
      alias: "s",
      describe: "list string resources",
      boolean: true,
      default: false,
    },
    blobs: {
      alias: "b",
      describe: "list blob resources",
      boolean: true,
      default: false,
    },
    files: {
      alias: "f",
      describe: "list file resources",
      boolean: true,
      default: false,
    },
  },
...
Yargs.command<UploadOptions>({
  ...
  builder: {
    noVacuum: {
      describe: "Don't vacuum source Db before importing",
      boolean: true,
    },
  },
...
Yargs.command<WorkspaceDbOpt>({
  ...
  describe:
    "clear the write lock for a cloud container. Note: this can be dangerous!",
...
async function runScript(arg: EditorProps & { scriptName: string }) {
  ...
  for (let line of lines) {
    ...
    await Yargs.parseAsync(
      line,
      {},
      (err: Error | undefined, _argv: any, _output: string) => {
        if (err) {
          console.error(
            `${arg.scriptName}:${i} [${line}] : ${BentleyError.getErrorMessage(
              err
            )}`
          );
          process.exit(1);
...
async function main() {
  if (process.argv.length > 1 && process.argv[2]?.[0] === "@") {
    ...
    await runCommand(runScript)({
      ...parsed,
      scriptName: process.argv[2].substring(1),
    });
    return;
  }
