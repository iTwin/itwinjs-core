...
- JSON Properties - Every Element has a Json property (JsonProperties) which can be used for adhoc addition of values without a schema update.
...
Json Properties are only discoverable by walking the instances in the model and query performance is not the same.  Keys must be unique because 
...
*This is the number I was quoted but I my experiements show the number to be 51

##Customization goals:
 - Allow users to customize properties in a schema in a contained manner
 - Enable Bentley updates to merge conflict free into customized schemas
 - Have zero impact on ECDb runtime performance and add minimal complication to ECDb source
...
####Idea 2 - Add Extension schema type which only allows abstract entity classes.  Primary schema and extension schema(s?) would be merged by ECObjects into a 'compiled' schema with the same name and version as the primary, but with an attribute identifying that the origin of the added property.
