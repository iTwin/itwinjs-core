...
2. The class of Element modeling the Object can allow "child" Elements, but then it is not allowed to be broken-down in a sub-Model. BIS calls this a "parent Element"—essentially modeling an Entity as an [aggregate](./glossary#aggregate). A parent Element is not redundant with its child Elements.
