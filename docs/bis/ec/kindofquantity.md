...
- When a unit label is not specified in the override, the label will be defaulted to the display label of the unit. See example [Null unit label in override](#format-overrides-with-nullempty-unit-labels).
- When a unit label is explicitly set to an empty string, the final label will be set to an empty string. See example [Empty unit label in override](#format-overrides-with-nullempty-unit-labels).
- Example
