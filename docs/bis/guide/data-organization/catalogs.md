...
There is currently no single BIS construct that identifies this bundle of elements as a unit. Discovering it requires traversing outward from the entry-point element: follow the `ModelModelsElement` relationship to its sub-model, the contained elements of that sub-model, navigation properties and relationships to referenced definitions, and owned aspects — recursively, until closure.

Two consequences of this structure are important when copying:

1. **Copying a component means copying its closure.** An application that imports a catalog entry must traverse and copy all of its dependencies, not just the entry-point element. Copying only the entry-point element produces a broken definition.
2. **Dependencies may be shared.** A single `Category` or `PhysicalMaterial` may be referenced by many components. Such shared dependencies belong to more than one component's closure and must be copied only once into the destination iModel.
...
With this mapping, the `FederationGuid` answers "*which version of which definition is this?*", the `ExternalSourceAspect` answers "*which catalog versions did it come from, and what is its stable identity there?*", and the `RepositoryLink` answers "*what catalog version is that?*".
