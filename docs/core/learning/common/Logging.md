...
An ActivityId is a globally unique ID that relates backend operations to frontend requests.
An ActivityId is:
* Specific to a single request.
* Generated by a frontend at the time it makes a request.
* Transmitted in the [x-correlation-id](https://en.wikipedia.org/wiki/List_of_HTTP_header_fields) HTTP header property.
* Applied by backends to the logging messages that they emit while carrying out the request.

The [ActivityLoggingContext]($bentleyjs-core) class manages the ActivityId for logging purposes.
