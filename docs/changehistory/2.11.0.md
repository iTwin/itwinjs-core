...
for (const id of settings.excludedElements) {
} // Replace this...
for (const id of settings.excludedElementIds) {
} // ...with this.
```
...
This rule used to include _BisCore.TypeDefinitionElement_ instance used as input along side _BisCore.GeometricElement3d_ instances. If previous behavior is desired `SelectedNodeInstance` specification can be added to the content rule:
