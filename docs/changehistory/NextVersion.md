...
[IModelHost.tileCacheService]($backend) remains to support older frontends, in which case [IModelHost.tileStorage]($backend) still has to be setup. Both are taken care of if [IModelHostOptions.tileCacheAzureCredentials]($backend) are used.
