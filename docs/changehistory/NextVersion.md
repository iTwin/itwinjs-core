...
The events generated by [TxnManager]($backend) and [BriefcaseTxns]($frontend) to notify of changes produced by [Txns]($docs/learning/InteractiveEditing.md) now include - in addition to the Ids of the changed [Element]($backend)s or [Model]($backend)s - the Id of each changed entity's [ECClass]($docs/bis/intro/glossary/#ecclass). The [OrderedId64Array]($bentleyjs-core) properties of [TxnChangedEntities]($backend) are deprecated in favor of the new [EntityIdAndClassIdIterable]($common) properties. Iterating the contents of a [ChangedEntities]($common) is easy using [ChangedEntitiesIterable]($common).
