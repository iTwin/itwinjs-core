...
### MapLayersUI initialization

In previous versions, MapLayersUI was intialized with an ever growing list of unnamed args. Now it is initialized with a single [MapLayersConfig]($map-layers) object with named args. The following code snippet demonstrates how to initialize MapLayersUI:

Previously:

```ts
await MapLayersUI.initialize(false, undefined, ...);
```

Now:

```ts
await MapLayersUI.initialize({ ...MapLayersConfig });
```

MapLayersUI will now always register UI items via [UiItemsProvider]($appui-abstract)s.
