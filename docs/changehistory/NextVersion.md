...
Because Electron has no `ipcMain.invoke`, this is implemented on top of the existing `send`/`addListener` primitives, so it works across all transports (Electron, mobile, and web sockets).
