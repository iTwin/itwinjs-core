...
ECSQL now supports [window functions](../learning/ECSqlReference/WindowFunctions.md). **This functionality add following new keywords that might collide with alias, parameter, property or class name and may break existing queries.** Application need to [escape](../learning/ECSqlReference/ECSqlKeywords.md#escaping-keywords) those name or try escaping all names in their queries to be on safe side in future.
