...
- `OidcBrowserClient` has been marked as `@deprecated in 3.x.` and split into the following classes:
...
The following items that were marked as @deprecated in 3.x. in the 1.x time frame have been removed:
...
A couple of already `@deprecated in 3.x.` APIs are now being hard-deprecated by adding a `DEPRECATED_` prefix to increase awareness about future removal of the APIs:
...
Some of already `@deprecated in 3.x.` APIs are now being hard-deprecated by adding a `DEPRECATED_` prefix to increase awareness about future removal of the APIs:
