...
- [Config]($bentley) has been relocated to `@itwin/core-bentley` from `@bentley/imodeljs-clients`. Update your imports:
...
Aside from the `signIn()` function supported by all FrontendAuthorizationClient implementations, there are also three new functions specific to [BrowserAuthorizationClient]($frontend-authorization-client) — `signInSilent()`, `signInPopup()`, and `signInRedirect()` (which is an alias of `signIn()`) — allowing more flexibility in how the signin is performed.
