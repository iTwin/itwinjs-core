...
- [BriefcaseDb.onOpen]($backend) and [BriefcaseDb.onOpened]($backend) events pass a context that may or may not include an AccessToken. i.e., they take [AuthorizedClientRequestContext]($itwin-client) | [ClientRequestContext]($core-bentley) as a parameter instead of [AuthorizedClientRequestContext]($itwin-client)
