...
[AgentAuthorizationClient]($clients-backend) now implements [AuthorizationClient]($itwin-client). The following methods are now marked deprecated -
* [AgentAuthorizationClient.getToken]($clients-backend)
* [AgentAuthorizationClient.refreshToken]($clients-backend)

The access token is now stored in the client, and can be accessed by calling [AgentAuthorizationClient.getAccessToken]($clients-backend). This method
