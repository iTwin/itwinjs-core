...
* Frontend applications that need to access protected data from the iModelHub must now provide an implementation of [AuthorizationClient]($itwin-client) through the [IModelApp.authorizationClient]($frontend) setting.
...
* None of the RPC interfaces now need to pass the [AccessToken]($itwin-client) as an argument. The RPC implementation provides the mechanism to to make this happen through a generic context. More on this below.
...
* The sub class [AuthorizedClientRequestContext]($itwin-client) includes the [AccessToken]($itwin-client) that carries the information on *`authorization`*. Note that this replaces the use of `ActivityLoggingContext` and `AccessToken` as parameters in various method calls. For example,
...
* Any frontend methods that contact multiple services (and not just the backend through RPC) need to explicitly create the context at the top level, and manage the context through multiple service calls. Right before the RPC request is made, call  [ClientRequestContext.useContextForRpc]($bentley) or [AuthorizedClientRequestContext.useContextForRpc]($itwin-client) to setup the use of that context (and the contained `activityId`) for the subsequent RPC request.
...
* `applicationId` may eventually be removed once it becomes possible to infer it from the [AccessToken]($itwin-client). A service to make this available is in the works.
...
* `IModelConnection.openSnapshot` throws an exception if [IModelApp.startup]($frontend) has not been called.
* `IModelDb.onOpened` takes a callback with a different signature -  [AuthorizedClientRequestContext]($itwin-client) is now passed as the first argument.
...
* [RequestGlobalOptions.timeout]($itwin-client) is now available to control global timeouts for requests to various Bentley services.
