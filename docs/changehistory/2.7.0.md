...
## New supported HTTP method for RPC backend servers

If you maintain an HTTP routing layer between your iModel.js frontend and backend (such as an API management system), please be aware that the frontend will now make an OPTIONS request before the first RPC operation.

This OPTIONS request is part of a new compatibility checking mechanism in the RPC system.
