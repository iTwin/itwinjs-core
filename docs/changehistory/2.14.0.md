...
The @bentley/ui-core package has added the [SettingsManager]($ui-core-react) class that allows any number of `SettingsProvider` classes to be registered. These providers provide [SettingsTabEntry]($ui-core-react) definitions used to populate the [SettingsContainer]($ui-core-react) UI component with setting pages used to manage application settings. These new classes are marked as beta in this release and are subject to minor modifications in future releases.

### Add Settings Page to set Quantity Formatting Overrides

The `QuantityFormatSettingsPanel` component has been added to the @bentley/ui-framework package to provide the UI to set both the `PresentationUnitSystem` and formatting overrides in the [QuantityFormatter]($frontend). This panel can be used in the new [SettingsContainer]($ui-core-react) UI component. The function `getQuantityFormatsSettingsManagerEntry` will return a [SettingsTabEntry]($ui-core-react) for use by the [SettingsManager]($ui-core-react). Below is an example of registering the `QuantityFormatSettingsPanel` with the `SettingsManager`.
