...
A gateway is actually a pair of classes: a proxy that is located in and used by clients and an implementation that is located in and executed by the service. iModelJs supplies transport mechanisms to marshall calls between client and service. These mechanisms are called *gateway configurations*. Configurations are applied to gateways at runtime. Available configurations include:
...
Marshalls calls on the gateway through high-bandwith, low-latency pipes between cooperating processes on the same computer.
