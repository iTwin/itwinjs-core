# Runtime Schema Binary Format

This document specifies the binary blob format produced by `RuntimeSchemaWriter` (C++, imodel-native) and consumed by `parseRuntimeSchemaBlob()` (TypeScript, ecschema-metadata). It is the transport format for `RuntimeSchemaContext` - see [RuntimeSchemaContext.md](./RuntimeSchemaContext.md) for the consumer API.
...
- **Pragma handler**: `PRAGMA runtime_schemas(N)` where N is the requested format version
