...
- **Origin trust and redirects** — every send issued through the handler is treated like a credentialed request by [MapLayerFormatRegistry.restrictCredentialsToTrustedOrigins]($frontend): while the restriction is enabled it is issued with `redirect: "error"`, so handler-injected values cannot silently reach an unlisted origin through a redirect. See [Map-layer security](./MapLayersAndBasemaps.md#map-layer-security).
- **SSO** — a send issued through the handler never triggers the NTLM/Negotiate retry with browser credentials; the handler is the authentication authority for every request it manages, including those it passes through unmodified. Layers served by Windows-Authentication-protected services therefore cannot be combined with a fetch handler.
