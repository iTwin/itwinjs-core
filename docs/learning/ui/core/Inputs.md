...
The [Inputs]($ui-core-react:Inputs) category in the `@bentley/ui-core` package includes
components for working with input controls, such as Input, IconInput, NumberInput TextArea.

## Input

The [Input]($ui-core-react) React component is a wrapper for the `<input type="text">` HTML element.
...
The [Textarea]($ui-core-react) React component is a wrapper for the `<textarea>` HTML element.
...
The [IconInput]($ui-core-react) React component is an Input component
...
The [NumberInput]($ui-core-react) React component is an input component that accepts numeric input.
