...
- Currently, it's possible for an app to (mis)configure their dependencies such that they end up with multiple versions of single core package - and hence, multiple independent copies of each `Symbol` defined in `Symbols.ts`. For now, we use `Symbol.for` to define those symbols so that they are looked up by name in a global registry, preventing duplication. In 5.x, we will prohibit taking a dependency on multiple versions of the same core package (which can lead to all sorts of other problems), and switch to using the `Symbol` constructor, which does not register the symbol in the global registry. This will make it impossible for anyone outside the package to look up the symbol by its name.
- In 5.x, we can delete all of the deprecated `@internal` APIs. At that point, the only APIs tagged as `@internal` should be nested APIs identified by a `Symbol`. Top-level internal APIs do not require a release tag, because they are not exported from the package's barrel file.
