...
export class RulesetEmbedder {
  ...
  public async insertRuleset(ruleset: Ruleset, options?: RulesetInsertOptions): Promise<Id64String> {
    ...
    `;
    const reader = this._imodel.createQueryReader(query, QueryBinder.from({ rulesetId: ruleset.id }), { rowFormat: QueryRowFormat.UseECSqlPropertyIndexes });
    while (await reader.step()) {
      const row = reader.current.toArray();
      const existingRulesetElementId: Id64String = row[0];
      const existingRuleset: Ruleset = JSON.parse(row[1]).jsonProperties;
