...
export class PresentationRpcImpl extends PresentationRpcInterface implements IDisposable {
  ...
  >(
    ...
    if (resultPromise) {
      ...
      if (rulesetVariables) managerRequestOptions.rulesetVariables = rulesetVariables.map(RulesetVariable.fromJSON);
  ...
  public override async getContentDescriptor(
    ...
    return this.makeRequest(token, "getContentDescriptor", requestOptions, async (options) => {
      ...
      if (options.transport === "unparsed-json") {
        ...
        return Presentation.getManager(requestOptions.clientId).getDetail().getContentDescriptor(options) as unknown as
          | DescriptorJSON
          | undefined;
      } else {
  ...
  public override async getContentInstanceKeys(
    ...
    return this.makeRequest(token, "getContentInstanceKeys", requestOptions, async (options) => {
      ...
      if (size === 0 || !content) return { total: 0, items: new KeySet().toJSON() };
  ...
  public override async getPagedDisplayLabelDefinitions(
    ...
    if (pageOpts.paging.size < requestOptions.keys.length) requestOptions.keys.splice(pageOpts.paging.size);
  ...
  public override async computeSelection(
    ...
    return this.makeRequest(token, "computeSelection", requestOptions, async (options) => {
      ...
      const keys = await this.getManager(requestOptions.clientId).computeSelection(
        options as ComputeSelectionRequestOptions<IModelDb>
      );
      return keys.toJSON();
    });
  }
