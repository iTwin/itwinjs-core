...
type ContentGetter<TResult = any, TOptions = any> = (
 ...
 * @internal
 */
export class PresentationRpcImpl
  extends PresentationRpcInterface
  implements IDisposable
{
  ...
  public constructor(props?: { requestTimeout?: number }) {
    ...
    this._pendingRequests = new TemporaryStorage({
      ...
      unusedValueLifetime:
        this._requestTimeout > 0 ? this._requestTimeout + 10 * 1000 : undefined,
      ...
      cleanupHandler: (id, _, reason) => {
        if (reason !== "request") {
          Logger.logTrace(
            PresentationBackendLoggerCategory.Rpc,
            `Cleaning up request without frontend retrieving it: ${id}.`
          );
  ...
  private successResponse<TResult>(
    result: TResult,
  ...
  private getIModel(token: IModelRpcProps): IModelDb {
    ...
    try {
      ...
      throw new PresentationError(
        ...
        "IModelRpcProps doesn't point to a valid iModel"
      );
    }
    return imodel;
  }
  ...
  >(
    ...
    Logger.logInfo(
      PresentationBackendLoggerCategory.Rpc,
      `Received '${requestId}' request. Params: ${requestKey}`
    );
    ...
    if (resultPromise) {
      ...
      Logger.logTrace(
        PresentationBackendLoggerCategory.Rpc,
        `Request not found, creating a new one`
      );
      let imodel: IModelDb;
      try {
      ...
      if (rulesetVariables)
        managerRequestOptions.rulesetVariables = rulesetVariables.map(
          RulesetVariable.fromJSON
        );
        ...
        managerRequestOptions.diagnostics = {
          ...
          handler: (d: Diagnostics) => {
            if (d.logs) {
              ...
              if (target.logs) target.logs.push(...d.logs);
              else target.logs = [...d.logs];
            }
          },
        };
      }
      ...
      resultPromise = request(managerRequestOptions)
        ...
        .catch((e: PresentationError) =>
          this.errorResponse(e.errorNumber, e.message, diagnostics)
        );
    ...
    if (this._requestTimeout === 0) {
      Logger.logTrace(
        PresentationBackendLoggerCategory.Rpc,
        `Request timeout not configured, returning promise without a timeout.`
      );
    ...
    Logger.logTrace(
      PresentationBackendLoggerCategory.Rpc,
      `Returning a promise with a timeout of ${this._requestTimeout}.`
    );
    return Promise.race([resultPromise, timeoutPromise])
      .catch<PresentationRpcResponseData>(() => {
        ...
        Logger.logTrace(
          PresentationBackendLoggerCategory.Rpc,
          `Request timeout, returning "BackendTimeout" status.`
        );
      ...
      .then((response: PresentationRpcResponseData<TResult>) => {
        if (response.statusCode !== PresentationStatus.BackendTimeout) {
          Logger.logTrace(
            PresentationBackendLoggerCategory.Rpc,
            `Request completed, returning result.`
          );
          this._pendingRequests.deleteValue(requestKey);
        }
  ...
  public override async getNodesCount(
    token: IModelRpcProps,
    requestOptions: HierarchyRpcRequestOptions
    ...
    return this.makeRequest(
      ...
      async (options) => {
        ...
        const [serializedNodesJson, count] = await Promise.all([
          this.getManager(requestOptions.clientId)
            .getDetail()
            .getNodes(options),
  ...
  public override async getNodesDescriptor(
    ...
    return this.makeRequest(
      ...
      async (options) => {
        ...
        return this.getManager(requestOptions.clientId)
          .getDetail()
          .getNodesDescriptor(options);
      }
    );
  }
    ...
    return this.makeRequest(
      ...
      async (options) => {
        const result = await this.getManager(
          requestOptions.clientId
        ).getNodePaths(options);
        // eslint-disable-next-line deprecation/deprecation
  ...
  public override async getContentSources(
    ...
    return this.makeRequest(
      ...
      async (options) => {
        ...
        const classesMap = {};
        const selectClasses = result.map((sci) =>
    ...
    return this.makeRequest(
      ...
      async (options) => {
        ...
        if (options.transport === "unparsed-json") {
          ...
          return Presentation.getManager(requestOptions.clientId)
            .getDetail()
            .getContentDescriptor(options) as unknown as
            | DescriptorJSON
            | undefined;
        } else {
          // Support for older frontends that still expect a parsed descriptor
          const descriptor = await Presentation.getManager(
            requestOptions.clientId
          ).getContentDescriptor(options);
  ...
  public override async getContentSetSize(
    token: IModelRpcProps,
    requestOptions: ContentRpcRequestOptions
  ): PresentationRpcResponse<number> {
    return this.makeRequest(
      token,
      "getContentSetSize",
      ...
      async (options) => {
        ...
        return this.getManager(requestOptions.clientId).getContentSetSize(
          options
        );
      }
    );
  ...
  public override async getPagedContentSet(
    ...
    return this.successResponse(
      content.result ? content.result.contentSet : { total: 0, items: [] }
    );
  }
    ...
    return this.makeRequest(
      ...
      async (options) => {
        ...
        const response = await this.getManager(
          requestOptions.clientId
        ).getPagedDistinctValues(options);
        return {
          ...response,
          // eslint-disable-next-line deprecation/deprecation
  ...
  public override async getContentInstanceKeys(
    ...
    return this.makeRequest(
      ...
      async (options) => {
        ...
        return {
          total: size,
          items: content.contentSet
            .reduce((keys, item) => keys.add(item.primaryKeys), new KeySet())
            .toJSON(),
        };
      }
    );
  ...
  public override async getDisplayLabelDefinition(
    ...
    return this.makeRequest(
      ...
      requestOptions,
      async (options) => {
        const label = await this.getManager(requestOptions.clientId)
          .getDetail()
          .getDisplayLabelDefinition(options);
        return label;
      }
    );
  }
  ...
  public override async getPagedDisplayLabelDefinitions(
    ...
    const pageOpts = enforceValidPageSize({
      paging: { start: 0, size: requestOptions.keys.length },
    });
    if (pageOpts.paging.size < requestOptions.keys.length)
      requestOptions.keys.splice(pageOpts.paging.size);
    return this.makeRequest(
      token,
      "getPagedDisplayLabelDefinitions",
      requestOptions,
      async (options) => {
    ...
    return this.makeRequest(
      ...
      async (options) => {
        ...
        const keys = await this.getManager(
          requestOptions.clientId
        ).computeSelection(options as ComputeSelectionRequestOptions<IModelDb>);
        return keys.toJSON();
      }
    );
  }
...
const enforceValidPageSize = <TOptions extends Paged<object>>(
  ...
  const validPageSize = getValidPageSize(
    requestOptions.paging?.size,
    maxPageSize
  );
  if (!requestOptions.paging || requestOptions.paging.size !== validPageSize)
    return {
      ...requestOptions,
      paging: { ...requestOptions.paging, size: validPageSize },
    };
...
const getValidPageSize = (size: number | undefined, maxPageSize: number) => {
  ...
  return requestedSize === 0 || requestedSize > maxPageSize
    ? maxPageSize
    : requestedSize;
};

// eslint-disable-next-line deprecation/deprecation
const nodeKeyFromJson = (
  json: NodeKeyJSON | undefined
): NodeKey | undefined => {
