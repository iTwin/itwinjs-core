...
export function getLocalizedStringEN(key: string) {
  ...
  if (namespace !== "Presentation") return key;
  const keySteps = identifier.split(".");
  for (const keyStep of keySteps) {
    if (keyStep in result === false) return key;
...
/** @internal */
export function combineDiagnosticsOptions(
  ...options: Array<BackendDiagnosticsOptions | undefined>
...
/** @internal */
export function reportDiagnostics<TContext>(
