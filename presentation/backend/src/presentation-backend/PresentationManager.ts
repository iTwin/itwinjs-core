...
export class PresentationManager {
  ...
  constructor(props?: PresentationManagerProps) {
    ...
    this._localizationHelper = new LocalizationHelper({
      getLocalizedString: props?.getLocalizedString ?? getLocalizedStringEN,
    });
  }
  ...
  public async computeSelection(
    requestOptions: (
      | (SelectionScopeRequestOptions<IModelDb> & {
          ids: Id64String[];
          scopeId: string;
        })
      | ComputeSelectionRequestOptions<IModelDb>
    ) &
      BackendDiagnosticsAttribute
  ): Promise<KeySet> {
...
async function buildElementsPropertiesInPages(
  className: string,
  ids: string[],
  getter: (keys: KeySet) => Promise<ElementProperties[]>
) {
