...
describe("SelectionScopesHelper", () => {
  ...
  describe("getSelectionScopes", () => {
    it("returns expected selection scopes", async () => {
      ...
      expect(result.map((s) => s.id)).to.deep.eq([
        "element",
        "assembly",
        "top-assembly" /* , "category", "model" */,
      ]);
    });
  });
  ...
  describe("computeSelection", () => {
      ...
      imodelMock
        .setup((x) =>
          x.withPreparedStatement(
            moq.It.is(
              (q) =>
                typeof q === "string" && q.includes("SELECT ECClassId FROM")
            ),
            moq.It.isAny()
          )
        )
    ...
    const setupIModelForInvalidId = () => {
      ...
      imodelMock
        ...
        .callback((_q, cb) => {
          const stmtMock = moq.Mock.ofType<ECSqlStatement>();
          stmtMock
            .setup((x) => x.bindId(moq.It.isAnyNumber(), moq.It.isAny()))
            .throws(
              new IModelError(DbResult.BE_SQLITE_ERROR, "Error binding Id")
            );
          stmtMock
            .setup((x) => x.step())
            .returns(() => DbResult.BE_SQLITE_ERROR);
          cb(stmtMock.object);
        });
    };
    ...
    const createRandomElementProps = (parentId?: Id64String): ElementProps => {
      if (!parentId) parentId = createRandomId();
      return {
    ...
    const createTransientElementId = () =>
      Id64.fromLocalAndBriefcaseIds(faker.random.number(), 0xffffff);

    const setupIModelForFunctionalKeyQuery = (props: {
      graphicalElementKey: InstanceKey;
      stepResult?: DbResult;
      functionalElementKey?: InstanceKey;
    }) => {
      const functionalKeyQueryIdentifier =
      ...
      imodelMock
        .setup((x) =>
          x.withPreparedStatement(
            moq.It.is(
              (q) =>
                typeof q === "string" &&
                q.includes(functionalKeyQueryIdentifier)
            ),
            moq.It.isAny()
          )
        )
        .returns((_q, cb) => {
          ...
          stmtMock
            .setup((x) => x.step())
            .returns(() => props.stepResult ?? DbResult.BE_SQLITE_ROW);
          stmtMock
            .setup((x) => x.getRow())
            .returns(() => ({
    ...
    const setupIModelForElementProps = (props?: {
      key?: InstanceKey;
      parentKey?: InstanceKey;
      isRemoved?: boolean;
    }) => {
      const key = props?.key ?? createRandomECInstanceKey();
      const elementProps = props?.isRemoved
        ? undefined
        : props?.parentKey
        ? createRandomElementProps(props.parentKey.id)
        : createRandomTopmostElementProps();
      elementsMock
        .setup((x) => x.tryGetElementProps(key.id))
        .returns(() => elementProps);
      return { key, props: elementProps };
    };

    const setupIModelDerivesFromClassQuery = (
      doesDeriveFromSuppliedClass: boolean
    ) => {
      ...
      imodelMock
        .setup((x) =>
          x.withPreparedStatement(
            moq.It.is(
              (q) =>
                typeof q === "string" &&
                q.includes(classDerivesFromQueryIdentifier)
            ),
            moq.It.isAny()
          )
        )
        .returns((_q, cb) => {
          ...
          stmtMock
            .setup((x) => x.step())
            .returns(() =>
              doesDeriveFromSuppliedClass
                ? DbResult.BE_SQLITE_ROW
    ...
    it("throws on invalid scopeId", async () => {
      await expect(
        SelectionScopesHelper.computeSelection(
    ...
    describe("scope: 'element'", () => {
      ...
      it("skips transient element ids", async () => {
        const keys = [
          createRandomECInstanceKey(),
          { className: "any:class", id: createTransientElementId() },
        ];
      ...
      it("handles invalid id", async () => {
        ...
        const result = await SelectionScopesHelper.computeSelection(
          { imodel: imodelMock.object },
          [validKeys[0].id, "not an id", validKeys[1].id],
          "element"
        );
        expect(result.size).to.eq(2);
      ...
      it("returns nth parent key", async () => {
        const parent3 = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
        });
        const parent2 = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
          parentKey: parent3.key,
        });
        const parent1 = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
          parentKey: parent2.key,
        });
        const element = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
    ...
    describe("scope: 'assembly'", () => {
      it("returns parent keys", async () => {
        ...
        const elementProps = parentKeys.map((pk) =>
          createRandomElementProps(pk.id)
        );
        elementProps.forEach((p) => {
          elementsMock
            .setup((x) => x.tryGetElementProps(p.id!))
            .returns(() => p);
        });
        const result = await SelectionScopesHelper.computeSelection(
          { imodel: imodelMock.object },
      ...
      it("does not duplicate keys", async () => {
        ...
        const elementProps = [
          createRandomElementProps(parentKey.id),
          createRandomElementProps(parentKey.id),
        ];
        elementProps.forEach((p) => {
          elementsMock
      ...
      it("skips removed elements", async () => {
        ...
        elementsMock
          .setup((x) => x.tryGetElementProps(existingElementProps.id!))
          .returns(() => existingElementProps);
        // set up removed element props
        const removedElementProps = createRandomElementProps();
        elementsMock
          .setup((x) => x.tryGetElementProps(removedElementProps.id!))
          .returns(() => undefined);
        setupIModelForNoResultStatement();
        // request
        const result = await SelectionScopesHelper.computeSelection(
    ...
    describe("scope: 'top-assembly'", () => {
      it("returns topmost parent key", async () => {
        ...
        elementsMock
          .setup((x) => x.tryGetElementProps(grandparentKey.id))
          .returns(() => grandparent);
        const parent = createRandomElementProps(grandparentKey.id);
        const parentKey = createRandomECInstanceKey();
        elementsMock
          .setup((x) => x.tryGetElementProps(parentKey.id))
          .returns(() => parent);
        const element = createRandomElementProps(parentKey.id);
        elementsMock
          .setup((x) => x.tryGetElementProps(element.id!))
          .returns(() => element);
      ...
      it("skips transient element ids", async () => {
        ...
        elementsMock
          .setup((x) => x.tryGetElementProps(elementProps.id!))
          .returns(() => elementProps);
    ...
    describe("scope: 'category'", () => {
      it("returns category key", async () => {
        ...
        elementsMock
          .setup((x) => x.tryGetElement(elementId))
          .returns(() => element);
        elementsMock
          .setup((x) => x.tryGetElementProps(category.id!))
          .returns(() => category);
      ...
      it("skips removed categories", async () => {
        ...
        elementsMock
          ...
          .returns(() => element);
        elementsMock
          .setup((x) => x.tryGetElementProps(categoryId))
          .returns(() => undefined);
    ...
    describe("scope: 'model'", () => {
      it("returns model key", async () => {
        ...
        elementsMock
          .setup((x) => x.tryGetElementProps(elementId))
          .returns(() => element);
        modelsMock
          .setup((x) => x.tryGetModelProps(model.id!))
          .returns(() => model);
        ...
        expect(result.size).to.eq(1);
        expect(result.has({ className: model.classFullName, id: model.id! })).to
          .be.true;
      });
      ...
      it("skips removed models", async () => {
        ...
        elementsMock
          ...
          .returns(() => element);
        modelsMock
          .setup((x) => x.tryGetModelProps(modelId))
          .returns(() => undefined);
    ...
    describe("scope: 'functional-element'", () => {
      ...
      it("returns GeometricElement2d key if it has parents but none of them have related functional elements", async () => {
        ...
        setupIModelForElementProps({
          key: graphicalElementKey,
          parentKey: graphicalParentElementKey,
        });
        setupIModelForFunctionalKeyQuery({
          graphicalElementKey: graphicalParentElementKey,
        });
        setupIModelForElementProps({
          key: graphicalParentElementKey,
          parentKey: graphicalGrandParentElementKey,
        });
        setupIModelForFunctionalKeyQuery({
          graphicalElementKey: graphicalGrandParentElementKey,
        });
      ...
      it("skips removed GeometricElement2d parents when looking for closest functional element", async () => {
        ...
        const functionalElement = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
        });
        const existingParent = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
        });
        const existingElement = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
          parentKey: existingParent.key,
        });
        setupIModelForFunctionalKeyQuery({
          graphicalElementKey: existingElement.key,
        });
        setupIModelForFunctionalKeyQuery({
          graphicalElementKey: existingParent.key,
          functionalElementKey: functionalElement.key,
        });

        // set up one element with removed parent
        const removedParent = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
          isRemoved: true,
        });
        const elementWithRemovedParent = setupIModelForElementProps({
          key: createRandomECInstanceKey(),
          parentKey: removedParent.key,
        });
        setupIModelForFunctionalKeyQuery({
          graphicalElementKey: elementWithRemovedParent.key,
        });
    ...
    describe("scope: 'functional-assembly'", () => {
      ...
      it("returns functional parent element key of the first GeometricElement2d parent that has a related functional element", async () => {
        ...
        setupIModelForElementProps({
          key: functionalElementKey,
          parentKey: functionalParentElementKey,
        });
    ...
    describe("scope: 'functional-top-assembly'", () => {
      ...
      it("returns functional topmost parent element key of the first GeometricElement2d parent that has a related functional element", async () => {
        ...
        setupIModelForElementProps({
          key: functionalParentElementKey,
