...
describe("RulesetEmbedder", () => {
  ...
  function initializeMocks() {
    ...
    // create code specs
    rulesetCodeSpec = CodeSpec.create(
      imodelMock.object,
      PresentationRules.CodeSpec.Ruleset,
      CodeScopeSpec.Type.Model
    );

    subjectCodeSpec = CodeSpec.create(
      imodelMock.object,
      BisCodeSpec.subject,
    ...
    informationPartitionCodeSpec = CodeSpec.create(
      imodelMock.object,
    ...
    codeSpecsMock
      .setup((x) => x.getByName(PresentationRules.CodeSpec.Ruleset))
      .returns(() => rulesetCodeSpec);
    codeSpecsMock
      .setup((x) => x.getByName(BisCodeSpec.subject))
      .returns(() => subjectCodeSpec);
    codeSpecsMock
      .setup((x) => x.getByName(BisCodeSpec.informationPartitionElement))
      .returns(() => informationPartitionCodeSpec);

    elementsMock
      .setup((x) => x.getRootSubject())
      .returns(() => rootSubjectMock.object);
    ...
    presentationRulesSubjectMock
      .setup((x) => x.id)
      .returns(() => presentationRulesSubjectId);
    presentationRulesSubjectMock
      .setup((x) => x.insert())
      .returns(() => presentationRulesSubjectId);
    ...
    definitionPartitionMock
      .setup((x) => x.id)
      .returns(() => definitionPartitionId);
    definitionPartitionMock
      .setup((x) => x.insert())
      .returns(() => definitionPartitionId);
  ...
  function setupMocksForHandlingPrerequisites() {
    imodelMock
      .setup((x) => x.containsClass(RulesetElements.Ruleset.classFullName))
      .returns(() => false);
    imodelMock
      .setup(async (x) => x.importSchemas(moq.It.isAny()))
      .returns(async () => undefined);
    codeSpecsMock
      .setup((x) => x.insert(rulesetCodeSpec))
      .returns(() => faker.random.uuid());
  }
  ...
  function setupMocksForGettingRulesetModel() {
    imodelMock
      ...
      .returns(() => true);
    modelsMock
      .setup((x) => x.getSubModel(definitionPartitionId))
      .returns(() => rulesetModelMock.object);
    elementsMock
      .setup((x) =>
        x.tryGetElement(
          new Code({
            spec: subjectCodeSpec.id,
            scope: rootSubjectMock.object.id,
      ...
      .setup((x) =>
        x.tryGetElement(
          DefinitionPartition.createCode(
            imodelMock.object,
            presentationRulesSubjectMock.object.id,
            "PresentationRules"
          )
        )
      )
  ...
  function setupMocksForCreatingRulesetModel() {
    ...
    elementsMock
      ...
      .returns(() => undefined);
    elementsMock
      .setup((x) => x.getElement(presentationRulesSubjectId))
      .returns(() => presentationRulesSubjectMock.object);
    elementsMock
      .setup((x) => x.getElement(definitionPartitionId))
      .returns(() => definitionPartitionMock.object);
    ...
    elementsMock
      .setup((x) => x.createElement(createSubjectProps))
      .returns(() => presentationRulesSubjectMock.object);
    ...
    const createPartitionProps = {
      ...
      model: modelId,
      code: DefinitionPartition.createCode(
        imodelMock.object,
        presentationRulesSubjectId,
        "PresentationRules"
      ),
      classFullName: DefinitionPartition.classFullName,
    };
    elementsMock
      .setup((x) => x.createElement(createPartitionProps))
      .returns(() => definitionPartitionMock.object);
    ...
    modelsMock
      .setup((x) => x.createModel(createModelProps))
      .returns(() => rulesetModelMock.object);
  }

  function setupMocksForQueryingExistingRulesets(
    rulesetId: string,
    rulesets: Array<{ ruleset: Ruleset; elementId: Id64String }>
  ) {
    ...
    ecSqlReaderMock
      ...
      .returns(async () => ++currIndex < results.length);
    ecSqlReaderMock
      .setup((x) => x.current)
      .returns(() => {
  ...
  function createRulesetElementProps(ruleset: Ruleset): DefinitionElementProps {
    return {
      model: modelId,
      code: RulesetElements.Ruleset.createRulesetCode(
        imodelMock.object,
        modelId,
        ruleset
      ),
  ...
  function setupMocksForInsertingNewRuleset(
    ruleset: Ruleset,
    rulesetElementId: string
  ) {
    definitionElementMock.setup((x) => x.id).returns(() => rulesetElementId);
    definitionElementMock
      .setup((x) => x.insert())
      .returns(() => rulesetElementId);
    elementsMock
      .setup((x) => x.createElement(createRulesetElementProps(ruleset)))
      .returns(() => definitionElementMock.object);
    elementsMock
      .setup((x) => x.getElement(rulesetElementId))
      .returns(() => definitionElementMock.object);
  }
  ...
  describe("insertRuleset", () => {
    it("sets up prerequisites when inserting element", async () => {
      ...
      imodelMock.verify(
        async (x) => x.importSchemas(moq.It.isAny()),
        moq.Times.once()
      );
    ...
    it("skips inserting ruleset with same id", async () => {
      ...
      expect(insertId).to.eq(rulesetElementId);
      elementsMock.verify(
        (x) => x.insertElement(createRulesetElementProps(ruleset)),
        moq.Times.never()
      );
    });
    ...
    it("updates a duplicate ruleset with same id and version", async () => {
      ...
      elementsMock
        .setup((x) => x.tryGetElement(rulesetElementId))
        .returns(() => rulesetElementMock.object);
  ...
  describe("getRulesets", () => {
    function setupMocksForQueryingAllRulesets(
      rulesets: Array<{ ruleset: Ruleset; elementId: Id64String }>
    ) {
      imodelMock
        ...
        .callback((_ecsql, callbackFun) => {
          ...
          rulesets.forEach((entry) => {
            ...
            statementMock
              .setup((x) => x.getRow())
              .returns(() => ({ id: entry.elementId }));

            const rulesetElementMock =
              moq.Mock.ofType<RulesetElements.Ruleset>();
            rulesetElementMock
              .setup((x) => x.jsonProperties)
              .returns(() => ({ jsonProperties: entry.ruleset }));
            elementsMock
              .setup((x) => x.getElement({ id: entry.elementId }))
              .returns(() => rulesetElementMock.object);
          });
          statementMock
            .setup((x) => x.step())
