...
} from "./favorite-properties/FavoritePropertiesManager";
import {
...
export class Presentation {
  ...
  public static async initialize(props?: PresentationProps): Promise<void> {
    ...
    for (const handler of initializationHandlers) {
      ...
      if (cleanup) terminationHandlers.push(cleanup);
    }
  }
  ...
  public static terminate(): void {
    ...
    if (localization) FrontendLocalizationHelper.unregisterNamespaces();

    if (presentationManager) presentationManager.dispose();
    presentationManager = undefined;

    if (favoritePropertiesManager) favoritePropertiesManager.dispose();
