...
export class SelectionManager implements ISelectionProvider {
  private _selectionContainerMap = new Map<
    IModelConnection,
    SelectionContainer
  >();
  private _imodelToolSelectionSyncHandlers = new Map<
    IModelConnection,
    { requestorsCount: number; handler: ToolSelectionSyncHandler }
  >();
  ...
  public setSyncWithIModelToolSelection(imodel: IModelConnection, sync = true) {
    ...
    if (sync) {
      if (!registration || registration.requestorsCount === 0) {
        this._imodelToolSelectionSyncHandlers.set(imodel, {
          requestorsCount: 1,
          handler: new ToolSelectionSyncHandler(imodel, this),
        });
      } else {
        this._imodelToolSelectionSyncHandlers.set(imodel, {
          ...registration,
          requestorsCount: registration.requestorsCount + 1,
        });
      }
    } else {
      if (registration && registration.requestorsCount > 0) {
  ...
  public suspendIModelToolSelectionSync(imodel: IModelConnection): IDisposable {
    const registration = this._imodelToolSelectionSyncHandlers.get(imodel);
    if (!registration) return { dispose: () => {} };
  ...
  private handleEvent(evt: SelectionChangeEventArgs): void {
    ...
    switch (evt.changeType) {
      ...
      case SelectionChangeType.Replace:
        if (
          selectedItemsSet.size !== evt.keys.size ||
          !selectedItemsSet.hasAll(evt.keys)
        ) {
    ...
    if (selectedItemsSet.guid === guidBefore) return;
...
class SelectionContainer {
  ...
  public getSelectionLevels(): number[] {
    ...
    for (const entry of this._selectedItemsSetMap.entries()) {
      if (!entry[1].isEmpty) levels.push(entry[0]);
    }
    return levels.sort();
  }
...
export class ToolSelectionSyncHandler implements IDisposable {
  ...
  public constructor(
    imodel: IModelConnection,
    logicalSelection: SelectionManager
  ) {
    this._imodel = imodel;
    this._logicalSelection = logicalSelection;
    this._imodelToolSelectionListenerDisposeFunc =
      imodel.selectionSet.onChanged.addListener(this.onToolSelectionChanged);
  }
  ...
  private onToolSelectionChanged = async (
...
const parseIds = (
  ids: Id64Arg
): { persistent: Id64Arg; transient: Id64Arg } => {
  let allPersistent = true;
  let allTransient = true;
  for (const id of Id64.iterable(ids)) {
    if (Id64.isTransient(id)) allPersistent = false;
    else allTransient = false;

    if (!allPersistent && !allTransient) break;
  }
  ...
  for (const id of Id64.iterable(ids)) {
    if (Id64.isTransient(id)) transientElementIds.push(id);
    else persistentElementIds.push(id);
  }
...
class ScopedSelectionChanger {
  ...
  public async add(
    transientIds: Id64Arg,
    persistentIds: Id64Arg,
    level: number
  ): Promise<void> {
    const keys = await this.manager.scopes.computeSelection(
      this.imodel,
      persistentIds,
      this.scope
    );
