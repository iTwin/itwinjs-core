...
export class RulesetVariablesManagerImpl implements RulesetVariablesManager {
  ...
  public getAllVariables(): RulesetVariable[] {
    ...
    for (const entry of this._clientValues) variables.push(entry[1]);
    return variables;
  }
  ...
  private async setValue(variable: RulesetVariable): Promise<void> {
    ...
    if (oldVariable && variablesEqual(oldVariable, variable)) return;

    this._clientValues.set(variable.id, variable);
    if (this._ipcHandler) {
      await this._ipcHandler.setRulesetVariable({
  ...
  public async unset(variableId: string): Promise<void> {
    ...
    if (this._ipcHandler) {
      await this._ipcHandler.unsetRulesetVariable({
        rulesetId: this._rulesetId,
        variableId,
      });
    }
  ...
  public async setId64s(
    ...
    await this.setValue({
      ...
      value: [...OrderedId64Iterable.sortArray(value)],
    });
  }
}
