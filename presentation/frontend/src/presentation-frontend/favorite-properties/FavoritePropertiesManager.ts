...
export class FavoritePropertiesManager implements IDisposable {
  ...
  private _adjustPropertyOrderInfos = async (iTwinId: string, imodelId: string) => {
    ...
    for (let i = propertiesOrder.length - 1; i >= 0; i--) {
      if (infosToAdd.has(propertiesOrder[i].name)) infosToAdd.delete(propertiesOrder[i].name);
  ...
  public has(field: Field, imodel: IModelConnection, scope: FavoritePropertiesScope): boolean {
    ...
    return (
      ...
      (scope === FavoritePropertiesScope.IModel &&
        setHasAny(this._imodelProperties.get(getiModelInfo(iTwinId, imodelId))!, fieldInfos))
    );
  }
  ...
  private _getBaseClassesByClass = async (
    ...
    if (this._imodelBaseClassesByClass.has(imodelInfo)) baseClasses = this._imodelBaseClassesByClass.get(imodelInfo)!;
    ...
    WHERE (derivedSchema.Name || ':' || derivedClass.Name) IN (${[...missingClasses]
    ...
    while (await reader.step()) {
      ...
      if (!(row.classFullName in baseClasses)) baseClasses[row.classFullName] = [];
  ...
  public async changeFieldPriority(
    ...
    if (field === afterField) throw Error("`field` can not be the same as `afterField`.");

    const allOrderInfos = this._propertiesOrder.get(getiModelInfo(iTwinId, imodelId))!;

    const findFieldOrderInfoData = (f: Field) => {
      if (!visibleFields.includes(f)) throw Error("Field is not contained in visible fields.");
      const infos = getFieldOrderInfos(f, allOrderInfos);

      if (infos.length === 0) throw Error("Field has no property order information.");
    ...
    for (let i = startIndex; i !== orderInfoIndex; i += direction) {
      ...
      const hasBaseClasses = baseClassesByClass[currOrderInfo.parentClassName].some((classId) =>
        relevantClasses.has(classId)
      );
      if (hasBaseClasses) continue;

      if (direction === Direction.Down) irrelevantOrderInfos.push(currOrderInfo);
    ...
    allOrderInfos.splice(
      afterOrderInfoIndex + 1 + (direction === Direction.Up ? movingOrderInfos.length : 0),
      0,
...
export const getFieldInfos = (field: Field): Set<PropertyFullName> => {
  ...
  if (field.isPropertiesField()) getPropertiesFieldPropertyNames(field).forEach((info) => fieldInfos.add(info));
  else if (field.isNestedContentField()) fieldInfos.add(getNestedContentFieldPropertyName(field));
  else fieldInfos.add(`${FavoritePropertiesManager.FAVORITES_IDENTIFIER_PREFIX}${field.name}`);
  return fieldInfos;
};
...
const getFieldOrderInfos = (field: Field, orderInfos: FavoritePropertiesOrderInfo[]): FavoritePropertiesOrderInfo[] => {
  ...
  if (field.isPropertiesField()) getPropertiesFieldPropertyNames(field).forEach(tryAddOrderInfo);
  else if (field.isNestedContentField()) tryAddOrderInfo(getNestedContentFieldPropertyName(field));
