...
import { compareInstanceKeys } from "../EC";
import { Id64 } from "@bentley/bentleyjs-core";

describe("compareInstanceKeys", () => {
  ...
  it("returns less than 0 when `lhs.className` < `rhs.className`", () => {
    ...
    expect(compareInstanceKeys(lhs, rhs)).to.be.lt(0);
  });
  ...
  it("returns 0 when `lhs.className` = `rhs.className` and `lhs.id` = `rhs.id`", () => {
    ...
    expect(compareInstanceKeys(lhs, rhs)).to.eq(0);
  });
  ...
  it("returns more than 0 when `lhs.className` > `rhs.className`", () => {
    ...
    expect(compareInstanceKeys(lhs, rhs)).to.be.gt(0);
  });
