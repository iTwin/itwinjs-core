...
} from "../../presentation-common/content/PropertyValueFormatter";
import {
  DisplayValuesArray,
  DisplayValuesMap,
...
describe("ContentPropertyValueFormatter", () => {
  ...
  it("formats content item value", async () => {
    const descriptor = createTestContentDescriptor({
      fields: [koqField, simplePropField],
    });
  ...
  it("handles merged nested field", async () => {
    ...
    const descriptor = createTestContentDescriptor({
      fields: [nestedContentField, koqField],
    });
    const contentItem = createTestContentItem({
      displayValues: {},
      values: {
...
describe("ContentPropertyValueFormatter", () => {
  ...
  describe("formats primitive", () => {
    ...
    it("'point3d' value", async () => {
      ...
      expect(
        await formatter.formatPropertyValue(field, {
          x: 1.234,
          y: 5.678,
          z: 1.234,
        })
      ).to.be.eq("X: 1.23 Y: 5.68 Z: 1.23");
    });
    ...
    it("'point3d' value", async () => {
      ...
      const value: NavigationPropertyValue = {
        id: "0x1",
