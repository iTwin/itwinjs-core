...
describe("ContentTraverser", () => {
  ...
  describe("traverseContentItem", () => {
    ...
    it("doesn't process content field if `visitor.startCategory` returns `false` for nested category", () => {
      ...
      const childCategory = createTestCategoryDescription({
        name: "child",
    ...
    it("processes primitive value nested under merged nested content item", () => {
      ...
      const item = createTestContentItem({
        values: {},
        displayValues: {},
        mergedFieldNames: [parentField.name],
      });
      traverseContentItem(visitor, descriptor, item);
      expect(spy).to.be.calledOnceWith({
    ...
    it("processes nested content item as struct array value", () => {
      ...
      const primitiveField2 = createTestSimpleContentField({
        name: "primitive2",
        category,
      });
      const parentField = createTestNestedContentField({
        nestedFields: [primitiveField1, primitiveField2],
        category,
      });
      const descriptor = createTestContentDescriptor({
        fields: [parentField],
  ...
  it("processes merged nested content under nested content item", () => {
    ...
    const mergedNestedField = createTestNestedContentField({
      name: "mergedField",
      nestedFields: [primitiveField],
      category,
    });
    const parentField = createTestNestedContentField({
      name: "parentField",
      nestedFields: [mergedNestedField],
      category,
    });
    const descriptor = createTestContentDescriptor({
      fields: [parentField],
...
describe("addFieldHierarchy", () => {
  ...
  it("adds given hierarchy under existing parent", () => {
    const category = createTestCategoryDescription();
    const sibling1 = createTestSimpleContentField({
      name: "sibling1",
      category,
    });
    const sibling2 = createTestSimpleContentField({
      name: "sibling2",
      category,
    });
    const parent = createTestNestedContentField({
      name: "parent",
      category,
      nestedFields: [sibling1, sibling2],
    });
    const list: FieldHierarchy[] = [
      {
        field: parent,
