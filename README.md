...
6. Check for API signature changes: `rush extract-api`. This will update the signature files, located in `common/api`.
    - Be sure that your branch is up to date with `master` (i.e. `git merge origin/master`)
    - Cleanup your cache: `rush clean`, `rush purge`
    - Rebuild the project: `rush udpate`, `rush build`
