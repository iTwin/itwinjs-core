...
const buggyMaliMatchers = [/Mali-G71/, /Mali-G72/, /Mali-G76/];
...
function isIntegratedGraphics(args: {
  ...
  if (
    ...
    integratedIntelGpuMatchers.some((x) => x.test(args.unmaskedRenderer!))
  )
    return true;
...
export class Capabilities {
  ...
  public init(
    ...
    this._maxAntialiasSamples = this._driverBugs.msaaWillHang
      ? 1
      : this._isWebGL2 && undefined !== gl2
      ? gl.getParameter(gl2.MAX_SAMPLES)
      : 1;
    ...
    const allowFloatRender =
      ...
      unmaskedRenderer !== "Mali-G72" &&
      unmaskedRenderer !== "Mali-G76";

    if (
      allowFloatRender &&
      undefined !== this.queryExtensionObject("EXT_float_blend") &&
      this.isTextureRenderable(gl, gl.FLOAT)
    ) {
