  ...
  public override async preExportElement(sourceElement: Element): Promise<void> {
    ...
    if (unresolvedReferences.length > 0) {
      for (const reference of unresolvedReferences) {
        ...
        if (ConcreteEntityIds.isModel(reference)) await this.exporter.exportModel(rawId);
        // FIXME: if it's a model, doesn't its element have to be imported first?
        else if (ConcreteEntityIds.isElement(reference)) await this.exporter.exportElement(rawId);
        else assert(false, "unsupported reference type: ${}, only model and element references are supported");
      }
    }
  }
