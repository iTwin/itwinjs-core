...
describe("ObservableMap", () => {
  ...
  it("should construct from iterable", () => {
    ...
    expect(observable).to.deep.equal(map);
  });
  ...
  it("getOrInsert should return existing and not raise event", () => {
    const map = new ObservableMap<string, string>();
    const listener = new Listener(map);

    listener.expectAdd(() => {
      const v = map.getOrInsert("k", "v");
      expect(v).to.equal("v");
    });

    listener.expectNone(() => {
      const v = map.getOrInsert("k", "x");
      expect(v).to.equal("v");
    });
  });

  it("getOrInsertComputed should compute only when missing", () => {
    const map = new ObservableMap<string, string>();
    const listener = new Listener(map);

    listener.expectAdd(() => {
      let called = false;
      const v = map.getOrInsertComputed("k", () => { called = true; return "v"; });
    ...
    listener.expectNone(() => {
      let called = false;
      const v = map.getOrInsertComputed("k", () => { called = true; return "x"; });
      expect(called).to.be.false;
      expect(v).to.equal("v");
    });
  });

  it("getOrInsert should treat existing undefined as present and not raise event", () => {
    const map = new ObservableMap<string, any>();
    const listener = new Listener(map);
    ...
    listener.expectNone(() => {
      const v = map.getOrInsert("u", "new");
      expect(v).to.equal(undefined);
    });
  });

  it("getOrInsertComputed should not invoke factory when key exists with undefined", () => {
    const map = new ObservableMap<string, any>();
    const listener = new Listener(map);

    map.set("u2", undefined);

    listener.expectNone(() => {
      let called = false;
      const v = map.getOrInsertComputed("u2", () => { called = true; return "x"; });
      expect(called).to.be.false;
      expect(v).to.equal(undefined);
    });
  });

  it("getOrInsert should treat null as a present value", () => {
    const map = new ObservableMap<string, any>();
    const listener = new Listener(map);
