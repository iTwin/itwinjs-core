...
export class InvertedUnits {
  ...
  public async create(
    ...
    const newUnit = (await schema.createInvertedUnit(
      name
    ...
    if (invertsUnit === undefined)
      return {
        errorMessage: `Unable to locate unit ${invertsUnitKey.fullName} in schema ${schema.fullName}.`,
      };

    if (invertsUnit.schemaItemType !== SchemaItemType.Unit)
      return { errorMessage: `${invertsUnit.fullName} is not of type Unit.` };

    newUnit.setInvertsUnit(
      new DelayedPromiseWithProps<SchemaItemKey, Unit>(
        invertsUnitKey,
        async () => invertsUnit
      )
    );
    ...
    if (unitSystem === undefined)
      return {
        errorMessage: `Unable to locate unit system ${unitSystemKey.fullName} in schema ${schema.fullName}.`,
      };

    if (unitSystem.schemaItemType !== SchemaItemType.UnitSystem)
      return {
        errorMessage: `${unitSystemKey.fullName} is not of type Unit System.`,
      };

    newUnit.setUnitSystem(
  ...
  public async createFromProps(
    schemaKey: SchemaKey,
    invertedUnitProps: InvertedUnitProps
  ): Promise<SchemaItemEditResults> {
    const schema = await this._schemaEditor.getSchema(schemaKey);
    if (schema === undefined)
      return {
  ...
  public async setInvertsUnit(
    invertedUnitKey: SchemaItemKey,
    invertsUnitKey: SchemaItemKey
  ): Promise<void> {
    const invertedUnit =
      await this._schemaEditor.schemaContext.getSchemaItem<MutableInvertedUnit>(
        invertedUnitKey
      );
    ...
    if (invertedUnit === undefined)
      throw new ECObjectsError(
        ...
        `Inverted Unit ${invertedUnitKey.fullName} not found in schema context.`
      );

    if (invertedUnit.schemaItemType !== SchemaItemType.InvertedUnit)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaItemType,
        `Expected ${invertedUnitKey.fullName} to be of type Inverted Unit.`
      );

    const invertsUnit =
      await this._schemaEditor.schemaContext.getSchemaItem<Unit>(
        invertsUnitKey
      );
    if (invertsUnit === undefined)
      throw new ECObjectsError(
        ECObjectsStatus.ClassNotFound,
        `Unit ${invertsUnitKey.fullName} not found in schema context.`
      );

    if (invertsUnit.schemaItemType !== SchemaItemType.Unit)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaItemType,
        `Expected ${invertsUnitKey.fullName} to be of type Unit.`
      );

    invertedUnit.setInvertsUnit(
      new DelayedPromiseWithProps<SchemaItemKey, Unit>(
        invertsUnitKey,
        async () => invertsUnit
      )
  ...
  public async setUnitSystem(
    ...
    const unitSystem =
      await this._schemaEditor.schemaContext.getSchemaItem<UnitSystem>(
        unitSystemKey
      );
    if (unitSystem === undefined)
      throw new ECObjectsError(
        ECObjectsStatus.ClassNotFound,
        `Unit ${unitSystemKey.fullName} not found in schema context.`
      );

    if (unitSystem.schemaItemType !== SchemaItemType.UnitSystem)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaItemType,
        `Expected ${unitSystemKey.fullName} to be of type Unit System.`
      );

    invertedUnit.setUnitSystem(
      new DelayedPromiseWithProps<SchemaItemKey, UnitSystem>(
        unitSystemKey,
        async () => unitSystem
      )
