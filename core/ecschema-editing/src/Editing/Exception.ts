...
export type AnyIdentifier =
  | ISchemaIdentifier
  | ISchemaItemIdentifier
  | IClassIdentifier
  | IPropertyIdentifier
  | ICustomAttributeIdentifier
  | IRelationshipConstraintIdentifier
  | IEnumeratorIdentifier;
...
export class SchemaEditingError extends Error {
   ...
   */
  public constructor(
    public readonly errorNumber: ECEditingStatus,
    public readonly identifier: AnyIdentifier,
    public readonly innerError?: AnyEditingError,
    ruleViolations?: AnyDiagnostic[],
    message?: string,
  ) {
    super(message);
    this._ruleViolations = ruleViolations;
    this._schemaKey = identifier.schemaKey, this.generateMessage();
  }
  ...
  private generateMessage() {
    ...
    switch (this.errorNumber) {
      ...
      case ECEditingStatus.SchemaItemNameNotSpecified:
        this.message =
          `Could not create a new ${this._schemaItemId.schemaItemType} in schema ${this._schemaKey.name}. No name was supplied within props.`;
        return;
      ...
      case ECEditingStatus.InvalidEnumeratorType:
        this.message =
          `The Enumeration ${this._enumeratorId.enumeration.fullName} has type ${this._enumeratorId.enumerationType}, while Enumerator ${this._enumeratorId.name} has type ${this._enumeratorId.enumeratorType}.`;
        return;
  ...
  private _createTaskErrorMessage() {
    ...
    switch (this.identifier.typeIdentifier) {
      ...
      case SchemaTypeIdentifiers.ClassIdentifier:
      case SchemaTypeIdentifiers.RelationshipConstraintIdentifier:
        return `While performing task '${
          ECEditingStatus[this.errorNumber]
        }' an error occurred editing ${this.identifier.typeIdentifier} ${this.identifier.name}.`;
      case SchemaTypeIdentifiers.PropertyIdentifier:
        return `While performing task '${
          ECEditingStatus[this.errorNumber]
        }' an error occurred editing ${this.identifier.typeIdentifier} ${this.identifier.fullName}.`;
      default:
        throw new Error("Invalid identifier.");
    }
  }
  ...
  private isSchemaItemIdentifier(identifier: AnyIdentifier) {
    if (
      identifier.typeIdentifier !== SchemaTypeIdentifiers.SchemaItemIdentifier &&
