...
export class PropertyCategories {
  ...
  public async createFromProps(
    schemaKey: SchemaKey,
    propertyCategoryProps: PropertyCategoryProps
  ): Promise<SchemaItemEditResults> {
    const schema = await this._schemaEditor.getSchema(schemaKey);
    if (schema === undefined)
      return {
  ...
  public async setPriority(
    ...
    if (propertyCategory === undefined)
      throw new ECObjectsError(
        ...
        `Property Category ${propCategoryKey.fullName} not found in schema context.`
      );

    if (propertyCategory.schemaItemType !== SchemaItemType.PropertyCategory)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaItemType,
        `Expected ${propCategoryKey.fullName} to be of type Property Category.`
      );
