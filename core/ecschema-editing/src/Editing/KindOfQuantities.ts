...
export class KindOfQuantities {
  ...
  public async createFromProps(
    ...
    const newKoQ = (await schema.createKindOfQuantity(
      koqProps.name
   ...
   */
  public async addPresentationFormat(
    koqKey: SchemaItemKey,
    format: SchemaItemKey,
    isDefault: boolean = false
  ): Promise<void> {
    const kindOfQuantity =
      await this._schemaEditor.schemaContext.getSchemaItem<MutableKindOfQuantity>(
        koqKey
      );
    ...
    if (kindOfQuantity === undefined)
      throw new ECObjectsError(
        ...
        `Entity Class ${koqKey.fullName} not found in schema context.`
      );

    if (kindOfQuantity.schemaItemType !== SchemaItemType.KindOfQuantity)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaItemType,
        `Expected ${koqKey.fullName} to be of type Kind Of Quantity.`
      );

    const presentationFormat =
      await this._schemaEditor.schemaContext.getSchemaItem<Format>(format);
    if (undefined === presentationFormat)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
        `Unable to locate format '${format.fullName}' for the presentation unit on KindOfQuantity ${koqKey.fullName}.`
      );

    if (presentationFormat.schemaItemType !== SchemaItemType.Format)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaItemType,
        `Expected ${presentationFormat.fullName} to be of type Format.`
      );

    kindOfQuantity.addPresentationFormat(presentationFormat, isDefault);
  }

  public async addPresentationOverrideFormat(
    koqKey: SchemaItemKey,
  ...
  public async createFormatOverride(
    ...
    const parentFormat =
      await this._schemaEditor.schemaContext.getSchemaItem<Format>(parent);
    if (undefined === parentFormat)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
        `Unable to locate format '${parent.fullName}' for the presentation unit on KindOfQuantity ${koqKey.fullName}.`
      );

    if (parentFormat.schemaItemType !== SchemaItemType.Format)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaItemType,
        `Expected ${parentFormat.fullName} to be of type Format.`
      );
