...
export class Units {
  ...
  public async create(
    ...
    if (phenomenonItem === undefined)
      return {
        errorMessage: `Unable to locate phenomenon ${phenomenon.fullName} in schema ${schema.fullName}.`,
      };

    if (phenomenonItem.schemaItemType !== SchemaItemType.Phenomenon)
      return {
        errorMessage: `${phenomenon.fullName} is not of type Phenomenon.`,
      };
    ...
    if (unitSystemItem === undefined)
      return {
        errorMessage: `Unable to locate unit system ${unitSystem.fullName} in schema ${schema.fullName}.`,
      };

    if (unitSystemItem.schemaItemType !== SchemaItemType.UnitSystem)
      return {
        errorMessage: `${unitSystem.fullName} is not of type UnitSystem.`,
      };
