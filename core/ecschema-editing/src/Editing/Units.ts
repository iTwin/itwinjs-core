...
import { DelayedPromiseWithProps, IUnitProps, Phenomenon, SchemaItemKey, SchemaItemType, SchemaKey, UnitSystem } from "@itwin/ecschema-metadata";
...
export class Units {
  ...
  public async createFromProps(schemaKey: SchemaKey, unitProps: IUnitProps): Promise<SchemaItemEditResults> {
    const schema = await this._schemaEditor.getSchema(schemaKey);
    if (schema === undefined)
      return { errorMessage: `Schema Key ${schemaKey.toString(true)} not found in context` };
