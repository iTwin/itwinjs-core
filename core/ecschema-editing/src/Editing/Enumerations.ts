  ...
  public async addEnumerator(
    ...
    if (enumeration === undefined)
      throw new ECObjectsError(
        ...
        `Unable to locate Enumeration class ${enumerationKey.fullName}.`
      );
    ...
    if (enumeration.isInt && typeof enumerator.value !== "number")
      throw new ECObjectsError(
        ...
        `The Enumeration ${enumeration.name} has type integer, while ${
          enumerator.name
        } has type ${typeof enumerator.value}.`
      );

    if (enumeration.isString && typeof enumerator.value !== "string")
      throw new ECObjectsError(
        ECObjectsStatus.InvalidPrimitiveType,
        `The Enumeration ${enumeration.name} has type string, while ${
          enumerator.name
        } has type ${typeof enumerator.value}.`
      );
