...
describe("Unit system merger tests", () => {
  it("should merge missing unit system", async () => {
    ...
    expect(mergedUnitSystem).toBeDefined();
    expect(mergedUnitSystem.label).toBe("Imperial");
    expect(mergedUnitSystem.description).toBe("Imperial Unit System");
  });
  ...
  it("should merge unit system with new label and description", async () => {
    ...
    const mergedUnitSystem = await mergedSchema.getItem("testUnitSystem") as UnitSystem;
    expect(mergedUnitSystem).toBeDefined();
    expect(mergedUnitSystem.label).toBe("New Imperial");
    expect(mergedUnitSystem.description).toBe("New Imperial Unit System");
  });
});
