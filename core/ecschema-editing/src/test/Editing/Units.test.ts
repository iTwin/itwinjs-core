...
describe("Units tests", () => {
  ...
  it("try creating Unit to unknown schema, throws error", async () => {
    ...
    await expect(testEditor.units.create(badKey, "testUnit", "testDefinition", phenomenonKey, unitSystemKey)).to.be.eventually.rejected.then(
      function(error) {
  ...
  it("try creating Unit with existing name, throws error", async () => {
    ...
    await expect(testEditor.units.create(testKey, "testUnit", "testDefinition", phenomenonKey, unitSystemKey)).to.be.eventually.rejected.then(
      function(error) {
        expect(error).to.have.property("errorNumber", ECEditingStatus.CreateSchemaItemFailed);
