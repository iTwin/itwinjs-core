...
describe("Inverted Units tests", () => {
  ...
  it("should create a valid Inverted Unit", async () => {
    ...
    expect(await invertedUnit.invertsUnit).toEqual(await testEditor.schemaContext.getSchemaItem(invertsUnitKey));
    expect(invertedUnit.fullName).toEqual("testSchema.testInvertedUnit");
  });
  ...
  it("try creating InvertedUnit in unknown schema, throws error", async () => {
    ...
    await expect(testEditor.invertedUnits.create(badKey, "testInvertedUnit", invertsUnitKey, unitSystemKey)).rejects.toMatchObject({
  ...
  it("try creating InvertedUnit with existing name, throws error", async () => {
    ...
    await expect(testEditor.invertedUnits.create(testKey, "testInvertedUnit", invertsUnitKey, unitSystemKey)).rejects.toMatchObject({
      errorNumber: ECEditingStatus.CreateSchemaItemFailed,
      innerError: {
        message: `InvertedUnit testSchema.testInvertedUnit already exists in the schema ${testKey.name}.`,
  ...
  it("try creating InvertedUnit with unknown invertsUnit, throws error", async () => {
    ...
    await expect(testEditor.invertedUnits.create(testKey, "testInvertedUnit", badKey, unitSystemKey)).rejects.toMatchObject({
      errorNumber: ECEditingStatus.CreateSchemaItemFailed,
      innerError: {
        message: `Unit ${badKey.fullName} could not be found in the schema ${testKey.name}.`,
  ...
  it("try creating InvertedUnit with unknown unitSystem, throws error", async () => {
    ...
    await expect(testEditor.invertedUnits.create(testKey, "testInvertedUnit", invertsUnitKey, badKey)).rejects.toMatchObject({
      errorNumber: ECEditingStatus.CreateSchemaItemFailed,
      innerError: {
        message: `UnitSystem ${badKey.fullName} could not be found in the schema ${testKey.name}.`,
        errorNumber: ECEditingStatus.SchemaItemNotFound,
      },
    });
  });
