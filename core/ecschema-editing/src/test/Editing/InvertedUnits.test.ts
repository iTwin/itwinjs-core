...
describe("Inverted Units tests", () => {
  ...
  it("try creating InvertedUnit in unknown schema, throws error", async () => {
    ...
    await expect(testEditor.invertedUnits.create(badKey, "testInvertedUnit", invertsUnitKey, unitSystemKey)).to.be.eventually.rejected.then(
      function(error) {
  ...
  it("try creating InvertedUnit with existing name, throws error", async () => {
    ...
    await expect(testEditor.invertedUnits.create(testKey, "testInvertedUnit", invertsUnitKey, unitSystemKey)).to.be.eventually.rejected.then(
      function(error) {
        expect(error).to.have.property("errorNumber", ECEditingStatus.CreateSchemaItemFailed);
        expect(error).to.have.nested.property(
          "innerError.message",
          `InvertedUnit testSchema.testInvertedUnit already exists in the schema ${testKey.name}.`,
        );
  ...
  it("try creating InvertedUnit with unknown unitSystem, throws error", async () => {
    ...
    await expect(testEditor.invertedUnits.create(testKey, "testInvertedUnit", invertsUnitKey, badKey)).to.be.eventually.rejected.then(
      function(error) {
        expect(error).to.have.property("errorNumber", ECEditingStatus.CreateSchemaItemFailed);
        expect(error).to.have.nested.property(
          "innerError.message",
          `UnitSystem ${badKey.fullName} could not be found in the schema ${testKey.name}.`,
        );
        expect(error).to.have.nested.property("innerError.errorNumber", ECEditingStatus.SchemaItemNotFound);
      },
    );
