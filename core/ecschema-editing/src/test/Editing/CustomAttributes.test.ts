...
describe("CustomAttribute tests", () => {
  ...
  it("should change custom attribute base class with class derived from", async () => {
    ...
    const caRes = await testEditor.customAttributes.create(
      testKey,
      "testCustomAttribute",
    ...
    const newBaseClassRes = await testEditor.customAttributes.create(
      testKey,
      "newBaseClass",
      CustomAttributeContainerType.AnyProperty,
      "newLabel",
  ...
  it("try adding base class to a custom attribute class where the base class cannot be located, returns error", async () => {
    ...
    await expect(testEditor.customAttributes.setBaseClass(caRes, baseClassKey)).to.be.eventually.rejected.then(function(error) {
      ...
      expect(error).to.have.nested.property(
        "innerError.message",
        `CustomAttributeClass ${baseClassKey.fullName} could not be found in the schema ${testKey.name}.`,
      );
  ...
  it("try changing the custom attribute base class to one that doesn't derive from, returns error", async () => {
    const baseClassRes = await testEditor.customAttributes.create(
      testKey,
      "testBaseClass",
  ...
  it("try creating custom attribute class to unknown schema, throws error", async () => {
    ...
    await expect(testEditor.customAttributes.create(badKey, "testCustomAttribute", CustomAttributeContainerType.StructProperty)).to.be.eventually
  ...
  it("try creating custom attribute class with unknown base class, throws error", async () => {
    const baseClassKey = new SchemaItemKey("testBaseClass", testKey);
    await expect(
      testEditor.customAttributes.create(testKey, "testCustomAttribute", CustomAttributeContainerType.StructProperty, undefined, baseClassKey),
  ...
  it("try creating custom attribute class with existing name, throws error", async () => {
    ...
    await expect(testEditor.customAttributes.create(testKey, "testCustomAttribute", CustomAttributeContainerType.StructProperty)).to.be.eventually
      .rejected.then(function(error) {
        expect(error).to.have.property("errorNumber", ECEditingStatus.CreateSchemaItemFailed);
        expect(error).to.have.nested.property(
          "innerError.message",
          `CustomAttributeClass testSchema.testCustomAttribute already exists in the schema ${testKey.name}.`,
        );
