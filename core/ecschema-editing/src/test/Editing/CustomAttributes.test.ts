...
describe("CustomAttribute tests", () => {
  ...
  it("should create a new customAttribute Class", async () => {
    ...
    expect(testEditor.schemaContext.getSchemaItemSync(customAttributeResult)?.name).toEqual("testCustomAttribute");
  });
  ...
  it("should add a base class to custom attribute class", async () => {
    ...
    expect(await testCA?.baseClass).toEqual(await testEditor.schemaContext.getSchemaItem<CustomAttributeClass>(baseClassRes));
  });
  ...
  it("should change custom attribute base class with class derived from", async () => {
    ...
    expect(await testCA?.baseClass).toEqual(await testEditor.schemaContext.getSchemaItem<CustomAttributeClass>(newBaseClassRes));
  });
  ...
  it("should remove a base class from custom attribute class", async () => {
    ...
    expect(await testCA?.baseClass).toEqual(undefined);
  });
  ...
  it("try adding base class to custom attribute class with different SchemaItemType, returns error", async () => {
    ...
    await expect(testEditor.customAttributes.setBaseClass(caRes, baseClassRes)).rejects.toMatchObject({
      errorNumber: ECEditingStatus.SetBaseClass,
      innerError: {
        message: `Expected ${baseClassRes.fullName} to be of type CustomAttributeClass.`,
  ...
  it("try adding base class to a custom attribute class where the base class cannot be located, returns error", async () => {
    ...
    await expect(testEditor.customAttributes.setBaseClass(caRes, baseClassKey)).rejects.toMatchObject({
      errorNumber: ECEditingStatus.SetBaseClass,
      innerError: {
  ...
  it("try adding base class to non-existing custom attribute class, returns error", async () => {
    ...
    await expect(testEditor.customAttributes.setBaseClass(caKey, baseClassRes)).rejects.toMatchObject({
      errorNumber: ECEditingStatus.SetBaseClass,
      innerError: {
        message: `CustomAttributeClass ${caKey.fullName} could not be found in the schema context.`,
  ...
  it("try changing the custom attribute base class to one that doesn't derive from, returns error", async () => {
    ...
    await expect(testEditor.customAttributes.setBaseClass(caRes, newBaseClassRes)).rejects.toMatchObject({
      errorNumber: ECEditingStatus.SetBaseClass,
      innerError: {
  ...
  it("try creating custom attribute class to unknown schema, throws error", async () => {
    ...
    await expect(testEditor.customAttributes.create(badKey, "testCustomAttribute", CustomAttributeContainerType.StructProperty)).rejects.toMatchObject({
