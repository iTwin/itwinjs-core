...
describe("SchemaEditingError tests", () => {
  ...
  it("Should create proper debug string with inner exception of type SchemaEditingError", async () => {
    ...
    expect(error.toDebugString()).to.equal(
      `ECEditingStatus.AddSchemaReference: While performing task '${
        ECEditingStatus[error.errorNumber]
      }' an error occurred editing Schema ${testKey.name}. Inner error: ECEditingStatus.RuleViolation: Rule violations occurred from Schema TestSchema: ${
        getRuleViolationMessage(diagnostics)
      }`,
    );
  });
  ...
  it("Should create proper debug string with inner exception of type Error", async () => {
    ...
    const error = new SchemaEditingError(
      ECEditingStatus.CreateSchemaItemFailed,
      new ClassId(SchemaItemType.EntityClass, "TestEntity", testKey),
      innerError,
    );
    expect(error.toDebugString()).to.equal(
      `ECEditingStatus.CreateSchemaItemFailed: While performing task '${
        ECEditingStatus[error.errorNumber]
      }' an error occurred editing Class TestSchema.TestEntity. Inner error: Could not create EntityClass testEntityClass for some unknown reason.`,
    );
  });
  ...
  it("Should create correct error message for Schema task error", async () => {
    ...
    expect(error.message).to.equal(
      `While performing task '${ECEditingStatus[error.errorNumber]}' an error occurred editing Schema ${identifier.name}.`,
    );
  });
  ...
  it("Should create correct error message for SchemaItem task error", async () => {
    ...
    expect(error.message).to.equal(
      `While performing task '${ECEditingStatus[error.errorNumber]}' an error occurred editing SchemaItem ${identifier.name}.`,
    );
  });
  ...
  it("Should create correct error message for Class task error", async () => {
    ...
    const error = new SchemaEditingError(ECEditingStatus.CreateElement, identifier, new Error("inner error"));
    expect(error.message).to.equal(
      `While performing task '${ECEditingStatus[error.errorNumber]}' an error occurred editing Class ${identifier.name}.`,
    );
  });
  ...
  it("Should create correct error message for Property task error", async () => {
    ...
    expect(error.message).to.equal(
      `While performing task '${ECEditingStatus[error.errorNumber]}' an error occurred editing Property ${identifier.fullName}.`,
    );
  });
  ...
  it("Should create correct error message for RelationshipConstraint task error", async () => {
    ...
    expect(error.message).to.equal(
      `While performing task '${ECEditingStatus[error.errorNumber]}' an error occurred editing RelationshipConstraint ${identifier.name}.`,
    );
  });
});
