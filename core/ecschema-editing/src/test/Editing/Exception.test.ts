...
describe("SchemaEditingError tests", () => {
  ...
  it("Should create proper debug string with inner exception of type SchemaEditingError", async () => {
    ...
    const error = new SchemaEditingError(SchemaEditType.AddSchemaReference, new SchemaId(testKey), innerError);
    expect(error.toDebugString()).to.equal(`SchemaEditType.AddSchemaReference: While performing task '${error.schemaEditType}' an error occurred editing Schema ${testKey.name}. Inner error: ECEditingStatus.RuleViolation: Rule violations occurred from Schema TestSchema: ${getRuleViolationMessage(diagnostics)}`);
  });
  ...
  it("Should create proper debug string with inner exception of type Error", async () => {
    ...
    const error = new SchemaEditingError(SchemaEditType.CreateSchemaItemFailed, new ClassId(SchemaItemType.EntityClass, "TestEntity", testKey), innerError);
    expect(error.toDebugString()).to.equal(`SchemaEditType.CreateSchemaItemFailed: While performing task '${error.schemaEditType}' an error occurred editing Class TestSchema.TestEntity. Inner error: Could not create EntityClass testEntityClass for some unknown reason.`);
  });

  it("Should create correct error message for Schema task error", async () => {
    const identifier = new SchemaId(testKey);
    const error = new SchemaEditingError(SchemaEditType.CreateElement, identifier, new Error("inner error"));
    expect(error.message).to.equal(`While performing task '${error.schemaEditType}' an error occurred editing Schema ${identifier.name}.`);
  });
  ...
  it("Should create correct error message for SchemaItem task error", async () => {
    ...
    const error = new SchemaEditingError(SchemaEditType.CreateSchemaItemFailed, identifier, new Error("inner error"));
    expect(error.message).to.equal(`While performing task '${error.schemaEditType}' an error occurred editing SchemaItem ${identifier.name}.`);
  });
  ...
  it("Should create correct error message for Class task error", async () => {
    ...
    const error = new SchemaEditingError(SchemaEditType.CreateElement, identifier, new Error("inner error"));
    expect(error.message).to.equal(`While performing task '${error.schemaEditType}' an error occurred editing Class ${identifier.name}.`);
  });
  ...
  it("Should create correct error message for Property task error", async () => {
    ...
    const error = new SchemaEditingError(SchemaEditType.SetPropertyName, identifier, new Error("inner error"));
    expect(error.message).to.equal(`While performing task '${error.schemaEditType}' an error occurred editing Property ${identifier.fullName}.`);
  });
  ...
  it("Should create correct error message for RelationshipConstraint task error", async () => {
    ...
    const error = new SchemaEditingError(SchemaEditType.AddCustomAttributeToConstraint, identifier, new Error("inner error"));
    expect(error.message).to.equal(`While performing task '${error.schemaEditType}' an error occurred editing RelationshipConstraint ${identifier.name}.`);
  });
