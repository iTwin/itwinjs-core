...
import { abstractConstraintMustExistWithMultipleConstraints, atLeastOneConstraintClassDefined, DiagnosticCodes } from "../../../Validation/ECRules";
...
describe("RelationshipConstraintRule tests", () => {
  ...
  describe("atLeastOneConstraintClassDefined rule tests", () => {
    it("constraints contains one concrete class, rule passes", async () => {
      ...
      const sourceResult = atLeastOneConstraintClassDefined(relationship.source);
      for await (const _diagnostic of sourceResult) {
        expect(false, "Rule should have passed").to.be.true;
      }

      const targetResult = atLeastOneConstraintClassDefined(relationship.target);
    ...
    it("source constraint contains no constraint class, rule violated", async () => {
      ...
      for await (const diagnostic of sourceResult) {
        ...
        expect(diagnostic.code).to.equal(DiagnosticCodes.AtLeastOneConstraintClassDefined);
  ...
  describe("abstractConstraintMustExistWithMultipleConstraints", () => {
    it("multiple constraints, abstract constraint exists, rule passes", async () => {
      ...
      const sourceResult = abstractConstraintMustExistWithMultipleConstraints(relationship.source);
      for await (const _diagnostic of sourceResult) {
        expect(false, "Rule should have passed").to.be.true;
      }

      const targetResult = abstractConstraintMustExistWithMultipleConstraints(relationship.target);
      for await (const _diagnostic of targetResult) {
        expect(false, "Rule should have passed").to.be.true;
      }
    });
