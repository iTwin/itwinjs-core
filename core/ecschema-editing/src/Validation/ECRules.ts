...
export const Diagnostics = {
  ...
  ReferenceCyclesNotAllowed: createSchemaDiagnosticClass<[string, string]>(
    getCode(3),
    "Schema '{0}' has reference cycles: {1}"
  ),

  /** EC-100: Required message parameters: childClass.FullName, baseClass.FullName */
  BaseClassIsSealed: createClassDiagnosticClass<[string, string]>(
    DiagnosticCodes.BaseClassIsSealed,
  ...
  IncompatibleUnitPropertyOverride: createPropertyDiagnosticClass<
    [string, string, string, string, string, string, string]
  >(
    DiagnosticCodes.IncompatibleUnitPropertyOverride,
  ...
  AbstractConstraintMustNarrowBaseConstraints: createSchemaItemDiagnosticClass<
    ...
    [string, string, string, string]
  >(
    DiagnosticCodes.AbstractConstraintMustNarrowBaseConstraints,
  ...
  DerivedConstraintsMustNarrowBaseConstraints: createSchemaItemDiagnosticClass<
    RelationshipClass,
    [string, string, string, string]
  >(
    DiagnosticCodes.DerivedConstraintsMustNarrowBaseConstraints,
  ...
  ConstraintClassesDeriveFromAbstractContraint: createSchemaItemDiagnosticClass<
    RelationshipClass,
    [string, string, string, string]
  >(
    DiagnosticCodes.ConstraintClassesDeriveFromAbstractContraint,
...
export const ECRuleSet: IRuleSet = {
  ...
  schemaRules: [validateSchemaReferences],
  classRules: [baseClassIsSealed, baseClassIsOfDifferentType],
  propertyRules: [
...
export function* validateSchemaReferencesSync(
  ...
  for (const schemaRef of schema.references) {
    ...
    if (aliases.has(schemaRef.alias)) {
      ...
      yield new Diagnostics.SchemaRefAliasMustBeUnique(schema, [
        schema.name,
        schemaRef.alias,
        currentRef.name,
        schemaRef.name,
 ...
 * @internal
 */
export async function* incompatibleValueTypePropertyOverride(
  ...
  async function callback(
    ...
    if (!propertyTypesMatch(property, baseProperty)) return;

    const baseType = getPrimitiveType(baseProperty);

    // Return if rule passed
    if (!baseType || primitiveType === baseType) return;

    return new Diagnostics.IncompatibleValueTypePropertyOverride(property, [
      property.class.fullName,
      property.name,
      baseClass.fullName,
      primitiveTypeToString(baseType),
 ...
 * @internal
 */
export async function* incompatibleTypePropertyOverride(
  property: AnyProperty
): AsyncIterable<PropertyDiagnostic<any[]>> {
  if (!property.class.baseClass) return;
  ...
  async function callback(
    ...
    return new Diagnostics.IncompatibleTypePropertyOverride(property, [
      property.class.fullName,
      property.name,
      baseClass.fullName,
      baseProperty.constructor.name,
 ...
 * @internal
 */
export async function* incompatibleUnitPropertyOverride(
  property: AnyProperty
): AsyncIterable<PropertyDiagnostic<any[]>> {
  ...
  async function callback(
    ...
    // return if rule passed
    if (unit.key.matches(baseUnit.key)) return;
 ...
 * @internal
 */
export async function* abstractConstraintMustNarrowBaseConstraints(
  ...
  const sourceResult = await applyAbstractConstraintMustNarrowBaseConstraints(
    ecClass,
    ecClass.source,
    baseRelationship
  );
  if (sourceResult) yield sourceResult;
  const targetResult = await applyAbstractConstraintMustNarrowBaseConstraints(
    ecClass,
    ecClass.target,
    baseRelationship
  );
 ...
 * @internal
 */
export async function* derivedConstraintsMustNarrowBaseConstraints(
  ecClass: RelationshipClass
): AsyncIterable<SchemaItemDiagnostic<RelationshipClass, any[]>> {
  if (!ecClass.baseClass) return;
  ...
  const sourceResult = await applyDerivedConstraintsMustNarrowBaseConstraints(
    ecClass,
    ecClass.source,
    baseRelationship
  );
  if (sourceResult) yield sourceResult;
  const targetResult = await applyDerivedConstraintsMustNarrowBaseConstraints(
    ecClass,
    ecClass.target,
    baseRelationship
  );
...
export async function* atLeastOneConstraintClassDefined(
  ...
  if (
    ...
    yield new Diagnostics.AtLeastOneConstraintClassDefined(constraint, [
      constraintType,
 ...
 * @internal
 */
export async function* enumerationTypeUnsupported(
  enumeration: Enumeration
): AsyncIterable<SchemaItemDiagnostic<Enumeration, any[]>> {
 ...
 * @internal
 */
export async function* mixinAppliedToClassMustDeriveFromConstraint(
  entityClass: EntityClass
): AsyncIterable<SchemaItemDiagnostic<EntityClass, any[]>> {
 ...
 * @internal
 */
export async function* validateCustomAttributeInstance(
...
async function* customAttributeNotOfConcreteClass(
  container: CustomAttributeContainerProps,
  customAttribute: CustomAttribute
): AsyncIterable<CustomAttributeContainerDiagnostic<any[]>> {
...
async function* customAttributeSchemaMustBeReferenced(
  container: CustomAttributeContainerProps,
  customAttribute: CustomAttribute
): AsyncIterable<CustomAttributeContainerDiagnostic<any[]>> {
  const schema = container.schema;
  const nameParts = customAttribute.className.split(".");
  if (nameParts.length === 1 || nameParts[0] === schema.name) return;

  if (schema.references.some((s) => s.name === nameParts[0])) return;

  yield new Diagnostics.CustomAttributeSchemaMustBeReferenced(container, [
    container.fullName,
    customAttribute.className,
  ]);
}
...
async function* customAttributeClassMustExist(
  container: CustomAttributeContainerProps,
  customAttribute: CustomAttribute
): AsyncIterable<CustomAttributeContainerDiagnostic<any[]>> {
  const schema = container.schema;
...
async function applyAbstractConstraintMustNarrowBaseConstraints(
  ...
  if (await baseConstraint.supportsClass(abstractConstraint)) return;

  const constraintType = constraint.isSource
    ? ECStringConstants.RELATIONSHIP_END_SOURCE
    : ECStringConstants.RELATIONSHIP_END_TARGET;
  return new Diagnostics.AbstractConstraintMustNarrowBaseConstraints(ecClass, [
...
async function applyDerivedConstraintsMustNarrowBaseConstraints(
  ecClass: RelationshipClass,
  constraint: RelationshipConstraint,
  baseRelationship: RelationshipClass
): Promise<SchemaItemDiagnostic<RelationshipClass, any[]> | undefined> {
  ...
  for (const classPromise of constraint.constraintClasses) {
    ...
    if (await baseConstraint.supportsClass(constraintClass)) continue;
...
async function applyConstraintClassesDeriveFromAbstractContraint(
  ecClass: RelationshipClass,
  constraint: RelationshipConstraint
): Promise<SchemaItemDiagnostic<RelationshipClass, any[]> | undefined> {
  const abstractConstraint = await getAbstractConstraint(constraint);
