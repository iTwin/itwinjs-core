...
export async function* incompatibleValueTypePropertyOverride(property: AnyProperty): AsyncIterable<PropertyDiagnostic<any[]>> {
  ...
  async function callback(baseClass: ECClass): Promise<PropertyDiagnostic<any[]> | undefined> {
    ...
    return new Diagnostics.IncompatibleValueTypePropertyOverride(property, [property.class.fullName, property.name, baseClass.fullName, primitiveTypeToString(baseType), primitiveType !== undefined ? primitiveTypeToString(primitiveType) : "undefined"]);
  }
