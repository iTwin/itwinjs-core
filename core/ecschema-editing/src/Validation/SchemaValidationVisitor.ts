...
export class SchemaValidationVisitor implements ISchemaPartVisitor {
  ...
  public registerReporter(...reporters: IDiagnosticReporter[]) {
    for (const reporter of reporters) this._reporters.push(reporter);
  }
  ...
  public async visitFullSchema(schema: Schema) {
    for (const index of Object.keys(this._ruleSets)) await this.applySchemaRules(schema, this._ruleSets[index]);
  }
  ...
  public async visitSchemaItem(schemaItem: SchemaItem) {
    for (const index of Object.keys(this._ruleSets)) await this.applySchemaItemRules(schemaItem, this._ruleSets[index]);
  }
  ...
  public async visitClass(ecClass: AnyClass): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyClassRules(ecClass, this._ruleSets[index]);
  }
  ...
  public async visitProperty(property: AnyProperty): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyPropertyRules(property, this._ruleSets[index]);
  }
  ...
  public async visitEntityClass(entity: EntityClass): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyEntityRules(entity, this._ruleSets[index]);
  }
  ...
  public async visitStructClass(struct: StructClass): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyStructRules(struct, this._ruleSets[index]);
  }
  ...
  public async visitMixin(mixin: Mixin): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyMixinRules(mixin, this._ruleSets[index]);
  }
  ...
  public async visitCustomAttributeContainer(container: CustomAttributeContainerProps): Promise<void> {
    ...
    if (undefined === container.customAttributes) return;
  ...
  public async visitKindOfQuantity(koq: KindOfQuantity) {
    for (const index of Object.keys(this._ruleSets)) await this.applyKindOfQuantityRules(koq, this._ruleSets[index]);
  }
  ...
  public async visitFormat(format: Format): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyFormatRules(format, this._ruleSets[index]);
  }
  ...
  public async visitUnit(unit: Unit): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyUnitRules(unit, this._ruleSets[index]);
  }
  ...
  public async visitUnitSystem(unitSystem: UnitSystem): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyUnitSystemRules(unitSystem, this._ruleSets[index]);
  }
  ...
  public async visitPhenomenon(phenomenon: Phenomenon): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyPhenomenonRules(phenomenon, this._ruleSets[index]);
  }
  ...
  public async visitConstant(constant: Constant): Promise<void> {
    for (const index of Object.keys(this._ruleSets)) await this.applyConstantRules(constant, this._ruleSets[index]);
  }

  public async applySchemaRules(schema: Schema, ruleSet: IRuleSet) {
    if (!ruleSet.schemaRules || this.excludeSchemaFromRuleSet(schema, ruleSet)) return;
  ...
  public async applySchemaItemRules(schemaItem: SchemaItem, ruleSet: IRuleSet) {
    if (!ruleSet.schemaItemRules || this.excludeSchemaFromRuleSet(schemaItem.schema, ruleSet)) return;
  ...
  public async applyClassRules(ecClass: AnyClass, ruleSet: IRuleSet) {
    if (!ruleSet.classRules || this.excludeSchemaFromRuleSet(ecClass.schema, ruleSet)) return;
  ...
  public async applyPropertyRules(property: AnyProperty, ruleSet: IRuleSet) {
    if (!ruleSet.propertyRules || this.excludeSchemaFromRuleSet(property.schema, ruleSet)) return;
  ...
  public async applyEntityRules(entityClass: EntityClass, ruleSet: IRuleSet) {
    if (!ruleSet.entityClassRules || this.excludeSchemaFromRuleSet(entityClass.schema, ruleSet)) return;
  ...
  public async applyStructRules(structClass: StructClass, ruleSet: IRuleSet) {
    if (!ruleSet.structClassRules || this.excludeSchemaFromRuleSet(structClass.schema, ruleSet)) return;
  ...
  public async applyMixinRules(mixin: Mixin, ruleSet: IRuleSet) {
    if (!ruleSet.mixinRules || this.excludeSchemaFromRuleSet(mixin.schema, ruleSet)) return;
  ...
  public async applyRelationshipRules(relationship: RelationshipClass, ruleSet: IRuleSet) {
    if (!ruleSet.relationshipRules || this.excludeSchemaFromRuleSet(relationship.schema, ruleSet)) return;

    for (const rule of ruleSet.relationshipRules) {
      const result = rule(relationship);
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.relationshipRuleSuppressions
          : undefined;
  ...
  public async applyRelationshipConstraintRules(constraint: RelationshipConstraint, ruleSet: IRuleSet) {
    if (!ruleSet.relationshipConstraintRules || this.excludeSchemaFromRuleSet(constraint.schema, ruleSet)) return;

    for (const rule of ruleSet.relationshipConstraintRules) {
      const result = rule(constraint);
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.relationshipConstraintRuleSuppressions
          : undefined;
  ...
  public async applyCustomAttributeRules(customAttribute: CustomAttributeClass, ruleSet: IRuleSet) {
    if (!ruleSet.customAttributeClassRules || this.excludeSchemaFromRuleSet(customAttribute.schema, ruleSet)) return;

    for (const rule of ruleSet.customAttributeClassRules) {
      const result = rule(customAttribute);
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.customAttributeRuleSuppressions
          : undefined;
  ...
  public async applyCustomAttributeContainerRules(container: CustomAttributeContainerProps, ruleSet: IRuleSet) {
    if (!ruleSet.customAttributeContainerRules || this.excludeSchemaFromRuleSet(container.schema, ruleSet)) return;

    for (const rule of ruleSet.customAttributeContainerRules) {
      const result = rule(container);
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.customAttributeContainerSuppressions
          : undefined;
  ...
  public async applyCustomAttributeInstanceRules(
    container: CustomAttributeContainerProps,
    customAttribute: CustomAttribute,
    ruleSet: IRuleSet
  ) {
    if (!ruleSet.customAttributeInstanceRules || this.excludeSchemaFromRuleSet(container.schema, ruleSet)) return;

    for (const rule of ruleSet.customAttributeInstanceRules) {
      const result = rule(container, customAttribute);
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.customAttributeInstanceSuppressions
          : undefined;
        await this.reportDiagnostics(diagnostic, container, suppressions);
      }
    }
  ...
  public async applyEnumerationRules(enumeration: Enumeration, ruleSet: IRuleSet) {
    if (!ruleSet.enumerationRules || this.excludeSchemaFromRuleSet(enumeration.schema, ruleSet)) return;

    for (const rule of ruleSet.enumerationRules) {
      const result = rule(enumeration);
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.enumerationRuleSuppressions
          : undefined;
  ...
  public async applyKindOfQuantityRules(kindOfQuantity: KindOfQuantity, ruleSet: IRuleSet) {
    if (!ruleSet.kindOfQuantityRules || this.excludeSchemaFromRuleSet(kindOfQuantity.schema, ruleSet)) return;
  ...
  public async applyPropertyCategoryRules(propertyCategory: PropertyCategory, ruleSet: IRuleSet) {
    if (!ruleSet.propertyCategoryRules || this.excludeSchemaFromRuleSet(propertyCategory.schema, ruleSet)) return;

    for (const rule of ruleSet.propertyCategoryRules) {
      const result = rule(propertyCategory);
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.propertyCategoryRuleSuppressions
          : undefined;
  ...
  public async applyFormatRules(format: Format, ruleSet: IRuleSet) {
    if (!ruleSet.formatRules || this.excludeSchemaFromRuleSet(format.schema, ruleSet)) return;
  ...
  public async applyUnitRules(unit: Unit, ruleSet: IRuleSet) {
    if (!ruleSet.unitRules || this.excludeSchemaFromRuleSet(unit.schema, ruleSet)) return;
  ...
  public async applyInvertedUnitRules(invertedUnit: InvertedUnit, ruleSet: IRuleSet) {
    if (!ruleSet.invertedUnitRules || this.excludeSchemaFromRuleSet(invertedUnit.schema, ruleSet)) return;

    for (const rule of ruleSet.invertedUnitRules) {
      const result = rule(invertedUnit);
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.invertedUnitRuleSuppressions
          : undefined;
  ...
  public async applyUnitSystemRules(unitSystem: UnitSystem, ruleSet: IRuleSet) {
    if (!ruleSet.unitSystemRules || this.excludeSchemaFromRuleSet(unitSystem.schema, ruleSet)) return;
  ...
  public async applyPhenomenonRules(phenomenon: Phenomenon, ruleSet: IRuleSet) {
    if (!ruleSet.phenomenonRules || this.excludeSchemaFromRuleSet(phenomenon.schema, ruleSet)) return;
  ...
  public async applyConstantRules(constant: Constant, ruleSet: IRuleSet) {
    if (!ruleSet.constantRules || this.excludeSchemaFromRuleSet(constant.schema, ruleSet)) return;
  ...
  private findSuppressionRule<T extends AnyECType, U = {}>(
    suppressionSet: Array<IRuleSuppressionMap<T, U>>,
    code: string
  ): ISuppressionRule<T, U> | undefined {
    if (!suppressionSet) return;
  ...
  private excludeSchemaFromRuleSet(schema: Schema, ruleSet: IRuleSet): boolean {
    if (!ruleSet.schemaExclusionSet) return false;

    return ruleSet.schemaExclusionSet.includes(schema.name);
  }

  private async reportDiagnostics<T extends AnyECType, U = {}>(
    diagnostic: AnyDiagnostic,
    ecType: T,
    suppressionMap: Array<IRuleSuppressionMap<T, U>> | undefined
  ) {
