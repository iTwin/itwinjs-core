...
export class SchemaValidationVisitor implements ISchemaPartVisitor {
  ...
  public registerRuleSet(ruleSet: IRuleSet) {
    if (undefined !== this.ruleSets[ruleSet.name])
      throw new Error(
        `A RuleSet with the name '${ruleSet.name}' has already been registered.`
      );
   ...
   */
  public async visitRelationshipClass(
   ...
   */
  public async visitRelationshipConstraint(
    constraint: RelationshipConstraint
  ): Promise<void> {
    for (const index of Object.keys(this._ruleSets))
      await this.applyRelationshipConstraintRules(
        constraint,
        this._ruleSets[index]
      );
  }
   ...
   */
  public async visitCustomAttributeClass(
    customAttribute: CustomAttributeClass
  ): Promise<void> {
    for (const index of Object.keys(this._ruleSets))
      await this.applyCustomAttributeRules(
        customAttribute,
        this._ruleSets[index]
      );
  }
   ...
   */
  public async visitCustomAttributeContainer(
    container: CustomAttributeContainerProps
  ): Promise<void> {
    for (const index of Object.keys(this._ruleSets))
      await this.applyCustomAttributeContainerRules(
        container,
        this._ruleSets[index]
      );
    ...
    for (const [, customAttribute] of container.customAttributes) {
      for (const index of Object.keys(this._ruleSets)) {
        await this.applyCustomAttributeInstanceRules(
          container,
          customAttribute,
          this._ruleSets[index]
        );
    ...
    for (const rule of ruleSet.schemaRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.schemaRuleSuppressions
          : undefined;
  ...
  public async applySchemaItemRules(schemaItem: SchemaItem, ruleSet: IRuleSet) {
    if (
      !ruleSet.schemaItemRules ||
      this.excludeSchemaFromRuleSet(schemaItem.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.schemaItemRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.schemaItemRuleSuppressions
          : undefined;
  ...
  public async applyClassRules(ecClass: AnyClass, ruleSet: IRuleSet) {
    if (
      !ruleSet.classRules ||
      this.excludeSchemaFromRuleSet(ecClass.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.classRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.classRuleSuppressions
          : undefined;
  ...
  public async applyPropertyRules(property: AnyProperty, ruleSet: IRuleSet) {
    if (
      ...
      this.excludeSchemaFromRuleSet(property.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.propertyRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.propertyRuleSuppressions
          : undefined;
  ...
  public async applyEntityRules(entityClass: EntityClass, ruleSet: IRuleSet) {
    if (
      !ruleSet.entityClassRules ||
      this.excludeSchemaFromRuleSet(entityClass.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.entityClassRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.entityRuleSuppressions
          : undefined;
  ...
  public async applyStructRules(structClass: StructClass, ruleSet: IRuleSet) {
    if (
      !ruleSet.structClassRules ||
      this.excludeSchemaFromRuleSet(structClass.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.structClassRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.structRuleSuppressions
          : undefined;
  ...
  public async applyMixinRules(mixin: Mixin, ruleSet: IRuleSet) {
    if (
      !ruleSet.mixinRules ||
      this.excludeSchemaFromRuleSet(mixin.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.mixinRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.mixinRuleSuppressions
          : undefined;
  ...
  public async applyRelationshipRules(
    relationship: RelationshipClass,
    ruleSet: IRuleSet
  ) {
    if (
      !ruleSet.relationshipRules ||
      this.excludeSchemaFromRuleSet(relationship.schema, ruleSet)
    )
      return;
  ...
  public async applyRelationshipConstraintRules(
    constraint: RelationshipConstraint,
    ruleSet: IRuleSet
  ) {
    if (
      !ruleSet.relationshipConstraintRules ||
      this.excludeSchemaFromRuleSet(constraint.schema, ruleSet)
    )
      return;
  ...
  public async applyCustomAttributeRules(
    customAttribute: CustomAttributeClass,
    ruleSet: IRuleSet
  ) {
    if (
      !ruleSet.customAttributeClassRules ||
      this.excludeSchemaFromRuleSet(customAttribute.schema, ruleSet)
    )
      return;
  ...
  public async applyCustomAttributeContainerRules(
    container: CustomAttributeContainerProps,
    ruleSet: IRuleSet
  ) {
    if (
      !ruleSet.customAttributeContainerRules ||
      this.excludeSchemaFromRuleSet(container.schema, ruleSet)
    )
      return;
  ...
  public async applyCustomAttributeInstanceRules(
    ...
    if (
      !ruleSet.customAttributeInstanceRules ||
      this.excludeSchemaFromRuleSet(container.schema, ruleSet)
    )
      return;
  ...
  public async applyEnumerationRules(
    enumeration: Enumeration,
    ruleSet: IRuleSet
  ) {
    if (
      !ruleSet.enumerationRules ||
      this.excludeSchemaFromRuleSet(enumeration.schema, ruleSet)
    )
      return;
  ...
  public async applyKindOfQuantityRules(
    kindOfQuantity: KindOfQuantity,
    ruleSet: IRuleSet
  ) {
    if (
      !ruleSet.kindOfQuantityRules ||
      this.excludeSchemaFromRuleSet(kindOfQuantity.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.kindOfQuantityRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.koqRuleSuppressions
          : undefined;
  ...
  public async applyPropertyCategoryRules(
    propertyCategory: PropertyCategory,
    ruleSet: IRuleSet
  ) {
    if (
      !ruleSet.propertyCategoryRules ||
      this.excludeSchemaFromRuleSet(propertyCategory.schema, ruleSet)
    )
      return;
  ...
  public async applyFormatRules(format: Format, ruleSet: IRuleSet) {
    if (
      !ruleSet.formatRules ||
      this.excludeSchemaFromRuleSet(format.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.formatRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.formatRuleSuppressions
          : undefined;
  ...
  public async applyUnitRules(unit: Unit, ruleSet: IRuleSet) {
    if (
      !ruleSet.unitRules ||
      this.excludeSchemaFromRuleSet(unit.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.unitRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.unitRuleSuppressions
          : undefined;
  ...
  public async applyInvertedUnitRules(
    invertedUnit: InvertedUnit,
    ruleSet: IRuleSet
  ) {
    if (
      !ruleSet.invertedUnitRules ||
      this.excludeSchemaFromRuleSet(invertedUnit.schema, ruleSet)
    )
      return;
  ...
  public async applyUnitSystemRules(unitSystem: UnitSystem, ruleSet: IRuleSet) {
    if (
      !ruleSet.unitSystemRules ||
      this.excludeSchemaFromRuleSet(unitSystem.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.unitSystemRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.unitSystemRuleSuppressions
          : undefined;
  ...
  public async applyPhenomenonRules(phenomenon: Phenomenon, ruleSet: IRuleSet) {
    if (
      !ruleSet.phenomenonRules ||
      this.excludeSchemaFromRuleSet(phenomenon.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.phenomenonRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.phenomenonRuleSuppressions
          : undefined;
  ...
  public async applyConstantRules(constant: Constant, ruleSet: IRuleSet) {
    if (
      !ruleSet.constantRules ||
      this.excludeSchemaFromRuleSet(constant.schema, ruleSet)
    )
      return;
    ...
    for (const rule of ruleSet.constantRules) {
      ...
      for await (const diagnostic of result) {
        const suppressions = this._ruleSuppressionSet
          ? this._ruleSuppressionSet.constantRuleSuppressions
          : undefined;
  ...
  private async reportDiagnostics<T extends AnyECType, U = {}>(
    ...
    if (suppressionMap) {
      const suppressRule = this.findSuppressionRule(
        suppressionMap,
        diagnostic.code
      );
      if (suppressRule) {
