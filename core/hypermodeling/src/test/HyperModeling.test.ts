...
describe("Package initialization", () => {
  ...
  it("throws if not initialized", () => {
    expect(() => HyperModeling.namespace).to.throw(
...
describe("Package configuration", () => {
  ...
  it("replaces configuration", () => {
    ...
    test({
      markers: {
        ...
        ignoreCategorySelector: undefined,
        hiddenSectionTypes: [SectionType.Elevation, SectionType.Plan],
      },
    });
    test({
      graphics: {
        ...
        hideSectionGraphics: undefined,
        hideSheetAnnotations: true,
      },
    });
  });
  ...
  it("updates configuration", () => {
    ...
    test({ markerHandler: handler }, { markerHandler: handler });

    test(
      {
        markers: {
          ignoreModelSelector: true,
          hiddenSectionTypes: [SectionType.Elevation],
        },
      },
      {
  ...
  it("updates marker configuration via key-in", async () => {
    ...
    await test("hypermodeling marker default config model=0", {
      ignoreModelSelector: true,
    });
    await test("hypermodeling marker default config cat=0", {
      ignoreModelSelector: true,
      ignoreCategorySelector: true,
    });
    await test("hypermodeling marker default config m=1 c=1", {
      ignoreModelSelector: false,
      ignoreCategorySelector: false,
    });
    await test("hypermodeling marker default config", {});
    await test("hypermodeling marker default config hidden=pe", {
      hiddenSectionTypes: [SectionType.Plan, SectionType.Elevation],
    });
    await test("hypermodeling marker default config h=@#$abcsxyz123", {
  ...
  it("updates graphics configuration via key-in", async () => {
    ...
    await test("hypermodeling graphics config drawing=0", {
      hideSectionGraphics: true,
    });
    await test("hypermodeling graphics config sh=0", {
      hideSheetAnnotations: true,
      hideSectionGraphics: true,
    });
    await test("hypermodeling graphics config d=1 s=1", {});
    await test("hypermodeling graphics config clip=0", { ignoreClip: true });
    await test("hypermodeling graphics config boundaries=1", {
      ignoreClip: true,
      debugClipVolumes: true,
    });
