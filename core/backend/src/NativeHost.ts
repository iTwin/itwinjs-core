...
class NativeAppHandler extends IpcHandler implements NativeAppFunctions {
  ...
  public async overrideInternetConnectivity(
    by: OverriddenBy,
  ...
  public async downloadBriefcase(
    ...
    const checkAbort = () => {
      ...
      return job && (job.request as any).abort === 1
    ...
    if (reportProgress) {
      const progressCallback: ProgressFunction = (loaded, total) => {
        IpcHost.send(`nativeApp.progress-${request.iModelId}`, {
          loaded,
          total,
        });
        return checkAbort();
      };
      args.onProgress = throttleProgressCallback(
        progressCallback,
        checkAbort,
        progressInterval
      );
  ...
  public async requestCancelDownloadBriefcase(
    fileName: string
  ): Promise<boolean> {
    const job = Downloads.isInProgress(fileName);
    if (job) (job.request as any).abort = 1;
  ...
  public async getCachedBriefcases(
    iModelId?: GuidString
  ): Promise<LocalBriefcaseProps[]> {
  ...
  public async storageGetValueType(
    storageId: string,
    key: string
  ): Promise<
...
export class NativeHost {
  ...
  public static readonly onInternetConnectivityChanged = new BeEvent<
  ...
  public static get appSettingsCacheDir(): string {
    return (this._appSettingsCacheDir ??= join(
      IModelHost.cacheDir,
      "appSettings"
    ));
  }
  ...
  public static notifyNativeFrontend<T extends keyof NativeAppNotifications>(
    methodName: T,
    ...args: Parameters<NativeAppNotifications[T]>
  ) {
  ...
  public static async startup(opt?: NativeHostOpts): Promise<void> {
    if (!this.isValid) {
      this._isValid = true;
      this.onInternetConnectivityChanged.addListener(
        (status: InternetConnectivityStatus) =>
          NativeHost.notifyNativeFrontend(
    ...
    if (IpcHost.isValid)
      // for tests, we use NativeHost but don't have a frontend
   ...
   * @internal
   */
  public static overrideInternetConnectivity(
    _overridenBy: OverriddenBy,
    status: InternetConnectivityStatus
  ): void {
