...
export function initializeTracing(enableOpenTelemetry: boolean = false) {
  RpcInvocation.runActivity = async () => RpcTrace.run(RpcTrace.expectCurrentActivity, async () => {}); // redirect the invocation processing to the tracer
  ...
  if (enableOpenTelemetry) {
    try {
      ...
      RpcInvocation.runActivity = async () => RpcTrace.runWithSpan(RpcTrace.expectCurrentActivity, async () => {}); // wrap invocation in an OpenTelemetry span in addition to RpcTrace
    } catch (e) {
