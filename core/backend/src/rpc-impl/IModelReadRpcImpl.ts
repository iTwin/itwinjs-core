...
export class IModelReadRpcImpl extends RpcInterface implements IModelReadRpcInterface {
  ...
  public async getClassHierarchy(tokenProps: IModelRpcProps, classFullName: string): Promise<string[]> {
    ...
    while (true) {
      ...
      if (!classMetaData.baseClasses || classMetaData.baseClasses.length === 0) break;
  ...
  public async getViewThumbnail(_tokenProps: IModelRpcProps, _viewId: string): Promise<Uint8Array> {
    ...
    if (undefined === thumbnail || 0 === thumbnail.image.length) throw new NoContentError();
    ...
    new Uint32Array(val.buffer, 0, 4).set([
      ...
      thumbnail.format === "jpeg" ? ImageSourceFormat.Jpeg : ImageSourceFormat.Png,
      thumbnail.width,
      thumbnail.height,
