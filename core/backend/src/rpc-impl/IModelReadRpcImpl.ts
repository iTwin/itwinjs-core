...
class ViewStateRequestMemoizer extends PromiseMemoizer<CustomViewState3dProps> {
  ...
  public static async perform(
    props: ViewStateRequestProps
  ): Promise<CustomViewState3dProps> {
    if (!this._instance) this._instance = new ViewStateRequestMemoizer();
  ...
  private async perform(
    ...
    if (memo.isPending) throw new RpcPendingResponse(); // eslint-disable-line deprecation/deprecation
 ...
 * @internal
 */
export class IModelReadRpcImpl
  extends RpcInterface
  implements IModelReadRpcInterface
{
  // eslint-disable-line deprecation/deprecation

  public static register() {
    RpcManager.registerImpl(IModelReadRpcInterface, IModelReadRpcImpl);
  }
  ...
  public async getCustomViewState3dData(
    ...
    return ViewStateRequestMemoizer.perform({
      accessToken,
      tokenProps,
      options,
    });
  ...
  public async hydrateViewState(
    tokenProps: IModelRpcProps,
    options: HydrateViewStateRequestProps
  ): Promise<HydrateViewStateResponseProps> {
    const iModelDb = await RpcBriefcaseUtility.findOpenIModel(
  ...
  public async querySubCategories(
    tokenProps: IModelRpcProps,
    compressedCategoryIds: CompressedId64Set
  ): Promise<SubCategoryResultRow[]> {
    const iModelDb = await RpcBriefcaseUtility.findOpenIModel(
      currentActivity().accessToken,
      tokenProps
    );
    const decompressedIds = CompressedId64Set.decompressArray(
  ...
  public async queryRows(
    ...
    if (iModelDb.isReadonly && request.usePrimaryConn === true) {
      Logger.logWarning(
        ...
        "usePrimaryConn is only supported on imodel that is opened in read/write mode. The option will be ignored.",
        request
      );
  ...
  public async queryModelRanges(
    ...
    return results
      .filter((x) => x.status === IModelStatus.Success)
      .map((x) => x.extents);
  }
  ...
  public async getModelProps(
    ...
    for (const id of modelIds) {
      try {
        ...
        if (modelIds.size === 1) throw error; // if they're asking for more than one model, don't throw on error.
      }
    }
  ...
  public async getElementProps(
    ...
    for (const id of elementIds) {
      try {
        ...
        if (elementIds.size === 1) throw error; // if they're asking for more than one element, don't throw on error.
      }
    }
    return elementProps;
  }

  public async loadElementProps(
    tokenProps: IModelRpcProps,
    ...
    if (typeof identifier === "string") {
      if (Id64.isId64(identifier)) props.id = identifier;
      else props.federationGuid = identifier;
    } else {
  ...
  public async getAllCodeSpecs(tokenProps: IModelRpcProps): Promise<any[]> {
    ...
    iModelDb.withPreparedStatement(
      "SELECT ECInstanceId AS id, name, jsonProperties FROM BisCore.CodeSpec",
      (statement) => {
        for (const row of statement)
          codeSpecs.push({
            id: row.id,
            name: row.name,
            jsonProperties: JSON.parse(row.jsonProperties),
          });
      }
    );
    return codeSpecs;
  ...
  public async getMassPropertiesPerCandidate(
    tokenProps: IModelRpcProps,
    props: MassPropertiesPerCandidateRequestProps
    ...
    const getSingleCandidateMassProperties = async (candidate: string) => {
      try {
        ...
        for (const op of props.operations) {
          const massProperties = await iModelDb.getMassProperties({
            operation: op,
        ...
        let singleCandidateResult: MassPropertiesPerCandidateResponseProps = {
        ...
        if (massPropResults.some((r) => r.status !== BentleyStatus.ERROR)) {
          ...
          for (const r of massPropResults.filter(
            (mpr) => mpr.status !== BentleyStatus.ERROR
          )) {
  ...
  public async getViewThumbnail(
    ...
    new Uint32Array(val.buffer, 0, 4).set([
      ...
      thumbnail.width,
      thumbnail.height,
    ]); // Put the metadata in the first 16 bytes.
  ...
  public async getDefaultViewId(
    ...
    if (undefined === blob || 8 !== blob.length) return Id64.invalid;

    const view = new Uint32Array(blob.buffer);
    return Id64.fromUint32Pair(view[0], view[1]);
  }
  public async getSpatialCategoryId(
    tokenProps: IModelRpcProps,
    categoryName: string
    ...
    const dictionary: DictionaryModel =
      iModelDb.models.getModel<DictionaryModel>(IModel.dictionaryId);
    return SpatialCategory.queryCategoryIdByName(
      iModelDb,
      dictionary.id,
      categoryName
    );
  ...
  public async queryTextureData(
    tokenProps: IModelRpcProps,
    textureLoadProps: TextureLoadProps
  ...
  public async generateElementMeshes(
    iModelToken: IModelRpcProps,
    props: ElementMeshRequestProps
  ): Promise<Uint8Array> {
    const db = await RpcBriefcaseUtility.findOpenIModel(
      currentActivity().accessToken,
      iModelToken
