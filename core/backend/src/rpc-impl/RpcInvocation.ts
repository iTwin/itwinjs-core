...
import {
  CommonLoggerCategory, CURRENT_INVOCATION, IModelError, IModelRpcProps, RpcActivity, RpcConfiguration, RpcInterface, RpcMarshaling,
  RpcNotFoundResponse, RpcOperation, RpcPendingResponse, RpcProtocol, RpcProtocolEvent, RpcRegistry, RpcRequestFulfillment, RpcRequestStatus,
  RpcSerializedValue, SerializedRpcRequest,
...
export class RpcInvocation {
  ...
  public static beginActivity: (activity: RpcActivity) => void = () => { };
  public static endActivity: () => void = () => { };
  ...
  private handleUnknownOperation(error: any): boolean {
    ...
    if (op.interfaceVersion === "CONTROL" && op.operationName === "describeEndpoints") {
      op = this.protocol.configuration.controlChannel.describeEndpoints;
      return true;
    }
  ...
  private async resolve(): Promise<any> {
    ...
    try {
      ...
      return await this.reject(error);
    } finally {
      RpcInvocation.endActivity();
    }
  }
