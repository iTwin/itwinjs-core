...
describe("IModelTransformerHub (#integration)", () => {
  ...
  it("should merge changes made on a branch back to master", async () => {
    ...
    try {
      ...
      const masterDbChangeSets = await IModelHost.hubAccess.downloadChangesets({ requestContext, iModelId: masterIModelId });
      ...
      const replayedDbChangeSets = await IModelHost.hubAccess.downloadChangesets({ requestContext, iModelId: replayedIModelId });
