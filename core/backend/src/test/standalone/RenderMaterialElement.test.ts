...
function removeUndefined(
  assetProps: RenderMaterialAssetProps
): RenderMaterialAssetProps {
...
function defaultBooleans(
  ...
  for (const boolKey of boolKeys)
    if (undefined === assetProps[boolKey]) assetProps[boolKey] = false;
...
describe("RenderMaterialElement", () => {
  ...
  describe("insert", () => {
    function test(
      params: Omit<RenderMaterialElementParams, "paletteName">,
      expected: RenderMaterialAssetProps
    ): RenderMaterialElement {
      const name = `material${++materialNumber}`;
      const paletteName = "palette";
      const id = RenderMaterialElement.insert(
        imodel,
        IModel.dictionaryId,
        name,
        { ...params, paletteName }
      ...
      expect(json.jsonProperties?.materialAssets?.renderMaterial).not.to.be
        .undefined;
      const actual = removeUndefined(
        json.jsonProperties!.materialAssets!.renderMaterial!
      );
    ...
    it("normal map with default values", () => {
      ...
      test(
        { normalMap: { TextureId: id } },
        { Map: { Normal: { TextureId: id } } }
      );
    });
