...
describe("Server-based locks", () => {
  ...
  before(async () => {
    ...
    const args: RequestNewBriefcaseProps = {
      iTwinId: iModelProps.iTwinId,
      iModelId,
    };
    briefcase1Props = await BriefcaseManager.downloadBriefcase({
      accessToken: "test token",
      ...args,
    });
    briefcase2Props = await BriefcaseManager.downloadBriefcase({
