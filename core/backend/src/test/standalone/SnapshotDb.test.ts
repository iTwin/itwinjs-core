...
describe("SnapshotDb.refreshContainer", () => {
  ...
  it("should restart default txn after inactivity", async () => {
    ...
    await clock.tickAsync(tenMinutes + 100);
    expect(defaultTxnSpy.callCount).equal(1);
    await checkpoint.refreshContainer(""); // activates timeout again by calling refresh on the timer.
    expect(defaultTxnSpy.callCount).equal(1);
    await clock.tickAsync(tenMinutes + 100);
    ...
    await checkpoint.refreshContainer("");
    await clock.tickAsync(tenMinutes - 100);
    await checkpoint.refreshContainer("");
    await clock.tickAsync(tenMinutes - 100);
    expect(defaultTxnSpy.callCount).equal(2);
  ...
  it("should refresh V2 checkpoint sasToken", async () => {
    ...
    void expect(checkpoint.refreshContainer("")).to.be.fulfilled;
    ...
    const attachPromise = checkpoint.refreshContainer("");
    const promise2 = checkpoint.refreshContainer(""); // gets copy of first promise
    const promise3 = checkpoint.refreshContainer(""); // "
  ...
  it("should not refreshContainer if SnapshotDb is not a V2 checkpoint", async () => {
    const clock = sinon.useFakeTimers();
    clock.setSystemTime(Date.parse("2021-01-01T00:00:00Z"));
    ...
    await snapshot.refreshContainer("");
