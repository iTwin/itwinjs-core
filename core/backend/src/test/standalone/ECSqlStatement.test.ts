...
import { NavigationValue, QueryConfig, QueryConfigBuilder, QueryParams, QueryRowFormat } from "@itwin/core-common";
...
const selectSingleRow = new QueryConfigBuilder().setLimit({ count: 1, offset: -1 }).config;
async function query(ecdb: ECDb, ecsql: string, params?: QueryParams, config?: QueryConfig, callback?: (row: any) => void) {
  ecdb.saveChanges();
...
async function queryRows(ecdb: ECDb, ecsql: string, params?: QueryParams, config?: QueryConfig) {
  ecdb.saveChanges();
  const rows = [];
  for await (const row of ecdb.query(ecsql, params, QueryRowFormat.UseJsPropertyNames, config)) {
    rows.push(row);
...
async function queryCount(ecdb: ECDb, ecsql: string, params?: QueryParams, config?: QueryConfig): Promise<number> {
  ecdb.saveChanges();
  for await (const row of ecdb.query(`SELECT COUNT(*) FROM (${ecsql})`, params, QueryRowFormat.Array, config)) {
...
describe("ECSqlStatement", () => {
  ...
  it("should page results", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "pagingresultset.ecdb",
      ...
      ecdb.saveChanges();
      for (let i = 1; i < ROW_COUNT; i++) {
        const rowCount = await queryCount(ecdb, "SELECT ECInstanceId, ECClassId, n FROM ts.Foo WHERE n <= ?", new QueryParams().bindInt(1, i));
      ...
      for (let i = 0; i < EXPECTED_ROW_COUNT.length; i++) {
        ready.push(queryRows(ecdb, QUERY, undefined, new QueryConfigBuilder().setLimit({ offset: i * PAGE_SIZE, count: PAGE_SIZE }).config));
      }
    ...
    await using(ECDbTestHelper.createECDb(outDir, "cancelquery.ecdb",
      ...
      const cb = async () => {
        return new Promise<void>(async (resolve, reject) => {
          try {
            for await (const _row of ecdb.query("SELECT * FROM ts.Foo", undefined, QueryRowFormat.UseJsPropertyNames, new QueryConfigBuilder().setRestartToken("tag").config)) {
              rowCount++;
            }
            successful++;
            resolve();
  ...
  it("should use cache statement for query()", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "pagingresultset.ecdb",
      ...
      for (const _testPageSize of [1, 2, 4, 5, 6, 7, 10, ROW_COUNT]) { // eslint-disable-line @typescript-eslint/no-unused-vars
        let rowNo = 1;
        for await (const row of ecdb.query("SELECT n FROM ts.Foo WHERE n != ? and ECInstanceId < ?", new QueryParams().bindInt(1, 123).bindInt(2, 30), QueryRowFormat.UseJsPropertyNames)) {
  ...
  it("concurrent query binding", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "pagingresultset.ecdb",
      ...
      ecdb.saveChanges();
      for await (const row of ecdb.query("SELECT count(*) as cnt FROM ts.Foo WHERE n in (:a, :b, :c)", new QueryParams().bindInt("a", 1).bindInt("b", 2).bindInt("c", 3), QueryRowFormat.UseJsPropertyNames)) {
        assert.equal(row.cnt, 3);
      }
      for await (const row of ecdb.query("SELECT count(*) as cnt FROM ts.Foo WHERE n in (?, ?, ?)", new QueryParams().bindInt(1, 1).bindInt(2, 2).bindInt(3, 3), QueryRowFormat.UseJsPropertyNames)) {
        assert.equal(row.cnt, 3);
      }
      const slm = new SequentialLogMatcher();
      slm.append().error().category("ECDb").message("No parameter index found for parameter name: d.");
      try {
        for await (const row of ecdb.query("SELECT count(*) as cnt FROM ts.Foo WHERE n in (:a, :b, :c)", new QueryParams().bindInt("a", 1).bindInt("b", 2).bindInt("c", 3).bindInt("d", 3), QueryRowFormat.UseJsPropertyNames)) {
          assert.equal(row.cnt, 3);
        }
        assert.isFalse(true);
  ...
  it("check HextoId() and IdToHex() ecsql functions", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "pagingresultset.ecdb",
      ...
      ecdb.saveChanges();
      for await (const row of ecdb.query("SELECT IdToHex(ECInstanceId) as hexId, ECInstanceId, HexToId('0x1') as idhex FROM ts.Foo WHERE n = ?", new QueryParams().bindInt(1, 1), QueryRowFormat.UseJsPropertyNames)) {
  ...
  it("should bind BeGuid", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "pagingresultset.ecdb",
      ...
      for (const guid of guids) {
        assert.equal(await query(ecdb, "SELECT guid FROM ts.Foo WHERE guid=?", new QueryParams().bindString(1, guid), undefined, (row: any) => {
        ...
        assert.equal(await query(ecdb, "SELECT guid FROM ts.Foo WHERE guid=StrToGuid(?)", new QueryParams().bindString(1, guid), undefined, (row: any) => {
          assert.equal(row.guid, guid);
        }), 1);
        assert.equal(await query(ecdb, "SELECT guid FROM ts.Foo WHERE GuidToStr(guid)=?", new QueryParams().bindString(1, guid), undefined, (row: any) => {
          assert.equal(row.guid, guid);
        }), 1);
        assert.equal(await query(ecdb, "SELECT guid FROM ts.Foo WHERE guid=?", new QueryParams().bindBlob(1, guidToUint8Array(guid)), undefined, (row: any) => {
          assert.equal(row.guid, guid);
        }), 1);
        assert.equal(await query(ecdb, "SELECT guid FROM ts.Foo WHERE guid=StrToGuid(?)", new QueryParams().bindString(1, guid), undefined, (row: any) => {
          assert.equal(row.guid, guid);
        }), 1);
        assert.equal(await query(ecdb, "SELECT GuidToStr(guid) as gstr FROM ts.Foo WHERE guid=StrToGuid(?)", new QueryParams().bindString(1, guid), undefined, (row: any) => {
  ...
  it("should bind Ids", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "bindids.ecdb"), async (ecdb: ECDb) => {
      ...
      const verify = async (ecdbToVerify: ECDb, actualRes: ECSqlInsertResult, expectedECInstanceId?: Id64String) => {
        ...
        assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Name FROM ecdbf.ExternalFileInfo WHERE ECInstanceId=?", new QueryParams().bindString(1, expectedId), new QueryConfigBuilder().setLimit({ count: 1, offset: -1 }).config, (row) => {
          assert.equal(row.id, expectedECInstanceId);
          assert.equal(row.className, "ECDbFileInfo.ExternalFileInfo");
          assert.equal(row.name, `${Id64.getLocalId(expectedECInstanceId).toString()}.txt`);
        }), 1);
  ...
  it("should bind numeric and date strings", async () => {
    ...
    await using(ECDbTestHelper.createECDb(outDir, "bindnumericanddatestrings.ecdb",
      ...
      await ecdb.withPreparedStatement(ecsqln, async (stmt: ECSqlStatement) => {
        ...
        stmt.reset();
        stmt.clearBindings();
        assert.equal(await queryCount(ecdb, ecsqln, new QueryParams().bindInt(1, nNum), selectSingleRow), 1);
        ...
        assert.equal(await queryCount(ecdb, ecsqln, new QueryParams().bindString(1, nStr), selectSingleRow), 1);
        ...
        assert.equal(await queryCount(ecdb, ecsqln, new QueryParams().bindString(1, nDt), selectSingleRow), 0);
        ...
        assert.equal(await queryCount(ecdb, ecsqln, new QueryParams().bindString(1, nHexStr), selectSingleRow), 0);
      ...
      await ecdb.withPreparedStatement(ecsqldt, async (stmt: ECSqlStatement) => {
        ...
        assert.equal(await ecdb.queryRowCount(ecsqldt, new QueryParams().bindString(1, dtStr)), 1);
        ...
        assert.equal(await ecdb.queryRowCount(ecsqldt, QueryParams.from([num])), 0);
        ...
        assert.equal(await ecdb.queryRowCount(ecsqldt, QueryParams.from([str])), 0);
        ...
        assert.equal(await ecdb.queryRowCount(ecsqldt, QueryParams.from([hexStr])), 0);
      ...
      await ecdb.withPreparedStatement(ecsqlfooId, async (stmt: ECSqlStatement) => {
        ...
        assert.equal(await ecdb.queryRowCount(ecsqldt, QueryParams.from([dt])), 0);
  ...
  it("should bind numbers", async () => {
    ...
    <ECEntityClass typeName="Foo" modifier="Sealed">
      ...
      assert.equal(await query(ecdb, "SELECT D,I,L,S FROM Test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), { limit: { count: 1 } }, (row: any) => {
        assert.equal(row.d, doubleVal);
        assert.equal(row.i, 3);
        assert.equal(row.l, 3);
        assert.equal(row.s, "3.5");
      ...
      assert.equal(await query(ecdb, "SELECT D,I,L,S FROM Test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), selectSingleRow, (row: any) => {
        assert.equal(row.d, smallIntVal);
        assert.equal(row.i, smallIntVal);
        assert.equal(row.l, smallIntVal);
        assert.equal(row.s, "3");
      ...
      assert.equal(await query(ecdb, "SELECT CAST(D AS TEXT) d,CAST(I AS TEXT) i,CAST(L AS TEXT) l,S FROM Test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), selectSingleRow, (row: any) => {
        assert.equal(row.d, largeUnsafeNumberHexStr);
        assert.equal(row.i, largeUnsafeNumberHexStr);
        assert.equal(row.l, largeUnsafeNumberHexStr);
        assert.equal(row.s, largeUnsafeNumberHexStr);
      ...
      assert.equal(await query(ecdb, "SELECT CAST(I AS TEXT) i, CAST(L AS TEXT) l,S FROM Test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), selectSingleRow, (row: any) => {
        assert.equal(row.i, largeNegUnsafeNumberStr);
        assert.equal(row.l, largeNegUnsafeNumberStr);
        assert.equal(row.s, largeNegUnsafeNumberStr);
      }), 1);
      ...
      await query(ecdb, "SELECT D,I,L,S FROM Test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), selectSingleRow, (row: any) => {
        assert.equal(row.d, largeNegSafeNumber);
        assert.equal(row.i, largeNegSafeNumber);
        assert.equal(row.l, largeNegSafeNumber);
        assert.equal(row.s, largeNegSafeNumberStr);
    ...
    <ECEntityClass typeName="Foo" modifier="Sealed">
      ...
      const verify = async (expectedId: Id64String) => {
        await ecdb.withPreparedStatement("SELECT Bl,Bo,D,Dt,I,P2d,P3d,S,Struct.Bl s_bl,Struct.Bo s_bo,Struct.D s_d,Struct.Dt s_dt,Struct.I s_i,Struct.P2d s_p2d,Struct.P3d s_p3d,Struct.S s_s FROM test.Foo WHERE ECInstanceId=?", async (stmt: ECSqlStatement) => {
          ...
          assert.equal(await query(ecdb, "SELECT Bl,Bo,D,Dt,I,P2d,P3d,S,Struct.Bl s_bl,Struct.Bo s_bo,Struct.D s_d,Struct.Dt s_dt,Struct.I s_i,Struct.P2d s_p2d,Struct.P3d s_p3d,Struct.S s_s FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([expectedId]), selectSingleRow, (row1: any) => {
    ...
    await using(ECDbTestHelper.createECDb(outDir, "bindstructs.ecdb",
      ...
      const verify = async (expectedId: Id64String) => {
        await ecdb.withPreparedStatement("SELECT Struct FROM test.Foo WHERE ECInstanceId=?", async (stmt: ECSqlStatement) => {
          ...
          assert.equal(await query(ecdb, "SELECT Struct FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([expectedId]), selectSingleRow, (row1: any) => {
  ...
  it("should bind arrays", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "bindarrays.ecdb",
      ...
      const verify = async (expectedId: Id64String) => {
        ...
        assert.equal(await query(ecdb, "SELECT I_Array, Dt_Array, Addresses FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([expectedId]), selectSingleRow, (row: any) => {
          // don't know why assert.equal doesn't work on arrays directly
          assert.equal(row.i_Array.length, intArray.length);
          for (let i = 0; i < intArray.length; i++) {
            assert.equal(row.i_Array[i], intArray[i]);
  ...
  it("should bind navigation", async () => {
    ...
    <ECRelationshipClass typeName="ParentHasChildren" modifier="None" strength="embedding">
      ...
      assert.equal(await query(ecdb, "SELECT Name,Parent FROM test.Child ORDER BY Name", QueryParams.from([]), undefined, (row: any) => {
        rowCount2++;
  ...
  it("check ECSqlBinder life time", async () => {
    ...
    <ECEntityClass typeName="Person" modifier="Sealed">
      ...
      let rowCount2: number = 0;
      assert.equal(await query(ecdb, "SELECT ECInstanceId,ECClassId,Name,Age,Location FROM test.Person ORDER BY ECInstanceId", QueryParams.from([]), undefined, (row: any) => {
        rowCount2++;
        if (rowCount2 === 1)
          assert.equal(row.id, id1);
        else
  ...
  it("getRow() with primitives values", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "getprimitives.ecdb",
      ...
      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), { abbreviateBlobs: true, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, id);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, blobVal.slice(0, 1));
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), { abbreviateBlobs: false, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, id);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, blobVal);
      }), 1);
      ...
      assert.equal(await query(ecdb, "SELECT s.ECInstanceId, c.ECInstanceId, c.Name, s.Name FROM meta.ECClassDef c JOIN meta.ECSchemaDef s ON c.Schema.Id=s.ECInstanceId WHERE s.Name='Test' AND c.Name='Foo'", QueryParams.from([]), selectSingleRow, (row: any) => {
        assert.equal(row.id, testSchemaId);
        assert.equal(row.id_1, fooClassId);
      }), 1);
      ...
      assert.equal(await query(ecdb, "SELECT count(*) cnt FROM meta.ECSchemaDef", QueryParams.from([]), selectSingleRow, (row: any) => {
        assert.isDefined(row.cnt);
        assert.equal(typeof (row.cnt), "number");
        assert.equal(row.cnt, 6);
      }), 1);
      ...
      assert.equal(await query(ecdb, "SELECT 1 FROM meta.ECSchemaDef LIMIT 1", QueryParams.from([]), undefined, (row: any) => {
        assert.equal(typeof (row["1"]), "number");
        assert.equal(row["1"], 1);
      }), 1);
      ...
      assert.equal(await query(ecdb, "SELECT NULL FROM meta.ECSchemaDef LIMIT 1", QueryParams.from([]), undefined, (row: any) => {
        assert.equal(Object.entries(row).length, 0);
      }), 1);
    });
  });
  ...
  it("getRow() with abbreviated blobs", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "getblobs.ecdb",
      ...
      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([fullId]), { abbreviateBlobs: true, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, fullId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, singleBlobVal);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([fullId]), { abbreviateBlobs: false, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, fullId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, blobVal);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([fullId]), selectSingleRow, (row: any) => {
        assert.equal(row.id, fullId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, blobVal);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([singleId]), { abbreviateBlobs: true, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, singleId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, singleBlobVal);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([singleId]), { abbreviateBlobs: false, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, singleId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, singleBlobVal);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([singleId]), { ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, singleId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, singleBlobVal);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([emptyId]), { abbreviateBlobs: true, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, emptyId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl.length, 0);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([emptyId]), { abbreviateBlobs: false, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, emptyId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl.length, 0);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([emptyId]), selectSingleRow, (row: any) => {
        assert.equal(row.id, emptyId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl.length, 0);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([nullId]), { abbreviateBlobs: true, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, nullId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, undefined);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([nullId]), { abbreviateBlobs: false, ...selectSingleRow }, (row: any) => {
        assert.equal(row.id, nullId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, undefined);
      }), 1);

      assert.equal(await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([nullId]), selectSingleRow, (row: any) => {
        assert.equal(row.id, nullId);
        assert.equal(row.className, "Test.Foo");
        assert.deepEqual(row.bl, undefined);
      }), 1);
  ...
  it("getRow() with navigation properties and relationships", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "getnavandrels.ecdb",
      ...
      rowCount = 0;
      assert.equal(await query(ecdb, "SELECT Name,Parent.Id,Parent.RelECClassId, Parent.Id myParentId, Parent.RelECClassId myParentRelClassId FROM test.Child ORDER BY Name", QueryParams.from([]), undefined, (row: any) => {
        rowCount++;
        assert.equal(row.name, `Child ${rowCount}`);
        assert.equal(row["parent.id"], parentId);
        assert.equal(row["parent.relClassName"], "Test.ParentHasChildren");
      ...
      assert.equal(await query(ecdb, "SELECT ECInstanceId,ECClassId,SourceECInstanceId,SourceECClassId,TargetECInstanceId,TargetECClassId FROM test.ParentHasChildren WHERE TargetECInstanceId=?", QueryParams.from([childId]), undefined, (row: any) => {
        assert.equal(row.id, childId);
        assert.equal(row.className, "Test.ParentHasChildren");
        assert.equal(row.sourceId, parentId);
        assert.equal(row.sourceClassName, "Test.Parent");
  ...
  it("getRow() with structs", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "getstructs.ecdb",
      ...
      assert.equal(await query(ecdb, "SELECT Struct FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), undefined, (row: any) => {
        assert.deepEqual(row.struct.bl, expectedStruct.bl);
        assert.equal(row.struct.bo, expectedStruct.bo);
        assert.equal(row.struct.d, expectedStruct.d);
        assert.equal(row.struct.dt, expectedStruct.dt);
      ...
      assert.equal(await query(ecdb, "SELECT Struct.Bl, Struct.Bo, Struct.D, Struct.Dt, Struct.I, Struct.P2d, Struct.P3d, Struct.S FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), undefined, (row: any) => {
        assert.deepEqual(row["struct.Bl"], expectedStruct.bl);
        assert.equal(row["struct.Bo"], expectedStruct.bo);
        assert.equal(row["struct.D"], expectedStruct.d);
        assert.equal(row["struct.Dt"], expectedStruct.dt);
  ...
  it("check ec enums", async () => {
    await using(ECDbTestHelper.createECDb(outDir, "ecenums.ecdb",
      ...
      assert.equal(await query(ecdb, "SELECT MyStat,MyStats, MyDomain,MyDomains FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([id]), undefined, (row: any) => {
        assert.equal(row.myStat, 1);
        assert.deepEqual(row.myStats, [1, 2]);
        assert.equal(row.myDomain, "Org");
        assert.deepEqual(row.myDomains, ["Org", "Com"]);
      ...
      assert.equal(await query(ecdb, "SELECT Type,Modifier FROM meta.ECClassDef WHERE Name='Foo'", QueryParams.from([id]), undefined, (row: any) => {
        assert.deepEqual(row, { type: 0, modifier: 2 });
      }), 1);
    });
  });
  ...
  it("check ORed ECEnums", async () => {
    ...
    await using(ECDbTestHelper.createECDb(outDir, "oredecenums.ecdb",
      ...
      assert.equal(await query(ecdb, "SELECT MyColor,MyDomain FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([ids.unored]), undefined, (row: any) => {
        assert.equal(row.myColor, 4);
        assert.equal(row.myDomain, "com");
      }), 1);

      assert.equal(await query(ecdb, "SELECT MyColor,MyDomain FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([ids.ored]), undefined, (row: any) => {
        assert.equal(row.myColor, 5);
        assert.equal(row.myDomain, "gov,com");
      }), 1);

      assert.equal(await query(ecdb, "SELECT MyColor,MyDomain FROM test.Foo WHERE ECInstanceId=?", QueryParams.from([ids.unmatched]), undefined, (row: any) => {
        assert.equal(row.myColor, 9);
        assert.equal(row.myDomain, "gov,de");
      }), 1);
      ...
      assert.equal(await query(ecdb, "SELECT CustomAttributeContainerType caType FROM meta.ECClassDef WHERE Type=meta.ECClassType.CustomAttribute AND Name='DateTimeInfo'", QueryParams.from([ids.unmatched]), undefined, (row: any) => {
        assert.equal(row.caType, 160);
      }), 1);
    });
    assert.isTrue(slm.finishAndDispose());
