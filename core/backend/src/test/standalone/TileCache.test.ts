...
const fakeRpc: RpcActivity = { // eslint-disable-line @typescript-eslint/no-deprecated
  accessToken: "dummy",
...
describe("TileCache open v1", () => {
  ...
  const verifyTileCache = async (dbPath: string) => {
    ...
    expect(tileProps);
    await RpcTrace.run(fakeRpc, async () => tileRpcInterface.generateTileContent(iModel.getRpcProps(), tileProps!.treeId, tileProps!.contentId, tileProps!.guid)); // eslint-disable-line @typescript-eslint/no-deprecated
...
describe("TileCache, open v2", async () => {
  it("should place .Tiles in tempFileBase for V2 checkpoints", async () => {
    ...
    await RpcTrace.run(fakeRpc, async () => { // eslint-disable-line @typescript-eslint/no-deprecated
