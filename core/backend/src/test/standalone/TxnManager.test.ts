...
describe("TxnManager", () => {
  ...
  it("get local changes", async () => {
    ...
    assert.deepEqual(txns.queryLocalChanges([], false), []);
    ...
    assert.deepEqual(txns.queryLocalChanges([], true), e0);
    ...
    assert.deepEqual(txns.queryLocalChanges([], false), e1);
    assert.deepEqual(txns.queryLocalChanges([], true), e1);
    ...
    assert.deepEqual(txns.queryLocalChanges([], true), e3);

    // Saved changes
    imodel.saveChanges("1 deleted");
    assert.deepEqual(txns.queryLocalChanges([], false), e3);
    assert.deepEqual(txns.queryLocalChanges([], true), e3);
  });
});
