...
import {
  ...
  ChannelControl,
  CodeService,
  DefinitionModel, DictionaryModel, DocumentListModel, Drawing, DrawingGraphic, OpenBriefcaseArgs, PullMergeMethod, SpatialCategory, Subject,
} from "../../core-backend";
...
for (const pullMergeMethod of ["Merge", "Rebase"] as PullMergeMethod[]) {
  describe(`IModelWriteTest (PullMergeMethod: ${pullMergeMethod})`, () => {
    ...
    it("Check busyTimeout option", async () => {
      ...
      db.pullMergeMethod = pullMergeMethod;
      db.saveChanges();
    ...
    it("WatchForChanges", async () => {
      ...
      roBC.pullMergeMethod = pullMergeMethod;
    ...
    it("should be able to upgrade a briefcase with an older schema", async () => {
      ...
      iModel.pullMergeMethod = pullMergeMethod;
      ...
      superIModel.pullMergeMethod = pullMergeMethod;
    ...
    it("should set a fake verifyCode for codeService that throws error for operations that affect code, if failed to open codeService ", async () => {
      ...
      briefcaseDb.pullMergeMethod = pullMergeMethod;
      ...
      briefcaseDb2.pullMergeMethod = pullMergeMethod;
