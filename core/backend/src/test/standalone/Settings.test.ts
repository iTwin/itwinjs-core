...
import {
  ...
  SettingSchemaGroup,
  SettingsSchemas,
} from "../../workspace/SettingsSchemas";
...
describe("Settings", () => {
  ...
  const app1: SettingSchemaGroup = {
    ...
    properties: {
      "app1/sub1": {
        type: "string",
        enum: ["va1", "alt1"],
        enumDescriptions: ["descr1", "descr2"],
        default: "val1",
  ...
  it("settings priorities", () => {
    ...
    ).to.throw("Use IModelSettings");

    expect(settings.getString("app1/sub1")).equals(
      imodel2Settings["app1/sub1"]
    );
    expect(settings.getString("app2/setting6")).equals(
      iTwinSettings["app2/setting6"]
    );
    expect(settings.getString("app1/setting5")).equals(
      app1Settings["app1/setting5"]
    ); // comes from app settings
    expect(settings.getSetting<any>("app1/sub2").arr).deep.equals(
      imodel2Settings["app1/sub2"].arr
    );
    ...
    expect(settings.getObject("app3/obj")).deep.equals(
      iTwinSettings["app3/obj"]
    ...
    expect(settings.getString("app1/strVal")).equals(
      app1.properties["app1/strVal"].default
    );
    ...
    (app1.properties["app1/strVal"] as Mutable<SettingSchema>).default =
      "new default";
    SettingsSchemas.addGroup(app1);
    ...
    expect(inspect[0]).to.deep.equal({
      value: "imodel2 value",
      dictionary: "iModel2.setting.json",
      priority: 500,
    });
    expect(inspect[1]).to.deep.equal({
      value: "imodel1 value",
      dictionary: "iModel1.setting.json",
      priority: 500,
    });
    expect(inspect[2]).to.deep.equal({
      value: "val3",
      dictionary: "iTwin.setting.json",
      priority: 400,
    });
    expect(inspect[3]).to.deep.equal({
      value: "app1 value",
      dictionary: "app1",
      priority: 200,
    });
    expect(inspect[4]).to.deep.equal({
      value: "val1",
      dictionary: "_default_",
      priority: 0,
    });
  ...
  it("read settings file", () => {
    ...
    const settingFileName =
      IModelTestUtils.resolveAssetFile("test.setting.json5");
    appSettings.addFile(settingFileName, SettingsPriority.application);
    expect(() =>
      iModelSettings.addFile(settingFileName, SettingsPriority.application)
    ).to.throw("Use IModelHost.appSettings");
    ...
    const token = appSettings.getSetting<any>(
      "editor/tokenColorCustomizations"
    )!;
    expect(
      token["Visual Studio Light"].textMateRules[0].settings.foreground
    ).equals("#d16c6c");
    expect(token["Default High Contrast"].comments).equals("#FF0000");
    expect(
      appSettings.getArray<string>("editor/enableFiletypes")!.length
    ).equals(17);
    appSettings.dropDictionary(settingFileName);
  });

  it("IModel persistent settings ", () => {
    const iModelName = IModelTestUtils.prepareOutputFile(
      "IModelSetting",
      "test.bim"
    );
    const iModel2 = IModelTestUtils.createSnapshotFromSeed(
      iModelName,
    ...
    expect(iModel3.workspace.settings.getObject("gcs/databases")).to.deep.equal(
      gcsDbDict["gcs/databases"]
    );
    expect(iModel3.workspace.settings.getString("imodel/setting1")).equal(
      setting1["imodel/setting1"]
    );
    ...
    expect(iModel3.workspace.settings.getString("imodel/setting1")).equal(
      setting1changed["imodel/setting1"]
    );
    ...
    expect(iModel3.workspace.settings.getString("imodel/setting1")).to.be
      .undefined;
    iModel3.close();
  });
});
