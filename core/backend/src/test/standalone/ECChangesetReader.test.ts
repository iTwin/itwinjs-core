...
import { ECChangeUnifierCache, PartialECChangeUnifier } from "../../ChangesetECAdaptor";
...
describe("ECChangesetReader API", async () => {
  ...
  it("openTxn() reads a saved transaction", async () => {
    ...
    using pcu = new PartialECChangeUnifier(rwIModel, ECChangeUnifierCache.createInMemoryCache());
    while (reader.step()) {
