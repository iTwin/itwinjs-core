...
describe("generateElementMeshes", () => {
  ...
  before(() => {
    imodel = SnapshotDb.createEmpty(
      IModelTestUtils.prepareOutputFile("generateElementMeshes", `${Guid.createValue()}.bim`),
      {
        rootSubject: {
  ...
  it("throws if source is not a geometric element", async () => {
    await expect(imodel.nativeDb.generateElementMeshes({ source: "NotAnId" })).rejectedWith(
  ...
  function insertTriangleElement(origin = [0, 0, 0]): string {
    ...
    bldr.appendGeometry(
      Loop.createPolygon([new Point3d(0, 0, 0), new Point3d(1, 0, 0), new Point3d(0, 1, 0), new Point3d(0, 0, 0)])
    );
  ...
  it("produces multiple polyfaces", async () => {
    ...
    bldr.appendGeometry(
      Loop.createPolygon([new Point3d(0, 0, 5), new Point3d(1, 0, 5), new Point3d(0, 1, 5), new Point3d(0, 0, 5)])
    );
