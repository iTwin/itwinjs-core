...
function verifyPrimitiveArrayBase(actualValue: IPrimitiveArrayBase, expectedValue: IPrimitiveArrayBase) {
  ...
  if (expectedValue.array_g) {
    ...
    expectedValue.array_g.forEach((value, index) => {
      expect(actualValue.array_g![index], "'geometry[]' type property did not roundtrip as expected").to.deep.equal(
        value
      );
    });
  }
...
function blobEqual(lhs: any, rhs: any) {
  if (!(lhs instanceof Uint8Array) || !(rhs instanceof Uint8Array)) throw new Error("expecting uint8array");
...
describe("Element and ElementAspect roundtrip test for all type of properties", () => {
  ...
  const primInst1: IPrimitiveBase = {
    ...
    g: GeomJson.Writer.toIModelJson(
      Cone.createAxisPoints(Point3d.create(0, 0, 0), Point3d.create(0, 0, 1), 0.5, 0.5, false)
    ),
  ...
  const primInst2: IPrimitiveBase = {
    ...
    g: GeomJson.Writer.toIModelJson(
      Cone.createAxisPoints(Point3d.create(0, 1, 0), Point3d.create(0, 0, 1), 0.5, 0.5, false)
    ),
  ...
  const primArrInst1: IPrimitiveArrayBase = {
    ...
    array_g: [
      GeomJson.Writer.toIModelJson(
        Cone.createAxisPoints(Point3d.create(0, 1, 0), Point3d.create(0, 0, 2), 0.5, 0.5, false)
      ),
      GeomJson.Writer.toIModelJson(
        Cone.createAxisPoints(Point3d.create(0, 1, 1), Point3d.create(0, 0, 2), 0.5, 0.5, false)
      ),
    ],
  };
  ...
  it("Roundtrip all type of properties via ElementApi, ConcurrentQuery and ECSqlStatement via insert and update", async () => {
    ...
    let rowCount = 0;
    for await (const row of imodel.createQueryReader("SELECT * FROM ts.TestElement", undefined, {
  ...
  it("Roundtrip all type of properties via ElementAspectApi, ConcurrentQuery and ECSqlStatement via insert and update", async () => {
    ...
    // verify via concurrent query
    let rowCount = 0;
    for await (const row of imodel.createQueryReader("SELECT * FROM ts.TestElementAspect", undefined, {
      rowFormat: QueryRowFormat.UseJsPropertyNames,
    })) {
  ...
  it("Roundtrip all type of properties via ElementRefersToElements, ConcurrentQuery and ECSqlStatement via insert and update", async () => {
    ...
    // verify via concurrent query
    let rowCount = 0;
    for await (const row of imodel.createQueryReader("SELECT * FROM ts.TestElementRefersToElements", undefined, {
      rowFormat: QueryRowFormat.UseJsPropertyNames,
    })) {
    ...
    // verify updated values
    const updatedValue = imodel.relationships.getInstance<TestElementRefersToElements>(
      expectedRelationshipValue.classFullName,
      relationshipId
    );
  ...
  it("Roundtrip placement when geom is undefined", async () => {
    ...
    const testBox = (props: TestBoxProps, expectedXOffset: number) => {
      ...
      if (undefined !== props.baseOriginX) box.baseOrigin = [props.baseOriginX, 1, 2];

      const geom = [{ header: { flags: 0 } }, { box: { ...box, baseX: 10, baseY: 20 } }];
