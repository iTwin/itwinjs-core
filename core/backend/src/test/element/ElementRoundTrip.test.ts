...
interface TestElementAspect
  extends IPrimitive,
    IPrimitiveArray,
    ElementAspectProps {}

interface TestElementRefersToElements
  extends IPrimitive,
    IPrimitiveArray,
    ElementRefersToElements {}

function verifyPrimitiveBase(
  actualValue: IPrimitiveBase,
  expectedValue: IPrimitiveBase
) {
  if (expectedValue.i)
    assert.equal(
      actualValue.i,
      expectedValue.i,
      "'integer' type property did not roundtrip as expected"
    );
  if (expectedValue.l)
    assert.equal(
      actualValue.l,
      expectedValue.l,
      "'long' type property did not roundtrip as expected"
    );
  if (expectedValue.d)
    assert.equal(
      actualValue.d,
      expectedValue.d,
      "'double' type property did not roundtrip as expected"
    );
  if (expectedValue.b)
    assert.equal(
      actualValue.b,
      expectedValue.b,
      "'boolean' type property did not roundtrip as expected"
    );
  if (expectedValue.dt)
    assert.equal(
      actualValue.dt,
      expectedValue.dt,
      "'dateTime' type property did not roundtrip as expected"
    );
  if (expectedValue.s)
    assert.equal(
      actualValue.s,
      expectedValue.s,
      "'string' type property did not roundtrip as expected"
    );
  if (expectedValue.p2d) {
    assert.equal(
      actualValue.p2d?.x,
      expectedValue.p2d?.x,
      "'Point2d.x' type property did not roundtrip as expected"
    );
    assert.equal(
      actualValue.p2d?.y,
      expectedValue.p2d?.y,
      "'Point2d.y' type property did not roundtrip as expected"
    );
  }
  if (expectedValue.p3d) {
    assert.equal(
      actualValue.p3d?.x,
      expectedValue.p3d?.x,
      "'Point3d.x' type property did not roundtrip as expected"
    );
    assert.equal(
      actualValue.p3d?.y,
      expectedValue.p3d?.y,
      "'Point3d.y' type property did not roundtrip as expected"
    );
    assert.equal(
      actualValue.p3d?.z,
      expectedValue.p3d?.z,
      "'Point3d.z' type property did not roundtrip as expected"
    );
  }
  if (expectedValue.bin)
    assert.isTrue(
      blobEqual(actualValue.bin, expectedValue.bin),
      "'binary' type property did not roundtrip as expected"
    );
  if (expectedValue.g)
    expect(
      actualValue.g,
      "'geometry' type property did not roundtrip as expected."
    ).to.deep.equal(expectedValue.g);
}

function verifyPrimitiveArrayBase(
  actualValue: IPrimitiveArrayBase,
  expectedValue: IPrimitiveArrayBase
) {
  if (expectedValue.array_bin) {
    assert.equal(
      actualValue.array_bin!.length,
      expectedValue.array_bin.length,
      "'binary[].length' array length mismatch"
    );
    expectedValue.array_bin.forEach((value, index) => {
      assert.isTrue(
        blobEqual(actualValue.array_bin![index], value),
        "'binary[]' type property did not roundtrip as expected"
      );
    });
  }
  if (expectedValue.array_i) {
    assert.equal(
      actualValue.array_i!.length,
      expectedValue.array_i.length,
      "'integer[].length' array length mismatch"
    );
    expectedValue.array_i.forEach((value, index) => {
      assert.equal(
        actualValue.array_i![index],
        value,
        "'integer[]' type property did not roundtrip as expected"
      );
    });
  ...
  if (expectedValue.array_l) {
    assert.equal(
      actualValue.array_l!.length,
      expectedValue.array_l.length,
      "'long[].length' array length mismatch"
    );
    expectedValue.array_l.forEach((value, index) => {
      assert.equal(
        actualValue.array_l![index],
        value,
        "'long[]' type property did not roundtrip as expected"
      );
    });
  ...
  if (expectedValue.array_d) {
    assert.equal(
      actualValue.array_d!.length,
      expectedValue.array_d.length,
      "'double[].length' array length mismatch"
    );
    expectedValue.array_d.forEach((value, index) => {
      assert.equal(
        actualValue.array_d![index],
        value,
        "'double[]' type property did not roundtrip as expected"
      );
    });
  ...
  if (expectedValue.array_b) {
    assert.equal(
      actualValue.array_b!.length,
      expectedValue.array_b.length,
      "'boolean[].length' array length mismatch"
    );
    expectedValue.array_b.forEach((value, index) => {
      assert.equal(
        actualValue.array_b![index],
        value,
        "'boolean[]' type property did not roundtrip as expected"
      );
    });
  ...
  if (expectedValue.array_dt) {
    assert.equal(
      actualValue.array_dt!.length,
      expectedValue.array_dt.length,
      "'dateTime[].length' array length mismatch"
    );
    expectedValue.array_dt.forEach((value, index) => {
      assert.equal(
        actualValue.array_dt![index],
        value,
        "'dateTime[]' type property did not roundtrip as expected"
      );
    });
  ...
  if (expectedValue.array_g) {
    assert.equal(
      actualValue.array_g!.length,
      expectedValue.array_g.length,
      "'geometry[].length' array length mismatch"
    );
    expectedValue.array_g.forEach((value, index) => {
      expect(
        actualValue.array_g![index],
        "'geometry[]' type property did not roundtrip as expected"
      ).to.deep.equal(value);
    });
  }
  ...
  if (expectedValue.array_s) {
    assert.equal(
      actualValue.array_s!.length,
      expectedValue.array_s.length,
      "'string[].length' array length mismatch"
    );
    expectedValue.array_s.forEach((value, index) => {
      assert.equal(
        actualValue.array_s![index],
        value,
        "'string[]' type property did not roundtrip as expected"
      );
    });
  ...
  if (expectedValue.array_p2d) {
    assert.equal(
      actualValue.array_p2d!.length,
      expectedValue.array_p2d.length,
      "'point2d[].length' array length mismatch"
    );
    expectedValue.array_p2d.forEach((value, index) => {
      assert.equal(
        actualValue.array_p2d![index].x,
        value.x,
        "'point2d[].x' type property did not roundtrip as expected"
      );
      assert.equal(
        actualValue.array_p2d![index].y,
        value.y,
        "'point2d[].y' type property did not roundtrip as expected"
      );
    });
  ...
  if (expectedValue.array_p3d) {
    assert.equal(
      actualValue.array_p3d!.length,
      expectedValue.array_p3d.length,
      "'point3d[].length' array length mismatch"
    );
    expectedValue.array_p3d.forEach((value, index) => {
      assert.equal(
        actualValue.array_p3d![index].x,
        value.x,
        "'point3d[].x' type property did not roundtrip as expected"
      );
      assert.equal(
        actualValue.array_p3d![index].y,
        value.y,
        "'point3d[].y' type property did not roundtrip as expected"
      );
      assert.equal(
        actualValue.array_p3d![index].z,
        value.z,
        "'point3d[].z' type property did not roundtrip as expected"
      );
    });
...
function verifyPrimitiveArray(
  actualValue: IPrimitiveArray,
  expectedValue: IPrimitiveArray
) {
  verifyPrimitiveArrayBase(actualValue, expectedValue);
  if (expectedValue.array_st) {
    assert.equal(
      actualValue.array_st!.length,
      expectedValue.array_st.length,
      "'struct[].length' array length mismatch"
    );
...
function verifyTestElement(
  actualValue: TestElement,
  expectedValue: TestElement
) {
  verifyPrimitive(actualValue, expectedValue);
  verifyPrimitiveArray(actualValue, expectedValue);
}

function verifyTestElementAspect(
  actualValue: TestElementAspect,
  expectedValue: TestElementAspect
) {
  verifyPrimitive(actualValue, expectedValue);
  verifyPrimitiveArray(actualValue, expectedValue);
}

function initElemProps(
  className: string,
  ...
  if (autoHandledProp) Object.assign(elementProps, autoHandledProp);

  return elementProps;
}

function initElementAspectProps(
  className: string,
  _iModelName: IModelDb,
  elId: Id64String,
  autoHandledProp: any
...
function initElementRefersToElementsProps(
  ...
  if (autoHandledProp) Object.assign(result, autoHandledProp);
...
describe("Element and ElementAspect roundtrip test for all type of properties", () => {
  ...
  const primInst1: IPrimitiveBase = {
    ...
    g: GeomJson.Writer.toIModelJson(
  ...
  const primArrInst2: IPrimitiveArrayBase = {
    ...
    array_bin: [
      new Uint8Array([1, 2, 3, 3, 4]),
      new Uint8Array([0, 0, 0, 0]),
      new Uint8Array([1, 2, 3, 4]),
    ],
    array_p2d: [
      new Point2d(-123, 244.23232),
      new Point2d(232, 324.2323),
      new Point2d(322, 2324.23322),
    ],
  ...
  before(async () => {
    ...
    const imodel = SnapshotDb.createEmpty(iModelPath, {
      rootSubject: { name: "RoundTripTest" },
    });
  ...
  it("Roundtrip all type of properties via ElementApi, ConcurrentQuery and ECSqlStatement via insert and update", async () => {
    const testFileName = IModelTestUtils.prepareOutputFile(
      subDirName,
      "roundtrip_correct_data.bim"
    );
    const imodel = IModelTestUtils.createSnapshotFromSeed(
      testFileName,
      iModelPath
    );
    const spatialCategoryId = SpatialCategory.queryCategoryIdByName(
      imodel,
      IModel.dictionaryId,
      categoryName
    );
    ...
    const expectedValue = initElemProps(
      "TestElement",
      imodel,
      newModelId,
      spatialCategoryId!,
      {
        ...
        array_st: [
          { ...primInst1, ...primArrInst2 },
          { ...primInst2, ...primArrInst1 },
        ],
      }
    ) as TestElement;
    ...
    Object.assign(actualValue, {
      ...
      array_st: [
        { ...primInst2, ...primArrInst2 },
        { ...primInst1, ...primArrInst1 },
      ],
    });
  ...
  it("Roundtrip all type of properties via ElementAspectApi, ConcurrentQuery and ECSqlStatement via insert and update", async () => {
    const testFileName = IModelTestUtils.prepareOutputFile(
      subDirName,
      "roundtrip_apsect_correct_data.bim"
    );
    const imodel = IModelTestUtils.createSnapshotFromSeed(
      testFileName,
      iModelPath
    ...
    const expectedAspectValue = initElementAspectProps(
      "TestElementAspect",
      imodel,
      elId,
      {
    ...
    const actualAspectValue: ElementAspectProps[] = imodel.elements
      .getAspects(elId, expectedAspectValue.classFullName)
      .map((x) => x.toJSON());
    ...
    let rowCount = 0;
    for await (const row of imodel.createQueryReader(
      "SELECT * FROM ts.TestElementAspect",
      undefined,
      { rowFormat: QueryRowFormat.UseJsPropertyNames }
    )) {
      verifyTestElementAspect(
        row.toRow() as TestElementAspect,
        expectedAspectValue
      );
      rowCount++;
    }
    ...
    // verify updated values
    const updatedAspectValue: ElementAspectProps[] = imodel.elements
      .getAspects(elId, expectedAspectValue.classFullName)
      .map((x) => x.toJSON());
  ...
  function verifyTestElementRefersToElements(
    actualValue: TestElementRefersToElements,
    expectedValue: TestElementRefersToElements
  ) {
    assert.equal(
      actualValue.sourceId,
      expectedValue.sourceId,
      "'sourceId' type property did not roundtrip as expected"
    );
    assert.equal(
      actualValue.targetId,
      expectedValue.targetId,
      "'targetId' type property did not roundtrip as expected"
    );
    verifyPrimitiveBase(actualValue, expectedValue);
    verifyPrimitiveArrayBase(actualValue, expectedValue);
  }
  ...
  it("Roundtrip all type of properties via ElementRefersToElements, ConcurrentQuery and ECSqlStatement via insert and update", async () => {
    const testFileName = IModelTestUtils.prepareOutputFile(
      subDirName,
      "roundtrip_relationships_correct_data.bim"
    );
    const imodel = IModelTestUtils.createSnapshotFromSeed(
      testFileName,
      iModelPath
    ...
    const expectedRelationshipValue = initElementRefersToElementsProps(
      "TestElementRefersToElements",
      imodel,
      elId1,
      elId2,
    ...
    const relationshipId: Id64String = imodel.relationships.insertInstance(
      instance as any
    ); // initElementRefersToElementsProps lies about return type.
    imodel.saveChanges();

    // verify inserted properties
    const actualRelationshipValue =
      imodel.relationships.getInstance<TestElementRefersToElements>(
    ...
    let rowCount = 0;
    for await (const row of imodel.createQueryReader(
      "SELECT * FROM ts.TestElementRefersToElements",
      undefined,
      { rowFormat: QueryRowFormat.UseJsPropertyNames }
    )) {
    ...
    for await (const row of imodel.createQueryReader(
      ...
      verifyTestElementRefersToElements(
        row.toRow() as TestElementRefersToElements,
  ...
  it("Roundtrip placement when geom is undefined", async () => {
    ...
    const insertAndVerifyPlacement = (
      ...
      const imodelPath = IModelTestUtils.prepareOutputFile(
        subDirName,
        `roundtrip_placement-${name}.bim`
      );
      let imodel = IModelTestUtils.createSnapshotFromSeed(
        imodelPath,
        iModelPath
      );
      ...
      const inMemoryCopy = imodel.elements.getElement<PhysicalObject>(
        { id: objId, wantGeometry: true },
        PhysicalObject
      );
      ...
      const readFromDbCopy = imodel.elements.getElement<PhysicalObject>(
        { id: objId, wantGeometry: true },
        PhysicalObject
      );
    ...
    const testBox = (props: TestBoxProps, expectedXOffset: number) => {
      const box: any = {};
      if (undefined !== props.originX) box.origin = [props.originX, 1, 2];
