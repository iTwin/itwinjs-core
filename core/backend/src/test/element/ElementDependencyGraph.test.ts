...
class TestHelper {
  ...
  constructor(testName: string, dbInfo: DbInfo) {
    ...
    assert.equal(
      this.db.nativeDb.addChildPropagatesChangesToParentRelationship("TestBim", "ChildPropagatesChangesToParent"),
      0
    );
...
describe("ElementDependencyGraph", () => {
  ...
  it("should invokeCallbacks through parents - geomodeler schema", () => {
    ...
    const ede_material_materialDepthRange = TestElementDrivesElement.create<TestElementDrivesElement>(
      helper.db,
      material,
      materialDepthRange
    );
