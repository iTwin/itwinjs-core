...
export class RevisionUtility {
  ...
  public static recompressRevision(sourceFile: string, targetFile: string, lzmaProps?: LzmaParams): BentleyStatus {
    ...
    return NativePlatform.RevisionUtility.recompressRevision(sourceFile, targetFile, lzmaProps ? JSON.stringify(lzmaProps) : undefined);
  }
  public static disassembleRevision(sourceFile: string, targetDir: string): BentleyStatus {
    if (!IModelJsFs.existsSync(sourceFile))
      throw new Error("SourceFile does not exists");
    return NativePlatform.RevisionUtility.disassembleRevision(sourceFile, targetDir);
  }
  public static assembleRevision(targetFile: string, rawChangesetFile: string, prefixFile?: string, lzmaProps?: LzmaParams): BentleyStatus {
    ...
    return NativePlatform.RevisionUtility.assembleRevision(targetFile, rawChangesetFile, prefixFile, lzmaProps ? JSON.stringify(lzmaProps) : undefined);
  }
  public static normalizeLzmaParams(lzmaProps?: LzmaParams): LzmaParams {
    return JSON.parse(NativePlatform.RevisionUtility.normalizeLzmaParams(lzmaProps ? JSON.stringify(lzmaProps) : undefined)) as LzmaParams;
  }
  public static computeStatistics(sourceFile: string, addPrefix: boolean = true): any {
    if (!IModelJsFs.existsSync(sourceFile))
      throw new Error("SourceFile does not exists");
    return JSON.parse(NativePlatform.RevisionUtility.computeStatistics(sourceFile, addPrefix));
  }
  public static getUncompressSize(sourceFile: string): ChangesetSizeInfo {
    if (!IModelJsFs.existsSync(sourceFile))
      throw new Error("SourceFile does not exists");
    return JSON.parse(NativePlatform.RevisionUtility.getUncompressSize(sourceFile)) as ChangesetSizeInfo;
  }
}
