...
import { ColorDef, FontType, FractionRun, LineBreakRun, LineLayoutResult, Run, RunLayoutResult, TextAnnotation, TextAnnotation2dProps, TextAnnotation3dProps, TextAnnotationAnchor, TextAnnotationProps, TextBlock, TextBlockGeometryPropsEntry, TextBlockMargins, TextRun, TextStringProps, TextStyleSettings, TextStyleSettingsProps } from "@itwin/core-common";
import { IModelDb, SnapshotDb } from "../../IModelDb";
import { AnnotationTextStyle, AnnotationTextStyleCreateArgs, TextAnnotation2d, TextAnnotation3d } from "../../TextAnnotationElement";
...
//       findTextStyle: (id: string) => TextStyleSettings.fromJSON(id === "block" ? { lineSpacingFactor: 12, fontName: "block" } : { lineSpacingFactor: 99, fontName: "run" }),
//     });
...
describe("AnnotationTextStyle element", () => {
  describe("persistence", () => {
    let imodel: SnapshotDb;
    ...
    function validate(textStyle: AnnotationTextStyle, expected: TextStyleSettingsProps): void {
      expect(textStyle).not.to.be.undefined;
      expect(textStyle instanceof AnnotationTextStyle).to.be.true;
      expect(textStyle.settings).not.to.be.undefined;
      expect(textStyle.settings instanceof TextStyleSettings).to.be.true;
      const expectedSettings = TextStyleSettings.fromJSON(expected);
      expect(textStyle.settings.equals(expectedSettings)).to.be.true;
    }

    it("inserts and persists correctly", () => {
      const testSettings = {fontName: "Arial", lineHeight: 0.5, widthFactor: 0.5, isBold: true, isItalic: false};
      // Create and insert using "create" method
      const settings = TextStyleSettings.fromJSON(testSettings);
      ...
      const textStyle1 = AnnotationTextStyle.create(createProps);
      validate(textStyle1, testSettings);
      const elId1 = textStyle1.insert();
      expect(Id64.isValidId64(elId1)).to.be.true;
      // Query by code and validate
      const code1 = AnnotationTextStyle.createCode(imodel, IModelDb.dictionaryId, "Test Style 1");
      const persistedTextStyle1 = imodel.elements.getElement<AnnotationTextStyle>(code1);
      validate(persistedTextStyle1, testSettings);
      // Create and insert using "fromJSON" method
      const props = persistedTextStyle1.toJSON();
      props.code = AnnotationTextStyle.createCode(imodel, IModelDb.dictionaryId, "Test Style 2").toJSON();
      const textStyle2 = AnnotationTextStyle.fromJSON(props, imodel);
      validate(textStyle2, testSettings);
      const elId2 = textStyle2.insert();
      expect(Id64.isValidId64(elId2)).to.be.true;
      // Query by ID and validate
      const persistedTextStyle2 = imodel.elements.getElement<AnnotationTextStyle>(elId2);
      validate(persistedTextStyle2, testSettings);
    });

    it("updates the persisted text style", () => {
      const testSettings = {fontName: "Arial", lineHeight: 0.5, widthFactor: 0.5, isBold: true, isItalic: false};
      // Create and insert using "create" method
      const settings = TextStyleSettings.fromJSON(testSettings);
      const createProps: AnnotationTextStyleCreateArgs = {
      ...
      const textStyle1 = AnnotationTextStyle.create(createProps);
      const elId3 = textStyle1.insert();
      expect(Id64.isValidId64(elId3)).to.be.true;
      // Query by ID and update settings
      const persistedTextStyle = imodel.elements.getElement<AnnotationTextStyle>(elId3);
      const updatedTestSettings = {fontName: "RealFontTM", lineHeight: 0.75, widthFactor: 0.75, isBold: false, isItalic: true};
      const updatedSettings = TextStyleSettings.fromJSON(updatedTestSettings);
      persistedTextStyle.settings = updatedSettings;
      persistedTextStyle.update();
      // Query by ID and validate
      const updatedPersistedTextStyle = imodel.elements.getElement<AnnotationTextStyle>(elId3);
      validate(updatedPersistedTextStyle, updatedTestSettings);
    });
  });
});