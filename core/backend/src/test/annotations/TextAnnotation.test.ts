...
describe("TextAnnotation element", () => {
  ...
  describe.only("clone", () => {
    ...
    async function makeEndpoint(which: "src" | "dst"): Promise<Endpoint> {
      const db = await createIModel(`TextAnnotation.clone.${which}`);
      const jobSubjectId = createJobSubjectElement(db, "Job").insert();
      if ("dst" === which) {
        // insert some unused elements to ensure remapped element Ids differ between the two iModels
        for (let i = 0; i < 3; i++) {
          DefinitionModel.insert(db, jobSubjectId, `UnusedDefinition${i}`);
        }
      }

      const defModel = DefinitionModel.insert(db, jobSubjectId, "Definition");
      const textStyleId = createAnnotationTextStyle(db, defModel, "TextStyle", {fontName: "not-a-font"}).insert();
      const { category, model } = insertDrawingModel(db, jobSubjectId, defModel);

      return { db, category, model, textStyleId };
    }

    before(async () => {
      src = await makeEndpoint("src");
      dst = await makeEndpoint("dst");
      for (const key of ["category", "model", "textStyleId"] as const) {
        expect(src[key]).not.to.equal(dst[key]);
      }
    });

    it("automatically copies default text style if one by the same name doesn't exist in destination", () => {
      
    });

    it("remaps text style by name if already exists in destination iModel", () => {

    });

    it("leaves text style undefined if unspecified", () => {

    });

    function expectText(imodel: StandaloneDb, expected: string, elemId: Id64String): void {
    ...
    it("remaps field hosts", () => {
      const textBlock = TextBlock.create({ styleOverrides: { fontName: "Karla" } });
      textBlock.appendRun(FieldRun.create({
        propertyHost: { elementId: src.model, schemaName: "BisCore", className: "Drawing" },
        propertyPath: { propertyName: "CodeValue" },
        cachedContent: "...",
      }));
      const textAnnotationData = TextAnnotation.create({ textBlock }).toJSON();
      const srcElemId = createElement2d(src.db, { model: src.model, category: src.category, textAnnotationData }).insert();
      src.db.saveChanges();

      expectText(src.db, "drawing code value", srcElemId);


      // const ctx = new IModelElementCloneContext(src.db, dst.db);
    });
    ...
    it("is a no-op if cloning within the same iModel", () => {
