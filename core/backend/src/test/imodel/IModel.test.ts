...
describe("iModel", () => {
  ...
  it("should optionally detect class mismatches", () => {
    ...
    const physicalElementValidated = imodel1.elements.tryGetElement<PhysicalElement>(
    ...
    const physicalObjectValidated = imodel1.elements.tryGetElement<PhysicalObject>(
      IModel.rootSubjectId,
      PhysicalObject
    ); // concrete class
    ...
    const geometricSubModelValidated = imodel1.models.tryGetSubModel<GeometricModel>(
      IModel.dictionaryId,
      GeometricModel
    ); // abstract class
  ...
  it("should insert a RenderMaterial", () => {
    ...
    expect(JSON.stringify(renderMaterial.jsonProperties.materialAssets.renderMaterial.color)).to.equal(
      JSON.stringify(color)
    );
    expect(renderMaterial.jsonProperties.materialAssets.renderMaterial.HasSpecularColor).to.equal(true);
    expect(JSON.stringify(renderMaterial.jsonProperties.materialAssets.renderMaterial.specular_color)).to.equal(
      JSON.stringify(specularColor)
    );
    ...
    expect(JSON.stringify(renderMaterial.jsonProperties.materialAssets.renderMaterial.reflect_color)).to.equal(
      JSON.stringify(reflectColor)
    );
  ...
  it("attempt to apply material to new element in imodel5", () => {
    ...
    const categoryId = SpatialCategory.insert(imodel5, IModel.dictionaryId, "GeoJSON Feature", {
  ...
  it("should create display styles", () => {
    ...
    for (const test of testCases) {
      ...
      const expectedBGColor =
        expected.backgroundColor instanceof ColorDef ? expected.backgroundColor.toJSON() : expected.backgroundColor;
  ...
  function checkElementMetaData(obj: EntityMetaData) {
    ...
    if (obj.customAttributes !== undefined) {
      for (const ca of obj.customAttributes) {
        if (ca.ecclass === "BisCore:ClassHasHandler") foundClassHasHandler = true;
  ...
  it("should do CRUD on models", () => {
    ...
    const [modeledElementId, newModelId] = IModelTestUtils.createAndInsertPhysicalPartitionAndModel(
      testImodel,
      Code.createEmpty(),
      true
    );
  ...
  it("should create link table relationship instances", () => {
    ...
    const g1byst = ElementGroupsMembers.getInstance<ElementGroupsMembers>(testImodel, {
      sourceId: r1.sourceId,
      targetId: r1.targetId,
    });
  ...
  it("should be able to create a snapshot IModel and set geolocation by GCS", async () => {
    ...
    assert.isTrue(
      iModel2.geographicCoordinateSystem!.horizontalCRS!.projection!.equals(gcs.horizontalCRS!.projection!)
    );
  ...
  it("Run plain SQL against readonly connection", () => {
    let iModel = SnapshotDb.createEmpty(
      IModelTestUtils.prepareOutputFile("IModel", "sqlitesqlreadonlyconnection.bim"),
