...
describe("iModel", () => {
  ...
  function replaceJSProps(ecProps: any): any {
    if ("CategorySelector" in ecProps)
      ecProps.categorySelectorId = ecProps.CategorySelector.Id;
    if ("DisplayStyle" in ecProps)
      ecProps.displayStyleId = ecProps.DisplayStyle.Id;
    if ("ModelSelector" in ecProps)
      ecProps.modelSelectorId = ecProps.ModelSelector.Id;

    // Bis.ViewDefinition2D
    if ("BaseModel" in ecProps)
      ecProps.baseModelId = ecProps.BaseModel.Id;

    if ("Category" in ecProps)
      ecProps.category = ecProps.Category.Id;

    return ecProps;
  }

  it.only("Test1234", async () => {
    const testIModel = SnapshotDb.openFile(`D:\\Projects\\Codebases\\DrainageBims\\Analysis\\DeletedManhole\\2-DeletedManhole.bim`);
    const props = testIModel.withPreparedStatement(`
        ...
        FROM bis.Element e
        WHERE e.ECInstanceId IN (0x200000001bd, 0x200000001c0, 0x200000001c0)
      ...
      for (const { id, className, ["e.$"]: $, categories, models } of stmt) {
        ...
        ecProps.classFullName = className.replace(".", ":"); // normalize (FIXME: use existing function)
        ...
        // ecProps = replaceJSProps(ecProps);
        props.push(ecProps);
      }
      return props;
    });
    ...
    for (const prop of props) {
      const elementVal = testIModel.constructEntity<Element>(prop);
      console.log(`\nElement Value: ${JSON.stringify(elementVal)}`);
    }
  });
