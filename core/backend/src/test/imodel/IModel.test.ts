...
describe("iModel", () => {
  ...
  it("should load a known model by Id from an existing iModel", () => {
    ...
    roundtripThroughJson(model);
    const modelExtents: AxisAlignedBox3d = (model as PhysicalModel).queryExtents(); // eslint-disable-line @typescript-eslint/no-deprecated
  ...
  it("should produce an array of rows", () => {
    const rows: any[] = IModelTestUtils.executeQuery(imodel1, `SELECT * FROM ${Category.classFullName}`); // eslint-disable-line @typescript-eslint/no-deprecated
    assert.exists(rows);
    assert.isArray(rows);
