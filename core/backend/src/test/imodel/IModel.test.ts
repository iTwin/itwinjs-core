...
function getOrCreateTestTxn(iModel: IModelDb): TestEditTxn {
  ...
  // Save any unsaved changes from implicit writes before starting the explicit transaction
  // eslint-disable-next-line @typescript-eslint/no-deprecated
  if (iModel[_nativeDb].hasUnsavedChanges())
    iModel.saveChanges();

  const txn = new TestEditTxn(iModel, "imodel test");
  txn.start();
...
async function generateTestSnapshot(targetFileName: string, seedAssetName: string): Promise<SnapshotDb> {
  ...
  await getOrCreateTestTxn(imodel).importSchemas([schemaPathname]); // will throw an exception if import fails
  getOrCreateTestTxn(imodel).saveChanges();
  return imodel;
}
...
describe("iModel", () => {
  ...
  it("should load a known element by Id from an existing iModel", () => {
    ...
    expect(() => getOrCreateTestTxn(imodel2).insertElement(newEl)).throws("invalid code scope").to.have.property("metadata");
    newEl.code.scope = "0x34322"; // valid id, but element doesn't exist
    expect(() => getOrCreateTestTxn(imodel2).insertElement(newEl)).throws("invalid code scope").to.have.property("metadata");

    newEl.code.scope = el3.federationGuid!;
    const newId = getOrCreateTestTxn(imodel2).insertElement(newEl); // code scope from FederationGuid should get converted to ElementId
    const a4 = imodel2.elements.getElementProps(newId);
    expect(a4.code.scope).equal(el3.id);

    a4.code.scope = "0x13343";
    expect(() => getOrCreateTestTxn(imodel2).updateElement(a4)).throws("invalid code scope").to.have.property("metadata");

    a4.code.scope = "0x1";
    getOrCreateTestTxn(imodel2).updateElement(a4); // should change the code scope to new element
    ...
    getOrCreateTestTxn(imodel2).updateElement(newProps);
  ...
  it("should create elements", () => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const elementId = getOrCreateTestTxn(imodel2).insertElement(element.toJSON());
  ...
  it("should insert a RenderMaterial", () => {
    ...
    const renderMaterialId = RenderMaterialElement.insertWithTxn(getOrCreateTestTxn(imodel2), IModel.dictionaryId, testMaterialName, renderMaterialParams);
  ...
  it("attempt to apply material to new element in imodel5", () => {
    ...
    const texId = Texture.insertTextureWithTxn(getOrCreateTestTxn(imodel5), IModel.dictionaryId, testTextureName, testTextureFormat, samplePngTexture.base64, testTextureDescription);

    /* eslint-disable @typescript-eslint/naming-convention */
    const matId = RenderMaterialElement.insertWithTxn(getOrCreateTestTxn(imodel5), IModel.dictionaryId, "test material name",
      {
        paletteName: "TestPaletteName",
        patternMap: {
          TextureId: texId,
    ...
    const categoryId = SpatialCategory.insertWithTxn(getOrCreateTestTxn(imodel5), IModel.dictionaryId, "GeoJSON Feature", { color: ColorDef.white.toJSON() });
    ...
    getOrCreateTestTxn(imodel5).insertElement(props);
    getOrCreateTestTxn(imodel5).saveChanges();
  });
  ...
  it("should insert a DisplayStyle", () => {
    ...
    const styleId = getOrCreateTestTxn(imodel2).insertElement(props);
    ...
    getOrCreateTestTxn(imodel2).updateElement(style.toJSON());
  ...
  it("should create display styles", () => {
    ...
    for (const test of testCases) {
      ...
      const styleId = DisplayStyle3d.insertWithTxn(getOrCreateTestTxn(imodel2), IModel.dictionaryId, `TestStyle${suffix++}`, expected);
  ...
  it("should insert and update auto-handled properties", () => {
    ...
    const newTestElemId = getOrCreateTestTxn(imodel4).insertElement(newTestElem.toJSON());
    ...
    try {
      getOrCreateTestTxn(imodel4).updateElement(editElem.toJSON());
    } catch {
    ...
    getOrCreateTestTxn(imodel4).updateElement(afterUpdateElemFetched.toJSON());
    ...
    getOrCreateTestTxn(imodel4).updateElement(afterShortenArray.toJSON());
    ...
    getOrCreateTestTxn(imodel4).deleteElement(elid);
    assert.throws(() => imodel4.elements.getElement(elid), IModelError);
  });

  it("should handle parent and child deletion properly", () => {
    const categoryId = SpatialCategory.insertWithTxn(getOrCreateTestTxn(imodel4), IModel.dictionaryId, "MyTestCategory", new SubCategoryAppearance());
    ...
    expect(() => getOrCreateTestTxn(imodel4).deleteElement(categoryId)).throws("error deleting element").to.have.property("metadata");
    ...
    const parentId = getOrCreateTestTxn(imodel4).insertElement(elementProps);
    elementProps.parent = new ElementOwnsChildElements(parentId);
    const childId1 = getOrCreateTestTxn(imodel4).insertElement(elementProps);
    const childId2 = getOrCreateTestTxn(imodel4).insertElement(elementProps);
    ...
    getOrCreateTestTxn(imodel4).deleteElement(parentId);
  ...
  it("should get metadata for a relationship", async () => {
    ...
    const partitionId = getOrCreateTestTxn(imodel).insertElement({
    ...
    for await (const row of imodel.createQueryReader(`SELECT * FROM bis.Element LIMIT ${1}`)) {
      const relId = getOrCreateTestTxn(imodel).insertRelationship({
  ...
  it("update the project extents", async () => {
    ...
    await getOrCreateTestTxn(imodel1).updateProjectExtents(newExtents);
  ...
  it("should create and insert CodeSpecs", () => {
    ...
    const codeSpecId = testImodel.codeSpecs.insertWithTxn(getOrCreateTestTxn(testImodel), codeSpec); // throws in case of error
    ...
    assert.throws(() => testImodel.codeSpecs.insertWithTxn(getOrCreateTestTxn(testImodel), codeSpecDup), "CodeSpec already exists");
    ...
    const codeSpec2Id = testImodel.codeSpecs.insertWithTxn(getOrCreateTestTxn(testImodel), codeSpec2); // throws in case of error
    ...
    const codeSpec3Id = testImodel.codeSpecs.insertWithTxn(getOrCreateTestTxn(testImodel), codeSpec3); // throws in case of error
    ...
    const codeSpec4Id = testImodel.codeSpecs.insertWithTxn(getOrCreateTestTxn(testImodel), codeSpec4); // throws in case of error
  ...
  it("validate CodeSpec properties", async () => {
    ...
    if (true) {
      ...
      const codeSpecId = iModelDb.codeSpecs.insertWithTxn(getOrCreateTestTxn(iModelDb), codeSpec);
      ...
      getOrCreateTestTxn(iModelDb).saveChanges();
      iModelDb.close();
    }
  ...
  it("should do CRUD on models", () => {
    ...
    const [modeledElementId, newModelId] = IModelTestUtils.createAndInsertPhysicalPartitionAndModel(getOrCreateTestTxn(testImodel), Code.createEmpty(), true);
    ...
    getOrCreateTestTxn(testImodel).updateModel(newModelPersist.toJSON());
    ...
    // Delete the model
    getOrCreateTestTxn(testImodel).deleteModel(newModelId);

    // Test insertModel error handling
    try {
      getOrCreateTestTxn(testImodel).insertModel({
  ...
  it("should create model with custom relationship to modeled element", async () => {
    ...
    if (true) {
      const newPartition1 = IModelTestUtils.createAndInsertPhysicalPartition(getOrCreateTestTxn(testImodel), Code.createEmpty());
      ...
      newModelId1 = IModelTestUtils.createAndInsertPhysicalModel(getOrCreateTestTxn(testImodel), modeledElementRef);
      assert.isTrue(Id64.isValidId64(newModelId1));
    }

    if (true) {
      [, newModelId2] = IModelTestUtils.createAndInsertPhysicalPartitionAndModel(getOrCreateTestTxn(testImodel), Code.createEmpty());
  ...
  it("should create link table relationship instances", () => {
    ...
    const spatialCategoryId = SpatialCategory.insertWithTxn(getOrCreateTestTxn(testImodel), IModel.dictionaryId, "MySpatialCategory", new SubCategoryAppearance({ color: ColorByName.darkRed }));
    ...
    const id0 = getOrCreateTestTxn(testImodel).insertElement(elementProps);
    const id1 = getOrCreateTestTxn(testImodel).insertElement(elementProps);
    const id2 = getOrCreateTestTxn(testImodel).insertElement(elementProps);
    ...
    r1.id = getOrCreateTestTxn(testImodel).insertRelationship(r1.toJSON());
    const r2 = ElementGroupsMembers.create(testImodel, id0, id2);
    r2.id = getOrCreateTestTxn(testImodel).insertRelationship(r2.toJSON());
    ...
    getOrCreateTestTxn(testImodel).updateRelationship(r1.toJSON());
    ...
    getOrCreateTestTxn(testImodel).saveChanges("step 1");

    // Delete relationship instance property
    getOrCreateTestTxn(testImodel).deleteRelationship(g11.toJSON());
    getOrCreateTestTxn(testImodel).saveChanges("step 2");
    assert.throws(() => ElementGroupsMembers.getInstance(testImodel, r1.id), IModelError);

    const d0 = getOrCreateTestTxn(testImodel).insertElement(elementProps);
    const d1 = getOrCreateTestTxn(testImodel).insertElement(elementProps);
    const ede1 = ElementDrivesElement.create(testImodel, d0, d1, 0);
    ede1.id = getOrCreateTestTxn(testImodel).insertRelationship(ede1.toJSON());
    getOrCreateTestTxn(testImodel).saveChanges("step 3");

    getOrCreateTestTxn(testImodel).deleteRelationship(ede1.toJSON());
    getOrCreateTestTxn(testImodel).saveChanges("step 4");
    testImodel.close();
  });
  ...
  it("should insert DefinitionSets", () => {
    ...
    const definitionContainerId = DefinitionContainer.insertWithTxn(getOrCreateTestTxn(iModelDb), IModel.dictionaryId, Code.createEmpty());
    ...
    const categoryId1 = SpatialCategory.insertWithTxn(getOrCreateTestTxn(iModelDb), definitionContainerId, "Category1", new SubCategoryAppearance());
    const categoryId2 = SpatialCategory.insertWithTxn(getOrCreateTestTxn(iModelDb), definitionContainerId, "Category2", new SubCategoryAppearance());
    const categoryId3 = SpatialCategory.insertWithTxn(getOrCreateTestTxn(iModelDb), definitionContainerId, "Category3", new SubCategoryAppearance());
    const definitionGroupId = DefinitionGroup.create(iModelDb, definitionContainerId, Code.createEmpty()).insertWithTxn(getOrCreateTestTxn(iModelDb));
  ...
  it("should set EC properties of various types", async () => {
    ...
    // Create a new physical model
    const [, newModelId] = IModelTestUtils.createAndInsertPhysicalPartitionAndModel(getOrCreateTestTxn(testImodel), Code.createEmpty(), true);

    // Find or create a SpatialCategory
    let spatialCategoryId = SpatialCategory.queryCategoryIdByName(testImodel, IModel.dictionaryId, "MySpatialCategory");
    if (undefined === spatialCategoryId) {
      spatialCategoryId = SpatialCategory.insertWithTxn(getOrCreateTestTxn(testImodel), IModel.dictionaryId, "MySpatialCategory", new SubCategoryAppearance());
    }
    ...
    if (true) {
      ...
      id1 = getOrCreateTestTxn(testImodel).insertElement(testImodel.elements.createElement(elementProps).toJSON());
      ...
      id2 = getOrCreateTestTxn(testImodel).insertElement(testImodel.elements.createElement(elementProps).toJSON());
    ...
    if (true) {
      ...
      getOrCreateTestTxn(testImodel).updateElement(el2Modified.toJSON());
  ...
  it("should be able to create a snapshot IModel", async () => {
    ...
    getOrCreateTestTxn(iModel).saveFileProperty(myPropsStr, myStrVal);
    ...
    testRange.fill(11);
    getOrCreateTestTxn(iModel).saveFileProperty(myPropsBlob, undefined, testRange);
  ...
  it("presence of a GCS imposes the ecef value", async () => {
    ...
    getOrCreateTestTxn(iModel2).saveChanges();
    iModel2.close();
  ...
  it("Check busyTimeout option", () => {
    ...
    getOrCreateTestTxn(db).saveChanges();
    // lock db so another connection cannot write to it.
    getOrCreateTestTxn(db).saveFileProperty({ name: "test", namespace: "test" }, "");
  ...
  it("Cache cleared on abandonChanges", () => {
    ...
    const id2 = getOrCreateTestTxn(db).insertElement(props2);
  ...
  it("Run plain SQL", () => {
    ...
    getOrCreateTestTxn(imodel1).saveChanges();
  ...
  it("should update Element code", () => {
    const elementId = getOrCreateTestTxn(imodel4).insertElement({
    ...
    const codeSpecId = imodel4.codeSpecs.insertWithTxn(getOrCreateTestTxn(imodel4), "TestCodeSpec", CodeScopeSpec.Type.Model);
    const codeValue = `${element.className}-1`;
    element.code = new Code({ spec: codeSpecId, scope: IModel.repositoryModelId, value: codeValue });
    element.updateWithTxn(getOrCreateTestTxn(imodel4));
  ...
  it("should update UserLabel", () => {
    ...
    const elementId = getOrCreateTestTxn(imodel1).insertElement(elementProps);
    ...
    element.updateWithTxn(getOrCreateTestTxn(imodel1));
    ...
    getOrCreateTestTxn(imodel1).updateElement({
      id: element.id,
  ...
  it("should update FederationGuid", () => {
    ...
    getOrCreateTestTxn(imodel1).updateElement(elProps);
    element = imodel1.elements.getElement<SpatialCategory>(elementId);
    assert.isUndefined(element.federationGuid);
    ...
    const s5Id = subject5.insertWithTxn(getOrCreateTestTxn(imodel1));
    const s5pre = imodel1.elements.getElement<Subject>(s5Id);
    subject5.description = "new descr";
    subject5.updateWithTxn(getOrCreateTestTxn(imodel1));
  ...
  it("should support partial update", () => {
    ...
    const subjectId1 = subject1.insertWithTxn(getOrCreateTestTxn(imodel1));
    const subjectId2 = subject2.insertWithTxn(getOrCreateTestTxn(imodel1));
    const subjectId3 = subject3.insertWithTxn(getOrCreateTestTxn(imodel1));
    const subjectId4 = subject4.insertWithTxn(getOrCreateTestTxn(imodel1));
    ...
    getOrCreateTestTxn(imodel1).updateElement<SubjectProps>({ id: subject1.id, description: "Description1-Updated" });
    subject1 = imodel1.elements.getElement<Subject>(subjectId1, Subject);
    ...
    getOrCreateTestTxn(imodel1).updateElement<SubjectProps>({ id: subject2.id, userLabel: "UserLabel2-Updated" });
    subject2 = imodel1.elements.getElement<Subject>(subjectId2, Subject);
    ...
    subject1.updateWithTxn(getOrCreateTestTxn(imodel1));
    subject2.updateWithTxn(getOrCreateTestTxn(imodel1));
    subject3.updateWithTxn(getOrCreateTestTxn(imodel1));
    subject4.updateWithTxn(getOrCreateTestTxn(imodel1));
    subject1 = imodel1.elements.getElement<Subject>(subjectId1, Subject);
    subject2 = imodel1.elements.getElement<Subject>(subjectId2, Subject);
    subject3 = imodel1.elements.getElement<Subject>(subjectId3, Subject);
    subject4 = imodel1.elements.getElement<Subject>(subjectId4, Subject);
    ...
    getOrCreateTestTxn(imodel1).updateElement<SubjectProps>({ id: subject3.id, description: undefined });
    subject3 = imodel1.elements.getElement<Subject>(subjectId3, Subject);
    ...
    getOrCreateTestTxn(imodel1).updateElement<SubjectProps>({ id: subject4.id, userLabel: undefined });
    subject4 = imodel1.elements.getElement<Subject>(subjectId4, Subject);
  ...
  it('should allow untrimmed codes when using "exact" codeValueBehavior', () => {
    ...
    const categ1Id = getOrCreateTestTxn(imodel).insertElement(code1.props);
    ...
    const categ2Id = getOrCreateTestTxn(imodel).insertElement(code2.props);
    ...
    const categ3Id = getOrCreateTestTxn(imodel).insertElement(code3.props);
  ...
  it("should throw iTwinErrors on element CRUD opertion fails", async () => {
    ...
    getOrCreateTestTxn(imodel1).insertElement(props);

    expect(() => getOrCreateTestTxn(imodel1).insertElement(props)).throws("Error inserting element [duplicate code]").to.have.property("iTwinErrorId");
    ...
    expect(() => getOrCreateTestTxn(imodel1).updateElement(updateProps)).throws(`Error updating element [missing id], id: ${updateProps.id}`).to.have.property("iTwinErrorId");
    expect(() => getOrCreateTestTxn(imodel1).deleteElement(updateProps.id!)).throws(`Error deleting element [missing id], id: ${updateProps.id}`).to.have.property("iTwinErrorId");

    expect(() => getOrCreateTestTxn(imodel1).insertModel({ classFullName: DefinitionModel.classFullName, modeledElement: { id: "0x10000000bad" } })).throws("Error inserting model [error=10004], class=BisCore:DefinitionModel").to.have.property("iTwinErrorId");
    expect(() => getOrCreateTestTxn(imodel1).updateModel({
      id: Id64.fromString("0x111111"),
    ...
    expect(() => getOrCreateTestTxn(imodel1).deleteModel(Id64.fromString("0x111111"))).throws(`Error deleting model [missing id], id: ${Id64.fromString("0x111111")}`).to.have.property("iTwinErrorId");
  });
  ...
  it("should throw \"constraint failed (BE_SQLITE_CONSTRAINT_UNIQUE)\" when inserting a relationsip instance with the same prop twice", () => {
    ...
    // create a SpatialCategory
    const spatialCategoryId = SpatialCategory.insertWithTxn(getOrCreateTestTxn(imodel), IModel.dictionaryId, "MySpatialCategory", new SubCategoryAppearance({ color: ColorByName.darkRed }));
    ...
    const id0 = getOrCreateTestTxn(imodel).insertElement(elementProps);
    const id1 = getOrCreateTestTxn(imodel).insertElement(elementProps);
    ...
    getOrCreateTestTxn(imodel).insertRelationship(props)
    expect(() => getOrCreateTestTxn(imodel).insertRelationship(props)).to.throw(`Failed to insert relationship [${imodelPath}]: rc=2067, constraint failed (BE_SQLITE_CONSTRAINT_UNIQUE)`);
  ...
  function insertElement(imodel: IModelDb, mId: Id64String, cId: Id64String, cName: string, propName: string): Id64String {
    ...
    const id = getOrCreateTestTxn(imodel).insertElement(geomElement.toJSON());
  ...
  it("Validate invalid relationship classes being inserted/updated", async () => {
    ...
    await getOrCreateTestTxn(testImodel).importSchemaStrings([
    ...
    if (!spatialCategoryId) {
      spatialCategoryId = SpatialCategory.insertWithTxn(
        getOrCreateTestTxn(testImodel),
        IModel.dictionaryId,
        "MySpatialCategory",
    ...
    getOrCreateTestTxn(testImodel).saveChanges();
    ...
    for (const { name, shouldSucceed, expectedRows } of testCases) {
      ...
      try {
        idA = getOrCreateTestTxn(testImodel).insertElement(geomElement.toJSON());
        if (shouldSucceed)
      ...
      if (expectedRows === 1 && idA !== undefined) {
        ...
        getOrCreateTestTxn(testImodel).updateElement(editElem);
        ...
        if (shouldSucceed) {
          // Delete the element
          getOrCreateTestTxn(testImodel).deleteElement(idA);
  ...
  it("should update codeValues that are switched between elements", async () => {
    ...
    categoryA.insertWithTxn(getOrCreateTestTxn(imodelDb));
    categoryB.insertWithTxn(getOrCreateTestTxn(imodelDb));
    getOrCreateTestTxn(imodelDb).saveChanges();
    ...
    categoryA.updateWithTxn(getOrCreateTestTxn(imodelDb));
    categoryB.code.value = "A";
    categoryB.updateWithTxn(getOrCreateTestTxn(imodelDb));
  ...
  describe("Delete relationship instances", () => {
    ...
    function setupRelationships(numOfRelationships: number, multipleClasses: boolean = false): RelationshipProps[] {
      ...
      for (let i = 0; i < numOfRelationships; ++i) {
        ...
        const sourceId = getOrCreateTestTxn(testImodel).insertElement(sourceProps);

        const targetProps = createElemProps(testImodel, newModelId, spatialCategoryId, "Generic:PhysicalObject");
        const targetId = getOrCreateTestTxn(testImodel).insertElement(targetProps);
        ...
        relationshipProps.id = getOrCreateTestTxn(testImodel).insertRelationship(relationshipProps);
    ...
    it("deleteInstances with an empty array", async () => {
      ...
      getOrCreateTestTxn(testImodel).deleteRelationships([]);
      getOrCreateTestTxn(testImodel).saveChanges();
    ...
    it("deleteInstances with a single relationship instance", async () => {
      ...
      getOrCreateTestTxn(testImodel).deleteRelationships([relationships[0]]);
      getOrCreateTestTxn(testImodel).saveChanges();
    ...
    it("deleteInstances with different relationship classes", async () => {
      ...
      getOrCreateTestTxn(testImodel).deleteRelationships(relationships);
      getOrCreateTestTxn(testImodel).saveChanges();
    ...
    it("deleteInstances for random relationship instances", async () => {
      ...
      getOrCreateTestTxn(testImodel).deleteRelationships(relationshipsToDelete);
      getOrCreateTestTxn(testImodel).saveChanges();
