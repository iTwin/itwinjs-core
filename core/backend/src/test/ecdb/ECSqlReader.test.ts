  ...
  describe("Common usages", () => {
    ...
    describe("Get id from each row", () => {
      ...
      it("Get id using iterable iterator with UseJsPropertyNames rowFormat", async () => {
        let counter = 1;
        for await (
          const row of iModel.createQueryReader("SELECT * FROM meta.ECSchemaDef", undefined, {
            limit: { count: 5 },
      ...
      it("Get id using step with UseJsPropertyNames rowFormat", async () => {
        let counter = 1;
        reader = iModel.createQueryReader("SELECT * FROM meta.ECSchemaDef", undefined, {
          limit: { count: 5 },
          rowFormat: QueryRowFormat.UseJsPropertyNames,
        });
        while (await reader.step()) {
    ...
    describe("Get duplicate property names", () => {
      it("Get duplicate property names using iterable iterator with unspecified rowFormat", async () => {
        ...
        let counter = 1;
        for await (
          const row of iModel.createQueryReader("SELECT * FROM bis.Element c JOIN bis.Element p ON p.ECInstanceId = c.ECInstanceId", undefined, {
            limit: { count: 5 },
          })
        ) {
    ...
    describe("Get specific values", () => {
      ...
      it("Get only ECInstanceId with UseJsPropertyNames rowFormat", async () => {
        let counter = 1;
        reader = iModel.createQueryReader("SELECT ECInstanceId FROM meta.ECSchemaDef ORDER BY ECInstanceId ASC", undefined, {
          limit: { count: 5 },
          rowFormat: QueryRowFormat.UseJsPropertyNames,
        });
        while (await reader.step()) {
      ...
      it("Get one column with custom name with unspecified rowFormat", async () => {
        let counter = 1;
        reader = iModel.createQueryReader("SELECT ECInstanceId customColumnName FROM meta.ECSchemaDef ORDER BY ECInstanceId ASC", undefined, {
          limit: { count: 5 },
        });
        while (await reader.step()) {
          const currentExpectedId = `0x${counter}`;
