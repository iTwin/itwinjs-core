...
export class ECDbTestHelper {
  public static createECDb(
    ...
    if (IModelJsFs.existsSync(outPath)) IModelJsFs.unlinkSync(outPath);

    const ecdb = new ECDb();
    ecdb.createDb(outPath);

    if (!schemaXml) return ecdb;

    const schemaPath = path.join(outDir, `${Guid.createValue()}.ecschema.xml`);
    if (IModelJsFs.existsSync(schemaPath)) IModelJsFs.unlinkSync(schemaPath);
