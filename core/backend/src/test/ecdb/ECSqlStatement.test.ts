...
const selectSingleRow = new QueryOptionsBuilder()
  .setLimit({ count: 1, offset: -1 })
  .setRowFormat(QueryRowFormat.UseJsPropertyNames)
  .getOptions();
async function query(
  ...
  for await (const queryRow of ecdb.createQueryReader(ecsql, params, {
    ...config,
    rowFormat: QueryRowFormat.UseJsPropertyNames,
  })) {
    rowCount++;
    if (callback) callback(queryRow.toRow());
  }
  return rowCount;
}
async function queryRows(
  ...
  ecdb.saveChanges();
  const reader = ecdb.createQueryReader(ecsql, params, {
    ...config,
    rowFormat: QueryRowFormat.UseJsPropertyNames,
  });
...
async function queryCount(
  ...
  for await (const row of ecdb.createQueryReader(
    ...
    params,
    { ...config, rowFormat: QueryRowFormat.UseECSqlPropertyIndexes }
  )) {
...
describe("ECSqlStatement", () => {
    ...
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        const r = await ecdb.withStatement(
          "INSERT INTO ts.Foo(n,dt,fooId) VALUES(20,TIMESTAMP '2018-10-18T12:00:00Z',20)",
  ...
  it("concurrent query get meta data", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        await ecdb.withStatement(
          "INSERT INTO ts.Foo(n,dt,fooId) VALUES(30,TIMESTAMP '2019-10-18T12:00:00Z',30)",
          async (stmt: ECSqlStatement) => {
            stmt.stepForInsert();
          }
        );
  ...
  it("null string accessor", async () => {
    await using(
      ECDbTestHelper.createECDb(outDir, "nullstring.ecdb"),
      async (ecdb: ECDb) => {
        assert.isTrue(ecdb.isOpen);
        await ecdb.withPreparedStatement(
          `VALUES(NULL)`,
          async (stmt: ECSqlStatement) => {
            stmt.step();
      ...
      async (ecdb: ECDb) => {
        ...
        for (let i = 1; i < ROW_COUNT; i++) {
          const rowCount = await queryCount(
            ecdb,
            "SELECT ECInstanceId, ECClassId, n FROM ts.Foo WHERE n <= ?",
            new QueryBinder().bindInt(1, i)
          );
          assert.equal(rowCount, i);
        }

        const temp = await queryRows(
          ecdb,
          "SELECT ECInstanceId FROM ONLY ts.Foo"
        );
          ...
          ready.push(
            queryRows(
              ...
              new QueryOptionsBuilder()
                .setLimit({ offset: i * PAGE_SIZE, count: PAGE_SIZE })
                .getOptions()
            )
          );
        }
  ...
  it("paging use cache statement queryRows()", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        const rca = await queryRows(
          ecdb,
          "SELECT count(*) as nRows FROM ts.Foo"
        );
  ...
  it("should restart query", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        ecdb.saveChanges();
        ConcurrentQuery.resetConfig(ecdb.nativeDb, {
          globalQuota: { time: 1 },
          ignoreDelay: false,
        });
        ...
        const scheduleQuery = async (delay: number) => {
          return new Promise<void>(async (resolve, reject) => {
            try {
              ...
              for await (const _row of ecdb.createQueryReader(
                "SELECT * FROM ts.Foo",
                undefined,
                options.getOptions()
              )) {
    ...
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        for (const _testPageSize of [1, 2, 4, 5, 6, 7, 10, ROW_COUNT]) {
          ...
          let rowNo = 1;
          for await (const row of ecdb.createQueryReader(
            "SELECT n FROM ts.Foo WHERE n != ? and ECInstanceId < ?",
            new QueryBinder().bindInt(1, 123).bindInt(2, 30),
            { rowFormat: QueryRowFormat.UseJsPropertyNames }
          )) {
  ...
  it("concurrent query binding", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        assert.isTrue(ecdb.isOpen);
        for (let i = 1; i <= 5; i++) {
          const r = await ecdb.withPreparedStatement(
            `insert into ts.Foo(n) values(${i})`,
            async (stmt: ECSqlStatement) => {
              return stmt.stepForInsert();
            }
        ...
        ecdb.saveChanges();
        for await (const row of ecdb.createQueryReader(
          "SELECT count(*) as cnt FROM ts.Foo WHERE n in (:a, :b, :c)",
          new QueryBinder().bindInt("a", 1).bindInt("b", 2).bindInt("c", 3),
          { rowFormat: QueryRowFormat.UseJsPropertyNames }
        )) {
          assert.equal(row.cnt, 3);
        }
        for await (const row of ecdb.createQueryReader(
          "SELECT count(*) as cnt FROM ts.Foo WHERE n in (?, ?, ?)",
          new QueryBinder().bindInt(1, 1).bindInt(2, 2).bindInt(3, 3),
          { rowFormat: QueryRowFormat.UseJsPropertyNames }
        )) {
          assert.equal(row.cnt, 3);
        }
        ...
        slm
          ...
          .error()
          .category("ECDb")
          .message("No parameter index found for parameter name: d.");
        try {
          for await (const row of ecdb.createQueryReader(
            "SELECT count(*) as cnt FROM ts.Foo WHERE n in (:a, :b, :c)",
            new QueryBinder()
  ...
  it("check HextoId() and IdToHex() ecsql functions", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        ecdb.saveChanges();
        for await (const row of ecdb.createQueryReader(
          "SELECT IdToHex(ECInstanceId) as hexId, ECInstanceId, HexToId('0x1') as idhex FROM ts.Foo WHERE n = ?",
          new QueryBinder().bindInt(1, 1),
          { rowFormat: QueryRowFormat.UseJsPropertyNames }
        )) {
    ...
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        for (let i = 0; i < maxRows; i++) {
          const r = await ecdb.withPreparedStatement(
            `insert into ts.Foo(guid) values(?)`,
            async (stmt: ECSqlStatement) => {
        ...
        const guidToUint8Array = (v: GuidString) => {
          if (v.length !== 36)
            throw new Error(
              "Guid is expected to have 36 characters xxxxxxxx-xxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
            );
        ...
        for (const guid of guids) {
          assert.equal(
            await query(
              ecdb,
              "SELECT guid FROM ts.Foo WHERE guid=?",
              new QueryBinder().bindString(1, guid),
              undefined,
              (row: any) => {
          ...
          assert.equal(
            await query(
              ecdb,
              `SELECT guid FROM ts.Foo WHERE guid='${guid}'`,
              undefined,
              undefined,
              (row: any) => {
                assert.equal(row.guid, guid);
          ...
          assert.equal(
            await query(
              ecdb,
              `SELECT guid FROM ts.Foo WHERE guid=StrToGuid('${guid}')`,
              undefined,
              undefined,
              (row: any) => {
                assert.equal(row.guid, guid);
          ...
          assert.equal(
            await query(
              ecdb,
              "SELECT guid FROM ts.Foo WHERE guid=StrToGuid(?)",
              new QueryBinder().bindString(1, guid),
              undefined,
              (row: any) => {
                assert.equal(row.guid, guid);
          ...
          assert.equal(
            await query(
              ecdb,
              "SELECT guid FROM ts.Foo WHERE GuidToStr(guid)=?",
              new QueryBinder().bindString(1, guid),
              undefined,
              (row: any) => {
                assert.equal(row.guid, guid);
          ...
          assert.equal(
            await query(
              ecdb,
              "SELECT guid FROM ts.Foo WHERE guid=?",
              new QueryBinder().bindBlob(1, guidToUint8Array(guid)),
              undefined,
              (row: any) => {
                assert.equal(row.guid, guid);
              }
          ...
          assert.equal(
            await query(
              ecdb,
              "SELECT GuidToStr(guid) as gstr FROM ts.Foo WHERE guid=StrToGuid(?)",
              new QueryBinder().bindString(1, guid),
              undefined,
              (row: any) => {
  ...
  it("should bind Ids", async () => {
    await using(
      ECDbTestHelper.createECDb(outDir, "bindids.ecdb"),
      async (ecdb: ECDb) => {
        assert.isTrue(ecdb.isOpen);

        const verify = async (
          ecdbToVerify: ECDb,
          actualRes: ECSqlInsertResult,
          expectedECInstanceId?: Id64String
        ) => {
          ...
          ecdbToVerify.withPreparedStatement(
            "SELECT ECInstanceId, ECClassId, Name FROM ecdbf.ExternalFileInfo WHERE ECInstanceId=?",
            (stmt: ECSqlStatement) => {
              ...
              assert.equal(
                row.name,
                `${Id64.getLocalId(expectedECInstanceId).toString()}.txt`
              );
            }
          );
          assert.equal(
        ...
        let r: ECSqlInsertResult = ecdb.withPreparedStatement(
          "INSERT INTO ecdbf.ExternalFileInfo(ECInstanceId,Name) VALUES(?,?)",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, expectedId);
        ...
        r = ecdb.withPreparedStatement(
          "INSERT INTO ecdbf.ExternalFileInfo(ECInstanceId,Name) VALUES(:id,:name)",
          (stmt: ECSqlStatement) => {
  ...
  it("should bind numeric and date strings", async () => {
    ...
    slm
      ...
      .category("ECDb")
      .message(
        "Type mismatch: only BindDateTime or BindText can be called for a column of the DateTime type."
      );
    slm
      .append()
      .error()
    ...
    slm
      ...
      .error()
      .category("ECDb")
      .message(/String must be a valid ISO 8601 date, time or timestamp/gm);
    slm
      .append()
      .error()
      .category("ECDb")
    ...
    slm
      ...
      .category("ECDb")
      .message(
        /only BindDateTime or BindText can be called for a column of the DateTime type/gm
      );

    await using(
      ECDbTestHelper.createECDb(
        outDir,
        "bindnumericanddatestrings.ecdb",
        `<ECSchema schemaName="Test" alias="ts" version="01.00.00" xmlns="http://www.bentley.com/schemas/Bentley.ECXML.3.2">
        <ECEntityClass typeName="Foo" modifier="Sealed">
          <ECProperty propertyName="n" typeName="int"/>
          <ECProperty propertyName="dt" typeName="dateTime"/>
      ...
      async (ecdb: ECDb) => {
        ...
        const r: ECSqlInsertResult = ecdb.withPreparedStatement(
          "INSERT INTO ts.Foo(n,dt,fooId) VALUES(20,TIMESTAMP '2018-10-18T12:00:00Z',20)",
          (stmt: ECSqlStatement) => {
            return stmt.stepForInsert();
          }
          ...
          async (stmt: ECSqlStatement) => {
            ...
            assert.equal(
              await queryCount(
                ecdb,
                ecsqln,
                new QueryBinder().bindInt(1, nNum),
                selectSingleRow
              ),
              1
            );
            ...
            assert.equal(
              await queryCount(
                ecdb,
                ecsqln,
                new QueryBinder().bindString(1, nStr),
                selectSingleRow
              ),
              1
            );
            ...
            assert.equal(
              ...
              "Date time string is not parsed. SQLite just converts it to something which does not match"
            );
            stmt.reset();
            stmt.clearBindings();
            ...
            assert.equal(
              await queryCount(
                ecdb,
                ecsqln,
                new QueryBinder().bindString(1, nDt),
                selectSingleRow
              ),
              0
            );
            ...
            assert.equal(
              stmt.step(),
              DbResult.BE_SQLITE_DONE,
              "Hex string is not parsed. SQLite just converts it to something which does not match"
            );
            stmt.reset();
            stmt.clearBindings();
            ...
            assert.equal(
              await queryCount(
                ecdb,
                ecsqln,
                new QueryBinder().bindString(1, nHexStr),
                selectSingleRow
              ),
              0
            );
          ...
          async (stmt: ECSqlStatement) => {
            ...
            assert.equal(
              await queryCount(
                ecdb,
                ecsqldt,
                new QueryBinder().bindString(1, dtStr)
              ),
              1
            );
            ...
            assert.equal(
              await queryCount(ecdb, ecsqldt, QueryBinder.from([num])),
              0
            );
            ...
            assert.equal(
              await queryCount(ecdb, ecsqldt, QueryBinder.from([str])),
              0
            );
            ...
            assert.equal(
              await queryCount(ecdb, ecsqldt, QueryBinder.from([hexStr])),
              0
            );
        ...
        await ecdb.withPreparedStatement(
          ...
          async (stmt: ECSqlStatement) => {
            ...
            assert.equal(
              stmt.step(),
              DbResult.BE_SQLITE_DONE,
              "DateTime string is not parsed into what it means. SQlite just uses its regular string conversion routines which don't match here"
            );
            stmt.reset();
            stmt.clearBindings();

            assert.equal(
              await queryCount(ecdb, ecsqldt, QueryBinder.from([dt])),
              0
            );
  ...
  it("should bind numbers", async () => {
    ...
    <ECEntityClass typeName="Foo" modifier="Sealed">
      ...
      async (ecdb: ECDb) => {
        ...
        let id = await ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindDouble')",
          async (stmt: ECSqlStatement) => {
        ...
        await ecdb.withPreparedStatement(
          "SELECT D,I,L,S FROM Test.Foo WHERE ECInstanceId=?",
          async (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindInteger, large unsafe number as string')",
          (stmt: ECSqlStatement) => {
        ...
        ecdb.withPreparedStatement(
          "SELECT Str(I) si, HexStr(I) hi, Str(L) sl, HexStr(L) hl FROM Test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindInteger, large unsafe number as hexstring')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindString, large unsafe number as string')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindString, large unsafe number as hexstring')",
          (stmt: ECSqlStatement) => {
        ...
        ecdb.withPreparedStatement(
          "SELECT CAST(D AS TEXT) d,CAST(I AS TEXT) i,CAST(L AS TEXT) l,S FROM Test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindInteger, large negative unsafe number as string')",
          (stmt: ECSqlStatement) => {
        ...
        ecdb.withPreparedStatement(
          "SELECT CAST(I AS TEXT) i, CAST(L AS TEXT) l,S FROM Test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindString, large negative unsafe number as string')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindInteger, large safe number')",
          (stmt: ECSqlStatement) => {
        ...
        ecdb.withPreparedStatement(
          "SELECT D,I, Str(I) si, HexStr(I) hi, L, Str(L) sl, HexStr(L) hl,S FROM Test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindInteger, large safe number as string')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindInteger, large safe number as hexstring')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindString, large safe number as string')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindString, large safe number as hexstring')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindInteger, large negative safe number')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindInteger, large negative safe number as string')",
          (stmt: ECSqlStatement) => {
        ...
        id = ecdb.withPreparedStatement(
          "INSERT INTO Test.Foo(D,I,L,S,Description) VALUES(?,?,?,?,'bindString, large negative safe number as string')",
          (stmt: ECSqlStatement) => {
    ...
    <ECEntityClass typeName="Foo" modifier="Sealed">
      ...
      async (ecdb) => {
        ...
        const verify = async (expectedId: Id64String) => {
          await ecdb.withPreparedStatement(
            "SELECT Bl,Bo,D,Dt,I,P2d,P3d,S,Struct.Bl s_bl,Struct.Bo s_bo,Struct.D s_d,Struct.Dt s_dt,Struct.I s_i,Struct.P2d s_p2d,Struct.P3d s_p3d,Struct.S s_s FROM test.Foo WHERE ECInstanceId=?",
            async (stmt: ECSqlStatement) => {
              stmt.bindId(1, expectedId);
              assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
              const row = stmt.getRow();
        ...
        ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Bl,Bo,D,Dt,I,P2d,P3d,S,Struct.Bl,Struct.Bo,Struct.D,Struct.Dt,Struct.I,Struct.P2d,Struct.P3d,Struct.S) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
          (stmt: ECSqlStatement) => {
        ...
        ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Bl,Bo,D,Dt,I,P2d,P3d,S,Struct.Bl,Struct.Bo,Struct.D,Struct.Dt,Struct.I,Struct.P2d,Struct.P3d,Struct.S) VALUES(:bl,:bo,:d,:dt,:i,:p2d,:p3d,:s,:s_bl,:s_bo,:s_d,:s_dt,:s_i,:s_p2d,:s_p3d,:s_s)",
          (stmt: ECSqlStatement) => {
  ...
  it("should bind structs", async () => {
    await using(
      ...
      async (ecdb) => {
        ...
        const verify = async (expectedId: Id64String) => {
          await ecdb.withPreparedStatement(
            "SELECT Struct FROM test.Foo WHERE ECInstanceId=?",
            async (stmt: ECSqlStatement) => {
              stmt.bindId(1, expectedId);
              assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
              const row = stmt.getRow();
        ...
        await ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Struct) VALUES(?)",
          async (stmt: ECSqlStatement) => {
        ...
        await ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Struct) VALUES(:str)",
          async (stmt: ECSqlStatement) => {
  ...
  it("should bind arrays", async () => {
    await using(
      ...
      async (ecdb) => {
        ...
        const addressArray = [
          { city: "London", zip: 10000 },
          { city: "Manchester", zip: 20000 },
          { city: "Edinburgh", zip: 30000 },
        ];

        const verify = async (expectedId: Id64String) => {
          await ecdb.withPreparedStatement(
            "SELECT I_Array, Dt_Array, Addresses FROM test.Foo WHERE ECInstanceId=?",
            async (stmt: ECSqlStatement) => {
              stmt.bindId(1, expectedId);
              assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
              const row = stmt.getRow();
        ...
        await ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(I_Array,Dt_Array,Addresses) VALUES(?,?,?)",
          async (stmt: ECSqlStatement) => {
        ...
        await ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(I_Array,Dt_Array,Addresses) VALUES(:iarray,:dtarray,:addresses)",
          async (stmt: ECSqlStatement) => {
  ...
  it("should bind navigation", async () => {
    ...
    <ECRelationshipClass typeName="ParentHasChildren" modifier="None" strength="embedding">
      ...
      async (ecdb) => {
        ...
        const parentId: Id64String = ecdb.withStatement(
          "INSERT INTO test.Parent(Code) VALUES('Parent 1')",
          (stmt: ECSqlStatement) => {
            const res: ECSqlInsertResult = stmt.stepForInsert();
            assert.equal(res.status, DbResult.BE_SQLITE_DONE);
            assert.isDefined(res.id);
        ...
        ecdb.withStatement(
          "INSERT INTO test.Child(Name,Parent) VALUES(?,?)",
          (stmt: ECSqlStatement) => {
            stmt.bindString(1, "Child 1");
            stmt.bindNavigation(2, {
              id: parentId,
              relClassName: "Test.ParentHasChildren",
            });
            let res: ECSqlInsertResult = stmt.stepForInsert();
            assert.equal(res.status, DbResult.BE_SQLITE_DONE);
            assert.isDefined(res.id);
            ...
            stmt.bindValues([
              "Child 2",
              { id: parentId, relClassName: "Test.ParentHasChildren" },
            ]);
            res = stmt.stepForInsert();
            assert.equal(res.status, DbResult.BE_SQLITE_DONE);
            assert.isDefined(res.id);
        ...
        ecdb.withStatement(
          "INSERT INTO test.Child(Name,Parent) VALUES(:name,:parent)",
          (stmt: ECSqlStatement) => {
            stmt.bindString("name", "Child 3");
            stmt.bindNavigation("parent", {
              id: parentId,
              relClassName: "Test.ParentHasChildren",
            });
            let res: ECSqlInsertResult = stmt.stepForInsert();
            ...
            stmt.bindValues({
              name: "Child 4",
              parent: { id: parentId, relClassName: "Test.ParentHasChildren" },
            });
            res = stmt.stepForInsert();
            assert.equal(res.status, DbResult.BE_SQLITE_DONE);
            assert.isDefined(res.id);
        ...
        ecdb.withPreparedStatement(
          "SELECT Name,Parent FROM test.Child ORDER BY Name",
          (stmt: ECSqlStatement) => {
            let rowCount: number = 0;
  ...
  it("should bind Range3d for parameter in spatial sql function", async () => {
    const iModel = SnapshotDb.createEmpty(
      IModelTestUtils.prepareOutputFile("ECSqlStatement", "BindRange3d.bim"),
      { rootSubject: { name: "BindRange3d" } }
    );
    try {
      iModel.withPreparedStatement(
        "SELECT e.ECInstanceId FROM bis.Element e, bis.SpatialIndex rt WHERE rt.ECInstanceId MATCH DGN_spatial_overlap_aabb(?) AND e.ECInstanceId=rt.ECInstanceId",
        (stmt: ECSqlStatement) => {
          stmt.bindRange3d(
            1,
            new Range3d(0.0, 0.0, 0.0, 1000.0, 1000.0, 1000.0)
          );
  ...
  it("should bind Range3d", async () => {
    await using(
      ...
      async (ecdb) => {
        ...
        const id: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Range) VALUES(?)",
          (stmt: ECSqlStatement) => {
        ...
        ecdb.saveChanges();
        ecdb.withPreparedStatement(
          "SELECT Range FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
  ...
  it("should bind IdSets", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        ecdb.withPreparedStatement(
          ...
          (stmt: ECSqlStatement) => {
            idNumbers.forEach((idNum: number) => {
              ...
              ecdb.withStatement(
                `SELECT ECInstanceId, ECClassId, Name FROM ecdbf.ExternalFileInfo WHERE ECInstanceId=${expectedId}`,
                (confstmt: ECSqlStatement) => {
                  ...
                  assert.equal(
                    row.name,
                    `${Id64.getLocalId(expectedId).toString()}.txt`
                  );
                }
              );
              stmt.reset();
        ...
        ecdb.withPreparedStatement(
          "SELECT ECInstanceId, ECClassId, Name from ecdbf.ExternalFileInfo WHERE InVirtualSet(?, ECInstanceId)",
          (stmt: ECSqlStatement) => {
  ...
  it("check ECSqlBinder life time", async () => {
    ...
    <ECEntityClass typeName="Person" modifier="Sealed">
      ...
      async (ecdb) => {
        ...
        using(
          ecdb.prepareStatement(
            "INSERT INTO test.Person(Name,Age,Location) VALUES(?,?,?)"
          ),
          (stmt: ECSqlStatement) => {
        ...
        using(
          ecdb.prepareStatement(
            "SELECT ECInstanceId,ECClassId,Name,Age,Location FROM test.Person ORDER BY ECInstanceId"
          ),
          (stmt: ECSqlStatement) => {
            let rowCount = 0;
            while (stmt.step() === DbResult.BE_SQLITE_ROW) {
              rowCount++;
              const row = stmt.getRow();
              if (rowCount === 1) assert.equal(row.id, id1);
              else assert.equal(row.id, id2);
        ...
        assert.equal(
          await query(
            ...
            (row: any) => {
              rowCount2++;
              if (rowCount2 === 1) assert.equal(row.id, id1);
              else assert.equal(row.id, id2);
  ...
  it("getRow() with primitives values", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        const id: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Bl,Bo,D,Dt,I,P2d,P3d,S) VALUES(?,?,?,?,?,?,?,?)",
          (stmt: ECSqlStatement) => {
            stmt.bindBlob(1, blobVal);
            stmt.bindBoolean(2, boolVal);
            stmt.bindDouble(3, doubleVal);
        ...
        ecdb.withPreparedStatement(
          "SELECT ECInstanceId, ECClassId, Bl,Bo,D,Dt,I,P2d,P3d,S FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        assert.equal(
          await query(
            ecdb,
            "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?",
            QueryBinder.from([id]),
        ...
        assert.equal(
          await query(
            ...
            "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?",
            QueryBinder.from([id]),
            { abbreviateBlobs: false, ...selectSingleRow },
            (row: any) => {
              assert.equal(row.id, id);
              assert.equal(row.className, "Test.Foo");
              assert.deepEqual(row.bl, blobVal);
        ...
        ecdb.withPreparedStatement(
          "SELECT Bl AS Blobby, I+10, Lower(S), Upper(S) CapitalS FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        const testSchemaId: Id64String = ecdb.withPreparedStatement(
          "SELECT ECInstanceId FROM meta.ECSchemaDef WHERE Name='Test'",
          (stmt: ECSqlStatement) => {
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
            assert.equal(stmt.step(), DbResult.BE_SQLITE_DONE);
        ...
        const fooClassId: Id64String = ecdb.withPreparedStatement(
          "SELECT ECInstanceId FROM meta.ECClassDef WHERE Name='Foo'",
          (stmt: ECSqlStatement) => {
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
            assert.equal(stmt.step(), DbResult.BE_SQLITE_DONE);
        ...
        ecdb.withPreparedStatement(
          "SELECT s.ECInstanceId, c.ECInstanceId, c.Name, s.Name FROM meta.ECClassDef c JOIN meta.ECSchemaDef s ON c.Schema.Id=s.ECInstanceId WHERE s.Name='Test' AND c.Name='Foo'",
          (stmt: ECSqlStatement) => {
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        ecdb.withPreparedStatement(
          "SELECT count(*) cnt FROM meta.ECSchemaDef",
          (stmt: ECSqlStatement) => {
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        ecdb.withPreparedStatement(
          "SELECT 1 FROM meta.ECSchemaDef LIMIT 1",
          (stmt: ECSqlStatement) => {
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
        ...
        ecdb.withPreparedStatement(
          "SELECT NULL FROM meta.ECSchemaDef LIMIT 1",
          (stmt: ECSqlStatement) => {
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
  ...
  it("getRow() with abbreviated blobs", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        const fullId: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Bl) VALUES(?)",
          (stmt: ECSqlStatement) => {
            stmt.bindBlob(1, blobVal);
            const res: ECSqlInsertResult = stmt.stepForInsert();
            assert.equal(res.status, DbResult.BE_SQLITE_DONE);
        ...
        const singleId: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Bl) VALUES(?)",
          (stmt: ECSqlStatement) => {
        ...
        const emptyId: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Bl) VALUES(?)",
          (stmt: ECSqlStatement) => {
        ...
        const nullId: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Bl) VALUES(?)",
          (stmt: ECSqlStatement) => {
            stmt.bindNull(1);
            const res: ECSqlInsertResult = stmt.stepForInsert();
  ...
  it("getRow() with navigation properties and relationships", async () => {
    await using(
      ...
      async (ecdb) => {
        assert.isTrue(ecdb.isOpen);
        let rowCount: number;
        const parentId: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Parent(Code) VALUES('Parent 1')",
          (stmt: ECSqlStatement) => {
            const res: ECSqlInsertResult = stmt.stepForInsert();
            assert.equal(res.status, DbResult.BE_SQLITE_DONE);
        ...
        ecdb.withPreparedStatement(
          "SELECT Name,Parent.Id,Parent.RelECClassId, Parent.Id myParentId, Parent.RelECClassId myParentRelClassId FROM test.Child ORDER BY Name",
          (stmt: ECSqlStatement) => {
            rowCount = 0;
            while (stmt.step() === DbResult.BE_SQLITE_ROW) {
              rowCount++;
              ...
              assert.equal(row["parent.id"], parentId);
              assert.equal(
                row["parent.relClassName"],
                "Test.ParentHasChildren"
              );
        ...
        ecdb.withPreparedStatement(
          "SELECT ECInstanceId,ECClassId,SourceECInstanceId,SourceECClassId,TargetECInstanceId,TargetECClassId FROM test.ParentHasChildren WHERE TargetECInstanceId=?",
          (stmt: ECSqlStatement) => {
        ...
        ecdb.withPreparedStatement(
          "SELECT ECInstanceId as MyId,ECClassId as MyClassId,SourceECInstanceId As MySourceId,SourceECClassId As MySourceClassId,TargetECInstanceId As MyTargetId,TargetECClassId As MyTargetClassId FROM test.ParentHasChildren WHERE TargetECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, childId);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row = stmt.getRow();
  ...
  it("getRow() with structs", async () => {
    await using(
      ...
      async (ecdb) => {
        ...
        ecdb.withPreparedStatement(
          "SELECT Struct.Bl, Struct.Bo, Struct.D, Struct.Dt, Struct.I, Struct.P2d, Struct.P3d, Struct.S FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row: any = stmt.getRow();
  ...
  it("check HexStr() sql function", async () => {
    ...
    slm
      ...
      .error()
      .category("ECDb")
      .message(/Step failed for ECSQL/gm);
    slm
      .append()
      .error()
      .category("ECDb")
    ...
    slm
      ...
      .error()
      .category("ECDb")
      .message(/Failed to prepare function expression/gm);
    slm
      .append()
      .error()
      .category("ECDb")
    ...
    await using(
      ...
      async (ecdb) => {
        ...
        const id: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(Bl,Bo,D,Dt,I,L,P2d,P3d,S) VALUES(:bl,:bo,:d,:dt,:i,:l,:p2d,:p3d,:s)",
          (stmt: ECSqlStatement) => {
            stmt.bindValues({
              bl: blobVal,
              bo: expectedRow.bo,
        ...
        ecdb.saveChanges();
        ecdb.withPreparedStatement(
          "SELECT I, HexStr(I) hex FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row: any = stmt.getRow();
        ...
        ecdb.withPreparedStatement(
          "SELECT L, HexStr(L) hex FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row: any = stmt.getRow();
        ...
        ecdb.withPreparedStatement(
          "SELECT Bl, HexStr(Bl) hex FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
        ...
        ecdb.withPreparedStatement(
          "SELECT Bo, HexStr(Bo) hex FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row: any = stmt.getRow();
        ...
        ecdb.withPreparedStatement(
          "SELECT D, HexStr(D) hex FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ERROR);
          }
        ...
        ecdb.withPreparedStatement(
          "SELECT Dt, HexStr(Dt) hex FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ERROR);
          }
        ...
        assert.throw(() =>
          ecdb.withPreparedStatement(
            "SELECT P2d, HexStr(P2d) hex FROM test.Foo WHERE ECInstanceId=?",
            () => {
              assert.fail();
            }
          )
        ...
        assert.throw(() =>
          ecdb.withPreparedStatement(
            "SELECT P3d, HexStr(P3d) hex FROM test.Foo WHERE ECInstanceId=?",
            () => {
              assert.fail();
            }
          )
        ...
        ecdb.withPreparedStatement(
          "SELECT S, HexStr(S) hex FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ERROR);
          }
  ...
  it("check ec enums", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        const id: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(MyStat,MyStats,MyDomain,MyDomains) VALUES(test.Status.[On],?,test.Domain.Org,?)",
          (stmt: ECSqlStatement) => {
        ...
        ecdb.withPreparedStatement(
          "SELECT MyStat,MyStats, MyDomain,MyDomains FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            ...
            assert.deepEqual(myStatVal.getEnum(), [
              { schema: "Test", name: "Status", key: "On", value: 1 },
            ]);
            ...
            assert.deepEqual(actualStatsEnums[0], [
              { schema: "Test", name: "Status", key: "On", value: 1 },
            ]);
            assert.deepEqual(actualStatsEnums[1], [
              { schema: "Test", name: "Status", key: "Off", value: 2 },
            ]);
            ...
            assert.deepEqual(myDomainVal.getEnum(), [
              { schema: "Test", name: "Domain", key: "Org", value: "Org" },
            ]);
            ...
            assert.deepEqual(actualDomainsEnums[0], [
              { schema: "Test", name: "Domain", key: "Org", value: "Org" },
            ]);
            assert.deepEqual(actualDomainsEnums[1], [
              { schema: "Test", name: "Domain", key: "Com", value: "Com" },
            ]);
          }
        );
        ...
        ecdb.withPreparedStatement(
          "SELECT Type,Modifier FROM meta.ECClassDef WHERE Name='Foo'",
          (stmt: ECSqlStatement) => {
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            // getRow just returns the enum values
            const row: any = stmt.getRow();
            ...
            assert.deepEqual(typeVal.getEnum(), [
              {
                schema: "ECDbMeta",
                name: "ECClassType",
                key: "Entity",
            ...
            assert.deepEqual(modifierVal.getEnum(), [
              {
                schema: "ECDbMeta",
                name: "ECClassModifier",
                key: "Sealed",
                value: 2,
              },
            ]);
  ...
  it("check ORed ECEnums", async () => {
    ...
    slm
      ...
      .category("ECDb")
      .message(
        /The value 9 cannot be broken down into a combination of ECEnumerators/gm
      );
    slm
      .append()
      .error()
      .category("ECDb")
      .message(
        /The value 'gov,de' cannot be broken down into a combination of ECEnumerators/gm
      );
    ...
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        const ids: {
          unored: Id64String;
          ored: Id64String;
          unmatched: Id64String;
        } = ecdb.withPreparedStatement(
          "INSERT INTO test.Foo(MyColor,MyDomain) VALUES(?,?)",
          (stmt: ECSqlStatement) => {
            stmt.bindValue(1, 4);
        ...
        ecdb.withPreparedStatement(
          "SELECT MyColor,MyDomain FROM test.Foo WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            ...
            assert.deepEqual(colVal.getEnum(), [
              { schema: "Test", name: "Color", key: "Blue", value: 4 },
            ]);
            ...
            assert.deepEqual(domainVal.getEnum(), [
              { schema: "Test", name: "Domain", key: "Com", value: "com" },
            ]);
            stmt.reset();
            stmt.clearBindings();
            ...
            assert.deepEqual(colVal.getEnum(), [
              { schema: "Test", name: "Color", key: "Red", value: 1 },
              { schema: "Test", name: "Color", key: "Blue", value: 4 },
            ]);
            ...
            assert.deepEqual(domainVal.getEnum(), [
              { schema: "Test", name: "Domain", key: "Com", value: "com" },
              { schema: "Test", name: "Domain", key: "Gov", value: "gov" },
            ]);
        ...
        assert.equal(
          await query(
            ecdb,
            "SELECT MyColor,MyDomain FROM test.Foo WHERE ECInstanceId=?",
            QueryBinder.from([ids.unmatched]),
            undefined,
            (row: any) => {
              assert.equal(row.myColor, 9);
              assert.equal(row.myDomain, "gov,de");
        ...
        // test some enums in the built-in schemas
        ecdb.withPreparedStatement(
          "SELECT CustomAttributeContainerType caType FROM meta.ECClassDef WHERE Type=meta.ECClassType.CustomAttribute AND Name='DateTimeInfo'",
          (stmt: ECSqlStatement) => {
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            const row: any = stmt.getRow();
            ...
            assert.deepEqual(caTypeVal.getEnum(), [
              {
                schema: "ECDbMeta",
                name: "ECCustomAttributeContainerType",
                key: "PrimitiveProperty",
                value: 32,
              },
              {
                schema: "ECDbMeta",
                name: "ECCustomAttributeContainerType",
                key: "PrimitiveArrayProperty",
                value: 128,
              },
            ]);
          }
  ...
  it("check column info", async () => {
    await using(
      ...
      async (ecdb: ECDb) => {
        ...
        const id: Id64String = ecdb.withPreparedStatement(
          "INSERT INTO test.MyClass(MyProperty) VALUES('Value')",
          (stmt: ECSqlStatement) => {
            const res: ECSqlInsertResult = stmt.stepForInsert();
            assert.equal(res.status, DbResult.BE_SQLITE_DONE);
            assert.isDefined(res.id);
        ...
        ecdb.withPreparedStatement(
          "SELECT MyProperty as MyAlias, 1 as MyGenerated FROM test.MyClass WHERE ECInstanceId=?",
          (stmt: ECSqlStatement) => {
            stmt.bindId(1, id);
            assert.equal(stmt.step(), DbResult.BE_SQLITE_ROW);
            // getRow just returns the enum values
