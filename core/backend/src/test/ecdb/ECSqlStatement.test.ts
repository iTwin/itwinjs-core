...
async function queryCount(ecdb: ECDb, ecsql: string, params?: QueryBinder, config?: QueryOptions): Promise<number> {
  ecdb.saveChanges();
  for await (
    const row of ecdb.createQueryReader(`SELECT COUNT(*) FROM (${ecsql})`, params, { ...config, rowFormat: QueryRowFormat.UseECSqlPropertyIndexes })
  ) {
...
describe("ECSqlStatement", () => {
  ...
  it("null string accessor", async () => {
    using(ECDbTestHelper.createECDb(outDir, "nullstring.ecdb"), async (ecdb: ECDb) => {
      assert.isTrue(ecdb.isOpen);
    ...
    using(
      ...
      async (ecdb: ECDb) => {
        ...
        for (const _testPageSize of [1, 2, 4, 5, 6, 7, 10, ROW_COUNT]) { // eslint-disable-line @typescript-eslint/no-unused-vars
          let rowNo = 1;
          for await (
            const row of ecdb.createQueryReader(
    ...
    using(
      ...
      async (ecdb: ECDb) => {
        ...
        for (const guid of guids) {
          assert.equal(
            await query(ecdb, "SELECT guid FROM ts.Foo WHERE guid=?", new QueryBinder().bindString(1, guid), undefined, (row: any) => {
          ...
          assert.equal(
            await query(ecdb, `SELECT guid FROM ts.Foo WHERE guid=StrToGuid('${guid}')`, undefined, undefined, (row: any) => {
              assert.equal(row.guid, guid);
            }),
            1,
          );
          assert.equal(
            await query(ecdb, "SELECT guid FROM ts.Foo WHERE guid=StrToGuid(?)", new QueryBinder().bindString(1, guid), undefined, (row: any) => {
              assert.equal(row.guid, guid);
            }),
            1,
          );
          assert.equal(
            await query(ecdb, "SELECT guid FROM ts.Foo WHERE GuidToStr(guid)=?", new QueryBinder().bindString(1, guid), undefined, (row: any) => {
              assert.equal(row.guid, guid);
            }),
            1,
          );
  ...
  it("should bind Ids", async () => {
    using(ECDbTestHelper.createECDb(outDir, "bindids.ecdb"), async (ecdb: ECDb) => {
      assert.isTrue(ecdb.isOpen);
  ...
  it("should bind numbers", async () => {
    ...
    <ECEntityClass typeName="Foo" modifier="Sealed">
      ...
      async (ecdb: ECDb) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT D,I,L,S FROM Test.Foo WHERE ECInstanceId=?", QueryBinder.from([id]), { limit: { count: 1 } }, (row: any) => {
            assert.equal(row.d, doubleVal);
            assert.equal(row.i, 3);
            assert.equal(row.l, 3);
            assert.equal(row.s, "3.5");
      ...
      async (ecdb) => {
        ...
        const verify = async (expectedId: Id64String) => {
          await ecdb.withPreparedStatement("SELECT Struct FROM test.Foo WHERE ECInstanceId=?", async (stmt: ECSqlStatement) => {
            ...
            assert.equal(
              await query(ecdb, "SELECT Struct FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([expectedId]), selectSingleRow, (row1: any) => {
  ...
  it("should bind navigation", async () => {
    ...
    <ECRelationshipClass typeName="ParentHasChildren" modifier="None" strength="embedding">
      ...
      async (ecdb) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT Name,Parent FROM test.Child ORDER BY Name", QueryBinder.from([]), undefined, (row: any) => {
            rowCount2++;
  ...
  it("check ECSqlBinder life time", async () => {
    ...
    <ECEntityClass typeName="Person" modifier="Sealed">
      ...
      async (ecdb) => {
        ...
        using(
          ecdb.prepareStatement("SELECT ECInstanceId,ECClassId,Name,Age,Location FROM test.Person ORDER BY ECInstanceId"),
          (stmt: ECSqlStatement) => {
            let rowCount = 0;
            while (stmt.step() === DbResult.BE_SQLITE_ROW) {
              rowCount++;
  ...
  it("getRow() with primitives values", async () => {
    using(
      ...
      async (ecdb: ECDb) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([id]), {
        ...
        assert.equal(
          await query(ecdb, "SELECT count(*) cnt FROM meta.ECSchemaDef", QueryBinder.from([]), selectSingleRow, (row: any) => {
            assert.isDefined(row.cnt);
            assert.equal(typeof (row.cnt), "number");
            assert.equal(row.cnt, 6);
          }),
        ...
        assert.equal(
          await query(ecdb, "SELECT NULL FROM meta.ECSchemaDef LIMIT 1", QueryBinder.from([]), undefined, (row: any) => {
            assert.equal(Object.entries(row).length, 0);
          }),
          1,
        );
  ...
  it("getRow() with abbreviated blobs", async () => {
    using(
      ...
      async (ecdb: ECDb) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([fullId]), {
            abbreviateBlobs: true,
            ...selectSingleRow,
          }, (row: any) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([singleId]), {
            abbreviateBlobs: true,
            ...selectSingleRow,
          }, (row: any) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([emptyId]), {
            abbreviateBlobs: true,
            ...selectSingleRow,
          }, (row: any) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT ECInstanceId, ECClassId, Bl FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([nullId]), {
            abbreviateBlobs: true,
            ...selectSingleRow,
          }, (row: any) => {
  ...
  it("getRow() with structs", async () => {
    using(
      ...
      async (ecdb) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT Struct FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([id]), undefined, (row: any) => {
            assert.deepEqual(row.struct.bl, expectedStruct.bl);
            assert.equal(row.struct.bo, expectedStruct.bo);
            assert.equal(row.struct.d, expectedStruct.d);
            assert.equal(row.struct.dt, expectedStruct.dt);
  ...
  it("check ec enums", async () => {
    using(
      ...
      async (ecdb: ECDb) => {
        ...
        assert.equal(
          await query(ecdb, "SELECT Type,Modifier FROM meta.ECClassDef WHERE Name='Foo'", QueryBinder.from([id]), undefined, (row: any) => {
            assert.deepEqual(row, { type: 0, modifier: 2 });
          }),
          1,
        );
  ...
  it("check ORed ECEnums", async () => {
    ...
    using(
      ...
      async (ecdb: ECDb) => {
        ...
        const ids: { unored: Id64String, ored: Id64String, unmatched: Id64String } = ecdb.withPreparedStatement(
        ...
        ecdb.withPreparedStatement("SELECT MyColor,MyDomain FROM test.Foo WHERE ECInstanceId=?", (stmt: ECSqlStatement) => {
          ...
          assert.deepEqual(colVal.getEnum(), [{ schema: "Test", name: "Color", key: "Red", value: 1 }, {
            schema: "Test",
            name: "Color",
            key: "Blue",
            value: 4,
        ...
        assert.equal(
          await query(ecdb, "SELECT MyColor,MyDomain FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([ids.unored]), undefined, (row: any) => {
            assert.equal(row.myColor, 4);
            assert.equal(row.myDomain, "com");
          }),
          1,
        ...
        assert.equal(
          await query(ecdb, "SELECT MyColor,MyDomain FROM test.Foo WHERE ECInstanceId=?", QueryBinder.from([ids.ored]), undefined, (row: any) => {
            assert.equal(row.myColor, 5);
            assert.equal(row.myDomain, "gov,com");
          }),
          1,
