...
import { Format, KindOfQuantity, type RuntimePresentationFormat, type RuntimeSchemaContext, SchemaUnitProvider, Unit } from "@itwin/ecschema-metadata";
 ...
 * through RuntimeSchemaContext and resolve the alias-qualified format/unit names
 ...
 */
describe("RuntimeSchemaContext KindOfQuantity presentation formats", () => {
  ...
  /**
   * Example: get a property's KoQ from RuntimeSchemaContext, then resolve the
   * alias-qualified format and unit names to full ecschema-metadata objects via
   * IModelDb.schemaContext.
   *
   * RuntimeSchemaContext uses alias-qualified names (e.g. "f:DefaultRealU", "u:M")
  ...
  it("should resolve KoQ format and unit names via ecschema-metadata", async () => {
    // --- Step 1: find a property with a KoQ via RuntimeSchemaContext ---
    ...
    const formats: readonly RuntimePresentationFormat[] = rKoq!.presentationFormats;
    ...
    // The Formats and Units schemas are excluded from RuntimeSchemaContext,
  ...
  /**
   * Example: get a KoQ from RuntimeSchemaContext, then query ECDbMeta via ECSQL
  ...
  it("should resolve KoQ format and unit names via ECSQL", async () => {
    // --- Step 1: find a KoQ via RuntimeSchemaContext ---
