...
export class Relationship extends Entity {
   ...
   * @deprecated in 5.10.0. Will not be removed until after 2027-06-01. Obtain a `SchemaView` via `iModel.getSchemaView()` and call `view.findClass(relationship.classFullName)` to retrieve a `SchemaView.Class`. The returned type differs from `RelationshipClass` and offers a synchronous, snapshot-style API.
   */
