...
export function appendLeadersToBuilder(builder: ElementGeometry.Builder, leaders: TextAnnotationLeader[], layout: TextBlockLayout, transform: Transform, params: GeometryParams, textStyleResolver: TextStyleResolver, scaleFactor: number): boolean {
  let result = true;
  const scaledBlockHeight = textStyleResolver.blockSettings.textHeight * scaleFactor;
  ...
  for (const leader of leaders) {
    ...
    if (leaderStyle.leader.wantElbow) {
      const elbowLength = leaderStyle.leader.elbowLength * scaledBlockHeight;
    ...
    const terminatorHeight = leaderStyle.leader.terminatorHeightFactor * scaledBlockHeight;
    const terminatorWidth = leaderStyle.leader.terminatorWidthFactor * scaledBlockHeight;
