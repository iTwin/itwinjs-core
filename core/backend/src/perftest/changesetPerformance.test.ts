import { IModelDb, NativePlatformRegistry, OpenParams, IModelJsFs } from "../imodeljs-backend";
...
async function getImodelAfterApplyingCS(csvPath: string) {
  ...
  fs.appendFileSync(csvPath, "ImodelPerformance, GetImodelFromHubAFterCSApplied," + elapsedTime2 + ",Time to get an imodel with third version from local cache\n");

}

describe("ImodelPerformance", async () => {
