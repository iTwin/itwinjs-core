...
export namespace WorkspaceEditor {
  /**
   * Create a new {@link WorkspaceEditor} for creating new versions of {@link WorkspaceDb}s.
   * @note the caller becomes the owner of the Workspace.Editor and is responsible for calling {@link WorkspaceEditor.close} on it when they are finished using it.
  ...
  /**
   * Create a new, empty, [[EditableWorkspaceDb]] file on the local filesystem for importing {@link Workspace} resources.
   */
...
/** Arguments supplied to {@link WorkspaceEditor.createNewCloudContainer} to create a new [[EditableWorkspaceContainer]].
 * @beta
 */
export interface CreateNewWorkspaceContainerArgs {
  ...
  /** The name of the default {@link WorkspaceDb} created inside the new container.
...
/**
 * A [[WorkspaceContainer]] opened for editing by a {@link WorkspaceEditor}.
 * You can create new {@link WorkspaceDb}s or new versions of existing {@link WorkspaceDb}s inside it.
...
export interface EditableWorkspaceContainer extends WorkspaceContainer {
  /**
   * Create a copy of an existing {@link WorkspaceDb} in this container with a new [[WorkspaceDbVersion]].
  ...
  /**
   * Create a new, empty {@link WorkspaceDb}.
  ...
  /**
   * Get an editable {@link WorkspaceDb} to add, delete, or update resources *within a newly created version* of a WorkspaceDb.
   ...
   * publishes and uploads the changes made to any {@link WorkspaceDb}s while the lock was held, after which those dbs become immutable.
   */
...
/**
 * Arguments supplied to {@link WorkspaceEditor.createNewWorkspaceDbVersion}.
 * @beta
 */
export interface CreateNewWorkspaceDbVersionArgs {
  /**
   * The properties that determine the source {@link WorkspaceDb} to serve as the basis for the new version.
...
/**
 * An editable {@link WorkspaceDb}. This is used only by tools to allow administrators to create and modify WorkspaceDbs.
 ...
 * Normally, only admins will have write access to a {@link Workspace}.
...
export interface EditableWorkspaceDb extends WorkspaceDb {
  ...
  /**
   * The cloud properties of the {@link WorkspaceDb}, if this is a cloud-based WorkspaceDb.
   */
  ...
  /**
   * Add or update a resource in this WorkspaceDb that can be loaded as a {@link SettingsDictionary}.
...
/** An object that permits administrators to modify the contents of a {@link Workspace}.
 * Use {@link WorkspaceEditor.construct} to obtain a WorkspaceEditor, and [[close]] when finished using it.
...
export interface WorkspaceEditor {
   ...
   * @note This workspace is independent from [[IModelHost.appWorkspace]] and all [[IModelDb.workspace]]s. It has its own {@link Settings} and {@link WorkspaceDb}s.
   */
  ...
  /**
   * Creates a new cloud container for holding WorkspaceDbs, from the {@link BlobContainer} service.
