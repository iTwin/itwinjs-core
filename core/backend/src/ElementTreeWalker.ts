...
function sortChildrenBeforeParents(
  ...
  for (const eid of ids) {
    ...
    if (parentId !== undefined && ids.includes(parentId)) children.push(eid);
    else parents.push(eid);
  }
...
export class ElementTreeWalkerScope {
  ...
  constructor(
    ...
    if (typeof arg1 === "string") {
      ...
      this.path.push(
        (this.enclosingModelInfo = {
          model: arg2,
          isDefinitionModel: isDefinitionModel(arg2),
        })
      );
  ...
  private fmtItem(v: Id64String | ElementTreeWalkerModelInfo): string {
    if (typeof v === "string") return `element ${v}`;
...
export abstract class ElementTreeBottomUp {
  ...
  private _processSubModel(
    ...
    model.iModel.withPreparedStatement(
      `select ECInstanceId from bis:Element where Model.id=? and Parent.Id is null`,
      (stmt) => {
  ...
  public deleteSpecialElements(iModel: IModelDb) {
    ...
    for (const m of this.definitionModels) {
      if (!isModelEmpty(iModel, m)) {
        logModel(
          "Model not empty - cannot delete - may contain Definitions that are still in use",
          iModel,
          m,
          undefined,
          true
    ...
    for (const e of this.subjects) {
      if (iModel.elements.queryChildren(e).length !== 0) {
        logElement(
          "Subject still has children - cannot delete - may have child DefinitionModels",
          iModel,
          e,
          undefined,
          true
 ...
 * @beta
 */
export function deleteElementSubTrees(
  iModel: IModelDb,
  topElement: Id64String,
  filter: ElementSubTreeDeleteFilter
): void {
