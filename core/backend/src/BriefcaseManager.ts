 ...
 */
  ...
  */
   ...
   * @note The special briefcaseId [[BriefcaseIdValue.Standalone]] is used for SyncMode.PullOnly. It creates local
   * file that can accept changesets but may not be used for editing. file as a "briefcase."
  ...
  public static async processChangeSets(requestContext: AuthorizedClientRequestContext, db: IModelDb, targetChangeSetId: string, targetChangeSetIndex?: number): Promise<void> {
    ...
    if (!db.isOpen || db.nativeDb.isReadonly()) // don't use db.isReadonly - we reopen the file writable just for this operation but the flag still says "readonly"
