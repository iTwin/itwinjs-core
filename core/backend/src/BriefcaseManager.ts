...
export class BriefcaseManager {
  ...
  private static async applyPotentialMigrationChangeset(briefcaseDb: BriefcaseDb, changeset: ChangesetFileProps, useSemanticRebase: boolean): Promise<void> {
    // Detect migration changesets and run the per-migration reinstatement cycle.
    //
    ...
    const isConfirmed = briefcaseDb.channels.getAppliedMigrations(channelKey).some((r) => r.id === migrationId);
    if (!isConfirmed)
      return;

    const { migration, channelHasRegistrations } = briefcaseDb.channels[_findRegisteredMigration](channelKey, migrationId);
    if (!channelHasRegistrations) {
      ...
      const record = briefcaseDb.channels.getAppliedMigrations(channelKey).find((r) => r.id === migrationId)!; // eslint-disable-line @typescript-eslint/no-non-null-assertion
      ...
      briefcaseDb[_nativeDb].saveChanges(`migrateLocalChanges:${migration.id}`);
      ...
      const reReversedTxns = briefcaseDb[_nativeDb].pullMergeReverseLocalChanges(useSemanticRebase);
      if (useSemanticRebase) {
        briefcaseDb[_nativeDb].clearECDbCache();
      }
   ...
   *
   * A structured description is a necessary (but not sufficient) condition for a changeset
   * to be considered a migration. It's also possible that a user typed in a structured
   * description on a regular changeset just to mess with us.
   * @internal
   */
  ...
  public static async pullAndApplyChangesets(db: IModelDb, arg: PullChangesArgs): Promise<void> {
    ...
    for (const changeset of changesets) {
      ...
      // Migration changesets require special handling in order to apply the migration to local
      // (and therefore not-yet-migrated) user changes.
      if (briefcaseDb && !reverse) {
        await BriefcaseManager.applyPotentialMigrationChangeset(briefcaseDb, changeset, useSemanticRebase);
      }
    }
    if (isPullMerge) {
