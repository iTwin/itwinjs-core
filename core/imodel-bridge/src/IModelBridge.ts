...
export abstract class IModelBridge {
   ...
   * must call synchronizer.recordDocument on the source data. Called in the [Repository channel]($docs/learning/backend/channels).
   */
   ...
   * The bridge creates a connection to this source data and performs any steps necessary before reading. Called in the [Repository channel]($docs/learning/backend/channels).
   */
   ...
   * imodels created by this bridge as well as any that are unique to this source data. Called in the [Repository channel]($docs/learning/backend/channels).
   */
  public abstract async importDefinitions(): Promise<any>;

  /** Import schema(s) that every imodel synchronized by this bridge will use. Called in the [Repository channel]($docs/learning/backend/channels). */
  public abstract async importDynamicSchema(requestContext?: AuthorizedClientRequestContext | ClientRequestContext): Promise<any>;

  /** Import schema(s) that are specific to this particular source, in addition to the previously imported domain schema(s). Called in the [Repository channel]($docs/learning/backend/channels). */
  public abstract async importDomainSchema(requestContext?: AuthorizedClientRequestContext | ClientRequestContext): Promise<any>;

  /** Convert the source data to BIS and insert into the imodel.  Use the Synchronizer to determine whether an item is new, changed, or unchanged. Called in the [bridge's private channel]($docs/learning/backend/channels). */
   ...
   * See [Channels]($docs/learning/backend/channels) for an explanation of the concept of channels.
   */
