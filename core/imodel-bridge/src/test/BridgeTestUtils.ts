...
import {
  BackendLoggerCategory, ECSqlStatement, ExternalSourceAspect, IModelDb, IModelHost, IModelHostConfiguration, IModelHubAccess, IModelJsFs, NativeLoggerCategory,
...
export class BridgeTestUtils {
  ...
  public static async getTestModelInfo(requestContext: AuthorizedClientRequestContext, testProjectId: string, iModelName: string): Promise<TestIModelInfo> {
    ...
    iModelInfo.changeSets = await IModelHubAccess.iModelClient.changeSets.get(requestContext, iModelInfo.id);
    return iModelInfo;
  }
