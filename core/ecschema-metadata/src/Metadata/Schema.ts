...
export class Schema implements CustomAttributeContainerProps {
  ...
  constructor(
    ...
    if (alias !== undefined && ECName.validate(alias)) {
      ...
      throw new ECObjectsError(
        ...
        `The Schema ${this.name} does not have the required 'alias' attribute.`
      );
    }
  }
  ...
  public getSchemaItemKey(fullName: string): SchemaItemKey {
    ...
    if (this.name !== schemaName) {
      ...
      if (undefined === newSchemaRef)
        throw new ECObjectsError(
          ECObjectsStatus.InvalidECJson,
          `Unable to find the referenced SchemaItem ${itemName}.`
        );
  ...
  protected addItem<T extends SchemaItem>(item: T): void {
    if (undefined !== this.getItemSync(item.name))
      throw new ECObjectsError(
        ECObjectsStatus.DuplicateItem,
        `The SchemaItem ${item.name} cannot be added to the schema ${this.name} because it already exists`
      );
  ...
  protected deleteClassSync(name: string): void {
    ...
    if (ECClass.isECClass(schemaItem)) this._items.delete(name.toUpperCase());
  }
   ...
   * @param name
   */
  protected async createMixinClass(name: string): Promise<Mixin> {
    return this.createClass<Mixin>(Mixin, name);
  }
   ...
   * @param name
   */
  protected async createEnumeration(
    name: string,
  ...
  protected createEnumerationSync(
    name: string,
    primitiveType?: PrimitiveType.Integer | PrimitiveType.String
  ): Enumeration {
  ...
  public setVersion(
    ...
    const newVersion = new ECVersion(
      ...
      writeVersion ?? this._schemaKey.writeVersion,
      minorVersion ?? this._schemaKey.minorVersion
    );
  ...
  public *getClasses(): IterableIterator<ECClass> {
    for (const [, value] of this._items) {
      if (ECClass.isECClass(value)) yield value;
    }
  }
  ...
  public toJSON(): SchemaProps {
    ...
    if (undefined !== this.references && this.references.length > 0)
      // references is optional
      schemaJson.references = this.references.map(({ name, schemaKey }) => ({
        name,
  ...
  public fromJSONSync(schemaProps: SchemaProps) {
    ...
    if (
      ...
      throw new ECObjectsError(
        ...
        `The Schema ${this.name} has an unsupported namespace '${schemaProps.$schema}'.`
      );
    ...
    if (ECName.validate(schemaProps.alias)) {
      ...
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
        `The Schema ${schemaProps.name} does not have the required 'alias' attribute.`
      );
    }
...
export abstract class MutableSchema extends Schema {
  ...
  public abstract override createCustomAttributeClassSync(
    name: string,
    modifier?: ECClassModifier
  ): CustomAttributeClass;
  public abstract override createRelationshipClass(
    name: string,
    modifier?: ECClassModifier
  ): Promise<RelationshipClass>;
  ...
  public abstract override createEnumerationSync(
    name: string,
    primitiveType?: PrimitiveType.Integer | PrimitiveType.String
  ): Enumeration;
