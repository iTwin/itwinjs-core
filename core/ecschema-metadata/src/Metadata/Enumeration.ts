...
export class Enumeration extends SchemaItem {
  ...
  private findDuplicateEnumerators(name: string, value: string | number) {
    this._enumerators.forEach((element: AnyEnumerator) => {
      // Name and value must be unique within the ECEnumerations
      if (element.name.toLowerCase() === name.toLowerCase())
        throw new ECObjectsError(
          ECObjectsStatus.InvalidECJson,
          `The Enumeration ${this.name} has a duplicate Enumerator with name '${name}'.`
        );
      if (element.value === value)
        throw new ECObjectsError(
          ECObjectsStatus.InvalidECJson,
          `The Enumeration ${this.name} has a duplicate Enumerator with value '${value}'.`
        );
    });
  }
  ...
  public createEnumerator(
    ...
    if (!this.isInt && typeof value === "number")
      // also throws if backing type is string and value is number
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
