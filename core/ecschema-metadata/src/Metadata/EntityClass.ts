...
export class EntityClass extends ECClass {
  ...
  public get mixins(): LazyLoadedMixin[] {
    if (!this._mixins) return [];
    return this._mixins;
  }

  public *getMixinsSync(): Iterable<Mixin> {
    if (!this._mixins) return (function* (): Iterable<Mixin> {})(); // empty iterable
  ...
  protected override async buildPropertyCache(
    result: Property[],
    existingValues?: Map<string, number>,
    resetBaseCaches: boolean = false
  ): Promise<void> {
    if (!existingValues) {
    ...
    if (this.baseClass) {
      ECClass.mergeProperties(
        result,
        existingValues,
        await (await this.baseClass).getProperties(resetBaseCaches),
        false
      );
    }
  ...
  protected override buildPropertyCacheSync(
    result: Property[],
    existingValues?: Map<string, number>,
    resetBaseCaches: boolean = false
  ): void {
...
/** @internal */
export async function createNavigationProperty(
  ecClass: ECClass,
  name: string,
  relationship: string | RelationshipClass,
  direction: string | StrengthDirection
  ...
  if (await ecClass.getProperty(name))
    throw new ECObjectsError(
      ...
      `An ECProperty with the name ${name} already exists in the class ${ecClass.name}.`
    );
  ...
  if (!resolvedRelationship)
    throw new ECObjectsError(
      ...
      `The provided RelationshipClass, ${relationship}, is not a valid RelationshipClassInterface.`
    );
  ...
  if (typeof direction === "string") {
    ...
    if (undefined === tmpDirection)
      throw new ECObjectsError(
        ...
        `The provided StrengthDirection, ${direction}, is not a valid StrengthDirection.`
      );
...
/** @internal */
export function createNavigationPropertySync(
  ecClass: ECClass,
  name: string,
  relationship: string | RelationshipClass,
  direction: string | StrengthDirection
