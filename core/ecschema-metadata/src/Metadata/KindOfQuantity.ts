...
export class KindOfQuantity extends SchemaItem {
  ...
  protected _presentationFormats: Array<Format | OverrideFormat> = new Array<
    Format | OverrideFormat
  >();
  protected _persistenceUnit?: LazyLoadedUnit | LazyLoadedInvertedUnit;

  /** The first presentation format in the list of Formats. */
  public get defaultPresentationFormat(): Format | OverrideFormat | undefined {
    return this.presentationFormats[0];
  }

  /** A list of presentation formats. */
  public get presentationFormats(): Array<Format | OverrideFormat> {
    return this._presentationFormats;
  }

  public get persistenceUnit():
    | LazyLoadedUnit
  ...
  protected addPresentationFormat(
    ...
    isDefault
      ? this._presentationFormats.splice(0, 0, format)
      : this._presentationFormats.push(format);
  }
  ...
  private parseFormatString(formatString: string): OverrideFormatProps {
    ...
    if (undefined === match[1])
      throw new ECObjectsError(
        ...
        `The format string, ${formatString}, on KindOfQuantity '${this.fullName}' is missing a format.`
      );
      ...
      if (tokens.length >= precisionIndx + 1) {
        if (tokens[precisionIndx].length > 0) {
          ...
          if (Number.isNaN(precision))
            throw new ECObjectsError(
              ECObjectsStatus.InvalidECJson,
              `The format string '${formatString}' on KindOfQuantity '${this.fullName}' has a precision override '${tokens[precisionIndx]}' that is not number.`
            );
  ...
  protected createFormatOverride(
    ...
    if (
      unitLabelOverrides &&
      parent.units &&
      parent.units.length !== unitLabelOverrides.length
    )
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
        `Cannot add presentation format to KindOfQuantity '${this.name}' because the number of unit overrides is inconsistent with the number in the Format '${parent.name}'.`
      );

    if (
      parent.units &&
      0 === parent.units.length &&
      unitLabelOverrides &&
      0 < unitLabelOverrides.length
    )
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
        `Cannot add a presentation format to KindOfQuantity '${this.name}' without any units and no unit overrides.`
      );
  ...
  private async processPresentationUnits(
    presentationUnitsJson: string | string[]
  ): Promise<void> {
    ...
    for (const formatString of presUnitsArr) {
      const presFormatOverride: OverrideFormatProps =
        this.parseFormatString(formatString);

      const format = await this.schema.lookupItem<Format>(
        presFormatOverride.name
      );
      if (
        undefined === format ||
        format.schemaItemType !== SchemaItemType.Format
      )
        throw new ECObjectsError(
          ECObjectsStatus.InvalidECJson,
          `Unable to locate Format '${presFormatOverride.name}' for the presentation unit on KindOfQuantity ${this.fullName}.`
        );

      if (
        undefined === presFormatOverride.precision &&
        undefined === presFormatOverride.unitAndLabels
      ) {
      ...
      if (undefined !== presFormatOverride.unitAndLabels) {
        ...
        for (const unitOverride of presFormatOverride.unitAndLabels) {
          const unit = await this.schema.lookupItem<Unit | InvertedUnit>(
            unitOverride[0]
          );
          if (undefined === unit)
            throw new ECObjectsError(
              ECObjectsStatus.InvalidECJson,
              `Unable to locate SchemaItem ${unitOverride[0]}.`
            );
      ...
      const overrideFormat: OverrideFormat = this.createFormatOverride(
        format,
        presFormatOverride.precision,
        unitAndLabels
      );
  ...
  private processPresentationUnitsSync(
    ...
    for (const formatString of presUnitsArr) {
      ...
      const format = this.schema.lookupItemSync<Format>(
        presFormatOverride.name
      );
      if (
        undefined === format ||
      ...
      if (undefined !== presFormatOverride.unitAndLabels) {
        ...
        unitAndLabels = [];
        for (const unitOverride of presFormatOverride.unitAndLabels) {
          const unit = this.schema.lookupItemSync<Unit | InvertedUnit>(
            unitOverride[0]
          );
          if (undefined === unit)
            throw new ECObjectsError(
  ...
  public override toJSON(
    ...
    if (
      undefined !== this.presentationFormats &&
      0 < this.presentationFormats.length
    )
      schemaJson.presentationUnits = this.presentationFormats.map(
        (format: Format | OverrideFormat) => format.fullName
      );
    return schemaJson;
  }
  ...
  public override async toXml(schemaXml: Document): Promise<Element> {
    ...
    if (undefined !== this.presentationFormats) {
      const presUnitStrings = this.presentationFormats.map(
        (format: Format | OverrideFormat) => {
          if (!OverrideFormat.isOverrideFormat(format))
            return XmlSerializationUtils.createXmlTypedName(
              this.schema,
              format.schema,
              format.name
            );
  ...
  public override fromJSONSync(kindOfQuantityProps: KindOfQuantityProps): void {
    ...
    const persistenceUnit = this.schema.lookupItemSync<Unit>(
      kindOfQuantityProps.persistenceUnit
    );
    if (undefined === persistenceUnit)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
        `The Unit ${kindOfQuantityProps.persistenceUnit} does not exist.`
      );

    if (
      persistenceUnit.schemaItemType !== SchemaItemType.Unit &&
      persistenceUnit.schemaItemType !== SchemaItemType.InvertedUnit
    )
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
        `The item ${kindOfQuantityProps.persistenceUnit} is not a Unit or InvertedUnit.`
      );

    this._persistenceUnit = new DelayedPromiseWithProps(
      persistenceUnit.key,
      async () => persistenceUnit
    );
  ...
  public override async fromJSON(
    kindOfQuantityProps: KindOfQuantityProps
  ): Promise<void> {
    await super.fromJSON(kindOfQuantityProps);
    this._relativeError = kindOfQuantityProps.relativeError;

    const persistenceUnit = await this.schema.lookupItem<Unit>(
      kindOfQuantityProps.persistenceUnit
    );
    if (undefined === persistenceUnit)
      throw new ECObjectsError(
    ...
    if (undefined !== kindOfQuantityProps.presentationUnits)
      await this.processPresentationUnits(
        kindOfQuantityProps.presentationUnits
      );
  }
}
/**
...
export abstract class MutableKindOfQuantity extends KindOfQuantity {
  public abstract override addPresentationFormat(
    format: Format | OverrideFormat,
    isDefault: boolean
  ): void;
