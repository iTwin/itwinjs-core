...
export class KindOfQuantity extends SchemaItem {
  ...
  public get persistenceUnit(): LazyLoadedUnit | LazyLoadedInvertedUnit | undefined {
  ...
  private parseFormatString(formatString: string): OverrideFormatProps {
    ...
    while (i < match.length) {
      ...
      if (undefined === match[i + 1]) throw new ECObjectsError(ECObjectsStatus.InvalidECJson, ``);

      if (undefined === returnValue.unitAndLabels) returnValue.unitAndLabels = [];
  ...
  private async processPresentationUnits(presentationUnitsJson: string | string[]): Promise<void> {
    ...
    for (const formatString of presUnitsArr) {
      ...
      if (undefined !== presFormatOverride.unitAndLabels) {
        if (4 < presFormatOverride.unitAndLabels.length) throw new ECObjectsError(ECObjectsStatus.InvalidECJson, ``);
