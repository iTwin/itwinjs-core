  ...
  implements CustomAttributeContainerProps
{
  ...
  public set baseClass(baseClass: LazyLoadedECClass | undefined) {
    this._baseClass = baseClass;
  }
  public get properties(): IterableIterator<Property> | undefined {
  ...
  constructor(schema: Schema, name: string, modifier?: ECClassModifier) {
    ...
    else this._modifier = ECClassModifier.None;
  }
  ...
  protected addProperty<T extends Property>(prop: T): T {
    if (!this._properties) this._properties = new Map<string, Property>();
  ...
  protected async deleteProperty(name: string): Promise<void> {
    if (this._properties) {
      ...
      if (property) this._properties.delete(name.toUpperCase());
    }
  }
  ...
  public getInheritedPropertySync(name: string): Property | undefined {
    ...
    if (baseClassObj) return baseClassObj.getPropertySync(name, true);
   ...
   * @param primitiveType The primitive type of property to create. If not provided the default is PrimitiveType.Integer
   */
  protected async createPrimitiveArrayProperty(
    name: string,
    primitiveType: PrimitiveType
  ): Promise<PrimitiveArrayProperty>;
  protected async createPrimitiveArrayProperty(
    name: string,
    primitiveType: Enumeration
  ): Promise<EnumerationArrayProperty>;
  protected async createPrimitiveArrayProperty(
    name: string,
    primitiveType?: string | PrimitiveType | Enumeration
  ): Promise<Property> {
   ...
   * @param type
   */
  protected async createStructArrayProperty(
    name: string,
    structType: string | StructClass
  ): Promise<StructArrayProperty> {
    if (await this.getProperty(name))
      throw new ECObjectsError(
        ECObjectsStatus.DuplicateProperty,
        `An ECProperty with the name ${name} already exists in the class ${this.name}.`
  ...
  protected async loadStructType(
    ...
    } else correctType = structType;

    if (!correctType)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidType,
        `The provided Struct type, ${structType}, is not a valid StructClass.`
      );
  ...
  protected async loadPrimitiveType(
    primitiveType: string | PrimitiveType | Enumeration | undefined,
    schema: Schema
  ): Promise<PrimitiveType | Enumeration> {
  ...
  protected loadPrimitiveTypeSync(
    primitiveType: string | PrimitiveType | Enumeration | undefined,
    schema: Schema
  ): PrimitiveType | Enumeration {
    if (primitiveType === undefined) return PrimitiveType.Integer;
    ...
    if (undefined !== classProps.modifier) {
      ...
      if (undefined === modifier)
        throw new ECObjectsError(
          ...
          `The string '${classProps.modifier}' is not a valid ECClassModifier.`
        );
  ...
  public async *getAllBaseClasses(): AsyncIterableIterator<ECClass> {
    ...
    const addBaseClasses = async (ecClass: AnyClass) => {
      ...
      if (ecClass.baseClass) baseClasses.push(await ecClass.baseClass);
    };

    while (baseClasses.length > 0) {
      const baseClass = baseClasses.pop() as AnyClass;
      await addBaseClasses(baseClass);
      if (baseClass !== this) yield baseClass as ECClass;
    }
  }
  ...
  public *getAllBaseClassesSync(): Iterable<AnyClass> {
    ...
    const addBaseClasses = (ecClass: AnyClass) => {
      ...
      if (baseClass) baseClasses.push(baseClass);
    };

    while (baseClasses.length > 0) {
      const baseClass = baseClasses.pop() as AnyClass;
      addBaseClasses(baseClass);
      if (baseClass !== this) yield baseClass;
    }
  }

  protected static mergeProperties(
    target: Property[],
    existingValues: Map<string, number>,
    propertiesToMerge: Property[],
  ...
  protected async buildPropertyCache(
    result: Property[],
    existingValues?: Map<string, number>,
  ...
  public getCustomAttributesSync(): CustomAttributeSet {
    ...
    this.traverseBaseClassesSync((ecClass: ECClass) => {
      if (undefined === ecClass.customAttributes) return false;

      for (const [className, customAttribute] of ecClass.customAttributes) {
        if (customAttributes!.has(className)) continue;
  ...
  public static isECClass(object: any): object is ECClass {
    if (!SchemaItem.isSchemaItem(object)) return false;

    return (
      object.schemaItemType === SchemaItemType.EntityClass ||
      object.schemaItemType === SchemaItemType.Mixin ||
      object.schemaItemType === SchemaItemType.RelationshipClass ||
      object.schemaItemType === SchemaItemType.StructClass ||
      object.schemaItemType === SchemaItemType.CustomAttributeClass
    );
  }
...
export abstract class MutableClass extends ECClass {
  ...
  public abstract override createPrimitiveArrayProperty(
    name: string,
    primitiveType: PrimitiveType
  ): Promise<PrimitiveArrayProperty>;
  public abstract override createPrimitiveArrayProperty(
  ...
  public abstract override createPrimitiveArrayPropertySync(
    name: string,
    primitiveType: PrimitiveType
  ): PrimitiveArrayProperty;
  public abstract override createPrimitiveArrayPropertySync(
  ...
  public abstract override createStructProperty(
    name: string,
    structType: string | StructClass
  ): Promise<StructProperty>;
  ...
  public abstract override createStructArrayPropertySync(
    name: string,
    structType: string | StructClass
  ): StructArrayProperty;
