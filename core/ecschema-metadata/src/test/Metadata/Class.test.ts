...
describe("ECClass", () => {
  ...
  describe("ECClassHierarchy tests with manual schema and class creation", async () => {
    it("class having base class in a schema which is not referenced but know to schema context locators", async () => {
      const context = new SchemaContext();
      ...
      for await (const baseClass of classB.getAllBaseClasses()) {
        baseClasses.push(baseClass.name);
      }
      expect(baseClasses).toEqual(["A"]);
    });
    it("class having base class in a schema which is not referenced but know to schema context locators sync", async () => {
      const context = new SchemaContext();
      const refSchema = new Schema(context, "RefSchema","ref", 1, 0, 1);
      ...
      const baseClasses: string[] = [];
      for (const baseClass of classB.getAllBaseClassesSync()) {
        baseClasses.push(baseClass.name);
      }
      expect(baseClasses).toEqual(["A"]);
    });
    it("class having base class in a schema which is not referenced and also not know to schema context locators", async () => {
      const context = new SchemaContext();
      const refSchema = new Schema(context, "RefSchema","ref", 1, 0, 1);
      const classA = await (refSchema as MutableSchema).createEntityClass("A");
      expect(classA).toBeDefined();
      // not adding refSchema to context
      ...
      expect(baseClasses).toEqual([]); // as the schema, in which the base class is defined, is not in the reference tree and also not in the schema context locators, so cannot load the base class
    });
  });
});
