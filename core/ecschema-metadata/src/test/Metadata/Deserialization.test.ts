...
describe("Full Schema Deserialization", () => {
  ...
  describe("with schema reference", () => {
    ...
    it("should succeed when referenced schema is already in the schema context", async () => {
      ...
      if (!schema.references) return;

      expect(schema.references.length).equal(1);
      if (!schema.references[0]) assert.fail();
    ...
    it("should throw for cyclic references", async () => {
      ...
      json = {
        ...baseJson,
        alias: "test",
        references: [{ name: "RefSchemaC", version: "1.0.0" }],
      };
      await expect(Schema.fromJson(json, context2)).to.be.rejectedWith(
        ECObjectsError,
        `Schema 'RefSchemaC' has reference cycles: RefSchemaF --> RefSchemaC, RefSchemaE --> RefSchemaF, RefSchemaC --> RefSchemaE\r\n`
      );
    });
  ...
  describe("with schema custom attributes", () => {
    ...
    it("with class containing attribute with no namespace, custom attribute not defined in schema, throws", () => {
      ...
      expect(() => reader.readSchemaSync(schema, document)).to.throw(
        ECObjectsError,
        "Unable to locate SchemaItem TestSchema.TestAttribute."
      );
    });
  });
