...
describe("Schema Context", () => {
  ...
  it("schema added, getCachedSchema called with different schema version and incompatible match type, returns undefined", async () => {
    ...
    const loadedSchema = await context.getCachedSchema(
      testKey,
  ...
  it("successfully finds schema from added locater", async () => {
    ...
    expect(
      await context.getSchema(schema.schemaKey, SchemaMatchType.Exact)
    ).to.equal(schema);
    ...
    expect(
      await context.getSchema(schema2.schemaKey, SchemaMatchType.Exact)
    ).to.equal(schema2);
