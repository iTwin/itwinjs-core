...
describe("Parsing/ToString Functions", () => {
  ...
  it("parseCustomAttributeContainerType", () => {
    ...
    expect(parseCustomAttributeContainerType("SouRcEReLatIoNShiPCoNstRaInT")).to.equal(
      CustomAttributeContainerType.SourceRelationshipConstraint
    );
    expect(parseCustomAttributeContainerType("TarGETreLATIoNShIPCOnSTrAInT")).to.equal(
      CustomAttributeContainerType.TargetRelationshipConstraint
    );
    expect(parseCustomAttributeContainerType("AnyRELaTioNShiPCoNSTrAInt")).to.equal(
    ...
    expect(parseCustomAttributeContainerType(";Schema|AnyClass,TargetRelationshipConstraint;StructProperty")).to.equal(
      combo
    );
  });
  ...
  it("containerTypeToString", () => {
    ...
    expect(containerTypeToString(CustomAttributeContainerType.SourceRelationshipConstraint)).to.equal(
      "SourceRelationshipConstraint"
    );
    expect(containerTypeToString(CustomAttributeContainerType.TargetRelationshipConstraint)).to.equal(
      "TargetRelationshipConstraint"
    );
    expect(containerTypeToString(CustomAttributeContainerType.AnyRelationshipConstraint)).to.equal(
...
describe("ECObjectsError ", () => {
  it("toDebugString", () => {
    ...
    expect(new ECObjectsError(ECObjectsStatus.DuplicateProperty, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.DuplicateProperty: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.DuplicateSchema, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.DuplicateSchema: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.ImmutableSchema, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.ImmutableSchema: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidContainerType, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidContainerType: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidECJson, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidECJson: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidECName, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidECName: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidECVersion, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidECVersion: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidEnumValue, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidEnumValue: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidModifier, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidModifier: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidMultiplicity, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidMultiplicity: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidPrimitiveType, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidPrimitiveType: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidSchemaItemType, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidSchemaItemType: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidStrength, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidStrength: msg"
    );
    ...
    expect(new ECObjectsError(ECObjectsStatus.InvalidRelationshipEnd, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidRelationshipEnd: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.InvalidType, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.InvalidType: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.MissingSchemaUrl, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.MissingSchemaUrl: msg"
    );
    expect(new ECObjectsError(ECObjectsStatus.UnableToLocateSchema, "msg").toDebugString()).to.equal(
      "ECObjectsStatus.UnableToLocateSchema: msg"
    );
