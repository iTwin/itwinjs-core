...
describe("Testing when unit conversion should throw", () => {
  ...
  describe("Sync", () => {
    it("should throw when schema name is not in context", () => {
      const converter = new UnitConverter(context);
      try {
        converter.calculateConversionSync("MockSchema:CM", "SIUnits:M");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Cannot find from's and/or to's schema");
      }
      try {
        converter.calculateConversionSync("SIUnits:M", "MockSchema:CM");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Cannot find from's and/or to's schema");
      }
    ...
    it("should throw when schema item is not in schema ", () => {
      const converter = new UnitConverter(context);
      try {
        converter.calculateConversionSync("SIUnits:MockUnit", "MetricUnits:CM");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Cannot find schema item");
      }
      try {
        converter.calculateConversionSync("MetricUnits:CM", "SIUnits:MockUnit");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Cannot find schema item");
      }
    ...
    it("should throw when source and target units are not the same phenomenon", () => {
      const converter = new UnitConverter(context);
      try {
        converter.calculateConversionSync("USUnits:SQ_FT", "SIUnits:M");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not belong to same phenomenon");
      }
      try {
        converter.calculateConversionSync("SIUnits:M", "USUnits:SQ_FT");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not belong to same phenomenon");
      }
    ...
    it("should throw when unit in definition is not found in schema (deci is not in ValidationUnits schema)", () => {
      const converter = new UnitConverter(context);
      try {
        converter.calculateConversionSync("ValidationUnits:DM", "ValidationUnits:KM");
        converter.calculateConversionSync("ValidationUnits:KM", "ValidationUnits:DM");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Cannot find schema item");
      }
    ...
    it("should throw when source and target units do not have the same base units", () => {
      const converter = new UnitConverter(context);
      try {
        converter.calculateConversionSync("ValidationUnits:FT", "ValidationUnits:KM");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not have matching base units");
      }
      try {
        converter.calculateConversionSync("ValidationUnits:KM", "ValidationUnits:FT");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not have matching base units");
      }
      ...
      try {
        converter.calculateConversionSync("ValidationUnits:YRD", "ValidationUnits:M");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not have matching base units");
      }
      try {
        converter.calculateConversionSync("ValidationUnits:M", "ValidationUnits:YRD");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not have matching base units");
      }
    ...
    it("should throw when source and target units do not have the same base units", () => {
      const converter = new UnitConverter(context);
      try {
        converter.calculateConversionSync("ValidationUnits:MM_PER_SEC", "ValidationUnits:FT_PER_DAY");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not have matching base units");
      }
      try {
        converter.calculateConversionSync("ValidationUnits:FT_PER_DAY", "ValidationUnits:MM_PER_SEC");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not have matching base units");
      }
      ...
      try {
        converter.calculateConversionSync("ValidationUnits:MM_PER_HR", "ValidationUnits:FT_PER_SEC");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not have matching base units");
      }
      try {
        converter.calculateConversionSync("ValidationUnits:FT_PER_SEC", "ValidationUnits:MM_PER_HR");
      } catch (err: any) {
        expect(err).to.be.an("error");
        expect(err.message).to.equal("Source and target units do not have matching base units");
      }
