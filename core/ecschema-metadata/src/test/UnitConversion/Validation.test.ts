...
describe("Testing when unit conversion should throw", () => {
  ...
  before(() => {
    ...
    const auSchemaFile = path.join(
      __dirname,
      "..",
      "assets",
      "ValidationUnits.ecschema.xml"
    );
  ...
  it("should throw when unit in definition is not found in schema (deci is not in ValidationUnits schema)", async () => {
    const converter = new UnitConverter(context);
    try {
      await converter.calculateConversion(
        "ValidationUnits:DM",
        "ValidationUnits:KM"
      );
      await converter.calculateConversion(
  ...
  it("should throw when source and target units do not have the same base units", async () => {
    ...
    try {
      await converter.calculateConversion(
        "ValidationUnits:MM_PER_SEC",
        "ValidationUnits:FT_PER_DAY"
      );
    } catch (err: any) {
      expect(err).to.be.an("error");
      expect(err.message).to.equal(
    ...
    try {
      await converter.calculateConversion(
        "ValidationUnits:MM_PER_HR",
        "ValidationUnits:FT_PER_SEC"
      );
    } catch (err: any) {
      expect(err).to.be.an("error");
      expect(err.message).to.equal(
