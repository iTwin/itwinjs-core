...
describe("Unit Provider tests", () => {
  ...
  describe("Initialized with SchemaContext", () => {
    ...
    it("should find units by unit names in MetricUnits schema", async () => {
      ...
      expect(unit2.name === "MetricUnits.M_PER_KM", `Unit name should be MetricUnits.M_PER_KM and not ${unit2.name}`).to
        .be.true;
    });
    ...
    it("should find units that belong to Units.LENGTH phenomenon", async () => {
      ...
      for (const unit of filteredUnits) {
        expect(unit.phenomenon === "Units.LENGTH", `Phenomenon name should be Units.LENGTH and not ${unit.phenomenon}`)
          .to.be.true;
      }
    });
    ...
    it("should find alternate display labels of Units.US_SURVEY_FT", () => {
      ...
      expect(altDisplayLabels, `Alternate display labels should be ${expectedLabels}`).to.include.members(
        expectedLabels
      );
    ...
    it("should find Units.DELTA_RANKINE with display label 'Δ°R'", async () => {
      ...
      expect(unit.name === "Units.DELTA_RANKINE", `Unit name should be Units.DELTA_RANKINE and not ${unit.name}`).to.be
        .true;
    });
