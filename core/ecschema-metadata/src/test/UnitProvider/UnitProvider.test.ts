...
class TestSchemaLocater implements ISchemaLocater {
  ...
  public getSchemaSync<T extends Schema>(
    ...
    if (schemaKey.name !== "Units") return undefined;
    ...
    const schema = deserializeXmlSync(
      schemaXml,
      context || new SchemaContext()
    );
    if (schema !== undefined) return schema as T;
...
describe("Unit Provider tests", () => {
  ...
  describe("Initialized with SchemaContext", () => {
    ...
    it("should find units by unit names in Units schema", async () => {
      ...
      expect(
        unit1.name === "Units.KM",
        `Unit name should be Units.KM and not ${unit1.name}`
      ).to.be.true;

      const unit2 = await provider.findUnitByName("Units.KM_PER_HR");
      expect(
        unit2.name === "Units.KM_PER_HR",
        `Unit name should be Units.KM_PER_HR and not ${unit2.name}`
      ).to.be.true;
    });
    ...
    it("should find units by unit names in MetricUnits schema", async () => {
      ...
      expect(
        unit1.name === "MetricUnits.KM",
        `Unit name should be MetricUnits.KM and not ${unit1.name}`
      ).to.be.true;

      const unit2 = await provider.findUnitByName("MetricUnits.M_PER_KM");
      expect(
        unit2.name === "MetricUnits.M_PER_KM",
        `Unit name should be MetricUnits.M_PER_KM and not ${unit2.name}`
      ).to.be.true;
    });
    ...
    it("should find units that belong to Units.LENGTH phenomenon", async () => {
      const filteredUnits: UnitProps[] = await provider.getUnitsByFamily(
        "Units.LENGTH"
      );
      for (const unit of filteredUnits) {
        expect(
          unit.phenomenon === "Units.LENGTH",
          `Phenomenon name should be Units.LENGTH and not ${unit.phenomenon}`
        ).to.be.true;
      }
    });
    ...
    it("should find units that belong to Units.VELOCITY phenomenon", async () => {
      ...
      for (const unit of filteredUnits) {
        expect(
          unit.phenomenon === "Units.VELOCITY",
          `Phenomenon name should be Units.LENGTH and not ${unit.phenomenon}`
        ).to.be.true;
      }
    });
    ...
    it("should find units that belong to SIUnits.LENGTH phenomenon across multiple schemas", async () => {
      ...
      for (const unit of filteredUnits) {
        expect(
          unit.phenomenon === "SIUnits.LENGTH",
          `Phenomenon name should be SIUnits.LENGTH and not ${unit.phenomenon}`
        ).to.be.true;
      }
    ...
    it("should find units that belong to SIUnits.SLOPE phenomenon across multiple schemas", async () => {
      ...
      for (const unit of filteredUnits) {
        expect(
          unit.phenomenon === "SIUnits.SLOPE",
          `Phenomenon name should be SIUnits.SLOPE and not ${unit.phenomenon}`
        ).to.be.true;
      }
    ...
    it("should not find any alternate display labels of Unit", () => {
      const altDisplayLabels =
        provider.getAlternateDisplayLabels("Units.CELSIUS");
    ...
    it("should find Units.DELTA_RANKINE with display label 'Δ°R'", async () => {
      ...
      expect(
        unit.name === "Units.DELTA_RANKINE",
        `Unit name should be Units.DELTA_RANKINE and not ${unit.name}`
      ).to.be.true;
    });
    ...
    it("should find Units.MICROMOL_PER_CUB_DM with display label 'µmol/dm³'", async () => {
      ...
      expect(
        unit.name === "Units.MICROMOL_PER_CUB_DM",
        `Unit name should be Units.MICROMOL_PER_CUB_DM and not ${unit.name}`
      ).to.be.true;
    });
    ...
    it("should find Units.FT with display label 'ft'", async () => {
      ...
      expect(
        unit.name === "Units.FT",
        `Unit name should be Units.FT and not ${unit.name}`
      ).to.be.true;
    });
    ...
    it("should find USUnits.FT with display label 'ft' with schemaName 'USUnits'", async () => {
      ...
      expect(
        unit.name === "USUnits.FT",
        `Unit name should be USUnits.FT and not ${unit.name}`
      ).to.be.true;
    });
    ...
    it("should find Units.YRD by corresponding alternate display labels", async () => {
      ...
      expect(
        unit1.name === "Units.YRD",
        `Unit name should be Units.YRD and not ${unit1.name}`
      ).to.be.true;

      const unit2 = await provider.findUnit("yrd");
      expect(
        unit2.name === "Units.YRD",
        `Unit name should be Units.YRD and not ${unit2.name}`
      ).to.be.true;
    });
    ...
    it("should find Units.ARC_SECOND with alternate display label 'sec'", async () => {
      ...
      expect(
        unit.name === "Units.ARC_SECOND",
        `Unit name should be Units.ARC_SECOND and not ${unit.name}`
      ).to.be.true;
    });
    ...
    it("should find Units.S with alternate display label 'sec' and phenomenon Units.TIME", async () => {
      ...
      expect(
        unit.name === "Units.S",
        `Unit name should be Units.S and not ${unit.name}`
      ).to.be.true;
    });
    ...
    it("should find Units.ARC_MINUTE with display label ''' ", async () => {
      const unit = await provider.findUnit("'");
      expect(
        unit.name === "Units.ARC_MINUTE",
        `Unit name should be Units.ARC_MINUTE and not ${unit.name}`
      ).to.be.true;
    });
