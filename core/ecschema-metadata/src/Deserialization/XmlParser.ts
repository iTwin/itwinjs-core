...
export class XmlParser extends AbstractParser<Element> {
  ...
  public parseSchema(): SchemaProps {
    ...
    if ("ECSchema" !== schemaMetadata.nodeName)
      throw new ECObjectsError(
        ...
        "An ECSchema is missing the required metadata."
      );

    const schemaDefDuplicates = this.getElementChildrenByTagName(
      schemaMetadata,
      "ECSchema"
    );
    if (schemaDefDuplicates.length > 1)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        "An ECSchema has more than one ECSchema definition. Only one is allowed."
      );
    ...
    if (this._ecXmlVersion === undefined)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `The ECSchema ${this._schemaName} has an invalid 'xmlns' attribute`
      );
    ...
    const description = this.getOptionalAttribute(
      schemaMetadata,
      "description"
    );
    const displayLabel = this.getOptionalAttribute(
      schemaMetadata,
      "displayLabel"
    );
  ...
  public *getReferences(): Iterable<SchemaReferenceProps> {
    const schemaReferences = this.getElementChildrenByTagName(
      this._rawSchema.documentElement,
      "ECSchemaReference"
    );
  ...
  public *getItems(): Iterable<[string, string, Element]> {
    if (!this._mapIsPopulated) {
      ...
      for (const item of schemaItems) {
        ...
        const customAttributesResult = this.getElementChildrenByTagName(
          item,
          "ECCustomAttributes"
        );
        if (customAttributesResult.length > 0) {
          const customAttributes = customAttributesResult[0];
          const isMixinResult = this.getElementChildrenByTagName(
            customAttributes,
            "IsMixin"
          );
          if (isMixinResult.length > 0) rawItemType = "Mixin";
        }
        ...
        if (itemType === undefined)
          throw new ECObjectsError(
            ECObjectsStatus.InvalidSchemaXML,
            `A SchemaItem in ${this._schemaName} has an invalid type. '${rawItemType}' is not a valid SchemaItem type.`
          );

        const itemName = this.getRequiredAttribute(
          item,
          "typeName",
        ...
        if (!ECName.validate(itemName))
          throw new ECObjectsError(
            ...
            `A SchemaItem in ${this._schemaName} has an invalid 'typeName' attribute. '${itemName}' is not a valid ECName.`
          );
  ...
  public parseEntityClass(xmlElement: Element): EntityClassProps {
    ...
    const baseClasses = this.getElementChildrenByTagName(
      xmlElement,
      "BaseClass"
    );
  ...
  public parseMixin(xmlElement: Element): MixinProps {
    ...
    if (baseClasses.length > 1)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `The Mixin ${this._currentItemFullName} has more than one base class which is not allowed.`
      );
    ...
    if (customAttributesResult.length < 1)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `The Mixin ${this._currentItemFullName} is missing the required 'IsMixin' tag.`
      );
    ...
    const appliesToResult = this.getElementChildrenByTagName(
      mixinAttributes,
    ...
    if (appliesToResult.length < 1)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `The Mixin ${this._currentItemFullName} is missing the required 'AppliesToEntityClass' tag.`
      );
  ...
  public parseCustomAttributeClass(
    ...
    const classProps = this.getClassProps(xmlElement);
    const appliesTo = this.getRequiredAttribute(
      xmlElement,
      "appliesTo",
  ...
  public parseRelationshipClass(xmlElement: Element): RelationshipClassProps {
    ...
    if (sourceResult.length !== 1) {
      if (sourceResult.length === 0)
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `The RelationshipClass ${this._currentItemFullName} is missing the required Source constraint tag.`
        );
      else
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `The RelationshipClass ${this._currentItemFullName} has more than one Source constraint tag. Only one is allowed.`
        );
    }
    ...
    if (targetResult.length !== 1) {
      if (targetResult.length === 0)
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `The RelationshipClass ${this._currentItemFullName} is missing the required Target constraint tag.`
        );
      else
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `The RelationshipClass ${this._currentItemFullName} has more than one Target constraint tag. Only one is allowed.`
        );
    }
  ...
  public parseEnumeration(xmlElement: Element): EnumerationProps {
    ...
    const enumType = this.getRequiredAttribute(
      xmlElement,
      "backingTypeName",
    ...
    if (/int/i.test(enumType)) tempBackingType = PrimitiveType.Integer;
    else if (/string/i.test(enumType)) tempBackingType = PrimitiveType.String;
    else
      throw new ECObjectsError(
        ECObjectsStatus.InvalidECJson,
        `The Enumeration ${this._currentItemFullName} has an invalid 'backingTypeName' attribute. It should be either "int" or "string".`
      );

    let isStrictString: string | undefined = this.getOptionalAttribute(
      xmlElement,
      "isStrict"
    );
    if (isStrictString === undefined) isStrictString = "true";

    const isStrict = this.parseBoolean(
      isStrictString,
      `The Enumeration ${this._currentItemFullName} has an invalid 'isStrict' attribute. It should either be "true" or "false".`
    );

    const enumeratorElements = this.getElementChildrenByTagName(
      xmlElement,
      "ECEnumerator"
    );
    ...
    for (const element of enumeratorElements) {
      ...
      if (PrimitiveType.Integer === tempBackingType) {
        ...
        if (isNaN(numericValue))
          throw new ECObjectsError(
            ECObjectsStatus.InvalidSchemaXML,
            `The Enumeration ${this._currentItemFullName} of type "int" has an enumerator with a non-integer value.`
          );
        value = numericValue;
      }
  ...
  public parseKindOfQuantity(xmlElement: Element): KindOfQuantityProps {
    ...
    const relativeErrorString = this.getRequiredAttribute(
      xmlElement,
      "relativeError",
    ...
    if (isNaN(relativeError))
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `The KindOfQuantity ${this._currentItemFullName} has an invalid 'relativeError' attribute. It should be a numeric value.`
      );

    const presentationUnitsString = this.getOptionalAttribute(
      xmlElement,
      "presentationUnits"
    );
    let presentationUnits: string[] | undefined;
    if (presentationUnitsString)
      presentationUnits = this.getQualifiedPresentationUnits(
        presentationUnitsString.split(";")
      );

    let persistenceUnit = this.getRequiredAttribute(
      xmlElement,
      "persistenceUnit",
  ...
  public parsePropertyCategory(xmlElement: Element): PropertyCategoryProps {
    ...
    const priorityString = this.getRequiredAttribute(
      xmlElement,
      "priority",
    ...
    if (isNaN(priority))
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `The PropertyCategory ${this._currentItemFullName} has an invalid 'priority' attribute. It should be a numeric value.`
      );
  ...
  public parseInvertedUnit(xmlElement: Element): InvertedUnitProps {
    ...
    let invertsUnit = this.getRequiredAttribute(
      xmlElement,
      "invertsUnit",
  ...
  public parseFormat(xmlElement: Element): SchemaItemFormatProps {
    ...
    const decimalSeparator = this.getOptionalAttribute(
      xmlElement,
      "decimalSeparator"
    );
    const thousandSeparator = this.getOptionalAttribute(
      xmlElement,
      "thousandSeparator"
    );
    const uomSeparator = this.getOptionalAttribute(xmlElement, "uomSeparator");
    const scientificType = this.getOptionalAttribute(
      xmlElement,
      "scientificType"
    );

    const stationOffsetSize = this.getOptionalIntAttribute(
      xmlElement,
      "stationOffsetSize",
      `The Format ${this._currentItemFullName} has an invalid 'stationOffsetSize' attribute. It should be a numeric value.`
    );

    const stationSeparator = this.getOptionalAttribute(
      xmlElement,
      "stationSeparator"
    );

    let composite: object | undefined;
    const compositeResult = this.getElementChildrenByTagName(
      xmlElement,
      "Composite"
    );
    if (compositeResult.length > 0) {
      ...
      const includeZeroString = this.getOptionalAttribute(
        compositeElement,
        "includeZero"
      );
      let includeZero: boolean | undefined;
      if (includeZeroString) {
        includeZero = this.parseBoolean(
          includeZeroString,
      ...
      const unitsResult = this.getElementChildrenByTagName(
        compositeElement,
        "Unit"
      );
      if (unitsResult.length < 1)
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `The Format ${this._currentItemFullName} has an invalid 'Composite' element. It should have 1-4 Unit elements.`
        );
      ...
      for (const unit of unitsResult) {
        ...
        if (null === name || 0 === name.length)
          throw new ECObjectsError(
            ECObjectsStatus.InvalidSchemaXML,
            `The Format ${this._currentItemFullName} has a Composite with an invalid Unit. One of the Units is missing the required 'name' attribute.`
          );
  ...
  public *getProperties(
    xmlElement: Element,
    itemName: string
  ): Iterable<[string, string, Element]> {
    const propertyTagRegex = /EC((Struct(Array)?)|Array|Navigation)?Property/;
    const children = this.getElementChildrenByTagName(
      xmlElement,
      propertyTagRegex
    );
    ...
    for (const child of children) {
      ...
      const propertyName = this.getRequiredAttribute(
        child,
        "propertyName",
      ...
      if (propertyType === undefined)
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `The ECProperty ${itemName}.${propertyName} has an invalid type. ${childType} is not a valid ECProperty type.`
        );
  ...
  public getPropertyCustomAttributeProviders(
    ...
    return this.getCustomAttributeProviders(
      xmlElement,
      "ECProperty",
      `${this._currentItemFullName}.${propName}`
    );
  }
  ...
  public getRelationshipConstraintCustomAttributeProviders(
    ...
    const targetCustomAttributes = this.getCustomAttributeProviders(
      targetElement,
      "Source Constraint of",
      this._currentItemFullName
    );
  ...
  private getOptionalAttribute(
    ...
    if (!xmlElement.hasAttribute(attributeName)) return undefined;
  ...
  private getSchemaReference(xmlElement: Element): SchemaReferenceProps {
    ...
    const version = this.getRequiredAttribute(
      xmlElement,
      "version",
  ...
  private getSchemaItemType(rawType: string): string | undefined {
    switch (rawType.toLowerCase()) {
      ...
      case "ecstructclass":
        return "StructClass";
      case "eccustomattributeclass":
        return "CustomAttributeClass";
      case "ecrelationshipclass":
  ...
  private getRelationshipConstraintProps(
    ...
    const constraintClassesResult = this.getElementChildrenByTagName(
      xmlElement,
      "Class"
    );
    if (constraintClassesResult.length < 1)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `The ${constraintName} is missing the required Class tags.`
      );
    ...
    for (const constraintClass of constraintClassesResult) {
      ...
      if (null === constraintClassId || 0 === constraintClassId.length)
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `The ${constraintName} has a Class that is missing the required 'class' attribute.`
        );
  ...
  private getPropertyProps(xmlElement: Element): PropertyProps {
    ...
    if (propType === undefined)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `The ECProperty ${this._currentItemFullName}.${propName} has an invalid type. ${propType} is not a valid ECProperty type.`
      );
    ...
    const priority = this.getOptionalIntAttribute(
      xmlElement,
      "priority",
    ...
    if (category) category = this.getQualifiedTypeName(category);
  ...
  private getPropertyTypeName(xmlElement: Element): string {
    const propName = this.getPropertyName(xmlElement);
    const rawTypeName = this.getRequiredAttribute(
      xmlElement,
      "typeName",
    ...
    if (primitiveType) return rawTypeName;
  ...
  private getPropertyMinAndMaxOccurs(xmlElement: Element): {
    minOccurs: number | undefined;
    maxOccurs: number | undefined;
  } {
    const propName = this.getPropertyName(xmlElement);
    const minOccurs = this.getOptionalIntAttribute(
      xmlElement,
      "minOccurs",
    ...
    if ("unbounded" === maxOccursStr)
      ...
      if (isNaN(maxOccurs))
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `The ECProperty ${this._currentItemFullName}.${propName} has an invalid 'maxOccurs' attribute. It should be a numeric value.`
        );
    }
  ...
  private *getCustomAttributeProviders(
    ...
    if (customAttributesResult.length < 1) return;

    const attributes = this.getElementChildren(customAttributesResult[0]);
    for (const attribute of attributes) {
      if ("ECClass" === type && "IsMixin" === attribute.tagName) continue;
  ...
  private getCustomAttributeProvider(
    ...
    if (
      null === ns ||
      !this.isSchemaFullNameValidForVersion(ns, this._ecXmlVersion)
    )
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
  ...
  private readPropertyValue(propElement: Element, parentClass: ECClass): any {
    ...
    if (propertyClass.isPrimitive()) {
      if (propertyClass.isEnumeration() && propertyClass.enumeration) {
        enumeration = propertyClass.schema.lookupItemSync(
          propertyClass.enumeration.fullName
        );
        if (!enumeration)
          throw new ECObjectsError(
            ECObjectsStatus.ClassNotFound,
            `The Enumeration class '${propertyClass.enumeration.fullName}' could not be found.`
          );
      }
      const primitiveType =
        enumeration && enumeration.type
          ? enumeration.type
          : propertyClass.primitiveType;
      return this.readPrimitivePropertyValue(propElement, primitiveType);
    }

    if (propertyClass.isStruct())
      return this.readStructPropertyValue(
        propElement,
        propertyClass.structClass
      );
  ...
  private readArrayPropertyValue(
    ...
    if (propertyClass.isPrimitive())
      return this.readPrimitiveArrayValues(
        propElement,
        (propertyClass as PrimitiveProperty).primitiveType
      );

    if (propertyClass.isStruct())
      return this.readStructArrayValues(
        propElement,
        propertyClass as StructArrayProperty
      );
  ...
  private readStructArrayValues(
    propElement: Element,
    propertyClass: StructArrayProperty
  ): any {
    const children = this.getElementChildren(propElement);
    const values: any = [];
    for (const child of children) {
      const value = this.readStructPropertyValue(
        child,
        propertyClass.structClass
      );
      values.push(value);
  ...
  private readStructPropertyValue(
    ...
    for (const child of children) {
      ...
      if (value !== undefined) structObj[child.tagName] = value;
    }
    return structObj;
  }
  ...
  private readPrimitivePropertyValue(
    ...
    if (!propElement.textContent)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `Primitive property '${propElement.tagName}' has an invalid property value.`
      );
    ...
    switch (primitiveType) {
      ...
      case PrimitiveType.DateTime:
        return this.getDatePropertyValue(
          propElement.textContent,
          propElement.tagName
        );
      case PrimitiveType.Point2d:
        return this.getPoint2DPropertyValue(
          propElement.textContent,
          propElement.tagName
        );
      case PrimitiveType.Point3d:
        return this.getPoint3DPropertyValue(
          propElement.textContent,
          propElement.tagName
        );
      case PrimitiveType.Boolean:
        return this.getBooleanPropertyValue(
          propElement.textContent,
          propElement.tagName
        );
      case PrimitiveType.Integer:
      case PrimitiveType.Long:
        return this.getIntegerPropertyValue(
          propElement.textContent,
          propElement.tagName
        );
      case PrimitiveType.Double:
        return this.getDoublePropertyValue(
          propElement.textContent,
          propElement.tagName
        );
      default:
        throw new ECObjectsError(
          ECObjectsStatus.InvalidSchemaXML,
          `Primitive property '${propElement.tagName}' has an invalid primitive type.`
        );
    }
  }
  ...
  private getBooleanPropertyValue(
    ...
    throw new ECObjectsError(
      ECObjectsStatus.InvalidSchemaXML,
      `Property '${propName}' has an invalid property value. An boolean value was expected.`
    );
  }
  ...
  private getIntegerPropertyValue(propValue: string, propName: string): number {
    ...
    if (isNaN(result) || result % 1 !== 0)
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `Property '${propName}' has an invalid property value. An integer value was expected.`
      );
  ...
  private getDatePropertyValue(propValue: string, propName: string): Date {
    ...
    if (isNaN(result))
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `Property '${propName}' has an invalid property value. A date in milliseconds was expected.`
      );
  ...
  private getDoublePropertyValue(propValue: string, propName: string): number {
    ...
    if (isNaN(result))
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `Property '${propName}' has an invalid property value. A double value was expected.`
      );
  ...
  private getPoint2DPropertyValue(propValue: string, propName: string): any {
    ...
    if (result.length !== 2) {
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `Property '${propName}' has an invalid property value. A Point 2D value was expected.`
      );
    }
  ...
  private getPoint3DPropertyValue(propValue: string, propName: string): any {
    ...
    if (result.length !== 3) {
      throw new ECObjectsError(
        ECObjectsStatus.InvalidSchemaXML,
        `Property '${propName}' has an invalid property value. A Point 3D value was expected.`
      );
    }
  ...
  private isSchemaFullNameValidForVersion(
    ...
    ecXmlVersion: ECXmlVersion
  ) {
    const schemaNameParts = schemaFullName.split(".");

    if (
      (ecXmlVersion.readVersion >= 3 && ecXmlVersion.writeVersion >= 2) ||
      ecXmlVersion.readVersion > 3
    ) {
      if (schemaNameParts.length < 4) return false;
    } else {
      if (schemaNameParts.length < 3) return false;
    }
    return true;
  }
  ...
  private getQualifiedTypeName(rawTypeName: string): string {
    ...
    if (nameParts.length !== 2) return `${this._schemaName}.${rawTypeName}`;
    ...
    if (this._schemaReferenceNames.has(nameParts[0].toLowerCase()))
      return `${this._schemaReferenceNames.get(nameParts[0].toLowerCase())}.${
        nameParts[1]
      }`;

    throw new ECObjectsError(
      ECObjectsStatus.InvalidSchemaXML,
      `No valid schema found for alias ${nameParts[0]}`
    );
  }
  ...
  private getQualifiedPresentationUnits(presentationUnits: string[]): string[] {
    ...
    for (const presentationUnit of presentationUnits) {
      ...
      qualifiedPresentation = this.getQualifiedTypeName(
        presentationUnitFormats[0]
      );
      if (presentationUnitFormats.length > 1) {
        for (let i = 1; i < presentationUnitFormats.length; ++i) {
          if (presentationUnitFormats[i].length === 0) continue;
          const verticalSlashIdx = presentationUnitFormats[i].indexOf("|");
          const overrideFormat = this.getQualifiedTypeName(
            presentationUnitFormats[i].slice(0, verticalSlashIdx)
          );
          const formatLabel =
            presentationUnitFormats[i].slice(verticalSlashIdx);
