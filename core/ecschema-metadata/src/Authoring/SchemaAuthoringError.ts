 ...
 *
 * Authoring operations report bad *data* as [Authoring.SchemaIssue]($ecschema-metadata)s and never throw for it - a document
 * is allowed to be invalid. This is thrown only where the caller has to change something about the
 * *call* for it to succeed, most often by putting a schema in the [Authoring.SchemaSet]($ecschema-metadata) first. Bugs and
...
export interface SchemaAuthoringError extends ITwinError {
  ...
  /** Path of the schema element involved, in the same form as [Authoring.SchemaIssue.location]($ecschema-metadata)
...
export namespace SchemaAuthoringError {
   ...
   * - `duplicate-schema-name` - a [Authoring.SchemaSet]($ecschema-metadata) already holds a schema of that name. Call
   *   [Authoring.SchemaSet.moveOut]($ecschema-metadata) for the incumbent first.
   */
  export type Key =
  ...
  /** Determines whether an error is a [Authoring.SchemaAuthoringError]($ecschema-metadata), optionally of one specific kind. */
  ...
  /** Instantiates and throws a [Authoring.SchemaAuthoringError]($ecschema-metadata). */
