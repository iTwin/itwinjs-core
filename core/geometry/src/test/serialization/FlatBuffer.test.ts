...
it("HelloCurveVector", () => {
  const ck = new Checker();
  const cvs = [Sample.createCappedArcPath(3.0, 10, 90), ...Sample.createSimpleLoops(), ...Sample.createSimpleParityRegions(false)];
  for (const cv of cvs) {
...
it("HelloSubdivisionSurface", () => {
  const ck = new Checker();
  const mesh = Sample.createTriangularUnitGridPolyface(Point3d.create(0, 0, 0), Vector3d.unitX(), Vector3d.unitY(), 3, 3);
  ...
  if (g instanceof GeometryQuery) {
    ...
    if (ck.testType(justTheBytes, Uint8Array, "to FB with signature") && justTheBytes && bytesWithSignature) {
      if (ck.testExactNumber(justTheBytes.length + 8, bytesWithSignature.length, "Signature adds 8 bytes")) {
        ...
        if (
          ck.testTrue(g2 instanceof GeometryQuery && g.isAlmostEqual(g2), "GeometryQuery round-tripped through FB with signature") &&
...
it("HelloBSplineSurface", () => {
  const ck = new Checker();
  const surfaces = [Sample.createXYGridBsplineSurface(6, 5, 4, 2)!, Sample.createWeightedXYGridBsplineSurface(7, 6, 5, 4, 1.1, 1.2, 1.3, 1.4)!];
