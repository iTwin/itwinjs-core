...
describe("BsplineCurve", () => {
  ...
  it("StrokeWithKnotsA", () => {
    const ck = new Checker();
    const poles: Point3d[] = [];
    for (let i = 0; i < 10; i++) poles.push(Point3d.create(i, i * i, 0));
  ...
  it("StrokeParabola", () => {
    ...
    for (let i = 0; ; i++) {
      ...
      if (i === 0) {
        options = undefined;
      } else if (i === 1) options = StrokeOptions.createForCurves();
      else if (i === 2) options = StrokeOptions.createForFacets();
      else if (i === 3) {
      ...
      for (const x1 of [30, 50, 80, 95]) {
        ...
        for (const y2 of [10, 40, 80]) {
          ...
          ck.testExactNumber(
            ...
            "bezier stroke counts the same for isolated and bspline"
          );
          y0Out += 100;
        }
        x0Out += 100;
