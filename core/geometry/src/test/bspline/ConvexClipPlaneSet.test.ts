...
} from "../../clipping/ConvexClipPlaneSet";
import {
...
describe("ConvexClipPlaneSet", () => {
  it("HelloWorld", () => {
    ...
    const errorSet1 = ConvexClipPlaneSet.fromJSON(
      1 as unknown as ConvexClipPlaneSetProps
    );
    ...
    for (const transform of [
      ...
      Transform.createFixedPointAndMatrix(
        ...
        Matrix3d.createRotationAroundVector(
          Vector3d.create(1, 2, 9),
          Angle.createDegrees(23)
        )!
      ),
    ]) {
      ...
      for (const f of [-2, -0.2, 0.001, 0.3, 0.998, 1.0002, 3]) {
        ...
        ck.testBoolean(
          boxA.isPointInside(pointM),
          boxB.isPointInside(pointM),
          "point inside",
          f,
          pointM
        );
        ...
        ck.testBoolean(
          boxA.isPointInside(pointM),
          Geometry.isIn01(f),
          "point inside versus segment fraction, ",
          pointM
        );
        ...
        ck.testBoolean(
          inOut,
          boxD.isPointInside(pointN),
          "inOut for transformed",
          f,
          pointN
        );
      }
  ...
  it("UnionOfConvexSets", () => {
    const ck = new Checker();
    const setA = UnionOfConvexClipPlaneSets.fromJSON(
      1 as unknown as UnionOfConvexClipPlaneSetsProps
    );
    const setB = UnionOfConvexClipPlaneSets.createEmpty(setA);
    const box01 = ConvexClipPlaneSet.createRange3dPlanes(
      Range3d.createXYZXYZ(0, 0, 0, 1, 1, 1)
    );
    const box12 = ConvexClipPlaneSet.createRange3dPlanes(
      Range3d.createXYZXYZ(1, 0, 0, 2, 1, 1)
    );
    const box23 = ConvexClipPlaneSet.createRange3dPlanes(
      Range3d.createXYZXYZ(2, 0, 0, 3, 1, 1)
    );
    const setC = UnionOfConvexClipPlaneSets.createConvexSets([box01, box12]);
    const setD = UnionOfConvexClipPlaneSets.createConvexSets([
      box01,
      box12,
      box23,
    ]);
    const setCReversed = UnionOfConvexClipPlaneSets.createConvexSets([
      box12,
      box01,
    ]);
    ...
    ck.testFalse(
      setC.isAlmostEqual(setCReversed),
      "almostEqual different order"
    );
    ck.testDefined(setB);
    const points: Point3d[] = [];
    const range = Range3d.createNull();
    setC.computePlanePlanePlaneIntersectionsInAllConvexSets(
      points,
      range,
      undefined,
      false
  ...
  it("parser", () => {
    ...
    const outBoxB = BooleanClipFactory.createCaptureClipOutside(
      boxB
    ) as BooleanClipNode;
