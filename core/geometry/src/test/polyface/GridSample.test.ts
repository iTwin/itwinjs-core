    ...
    if (n > 1) {
      for (let j = -n; j <= n; j++) {
        for (let i = -n; i <= n; i++) {
          ...
          if (r < coneRadius) this.setGridZToMax(iMid + i, jMid + j, zNew * (1.0 - r / coneRadius));
        }
      }
    }
  }
...
function buildSampledGrid(
  ...
  for (const point of points) {
    ...
    if (templateType === 3) grid.setGridZToCubicMax(i, j, point.z, templateWidth);
    if (templateType === 2) grid.setGridZToConeMax(i, j, point.z, templateWidth);
