...
import {
  XYIndexGrid,
  XYPointBuckets,
} from "../../polyface/multiclip/XYPointBuckets";
...
describe("XYPointBuckets", () => {
  it("HelloWorld", () => {
    ...
    ck.testUndefined(
      XYIndexGrid.createWithEstimatedCounts(Range2d.createNull(), 100, 10),
      "expect null grid"
    );
    ck.testUndefined(
      XYPointBuckets.create(new GrowableXYZArray(), 10),
      "no buckets for no points"
    );
    const numX = 10;
    for (const yDiff of [-4, 0, 5]) {
      const numY = numX + yDiff;
      const points = GrowableXYZArray.create(
        Sample.createXYGrid(numX, numY, 1, 1)
      );
      ...
      let numHit = 0;
      searcher.announcePointsInRange(
        range,
      ...
      if (
        ck.testDefined(indexArrayA) &&
        ck.testDefined(indexArrayA1) &&
        indexArrayA &&
        indexArrayA1
      )
        ck.testExactNumber(
    ...
    for (const numY of [6, 10]) {
      ...
      if (numY > 8) {
        ...
        for (let i = 0; i < num0; i++) {
          const extraPoints = Sample.createStar(
