...
function verifyMonotoneArraySearch(ck: Checker, data: number[], name: string) {
  for (let i0 = 0; i0 + 1 < data.length; i0++) {
    for (let k = data[i0]; k < data[i0 + 1]; k++) {
      ...
      if (ck.testDefined(i, "expect searchMonotoneNumbers to return defined"))
        ck.testExactNumber(i0, i, `searchMonotoneNumbers(${k}) on ${name} indices returns the floor index`);
    }
  }
}
...
describe("IndexedPolyfaceWalker", () => {
  it("edgeIndexToFacetIndex", () => {
    ...
    verifyMonotoneArraySearch(ck, triIndices, "triangle");
    const quadIndices = [0, 4, 8, 12, 16];
    verifyMonotoneArraySearch(ck, quadIndices, "quad");
