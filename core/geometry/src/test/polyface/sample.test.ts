  ...
  it("PlateWithHoles", () => {
    ...
    for (let i = 0; i < numCircle; i++) {
      const arc = Arc3d.createXY(
        Point3d.create((-0.5 * i * a) / (numCircle + 1), b / 2),
        r,
        AngleSweep.createStartEndDegrees(180, -180)
      );
      plateBaseWithHoles.tryAddChild(Loop.create(arc));
    }
    const plateSolidWithHoles = LinearSweep.create(
      plateBaseWithHoles,
      Vector3d.create(0, 0, c),
      true
    )!;
    let x0 = 0;
    const xStep = 2.0 * a;
    ...
    x0 += xStep;
    y0 = 0;
    const plateBlankSolid = LinearSweep.createZSweep(
      plateBasePoints,
      0,
      c,
      true
    ...
    for (const baseLoop of plateBaseWithHoles.children) {
      if (baseLoop instanceof Loop && baseLoop.children[0] instanceof Arc3d) {
        ...
        const clipperA = ConvexClipPlaneSet.createSweptPolyline(
          strokes.getPoint3dArray(),
        ...
        GeometryCoreTestIO.captureCloneGeometry(
          geometryToSave,
          clipBuilderA.claimPolyface(1, true),
          x0 + xStep,
          y0
        );
        y0 += yStep;
