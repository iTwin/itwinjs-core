    ...
    if (s instanceof SolidPrimitive) {
      ...
      if (ck.testPointer(s1, "solid clone") && s1) {
        ...
        if (ck.testPointer(s2) && s1.tryTransformInPlace(scaleTransform)) {
          ...
          rangeScaledExpanded.expandInPlace(0.1 * rangeScaledExpanded.diagonal().magnitude()); // HACK -- ranges are not precise.  Allow fuzz.
      ...
      if (sC instanceof SolidPrimitive) {
        ...
        else ck.testFalse(s.isAlmostEqual(sC), "isAlmostEqual should detected cap change.");
      }
    }
  }
}
describe("Solids", () => {
  ...
  it("RotationalSweeps", () => {
    ...
    ck.testUndefined(
      RotationalSweep.create(contour, Ray3d.createXYZUVW(0, 0, 0, 0, 0, 0), Angle.createDegrees(180), false)
    );
    expect(ck.getNumErrors()).equals(0);
  });
