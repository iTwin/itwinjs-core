...
describe("PipePath", () => {
  ...
  it("TorusPipeTransformed", () => {
    ...
    // test invalid inputs
    let pipeBad = TorusPipe.createDgnTorusPipe(
      center,
      Vector3d.createZero(),
      vectorY,
      majorRadius,
      ...
      for (const yAxis of [vectorY, vectorY.negate(), Vector3d.create(1, 1, 0)]) {
        ...
        for (const scale of [Point2d.create(10, 10), Point2d.create(5, 10), Point2d.create(10, 5)]) {
          ...
          if (
            !ck.testCoordinate(
              1,
              pipeScaledRadii.cloneVectorX().magnitude(),
              "TorusPipe.cloneVectorX returns unit vector"
            ) ||
            !ck.testCoordinate(
              1,
              pipeScaledRadii.cloneVectorY().magnitude(),
              "TorusPipe.cloneVectorY returns unit vector"
            ) ||
            !ck.testCoordinate(
              1,
              pipeScaledRadii.cloneVectorZ().magnitude(),
              "TorusPipe.cloneVectorZ returns unit vector"
            )
          )
            continue;
          ...
          GeometryCoreTestIO.captureCloneGeometry(
            allGeometry,
            [pipeScaledRadii, builder.claimPolyface()],
            (x0 += 5),
            y0,
            z0
          );
          ...
          const scaleInLocalCoords = Transform.createOriginAndMatrix(
            Point3d.createZero(),
            Matrix3d.createScale(scale.x, scale.x, scale.y)
          );
          ...
          GeometryCoreTestIO.captureCloneGeometry(
            allGeometry,
            [pipeCloneUnScaled0, builder.claimPolyface()],
            x0,
            y0,
            z0
          );
          ...
          GeometryCoreTestIO.captureCloneGeometry(
            allGeometry,
            [pipeCloneUnScaled1, builder.claimPolyface()],
            x0,
            y0,
            z0
          );
