 ...
 *--------------------------------------------------------------------------------------------*/
import { YawPitchRollAngles, Geometry, Angle, AngleSweep, AxisOrder, Complex, Point3d, RotMatrix } from "../geometry-core";
...
function testEqualOrderedRotationAngles(a: OrderedRotationAngles, b: OrderedRotationAngles, ck: Checker) {
  ck.testAngleNoShift(a.xAngle, b.xAngle);
  ck.testAngleNoShift(a.yAngle, b.yAngle);
  ck.testAngleNoShift(a.zAngle, b.zAngle);
}
...
describe("OrderedRotationAngles", () => {
  it("OrderedRotationAngles.RoundTrip", () => {
    ...
    // One Rotation
    const originalAngles = OrderedRotationAngles.createDegrees(45, 0, 0, AxisOrder.XYZ);
    const matrix = originalAngles.toRotMatrix();
    const angles = matrix.toOrderedRotationAngles(AxisOrder.ZYX); // order shouldn't matter with only one axis
    testEqualOrderedRotationAngles(originalAngles, angles, ck);

    OrderedRotationAngles.createRadians(0, 0, 0.0001, AxisOrder.XZY, originalAngles);
    originalAngles.toRotMatrix(matrix);
    matrix.toOrderedRotationAngles(AxisOrder.YZX, angles);
    testEqualOrderedRotationAngles(originalAngles, angles, ck);

    // Two Rotations
    OrderedRotationAngles.createDegrees(22, 0, 19, AxisOrder.ZYX, originalAngles);
    originalAngles.toRotMatrix(matrix);
    matrix.toOrderedRotationAngles(AxisOrder.ZYX, angles);  // here, same order matters
    testEqualOrderedRotationAngles(originalAngles, angles, ck);

    OrderedRotationAngles.createDegrees(0, .001, 89.999, AxisOrder.ZYX, originalAngles);
    originalAngles.toRotMatrix(matrix);
    matrix.toOrderedRotationAngles(AxisOrder.ZYX, angles);
    testEqualOrderedRotationAngles(originalAngles, angles, ck);

    // Three Rotations
    OrderedRotationAngles.createDegrees(22, 2, 19, AxisOrder.XYZ, originalAngles);
    originalAngles.toRotMatrix(matrix);
    matrix.toOrderedRotationAngles(AxisOrder.XYZ, angles);
    testEqualOrderedRotationAngles(originalAngles, angles, ck);

    OrderedRotationAngles.createDegrees(89, 88, 87, AxisOrder.YZX, originalAngles);
    originalAngles.toRotMatrix(matrix);
    matrix.toOrderedRotationAngles(AxisOrder.YZX, angles);
    testEqualOrderedRotationAngles(originalAngles, angles, ck);
  ...
  it("OrderedRotationAngles.FromRotMatrix", () => {
    const ck = new Checker();
    let /*x = .0192, y = .7564,*/ z = Math.PI / 2;

    // No Rotation
    const matrix = RotMatrix.createIdentity();
    const angles = matrix.toOrderedRotationAngles(AxisOrder.YZX); // order doesn't matter
    const expectedAngles = OrderedRotationAngles.createRadians(0, 0, 0, AxisOrder.ZXY);
    testEqualOrderedRotationAngles(angles, expectedAngles, ck);
    ...
    matrix.toOrderedRotationAngles(AxisOrder.YXZ, angles);
    OrderedRotationAngles.createRadians(0, 0, Math.PI / 2, AxisOrder.XYZ, expectedAngles);
    testEqualOrderedRotationAngles(angles, expectedAngles, ck);
    matrix.toOrderedRotationAngles(AxisOrder.ZXY, angles);  // order doesn't matter
    testEqualOrderedRotationAngles(angles, expectedAngles, ck);
