  ...
  */
  ...
  it("DartInTriangle", () => {
    ...
    if (graph1) {
      const polyface1 = PolyfaceBuilder.graphToPolyface(graph1);
      ck.testCoordinate(
        Math.abs(outerArea) - Math.abs(innerArea),
        PolyfaceQuery.sumFacetAreas(polyface1),
        "area of dart in triangle"
      );
    ...
    if (graph2) {
      ...
      ck.testCoordinate(
        Math.abs(outerArea) - Math.abs(innerArea),
        PolyfaceQuery.sumFacetAreas(polyface2),
        "area of reversed dart in triangle"
      );
    ...
    if (ck.testDefined(graph1, "Triangulation failed") && graph1) {
      ...
      if (graph2) {
        ...
        ck.testExactNumber(
          numRemovedEdges1,
          numRemovedEdges2,
          "expandConvexFaces methods removed different numbers of edges."
        );
        position.x += range.xLength();
      }
      ...
      for (const perpendicularFactor of [0.85, -1.0, -0.5]) {
        for (const generatorFunction of [
          ...
          for (const degrees of [0, 10, 79]) {
            ...
            ck.testTrue(
              tryExpandConvex2(ck, allGeometry, graph1, graph2, position),
              "tryExpandConvex2 failed on Fractals."
            );
          }
        }
      }
