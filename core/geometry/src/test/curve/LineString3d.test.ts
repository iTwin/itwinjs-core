...
function exerciseClonePartialLineString3d(
  ...
  for (const extendFraction of [0.05, 0.721, 1.4, 3.96]) {
    ...
    const preLocalFractionHalf = Math.abs(
      lsA.globalFractionToSegmentIndexAndLocalFraction(-extendFraction / 2).fraction
    );
    ...
    if (ck.testPointer(ce01) && expectValidResults) {
      ...
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          ce01.curveLength(),
          lsA.curveLength() + preLocalFraction * lsA.points[0].distance(lsA.points[1]),
    ...
    if (ck.testPointer(ce01e) && expectValidResults) {
      ...
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          ...
          preLocalFraction * lsA.points[0].distance(lsA.points[1]) +
            lsA.curveLength() +
            postLocalFraction * lsA.points[lsA.numPoints() - 2].distance(lsA.points[lsA.numPoints() - 1]),
    ...
    if (ck.testPointer(c1ee) && expectValidResults) {
      ...
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          c1ee.curveLength(),
          postLocalFractionHalf * lsA.points[lsA.numPoints() - 2].distance(lsA.points[lsA.numPoints() - 1]),
...
describe("LineString3d", () => {
  ...
  it("createXY", () => {
    const ck = new Checker();
    const xyArray = [Point2d.create(1, 1), Point2d.create(4, 1), Point2d.create(4, 2), Point2d.create(0, 2)];
    const jsArrayXY = [];
    const jsArrayXYZ = [];
    const dz = 10.0;
  ...
  it("addAuxData", () => {
    ...
    if (ck.testPointer(ls.fractions)) ck.testExactNumber(n, ls.fractions.length);
    if (ck.testPointer(ls.packedDerivatives)) ck.testExactNumber(n, ls.packedDerivatives.length);
    if (ck.testPointer(n, ls.packedDerivatives)) ck.testExactNumber(n, ls.packedDerivatives!.length);
  ...
  it("AnnounceClipIntervals", () => {
    ...
    let numAnnounce = 0;
    ls.announceClipIntervals(clipper, (_a0: number, _a1: number, _cp: CurvePrimitive) => {
      numAnnounce++;
    });
...
class LineStringWithIterator {
  ...
  public pointAt(index: number): Point3d | undefined {
    const k = index * 3;
    if (k < this._data.length) return new Point3d(this._data[k], this._data[k + 1], this._data[k + 2]);
    return undefined;
  }
}
...
describe("LineStringAnnotation", () => {
  it("DegenerateCases", () => {
    ...
    ck.testExactNumber(
      0,
      ls0.appendPlaneIntersectionPoints(Plane3dByOriginAndUnitNormal.createXYPlane(), intersections)
    );
    ...
    for (const f0 of fractions) {
      for (const f1 of fractions) {
        if (f0 !== f1) {
          ...
          ck.testRange3d(partialSegment.range(), segment.rangeBetweenFractions(f0, f1), {
            title: "segment rangeBetweenFractions",
            f0,
            f1,
          });
          ck.testRange3d(partialString.range(), linestring.rangeBetweenFractions(f0, f1), {
            title: "Linestring rangeBetweenFractions",
            f0,
            f1,
          });
          ...
          ck.testCoordinate(partialSegment.curveLength(), segment.curveLengthBetweenFractions(f0, f1), {
            title: "segment length between fractions",
            f0,
            f1,
          });
          ck.testCoordinate(partialString.curveLength(), linestring.curveLengthBetweenFractions(f0, f1), {
            title: "linestring length between fractions",
            f0,
            f1,
          });
          ck.testCoordinate(partialSegment.curveLength(), partialString.curveLength(), {
            title: "partial linestring, partial segment  length",
            f0,
            f1,
          });
        }
