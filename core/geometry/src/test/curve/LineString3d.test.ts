...
function exerciseClonePartialLineString3d(
  ck: Checker,
  ...
  for (const extendFraction of [0.05, 0.721, 1.4, 3.96]) {
    ...
    const c1ee = lsA.clonePartialCurve(
      1 + extendFraction / 2,
      1 + extendFraction
    ); // does not contain [0,1]

    // first segment distance multipliers
    const preLocal = lsA.globalFractionToSegmentIndexAndLocalFraction(
      -extendFraction
    );
    ...
    ck.testExactNumber(
      0,
      preLocal.index,
      "global parameter < 0 converts to local index 0"
    );
    const preLocalFractionHalf = Math.abs(
      lsA.globalFractionToSegmentIndexAndLocalFraction(-extendFraction / 2)
        .fraction
    );
    // last segment distance multipliers
    const postLocal = lsA.globalFractionToSegmentIndexAndLocalFraction(
      1 + extendFraction
    );
    ...
    ck.testExactNumber(
      lsA.numPoints() - 2,
      postLocal.index,
      "global parameter > 1 converts to local index numPoints-2"
    );
    const postLocalFractionHalf =
      lsA.globalFractionToSegmentIndexAndLocalFraction(1 + extendFraction / 2)
        .fraction - 1;
    ...
    if (ck.testPointer(cee0) && expectValidResults) {
      ck.testExactNumber(
        cee0.numPoints(),
        2,
        "isolated pre-extension is a segment"
      );
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          cee0.curveLength(),
          preLocalFractionHalf * lsA.points[0].distance(lsA.points[1]),
          "isolated pre-extension length is fraction of first segment"
        );
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        cee0,
    ...
    if (ck.testPointer(ce0) && expectValidResults) {
      ...
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          ce0.curveLength(),
          preLocalFraction * lsA.points[0].distance(lsA.points[1]),
          "pre-extension length is fraction of first segment"
        );
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        ce0,
    ...
    if (ck.testPointer(ce01) && expectValidResults) {
      ck.testExactNumber(
        ce01.numPoints(),
        lsA.numPoints(),
        "pre-extended linestring has same point count"
      );
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          ce01.curveLength(),
          lsA.curveLength() +
            preLocalFraction * lsA.points[0].distance(lsA.points[1]),
          "pre-extended linestring has expected length"
        );
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        ce01,
    ...
    if (ck.testPointer(ce01e) && expectValidResults) {
      ck.testExactNumber(
        ce01e.numPoints(),
        lsA.numPoints(),
        "bi-extended linestring has same point count"
      );
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          ce01e.curveLength(),
          preLocalFraction * lsA.points[0].distance(lsA.points[1]) +
            lsA.curveLength() +
            postLocalFraction *
              lsA.points[lsA.numPoints() - 2].distance(
                lsA.points[lsA.numPoints() - 1]
              ),
          "bi-extended linestring has expected length"
        );
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        ce01e,
    ...
    if (ck.testPointer(c01e) && expectValidResults) {
      ck.testExactNumber(
        c01e.numPoints(),
        lsA.numPoints(),
        "post-extended linestring has same point count"
      );
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          c01e.curveLength(),
          ...
          "post-extended linestring has expected length"
        );
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        c01e,
    ...
    if (ck.testPointer(c1e) && expectValidResults) {
      ...
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          ...
          "post-extension length is fraction of last segment"
        );
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        c1e,
    ...
    if (ck.testPointer(c1ee) && expectValidResults) {
      ck.testExactNumber(
        c1ee.numPoints(),
        2,
        "isolated post-extension is a segment"
      );
      if (!lsA.isPhysicallyClosed)
        ck.testCoordinate(
          c1ee.curveLength(),
          postLocalFractionHalf *
            lsA.points[lsA.numPoints() - 2].distance(
              lsA.points[lsA.numPoints() - 1]
            ),
          "isolated post-extension length is fraction of last segment"
        );
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        c1ee,
...
function exerciseLineString3d(ck: Checker, lsA: LineString3d) {
  ...
  for (
    let segmentIndex = 0;
    segmentIndex === 0 || segmentIndex + 1 < lsA.numPoints();
    segmentIndex++
  ) {
    for (const localFraction of [0.1, 0.1, 0.6, 0.6]) {
      const globalFraction = lsA.segmentIndexAndLocalFractionToGlobalFraction(
        segmentIndex,
        localFraction
      );
  ...
  if (
    ck.testPointer(partA, "forward partial") &&
    ck.testPointer(partB, "backward partial")
  )
    ck.testCoordinate(
      lsA.curveLength(),
      partA.curveLength() + partB.curveLength(),
      "Partial curves sum to length",
      lsA,
      partA,
      partB
    );
...
describe("LineString3d", () => {
  it("HelloWorld", () => {
    ...
    ck.testUndefined(
      lsC.quickUnitNormal(),
      "quickUnitNormal expected failure 1 point"
    );
    lsC.addPoint(point420);
    ck.testUndefined(
      lsC.quickUnitNormal(),
      "quickUnitNormal expected failure 2 point"
    );
    lsC.addPoint(point420.interpolate(0.6, point100));
    ck.testUndefined(
      lsC.quickUnitNormal(),
      "quickUnitNormal expected failure 3 point colinear"
    );
  ...
  it("ClonePartialFromExtendedClosestPointDetailFraction", () => {
    ...
    ck.testLT(
      detail0.fraction,
      1,
      "Point projected off curve start has projection fraction < 0"
    );
    ck.testLT(
      1,
      detail1.fraction,
      "Point projected off curve end has projection fraction > 1"
    );
    ck.testPoint3d(
      detail0.point,
      ls0.startPoint(),
      "Point projected off curve start equals start of clonedPartialCurve at projection fraction < 0"
    );
    ck.testPoint3d(
      detail0.point,
      ls.fractionToPoint(detail0.fraction),
      "Point projected off curve start equals fractionToPoint at projection fraction < 0"
    );
    ck.testPoint3d(
      detail1.point,
      ls1.endPoint(),
      "Point projected off curve end equals end of clonedPartialCurve at projection fraction > 1"
    );
    ck.testPoint3d(
      detail1.point,
      ls.fractionToPoint(detail1.fraction),
      "Point projected off curve end equals fractionToPoint at projection fraction > 1"
    );
  ...
  it("addResolvedPoint", () => {
    ...
    const ls = LineString3d.createXY(
      [Point2d.create(0, 1), Point2d.create(0.5 * a, 1), Point2d.create(a, 1)],
      0
    );
    const lsZ = ls as any;
  ...
  it("RegularPolygon", () => {
    ...
    const poly4F = LineString3d.createRegularPolygonXY(
      center,
      4,
      radius,
      false
...
class IterableLineStringPoint3dIterator implements Iterator<Point3d> {
  ...
  public next(): IteratorResult<Point3d> {
    ...
    if (point) return { done: false, value: point };
    return { done: true, value: undefined } as any as IteratorResult<Point3d>;
  }
  public [Symbol.iterator](): IterableIterator<Point3d> {
    return this;
  }
}
/**
...
class LineStringWithIterator {
  ...
  public [Symbol.iterator](): IterableIterator<Point3d> {
    return new IterableLineStringPoint3dIterator(this);
  }
  /**
...
describe("LineStringAnnotation", () => {
  it("DegenerateCases", () => {
    ...
    ck.testFalse(
      ls0.isAlmostEqual(LineSegment3d.createXYZXYZ(0, 0, 0, 1, 1, 1))
    );
    ck.testExactNumber(
      0,
      ls0.appendPlaneIntersectionPoints(
  ...
  it("AppendInterpolated", () => {
    ...
    for (const i of [0, 1]) {
      ...
      if (i === 0) ls0.ensureEmptyFractions();
    }
  ...
  it("CreateAnnotations", () => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (ck.testPointer(vN)) ck.testVector3d(v, vN);
      ls.addDerivative(v);
      const v1 = ls.derivativeAt(i);
      if (ck.testPointer(v1)) ck.testVector3d(v, v1);
      if ((i & 0x01) !== 0) ls.addUVParam(Point2d.create(0, i));
      else ls.addUVParamAsUV(i, 0);
    }
    ...
    for (const f0 of fractions) {
      for (const f1 of fractions) {
        if (f0 !== f1) {
          ...
          ck.testRange3d(
            partialSegment.range(),
            segment.rangeBetweenFractions(f0, f1),
            { title: "segment rangeBetweenFractions", f0, f1 }
          );
          ck.testRange3d(
            partialString.range(),
            linestring.rangeBetweenFractions(f0, f1),
            { title: "Linestring rangeBetweenFractions", f0, f1 }
          );
          ck.testRange3d(partialSegment.range(), partialString.range(), {
            title: "partial linestring, partial segment range",
            f0,
            f1,
          });
          ck.testCoordinate(
            partialSegment.curveLength(),
            segment.curveLengthBetweenFractions(f0, f1),
            { title: "segment length between fractions", f0, f1 }
          );
          ck.testCoordinate(
            partialString.curveLength(),
            linestring.curveLengthBetweenFractions(f0, f1),
            { title: "linestring length between fractions", f0, f1 }
          );
          ck.testCoordinate(
            partialSegment.curveLength(),
            partialString.curveLength(),
            { title: "partial linestring, partial segment  length", f0, f1 }
          );
        }
      }
    }
