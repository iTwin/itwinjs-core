...
function testIntersectionsXYZ(
  ck: Checker,
  intersections: CurveLocationDetailArrayPair,
  minExpected: number,
  maxExpected: number,
  ...
  if (
    ck.testExactNumber(
      intersections.dataA.length,
      intersections.dataB.length,
      "intersections A B match"
    )
  ) {
    ...
    if (testCoordinates) {
      for (let i = 0; i < n; i++) {
        ck.testPoint3d(
          intersections.dataA[i].point,
          intersections.dataB[i].point,
...
describe("CurveCurveIntersectionXYZ", () => {
  it("LineLineMapped", () => {
    ...
    const intersectionsAB = CurveCurve.intersectionXYZ(
      segment0,
      false,
      segment1,
      false
    ...
    const intersectionsBA = CurveCurve.intersectionXYZ(
      segment2,
      false,
      segment0,
      false
  ...
  it("LineArcMapped", () => {
    ...
    const arc1 = Arc3d.create(
      ...
      Vector3d.create(5, 1, 0),
      Vector3d.create(-1, 7, 0)
    );
    const f0 = 0.0;
    const f1 = 0.25;
    ...
    for (const trans of [
      ...
      Transform.createRefs(
        ...
        Matrix3d.createRotationAroundVector(
          Vector3d.create(-1, 1, 2),
          Angle.createDegrees(-27)
        )!
      ),
    ]) {
      ...
      for (let i = 0; i < 10; ++i) {
        ...
        const intersections = CurveCurve.intersectionXYZ(
          seg,
          false,
          arc,
          false
        ...
        if (
          ck.testLE(1, intersections.dataA.length) &&
          ck.testLE(1, intersections.dataB.length) &&
          ck.testExactNumber(
            intersections.dataA.length,
            intersections.dataB.length
          )
  ...
  it("LineStringLineString", () => {
    ...
    const intersectionsX = CurveCurve.intersectionXYZ(
      linestring0,
      true,
      linestring1,
      true
  ...
  it("LineLineString", () => {
    const ck = new Checker();
    // const segment0 = LineSegment3d.createXYXY(1, 2, 4, 2);
    const lineSegment0 = LineSegment3d.create(
      Point3d.create(1, 4),
      Point3d.create(5, 1)
    );
    const lineString1 = LineString3d.create(
    ...
    const intersectionsX1 = CurveCurve.intersectionXYZ(
      lineString1,
      true,
      lineSegment0,
      true
  ...
  it("ArcArcInPlane", () => {
    ...
    const arcB = Arc3d.create(
      Point3d.create(4, 1, 1),
      Vector3d.create(2, 2, 0),
      Vector3d.create(-2, 2)
    );
    const intersectionsAB = CurveCurve.intersectionXYZ(arcA, true, arcB, true);
    testIntersectionsXYZ(ck, intersectionsAB, 2, 2);
