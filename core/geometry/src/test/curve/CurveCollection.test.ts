...
function verifyCurveCollection(ck: Checker, collection: CurveCollection) {
  ...
  if (ck.testPointer(path3)) {
    if (
      !ck.testBoolean(
        false,
        path3.isAlmostEqual(collection),
        "cloneTransform not almostEqual"
      )
    ) {
      ...
      if (ck.testPointer(path3A)) {
        ck.testBoolean(
          false,
          path3A.isAlmostEqual(collection),
          "cloneTransform not almostEqual"
        );
      }
    }
  ...
  ck.testBoolean(
    true,
    collection.isAlmostEqual(collection),
    "isAlmostEqual on self"
  );
  ...
  if (path3) {
    ...
    ck.testCoordinate(
      length1,
      length3,
      "length of clone(transform), transformInPlace"
    );
  ...
  ck.testCoordinate(
    range0.xLength() * scaleFactor,
    range1.xLength(),
    "scaled rangeX"
  );
  ck.testCoordinate(
    range0.yLength() * scaleFactor,
    range1.yLength(),
    "scaled rangeY"
  );
  ck.testCoordinate(
    range0.zLength() * scaleFactor,
    range1.zLength(),
    "scaled rangeZ"
  );
  ...
  ck.testFalse(
    collection.isOpenPath && collection.isClosedPath,
    "Collection cannot be both open and closed path"
  );
  ck.testFalse(
    collection.isOpenPath && collection.isAnyRegionType,
    "Collection cannot be both open and region"
  );
...
describe("CurveCollection", () => {
  it("HelloWorld", () => {
    ...
    for (const data of Sample.createBagOfCurves())
      verifyCurveCollection(ck, data);
    for (const data of Sample.createSimpleParityRegions())
      verifyCurveCollection(ck, data);
    for (const data of Sample.createSimplePaths(true))
      verifyCurveCollection(ck, data);
    for (const data of Sample.createSimpleLoops())
      verifyCurveCollection(ck, data);
    for (const data of Sample.createSimpleUnions())
      verifyCurveCollection(ck, data);
  ...
  it("hasNonLinearPrimitives", () => {
    ...
    for (const data of Sample.createBagOfCurves())
      if (data.checkForNonLinearPrimitives()) counts.nonLinearTrue++;
      else counts.nonLinearFalse++;
    // We think (hard to prove!!) that the array of BagOfCurves had some members in each category ... flag if not.
    ck.testLE(
      0,
      counts.nonLinearFalse,
      "BagOfCurves samples should have some linear-only"
    );
    ck.testLE(
      0,
      counts.nonLinearTrue,
      "BagOfCurves samples should have some nonLinear"
    );
  ...
  it("CyclicIndex", () => {
    ...
    ck.testUndefined(
      loop.cyclicCurvePrimitive(0),
      "cyclicCurvePrimitive returns undefined in empty loop"
    );
    const path = Path.create();
    ck.testUndefined(
      path.cyclicCurvePrimitive(0),
      "cyclicCurvePrimitive returns undefined in empty path"
    );
    ...
    ck.testPointer(
      loop.cyclicCurvePrimitive(0),
      "cyclicCurvePrimitive in singleton loop"
    );
    ck.testPointer(
      path.cyclicCurvePrimitive(0),
      "cyclicCurvePrimitive in singleton path"
    );
    for (const g of [loop, path]) {
      ...
      for (let k = 0; k < n; k++) {
        ...
        for (let i = -2; i < 4; i++) {
          ck.testTrue(
            loop.cyclicCurvePrimitive(k + i * n) === c0,
...
describe("ConsolidateAdjacentPrimitives", () => {
  it("Lines", () => {
    ...
    const yStep = 5.0;
    const y0 = 0.0;
    for (const originalPoints of [
      Sample.createStar(0, 0, 0, 3, 1, 40, false),
      Sample.createStar(0, 0, 0, 1, Math.sqrt(2), 4, false),
    ]) {
      const path0 = Path.create();
      let i0 = 0;
      for (const edgesInBlock of [2, 5, 1, 9]) {
        if (i0 + edgesInBlock >= originalPoints.length) break;
        if (edgesInBlock === 1) {
          path0.children.push(
            LineSegment3d.create(originalPoints[i0], originalPoints[i0 + 1])
          );
          i0 += 1;
        } else {
          const linestring = LineString3d.create();
  ...
  it("LinesAndArcs", () => {
    ...
    const points = [
      ...
      Point3d.create(2, 3, 1),
      Point3d.create(4, 3, 1),
  ...
  it("consolidateAdjacentFiles", () => {
    ...
    for (const filename of ["consolidateAdjacent00"]) {
      const stringData = fs.readFileSync(
        `${consolidateAdjacentPath}${filename}.imjs`,
        "utf8"
      );
      if (stringData) {
        ...
        if (Array.isArray(fragments)) {
          for (const g of fragments) {
            let g1 = g;
            if (g instanceof CurvePrimitive) g1 = Path.create(g);
            if (g1 instanceof CurveCollection) {
              ...
              ck.testCoordinate(
                lengthA,
                lengthB,
                "consolidateAdjacentPrimitives should not change length"
              );
              GeometryCoreTestIO.captureCloneGeometry(
                allGeometry,
                g1,
  ...
  it("Misc", () => {
    ...
    const loop1 = Loop.create(
      unitCircle.clonePartialCurve(0, 0.25)!,
      unitCircle.clonePartialCurve(0.25, 1.0)!
    );
    ...
    ck.testExactNumber(
      2,
      arcArcPath.children.length,
      " arcs with cusp are not consolidated"
    );

    const zero = Point3d.create(0, 0, 0);
    const singlePointPathA = Path.create(
      LineString3d.createPoints([zero, zero, zero])
    );
    RegionOps.consolidateAdjacentPrimitives(singlePointPathA);
    ck.testExactNumber(
      1,
      singlePointPathA.children.length,
      "Single point path consolidates to stub"
    );
    ...
    const singlePointPathB = Path.create(
      LineString3d.create(Point3d.create(1, 1, 2))
    );
    RegionOps.consolidateAdjacentPrimitives(singlePointPathB);
    ck.testExactNumber(
      1,
      singlePointPathB.children.length,
      "Single point path consolidates to stub"
    );
    expect(ck.getNumErrors()).equals(0);
  });
  it("ClosestPointInCollection", () => {
    ...
    for (const x of [-0.2, 0.3, 0.8, 1.1]) {
      for (const y of [-0.4, 0.45, 1.2]) localPoints.push(Point3d.create(x, y));
    }
...
function markLimits(
  ...
  for (const p of primitives) {
    GeometryCoreTestIO.createAndCaptureXYCircle(
      allGeometry,
      p.fractionToPoint(shiftFraction),
      markerSize0,
      x0,
      y0
    );
    GeometryCoreTestIO.createAndCaptureXYCircle(
      allGeometry,
      p.fractionToPoint(1.0 - shiftFraction),
      markerSize1,
      x0,
      y0
    );
