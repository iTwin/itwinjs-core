      ...
      for (const fA1 of fractionsOnA) {
        ...
        if (pair) {
          ...
          if (
            ...
            !ck.testCoordinate(
              rangeA01.length(),
              pair.detailA.point.distance(pair.detailA.point1),
              "length A",
              fA0,
              fA1
            )
            ...
            GeometryCoreTestIO.createAndCaptureXYMarker(
              allGeometry,
              0,
              pointA0.interpolate(2.0, pointA1),
              endMarkerSize,
              x0,
              y0
            );
          if (
            ...
            !ck.testCoordinate(
              rangeA01.length(),
              pair.detailB.point.distance(pair.detailB.point1),
              "length B",
              fA0,
              fA1
            )
            ...
            GeometryCoreTestIO.createAndCaptureXYMarker(
              allGeometry,
              0,
              pointB0.interpolate(2.0, pointB1),
              endMarkerSize,
              x0,
              y0 + dy1
            );
