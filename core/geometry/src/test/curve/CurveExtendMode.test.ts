...
describe("CurveExtendMode", () => {
  it("HelloWorld", () => {
    ...
    const qTangentNone: [CurveExtendMode, CurveExtendMode] = [
      CurveExtendMode.OnTangent,
      CurveExtendMode.None,
    ];
    ck.testExactNumber(
    ...
    const qNoneTangent: [CurveExtendMode, CurveExtendMode] = [
      CurveExtendMode.None,
      CurveExtendMode.OnTangent,
    ];
    ck.testExactNumber(
    ...
    for (const fraction of [-0.1, 0.1, 0.9, 1.1]) {
      ...
      const fractionCap = CurveExtendOptions.resolveRadiansToSweepFraction(
        CurveExtendMode.None,
        radians,
        sweep
      );
      const fraction0 = CurveExtendOptions.resolveRadiansToSweepFraction(
        extend0,
        radians,
        sweep
      );
      const fraction1 = CurveExtendOptions.resolveRadiansToSweepFraction(
        extend1,
        radians,
        sweep
      );
      const fractionBoth = CurveExtendOptions.resolveRadiansToSweepFraction(
        extendBoth,
        radians,
        sweep
      );
      ...
      if (fraction > 1.0) {
        ...
        ck.testCoordinate(
          fraction1,
          fraction0 + fractionPeriod,
          "resolveRadians with sign"
        );
      ...
      ck.testCoordinate(
        fraction,
        CurveExtendOptions.correctFraction(extendBoth, fraction),
        "unbounded"
      );
      ck.testCoordinate(
        fraction > 1 ? 1 : fraction,
        CurveExtendOptions.correctFraction(extend0, fraction),
        "extend0"
      );
      ck.testCoordinate(
        fraction < 0 ? 0 : fraction,
        CurveExtendOptions.correctFraction(extend1, fraction),
        "extend1"
      );
    }
