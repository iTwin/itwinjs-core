...
describe("AustralianRailCorpSpiral", () => {
  it("HelloWorld", () => {
    ...
    for (const nominalRadius1 of [400, 1000]) {
      ...
      const evaluator1 = AustralianRailCorpXYEvaluator.create(
        nominalRadius1,
        nominalLength1
      )!;
      for (const scaleFactor of [1.0, 2.0, 5.0]) {
        const evaluatorA = AustralianRailCorpXYEvaluator.create(
          nominalRadius1 * scaleFactor,
          nominalLength1 * scaleFactor
        )!;
        ...
        for (const fraction of [0, 0.2, 0.8, 1.0]) {
          ...
          ck.testTightNumber(
            x1 * scaleFactor,
            xA,
            " x from scaled construction"
          );
          ck.testTightNumber(
            y1 * scaleFactor,
            yA,
            " y from scaled construction"
          );
      ...
      const spiral = DirectSpiral3d.createAustralianRail(
      ...
      for (const fractionOfDistance of [0, 0.2, 0.8, 1.0]) {
        ...
        const xA =
          evaluator.distanceAlongSpiralToAustralianApproximateX(distanceA);
  ...
  it("CzechDistanceApproximation", () => {
    ...
    for (const nominalRadius1 of [400, 1000]) {
      const spiral = DirectSpiral3d.createCzechCubic(
        Transform.createIdentity(),
        nominalLength1,
        nominalRadius1
      )!;
      const evaluator = spiral.evaluator as CzechSpiralEvaluator;
      const gaussLength10 = Quadrature.doGaussIntegral(
        0,
        1,
        (f1: number) => evaluator.fractionToTangentMagnitude(f1),
        10
      );
      const gaussLength5 = Quadrature.doGaussIntegral(
        0,
        1,
        (f2: number) => evaluator.fractionToTangentMagnitude(f2),
        10
      );
      ...
      if (noisy)
        GeometryCoreTestIO.consoleLog({
          czechLengthA: czechLength,
          gaussLengthA: gaussLength10,
          fOverE: f / e,
        });
      const distanceRoundTripErrorRange = Range1d.createNull();
      ...
      if (noisy) GeometryCoreTestIO.consoleLog(distanceRoundTripErrorRange);
      ck.testLT(distanceRoundTripErrorRange.length(), 1e-12);
    }
    expect(ck.getNumErrors()).equals(0);
  });
  it("Coverage", () => {
    ...
    const spiral1 = DirectSpiral3d.createAustralianRail(
      Transform.createIdentity(),
      2 * r1,
      l1
    );
    const spiral2 = DirectSpiral3d.createAustralianRail(
      Transform.createIdentity(),
      r1,
      2 * l1
    );
    ...
    const phiA0 = AustralianRailCorpXYEvaluator.radiusAndAxisLengthToPhi(
      1,
      100
    );
    const phiA1 = AustralianRailCorpXYEvaluator.radiusAndAxisLengthToPhi(
      1,
      110
    );
    ck.testExactNumber(phiA0, phiA1, "limit case phi");
    const phiB0 = AustralianRailCorpXYEvaluator.radiusAndAxisLengthToPhi(
      -1,
      100
    );
    const phiB1 = AustralianRailCorpXYEvaluator.radiusAndAxisLengthToPhi(
      -1,
      110
    );
    ...
    const phiC0 = AustralianRailCorpXYEvaluator.radiusAndAxisLengthToPhi(
      200,
      10
    );
    const phiC1 = AustralianRailCorpXYEvaluator.radiusAndAxisLengthToPhi(
      200,
      12
    );
    const phiC2 = AustralianRailCorpXYEvaluator.radiusAndAxisLengthToPhi(
      200,
      14
    );
    ck.testTrue(
      (phiC2 - phiC1) * (phiC1 - phiC0) > 0,
      "normal phi variation is monotone"
    );
