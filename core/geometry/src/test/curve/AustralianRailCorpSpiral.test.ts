  ...
  it("HelloWorld", () => {
    ...
    for (const nominalRadius1 of [400, 1000]) {
      ...
      for (const tol1 of [1.0e-4, 1.0e-5, 1.0e-8, 1.0e-12]) {
        const axisLength1 = AustralianRailCorpXYEvaluator.radiusAndNominalLengthToAxisLength(
          nominalRadius1,
          nominalLength1,
          tol1,
          1
  ...
  it("CzechDistanceApproximation", () => {
    ...
    for (const nominalRadius1 of [400, 1000]) {
      ...
      for (const fractionOfX of [0, 0.2, 0.8, 1.0]) {
        ...
        if (ck.testIsFinite(xB, "czech invert")) distanceRoundTripErrorRange.extendX(xA - xB);
      }
