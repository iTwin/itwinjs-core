...
describe("UnionFind", () => {
  it("BinaryTree", () => {
    ...
    for (const numLeaf of [4, 2, 16, 1, 128, 5, 29, 77, 78, 79]) {
      ...
      if (noisy) GeometryCoreTestIO.consoleLog(`tree size ${numLeaf}`);
      ...
      } else context = new UnionFindContext(numLeaf);
      ...
      for (let blockSize = 1; blockSize < numLeaf; blockSize *= 2) {
        if (noisy) GeometryCoreTestIO.consoleLog(`blockSize ${blockSize}`);
      ...
      for (let i = 0; i < numLeaf; i++) context.findRoot(i);
