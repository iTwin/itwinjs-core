...
describe("AnalyticRoots.SolveQuadric", () => {
  ...
  it("RowElimination", () => {
    const ck = new Checker();
    for (const rowA of [new Float64Array([4, 3, 4, 5, 6])]) {
...
function maxDiffMatchedArrays(target: number[], actual: GrowableFloat64Array) {
  if (target.length !== actual.length) return Number.MAX_VALUE;
  const tempArray: number[] = [];
  for (let i = 0; i < actual.length; i++) {
    tempArray.push(actual.atUncheckedIndex(i));
  }
      ...
      for (
        ...
        if (
          ...
          if (printAll || eSafe >= (printTrigger * uMax * uMax) / e) {
            ...
            GeometryCoreTestIO.consoleLog(
              "Cubic root variances.  These may be expected behavior under extreme origin conditions"
            );
            GeometryCoreTestIO.consoleLog(
...
function checkQuartic(
  ...
  if (!ck.testPointer(actual, "?No roots for quartic") || !actual) return;
