 ...
 */
function makePointAdjustments(
  point: Point3d,
  adjustment: PointAdjustment
): Array<Point3d | undefined> {
...
function checkPointProximity(
  ...
  ck.testTrue(
    clipVector.pointInside(pointOnEdge),
    "Point on ClipShape edge is inside ClipVector"
  );
  if (pointInside)
    ck.testTrue(
      clipVector.pointInside(pointInside),
      "Point within ClipShape bounds is inside ClipVector"
    );
  if (pointOutside)
    ck.testTrue(
      !clipVector.pointInside(pointOutside),
...
function clipVectorsAreEqual(
  vector0: ClipVector,
  vector1: ClipVector
): boolean {
  if (vector0.clips.length !== vector1.clips.length) return false;
...
describe("ClipVector", () => {
   ...
   */
  before(() => {
    clipShape0 = ClipShape.createBlock(
      Range3d.createXYZXYZ(-5, -4, -50, -3, -2, 50),
      ClipMaskXYZRangePlanes.All
    );
    clipShape1 = ClipShape.createShape([
    ...
    clipVector012 = ClipVector.createCapture([
      clipShape0,
      clipShape1,
      clipShape2,
    ]);
  ...
  it("ClipVector creation and to/from JSON", () => {
    ...
    const newlyCreatedClipVector = ClipVector.createCapture([
      clipShape0,
      clipShape1,
      clipShape2,
      clipShape3,
    ...
    ck.testTrue(
      !clipVectorTester1.isValid,
      "Empty ClipVector should not be valid"
    );
    ClipVector.createCapture(clipVectorTester0.clips, clipVectorTester1);
    ck.testTrue(
      clipVectorTester1.isValid,
      "ClipVector should not be empty after copy"
    );
    ClipVector.create(clipVectorTester0.clips, clipVectorTester1);
    ck.testTrue(
      clipVectorTester1.isValid,
      "ClipVector should not be empty after clone"
    );
    ClipVector.createEmpty(clipVectorTester1);
    const arrLen = clipVector012.clips.length;
    for (let i = 0; i < arrLen; i++) {
      ck.testTrue(
        clipVector012.clips[i] !== clipVectorTester0.clips[i],
        "ClipVector created with clones should have deep copies of each ClipShape"
      );
      ck.testTrue(
        clipPrimitivesAreEqual(
          clipVector012.clips[i],
          clipVectorTester0.clips[i]
        ),
      ...
      ck.testTrue(
        clipVector012.clips[i] === clipVectorTester1.clips[0],
        "ClipShapes appended by reference should be shallow copies of each other"
      );
      clipVectorTester1.clear();
      clipVectorTester1.appendClone(clipVector012.clips[i]);
      ck.testTrue(
        clipVector012.clips[i] !== clipVectorTester1.clips[0],
        "ClipVector with appended clone should have deep copy of ClipShape"
      );
      ck.testTrue(
        clipPrimitivesAreEqual(
          clipVector012.clips[i],
          clipVectorTester1.clips[0]
        ),
    ...
    clipVectorTester1.appendShape(
      clipShape2.polygon,
      clipShape2.zLow,
      clipShape2.zHigh,
      undefined,
      clipShape2.isMask,
      clipShape2.invisible
    );
    ck.testTrue(
      clipPrimitivesAreEqual(clipShape2, clipVectorTester1.clips[0]),
      "ClipShape can be appended using points array."
    );

    // Test the to/from JSON methods
    const clipJSON = clipVector012.toJSON();
    ck.testTrue(
      clipJSON.length === clipVector012.clips.length,
      "Converted clipVector to a JSON representation as an array of ClipShapes"
    );
    for (const primitive of clipJSON) {
      const shape = primitive as ClipPrimitiveShapeProps;
      ck.testTrue(
        shape.shape !== undefined &&
          shape.shape.points !== undefined &&
    ...
    ck.testTrue(
      clipVectorsAreEqual(clipVector012, parsedClipVector),
      "ClipVector is the same after roundtrip to and from JSON"
    );
  ...
  it("Point proximity and classification", () => {
    ...
    const shape0PointAdj: PointAdjustment[] = [
    ...
    const shape1PointAdj: PointAdjustment[] = [
      PointAdjustment.SubY,
      PointAdjustment.AddX,
      PointAdjustment.AddY,
      PointAdjustment.None,
    ];
    const shape2Extremities: Point3d[] = [
      Point3d.create(7, 1, -0.15),
      Point3d.create(9, -1, -0.15),
      Point3d.create(10, -4, -0.15),
      Point3d.create(6, -4, -0.15),
      Point3d.create(7, -3, -0.2),
      Point3d.create(7, -3, -0.1),
    ];
    ...
    const shapeExtremities: Point3d[][] = [
      shape0Extremities,
      shape1Extremities,
      shape2Extremities,
    ];
    const shapePointAdjustments: PointAdjustment[][] = [
      shape0PointAdj,
      shape1PointAdj,
      shape2PointAdj,
    ];
    ...
    for (const arr of shapeExtremities) {
      ck.testTrue(
        clipVector012.isAnyLineStringPointInside(arr),
        "Segments connecting ClipShape boundaries should all fall inside ClipVector"
      );
      ck.testTrue(
        clipVector012.isLineStringCompletelyContained(arr),
        "Segments connecting ClipShape boundaries should all fall inside ClipVector"
      );
    }
    ...
    for (let i = 0; i < clipVector012.clips.length; i++) {
      clipVectorSingleShape = ClipVector.create(
        [clipVector012.clips[i]],
        clipVectorSingleShape
      );
      for (let j = 0; j < shapeExtremities[i].length; j++) {
        const pointOnEdge = shapeExtremities[i][j];
        const pointAdjustments = makePointAdjustments(
          pointOnEdge,
          shapePointAdjustments[i][j]
        );
        checkPointProximity(
          clipVectorSingleShape,
          pointOnEdge,
        ...
        if (pointAdjustments[0])
          ck.testExactNumber(
            clipVectorSingleShape.classifyPointContainment([
              pointAdjustments[0]!,
            ]),
            1,
    ...
    ck.testFalse(
      clipVector012.pointInside(Point3d.create(-4, -3)),
      "Point inside check should fail for non-intersecting ClipShapes"
    );
    ...
    ck.testTrue(
      intersectionClipVector.pointInside(Point3d.create(0, 0, 0)),
      "Origin point is inside ClipVector of intersecting triangles"
    );
    ck.testFalse(
      intersectionClipVector.pointInside(Point3d.create(0.00011)),
      "Point inside one of two triangles fails pointInside check"
    );
    ck.testFalse(
      intersectionClipVector.pointInside(Point3d.create(-0.00011)),
      "Point inside one of two triangles fails pointInside check"
  ...
  it("Transformations and matrix multiplication", () => {
    ...
    ck.testTrue(
      clipVectorsAreEqual(clipVectorClone, clipVector012),
      "Multiplying by identity matrix does not alter the ClipVector"
    );
    ck.testTrue(clipVectorClone.transformInPlace(t0));
    ck.testTrue(
      clipVectorsAreEqual(clipVectorClone, clipVector012),
      "Transforming with identity transform has no effect"
    );

    clipVectorClone.appendShape(
      [
  ...
  it("Extract boundary loops", () => {
    ...
    const expClipMask =
      ...
      (lastShape.zLowValid ? ClipMaskXYZRangePlanes.ZLow : 0) |
    ...
    ck.testExactNumber(
      expClipMask,
      retVal[0],
      "ClipMask returned matches expected"
    );
    ...
    for (let loopNum = 0; loopNum < loopPoints.length; loopNum++) {
      ck.testTrue(
        loopPoints[loopNum].length ===
          (clipVector012.clips[loopNum] as ClipShape).polygon.length,
      ...
      for (let pointNum = 0; pointNum < loopPoints[loopNum].length; pointNum++)
        ck.testTrue(
          loopPoints[loopNum][pointNum].isAlmostEqual(
            (clipVector012.clips[loopNum] as ClipShape).polygon[pointNum]
          ),
  ...
  it("converts to compact string representation", () => {
    ...
    const set2 = ConvexClipPlaneSet.createPlanes([
      ClipPlane.createNormalAndDistance(new Vector3d(1, 0, 0), 4, false, true)!,
    ]);
    cv = ClipVector.createCapture([
      primitive,
      ClipPrimitive.createCapture(set2, true),
    ]);
    expect(cv.toCompactString()).to.equal(
      "010_1_0_-5_30_0_-1_5e-11___121_0_0_4____"
    );
  });
});
...
describe("StringifiedClipVector", () => {
  it("creates from ClipVector", () => {
    ...
    expect(StringifiedClipVector.fromClipVector(ClipVector.createEmpty())).to.be
      .undefined;

    const cv = ClipVector.createCapture([
  ...
  it("OuterAndMask", () => {
    ...
    const innerCircle = Sample.createArcStrokes(
      2,
      Point3d.create(0, 0),
      5.0,
    ...
    for (const isMask of [true, false]) {
      ...
      const jsonTriangle = [
        { shape: { points: outer } },
        { shape: { points: triangle, mask: isMask } },
      ];
      const jsonOutsideCircle = [
        { shape: { points: innerCircle, mask: isMask } },
      ];
      const jsonCircle = [
        { shape: { points: outer } },
        { shape: { points: innerCircle, mask: isMask } },
      ...
      const jsonC = [
        { shape: { points: outer } },
        { shape: { points: innerNonConvex, mask: isMask } },
      ];
      const jsonD = [
        { shape: { points: outer } },
        { shape: { points: innerU, mask: isMask } },
      ];
      const jsonDart = [
        { shape: { points: outer } },
        { shape: { points: innerDart, mask: isMask } },
      ];
      const jsonDart1 = [
        { shape: { points: outer } },
        { shape: { points: innerDart1, mask: isMask } },
      ];
      const jsonTwoInlets = [
        { shape: { points: outer } },
        { shape: { points: twoInletsWithWhisker, mask: isMask } },
      ];
      const jsonTwoInletsReversed = [
        { shape: { points: outer } },
        {
          shape: {
            points: twoInletsWithWhisker.slice().reverse(),
            mask: isMask,
          },
        },
      ];
      const jsonDart1Trans = [
        { shape: { points: outer } },
        {
          shape: { points: innerDart, mask: isMask, trans: shiftAndRotateJson },
        },
      ];
      const jsonTriangleTrans = [
        { shape: { points: outer } },
        {
          shape: { points: triangle, mask: isMask, trans: shiftAndRotateJson },
        },
      ];
  ...
  it("OuterAndMaskLargeCoordinateAndTransform", () => {
    ...
    GeometryCoreTestIO.saveGeometry(
      allGeometry,
      "ClipVector",
      "OuterAndMaskLargeCoordinateAndTransform"
    );
    expect(ck.getNumErrors()).equals(0);
  });
  it("OuterAndMask", () => {
  ...
  it("ClipperInterfaces", () => {
    ...
    ck.testLT(
      2 * b,
      a,
      "Confirm diamonds will fit well within outer rectangle -- needed for test conditions."
    );
    const outerPoints = Sample.createCenteredRectangleXY(0, 0, 2 * a, a);
    const outerRange = Range3d.createArray(outerPoints);
    const leftHolePoints = Sample.createArcStrokes(
      1,
    ...
    const rightHolePoints = Sample.createArcStrokes(
      1,
      Point3d.create(a, 0, 0),
      b,
      Angle.createDegrees(0),
    ...
    const outerPrimitive = ClipShape.createShape(
      outerPoints,
      undefined,
      undefined,
      undefined,
    ...
    const leftHolePrimitive = ClipShape.createShape(
      leftHolePoints,
      undefined,
      undefined,
      undefined,
    ...
    const rightHolePrimitive = ClipShape.createShape(
      rightHolePoints,
      undefined,
      undefined,
      undefined,
    ...
    const clipVector = ClipVector.createCapture([
      outerPrimitive,
      leftHolePrimitive,
      rightHolePrimitive,
    ]);
    ...
    const inPoints = [
      Point3d.create(0, 0),
      leftRange.fractionToPoint(e0, e0, 0),
      rightRange.fractionToPoint(e1, e1, 0),
    ];
    const outPoints = [
      Point3d.create(-a, 0),
      Point3d.create(a, 0),
    ...
    for (const point of inPoints)
      ck.testTrue(
        clipVector.isPointOnOrInside(point),
        prettyPrint({ expectIn: point })
      );
    for (const point of outPoints)
      ck.testFalse(
        clipVector.isPointOnOrInside(point),
        prettyPrint({ expectOut: point })
      );
    ...
    for (const testCase of [
      ...
      ClipUtilities.announcePolylineClip(
        clipVector,
        [pointA, pointB, pointC, pointA],
      ...
      const clippedLength = ClipUtilities.sumPolylineClipLength(clipVector, [
        pointA,
        pointC,
      ]);
      const expectedClipFraction = testCase.isHole
        ? 1.0 - fractionInRange
        : fractionInRange;
      ck.testCoordinate(
        clippedLength,
        expectedClipFraction * lengthAC,
        "Length of clippedStroke"
      );
    }
    /*
...
function exerciseClipVector(
  ...
  if (ck.testType(cv, ClipVector, "parsed clip vector")) {
    ...
    cv.appendPolygonClip(
      GrowableXYZArray.create(polygonToClip),
      insideClip,
      outsideClip,
      cache
    );
    ...
    for (let ii = 1; ii < testDensity; ii++) {
      for (let jj = 1; jj < testDensity; jj++) {
        const xyz = range.fractionToPoint(
          ii / testDensity,
          jj / testDensity,
          0
        );
...
export function exerciseClipPrimitive(
  ...
  if (primitive instanceof ClipShape) {
    if (primitive.transformIsValid) {
      const transformedPolygon =
        primitive.transformFromClip!.multiplyPoint3dArray(primitive.polygon);
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        transformedPolygon,
        x0,
  ...
  if (planeSets) {
      ...
      if (range1.containsRange(range2)) {
        ...
        if (primitive.isMask)
          ck.testCoordinate(
            area0,
            area2 - area1,
            "Expect mask to remove clip shape from containing polygon"
          );
        else
          ck.testCoordinate(
            area0,
            area1,
            "Expect clipper to excise clip shape from containing polygon"
          );
      }
    }
    ...
    for (const c of compositeClip) {
      ...
      GeometryCoreTestIO.captureCloneGeometry(
        allGeometry,
        Loop.create(LineString3d.create(c)),
        x0,
        y0 + 2 * range2.yLength()
      );
    }
