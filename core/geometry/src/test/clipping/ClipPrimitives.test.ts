...
function convexSetsAreEqual(
  convexSet0: ConvexClipPlaneSet,
  convexSet1: ConvexClipPlaneSet
): boolean {
  if (convexSet0.planes.length !== convexSet1.planes.length) return false;
...
function clipPlaneSetsAreEqual(
  set0: UnionOfConvexClipPlaneSets | undefined,
  set1: UnionOfConvexClipPlaneSets | undefined
): boolean {
  // both undefined is equal . ..
  if (set0 === undefined && set1 === undefined) return true;
  if (set0 === undefined || set1 === undefined) return false;
  if (set0.convexSets.length !== set1.convexSets.length) return false;
...
export function clipShapesAreEqual(
  clip0: ClipShape,
  clip1: ClipShape
): boolean {
  if (
    !clipPlaneSetsAreEqual(
      clip0.fetchClipPlanesRef(),
      clip1.fetchClipPlanesRef()
    )
  )
    return false;
  if (clip0.invisible !== clip1.invisible) return false;
  for (
    let i = 0;
    i < clip0.polygon.length;
    i++ // Polygon points should be in the same order
  )
    if (!clip0.polygon[i].isAlmostEqual(clip1.polygon[i])) return false;
  if (clip0.isMask !== clip1.isMask) return false;
  if (
    clip0.zLowValid !== clip1.zLowValid ||
    clip0.zHighValid !== clip1.zHighValid ||
    clip0.transformValid !== clip1.transformValid
  )
    return false;
  if (clip0.zLow !== clip1.zLow || clip0.zHigh !== clip1.zHigh) return false;
  if (
...
export function clipPrimitivesAreEqual(
  clip0: ClipPrimitive,
  clip1: ClipPrimitive
): boolean {
...
function compareXYSector1(a: ClipPlane, b: ClipPlane): number {
  if (a.inwardNormalRef.x < b.inwardNormalRef.x) return -1;
  else if (a.inwardNormalRef.x > b.inwardNormalRef.x) return 1;
  else if (a.inwardNormalRef.y < a.inwardNormalRef.y) return -1;
  else if (a.inwardNormalRef.y > b.inwardNormalRef.y) return 1;
  return 0;
}
...
function sortConvexSetPlanesToBoundary(convexSet: ConvexClipPlaneSet) {
  ...
  if (planeFound !== undefined) newPlanes.push(planeFound);
  ...
  for (const plane of tempSortedPlanes) newPlanes.push(plane);
  ...
  for (const plane of newPlanes) convexSet.planes.push(plane);
}

/** Given a convex set of clipping planes, return the points at which the planes intersect. */
function getPointIntersectionsOfConvexSetPlanes(
  ...
  for (let i = 0; i < convexSet.planes.length - 1; i++) {
    ...
    const planeDistanceVec = Vector3d.create(
      planeA0.distance,
      planeA1.distance,
      0
    );
    intersections.push(
      Point3d.createFrom(planeMatrix.multiplyVector(planeDistanceVec))
    );
  }
  ...
  const plane01DistanceVec = Vector3d.create(
    plane0.distance,
    plane1.distance,
    0
  );
  intersections.push(
    Point3d.createFrom(plane01Matrix.multiplyVector(plane01DistanceVec))
  );
...
function pointArrayIsSubsetOfOther(
  arrayA: Point3d[],
  arrayB: Point3d[]
): boolean {
...
describe("ClipPrimitive", () => {
  ...
  it("GetRange", () => {
    ...
    for (let i = 0; i < numIterations; i++) {
      ...
      ClipShape.createBlock(
        Range3d.createXYZXYZ(p, p, p, p + 1, p + 1, p + 1),
      ...
      convexSet.addZClipPlanes(false, p, p + 1);
      clipPrimitive.setPolygon([
        Point3d.create(-p - 1, -p - 1),
        Point3d.create(-p - 1, -p),
        Point3d.create(-p, -p),
        Point3d.create(-p, -p - 1),
      ]);
      convexSetRange.setNull();
      convexSet.computePlanePlanePlaneIntersections(undefined, convexSetRange);
      ck.testTrue(convexSetRange !== undefined);
    ...
    ck.testTrue(
      clipPrimitive.containsZClip(),
      "Expected clip primitive to contain a normal along the z-axis"
    );
    ...
    ck.testFalse(
      clipPrimitive.containsZClip(),
      "Expected clip primitive of box with open top and bottom to not contain z-clip"
    );
  ...
  it("Transformations", () => {
    ...
    let clipShape = ClipShape.createEmpty(
      false,
      false,
    ...
    ck.testPoint3d(
      originalPoint,
      testPoint,
      "Point should be unchanged when transformed with ClipShape containing identity transform"
    );
    ...
    clipShape = ClipShape.createEmpty(
      false,
      false,
  ...
  it("ClipShape creation (linear) and point classification", () => {
    ...
    ck.testPointer(
      clipShape,
      "Can create ClipShape that will be parsed into a linear set of planes"
    );
    clipShape.fetchClipPlanesRef();
    ck.testExactNumber(
      clipShape.classifyPointContainment(
        [Point3d.create(-5.00001, 0, 0)],
        true
      ),
      3,
    ...
    ck.testExactNumber(
      clipShape.classifyPointContainment(
        [Point3d.create(0, 3, 0), Point3d.create(2, -5, 0)],
        true
      ),
      2,
    ...
    ck.testExactNumber(
      clipShape.classifyPointContainment(
        [Point3d.create(0, -0.00001, 0), Point3d.create(0, 0.00001, 0)],
        true
      ),
      2,
      "Points cross line and within sides - ambiguous"
    ...
    ck.testExactNumber(
      clipShape.classifyPointContainment(
        [Point3d.create(4.999, 0, 2.999), Point3d.create(0, 0, 0)],
        true
      ),
      1,
  ...
  it("ClipShapePointTests", () => {
    ...
    let x0 = 0;
    const y0 = 0;
    const circlePoints = Sample.createArcStrokes(
      0,
      Point3d.create(1, 2),
      2.0,
    ...
    ck.testFalse(
      clipShape0.isXYPolygon,
      "ClipShape does not contain polygon when no points are present"
    );
    ck.testTrue(
      ClipShape.createShape(
        circlePoints,
    ...
    ck.testFalse(
      clipShape0.pointInside(centroid),
      "Centroid of polygon is not inside due to mask."
    );
    const containment = clipShape0.classifyPointContainment([centroid], true);
    ck.testExactNumber(
      containment,
      3,
      "centroid is completely outside when ClipShape is a mask"
    );
    let clipShape1 = ClipShape.createShape(
      circlePoints,
      minZ,
      maxZ,
      undefined,
    ...
    ck.testTrue(
      clipShape1!.pointInside(centroid, 0),
      "Midpoint of polygon is inside."
    );

    // Test createFrom method
    clipShape1 = ClipShape.createFrom(clipShape0, clipShape1);
    ck.testTrue(
      clipShapesAreEqual(clipShape0, clipShape1),
      "createFrom() method should clone the ClipShape"
    );
    ...
    ck.testTrue(
      shape.points !== undefined &&
        shape.points.length === clipShape1.polygon.length,
      "Points prop created in toJSON"
    );
    ck.testUndefined(
      shape.trans,
      "Transform is undefined prop in toJSON having not given one to original ClipShape"
    );
    ck.testTrue(
      shape.mask !== undefined && shape.mask === true,
      "Mask prop created in toJSON"
    );
    if (minZ === undefined) ck.testTrue(shape.zlow === undefined);
    else
      ck.testTrue(
        shape.zlow !== undefined && shape.zlow === clipShape1.zLow,
        "ZLow prop created in toJSON"
      );

    if (maxZ === undefined) ck.testTrue(shape.zhigh === undefined);
    else
      ck.testTrue(
        shape.zhigh !== undefined && shape.zhigh === clipShape1.zHigh,
        "ZHigh prop is set in toJSON"
      );

    const clipShape1Copy = ClipShape.fromJSON(jsonValue) as ClipShape;
    ck.testTrue(clipShape1Copy !== undefined);
    ck.testTrue(
      clipShapesAreEqual(clipShape1, clipShape1Copy),
      "to and from JSON yields same ClipPrimitive"
    );

    // Test clone method
    const clipShape2 = clipShape1Copy.clone();
    ck.testTrue(
      clipShapesAreEqual(clipShape2, clipShape1),
      "clone method produces a copy of ClipShape"
    );
  ...
  it("ClipShape plane parsing for simple concave polygon", () => {
    ...
    const areaRestriction =
      (originalRange.high.x - originalRange.low.x) *
      (originalRange.high.y - originalRange.low.y);
    ...
    if (ck.testPointer(convexSetUnion)) {
      ...
      for (const convexSet of convexSetUnion.convexSets) {
        const trianglePoints = getPointIntersectionsOfConvexSetPlanes(
          convexSet,
          ck
        );
      ...
      ck.testRange3d(
        originalRange,
        unionRange,
        "Range extended by all convex regions should match range of entire concave region."
      );
      ck.testTrue(
        unionArea <= areaRestriction,
        "Total area of union of convex triangles should be less than or equal to area of entire range."
      );
    }
    ck.checkpoint();
    expect(ck.getNumErrors()).equals(0);
  ...
  it("ClipShape with simple concave area clips simple polygon", () => {
    ...
    const clippedPolygons =
      ClipUtilities.clipPolygonToClipShapeReturnGrowableXYZArrays(
        polygonA,
        clipShape
      );
    ...
    for (const polygon of clippedPolygons) {
      if (polygon.length <= 3) {
        if (polygon.length === 3)
          clippedPolygonArea += Math.abs(
            polygon.crossProductIndexIndexIndex(0, 1, 2)!.z * 0.5
          );
        continue;
      }

      const polygonGraph =
        Triangulator.createTriangulatedGraphFromSingleLoop(polygon);
      if (ck.testDefined(polygonGraph) && polygonGraph) {
        Triangulator.flipTriangles(polygonGraph);

        polygonGraph.announceFaceLoops(
          (_graph: HalfEdgeGraph, edge: HalfEdge): boolean => {
            if (!edge.getMask(HalfEdgeMask.EXTERIOR)) {
              ...
              ck.testExactNumber(
                3,
                subTrianglePoints.length,
                "Length clipped polygon piece after further triangulation must be 3"
              );
              clippedPolygonArea += triangleAreaXY(
                subTrianglePoints[0],
                subTrianglePoints[1],
                subTrianglePoints[2]
              );
  ...
  it("ClipShape with complex concave area clips polygon", () => {
    ...
    clipShape.fetchClipPlanesRef();
    const clippedPolygons = ClipUtilities.clipPolygonToClipShape(
      polygonB,
      clipShape
    );
  ...
  it("ClipPrimitive base class", () => {
    const ck = new Checker();
    for (const invert of [false]) {
      // EDL sept 2021 invert bit on simple plane set has no effect.  Don't test with true.
  ...
  it("NonConvexClipShapeClipPolygon", () => {
    ...
    const da = 10;
    const db = 110;
    const clipShapeOut = ClipShape.createShape(
      clipPointsA,
      undefined,
      undefined,
      undefined,
    ...
    const clipShapeIn = ClipShape.createShape(
      clipPointsA,
      undefined,
      undefined,
      undefined,
    ...
    for (const polygon of polygons) {
      ...
      const clippedPolygonsIn = ClipUtilities.clipPolygonToClipShape(
        polygon,
        clipShapeIn
      );
      const clippedPolygonsOut = ClipUtilities.clipPolygonToClipShape(
        polygon,
        clipShapeOut
      );
  ...
  it("jsonFragment", () => {
    ...
    const json = [
      {
        planes: {
          clips: [
            [
              {
                dist: -0.09250245365197406,
                normal: [0, 6.123233995736765e-17, 0.9999999999999999],
              },
              {
                dist: -4.284169242532198,
                normal: [0, -6.123233995736765e-17, -0.9999999999999999],
              },
              {
                dist: -0.09250245365197413,
                normal: [0.9999999999999999, 0, 0],
              },
              { dist: -4.620474647250288, normal: [-0.9999999999999999, 0, 0] },
              {
                dist: -6.984123210872675,
                normal: [0, -0.9999999999999999, 6.123233995736765e-17],
              },
              {
                dist: -0.09250245365197496,
                normal: [0, 0.9999999999999999, -6.123233995736765e-17],
              },
            ],
          ],
    ...
    if (
      ck.testPointer(clipper, "ClipVector.fromJSON for test fragment") &&
      clipper
    ) {
      ...
      for (const x of [0, 4]) {
        for (const y of [0, 6]) {
          for (const z of [0, 4]) {
            ...
            ck.testFalse(
              clipper.pointInside(Point3d.create(x + q, y, z)),
              x,
              y,
              z
            );
            ...
            ck.testFalse(
              clipper.pointInside(Point3d.create(x, y + q, z)),
              x,
              y,
              z
            );
            ...
            ck.testFalse(
              clipper.pointInside(Point3d.create(x, y, z + q)),
              x,
              y,
              z
            );
        ...
        for (const cs of convexSets.convexSets) {
          ...
          if (Checker.noisy.convexSetCorners) {
            GeometryCoreTestIO.consoleLog(
              ` Convex Set range ${prettyPrint(r.toJSON())}`
            );
            for (const xyz of points) {
  ...
  it("ClipVectorWithHole", () => {
    ...
    const convexClip = ConvexClipPlaneSet.createRange3dPlanes(
      outerRange,
      true,
      true,
      true,
    ...
    const outerClip = ClipShape.createBlock(
      outerRange,
    ...
    const holeClip = ClipShape.createBlock(
      holeRange,
      ClipMaskXYZRangePlanes.XAndY,
      true
    );
    ...
    const bigQ = 1000.0;
    const bigRange = Range3d.createXYZXYZ(
      -bigQ,
      -bigQ,
      -bigQ,
      bigQ,
    ...
    for (const range of [bigRange, rangeB]) {
      const rangeOfUndefinedClipper =
        ClipUtilities.rangeOfClipperIntersectionWithRange(undefined, range);
      ck.testRange3d(rangeOfUndefinedClipper, range, "undefined clipper");
      ck.testBoolean(
        ClipUtilities.doesClipperIntersectRange(undefined, range),
        !rangeOfUndefinedClipper.isNull
      );

      const outerClipRange = ClipUtilities.rangeOfClipperIntersectionWithRange(
        outerClip,
        range
      );
      ck.testBoolean(
        ClipUtilities.doesClipperIntersectRange(outerClip, range),
        !outerClipRange.isNull,
        "outer clipper"
      );

      const holeClipRange = ClipUtilities.rangeOfClipperIntersectionWithRange(
        holeClip,
        range
      );
      ck.testBoolean(
        ClipUtilities.doesClipperIntersectRange(holeClip, range),
        !holeClipRange.isNull,
        "hole clipper"
      );

      const clippedRange = ClipUtilities.rangeOfClipperIntersectionWithRange(
        clipVector1,
        range
      );
      ck.testBoolean(
        ClipUtilities.doesClipperIntersectRange(clipVector1, range),
        !clippedRange.isNull,
        "ClipVector clipper"
      );

      const convexClipRange = ClipUtilities.rangeOfClipperIntersectionWithRange(
        convexClip,
        range
      );
      ck.testBoolean(
        ClipUtilities.doesClipperIntersectRange(convexClip, range),
        !convexClipRange.isNull,
        "convex clipper"
      );
  ...
  it("ClipPrimitiveMasking", () => {
    ...
    const uncappedBoxSetA = ConvexClipPlaneSet.createRange3dPlanes(
      clipRange,
      true,
      true,
      true,
...
function clipAndOutput(
  ...
  if (unionOfConvexSets) {
    const builders = ClippedPolyfaceBuilders.create(
      true,
      true,
      buildClosureFaces
    );
    ...
    GeometryCoreTestIO.captureCloneGeometry(
      allGeometry,
      builders.builderA?.claimPolyface(),
      x0,
      y0
    );
    GeometryCoreTestIO.captureRangeEdges(allGeometry, clipRange, x0, y0);
    ...
    GeometryCoreTestIO.captureCloneGeometry(
      allGeometry,
      builders.builderB?.claimPolyface(),
      x0,
      y0
    );
    GeometryCoreTestIO.captureRangeEdges(allGeometry, clipRange, x0, y0);
