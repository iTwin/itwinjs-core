...
export function clipShapesAreEqual(clip0: ClipShape, clip1: ClipShape): boolean {
  if (!clipPlaneSetsAreEqual(clip0.fetchClipPlanesRef(), clip1.fetchClipPlanesRef())) return false;
...
export function clipPrimitivesAreEqual(clip0: ClipPrimitive, clip1: ClipPrimitive): boolean {
  if (clip0 instanceof ClipShape && clip1 instanceof ClipShape) return clipShapesAreEqual(clip0, clip1);
...
function sortConvexSetPlanesToBoundary(convexSet: ConvexClipPlaneSet) {
  ...
  for (const plane of planes) {
    if (plane.inwardNormalRef.x > 0 && plane.inwardNormalRef.y < 0) tempSortedPlanes.push(plane);
  }
...
describe("ClipPrimitive", () => {
  ...
  it("ClipShapePointTests", () => {
    ...
    ck.testTrue(
      shape.points !== undefined && shape.points.length === clipShape1.polygon.length,
    ...
    else ck.testTrue(shape.zlow !== undefined && shape.zlow === clipShape1.zLow, "ZLow prop created in toJSON");

    if (maxZ === undefined) ck.testTrue(shape.zhigh === undefined);
    else ck.testTrue(shape.zhigh !== undefined && shape.zhigh === clipShape1.zHigh, "ZHigh prop is set in toJSON");
  ...
  it("ClipShape with simple concave area clips simple polygon", () => {
    ...
    for (const polygon of clippedPolygons) {
      if (polygon.length <= 3) {
        if (polygon.length === 3) clippedPolygonArea += Math.abs(polygon.crossProductIndexIndexIndex(0, 1, 2)!.z * 0.5);
        continue;
      }
