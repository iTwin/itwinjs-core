...
describe("ClipNodes", () => {
  it("ClipManyBooleans", () => {
    ...
    for (const clipper of allClippers) {
      ...
      if (clipper1.toJSON) {
        ...
        ck.announceError({
          "clipper does not have toJSON": clipperName.get(clipper),
        });
      }
    }
    const arcs = [];
    ...
    for (const sweepDegrees of [40, 65, 170, 360]) {
      for (
        ...
        const arc3 = Arc3d.createXY(
          sharedOrigin,
          radius + radiusStep * 0.75,
          AngleSweep.createStartSweepDegrees(startDegrees - 360, sweepDegrees)
        );
        arcs.push(arc3);
  ...
  it("ClipManyBoxes", () => {
    ...
    const clipperA = createTranslatedClipperUnion(
      allGeometry,
      8,
      unitBoxPoints,
      Point3d.create(0, 0, 0),
      Point3d.create(20, 8, 0),
      1.0,
      7.0,
      x0,
      y0
...
function createTranslatedClipper(
  origin: Point3d,
