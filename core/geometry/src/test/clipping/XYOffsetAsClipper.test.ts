...
function captureClippedPolygon(
  allGeometry: GeometryQuery[],
  points: Point3d[],
  clipper: UnionOfConvexClipPlaneSets,
  x0: number,
  y0: number
) {
...
describe("OffsetByClip", () => {
  it("LongLineString", () => {
    ...
    const pointsB = [];
    pointsB.push(
      Point3d.create(-53.94490516785141, 46.9407384215234, -3.8955028593080985)
    );
    pointsB.push(
      Point3d.create(-53.94490516785141, 6.495277504543655, -3.8955028593080985)
    );
    pointsB.push(
      Point3d.create(
        -53.94490516785141,
    ...
    pointsB.push(
      Point3d.create(7.83254294135434, -37.759261578476604, -3.8955028593080985)
    );
    pointsB.push(
      Point3d.create(75.4280760324446, -37.759261578476604, -3.8955028593080985)
    );
    pointsB.push(
      Point3d.create(75.4280760324446, 2.6861993385031404, -3.8955028593080985)
    );
    pointsB.push(
      Point3d.create(75.4280760324446, 46.9407384215234, -3.8955028593080985)
    );
    pointsB.push(
      Point3d.create(13.650627923238844, 46.9407384215234, -3.8955028593080985)
    );
    pointsB.push(
      Point3d.create(-53.94490516785141, 46.9407384215234, -3.8955028593080985)
    );
    ...
    for (const points of [pointsC, pointsB, pointsA]) {
      let x0 = 0.0;
      for (const leftSign of [1, 0.25, 0, -0.5, -2.0]) {
        let y0 = y00;
  ...
  it("LongLineStringB", () => {
    ...
    const zClipper = ClipUtilities.createXYOffsetClipFromLineString(
      [],
      1,
      1,
      -1,
  ...
  it("DiegoProblemCases", () => {
    ...
    const shape1 = IModelJson.Reader.parse(
      JSON.parse(
        fs.readFileSync(
          "./src/test/testInputs/clipping/DiegoTrickyBuilding/case1.imjs",
          "utf8"
        )
      )
    ) as CurveCollection;
    const shape2 = IModelJson.Reader.parse(
      JSON.parse(
        fs.readFileSync(
          "./src/test/testInputs/clipping/DiegoTrickyBuilding/case2.imjs",
          "utf8"
        )
      )
    ) as CurveCollection;
  ...
  it("ArnoldasLaneClip", () => {
    const ck = new Checker();
    const allGeometry: GeometryQuery[] = [];
    const fullRoadMesh = IModelJson.Reader.parse(
      JSON.parse(
        fs.readFileSync(
          "./src/test/testInputs/clipping/arnoldasLaneClipper/fullRoadMesh.imjs",
          "utf8"
        )
      )
    );
    ...
    if (fullRoadMesh instanceof IndexedPolyface) {
      ...
      fullRoadMesh.tryTranslateInPlace(
        -meshRangeA.low.x,
        -meshRangeA.low.y,
        -meshRangeA.low.z
      );
      const meshRange = fullRoadMesh.range();
      const rangeRectangle = Sample.createRectangleXY(
        meshRange.low.x,
        meshRange.low.y,
        meshRange.xLength(),
        meshRange.yLength(),
      ...
      const boundary = PolyfaceQuery.boundaryOfVisibleSubset(
        fullRoadMesh,
        0,
        Vector3d.unitZ(),
        Angle.createDegrees(2.0)
      );
      if (boundary) {
        GeometryCoreTestIO.captureCloneGeometry(
          allGeometry,
        ...
        if (chainsA instanceof Path) {
            ...
            if (primitive instanceof LineString3d)
              filterSegments(
                ...
                (pointA: Point3d, pointB: Point3d) => {
                  return (
                    pointB.x - pointA.x >= 0.0 && pointB.y - pointA.y >= 0.0
                  );
                },
          ...
          const offsetA = 3.0;
          for (const offsetB of [0.0, -1.0]) {
            // we expect only one right side chain ..
            for (const ls of rightSideChains) {
              const clipper = ClipUtilities.createXYOffsetClipFromLineString(
                ls.packedPoints,
                offsetA,
                offsetB,
                meshRange.low.z - 1,
              ...
              if (
                ck.testDefined(clipA) &&
                clipA &&
                ck.testDefined(clipB) &&
                clipB
              ) {
  ...
  it("ExcessEdgesAroundAnnulus", () => {
    ...
    for (const annulusA of [
      Sample.createAnnulusPolyline(
        4,
        Point3d.create(4, 1.1, 0),
        2,
        2.6,
        Angle.createDegrees(100),
        Angle.createDegrees(170),
        true
      ),
      Sample.createAnnulusPolyline(
        4,
        Point3d.create(6, 6, 0),
        2,
        2.6,
        Angle.createDegrees(-80),
      ...
      const grid0 = Sample.createTriangularUnitGridPolyface(
      ...
      const grid0A = Sample.createTriangularUnitGridPolyface(
      ...
      const grid1 = Sample.createTriangularUnitGridPolyface(
        Point3d.create(0, 0, 0),
        Vector3d.create(1.0, 0, 0),
        Vector3d.create(0, 2.0, 0),
        11,
      ...
      const grid2 = Sample.createTriangularUnitGridPolyface(
        Point3d.create(1, 2, 0),
        Vector3d.create(1.0, 0, 0),
        Vector3d.create(0, 2.0, 0),
        4,
      ...
      const grid3 = Sample.createTriangularUnitGridPolyface(
        Point3d.create(7, 4, 0),
        Vector3d.create(1.0, 0, 0),
        Vector3d.create(0, 2.0, 0),
        3,
        2,
  ...
  it("NonXYClip", () => {
    ...
    let x0 = 0;
    let y0 = 0;
    const clipRectangle = SweepContour.createForPolygon(
      Sample.createRectangleXY(1, 1, 2, 3, 0)
    );
    const rectangleClipper =
      clipRectangle!.sweepToUnionOfConvexClipPlaneSets()!;
  ...
  it("InwardCornerClip", () => {
    ...
    const angles = [
      ...
      Angle.createDegrees(-120),
    ];
    // Make "not quite filleted" line-arc-line paths with quirky shapes
    for (const shiftRadius of [0.0, 0.1, -0.1, -0.3, 1.0]) {
      for (const angle of angles)
        paths.push(
          quirkyArcPath(shiftRadius * angle.cos(), shiftRadius * angle.sin())
        );
    }
    paths.push(quirkyArcPath(0, 0, 0, 1));
    paths.push(quirkyArcPath(0, 0, 0, -1));

    for (const radius0 of [1.0, 2.0]) {
      // nominally possible to have undefined return -- but it will get tossed later . ..
    ...
    for (const path of paths) {
      ...
      if (path) {
        ...
        for (const offsetDistance of [2.0, 3.0, 0.9, 1.0, 1.1]) {
          // 7.0 makes bow ties
  ...
  it("IncompletePaste", () => {
    ...
    const polygonsToPaste: Point3d[][] = [
      [
        ...
        Point3d.create(22.0, 22.37389822604068, 0.0),
        Point3d.create(21.693771833736626, 22.300546394411093, 0.0),
      ],
      [
        Point3d.create(21.693771833736626, 22.300546394411093, 0.0),
      ...
      [
        Point3d.create(22.16152236891498, 22.938477631085025, 0.0),
        Point3d.create(21.693771833736626, 22.300546394411093, 0.0),
        Point3d.create(21.693771833736626, 22.300546394411093, 0.0),
        Point3d.create(22.0, 22.718188915620175, 0.0),
        Point3d.create(22.16152236891498, 22.938477631085025, 0.0),
    ...
    for (const candidates of [
      ...
      const insidePieces = RegionOps.polygonBooleanXYToLoops(
        candidates,
  ...
  it("IncompletePasteWithDoublePoint", () => {
    ...
    GeometryCoreTestIO.saveGeometry(
      allGeometry,
      "OffsetByClip",
      "IncompletePasteWithDoublePoint"
    );
    expect(ck.getNumErrors()).equals(0);
  });
});
...
function filterSegments(
  ...
  linestringAcceptFunction: (ls: LineString3d) => void
): void {
  ...
  if (currentLineString) linestringAcceptFunction(currentLineString);
}
...
function quirkyArcPath(
  ...
  if (arc)
    return Path.create(
      LineSegment3d.create(pointA, point0),
      arc,
      LineSegment3d.create(point2, pointB)
    );
  return undefined;
}
