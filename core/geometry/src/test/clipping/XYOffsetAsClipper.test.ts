...
describe("OffsetByClip", () => {
  ...
  it("LongLineStringB", () => {
    ...
    for (const points of [pointsC, pointsA, pointsB]) {
      ...
      for (const leftOffset of [10]) {
        ...
        if (ck.testType(offsetClipper, UnionOfConvexClipPlaneSets)) {
          for (const c of offsetClipper.convexSets) {
            ClipUtilities.announceLoopsOfConvexClipPlaneSetIntersectRange(c, range, (loopPoints: GrowableXYZArray) => {
              GeometryCoreTestIO.createAndCaptureLoop(allGeometry, loopPoints, x0, y0);
            });
          }
          y0 += 30.0;
  ...
  it("DiegoProblemCases", () => {
    ...
    const shape1 = IModelJson.Reader.parse(
      JSON.parse(fs.readFileSync("./src/test/testInputs/clipping/DiegoTrickyBuilding/case1.imjs", "utf8"))
    ) as CurveCollection;
    const shape2 = IModelJson.Reader.parse(
      JSON.parse(fs.readFileSync("./src/test/testInputs/clipping/DiegoTrickyBuilding/case2.imjs", "utf8"))
    ) as CurveCollection;
  ...
  it("ArnoldasLaneClip", () => {
    ...
    const fullRoadMesh = IModelJson.Reader.parse(
      JSON.parse(fs.readFileSync("./src/test/testInputs/clipping/arnoldasLaneClipper/fullRoadMesh.imjs", "utf8"))
    );
        ...
        if (chainsA instanceof Path) {
            ...
            if (primitive instanceof LineString3d)
              filterSegments(
                ...
                (pointA: Point3d, pointB: Point3d) => {
                  return pointB.x - pointA.x >= 0.0 && pointB.y - pointA.y >= 0.0;
                },
          ...
          for (const offsetB of [0.0, -1.0]) {
            ...
            for (const ls of rightSideChains) {
              ...
              if (ck.testDefined(clipA) && clipA && ck.testDefined(clipB) && clipB) {
  ...
  it("InwardCornerClip", () => {
    ...
    for (const shiftRadius of [0.0, 0.1, -0.1, -0.3, 1.0]) {
      for (const angle of angles) paths.push(quirkyArcPath(shiftRadius * angle.cos(), shiftRadius * angle.sin()));
    }
  ...
  it("IncompletePasteWithDoublePoint", () => {
    ...
    for (const candidates of [
      ...
      GeometryCoreTestIO.captureCloneGeometry(allGeometry, candidates, x0, y0);
      if (showVertexNeighborhoods) HalfEdgeGraphMerge.announceVertexNeighborhoodFunction = vertexNeighborhoodFunction;
      const insidePieces = RegionOps.polygonBooleanXYToLoops(candidates, RegionBinaryOpType.Union, []);
...
function quirkyArcPath(
  ...
  if (arc) return Path.create(LineSegment3d.create(pointA, point0), arc, LineSegment3d.create(point2, pointB));
  return undefined;
}
