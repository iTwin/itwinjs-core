...
function testParallelPair(
  ...
  ck.testCoordinate(
    a,
    plane0.altitude(planeA.getOriginRef()),
    "expected altitude"
  );
  const f = 0.5;
  ...
  ck.testTrue(
    planeB1.isAlmostEqual(planeB1FromJSON),
  ...
  ck.testCoordinate(
    0,
    plane0.velocity(triad.multiplyXY(3, 4)),
    "in plane vector has zero velocity"
  );
  const v = 23.4;
  ck.testCoordinate(
    v,
    plane0.velocity(triad.multiplyXYZ(3, 4, v)),
    "in plane vector has zero velocity"
  );
}
...
describe("Plane3dByOriginAndUnitNormal", () => {
  it("HelloWorld", () => {
    ...
    const planeABU = Plane3dByOriginAndUnitNormal.createPointPointVectorInPlane(
      pointA,
      pointB,
      vectorU
    )!;
    ...
    const planeABUh = Plane3dByOriginAndUnitNormal.create(
      pointC,
      planeABU.getNormalRef()
    );
  ...
  it("altitude", () => {
    ...
    ck.testUndefined(
      Plane3dByOriginAndUnitNormal.createXYZUVW(3, 2, 1, 0, 0, 0)
    );
    expect(ck.getNumErrors()).equals(0);
  });
  ...
  it("nulls", () => {
    const ck = new Checker();
    const pointA = Point3d.create(3, 2, 9);
    const failPlane = Plane3dByOriginAndUnitNormal.create(
      pointA,
      Vector3d.createZero()
    );
    ck.testUndefined(failPlane, "plane with null normal");
    const plane = Plane3dByOriginAndUnitNormal.createXYPlane();
    const plane1 = plane.cloneTransformed(
      Transform.createRowValues(1, 0, 0, 1, 0, 1, 0, 3, 0, 0, 0, 1)
    );
  ...
  it("assortedCreate", () => {
    ...
    Plane3dByOriginAndUnitNormal.createXYZUVW(
      origin.x,
      origin.y,
      origin.z,
      normal.x,
