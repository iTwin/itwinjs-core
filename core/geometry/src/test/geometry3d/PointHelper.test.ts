...
describe("FrameBuilder", () => {
  it("HelloWorld", () => {
    ...
    for (const points of [
      ...
      if (
        ck.testPointer(rFrame, "expect right handed frame") &&
        rFrame &&
...
function testCentroidNormal(
  ...
  for (const transform of Sample.createRigidTransforms()) {
    ...
    if (
      ...
      ck.testPointer(ray, "area computed") &&
      ray &&
      ray.a
    )
    ...
    for (const radiusA of [1, 2, 3.5]) {
      for (const degreesY of [0, 10, 78]) {
        for (const degreesZ of [0, -5, 20]) {
          const rotateZ = Matrix3d.createRotationAroundVector(
            Vector3d.unitZ(),
            Angle.createDegrees(degreesZ)
          ) as Matrix3d;
          const rotateY = Matrix3d.createRotationAroundVector(
            Vector3d.unitY(),
            Angle.createDegrees(degreesY)
          ) as Matrix3d;
...
describe("PolygonOps", () => {
  it("PolygonClip", () => {
    ...
    const points3d = [];
    for (const p of points) points3d.push(Point3d.create(p.x, p.y));
    ...
    ck.testExactNumber(
      1,
      PolygonOps.classifyPointInPolygonXY(xHit.x, xHit.y, carrier)!,
      "IN with horizontal vertex hits"
    );
    ck.testExactNumber(
      1,
...
describe("Point3dArray", () => {
  ...
  it("Point3dArrayCarrierBadIndex", () => {
    const ck = new Checker();
    const carrier = new Point3dArrayCarrier([
      Point3d.create(1, 2, 3),
...
describe("PolygonAreas", () => {
  ...
  it("SimpleNonConvex", () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (const degrees of [17.0, 197.4]) {
        const rotationMatrix = Matrix3d.createRotationAroundVector(
  ...
  it("ConvexHullManyPoints", () => {
    ...
    for (const numPoints of [9, 42, 273]) {
      ...
      GeometryCoreTestIO.captureGeometry(
        allGeometry,
        LineString3d.create([0, 0], [1, 0], [1, 1], [0, 1], [0, 0]),
        x0,
        y0
      );
