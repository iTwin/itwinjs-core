...
function exerciseWithPoints(ck: Checker, points: Point3d[]) {
  ...
  for (let i = 0; i < points.length; ) {
    if (i + 3 < points.length) {
      range6.extendRange(Range3d.create(points[i], points[i + 1], points[i + 2]));
      i += 3;
    } else {
...
describe("Range3d", () => {
  ...
  it("ScalarQueries", () => {
    ...
    for (const rangeB of intervalBArray) {
      ...
      if (!rangeB.isNull) {
        ...
        ck.testTrue(
          r3.containsXYZ(rangeX.fractionToPoint(0.5), rangeY.fractionToPoint(0.5), rangeZ.fractionToPoint(0.5))
        );
  ...
  it("Containment1d", () => {
    ...
    for (const rangeB of intervalBArray) {
      ...
      ck.testBoolean(intervalA.containsRange(rangeB), rangeAB.isAlmostEqual(rangeB), "contained range matches operand");
      ck.testCoordinate(d1, d2, "distance between ranges is symmetric");
      if (rangeAB.isNull) ck.testLT(0, d1, "Empty intersection must have nonzero distance");
      else ck.testCoordinate(0, d1, "nonempty intersection must have zero distance");
  ...
  it("DiagonalContainment3d", () => {
    ...
    for (const intervalB of intervalBArray) {
      ...
      ck.testCoordinate(d1, d2, "distance between ranges is symmetric");
      ck.testBoolean(rangeA.containsRange(rangeB), rangeAB.isAlmostEqual(rangeB), "contained range matches operand");
      if (rangeAB.isNull) ck.testLT(0, d1, "Empty intersection must have nonzero distance");
      else ck.testCoordinate(0, d1, "nonempty intersection must have zero distance");
  ...
  it("DiagonalContainment2d", () => {
    const ck = new Checker();
    const rangeA = Range2d.createXYXY(intervalA.low, intervalA.low, intervalA.high, intervalA.high);
    // GeometryCoreTestIO.consoleLog("rangeA:", JSON.stringify(rangeA));
    for (const intervalB of intervalBArray) {
      const rangeB = Range2d.createXYXYOrCorrectToNull(intervalB.low, intervalB.low, intervalB.high, intervalB.high);
      const rangeAB = rangeA.intersect(rangeB);
      const rangeQ = rangeAB.clone();
      rangeQ.extendRange(rangeB);
      ck.testTrue(rangeQ.containsRange(rangeB), "extend acts as union");
