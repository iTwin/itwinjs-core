...
describe("GrowableXYArray", () => {
  it("PointMoments", () => {
    ...
    for (let n = 3; n < 100; n *= 2) {
      ...
      for (let i = 0; i < n; i++) ck.testPoint2d(pointB[i], pointA.getPoint2dAtUncheckedPointIndex(i));
  ...
  it("Transform", () => {
    ...
    const transform = Transform.createRowValues(1, 2, 4, 3, 2, 5, 4, -3, 0.2, 0.3, 0.4, 0.5);
  ...
  it("IndexedXYCollection", () => {
    ...
    for (let i = 1; i + 1 < points.length; i++) {
      ...
      ck.testCoordinate(iPoints.crossProductIndexIndexIndex(0, i, j)!, gPoints.crossProductIndexIndexIndex(0, i, j)!);
      ck.testCoordinate(
  ...
  it("resizeAndBoundsChecks", () => {
    ...
    for (let i0 = 3; i0 < n2; i0 += 5) {
      for (let i1 = 0; i1 < n2; i1 += 3) {
        ...
        if (vectorB) ck.testVector2d(Vector2d.create(vectorA.x, vectorA.y), vectorB);
