...
describe("GrowableXYArray", () => {
  it("PointMoments", () => {
    ...
    for (let n = 3; n < 100; n *= 2) {
      ...
      for (let i = 0; i + 1 < n; i++) {
        lengthA += pointA
          .getPoint2dAtUncheckedPointIndex(i)
          .distance(pointA.getPoint2dAtUncheckedPointIndex(i + 1));
      }
  ...
  it("Transfer", () => {
    const ck = new Checker();
    const n = 11;
    const pointA = GrowableXYArray.create(
      Sample.createGrowableArrayCirclePoints(1.0, n, false)
    );
    const pointB = new GrowableXYArray(10);
    ck.testFalse(pointA.isAlmostEqual(pointB), "isAlmostEqual detects length");
    ck.testFalse(
      GrowableXYArray.isAlmostEqual(pointA, pointB),
    ...
    ck.testFalse(
      GrowableXYArray.isAlmostEqual(pointA, pointG),
      "static isAlmostEqual"
    );
  ...
  it("Transform", () => {
    ...
    const transformG0 = Transform.createOriginAndMatrix(
      Point3d.createZero(),
      transform.matrix
    );
    ...
    pointH2.multiplyMatrix3dInPlace(
      Matrix3d.createScale(factor, factor, factor)
    );
  ...
  it("AreaXY", () => {
    ...
    for (const n of [4, 9, 64]) {
      ...
      ck.testCoordinate(
        areaA,
        n * r * r * Math.cos(radians) * Math.sin(radians)
      );
    }
    expect(ck.getNumErrors()).equals(0);
  });
  ...
  it("Wrap", () => {
    ...
    for (let n = 5; n < 100; n *= 2) {
      ...
      for (let i = 0; i < numWrap; i++) {
        ck.testPoint2d(
          pointA.getPoint2dAtUncheckedPointIndex(i),
          pointA.getPoint2dAtUncheckedPointIndex(n + i),
          "wrapped point"
        );
      }
      let numDup = 0;
      ...
      for (let i = 0; i + 1 < pointA.length; i++) {
        ...
        if (
          pointA
            .getPoint2dAtUncheckedPointIndex(k0)
            .isAlmostEqual(pointA.getPoint2dAtUncheckedPointIndex(k1))
        ) {
  ...
  it("IndexedXYCollection", () => {
    ...
    const points2d = [];
    for (const p of points) points2d.push(Point2d.create(p.x, p.y));
  ...
  it("resizeAndBoundsChecks", () => {
    ...
    for (const p of points) xyPoints.push(p);

    ck.testTrue(
      GrowableXYArray.isAlmostEqual(xyPoints, xyPoints),
      "isAlmostEqual duplicate pair"
    );
    ck.testTrue(
      GrowableXYArray.isAlmostEqual(undefined, undefined),
      "isAlmostEqual undefined pair"
    );

    ck.testFalse(
      GrowableXYArray.isAlmostEqual(undefined, xyPoints),
      "isAlmostEqual one undefined"
    );
    ck.testFalse(
      GrowableXYArray.isAlmostEqual(xyPoints, undefined),
      "isAlmostEqual one undefined"
    );
    ...
    for (let i0 = 3; i0 < n2; i0 += 5) {
      for (let i1 = 0; i1 < n2; i1 += 3) {
        ...
        else ck.announceError("vectorIndexIndex?", i0, i1, vectorA, vectorB);
      }
    }
    ...
    for (let i0 = 2; i0 < n2; i0 += 6) {
      ...
      const distance1 = xyPoints
        .getPoint2dAtCheckedPointIndex(i0)!
        .distance(spacePoint);
    ...
    for (let i = 1; i < xyPoints.length; i++) {
      ck.testExactNumber(
        xyPoints.distance(0, i)!,
        xyPoints.distanceIndexToPoint(i, point0)!
      );
    }
    ck.testUndefined(
      xyPoints.distanceIndexToPoint(-1, spacePoint),
      "distance to invalid indexA"
    );

    ck.testFalse(
      xyPoints.setXYAtCheckedPointIndex(-5, 1, 2),
      "negative index for setCoordinates"
    );
    ck.testFalse(
      xyPoints.setXYAtCheckedPointIndex(100, 1, 2),
      "huge index for setCoordinates"
    );

    ck.testFalse(
      xyPoints.setAtCheckedPointIndex(-5, spacePoint),
      "negative index for setAt"
    );
    ck.testFalse(
      xyPoints.setAtCheckedPointIndex(100, spacePoint),
      "huge index for setAt"
    );
    ck.testUndefined(
      xyPoints.vectorXAndYIndex(spacePoint, -5),
  ...
  it("transferAndSet", () => {
    ...
    for (const p of points) array0.push(p);
    ...
    ck.testExactNumber(
      0,
      array1.pushFromGrowableXYArray(array0, -1),
      "invalid source index for pushFromGrowable"
    );
    ck.testExactNumber(
      0,
      array1.pushFromGrowableXYArray(array0, n0 + 1),
      "invalid source index for pushFromGrowable"
    );
    // Any transfer into empty array is bad . ..
    ck.testFalse(
      array1.transferFromGrowableXYArray(-1, array0, 1),
      "invalid source index transferFromGrowable"
    );
    ck.testFalse(
      array1.transferFromGrowableXYArray(0, array0, 1),
      "invalid source index transferFromGrowable"
    );
    ck.testFalse(
      array1.transferFromGrowableXYArray(100, array0, 1),
      "invalid source index transferFromGrowable"
    );
    ...
    ck.testUndefined(
      array1.crossProductXAndYIndexIndex(spacePoint, -1, 0),
      "bad indexA for cross product"
    );
    ck.testUndefined(
      array1.crossProductXAndYIndexIndex(spacePoint, 0, -1),
    ...
    for (let k = 1; k + 2 < n0; k++) {
      ck.testExactNumber(
        1,
        array1.pushFromGrowableXYArray(array0, k),
      ...
      if (
        ...
        if (
          ck.testIsFinite(crossA) &&
          crossA !== undefined &&
          ck.testIsFinite(crossB) &&
          crossB !== undefined
        ) {
  ...
  it("LoadFromArray", () => {
    const ck = new Checker();
    const n = 5;
    const pointA = GrowableXYZArray.create(
      Sample.createGrowableArrayCirclePoints(1.0, n, false)
    );
  ...
  it("MethodsImplementedByInterface", () => {
    ...
    for (const p of wrapper.points) {
      ...
      ck.testPoint3d(
        p,
        growablePoints.getPoint3dAtUncheckedPointIndex(i),
        "wrapper vs growable"
      );
      i++;
    }
  ...
  it("removeClosurePoints", () => {
    ...
    ck.testExactNumber(
      originalTrim,
      points.length,
      "original closure point=>no change"
    );
    GrowableXYZArray.removeClosure(wrapper);
    ck.testExactNumber(
      originalTrim,
      points.length,
      "no closure point=>no change"
    );
    for (const numAdd of [1, 3]) {
      for (let i = 0; i < numAdd; i++) wrapper.push(origin);
      ck.testExactNumber(
        originalTrim + numAdd,
        wrapper.length,
        `after adding  ${numAdd} closure points`
      );
      GrowableXYZArray.removeClosure(wrapper);
      ck.testExactNumber(
        originalTrim,
        wrapper.length,
        `after removeClosure ${numAdd}`
      );
    }
  });
});
