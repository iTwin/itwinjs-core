...
function verifyTriangle(ck: Checker, triangle: BarycentricTriangle) {
  ...
  const subTriangle0 = BarycentricTriangle.create(
    centroid,
    triangle.points[1],
    triangle.points[2]
  );
  const subTriangle1 = BarycentricTriangle.create(
    centroid,
    triangle.points[2],
    triangle.points[0]
  );
  const subTriangle2 = BarycentricTriangle.create(
    centroid,
    triangle.points[0],
    triangle.points[1]
  );
  ck.testTrue(
    subTriangle0.aspectRatio < triangle.aspectRatio,
    "subtriangle aspect ratio smaller than full"
  );
  ck.testCoordinate(
    triangle.area,
  ...
  const cloneB = BarycentricTriangle.create(
    triangle.points[0],
    triangle.points[1],
    triangle.points[2]
  );
  ...
  // clone with reversed
  const cloneC1Reverse = BarycentricTriangle.create(
    cloneC1.points[0],
    cloneC1.points[2],
    cloneC1.points[1]
  );
  const dotSelf =
    cloneC1Reverse.dotProductOfCrossProductsFromOrigin(cloneC1Reverse);
  const dotReverse =
    cloneC1.dotProductOfCrossProductsFromOrigin(cloneC1Reverse);
...
describe("BarycentricTriangle", () => {
  it("Create", () => {
    ...
    verifyTriangle(
      ck,
      BarycentricTriangle.createXYZXYZXYZ(0, 0, 0, 1, 0, 0, 0, 1, 0)
    );
    verifyTriangle(
      ck,
      BarycentricTriangle.createXYZXYZXYZ(1, 4, 2, 7, -2, 1.5, 2.2, 9.0, 10)
    );
  ...
  it("closestPoint", () => {
    ...
    const triangle = BarycentricTriangle.create(
      vertices[0],
      vertices[1],
      vertices[2]
    );
    ...
    ck.testTrue(
      PolygonOps.isConvex(vertices),
      "triangle returns true for isConvex"
    );
    ck.testTrue(
      PolygonOps.isConvex([...vertices, vertices[0]]),
      "triangle with closure point returns true for isConvex"
    );
    ck.testFalse(
      PolygonOps.isConvex([vertices[0], vertices[1]]),
      "isConvex returns false for degenerate triangle"
    );
    ck.testFalse(
      PolygonOps.isConvex([vertices[0], vertices[1], vertices[1]]),
      "isConvex returns false for degenerate triangle"
    );
    ck.testFalse(
      PolygonOps.isConvex([...vertices, incenter]),
      "isConvex returns false on chevron"
    );
    ck.testCoordinate(
      Math.sqrt(13) + Math.sqrt(40) + 9,
      triangle.perimeter,
    ...
    GeometryCoreTestIO.captureGeometry(
      allGeometry,
      LineString3d.create(...vertices, vertices[0])
    );

    // intersectSegment coverage
    const centroid = triangle.centroid();
    const loc1 = triangle.intersectSegment(
      Point3d.create(centroid.x, centroid.y, -10),
      Point3d.create(centroid.x, centroid.y, 10)
    );
    ...
    ck.testPoint3d(
      centroid,
      loc1.world,
      "expected world coords of intersection"
    );
    ck.testPoint3d(
      Point3d.create(1 / 3, 1 / 3, 1 / 3),
      loc1.local,
      "expected barycentric coords of intersection"
    );
    ck.testCoordinate(
      0.5,
      loc1.a,
      "expected segment parameter of intersection"
    );
    ck.testFalse(
    ...
    ck.testFalse(
      PolygonOps.closestPointOnBoundary([], incenter).isValid,
      "0-pt 'polygon' closest point is invalid"
    );
    ...
    ck.testTrue(
      PolygonOps.closestPointOnBoundary([vertices[0], vertices[1]], incenter)
        .isValid,
      "2-pt 'polygon' closest point is invalid"
    );
    loc0 = PolygonOps.closestPointOnBoundary(
      [vertices[0], vertices[1], vertices[1], vertices[2], vertices[2]],
      incenter
    );
    ...
    for (const specialPt of barycentricInsideOn) {
      ck.testBoolean(
        specialPt[0].isInsideOrOn,
        specialPt[1],
        "special point containment"
      );
      ck.testExactNumber(
        specialPt[0].classify,
        specialPt[1]
    ...
    const diag = Range3d.create(...vertices, Point3d.createZero())
      .diagonal()
      .magnitude();
    for (let i = 0; i < numCirclePoints; ++i) {
      ...
      if (
        ...
        ck.testPoint3d(
          xyz,
          triangle.fractionToPoint(loc.local.x, loc.local.y, loc.local.z),
          "recover circle pt from barycentric"
        )
      )
    ...
    for (const b of [
      Point3d.create(0.2, 0.5, 0.3),
      Point3d.create(0.5, 0.1, 0.4), // inside triangle
      Point3d.create(1, 0, 0),
      Point3d.create(0, 1, 0),
      Point3d.create(0, 0, 1), // at vertices
      Point3d.create(0, 0.4, 0.6),
      Point3d.create(0.2, 0, 0.8),
      Point3d.create(0.3, 0.7, 0), // inside edges
      Point3d.create(0, -4, 5),
      Point3d.create(0, -0.5, 1.5),
      Point3d.create(0, 1.5, -0.5),
      Point3d.create(0, 5, -4), // on extended edge 0
      Point3d.create(-4, 0, 5),
      Point3d.create(-0.5, 0, 1.5),
      Point3d.create(1.5, 0, -0.5),
      Point3d.create(5, 0, -4), // on extended edge 1
      Point3d.create(-4, 5, 0),
      Point3d.create(-0.5, 1.5, 0),
      Point3d.create(1.5, -0.5, 0),
      Point3d.create(5, -4, 0), // on extended edge 2
      ...specialPoints,
      ...circlePoints,
    ]) {
      ...
      const proj = vertices[data.closestEdgeIndex].interpolate(
        data.closestEdgeParam,
        vertices[Geometry.cyclic3dAxis(data.closestEdgeIndex + 1)]
      );
      GeometryCoreTestIO.captureGeometry(
        allGeometry,
        LineSegment3d.create(pt, proj)
      );
      ...
      for (let i = 0; i < 3; ++i) {
        if (b.at(i) === 0) {
          ...
          if (b.at(j) === 0) {
            // at vertex
            ck.testExactNumber(
              triangle.pointToFraction(pt).classify,
              PolygonLocation.OnPolygonVertex,
              "vertex classify"
            );
            const k = Geometry.cyclic3dAxis(j + 1);
            ck.testExactNumber(
              k,
              data.closestEdgeIndex,
              "vertex hit has expected edge index"
            );
            ck.testExactNumber(
              0.0,
              data.closestEdgeParam,
              "vertex hit has expected edge param"
            );
          } else if (b.at(j) > 0.0 && b.at(j) < 1.0) {
            // inside edge
            ck.testExactNumber(
              triangle.pointToFraction(pt).classify,
            ...
            ck.testExactNumber(
              j,
              data.closestEdgeIndex,
              "edge hit has expected edge index"
            );
            ck.testExactNumber(
              1 - b.at(j),
              data.closestEdgeParam,
              "edge hit has expected edge param"
            );
          }
        }
      }
      ...
      if (!triangle.incenter().isAlmostEqual(pt)) {
        // incenter is equidistant from edges, so it may project to any edge!
        ck.testExactNumber(
          data.closestEdgeIndex,
          loc.closestEdgeIndex,
          "closest edge index same in both algorithms"
        );
        ck.testCoordinate(
          data.closestEdgeParam,
          loc.closestEdgeParam,
          "closest edge param same in both algorithms"
        );
      }
      ...
      ck.testBoolean(
        undefined !== b2,
        BarycentricTriangle.isInsideOrOnTriangle(b.x, b.y, b.z),
      ...
      if (undefined !== b2) {
        ck.testCoordinate(
          b.x,
          b2[0],
          "convex barycentric x equals BarycentricTriangle x"
        );
        ck.testCoordinate(
          b.y,
          b2[1],
