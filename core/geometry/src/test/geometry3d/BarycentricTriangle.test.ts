...
function verifyTriangle(ck: Checker, triangle: BarycentricTriangle) {
  ...
  const transform = Transform.createFixedPointAndMatrix(
    ...
    Matrix3d.createRotationAroundVector(Vector3d.create(0.2, 3.1, -0.3), Angle.createDegrees(22))!
  );
  ...
  const y0 = 1;
  const z = 2.0;
  const cloneC1 = BarycentricTriangle.createXYZXYZXYZ(x0, y0, z, x0 + 2 * a, y0, z, x0 + a, y0 + a, z, cloneC);
...
describe("BarycentricTriangle", () => {
  ...
  it("closestPoint", () => {
    ...
    ck.testFalse(
      triangle.intersectSegment(centroid, Point3d.create(centroid.x + 10, centroid.y, centroid.z)).isValid,
      "parallel segment intersection is invalid"
    );

    // degenerate input coverage
    ck.testFalse(
      BarycentricTriangle.create(vertices[0], vertices[1], vertices[1]).pointToFraction(incenter).isValid,
