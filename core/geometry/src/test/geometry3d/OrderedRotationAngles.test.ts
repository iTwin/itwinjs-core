      ...
      for (const treatVectorsAsColumns of [true, false]) {
        ...
        for (const factor of [1.0, 0.05, 2.0, 4.2343, 0.001, 0.0001, 0.00001, -1.0, -0.4]) {
          ...
          for (const anglesA of [
            ...
            if (ck.testDefined(anglesB) && anglesB) testEqualOrderedRotationAngles(ck, anglesA, anglesB);
...
describe("OrderedRotationAngles", () => {
  it("OrderedRotationAngles.toMatrix3dColumnBased", () => {
    ...
    const rX = Matrix3d.createRowValues(1, 0, 0, 0, Math.cos(x), -Math.sin(x), 0, Math.sin(x), Math.cos(x));
    const rY = Matrix3d.createRowValues(Math.cos(y), 0, Math.sin(y), 0, 1, 0, -Math.sin(y), 0, Math.cos(y));
    const rZ = Matrix3d.createRowValues(Math.cos(z), -Math.sin(z), 0, Math.sin(z), Math.cos(z), 0, 0, 0, 1);
      ...
      for (const pitchAngle of degreesChoices) {
        ...
        for (const yawAngle of degreesChoices) {
          ...
          if (!ck.testCoordinate(0, yprMatrix.maxDiff(matrixZYX))) {
            GeometryCoreTestIO.consoleLog(
              `${JSON.stringify(ypr.toJSON())} maxDiff ypr:(Z)(-Y)(X)   ${yprMatrix.maxDiff(matrixZYX)}`
            );
          ...
          if (!ck.testMatrix3d(yprMatrix, orderedMatrix)) {
            GeometryCoreTestIO.consoleLog(
              `${JSON.stringify(ypr.toJSON())} maxDiff ypr:(Z)(-Y)(X)   ${yprMatrix.maxDiff(orderedMatrix)}`
            );
            GeometryCoreTestIO.consoleLog("ypr matrix", yprMatrix);
...
describe("OrderedRotationAngles", () => {
  it("OrderedRotationAngles.createFromMatrix3dColumnBased", () => {
    ...
    if (ck.testDefined(angles) && angles) testEqualOrderedRotationAngles(ck, angles, expectedAngles);
