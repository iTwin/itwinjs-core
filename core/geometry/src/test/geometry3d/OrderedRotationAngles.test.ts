...
function multiplyMatricesByAxisOrder(
  xMatrix: Matrix3d,
  yMatrix: Matrix3d,
  zMatrix: Matrix3d,
  axisOrder: AxisOrder
  ...
  switch (axisOrder) {
    case AxisOrder.XYZ:
      return zMatrix.multiplyMatrixMatrix(
        yMatrix.multiplyMatrixMatrix(xMatrix)
      );
    case AxisOrder.XZY:
      return yMatrix.multiplyMatrixMatrix(
        zMatrix.multiplyMatrixMatrix(xMatrix)
      );
    case AxisOrder.YXZ:
      return zMatrix.multiplyMatrixMatrix(
        xMatrix.multiplyMatrixMatrix(yMatrix)
      );
    case AxisOrder.YZX:
      return xMatrix.multiplyMatrixMatrix(
        zMatrix.multiplyMatrixMatrix(yMatrix)
      );
    case AxisOrder.ZXY:
      return yMatrix.multiplyMatrixMatrix(
        xMatrix.multiplyMatrixMatrix(zMatrix)
      );
    case AxisOrder.ZYX:
      return xMatrix.multiplyMatrixMatrix(
        yMatrix.multiplyMatrixMatrix(zMatrix)
      );
  }
}

function multipleDegreeRotationsByAxisOrder(
  xDegrees: number,
  yDegrees: number,
  zDegrees: number,
  axisOrder: AxisOrder
...
function testEqualOrderedRotationAngles(
  ck: Checker,
  a: OrderedRotationAngles,
  b: OrderedRotationAngles
) {
  ...
  if (
    !ck.testMatrix3d(
      matrixA,
      matrixB,
      "matrix images of OrderedRotationAngle pair"
    )
  ) {
...
function testMultiAngleEquivalence(
  ck: Checker,
  xDegrees: number,
  yDegrees: number,
  zDegrees: number,
  treatVectorsAsColumns: boolean
) {
  ...
  for (const orderPair of [
    ...
    const angles = OrderedRotationAngles.createDegrees(
      xDegrees,
      yDegrees,
      zDegrees,
      orderPair[0]
    ...
    const angles2 = OrderedRotationAngles.createDegrees(
      xDegrees,
      yDegrees,
      zDegrees,
      orderPair[0],
    ...
    const matrixB = multipleDegreeRotationsByAxisOrder(
      xDegrees,
      yDegrees,
      zDegrees,
      orderPair[0]
    ...
    const matrixC = multipleDegreeRotationsByAxisOrder(
      -xDegrees,
      -yDegrees,
      -zDegrees,
      orderPair[1]
  ...
  it("OrderedRotationAngles.toMatrix3dFromMatrix3d", () => {
    ...
    for (const axisOrder of [
      ...
      for (const treatVectorsAsColumns of [true, false]) {
        OrderedRotationAngles.treatVectorsAsColumns = treatVectorsAsColumns;
        for (const factor of [
          1.0, 0.05, 2.0, 4.2343, 0.001, 0.0001, 0.00001, -1.0, -0.4,
        ]) {
          ...
          for (const anglesA of [
            ...
            const anglesB = OrderedRotationAngles.createFromMatrix3d(
              matrixA,
              axisOrder
            );
...
describe("OrderedRotationAngles", () => {
  it("OrderedRotationAngles.toMatrix3dColumnBased", () => {
    ...
    OrderedRotationAngles.createRadians(
      x,
      y,
      z,
      AxisOrder.XYZ,
    ...
    angles.toMatrix3d(matrix);
    expectedMatrix = rZ
      .multiplyMatrixMatrix(rY, expectedMatrix)
      .multiplyMatrixMatrix(rX, expectedMatrix);
    ck.testMatrix3d(matrix, expectedMatrix);
    ...
    expectedMatrix = rY
      .multiplyMatrixMatrix(rX, expectedMatrix)
      .multiplyMatrixMatrix(rZ, expectedMatrix);
    ck.testMatrix3d(matrix, expectedMatrix);
...
describe("OrderedRotationAngles", () => {
  it("OrderedRotationAnglesVersusYawPitchRollWithDefaultRotation", () => {
    const ck = new Checker();
    const degreesChoices = [
      Angle.createDegrees(0.0),
      Angle.createDegrees(10.0),
      Angle.createDegrees(5.0),
      Angle.createDegrees(15.0),
    ];
    ...
    for (const rollAngle of degreesChoices) {
      ...
      for (const pitchAngle of degreesChoices) {
        ...
        for (const yawAngle of degreesChoices) {
          ...
          const matrixZYX = matrixZ.multiplyMatrixMatrix(
            matrixY.multiplyMatrixMatrix(matrixX)
          );
          const ypr = YawPitchRollAngles.createRadians(
            yawAngle.radians,
            pitchAngle.radians,
            rollAngle.radians
          );
          ...
          if (!ck.testCoordinate(0, yprMatrix.maxDiff(matrixZYX))) {
            GeometryCoreTestIO.consoleLog(
              `${JSON.stringify(
                ypr.toJSON()
              )} maxDiff ypr:(Z)(-Y)(X)   ${yprMatrix.maxDiff(matrixZYX)}`
            );
          ...
          if (!ck.testMatrix3d(yprMatrix, orderedMatrix)) {
            GeometryCoreTestIO.consoleLog(
              `${JSON.stringify(
                ypr.toJSON()
              )} maxDiff ypr:(Z)(-Y)(X)   ${yprMatrix.maxDiff(orderedMatrix)}`
            );
            GeometryCoreTestIO.consoleLog("ypr matrix", yprMatrix);
    ...
    it("OrderedRotationAnglesVersusYawPitchRollWithNonDefaultRotation", () => {
      ...
      for (const rollAngle of degreesChoices) {
        ...
        for (const pitchAngle of degreesChoices) {
          // this is PITCH
          const matrixY = Matrix3d.createRotationAroundVector(
            unitY,
            pitchAngle
          )!;
          matrixY.transposeInPlace(); // PITCH is NEGATIVE Y
          for (const yawAngle of degreesChoices) {
            // this is YAW
            const matrixZ = Matrix3d.createRotationAroundVector(
              unitZ,
              yawAngle
            )!;
...
describe("OrderedRotationAngles", () => {
  it("OrderedRotationAngles.createFromMatrix3dColumnBased", () => {
    ...
    const x = 0.0174533,
      y = 0.698132,
      z = Math.PI / 2; // 1, 40, and 90 degrees
    OrderedRotationAngles.treatVectorsAsColumns = true;

    // No Rotation
    const matrix = Matrix3d.createIdentity();
    const angles = OrderedRotationAngles.createFromMatrix3d(
      matrix,
      AxisOrder.XYZ
    ); // order doesn't matter
    const expectedAngles = OrderedRotationAngles.createRadians(
      0,
      0,
      0,
      AxisOrder.XYZ
...
describe("OrderedRotationAngles", () => {
  it("OrderedRotationAngles.createFromMatrix3dRowBased", () => {
    ...
    // Three Rotations
    const rZrYrX = rZ
      .multiplyMatrixMatrix(rY)
      .multiplyMatrixMatrix(rX)
      .transpose();
    OrderedRotationAngles.createFromMatrix3d(rZrYrX, AxisOrder.XYZ, angles);
    OrderedRotationAngles.createRadians(
      x,
