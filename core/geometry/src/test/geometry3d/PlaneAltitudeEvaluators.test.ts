 ...
 */
function verifyMatchedPlaneAltitudeEvaluator(
  ck: Checker,
  planeA: Plane3d,
  planeB: Plane3d
) {
  ...
  for (const p of points) {
    ck.testCoordinate(planeA.altitude(p), planeB.altitude(p), {
      altitudeOf: p,
    });
    ck.testCoordinate(planeA.altitudeXYZ(p.x, p.y, p.z), planeA.altitude(p), {
      altitudeOf: p,
    });
    ck.testCoordinate(
      planeA.altitudeXYZ(p.x, p.y, p.z),
      planeB.altitudeXYZ(p.x, p.y, p.z),
      { altitudeOf: p }
    );
    ck.testCoordinate(planeA.altitudeXYZ(p.x, p.y, p.z), planeA.altitude(p), {
      altitudeOf: p,
    ...
    ck.testCoordinate(
      planeA.weightedAltitude(p4),
      planeB.weightedAltitude(p4),
      { weightedAltitude1: p4 }
    );
    p4.z = 2.0;
    ck.testCoordinate(
      planeA.weightedAltitude(p4),
      planeB.weightedAltitude(p4),
      { weightedAltitude2: p4 }
    );
  ...
  for (const v of vectors) {
    ck.testCoordinate(planeA.velocity(v), planeB.velocity(v), {
      velocityOf: v,
    });
    ck.testCoordinate(planeA.velocityXYZ(v.x, v.y, v.z), planeA.velocity(v), {
      velocityOf: v,
    });
    ck.testCoordinate(
      planeA.velocityXYZ(v.x, v.y, v.z),
      planeB.velocityXYZ(v.x, v.y, v.z),
      { velocityOf: v }
    );
    ck.testCoordinate(planeA.velocityXYZ(v.x, v.y, v.z), planeA.velocity(v), {
      velocityXYZ: v,
...
describe("PlaneEvaluator", () => {
  it("HelloWorld", () => {
    ...
    for (const normal of [
      Vector3d.createNormalized(0, 0, 1)!,
      ...
      const planeA = Plane3dByOriginAndUnitNormal.createOriginAndVectors(
        pointA,
        vectorU,
        vectorV
      )!;
      const planeB = Plane3dByOriginAndVectors.createOriginAndVectors(
        pointA,
        vectorU,
        vectorV
      );
      ...
      if (
        ck.testType(planeC, ClipPlane) &&
        ck.testType(planeD, Plane3dByOriginAndUnitNormal)
      )
        verifyMatchedPlaneAltitudeEvaluator(ck, planeC, planeD);

      const planeA1 = Plane3dByOriginAndUnitNormal.createOriginAndTargets(
        pointA,
        pointU,
        pointV
      )!;
      const planeB1 = Plane3dByOriginAndVectors.createOriginAndTargets(
        pointA,
        pointU,
        pointV
      );
      ...
      for (const sourcePlane of [
        ...
        if (
          ck.testType(
            ...
            "Convert to Plane3dByOriginAndUnitNormal"
          )
        ) {
        ...
        if (
          ck.testType(
            ...
            "Convert to Plane3dByOriginAndVectors"
          )
        ) {
