...
class PolylineCompressionChecker {
  ...
  public verifyGlobalChordErrorCompression(
    ...
    const result = PolylineOps.compressByChordError(
      points,
      globalEdgeTolerance
    );
    const y0 = this.y0;
    this.x0 += 2.0 * globalEdgeTolerance;
    if (numExpected > 0) this.ck.testExactNumber(numExpected, result.length);
    ...
    const edgeCompress = PolylineOps.compressShortEdges(
      points,
      globalEdgeTolerance
    );
    GeometryCoreTestIO.captureCloneGeometry(
      this.allGeometry,
      LineString3d.create(edgeCompress),
      this.x0,
      this.y0
    );
    GeometryCoreTestIO.createAndCaptureXYCircle(
    ...
    if (areaTolerance > 0) {
      const areaCompress = PolylineOps.compressSmallTriangles(
        points,
        areaTolerance
      );
      GeometryCoreTestIO.captureCloneGeometry(
        this.allGeometry,
        LineString3d.create(areaCompress),
        this.x0,
        this.y0
      );
      GeometryCoreTestIO.createAndCaptureXYCircle(
    ...
    if (perpendicularDistanceTolerance > 0) {
      ...
      for (const numPass of [1, 2, 3, 4, 5, 6, 7, 8]) {
        ...
        const perpCompress = PolylineOps.compressByPerpendicularDistance(
          points,
          perpendicularDistanceTolerance,
          numPass
        );
        ...
        if (perpCompress.length > 3)
          this.ck.testLE(
            edgeLengthRange0.high,
            edgeLengthRange1.high,
            "Compression does not reduce max edge length"
          );
        const num1 = perpCompress.length;
        GeometryCoreTestIO.captureCloneGeometry(
          this.allGeometry,
          LineString3d.create(perpCompress),
          this.x0,
          this.y0
        );
        if (num1 === num0) break;
...
describe("GlobalCompression", () => {
  it("HelloWorld", () => {
    ...
    context.verifyGlobalChordErrorCompression(
      2,
      [Point3d.create(0, 0, 0), Point3d.create(10, 0, 0)],
      1.0
    );
    const point3 = [
  ...
  it("SmallArrays", () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ck.testExactNumber(
        points.length,
        pointsB.length,
        "No Change triangle case"
      );

      const pointsC = PolylineOps.compressByPerpendicularDistance(
        points,
        0.1,
        1
      );
      ...
      const pointsD = PolylineOps.compressByPerpendicularDistance(
        points,
        0.1,
        2
      );
      ...
      ck.testExactNumber(
        points.length,
        pointsE.length,
        "No Change global chord error case"
      );
    }
  ...
  it("ColinearThroughStart", () => {
    ...
    const pointsWithColinearThroughStart = [
      ...
      Point3d.createFrom({ x: 0, y: 0, z: -3538.3128322623243 })!,
      Point3d.createFrom({
        x: 1746.2903617595616,
        y: 0,
        z: -3538.3128322623243,
      })!,
      Point3d.createFrom({
        x: 1746.2903617595616,
        y: -1151.9060537233227,
        z: -3538.3128322623243,
      })!,
      Point3d.createFrom({
        x: 4102.778912210693,
  ...
  it("Danglers", () => {
    ...
    const compressAndTest = (pointsWithDanglers: Point3d[]) => {
      ...
      ck.testExactNumber(
        ...
        compressedPoints.length,
        "point count after compression"
      );
    ...
    for (const baseIndex of [0, 1, n - 2, n - 1]) {
      ...
      const pointsA = insertPointsOnVector(
        originalPoints,
        baseIndex,
        vectorX4,
        [1, 0]
      ...
      const pointsB = insertPointsOnVector(
        pointsA,
        baseIndex + 1,
        vectorY2,
        [1, 1.5, 1, 0]
      ...
      const pointsC = insertPointsOnVector(
        pointsB,
        baseIndex + 1,
        vectorY2,
        [-1, -1.5, -1, 0]
      ...
      for (const shift of [2, 3, 5]) {
        const pointsD = insertPointsOnVector(
          pointsC,
          (baseIndex + originalCount - shift) % originalCount,
          vectorQ,
          [0, 1, 0.5, -0.25, -1.0, 0]
        );
