...
export class SimpleFactory {
  ...
  private static _point1 = [g.Point3d.create(1, 2, 3), g.Point3d.create(-2, 3, 1), g.Point3d.create(-1, -2, 3)];
  // _vector1 has integer coordinates
  private static _vector1 = [g.Vector3d.create(0, 1, 0), g.Vector3d.create(1, 0, 0), g.Vector3d.create(1, 2, 4)];
  ...
  public static createDefaultLineSegment3d(select: number): g.LineSegment3d | undefined {
    if (select === 0) return g.LineSegment3d.create(g.Point3d.create(0, 0, 0), g.Point3d.create(1, 0, 0));
    if (select === 1) return g.LineSegment3d.create(SimpleFactory._point1[0], SimpleFactory._point1[1]);
    if (select === 2)
  ...
  public static createDefaultLineString3d(select: number): g.LineString3d | undefined {
    if (select === 0) return g.LineString3d.create(g.Point3d.create(0, 0, 0), g.Point3d.create(1, 0, 0));
    if (select === 1) return g.LineString3d.create(SimpleFactory._point1[0], SimpleFactory._point1[1]);
    if (select === 2)
  ...
  public static createDefaultPointString3d(select: number): g.PointString3d | undefined {
    if (select === 0) return g.PointString3d.create(g.Point3d.create(0, 0, 0), g.Point3d.create(1, 0, 0));
    if (select === 1) return g.PointString3d.create(SimpleFactory._point1[0], SimpleFactory._point1[1]);
    if (select === 2)
  ...
  public static createDefaultAngle(select: number): g.Angle | undefined {
    if (select === 0) return g.Angle.createDegrees(0);
    if (select === 1) return g.Angle.createDegrees(45.0);
    if (select === 2) return g.Angle.createDegrees(15.23123122);
    return undefined;
  }

  public static createDefaultAngleSweep(select: number): g.AngleSweep | undefined {
    if (select === 0) return g.AngleSweep.create360();
    if (select === 1) return g.AngleSweep.createStartEndDegrees(5.0, 90.0);
    if (select === 2) return g.AngleSweep.createStartEndDegrees(15.23123122, 100.3234);
    return undefined;
  }

  public static createDefaultMatrix3d(select: number): g.Matrix3d | undefined {
    if (select === 0) return g.Matrix3d.createIdentity();
    if (select === 1) return g.Matrix3d.createRowValues(10, 2, 3, -2, 12, 5, 1, -4, 8);
    if (select === 2)
  ...
  public static createDefaultPlane3dByOriginAndUnitNormal(select: number): g.Plane3dByOriginAndUnitNormal | undefined {
    if (select === 0) return g.Plane3dByOriginAndUnitNormal.createXYPlane();
    if (select === 1)
      return g.Plane3dByOriginAndUnitNormal.create(g.Point3d.create(1, 2, 3), g.Vector3d.create(2, 4, -1))!;
    if (select === 2) return g.Plane3dByOriginAndUnitNormal.create(SimpleFactory._point2[0], SimpleFactory._vector2[1]);
    return undefined;
  }

  public static createDefaultPlane3dByOriginAndVectors(select: number): g.Plane3dByOriginAndVectors | undefined {
    if (select === 0) return g.Plane3dByOriginAndVectors.createOriginAndVectorsXYZ(0, 0, 0, 1, 0, 0, 0, 1, 0)!;
    if (select === 1)
      return g.Plane3dByOriginAndVectors.createOriginAndVectors(
        SimpleFactory._point1[1],
        SimpleFactory._vector1[0],
        SimpleFactory._vector1[2]
      );
    if (select === 2)
      return g.Plane3dByOriginAndVectors.createOriginAndVectors(
        SimpleFactory._point2[1],
        SimpleFactory._vector2[0],
        SimpleFactory._vector2[2]
      );
    return undefined;
  }

  public static createDefaultRange3d(select: number): g.Range3d | undefined {
    if (select === 0) return g.Range3d.createNull();
    if (select === 1) return g.Range3d.createArray(SimpleFactory._point1);
    if (select === 2) return g.Range3d.createArray(SimpleFactory._point2);
    return undefined;
  }

  public static createDefaultRange1d(select: number): g.Range1d | undefined {
    if (select === 0) return g.Range1d.createNull();
    if (select === 1) return g.Range1d.createX(10.0);
    if (select === 2) return g.Range1d.createXX(1.234, -0.32432);
    return undefined;
  }

  public static createDefaultRange2d(select: number): g.Range2d | undefined {
    if (select === 0) return g.Range2d.createNull();
    if (select === 1) return g.Range2d.createXYXY(-1, 2, 3, 1);
    if (select === 2) return g.Range2d.createXYXY(0.234239, -1.34, Math.sqrt(3), Math.exp(1.2));
    return undefined;
  }

  public static createDefaultRay3d(select: number): g.Ray3d | undefined {
    if (select === 0) return g.Ray3d.createXAxis();
    if (select === 1) return g.Ray3d.create(SimpleFactory._point1[0], SimpleFactory._vector1[2]);
    if (select === 2) return g.Ray3d.create(SimpleFactory._point2[0], SimpleFactory._vector2[2]);
  ...
  public static createDefaultTransform(select: number): g.Transform | undefined {
    if (select === 0) return g.Transform.createIdentity();
    if (select === 1)
    ...
    if (select === 2)
      return g.Transform.createOriginAndMatrix(
        ...
        g.Matrix3d.createRotationAroundAxisIndex(2, g.Angle.createDegrees(30))
      );
    if (select === 3)
      return g.Transform.createOriginAndMatrix(
        g.Point3d.create(-1, 2, 3),
  ...
  public static createDefaultMap4d(select: number): g.Map4d | undefined {
    if (select === 0) return g.Map4d.createIdentity();
    const matrix = SimpleFactory.createDefaultMatrix4d(select);
    if (matrix !== undefined) {
      const inverse = matrix.createInverse();
      if (inverse) return g.Map4d.createRefs(matrix, inverse);
    }
    return undefined;
  }
  ...
  public static createDefaultMatrix4d(select: number): g.Matrix4d | undefined {
    switch (select) {
      ...
      case 2:
        return g.Matrix4d.createRowValues(12, 1, 0.2, 0.1, 0.13, 10, 1.1, 2, -0.3, 0.9, 12, 0.8, 0.2, -0.23, 0.26, 15);
    }
    return undefined;
  }

  public static createDefaultPoint4d(select: number): g.Point4d | undefined {
    if (select === 0) return g.Point4d.create();
    if (select === 1) return g.Point4d.create(1, 2, 3, 4);
    if (select === 2) return g.Point4d.create(Math.sqrt(2), Math.sqrt(3), -Math.expm1(2), 11 / 13);
    return undefined;
  }
