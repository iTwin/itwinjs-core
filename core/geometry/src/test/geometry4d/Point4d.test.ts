...
describe("Point4d", () => {
  it("HelloWorld", () => {
    ...
    pointA.x = 10;
    testExactPoint4dXYZW(ck, pointA, 10, 4, 6, 8);
    pointA.y = 20;
    testExactPoint4dXYZW(ck, pointA, 10, 20, 6, 8);
    pointA.z = 30;
    testExactPoint4dXYZW(ck, pointA, 10, 20, 30, 8);
    pointA.w = 40;
    testExactPoint4dXYZW(ck, pointA, 10, 20, 30, 40);

    const pointB = Point4d.create(-1, -2, -3, -4);
    pointB.setFrom(pointA);
    testExactPoint4dPoint4d(ck, pointA, pointB);
    const pointC = pointA.clone();
    testExactPoint4dPoint4d(ck, pointB, pointC);
    ...
    pointA1.x += f;
    ck.testFalse(pointA1.isAlmostEqual(pointA));
    pointA1.setFrom(pointA);
    pointA1.y += f;
    ck.testFalse(pointA1.isAlmostEqual(pointA));
    pointA1.setFrom(pointA);
    pointA1.z += f;
  ...
  it("Shifts", () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (i !== 3) ck.testCoordinate(vectorAB.magnitudeSquaredXYZ(), e * e);
  ...
  it("Planes", () => {
    ...
    ck.testCoordinate(
      pointB.velocityXYZ(pointU0.x, pointU0.y, pointU0.z),
    ...
    const randomCoordinate = (size: number) => {
      ...
      if (sign < 1) c = -1;
      else if (sign < 2) c = -Math.random();
      else if (sign < 3) c = 0;
      else if (sign < 4) c = Math.random();
      return c * size;
    };
    // verify new impl is at least as successful and accurate as old impl
    for (let size = 1.0e-7; size < 1e8; size *= 10) {
      for (let count = 0; count < 100; ++count) {
        const pt = Point4d.create(
        ...
        if (oldPlane && newPlane) {
          ...
          ck.testDefined(oldPlane1);
          ck.testUndefined(newPlane1); // silence linter
        }
      }
    }
  ...
  it("Distances", () => {
    ...
    const vectorA = xyzA.clone();
    vectorA.w = 0.0;
    const vectorB = xyzB.clone();
    vectorB.w = 0.0;
  ...
  it("Quaternions", () => {
    const ck = new Checker();
    const quatA = Point4d.create(3, 6, 9, 12);
    quatA.normalizeQuaternion();
    const quatB = Point4d.create(-1.1, 0.2, 5.3, 1.4);
    quatB.normalizeQuaternion();
    ...
    const quat20 = Point4d.interpolateQuaternions(quat0, 0.2, quat1);
    const quat80 = Point4d.interpolateQuaternions(quat0, 0.8, quat1);
    const quat50 = Point4d.interpolateQuaternions(quat0, 0.5, quat1);
