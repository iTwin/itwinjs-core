...
export abstract class BezierCoffs {
  ...
  public subdivide(u: number, left: BezierCoffs, right: BezierCoffs): boolean {
    ...
    for (let i0 = 0; i0 < order; i0++) {
      ...
      for (let i = 0; i < n1; i++) right.coffs[i] = v * right.coffs[i] + u * right.coffs[i + 1];
      n1--;
    }
    return true;
  }
...
export class UnivariateBezier extends BezierCoffs {
  ...
  public evaluate(u: number): number {
    this._basisValues = PascalCoefficients.getBezierBasisValues(this.order, u, this._basisValues);
    let sum = 0;
    for (let i = 0; i < this.order; i++) sum += this._basisValues[i] * this.coffs[i];
    return sum;
  }
  /**
  ...
  public runNewton(startFraction: number, tolerance: number = 1.0e-11): number | undefined {
    ...
    for (let iterations = 0; iterations++ < 10; ) {
      ...
      f = 0;
      for (let i = 0; i < order; i++) f += coffs[i] * UnivariateBezier._basisBuffer[i];
      ...
      df = 0;
      for (let i = 0; i < orderD; i++) df += (coffs[i + 1] - coffs[i]) * UnivariateBezier._basisBuffer1[i];
      df *= derivativeFactor;
...
export class Order4Bezier extends BezierCoffs {
  ...
  public evaluate(u: number): number {
    ...
    const v2 = v1 * v1;
    const v3 = v2 * v1;
    return this.coffs[0] * v3 + u * (3.0 * this.coffs[1] * v2 + u * (3.0 * this.coffs[2] * v1 + u * this.coffs[3]));
  }
  /**
...
export class Order5Bezier extends BezierCoffs {
  ...
  public evaluate(u: number): number {
    ...
    return (
      this.coffs[0] * v4 +
      u *
        (4.0 * this.coffs[1] * v3 + u * (6.0 * this.coffs[2] * v2 + u * (4.0 * this.coffs[3] * v1 + u * this.coffs[4])))
    );
  }
