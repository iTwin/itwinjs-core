...
export class CurveCurveCloseApproachXYRRtoRRD extends NewtonEvaluatorRRtoRRD {
  ...
  public evaluate(fractionU: number, fractionV: number): boolean {
     ...
     *   [u_{n+1}]     [u_n]          [P''(u).(P(u) - Q(v)) + P'(u).P'(u)    -Q'(v)]    [P'(u).(P(u) - Q(v))]
     *              =         -  Inv(                                                )
     *   [v_{n+1}]     [v_n]          [P'(u)     Q''(v).(P(u) - Q(v)) - Q'(v).Q'(v)]    [Q'(v).(P(u) - Q(v))]
     * Note that this is xy close approach so we can ignore z.
     */
    this._curveP.fractionToPointAnd2Derivatives(fractionU, this._planeP);
    ...
    this.currentF.setOriginAndVectorsXYZ(
      pDerivX * pMinusQx + pDerivY * pMinusQy, qDerivX * pMinusQx + qDerivY * pMinusQy, 0.0,
      p2DerivX * pMinusQx + p2DerivY * pMinusQy + pDerivX * pDerivX + pDerivY * pDerivY, -qDerivX - qDerivY, 0.0,
      pDerivX + pDerivY, q2DerivX * pMinusQx + q2DerivY * pMinusQy - qDerivX * qDerivX - qDerivY * qDerivY, 0.0,
    );
    return true;
  }
}
