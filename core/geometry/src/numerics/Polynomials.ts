 ...
 * * The family of derived classes is expected to begin with low order (at least linear through cubic) with highly optimized calculations.
...
export abstract class BezierCoffs {
   ...
   */
  public abstract basisFunctions(u: number): number[];
   ...
   * * The base class has an implementation which always fails.
   * * Order-specific implementations apply special case  analytic logic, e.g. for degree 1,2,3,4.
   */
  public roots(_targetValue: number, _restrictTo01: boolean): number[] | undefined {
    return undefined;
  }
...
export class Bezier extends BezierCoffs {
  public constructor(order: number) { super(order); }
  public clone(): Bezier {
    const result = new Bezier(this.order);
...
export class Order2Bezier extends BezierCoffs {
   ...
   * @returns Return a (newly allocated) array of basis function values.
   */
  public basisFunctions(u: number): number[] { return [1.0 - u, u]; }
  /**
   * Evaluate the bezier function at a parameter value.  (i.e. summ the basis functions times coefficients)
   * @param u parameter for evaluation
   */
...
export class Order4Bezier extends BezierCoffs {
  ...
  public basisFunctions(u: number): number[] {
    ...
    const uu = u * u;
    const vv = v * v;
    return [vv * v, 3.0 * vv * u, 3.0 * v * uu, u * uu];
  }
  /**
   * Evaluate the bezier function at a parameter value.  (i.e. summ the basis functions times coefficients)
   * @param u parameter for evaluation
...
export class Order5Bezier extends BezierCoffs {
  ...
  public basisFunctions(u: number): number[] {
    ...
    const vv = v * v;
    const vvv = vv * v;
    return [vv * vv, 4.0 * vvv * u, 6.0 * vv * uu, 4.0 * v * uuu, uu * uu];
  }
  /**
   * Evaluate the bezier function at a parameter value.  (i.e. summ the basis functions times coefficients)
   * @param u parameter for evaluation
