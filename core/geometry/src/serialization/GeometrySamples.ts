...
export class Sample {
  ...
  public static createPoint3dLattice(low: number, step: number, high: number): Point3d[] {
    ...
    for (let z = low; z <= high; z += step)
      for (let y = low; y <= high; y += step)
        for (let x = low; x <= high; x += step) points.push(Point3d.create(x, y, z));
    return points;
  }
  /**
  ...
  public static createPoint2dLattice(low: number, step: number, high: number): Point2d[] {
    const points = [];
    for (let y = low; y <= high; y += step) for (let x = low; x <= high; x += step) points.push(Point2d.create(x, y));
    return points;
  }
  ...
  public static pushMove(data: Point3d[], dx: number, dy: number, dz: number = 0.0) {
    ...
    if (dx !== 0 || dy !== 0 || dz !== 0) data.push(Point3d.create(back.x + dx, back.y + dy, back.z + dz));
  }
  ...
  public static createClipPlaneSets(): UnionOfConvexClipPlaneSets[] {
    ...
    result.push(UnionOfConvexClipPlaneSets.createConvexSets([quadrant1.clone(), quadrant4.clone()]));
    result.push(UnionOfConvexClipPlaneSets.createConvexSets([quadrant1.clone(), quadrant2.clone(), quadrant4.clone()]));
    return result;
  }
  /**
  ...
  public static createBsplineCurves(includeMultipleKnots: boolean = false): BSplineCurve3d[] {
    ...
    if (includeMultipleKnots) {
      ...
      for (const order of [3, 4]) {
        const numPoints = 8;
        const points = [];
        for (let i = 0; i < numPoints; i++) points.push(Point3d.create(i, i * i, 0));
        const knots = [];
  ...
  public static createBspline3dHCurves(): BSplineCurve3dH[] {
    ...
    for (const weightVariation of [0, 0.125]) {
      for (const order of [2, 3, 4, 5]) {
        ...
        for (const x of [0, 1, 2, 3, 4, 5, 7]) {
          points.push(
            Point4d.create(x, yScale * (1 + x * x), 0.0, 1.0 + weightVariation * Math.sin(x * Math.PI * 0.25))
          );
        }
  ...
  public static createRay(x: number, y: number, z: number, u: number, v: number, w: number): Ray3d {
    return Ray3d.create(Point3d.create(x, y, z), Vector3d.create(u, v, w).normalize() as Vector3d);
  }
  ...
  public static createLineStrings(): LineString3d[] {
    return [
      LineString3d.createPoints([Point3d.create(0, 0, 0), Point3d.create(1, 0, 0)]),
      LineString3d.createPoints([Point3d.create(0, 0, 0), Point3d.create(1, 0, 0), Point3d.create(1, 1, 0)]),
  ...
  public static createMatrix3dArray(): Matrix3d[] {
    return [
      ...
      Matrix3d.createRotationAroundVector(Vector3d.create(1, 0, 0), Angle.createDegrees(10)) as Matrix3d,
      Matrix3d.createRotationAroundVector(Vector3d.create(1, -2, 5), Angle.createDegrees(-6.0)) as Matrix3d,
      Matrix3d.createRotationAroundVector(Vector3d.create(1, 2, 3), Angle.createDegrees(49.0)) as Matrix3d,
  ...
  public static createInvertibleTransforms(): Transform[] {
    return [
      ...
      Transform.createFixedPointAndMatrix(
        ...
        Matrix3d.createRotationAroundVector(Vector3d.create(1, 2, 3), Angle.createRadians(10)) as Matrix3d
      ),
    ];
  }
  /**
  ...
  public static createRigidTransforms(distanceScale: number = 4.0): Transform[] {
    ...
    return [
      ...
      Transform.createFixedPointAndMatrix(
        Point3d.create(0, 0, 0),
        Matrix3d.createRotationAroundVector(Vector3d.unitY(), Angle.createDegrees(10)) as Matrix3d
      ),
      Transform.createFixedPointAndMatrix(
        Point3d.create(distanceScale4 * 4, distanceScale4 * 1, -distanceScale4 * 2),
        Matrix3d.createRotationAroundVector(Vector3d.create(1, 2, 3), Angle.createDegrees(10)) as Matrix3d
      ),
      Transform.createFixedPointAndMatrix(
        Point3d.create(distanceScale4 * 4, distanceScale4 * 1, -distanceScale4 * 2),
        Matrix3d.createRotationAroundVector(Vector3d.create(-2, 1, 4), Angle.createDegrees(35)) as Matrix3d
      ),
    ];
  }
  ...
  public static createRigidAxes(): Matrix3d[] {
    return [
      Matrix3d.createIdentity(),
      Matrix3d.createRotationAroundVector(Vector3d.unitX(), Angle.createDegrees(10)) as Matrix3d,
      Matrix3d.createRotationAroundVector(Vector3d.unitY(), Angle.createDegrees(10)) as Matrix3d,
      Matrix3d.createRotationAroundVector(Vector3d.unitZ(), Angle.createDegrees(10)) as Matrix3d,
      Matrix3d.createRotationAroundVector(Vector3d.create(1, 2, 3), Angle.createDegrees(10)) as Matrix3d,
    ];
  }
  /**
  ...
  public static createMatrix4ds(includeIrregular: boolean = false): Matrix4d[] {
    const result = [];
    let transform;
    for (transform of Sample.createInvertibleTransforms()) result.push(Matrix4d.createTransform(transform));
    if (includeIrregular) {
  ...
  public static createBagOfCurves(): BagOfCurves[] {
    ...
    const result = [
      ...
      BagOfCurves.create(
        LineString3d.create(Point3d.create(0, 0, 0), Point3d.create(1, 1, 0), Point3d.create(2, 1, 0))
      ),
    ];
    return result;
  }
  ...
  public static createSimpleLinearSweeps(): LinearSweep[] {
    ...
    const xyPoints = [Point2d.create(0, 0), Point2d.create(1, 0), Point2d.create(1, 1)];
  ...
  public static createTorusPipes(): TorusPipe[] {
    ...
    const frame = Matrix3d.createRotationAroundVector(Vector3d.create(1, 2, 3), Angle.createRadians(10)) as Matrix3d;
    ...
    result.push(
      TorusPipe.createDgnTorusPipe(center, vectorY, vectorZ, 10, 1, Angle.createDegrees(45), true) as TorusPipe
    );
    return result;
  }
  ...
  public static createBoxes(capped: boolean = true): Box[] {
    ...
    const bY = 1.0;
    const h = 5.0;
    const frame = Matrix3d.createRotationAroundVector(Vector3d.create(0, 0, 1), Angle.createDegrees(10)) as Matrix3d;
    const vectorX = frame.columnX();
    const vectorY = frame.columnY();
    const cornerB = Matrix3d.xyzPlusMatrixTimesCoordinates(cornerA, frame, 0, 0, h);
    result.push(Box.createDgnBox(cornerA, Vector3d.unitX(), Vector3d.unitY(), cornerB, aX, aY, aX, aY, capped) as Box);
    result.push(Box.createDgnBox(cornerA, Vector3d.unitX(), Vector3d.unitY(), cornerB, aX, aY, bX, bY, capped) as Box);
    result.push(Box.createDgnBox(cornerA, vectorX, vectorY, cornerB, aX, aY, bX, bY, capped) as Box);
    const frameY = Matrix3d.createRotationAroundVector(Vector3d.create(0, 1, 0), Angle.createDegrees(10)) as Matrix3d;
    result.push(
      Box.createDgnBox(
        cornerA,
        frameY.columnX(),
  ...
  private static pushIfDistinct(points: Point3d[], xyz: Point3d, tol: number = 1.0e-12) {
    if (points.length === 0 || points[points.length - 1].distanceXY(xyz) > tol) points.push(xyz);
  }
  ...
  public static nonConvexQuadSimpleFractal(numRecursion: number, perpendicularFactor: number): Point3d[] {
    const pattern: Point2d[] = [Point2d.create(), Point2d.create(0.5, 0.1), Point2d.create(1.0, 0.0)];
    const poles: Point3d[] = [
      Point3d.create(0, 0, 0),
  ...
  public static appendSplits(points: Point3d[], target: Point3d, numSplit: number, includeTarget: boolean) {
    ...
    for (let i = 0; i < numSplit; i++) points.push(pointA.interpolate(i / numSplit, target));
  ...
  public static createCurveChainWithDistanceIndex(): CurveChainWithDistanceIndex[] {
    ...
    result.push(
      CurveChainWithDistanceIndex.createCapture(
        Path.create(
          Arc3d.createCircularStartMiddleEnd(Point3d.create(0, 0, 0), Point3d.create(3, 3, 0), Point3d.create(6, 0, 0))!
        )
      )!
    );
    result.push(CurveChainWithDistanceIndex.createCapture(Path.create(LineString3d.create(pointsA)))!);
    result.push(
      CurveChainWithDistanceIndex.createCapture(Path.create(BSplineCurve3d.createUniformKnots(pointsA, 3)!))!
    );
    result.push(
      CurveChainWithDistanceIndex.createCapture(Path.create(BSplineCurve3d.createUniformKnots(pointsA, 4)!))!
    );
    result.push(
      CurveChainWithDistanceIndex.createCapture(
        Path.create(
      ...
      CurveChainWithDistanceIndex.createCapture(
        Path.create(
          DirectSpiral3d.createDirectHalfCosine(
            Transform.createOriginAndMatrix(
              Point3d.createZero(),
              Matrix3d.createRotationAroundAxisIndex(2, Angle.createDegrees(110))
            ),
            50,
            350,
            Segment1d.create(0, 1)
  ...
  public static createClosedSolidSampler(
    ...
    if (!capped) result.push(RotationalSweep.create(contourZ.clone()!, axis.clone(), rotationAngle, false)!);
  ...
  public static createCutPie(
    ...
    for (let i = 1; i < numRadialEdges; i++) points.push(center.interpolate(i / numRadialEdges, pointA));
    points.push(pointA);
    ...
    points.push(pointB);
    for (let i = 1; i < numRadialEdges; i++) points.push(pointB.interpolate(i / numRadialEdges, center));
  ...
  public static createAllGeometryQueryTypes(): GeometryQuery[] {
    ...
    result.push(
      this.createTriangularUnitGridPolyface(pointA, Vector3d.unitX(), Vector3d.unitY(), 4, 5, true, true, true, false)
    );
