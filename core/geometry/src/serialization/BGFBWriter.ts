...
export class BGFBWriter {
  ...
  public writeInterpolationCurve3dAsFBVariantGeometry(curve: InterpolationCurve3d): number | undefined {
    ...
    if (props.order) BGFBAccessors.InterpolationCurve.addOrder(this.builder, props.order);
    if (props.closed) BGFBAccessors.InterpolationCurve.addClosed(this.builder, props.closed);
    if (props.isChordLenKnots) BGFBAccessors.InterpolationCurve.addIsChordLenKnots(this.builder, props.isChordLenKnots);
    ...
    if (knotOffset !== 0) BGFBAccessors.InterpolationCurve.addKnots(this.builder, knotOffset);
  ...
  public writeCurvePrimitiveAsFBVariantGeometry(curvePrimitive: CurvePrimitive): number | undefined {
    if (curvePrimitive instanceof LineSegment3d) {
      ...
      const transitionTableOffset = BGFBAccessors.TransitionSpiral.createTransitionSpiral(
        this.builder,
        spiralDetailOffset,
        0,
        0
  ...
  public writeGeometryQueryArrayAsFBVariantGeometry(
    ...
    if (Array.isArray(allGeometry)) {
      ...
      if (allOffsets.length > 0) {
        ...
        const vectorOffset = BGFBAccessors.VectorOfVariantGeometry.createVectorOfVariantGeometry(
          this.builder,
          membersOffset
        );
        return BGFBAccessors.VariantGeometry.createVariantGeometry(
    ...
    } else if (allGeometry instanceof GeometryQuery) return this.writeGeometryQueryAsFBVariantGeometry(allGeometry);
    return undefined;
  }
  /**
