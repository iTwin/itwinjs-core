...
export class BGFBWriter {
   ...
   */
  public writeDoubleArray(
    data: Float64Array | number[] | undefined,
    count?: number
  ): number {
    if (data === undefined) return 0;
    let numFloats = data.length;
    if (numFloats === 0) return 0;
    if (count !== undefined && count < numFloats) numFloats = count;
  ...
  public writeCurveCollectionAsFBCurveVector(
    cv: CurveCollection
  ): number | undefined {
    ...
    for (const child of cv.children!) {
      if (child instanceof CurvePrimitive) {
        ...
        if (childOffset) childrenOffsets.push(childOffset);
    ...
    const childrenVectorOffset = BGFBAccessors.CurveVector.createCurvesVector(
      this.builder,
      childrenOffsets
    );
    let cvType = 0;
    ...
    const curveVectorOffset = BGFBAccessors.CurveVector.createCurveVector(
      this.builder,
      cvType,
      childrenVectorOffset
    );
    return curveVectorOffset;
  }

  public writeCurveCollectionAsFBVariantGeometry(
    cv: CurveCollection
  ): number | undefined {
    const curveVectorOffset = this.writeCurveCollectionAsFBCurveVector(cv);
    if (curveVectorOffset === undefined) return undefined;
    return BGFBAccessors.VariantGeometry.createVariantGeometry(
      this.builder,
      BGFBAccessors.VariantGeometryUnion.tagCurveVector,
      curveVectorOffset,
      0
    );
  }

  public writeInterpolationCurve3dAsFBVariantGeometry(
    curve: InterpolationCurve3d
  ): number | undefined {
    const props = curve.cloneProps();
    const fitPointsOffset = this.writeDoubleArray(
      curve.copyFitPointsFloat64Array()
    );
    ...
    BGFBAccessors.InterpolationCurve.addFitPoints(
      this.builder,
      fitPointsOffset
    );
    if (props.order)
    ...
    if (props.isChordLenKnots)
      BGFBAccessors.InterpolationCurve.addIsChordLenKnots(
        this.builder,
        props.isChordLenKnots
      );
    if (props.isColinearTangents)
      BGFBAccessors.InterpolationCurve.addIsColinearTangents(
        this.builder,
        props.isColinearTangents
      );
    if (props.isChordLenKnots)
      BGFBAccessors.InterpolationCurve.addIsChordLenKnots(
        this.builder,
    ...
    if (props.isNaturalTangents)
      BGFBAccessors.InterpolationCurve.addIsNaturalTangents(
        this.builder,
        props.isNaturalTangents
      );
    if (props.startTangent !== undefined) {
      ...
      BGFBAccessors.InterpolationCurve.addStartTangent(
        this.builder,
        startTangentOffset
      );
    }
    if (props.endTangent !== undefined) {
      const endTangentOffset = BGFBAccessors.DPoint3d.createDPoint3d(
        this.builder,
        XYZ.x(props.endTangent),
        XYZ.y(props.endTangent),
        XYZ.z(props.endTangent)
      );
      BGFBAccessors.InterpolationCurve.addEndTangent(
        this.builder,
        endTangentOffset
      );
    }
    ...
    const headerOffset = BGFBAccessors.InterpolationCurve.endInterpolationCurve(
      this.builder
    );
    return BGFBAccessors.VariantGeometry.createVariantGeometry(
      this.builder,
  ...
  public writeAkimaCurve3dAsFBVariantGeometry(
    curve: AkimaCurve3d
  ): number | undefined {
    const fitPointsOffset = this.writeDoubleArray(
      curve.copyFitPointsFloat64Array()
    ...
    return BGFBAccessors.VariantGeometry.createVariantGeometry(
      this.builder,
      BGFBAccessors.VariantGeometryUnion.tagAkimaCurve,
      headerOffset,
      0
    );
  }

  public writeBsplineCurve3dAsFBVariantGeometry(
    ...
    if (polesOffset === undefined) return undefined;
    ...
    return BGFBAccessors.VariantGeometry.createVariantGeometry(
      this.builder,
      BGFBAccessors.VariantGeometryUnion.tagBsplineCurve,
      headerOffset,
      0
    );
  }

  public writeBSplineSurfaceAsFBVariantGeometry(
    bsurf: BSplineSurface3d | BSplineSurface3dH
  ): number | undefined {
    ...
    return BGFBAccessors.VariantGeometry.createVariantGeometry(
      this.builder,
      BGFBAccessors.VariantGeometryUnion.tagBsplineSurface,
      headerOffset,
      0
    );
  }

  public writeBsplineCurve3dAHsFBVariantGeometry(
    bcurve: BSplineCurve3dH
  ): number | undefined {
    const order = bcurve.order;
    const closed = false; // typescript bcurves are not closed.  There is API to impose wrapping . . .
    const polesOffset = this.writeDoubleArray(
      bcurve.copyXYZFloat64Array(false)
    );
    const weightsOffset = this.writeDoubleArray(
      bcurve.copyWeightsFloat64Array()
    );
    const knotsOffset = this.writeDoubleArray(bcurve.copyKnots(true));
    const headerOffset = BGFBAccessors.BsplineCurve.createBsplineCurve(
      this.builder,
  ...
  public writeCurvePrimitiveAsFBVariantGeometry(
    ...
    if (curvePrimitive instanceof LineSegment3d) {
      ...
      const lineSegmentOffset = BGFBAccessors.LineSegment.createLineSegment(
        this.builder,
        segmentDataOffset
      );
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
      ...
      const arcOffset = BGFBAccessors.EllipticArc.createEllipticArc(
        this.builder,
        arcDataOffset
      );
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
        BGFBAccessors.VariantGeometryUnion.tagEllipticArc,
        arcOffset,
        0
      );
      ...
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
        BGFBAccessors.VariantGeometryUnion.tagLineString,
        lineStringOffset,
        0
      );
      ...
      const typeCode = DgnSpiralTypeQueries.stringToTypeCode(
        curvePrimitive.spiralType,
        true
      )!;
      ...
      const transitionTableOffset =
        BGFBAccessors.TransitionSpiral.createTransitionSpiral(
          this.builder,
          spiralDetailOffset,
          0,
          0
  ...
  public writePointString3dAsFBVariantGeometry(
    pointString: PointString3d
  ): number | undefined {
    if (pointString instanceof PointString3d) {
      ...
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
        BGFBAccessors.VariantGeometryUnion.tagPointString,
        headerOffset,
        0
      );
    }
  ...
  public writeSolidPrimitiveAsFBVariantGeometry(
    ...
    if (solid instanceof Box) {
      ...
      const carrierOffset = BGFBAccessors.DgnBox.createDgnBox(
        this.builder,
        detailOffset
      );
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
        BGFBAccessors.VariantGeometryUnion.tagDgnBox,
        carrierOffset,
        0
      );
      ...
      const carrierOffset = BGFBAccessors.DgnSphere.createDgnSphere(
        this.builder,
        detailOffset
      );
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
        BGFBAccessors.VariantGeometryUnion.tagDgnSphere,
        carrierOffset,
        0
      );
      ...
      const carrierOffset = BGFBAccessors.DgnCone.createDgnCone(
        this.builder,
        detailOffset
      );
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
        BGFBAccessors.VariantGeometryUnion.tagDgnCone,
        carrierOffset,
        0
      );
      ...
      const detailOffset =
        BGFBAccessors.DgnTorusPipeDetail.createDgnTorusPipeDetail(
          this.builder,
          center.x,
          center.y,
          center.z,
      ...
      const carrierOffset = BGFBAccessors.DgnTorusPipe.createDgnTorusPipe(
        this.builder,
        detailOffset
      );
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
        BGFBAccessors.VariantGeometryUnion.tagDgnTorusPipe,
        carrierOffset,
        0
      );
    } else if (solid instanceof LinearSweep) {
      const baseCurveOffset = this.writeCurveCollectionAsFBCurveVector(
        solid.getSweepContourRef().getCurves()
      )!;
      ...
      const extrusionVectorOffset = BGFBAccessors.DVector3d.createDVector3d(
        this.builder,
        sweepVector.x,
        sweepVector.y,
        sweepVector.z
      );
      BGFBAccessors.DgnExtrusion.addExtrusionVector(
        this.builder,
        extrusionVectorOffset
      );
      BGFBAccessors.DgnExtrusion.addCapped(this.builder, solid.capped);
      const dgnExtrusionOffset = BGFBAccessors.DgnExtrusion.endDgnExtrusion(
        this.builder
      );
      ...
      BGFBAccessors.DgnRotationalSweep.addBaseCurve(
        this.builder,
        baseCurveOffset
      );
      ...
      BGFBAccessors.DgnRotationalSweep.addSweepRadians(
        this.builder,
        sweepAngle.radians
      );
      BGFBAccessors.DgnRotationalSweep.addCapped(this.builder, solid.capped);
      const dgnRotationalSweepOffset =
        BGFBAccessors.DgnRotationalSweep.endDgnRotationalSweep(this.builder);
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
      ...
      for (const contour of contours) {
        const contourOffset = this.writeCurveCollectionAsFBCurveVector(
          contour.getCurves()
        );
        if (contourOffset !== undefined) contourOffsets.push(contourOffset);
      }
      const contoursVectorOffset =
        BGFBAccessors.DgnRuledSweep.createCurvesVector(
          this.builder,
          contourOffsets
        );
      const ruledSweepTable = BGFBAccessors.DgnRuledSweep.createDgnRuledSweep(
        this.builder,
        contoursVectorOffset,
        solid.capped
      );
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
  ...
  public writePolyfaceAuxChannelDataAsFBVariantGeometry(
    channelData: AuxChannelData
  ): number | undefined {
    if (channelData instanceof AuxChannelData) {
      const valuesOffset =
        BGFBAccessors.PolyfaceAuxChannelData.createValuesVector(
          this.builder,
          channelData.values
        );
      return BGFBAccessors.PolyfaceAuxChannelData.createPolyfaceAuxChannelData(
        this.builder,
        channelData.input,
        valuesOffset
      );
  ...
  public writePolyfaceAuxChannelAsFBVariantGeometry(
    channel: AuxChannel
  ): number | undefined {
    if (channel instanceof AuxChannel) {
      const channelDataOffsets: number[] = [];
      for (const channelData of channel.data) {
        channelDataOffsets.push(
          this.writePolyfaceAuxChannelDataAsFBVariantGeometry(channelData)!
        );
      }
      const valuesOffset = BGFBAccessors.PolyfaceAuxChannel.createDataVector(
        this.builder,
        channelDataOffsets
      );
      const nameOffset = channel.name
        ? this.builder.createString(channel.name)
        : 0;
      const inputNameOffset = channel.inputName
        ? this.builder.createString(channel.inputName)
        : 0;
      return BGFBAccessors.PolyfaceAuxChannel.createPolyfaceAuxChannel(
        this.builder,
        channel.dataType,
        nameOffset,
        inputNameOffset,
  ...
  public writePolyfaceAuxDataAsFBVariantGeometry(
    data: PolyfaceAuxData
  ): number | undefined {
    if (data instanceof PolyfaceAuxData) {
      const channelOffsets: number[] = [];
      for (const channel of data.channels) {
        channelOffsets.push(
          this.writePolyfaceAuxChannelAsFBVariantGeometry(channel)!
        );
      }
      ...
      const indicesOffset = BGFBAccessors.PolyfaceAuxData.createIndicesVector(
        this.builder,
        data.indices
      );
      return BGFBAccessors.PolyfaceAuxData.createPolyfaceAuxData(
        this.builder,
        indicesOffset,
  ...
  public writeTaggedNumericDataArray(
    data: TaggedNumericData | undefined
  ): number {
    if (data) {
      const intDataOffset = this.writeIntArray(data.intData);
      const doubleDataOffset = this.writeDoubleArray(data.doubleData);
      return BGFBAccessors.TaggedNumericData.createTaggedNumericData(
        this.builder,
        data.tagA,
        data.tagB,
        intDataOffset,
  ...
  public writePolyfaceAsFBVariantGeometry(
    ...
    if (mesh instanceof IndexedPolyface) {
      ...
      copyToPackedNumberArray(
        numberArray,
        mesh.data.point.float64Data(),
        mesh.data.point.float64Length
      );
      const pointOffset = BGFBAccessors.Polyface.createPointVector(
        this.builder,
        numberArray
      );
      ...
      const numPerFace = 0;
      this.fillOneBasedIndexArray(
        mesh,
        mesh.data.pointIndex,
        mesh.data.edgeVisible,
        0,
        indexArray
      );

      const twoSided = mesh.twoSided;
      const pointIndexOffset = BGFBAccessors.Polyface.createPointIndexVector(
        this.builder,
        indexArray
      );
      if (
        mesh.data.paramIndex !== undefined &&
        mesh.data.paramIndex.length > 0
      ) {
        this.fillOneBasedIndexArray(
          mesh,
          mesh.data.paramIndex,
          undefined,
          0,
          indexArray
        );
        paramIndexOffset = BGFBAccessors.Polyface.createParamIndexVector(
          this.builder,
          indexArray
        );
      }
      ...
      if (
        ...
        mesh.data.normalIndex.length > 0
      ) {
        this.fillOneBasedIndexArray(
          mesh,
          mesh.data.normalIndex,
          undefined,
          0,
          indexArray
        );
        normalIndexOffset = BGFBAccessors.Polyface.createNormalIndexVector(
          this.builder,
          indexArray
        );
      }
      ...
      if (
        ...
        mesh.data.colorIndex.length > 0
      ) {
        this.fillOneBasedIndexArray(
          mesh,
          mesh.data.colorIndex,
          undefined,
          0,
          indexArray
        );
        colorIndexOffset = BGFBAccessors.Polyface.createColorIndexVector(
          this.builder,
          indexArray
        );
      }
      ...
      if (mesh.data.color !== undefined && mesh.data.color.length > 0) {
        intColorOffset = BGFBAccessors.Polyface.createIntColorVector(
          this.builder,
          mesh.data.color
        );
      }
      ...
      if (mesh.data.normal) {
        copyToPackedNumberArray(
          numberArray,
          mesh.data.normal.float64Data(),
          mesh.data.normal.float64Length
        );
        normalOffset = BGFBAccessors.Polyface.createNormalVector(
          this.builder,
          numberArray
        );
      }
      ...
      if (mesh.data.param) {
        copyToPackedNumberArray(
          numberArray,
          mesh.data.param.float64Data(),
          mesh.data.param.float64Length
        );
        paramOffset = BGFBAccessors.Polyface.createPointVector(
          this.builder,
          numberArray
        );
      }
      ...
      if (mesh.data.auxData) {
        auxDataOffset = this.writePolyfaceAuxDataAsFBVariantGeometry(
          mesh.data.auxData
        )!;
      }

      if (mesh.data.taggedNumericData)
        taggedNumericDataOffset = this.writeTaggedNumericDataArray(
          mesh.data.taggedNumericData
        );
      ...
      return BGFBAccessors.VariantGeometry.createVariantGeometry(
        this.builder,
        BGFBAccessors.VariantGeometryUnion.tagPolyface,
        polyfaceOffset,
        0
      );
    }
  ...
  public fillOneBasedIndexArray(
    ...
    for (let facetIndex = 0; facetIndex < numFacet; facetIndex++) {
      ...
      for (let k = k0; k < k1; k++) {
        ...
        if (visible !== undefined && !visible[k]) q = -q;
        destIndex.push(q);
      }
      if (facetTerminator !== undefined) destIndex.push(facetTerminator);
    }
  }
  public writeGeometryQueryAsFBVariantGeometry(
    g: GeometryQuery
  ): number | undefined {
    let offset: number | undefined;
    if (
      g instanceof CurvePrimitive &&
      (offset = this.writeCurvePrimitiveAsFBVariantGeometry(g)) !== undefined
    )
      return offset;
    if (
      g instanceof CurveCollection &&
      (offset = this.writeCurveCollectionAsFBVariantGeometry(g)) !== undefined
    )
      return offset;
    if (
      g instanceof IndexedPolyface &&
      (offset = this.writePolyfaceAsFBVariantGeometry(g)) !== undefined
    )
      return offset;
    if (
      g instanceof SolidPrimitive &&
      (offset = this.writeSolidPrimitiveAsFBVariantGeometry(g)) !== undefined
    )
      return offset;
    if (
      g instanceof BSplineSurface3d &&
      (offset = this.writeBSplineSurfaceAsFBVariantGeometry(g)) !== undefined
    )
      return offset;
    if (
      g instanceof BSplineSurface3dH &&
      (offset = this.writeBSplineSurfaceAsFBVariantGeometry(g)) !== undefined
    )
      return offset;
    if (
      g instanceof PointString3d &&
      (offset = this.writePointString3dAsFBVariantGeometry(g)) !== undefined
    )
      return offset;
  ...
  public writeGeometryQueryArrayAsFBVariantGeometry(
    allGeometry: GeometryQuery | GeometryQuery[] | undefined
  ): number | undefined {
    if (Array.isArray(allGeometry)) {
      const allOffsets: number[] = [];
      for (const g of allGeometry) {
        const offset = this.writeGeometryQueryAsFBVariantGeometry(g);
        if (offset !== undefined) allOffsets.push(offset);
      }
      if (allOffsets.length > 0) {
        const membersOffset =
          BGFBAccessors.VectorOfVariantGeometry.createMembersVector(
            this.builder,
            allOffsets
          );
        const vectorOffset =
          BGFBAccessors.VectorOfVariantGeometry.createVectorOfVariantGeometry(
            this.builder,
            membersOffset
          );
        return BGFBAccessors.VariantGeometry.createVariantGeometry(
  ...
  public static geometryToBytes(
    ...
    if (rootOffset !== undefined) {
      ...
      if (!signatureBytes) {
        ...
        let i = i0;
        for (const k of signatureBytes) buffer.bytes()[i++] = k;
...
function extractNumberArray(data: GrowableXYZArray | Point3d[]): number[] {
  ...
  if (data instanceof GrowableXYZArray) {
    ...
    for (let i = 0; i < numCoordinate; i++) result.push(source[i]);
    return result;
  } else if (Array.isArray(data)) {
    for (const xyz of data) result.push(xyz.x, xyz.y, xyz.z);
  }
  return result;
}
/** Copy the active data to a simple number array. */
function copyToPackedNumberArray(
  dest: number[],
  source: Float64Array,
  count: number
) {
  dest.length = 0;
  for (let i = 0; i < count; i++) dest.push(source[i]);
}
