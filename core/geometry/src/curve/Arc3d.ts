...
export class Arc3d extends CurvePrimitive implements BeJSONFunctions {
  ...
  public static createCircularStartMiddleEnd(
    ...
    const ac = vectorAC.magnitude();
    const normal = vectorAB.sizedCrossProduct(vectorAC, Math.sqrt(ab * ac));
    if (normal) {
      const vectorToCenter = SmallSystem.linearSystem3d(
        ...
        0.5 * ab * ab, // vectorToCenter DOT vectorAB = 0.5 * vectorAB DOT vectorAB (Rayleigh quotient)
        0.5 * ac * ac, // vectorToCenter DOT vectorAC = 0.5 * vectorAC DOT vectorAC (Rayleigh quotient)
      );
      if (vectorToCenter) {
