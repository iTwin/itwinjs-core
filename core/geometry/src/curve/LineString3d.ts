...
public override rangeBetweenFractions(fraction0: number, fraction1: number): Range3d {
  const range = Range3d.create ();
  if (fraction1 === fraction0 || this.points.length < 1)
    return range;
  const numSegments = this._points.length - 1;
  if (fraction1 < fraction0)
    return this.rangeBetweenFractions(fraction1, fraction0);
  const scaledFraction0 = fraction0 * numSegments;
  const scaledFraction1 = fraction1 * numSegments;
