...
export class ConsolidateAdjacentCurvePrimitivesContext extends NullGeometryHandler {
  ...
  public constructor(options?: ConsolidateAdjacentCurvePrimitivesOptions) {
    ...
    this._options = options
      ? options
      : new ConsolidateAdjacentCurvePrimitivesOptions();
  }
  ...
  public handleCurveChain(g: CurveChain) {
    ...
    for (let i0 = 0; i0 < numOriginal; ) {
      ...
      if (
        this._options.consolidateLinearGeometry &&
        (basePrimitive instanceof LineSegment3d ||
          basePrimitive instanceof LineString3d)
      ) {
        points.length = 0;
        let i1 = i0;
        ...
        if (points.length > 1) {
          ...
          const compressedPointsA = PolylineOps.compressShortEdges(
            points,
            tolerance
          );
          const compressedPointsB = PolylineOps.compressByPerpendicularDistance(
            compressedPointsA,
            tolerance
          );
          ...
          if (compressedPointsB.length < 2) {
            ...
            g.children[numAccept++] = LineSegment3d.create(
              compressedPointsB[0],
              compressedPointsB[1]
            );
          } else {
            g.children[numAccept++] =
              LineString3d.createPoints(compressedPointsB);
          }
        } else {
        ...
        this._options.consolidateCompatibleArcs &&
        basePrimitive instanceof Arc3d
      ) {
        ...
        for (; ++i0 < g.children.length; ) {
          ...
          if (!(nextPrimitive instanceof Arc3d)) break;
  ...
  public override handleParityRegion(g: ParityRegion): any {
    for (const child of g.children) child.dispatchToGeometryHandler(this);
  }
