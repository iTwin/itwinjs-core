...
export class PlanarSubdivision {
  ...
  public static assembleHalfEdgeGraph(
    ...
    for (const entry of detailByPrimitive.primitiveToPair.entries()) {
      ...
      details.sort((pairA: CurveLocationDetailPair, pairB: CurveLocationDetailPair) => {
        ...
        if (fractionA === undefined || fractionB === undefined) return -1000.0;
  ...
  public static createLoopInFace(
    ...
    do {
      ...
      if (detail) {
        let curve;
        if (he.sortData! > 0) curve = detail.curve!.clonePartialCurve(detail.fraction, detail.fraction1!);
        else curve = detail.curve!.clonePartialCurve(detail.fraction1!, detail.fraction);
        if (curve) {
