 ...
 * @internal
 */
export class ClosestPointStrokeHandler
  ...
  implements IStrokeHandler
{
  ...
  public announceSegmentInterval(
    ...
    let localFraction = this._spacePoint.fractionOfProjectionToLine(
      point0,
      point1,
      0.0
    );
    ...
    const globalFraction = Geometry.interpolate(
      fraction0,
      localFraction,
      fraction1
    );
  ...
  private announceSolutionFraction(fraction: number) {
    if (this._curve)
      this.announceCandidate(
        this._curve,
        fraction,
        this._curve.fractionToPoint(fraction)
      );
  }
  ...
  public evaluate(fraction: number): boolean {
    ...
    if (curve) {
      this._workRay = curve.fractionToPointAndDerivative(
        fraction,
        this._workRay
      );
