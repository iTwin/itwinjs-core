...
class Joint {
  ...
  public nextJointFraction0(defaultValue: number): number {
    if (this.nextJoint && this.nextJoint.fraction0 !== undefined) return this.nextJoint.fraction0;
    return defaultValue;
  }
  ...
  private static adjustJointToPrimitives(joint: Joint) {
    ...
    if (ls instanceof LineString3d) {
      if (joint.curve0) {
        ...
        if (!curvePoint.isAlmostEqual(jointPoint0)) ls.packedPoints.setAtCheckedPointIndex(0, curvePoint);
      }
      if (joint.curve1) {
...
export class PolygonWireOffsetContext {
  ...
  public constructPolygonWireXYOffset(
    ...
    if (n > 1) {
      if (chain.packedPoints.front()!.isAlmostEqual(chain.packedPoints.back()!)) return Loop.create(chain);
