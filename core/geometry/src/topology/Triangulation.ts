...
export class Triangulator {
  ...
  public static triangulateAllPositiveAreaFaces(graph: HalfEdgeGraph): boolean {
    ...
    for (const face of seeds) {
      if (face.countEdgesAroundFace() > 3) {
        ...
        if (area > 0.0) if (!Triangulator.triangulateSingleFace(graph, face)) numFail++;
      }
    }
    return numFail === 0;
  }
  ...
  private static interiorEdgeSplit(
    ...
    if (Triangulator.isAlmostEqualXAndYXY(baseNode.faceSuccessor, x, y)) return baseNode;
  ...
  private static maskAndOrientNewFaceLoop(
    ...
    if (base) {
      ...
      if (maskForOtherSide !== HalfEdgeMask.NULL_MASK) otherNode.setMaskAroundFace(maskForOtherSide);
  ...
  private static isInteriorTriangle(a: HalfEdge) {
    if (!a.isMaskSet(HalfEdgeMask.TRIANGULATED_FACE) || a.isMaskSet(HalfEdgeMask.EXTERIOR)) return false;
    const b = a.faceSuccessor;
    if (!b.isMaskSet(HalfEdgeMask.TRIANGULATED_FACE) || b.isMaskSet(HalfEdgeMask.EXTERIOR)) return false;
    const c = b.faceSuccessor;
    if (!c.isMaskSet(HalfEdgeMask.TRIANGULATED_FACE) || c.isMaskSet(HalfEdgeMask.EXTERIOR)) return false;
      ...
      if (earRange.intersectsRange(edgeRange)) {
        ...
        if (!edgeInterval.isNull) {
          ...
          if (mate === a || mate === b) {
            ...
            if (!a.findAroundVertex(q) && !b.findAroundVertex(q) && !c.findAroundVertex(q)) return false;
          } else if (edgeInterval.high < zeroPlus) {
            // the start (p) just touches ... if it is at one of the vertex loops it's ok ...
            if (!a.findAroundVertex(p) && !b.findAroundVertex(p) && !c.findAroundVertex(p)) return false;
          } else {
  ...
  public static triangulateSingleMonotoneFace(graph: HalfEdgeGraph, start: HalfEdge): boolean {
    ...
    while (left !== right && right !== start && right.faceSuccessor !== left) {
      ...
      if (HalfEdge.crossProductXYAlongChain(left, start, right) <= 0) return false;
      ...
      if (left.belowYX(right)) {
        ...
        while (P0 !== P2 && P0.belowYX(right)) {
          while (P2 !== right && P2 !== P0 && P2 !== P1 && HalfEdge.crossProductXYAlongChain(P0, P1, P2) > 0) {
        ...
        while (P0 !== P2 && P2.belowYX(left)) {
          while (P0 !== left && P2 !== P0 && P2 !== P1 && HalfEdge.crossProductXYAlongChain(P0, P1, P2) > 0) {
            upperSideOfNewEdge = Triangulator.splitFace(graph, P0, P2);
            if (upperSideOfNewEdge === undefined) return false;
