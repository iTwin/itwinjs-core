...
export class PointSearchContext {
  ...
  public reAimFromEdge(edgeHit: HalfEdgePositionDetail, ray: Ray3d, targetDistance: number): HalfEdgePositionDetail {
    ...
    if (sideA * sideB < 0) {
      ...
      if (alongA === 0 && sideA === 0) {
        ...
        if (dataA.u > dataB.u) result = edgeHit.resetAsVertex(dataA.node);
        else result = edgeHit.resetAsVertex(dataB.node);
      } else {
  ...
  public reAimAroundFace(
    ...
    // ! CALLER CREATED -- reset as first hit on positive side of ray.
