...
export class BSpline1dNd {
  ...
  public addKnot(knot: number, totalMultiplicity: number): boolean {
    if (knot < this.knots.leftKnot || knot > this.knots.rightKnot) return false; // invalid input
    ...
    for (let iter = 0; iter < numKnotsToAdd; ++iter) {
      ...
      for (const d of dataBuf) newPackedData[iData++] = d; // overwrite degree new poles
