...
export class XYZ implements XYAndZ {
  ...
  public static x(xyz: XYZProps | undefined, defaultValue: number = 0): number {
    if (xyz === undefined) return defaultValue;
    if (Array.isArray(xyz)) return xyz[0];
    if (xyz.x !== undefined) return xyz.x;
    return defaultValue;
  }
  /** Access x part of XYZProps (which may be .x or [0]) */
  public static y(xyz: XYZProps | undefined, defaultValue: number = 0): number {
    if (xyz === undefined) return defaultValue;
    if (Array.isArray(xyz)) return xyz[1];
    if (xyz.y !== undefined) return xyz.y;
    return defaultValue;
  }
  /** Access x part of XYZProps (which may be .x or [0]) */
  public static z(xyz: XYZProps | undefined, defaultValue: number = 0): number {
    if (xyz === undefined) return defaultValue;
    if (Array.isArray(xyz)) return xyz[2];
    if (xyz.z !== undefined) return xyz.z;
    return defaultValue;
  }
}
...
export class Point3d extends XYZ {
  ...
  public static createFrom(data: XYAndZ | XAndY | Float64Array, result?: Point3d): Point3d {
    if (data instanceof Float64Array) {
      ...
      let y = 0;
      let z = 0;
      if (data.length > 0) x = data[0];
      if (data.length > 1) y = data[1];
      if (data.length > 2) z = data[2];
...
export class Vector3d extends XYZ {
   ...
   * * the magnitude of the vector is twice the area of the triangle.
   */
  public static createCrossProductToPoints(
    origin: XYAndZ,
    pointA: XYAndZ,
    pointB: XYAndZ,
    result?: Vector3d
   ...
   * * the magnitude of the vector is twice the area of the triangle.
   */
  public static createUnitCrossProductToPoints(
    origin: XYAndZ,
    pointA: XYAndZ,
    pointB: XYAndZ,
    result?: Vector3d
  ): Vector3d | undefined {
    const vector = Vector3d.createCrossProduct(
      pointA.x - origin.x,
      pointA.y - origin.y,
      pointA.z - origin.z,
      pointB.x - origin.x,
  ...
  public normalizeWithLength(result?: Vector3d): {
    ...
    return {
      v: this.safeDivideOrNull(correctedMagnitude, result),
      mag: originalMagnitude,
    };
  }
  /**
   ...
   */
  public static createAdd2ScaledXYZ(
    ax: number,
    ay: number,
    az: number,
    scaleA: number,
  ...
  public signedRadiansTo(vectorB: Vector3d, vectorW: Vector3d): number {
    ...
    if (vectorW.dotProduct(p) < 0.0) return -theta;
    else return theta;
  }
  /**
