...
export abstract class IndexedXYZCollection {
  ...
  public dotProductIndexIndexXYAndZ(
    ...
    const z0 = this.getZAtUncheckedPointIndex(origin);
    return (
      (this.getXAtUncheckedPointIndex(indexA) - x0) * (targetB.x - x0) +
   ...
   * @returns undefined if either index is out of bounds
   */
  public abstract crossProductXYAndZIndexIndex(
    origin: XYAndZ,
    indexA: number,
    indexB: number,
    result?: Vector3d
   ...
   */
  public crossProductIndexIndexXYAndZ(
    origin: number,
    indexA: number,
    targetB: XYAndZ,
    result?: Vector3d
   ...
   */
  public abstract accumulateCrossProductIndexIndexIndex(
    origin: number,
    indexA: number,
    indexB: number,
    result: Vector3d
    ...
    if (this.length > 1) {
      for (let i = 0; i < this.length - 1; ) {
        ...
        for (; j < this.length; ++j) {
          ...
          if (dist2 !== undefined && dist2 < tol2) indices.push(j);
          else break;
        }
