...
export class MomentData {
  ...
  public static sortColumnsForIncreasingMoments(
    ...
    if (axes.determinant() < 0) axes.scaleColumnsInPlace(-1.0, -1.0, -1.0);
    // prefer x and z positive -- y falls wherever . ..
    if (axes.at(0, 0) < 0.0) axes.scaleColumnsInPlace(-1.0, -1.0, 1.0);
    if (axes.at(2, 2) < 0.0) axes.scaleColumnsInPlace(1.0, -1.0, -1.0);
  ...
  public static inertiaProductsToPrincipalAxes(
    ...
    if (!moments.shiftOriginAndSumsToCentroidOfSums()) return undefined;
    const products = moments.sums.matrixPart();
    const w = moments.sums.weight();
    if (w < 0.0) products.scaleColumnsInPlace(-1, -1, -1);
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          if (zA.isParallelTo(zB, true)) return true;
          return false;
        }
        ...
        for (let i = 0; i < 3; i++) {
          ...
          if (!vectorA.isParallelTo(vectorB, true)) return false;
        }
        return true;
      }
    }
  ...
  public clearSums(origin?: Point3d) {
    this.sums.setZero();
    if (origin) this.origin.setFrom(origin);
    else this.origin.setZero();
  }
