...
export class Point4d extends Plane3d implements BeJSONFunctions {
  ...
  public clone(result?: Point4d): Point4d {
    return result
      ? result.setFrom(this)
      : new Point4d(this.xyzw[0], this.xyzw[1], this.xyzw[2], this.xyzw[3]);
  }
  ...
  public isAlmostEqual(other: Point4d): boolean {
    return (
      Geometry.isSameCoordinate(this.x, other.x) &&
  ...
  public realDistanceXY(other: Point4d): number | undefined {
    ...
    const wB = other.w;
    if (
      Geometry.isSmallMetricDistance(wA) ||
      Geometry.isSmallMetricDistance(wB)
    )
      return undefined;
  ...
  public crossWeightedMinusPoint3d(
    other: Point3d,
    result?: Vector3d
  ): Vector3d {
    const wa = this.xyzw[3];
    return Vector3d.create(
      this.xyzw[0] - wa * other.x,
      this.xyzw[1] - wa * other.y,
      this.xyzw[2] - wa * other.z,
      result
    );
  }
  ...
  public static createZero(): Point4d {
   ...
   */
  public static createPlanePointPointZ(
    pointA: Point4d,
    pointB: Point4d,
    result?: Point4d
  ) {
  ...
  public static createFromPoint(
    point: XAndY | XYAndZ | Point4d | number[]
  ): Point4d {
    if (point instanceof Point2d) return new Point4d(point.x, point.y, 0, 1);
    if (point instanceof Point3d)
      return new Point4d(point.x, point.y, point.z, 1);
    if (point instanceof Point4d) return point.clone();
  ...
  public plus3Scaled(
    ...
    return Point4d.create(
      this.xyzw[0] +
        vectorA.xyzw[0] * scalarA +
        vectorB.xyzw[0] * scalarB +
        vectorC.xyzw[0] * scalarC,
      this.xyzw[1] +
        vectorA.xyzw[1] * scalarA +
        vectorB.xyzw[1] * scalarB +
        vectorC.xyzw[1] * scalarC,
      this.xyzw[2] +
        vectorA.xyzw[2] * scalarA +
        vectorB.xyzw[2] * scalarB +
        vectorC.xyzw[2] * scalarC,
      this.xyzw[3] +
        vectorA.xyzw[3] * scalarA +
        vectorB.xyzw[3] * scalarB +
        vectorC.xyzw[3] * scalarC,
      result
    );
  }
  /** Return `point + vectorA * scalarA + vectorB * scalarB` */
  ...
  public dotVectorsToTargets(targetA: Point4d, targetB: Point4d): number {
    return (
      (targetA.xyzw[0] - this.xyzw[0]) * (targetB.xyzw[0] - this.xyzw[0]) +
  ...
  public dotProductXYZW(x: number, y: number, z: number, w: number): number {
    return (
      this.xyzw[0] * x + this.xyzw[1] * y + this.xyzw[2] * z + this.xyzw[3] * w
    );
  }
  ...
  public altitudeXYZ(x: number, y: number, z: number): number {
    return (
      this.xyzw[0] * x + this.xyzw[1] * y + this.xyzw[2] * z + this.xyzw[3]
    );
  }
  ...
  public velocity(vector: Vector3d): number {
    return (
      this.xyzw[0] * vector.x +
      this.xyzw[1] * vector.y +
      this.xyzw[2] * vector.z
    );
  }
  ...
  /** unit X vector */
  public static unitX(): Point4d {
    return new Point4d(1, 0, 0, 0);
  }
  /** unit Y vector */
  public static unitY(): Point4d {
    return new Point4d(0, 1, 0, 0);
  }
  /** unit Z vector */
  public static unitZ(): Point4d {
    return new Point4d(0, 0, 1, 0);
  }
  /** unit W vector */
  public static unitW(): Point4d {
  ...
  public projectPointToPlane(spacePoint: Point3d, result?: Point3d): Point3d {
    ...
    if (alpha === undefined) return spacePoint.clone(result);
  ...
  public realPointOrVector(): Point3d | Vector3d {
    const mag = Geometry.correctSmallFraction(this.xyzw[3]);
    if (mag === 0.0) return Vector3d.create(this.x, this.y, this.z);
    const a = 1.0 / mag; // in zero case everything multiplies right back to true zero.
   ...
   * @param result optional result
   */
  public static createRealPoint3dDefault000(
    x: number,
    y: number,
    z: number,
    w: number,
   ...
   * @param result optional result
   */
  public static createRealDerivativeRay3dDefault000(
    x: number,
    y: number,
    z: number,
    w: number,
   ...
   * @param result optional result
   */
  public static createRealDerivativePlane3dByOriginAndVectorsDefault000(
    x: number,
    y: number,
    z: number,
    w: number,
  ...
  public realPointDefault000(result?: Point3d): Point3d {
    const mag = Geometry.correctSmallFraction(this.xyzw[3]);
    if (mag === 0.0) return Point3d.create(0, 0, 0, result);
   ...
   */
  public static determinantIndexed3X3(
    pointA: Point4d,
    pointB: Point4d,
    pointC: Point4d,
    i: number,
   ...
   */
  public static perpendicularPoint4dPlane(
    pointA: Point4d,
    pointB: Point4d,
    pointC: Point4d
  ): Point4d {
  ...
  public toPlane3dByOriginAndUnitNormal(
  ...
  public static interpolateQuaternions(
    quaternion0: Point4d,
    fractionParameter: number,
    quaternion1: Point4d,
    result?: Point4d
  ): Point4d {
    if (!result) result = new Point4d();
    ...
    result = Point4d.createAdd2Scaled(
      q0,
      Math.cos(angleOfInterpolation),
      q2,
      Math.sin(angleOfInterpolation)
    );
    return result;
  }
