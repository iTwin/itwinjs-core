...
class FacetSector {
  ...
  public copyContentsFrom(other: FacetSector) {
    ...
    if (this.sectionDerivative) this.sectionDerivative.setFrom(other.sectionDerivative);
  }
  ...
  public loadIndexedPointAndDerivativeCoordinatesFromPackedArrays(
    ...
    if (fractions && v !== undefined) this.uv = Point2d.create(fractions.atUncheckedIndex(i), v);
    this.xyzIndex = -1;
    this.normalIndex = -1;
    this.uvIndex = -1;
...
export class PolyfaceBuilder extends NullGeometryHandler {
  ...
  public addTransformedRangeMesh(transform: Transform, range: Range3d, faceSelector?: boolean[]) {
    ...
    for (const p of points) this._polyface.addPoint(transform.multiplyPoint3d(p));
    let faceCounter = 0;
  ...
  public addTrianglesInUncheckedConvexPolygon(ls: LineString3d, toggle: boolean): void {
    ...
    if (n > 2) {
      ...
      for (let i = 2; i < numEdge; i++, pointIndex1 = pointIndex2, paramIndex1 = paramIndex2) {
        ...
        if (normalIndex !== undefined) this.addIndexedTriangleNormalIndexes(normalIndex, normalIndex, normalIndex);
        if (packedUV) {
  ...
  public findOrAddParamInLineString(
    ...
    const u =
      ls.fractions && index < ls.fractions.length ? ls.fractions.atUncheckedIndex(index) : index / ls.points.length;
  ...
  private setSectorIndices(sector: FacetSector) {
    ...
    if (sector.normal) sector.normalIndex = this._polyface.addNormal(sector.normal);
  ...
  public addBetweenLineStringsWithRuleEdgeNormals(
    ...
    if (needNormals) FacetSector.computeNormalsAlongRuleLine(sectorA0, sectorB0);
    ...
    if (addClosure) this.addSectorQuadA01B01(sectorA0, sectorA00, sectorB0, sectorB00);
  }
  ...
  public addBetweenLineStringsWithStoredIndices(lineStringA: LineString3d, lineStringB: LineString3d) {
    ...
    for (let i = 1; i < numPoints; i++) {
      if (this.options.shouldTriangulate) {
        ...
        if (
          distinctIndices(pointB.atUncheckedIndex(i), pointB.atUncheckedIndex(i - 1), pointA.atUncheckedIndex(i - 1))
        ) {
          this.addIndexedTrianglePointIndexes(
            pointA.atUncheckedIndex(i - 1),
            pointB.atUncheckedIndex(i),
  ...
  public addBetweenTransformedLineStrings(
    ...
    if (curves instanceof LineString3d) {
      ...
      if (addClosure) this.addIndexedQuadPointIndexes(indexA0, indexA00, indexB0, indexB00);
    } else {
  ...
  public addCone(cone: Cone) {
    ...
    let strokeCount = 16;
    if (this._options) strokeCount = this._options.applyTolerancesToArc(cone.getMaxRadius());
    ...
    if (StrokeCountSection.enforceStrokeCountCompatibility(sectionMaps)) {
      ...
      for (let i = 0; i < contours.length; i++) {
        ...
        if (i > 0 && stroke0 && stroke1) {
          ...
          if (
            StrokeCountSection.extendDistanceRangeBetweenStrokes(stroke0, stroke1, distanceRange) &&
  ...
  public addPolygon(points: Point3d[], numPointsToUse?: number) {
    ...
    while (numPointsToUse > 1 && points[numPointsToUse - 1].isAlmostEqual(points[0])) numPointsToUse--;
    let index = 0;
    if (!this._reversed) {
  ...
  public addPolygonGrowableXYZArray(points: GrowableXYZArray) {
    ...
    while (
      numPointsToUse > 2 &&
      Geometry.isSmallMetricDistance(points.distanceIndexIndex(numPointsToUse - 2, numPointsToUse - 1)!)
    )
      numPointsToUse--;
  ...
  public addFacetFromGrowableArrays(
    ...
    if (edgeVisible && edgeVisible.length < numPointsToUse) edgeVisible = undefined;
    if (!this._reversed) {
      for (let i = 0; i < numPointsToUse; i++) {
        index = this.findOrAddPointInGrowableXYZArray(points, i)!;
  ...
  public static polygonToTriangulatedPolyface(
    ...
    if (!localToWorld) localToWorld = FrameBuilder.createFrameWithCCWPolygon(points);
    if (localToWorld) {
  ...
  public addCoordinateFacets(
    ...
    for (let i = 0; i < pointArray.length; i++) {
      ...
      if (pointArray[i].length === 3) this.addTriangleFacet(pointArray[i], params, normals);
      else if (pointArray[i].length === 4) this.addQuadFacet(pointArray[i], params, normals);
    }
  ...
  private getEdgeIndices(edge: SortableEdge): { edgeIndexA: number; edgeIndexB: number } | undefined {
    ...
    for (let i = this._polyface.facetIndex0(edge.facetIndex); i < this._polyface.facetIndex1(edge.facetIndex); ++i) {
      ...
      else if (edge.vertexIndexB === this._polyface.data.pointIndex[i]) indexB = i;
    }
  ...
  private addSweptFace(baseBoundaryEdge: SortableEdge, numBaseFacets: number): boolean {
    ...
    if (numBaseFacetEdges !== this._polyface.numEdgeInFacet(sweptFacetIndex)) return false;
  ...
  public addSweptIndexedPolyface(
    ...
    if (Geometry.isAlmostEqualNumber(0.0, totalProjectedArea)) isSimpleSweep = false;
    ...
    for (const edgeOrCluster of baseBoundaryEdges) {
      if (edgeOrCluster instanceof SortableEdge) this.addSweptFace(edgeOrCluster, numBaseFacets);
      else if (Array.isArray(edgeOrCluster)) for (const edge of edgeOrCluster) this.addSweptFace(edge, numBaseFacets);
    }
