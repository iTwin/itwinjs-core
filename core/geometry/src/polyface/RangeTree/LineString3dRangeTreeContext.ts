  ...
  public static createCapture(points: Point3d[] | LineString3d, maxChildPerNode: number = 4, maxAppDataPerLeaf: number = 4): LineString3dRangeTreeContext | undefined {
    ...
    const rangeTreeRoot = RangeTreeOps.createByIndexSplits<number>(
      ((index: number): Range3d => {
        const p0 = linestring.pointAt(index);
        assert(undefined !== p0, "LineString3dRangeTreeContext.createCapture: pointAt(index) should be defined");
        const p1 = linestring.pointAt(index + 1);
        assert(undefined !== p1, "LineString3dRangeTreeContext.createCapture: pointAt(index + 1) should be defined");
        return Range3d.create(p0, p1);
      }),
