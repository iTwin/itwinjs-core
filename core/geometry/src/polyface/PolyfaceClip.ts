...
export class ClippedPolyfaceBuilders {
  ...
  private constructor(
    builderA: PolyfaceBuilder | undefined,
    builderB: PolyfaceBuilder | undefined,
    buildClosureFaces: boolean = false
  ) {
  ...
  public static create(
    ...
    buildSideFaces: boolean = false
  ) {
    return new ClippedPolyfaceBuilders(
      keepInside ? PolyfaceBuilder.create() : undefined,
      keepOutside ? PolyfaceBuilder.create() : undefined,
      buildSideFaces
    );
  }

  public claimPolyface(
    selector: 0 | 1,
    fixup: boolean
...
export class PolyfaceClip {
   ...
   */
  public static clipPolyfaceClipPlaneWithClosureFace(
    polyface: Polyface,
    clipper: ClipPlane,
    insideClip: boolean = true,
   ...
   */
  public static clipPolyfaceConvexClipPlaneSet(
    polyface: Polyface,
   ...
   * @internal
   */
  public static clipPolyfaceUnionOfConvexClipPlaneSetsToBuilders(
    polyface: Polyface,
    allClippers: UnionOfConvexClipPlaneSets,
    destination: ClippedPolyfaceBuilders,
    outputSelector: number = 1
  ) {
    ...
    for (visitor.reset(); visitor.moveToNextFacet(); ) {
      residualPolygons.push(
        new ClipCandidate(cache.grabAndFill(visitor.point), 0)
      );
      while ((candidate = residualPolygons.pop()) !== undefined) {
        ...
        if (convexSetIndex >= numConvexSet) {
          ...
          if (candidate.points.length > 2) outsideShards.push(candidate.points);
        } else {
          ...
          const insidePart = clipper.clipInsidePushOutside(
            candidate.points,
            outsideParts,
            cache
          );
          if (insidePart) {
            if (insidePart.length > 2) insideShards.push(insidePart);
            // Keep outside parts active for clip by later facets . . .
            for (const outsidePolygon of outsideParts) {
              residualPolygons.push(
                new ClipCandidate(outsidePolygon, convexSetIndex + 1)
              );
            }
          } else {
    ...
    if (destination.buildClosureFaces) {
      for (const clipper of allClippers.convexSets) {
        this.buildClosureFacesForConvexSet(
          visitor,
          clipper,
          destination,
          cache
  ...
  private static addRegion(
    builder: PolyfaceBuilder | undefined,
    region: AnyRegion | undefined
  ) {
    if (builder !== undefined && region !== undefined) {
      if (
        region instanceof Loop &&
        region.children.length === 1 &&
        region.children[0] instanceof LineString3d
      ) {
        builder.addPolygonGrowableXYZArray(region.children[0].packedPoints);
      } else if (region instanceof UnionRegion) {
        for (const child of region.children) this.addRegion(builder, child);
      }
    }
  }
  ...
  private static cleanupAndAddRegion(
    builder: PolyfaceBuilder | undefined,
    shards: GrowableXYZArray[],
    worldToLocal: Transform | undefined,
    localToWorld: Transform | undefined
  ) {
    if (builder !== undefined && shards.length > 0) {
      if (worldToLocal)
        GrowableXYZArray.multiplyTransformInPlace(worldToLocal, shards);
      const outsidePieces = RegionOps.polygonBooleanXYToLoops(
        shards,
        RegionBinaryOpType.Union,
        []
      );
      if (outsidePieces && outsidePieces.children.length > 0) {
        if (localToWorld) outsidePieces.tryTransformInPlace(localToWorld);
  ...
  private static addPolygonToBuilderAndDropToCache(
    polygon: GrowableXYZArray | undefined,
    builder: PolyfaceBuilder | undefined,
    cache: GrowableXYZArrayCache
  ) {
    if (polygon) {
      if (builder) builder.addPolygonGrowableXYZArray(polygon);
      cache.dropToCache(polygon);
    }
  }
  private static addPolygonArrayToBuilderAndDropToCache(
    polygonArray: GrowableXYZArray[],
    builder: PolyfaceBuilder | undefined,
    cache: GrowableXYZArrayCache
  ) {
    let polygon;
   ...
   * @internal
   */
  public static clipPolyfaceConvexClipPlaneSetToBuilders(
    polyface: Polyface,
    clipper: ConvexClipPlaneSet,
    destination: ClippedPolyfaceBuilders
  ) {
    ...
    for (visitor.reset(); visitor.moveToNextFacet(); ) {
      ...
      this.addPolygonArrayToBuilderAndDropToCache(
        outsideParts,
        builderB,
        cache
      );
  ...
  private static buildClosureFacesForConvexSet(
    ...
    if (destination.buildClosureFaces) {
      const chainContexts =
        this.createChainContextsForConvexClipPlaneSet(clipper);
  ...
  private static evaluateInwardPlaneNormal(
    ...
    return Vector3d.create(
      plane.velocityXYZ(scale, 0, 0),
      plane.velocityXYZ(0, scale, 0),
      plane.velocityXYZ(0, 0, scale)
    );
  }
  /**
  ...
  private static addClippedContour(
    ...
    clipper: ConvexClipPlaneSet | undefined,
    destination: ClippedPolyfaceBuilders,
    cache: GrowableXYZArrayCache
  ) {
    const polygonA = cache.grabFromCache();
    const polygonB = cache.grabFromCache();
    if (destination.builderB) destination.builderB.toggleReversedFacetFlag();
  ...
  private static addClosureFacets(
    chainContext: ChainMergeContext,
    destination: ClippedPolyfaceBuilders,
    cache: GrowableXYZArrayCache
  ) {
    ...
    if (loops.length > 1) {
      ...
      for (const loopSet of loopSets) {
        PolygonOps.orientLoopsCCWForOutwardNormalInPlace(
          loopSet,
          outwardNormal
        );
   ...
   * @internal
   */
  public static clipPolyfaceClipPlaneToBuilders(
    polyface: Polyface,
    clipper: PlaneAltitudeEvaluator,
    destination: ClippedPolyfaceBuilders
  ) {
    const builderA = destination.builderA;
    const builderB = destination.builderB;
    ...
    for (visitor.reset(); visitor.moveToNextFacet(); ) {
      ...
      if (builderA) builderA.addPolygonGrowableXYZArray(inside);
      if (builderB) builderB.addPolygonGrowableXYZArray(outside);
    }
   ...
   */
  public static clipPolyfaceInsideOutside(
    polyface: Polyface,
    ...
    if (clipper instanceof ClipPlane) {
      ...
      this.clipPolyfaceConvexClipPlaneSetToBuilders(
        polyface,
        clipper,
        destination
      );
    } else if (clipper instanceof UnionOfConvexClipPlaneSets) {
      this.clipPolyfaceUnionOfConvexClipPlaneSetsToBuilders(
        polyface,
        clipper,
        destination,
        outputSelect
   ...
   */
  public static sectionPolyfaceClipPlane(
    polyface: Polyface,
    clipper: ClipPlane
  ): LineString3d[] {
    const chainContext = ChainMergeContext.create();
  ...
  public static clipPolyfaceUnderOverConvexPolyfaceIntoBuilders(
    ...
    const searchA =
      RangeSearch.create2dSearcherForRangeLengthData<number>(rangeDataA);
    if (!searchA) return;
    ...
    const planeOfFacet = ClipPlane.createNormalAndPointXYZXYZ(
      0,
      0,
      1,
      0,
    ...
    for (visitorB.reset(); visitorB.moveToNextFacet(); ) {
      ...
      ConvexClipPlaneSet.setPlaneAndXYLoopCCW(
        visitorB.point,
        planeOfFacet,
        xyFrustum
      );
   ...
   * @param orientUnderMeshDownward if true, the "meshAUnderB" output is oriented with its normals reversed so it can act as the bottom side of a cut-fill pair.
   */
  public static computeCutFill(
    meshA: IndexedPolyface,
    meshB: IndexedPolyface
  ): { meshAUnderB: IndexedPolyface; meshAOverB: IndexedPolyface } {
    ...
    this.clipPolyfaceUnderOverConvexPolyfaceIntoBuilders(
      visitorA,
      visitorB,
      builderAUnderB,
      builderAOverB
