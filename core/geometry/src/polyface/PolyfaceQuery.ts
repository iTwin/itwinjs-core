...
import {
  MultiChainCollector,
  OffsetHelpers,
} from "../curve/internalContexts/MultiChainCollector";
...
} from "../geometry3d/BarycentricTriangle";
/**
...
export class OffsetMeshOptions {
  ...
  private constructor(
    ...
    this.smoothSingleAngleBetweenNormals =
      smoothSingleAngleBetweenNormals.clone();
    this.smoothAccumulatedAngleBetweenNormals =
      smoothAccumulatedAngleBetweenNormals.clone();
  ...
  public static create(
    ...
    const mySmoothSingleRadiansBetweenNormals =
      smoothSingleAngleBetweenNormals.clone();
    const mySmoothAccumulatedRadiansBetweenNormals =
      smoothAccumulatedAngleBetweenNormals.clone();
    const myChamferTurnAngleBetweenNormals =
      chamferTurnAngleBetweenNormals.clone();
    ...
    return new OffsetMeshOptions(
      mySmoothSingleRadiansBetweenNormals,
      mySmoothAccumulatedRadiansBetweenNormals,
      myChamferTurnAngleBetweenNormals
    );
  }
}
...
export class PolyfaceQuery {
  ...
  public static sumFacetAreas(
    ...
    if (source !== undefined) {
      if (source instanceof Polyface)
        return PolyfaceQuery.sumFacetAreas(
          source.createVisitor(1),
          vectorToEye
        );
      let unitVectorToEye: Vector3d | undefined;
      if (vectorToEye !== undefined) unitVectorToEye = vectorToEye.normalize();
      source.reset();
      while (source.moveToNextFacet()) {
        const scaledNormal = PolygonOps.areaNormal(
          source.point.getPoint3dArray()
        );
   ...
   */
  public static sumTetrahedralVolumes(
    source: Polyface | PolyfaceVisitor,
    origin?: Point3d
  ): number {
    let s = 0;
    if (source instanceof Polyface)
      return PolyfaceQuery.sumTetrahedralVolumes(
        source.createVisitor(0),
        origin
      );
    let myOrigin = origin;
  ...
  public static sumVolumeBetweenFacetsAndPlane(
    ...
    if (source instanceof Polyface)
      return PolyfaceQuery.sumVolumeBetweenFacetsAndPlane(
        source.createVisitor(0),
        plane
      );
    const facetOrigin = Point3d.create();
    ...
    while (source.moveToNextFacet()) {
      ...
      PolygonOps.addSecondMomentAreaProducts(
        source.point,
        facetOrigin,
        singleFacetProducts
      );
      ...
      if (singleFacetArea > 0) {
        positiveAreaMomentSums.accumulateProductsFromOrigin(
          facetOrigin,
          singleFacetProducts,
          1.0
        );
      } else {
        negativeAreaMomentSums.accumulateProductsFromOrigin(
          facetOrigin,
          singleFacetProducts,
          1.0
        );
    ...
    const positiveAreaMoments = MomentData.inertiaProductsToPrincipalAxes(
      positiveAreaMomentSums.origin,
      positiveAreaMomentSums.sums
    );
    const negativeAreaMoments = MomentData.inertiaProductsToPrincipalAxes(
      negativeAreaMomentSums.origin,
      negativeAreaMomentSums.sums
    );
  ...
  public static sumFacetSecondAreaMomentProducts(
    source: Polyface | PolyfaceVisitor,
    origin: Point3d
  ): Matrix4d {
    if (source instanceof Polyface)
      return PolyfaceQuery.sumFacetSecondAreaMomentProducts(
        source.createVisitor(0),
        origin
      );
  ...
  public static sumFacetSecondVolumeMomentProducts(
    source: Polyface | PolyfaceVisitor,
    origin: Point3d
  ): Matrix4d {
    if (source instanceof Polyface)
      return PolyfaceQuery.sumFacetSecondVolumeMomentProducts(
        source.createVisitor(0),
        origin
      );
    const products = Matrix4d.createZero();
  ...
  public static computePrincipalAreaMoments(
    ...
    const inertiaProducts = PolyfaceQuery.sumFacetSecondAreaMomentProducts(
      source,
      origin
    );
   ...
   */
  public static computePrincipalVolumeMoments(
    source: Polyface
  ): MomentData | undefined {
    const origin = source.data.getPoint(0);
    if (!origin) return undefined;
    const inertiaProducts = PolyfaceQuery.sumFacetSecondVolumeMomentProducts(
      source,
      origin
    );
    return MomentData.inertiaProductsToPrincipalAxes(origin, inertiaProducts);
   ...
   */
  public static isConvexByDihedralAngleCount(
    source: Polyface,
   ...
   */
  public static dihedralAngleSummary(
    source: Polyface,
    ignoreBoundaries: boolean = false
  ): number {
    ...
    edges.sortAndCollectClusters(
      manifoldClusters,
      ignoreBoundaries ? undefined : badClusters,
      undefined,
      badClusters
    );
    if (badClusters.length > 0) return 0;
    let numPositive = 0;
    let numPlanar = 0;
    let numNegative = 0;
    ...
    for (const cluster of manifoldClusters) {
      ...
      if (
        sideA instanceof SortableEdge &&
        sideB instanceof SortableEdge &&
        source.data.point.vectorIndexIndex(
          sideA.vertexIndexA,
          sideA.vertexIndexB,
          edgeVector
        )
      ) {
        const dihedralAngle = centroidNormal[
          sideA.facetIndex
        ].direction.signedAngleTo(
          centroidNormal[sideB.facetIndex].direction,
          edgeVector
        );
    ...
    if (numPositive > 0 && numNegative === 0) return 1;
    if (numNegative > 0 && numPositive === 0) return -1;
    // problem case: if all edges have zero dihedral angle, record it as convex.
    if (numPlanar > 0 && numPositive === 0 && numNegative === 0) return 1;
    return 0;
  }
   ...
   */
  public static isPolyfaceManifold(
    source: Polyface,
    allowSimpleBoundaries: boolean = false
  ): boolean {
    const edges = new IndexedEdgeMatcher();
    const visitor = source.createVisitor(1);
    visitor.reset();
    ...
    edges.sortAndCollectClusters(
      undefined,
      allowSimpleBoundaries ? undefined : badClusters,
      undefined,
      badClusters
    );
  ...
  public static boundaryEdges(
    ...
    if (result.children.length === 0) return undefined;
    return result;
  }
  /**
   * Test if the facets in `source` occur in perfectly mated pairs, as is required for a closed manifold volume.
  ...
  public static announceBoundaryEdges(
    ...
    const edges = new IndexedEdgeMatcher();
    const visitor =
      source instanceof Polyface ? source.createVisitor(1) : source;
    ...
    const badList = [];
    if (includeDanglers && bad1.length > 0) badList.push(bad1);
    if (includeMismatch && bad2.length > 0) badList.push(bad2);
    if (includeNull && bad0.length > 0) badList.push(bad0);
    if (badList.length === 0) return undefined;
  ...
  private static async continueAnnounceSweepLinestringToConvexPolyfaceXY(
    ...
    while (workCount < this.asyncWorkLimit && visitor.moveToNextFacet()) {
      workCount += context.projectToPolygon(
        visitor.point,
        announce,
        visitor.clientPolyface()!,
        visitor.currentReadIndex()
      );
    }
    return workCount;
  ...
  private static _asyncWorkLimit = 1e6;
  ...
  public static async asyncAnnounceSweepLinestringToConvexPolyfaceXY(
    ...
    if (context) {
      ...
      while (
        0 <
        (workCount = await Promise.resolve(
          PolyfaceQuery.continueAnnounceSweepLinestringToConvexPolyfaceXY(
            context,
            visitor,
            announce
          )
   ...
   */
  public static partitionFacetIndicesByVertexConnectedComponent(
    polyface: Polyface | PolyfaceVisitor
  ): number[][] {
    if (polyface instanceof Polyface) {
      return this.partitionFacetIndicesByVertexConnectedComponent(
        polyface.createVisitor(0)
      );
    }
    // The polyface is really a visitor !!!
    const context = new UnionFindContext(
      this.visitorClientPointCount(polyface)
    );
    ...
    for (polyface.reset(); polyface.moveToNextFacet(); ) {
      const firstVertexIndexOnThisFacet = polyface.pointIndex[0];
      const rootVertexForThisFacet = context.findRoot(
        firstVertexIndexOnThisFacet
      );
  ...
  public static partitionFacetIndicesByVisibilityVector(
    ...
    if (polyface instanceof Polyface) {
      return this.partitionFacetIndicesByVisibilityVector(
        polyface.createVisitor(0),
        vectorToEye,
        sideAngleTolerance
      );
  ...
  public static boundaryOfVisibleSubset(
    ...
    const partitionedIndices = this.partitionFacetIndicesByVisibilityVector(
      polyface,
      vectorToEye,
      sideAngleTolerance
    );
    if (partitionedIndices[visibilitySelect].length === 0) return undefined;
    const visitor = IndexedPolyfaceSubsetVisitor.createSubsetVisitor(
      polyface,
      partitionedIndices[visibilitySelect],
      1
    );
  ...
  public static announceBoundaryChainsAsLineString3d(
    ...
    const collector = new MultiChainCollector(
    ...
    PolyfaceQuery.announceBoundaryEdges(
      ...
      (pointA: Point3d, pointB: Point3d, _indexA: number, _indexB: number) =>
        collector.captureCurve(LineSegment3d.create(pointA, pointB)),
      true,
      false,
      false
    );
  ...
  public static cloneWithMaximalPlanarFacets(
    ...
    const partitions =
      PolyfaceQuery.partitionFacetIndicesBySortableEdgeClusters(
        smoothEdges,
        numFacets
      );
    ...
    for (const fragment of fragmentPolyfaces) {
      ...
      const chains = OffsetHelpers.collectChains(
        edges,
        gapTolerance,
        planarityTolerance
      );
      if (chains) {
        ...
        if (frame !== undefined) {
          ...
          if (inverseFrame !== undefined) {
            ...
            const graph = HalfEdgeGraphMerge.formGraphFromChains(
              edgeStrings,
              true,
  ...
  public static fillSimpleHoles(
    ...
    unfilledChains?: LineString3d[]
  ): IndexedPolyface | undefined {
    if (mesh instanceof Polyface)
      return this.fillSimpleHoles(
        mesh.createVisitor(0),
        options,
        unfilledChains
      );
    const builder = PolyfaceBuilder.create();
    ...
    for (const c of chains) {
      ...
      let rejected = false;
      if (!c.isPhysicallyClosed) rejected = true;
      else if (
        options.maxEdgesAroundHole !== undefined &&
        points.length > options.maxEdgesAroundHole
      )
        rejected = true;
      else if (
        options.maxPerimeter !== undefined &&
        Point3dArray.sumEdgeLengths(points, false) > options.maxPerimeter
      )
        rejected = true;
      else if (
        options.upVector !== undefined &&
        PolygonOps.sumTriangleAreasPerpendicularToUpVector(
          points,
          options.upVector
        ) <= 0.0
      )
      ...
      if (rejected && unfilledChains !== undefined) unfilledChains.push(c); // yes, capture it -- this scope owns the chains and has no further use for it.
    }
    if (
      options.includeOriginalMesh !== undefined &&
      options.includeOriginalMesh
    ) {
  ...
  public static cloneFiltered(
    source: Polyface | PolyfaceVisitor,
    filter: (visitor: PolyfaceVisitor) => boolean
  ): Polyface {
    if (source instanceof Polyface) {
    ...
    for (; source.moveToNextFacet(); ) {
      if (filter(source)) builder.addFacetFromVisitor(source);
    }
    return builder.claimPolyface(true);
  }
  ...
  public static visitorClientPointCount(visitor: PolyfaceVisitor): number {
    ...
    if (polyface !== undefined) return polyface.data.point.length;
    visitor.reset();
    let maxIndex = -1;
    while (visitor.moveToNextFacet()) {
  ...
  public static visitorClientFacetCount(visitor: PolyfaceVisitor): number {
    ...
    let facetCount = 0;
    visitor.reset();
    while (visitor.moveToNextFacet()) ++facetCount;
    return facetCount;
  }
  ...
  private static partitionFacetIndicesBySortableEdgeClusters(
    edgeClusters: SortableEdgeCluster[],
    numFacets: number
  ): number[][] {
  ...
  public static partitionFacetIndicesByEdgeConnectedComponent(
    polyface: Polyface | PolyfaceVisitor,
    stopAtVisibleEdges: boolean = false
  ): number[][] {
    if (polyface instanceof Polyface) {
      return this.partitionFacetIndicesByEdgeConnectedComponent(
        polyface.createVisitor(0),
        stopAtVisibleEdges
      );
    }
    ...
    while (polyface.moveToNextFacet()) {
      ...
      for (let i = 0; i < numEdges; i++) {
        if (stopAtVisibleEdges && polyface.edgeVisible[i]) {
          ...
          matcher.addEdge(
            polyface.clientPointIndex(i),
            polyface.clientPointIndex(i + 1),
            polyface.currentReadIndex()
          );
        }
      }
    }
    ...
    return this.partitionFacetIndicesBySortableEdgeClusters(
      allEdges,
      numFacets
    );
  }
   ...
   * * Facets are ASSUMED to be convex and planar.
   */
  public static sweepLinestringToFacetsXYreturnSweptFacets(
    linestringPoints: GrowableXYZArray,
    polyface: Polyface
  ): Polyface {
    const builder = PolyfaceBuilder.create();
    this.announceSweepLinestringToConvexPolyfaceXY(
      ...
      polyface,
      (
        _linestring: GrowableXYZArray,
        _segmentIndex: number,
        _polyface: Polyface,
        _facetIndex: number,
        points: Point3d[]
      ) => {
        if (points.length === 4) builder.addQuadFacet(points);
        else if (points.length === 3) builder.addTriangleFacet(points);
      }
    );
    return builder.claimPolyface(true);
  }
  ...
  public static sweepLinestringToFacetsXYReturnLines(
    ...
    this.announceSweepLinestringToConvexPolyfaceXY(
      ...
      (
        ...
        drapeGeometry.push(
          LineSegment3d.create(points[indexA], points[indexB])
        );
      }
    );
   ...
   */
  public static collectRangeLengthData(
    polyface: Polyface | PolyfaceVisitor
  ): RangeLengthData {
    if (polyface instanceof Polyface) {
  ...
  public static cloneWithTVertexFixup(polyface: Polyface): IndexedPolyface {
    ...
    for (oldFacetVisitor.reset(); oldFacetVisitor.moveToNextFacet(); ) {
      ...
      for (let i = 0; i + 1 < oldFacetVisitor.point.length; i++) {
        ...
        rangeSearcher.announcePointsInRange(
          edgeRange,
          (index: number, _x: number, _y: number, _z: number) => {
            // x,y,z has x,y within the range of the search ... test for exact on (in full 3d!)
            polyface.data.point.getPoint3dAtUncheckedPointIndex(
              index,
              spacePoint
            );
            ...
            if (undefined !== detail) {
              if (
                detail.fraction >= 0.0 &&
                detail.fraction < 1.0 &&
                !detail.point.isAlmostEqual(point0) &&
                !detail.point.isAlmostEqual(point1) &&
                spacePoint.isAlmostEqual(detail.point)
              ) {
                if (detailArray === undefined) detailArray = [];
                detail.a = index;
        ...
        if (detailArray !== undefined) {
          detailArray.sort(
            (a: CurveLocationDetail, b: CurveLocationDetail) =>
              a.fraction - b.fraction
          );
          for (const d of detailArray) {
            newFacetVisitor.pushInterpolatedDataFrom(
              oldFacetVisitor,
              i,
              d.fraction,
              i + 1
      ...
      builder.addFacetFromGrowableArrays(
        newFacetVisitor.point,
        newFacetVisitor.normal,
        newFacetVisitor.param,
        newFacetVisitor.color,
        newFacetVisitor.edgeVisible
      );
    }
  ...
  private static compareFacetIndexAndVertexIndices(
    ...
    if (arrayA.length !== arrayB.length) return arrayA.length - arrayB.length;
   ...
   */
  public static collectDuplicateFacetIndices(
    polyface: Polyface,
  ...
  public static announceDuplicateFacetIndices(
    ...
    for (visitor.reset(); visitor.moveToNextFacet(); ) {
      ...
      for (let i = 1; i < visitor.pointIndex.length; i++) {
        if (pointIndex[i] < pointIndex[lowIndex]) lowIndex = i;
      }
      // find its lowest neighbor -- assemble sort array in that direction
      if (
        pointIndex[(lowIndex + 1) % numPointsThisFacet] <
        pointIndex[(lowIndex + numPointsThisFacet - 1) % numPointsThisFacet]
      ) {
        ...
        for (let i = 0; i < numPointsThisFacet; i++) {
          entry.push(
            pointIndex[(lowIndex + numPointsThisFacet - i) % numPointsThisFacet]
          );
        }
      }
    ...
    for (i0 = 0; i0 < n; i0 = i1) {
      ...
      while (
        i1 < n &&
        0 ===
          this.compareFacetIndexAndVertexIndices(
            facetIndexAndVertexIndices[i0],
            facetIndexAndVertexIndices[i1]
          )
      ) {
  ...
  public static cloneByFacetDuplication(
    ...
    clusterSelector: DuplicateFacetClusterSelector
  ): Polyface {
    const builder = PolyfaceBuilder.create();
    ...
    this.announceDuplicateFacetIndices(
      ...
      (clusterFacetIndices: number[]) => {
        ...
        if (clusterFacetIndices.length === 1) {
          if (includeSingletons) numToSelect = 1;
          ...
          if (clusterSelector === DuplicateFacetClusterSelector.SelectAny)
            ...
            clusterSelector === DuplicateFacetClusterSelector.SelectOneByParity
          )
  ...
  public static setSingleEdgeVisibility(
    ...
    for (let i = index0; i < index1; i++)
      if (data.pointIndex[i] === vertexIndex) data.edgeVisible[i] = value; // actually sets visibility on all edges in the face that start at this vertex
  }
  /**
  ...
  public static getSingleEdgeVisibility(
    ...
    const index1 = polyface.facetIndex1(facetIndex);
    for (let i = index0; i < index1; i++)
      if (data.pointIndex[i] === vertexIndex) return data.edgeVisible[i]; // return visibility of first edge in the face that starts at this vertex
    return undefined;
  }
  /** Load all half edges from a mesh to an IndexedEdgeMatcher.
   * @param polyface a mesh, or a visitor assumed to have numWrap === 1
   */
  public static createIndexedEdges(
    polyface: Polyface | PolyfaceVisitor
  ): IndexedEdgeMatcher {
    if (polyface instanceof Polyface)
  ...
  public static collectEdgesByDihedralAngle(
    mesh: Polyface | PolyfaceVisitor,
    maxSmoothEdgeAngle?: Angle,
    sharpEdges: boolean = false
  ): SortableEdgeCluster[] {
    if (mesh instanceof Polyface)
      return this.collectEdgesByDihedralAngle(
        mesh.createVisitor(1),
        maxSmoothEdgeAngle,
        sharpEdges
      );
    mesh.setNumWrap(1);
    ...
    for (const pair of manifoldEdges) {
      if (Array.isArray(pair) && pair.length === 2) {
        ...
        if (
          undefined !==
            PolyfaceQuery.computeFacetUnitNormal(
              mesh,
              e0.facetIndex,
              normal0
            ) &&
          undefined !==
            PolyfaceQuery.computeFacetUnitNormal(mesh, e1.facetIndex, normal1)
        ) {
   ...
   */
  public static markPairedEdgesInvisible(
  ...
  public static computeFacetUnitNormal(
    ...
    if (visitor.moveToReadIndex(facetIndex)) {
      if (PolygonOps.unitNormal(visitor.point, result)) return result;
    }
    return undefined;
  }
  /**
   ...
   * @param mesh
   */
  public static reorientVertexOrderAroundFacetsForConsistentOrientation(
    mesh: IndexedPolyface
  ): boolean {
   ...
   */
  public static buildAverageNormals(
    polyface: IndexedPolyface,
    toleranceAngle: Angle = Angle.createDegrees(31.0)
  ) {
    BuildAverageNormalsContext.buildFastAverageNormals(
      polyface,
      toleranceAngle
    );
  }
  ...
  public static cloneOffset(
    ...
    offsetOptions: OffsetMeshOptions = OffsetMeshOptions.create()
  ): IndexedPolyface {
    const strokeOptions = StrokeOptions.createForFacets();
    const offsetBuilder = PolyfaceBuilder.create(strokeOptions);
    OffsetMeshContext.buildOffsetMeshWithEdgeChamfers(
      source,
      offsetBuilder,
  ...
  public static intersectRay3d(
    ...
    ray: Ray3d,
    options?: FacetIntersectOptions
  ): FacetLocationDetail | undefined {
    if (visitor instanceof Polyface)
      return PolyfaceQuery.intersectRay3d(
        visitor.createVisitor(0),
        ray,
        options
      );
    ...
    while (visitor.moveToNextFacet()) {
      ...
      if (3 === numEdges) {
        ...
        const detail3 = (this._workTriDetail = tri.intersectRay3d(
          ray,
          this._workTriDetail
        ));
        tri.snapLocationToEdge(
          detail3,
        ...
        if (PolygonOps.isConvex(vertices))
          detail = this._workFacetDetailC = ConvexFacetLocationDetail.create(
            ...
            numEdges,
            detailN,
            this._workFacetDetailC
          );
        else
          detail = this._workFacetDetailNC =
            NonConvexFacetLocationDetail.create(
              visitor.currentReadIndex(),
              numEdges,
              detailN,
              this._workFacetDetailNC
            );
      }
      if (detail.isInsideOrOn) {
        // set optional caches, process the intersection
        ...
        if (options?.needBarycentricCoordinates)
          detail.getBarycentricCoordinates(
            vertices,
            options?.distanceTolerance
          );
        if (
          options?.acceptIntersection &&
          !options.acceptIntersection(detail, visitor)
        )
          continue;
        return detail;
      }
...
export type AnnounceDrapePanel = (
  linestring: GrowableXYZArray,
  segmentIndex: number,
  polyface: Polyface,
  facetIndex: number,
  points: Point3d[],
  indexAOnFacet: number,
  indexBOnFacet: number
) => any;
