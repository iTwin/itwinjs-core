...
export class OffsetMeshOptions {
  ...
  public static create(
    ...
    if (mySmoothSingleRadiansBetweenNormals.degrees < 1) mySmoothAccumulatedRadiansBetweenNormals.setDegrees(1.0);
...
export class PolyfaceQuery {
  ...
  public static sumFacetAreas(source: Polyface | PolyfaceVisitor | undefined, vectorToEye?: Vector3d): number {
    let s = 0;
    if (source !== undefined) {
      if (source instanceof Polyface) return PolyfaceQuery.sumFacetAreas(source.createVisitor(1), vectorToEye);
      ...
      while (source.moveToNextFacet()) {
        ...
        if (unitVectorToEye !== undefined) {
          ...
          if (scale !== undefined) area *= scaledNormal.dotProduct(unitVectorToEye) * scale;
        }
        s += area;
      }
    }
  ...
  public static sumTetrahedralVolumes(source: Polyface | PolyfaceVisitor, origin?: Point3d): number {
    let s = 0;
    if (source instanceof Polyface) return PolyfaceQuery.sumTetrahedralVolumes(source.createVisitor(0), origin);
    let myOrigin = origin;
    ...
    source.reset();
    while (source.moveToNextFacet()) {
      if (myOrigin === undefined) myOrigin = source.point.getPoint3dAtUncheckedPointIndex(0);
  ...
  public static sumVolumeBetweenFacetsAndPlane(
    ...
    if (source instanceof Polyface) return PolyfaceQuery.sumVolumeBetweenFacetsAndPlane(source.createVisitor(0), plane);
    const facetOrigin = Point3d.create();
    const targetA = Point3d.create();
    const targetB = Point3d.create();
  ...
  public static announceBoundaryEdges(
    ...
    for (const list of badList) {
      for (const e of list) {
        ...
        if (pointA && pointB) announceEdge(pointA, pointB, indexA, indexB, visitor.currentReadIndex());
      }
    }
  }
    ...
    if (context) {
      ...
      while (
        ...
        (workCount = await Promise.resolve(
          PolyfaceQuery.continueAnnounceSweepLinestringToConvexPolyfaceXY(context, visitor, announce)
        ))
      ) {
        workTotal += workCount;
  ...
  public static partitionFacetIndicesByVertexConnectedComponent(polyface: Polyface | PolyfaceVisitor): number[][] {
    ...
    for (polyface.reset(); polyface.moveToNextFacet(); ) {
      ...
      for (const vertexIndex of polyface.pointIndex) context.mergeSubsets(firstVertexIndexOnThisFacet, vertexIndex);
    }
  ...
  public static cloneWithMaximalPlanarFacets(mesh: Polyface | PolyfaceVisitor): IndexedPolyface | undefined {
    if (mesh instanceof Polyface) return this.cloneWithMaximalPlanarFacets(mesh.createVisitor(0));
    ...
    for (const partition of partitions) {
      if (partition.length === 1) {
        if (visitor.moveToReadIndex(partition[0])) builder.addFacetFromVisitor(visitor);
      } else {
  ...
  public static fillSimpleHoles(
    ...
    if (mesh instanceof Polyface) return this.fillSimpleHoles(mesh.createVisitor(0), options, unfilledChains);
    const builder = PolyfaceBuilder.create();
    const chains: LineString3d[] = [];
    PolyfaceQuery.announceBoundaryChainsAsLineString3d(mesh, (ls: LineString3d) => {
      ls.reverseInPlace();
      chains.push(ls);
    });
    ...
    for (const c of chains) {
      ...
      if (!c.isPhysicallyClosed) rejected = true;
      else if (options.maxEdgesAroundHole !== undefined && points.length > options.maxEdgesAroundHole) rejected = true;
        ...
        PolygonOps.sumTriangleAreasPerpendicularToUpVector(points, options.upVector) <= 0.0
      )
        rejected = true;

      if (
        !rejected &&
        SpacePolygonTriangulation.triangulateSimplestSpaceLoop(points, (_loop: Point3d[], triangles: Point3d[][]) => {
    ...
    if (options.includeOriginalMesh !== undefined && options.includeOriginalMesh) {
      for (mesh.reset(); mesh.moveToNextFacet(); ) builder.addFacetFromVisitor(mesh);
    }
  ...
  public static visitorClientPointCount(visitor: PolyfaceVisitor): number {
    ...
    let maxIndex = -1;
    while (visitor.moveToNextFacet()) {
      for (const pointIndex of visitor.pointIndex) if (pointIndex > maxIndex) maxIndex = pointIndex;
    }
    return maxIndex + 1;
  }
  ...
  public static visitorClientFacetCount(visitor: PolyfaceVisitor): number {
    const polyface = visitor.clientPolyface();
    if (polyface !== undefined && polyface.facetCount !== undefined) return polyface.facetCount;
    let facetCount = 0;
    visitor.reset();
  ...
  private static partitionFacetIndicesBySortableEdgeClusters(
    ...
    for (const cluster of edgeClusters) {
      if (cluster instanceof SortableEdge) {
        ...
        for (let i = 1; i < cluster.length; i++) context.mergeSubsets(edge0.facetIndex, cluster[i].facetIndex);
      }
    }
  ...
  public static collectRangeLengthData(polyface: Polyface | PolyfaceVisitor): RangeLengthData {
    ...
    for (polyface.reset(); polyface.moveToNextFacet(); ) rangeData.accumulateGrowableXYZArrayRange(polyface.point);
    return rangeData;
  }
  ...
  public static collectDuplicateFacetIndices(polyface: Polyface, includeSingletons: boolean = false): number[][] {
    const result: number[][] = [];
    this.announceDuplicateFacetIndices(polyface, (clusterFacetIndices: number[]) => {
      if (includeSingletons || clusterFacetIndices.length > 1) result.push(clusterFacetIndices.slice());
    });
    return result;
  }
  /**
  ...
  public static announceDuplicateFacetIndices(
    ...
    for (i0 = 0; i0 < n; i0 = i1) {
      ...
      while (
        i1 < n &&
        0 === this.compareFacetIndexAndVertexIndices(facetIndexAndVertexIndices[i0], facetIndexAndVertexIndices[i1])
      ) {
  ...
  public static cloneByFacetDuplication(
    ...
    this.announceDuplicateFacetIndices(source, (clusterFacetIndices: number[]) => {
      let numToSelect = 0;
      if (clusterFacetIndices.length === 1) {
        if (includeSingletons) numToSelect = 1;
      } else if (clusterFacetIndices.length > 1) {
        if (clusterSelector === DuplicateFacetClusterSelector.SelectAny) numToSelect = 1;
        else if (clusterSelector === DuplicateFacetClusterSelector.SelectAll) numToSelect = clusterFacetIndices.length;
  ...
  private static setEdgeVisibility(polyface: IndexedPolyface, clusters: SortableEdgeCluster[], value: boolean) {
    for (const cluster of clusters) {
      if (cluster instanceof SortableEdge) {
        ...
        for (const e1 of cluster) this.setSingleEdgeVisibility(polyface, e1.facetIndex, e1.vertexIndexA, value);
      }
    }
  }
  /**
  ...
  public static setSingleEdgeVisibility(
    ...
    for (let i = index0; i < index1; i++) if (data.pointIndex[i] === vertexIndex) data.edgeVisible[i] = value; // actually sets visibility on all edges in the face that start at this vertex
  }
  /**
  ...
  public static getSingleEdgeVisibility(
    ...
    const index0 = polyface.facetIndex0(facetIndex);
    const index1 = polyface.facetIndex1(facetIndex);
    for (let i = index0; i < index1; i++) if (data.pointIndex[i] === vertexIndex) return data.edgeVisible[i]; // return visibility of first edge in the face that starts at this vertex
    return undefined;
  }
  ...
  public static createIndexedEdges(polyface: Polyface | PolyfaceVisitor): IndexedEdgeMatcher {
    if (polyface instanceof Polyface) return this.createIndexedEdges(polyface.createVisitor(1));
    const edges = new IndexedEdgeMatcher();
    polyface.reset();
    while (polyface.moveToNextFacet()) {
      const numEdges = polyface.pointCount - 1;
  ...
  public static collectEdgesByDihedralAngle(
    ...
    for (const pair of manifoldEdges) {
      if (Array.isArray(pair) && pair.length === 2) {
        ...
        const e1 = pair[1];
        if (
          undefined !== PolyfaceQuery.computeFacetUnitNormal(mesh, e0.facetIndex, normal0) &&
          ...
          if (sharpEdges) {
            if (edgeAngle.radians > maxSmoothEdgeAngle.radians) outEdges.push(pair);
          } else {
            if (edgeAngle.radians <= maxSmoothEdgeAngle.radians) outEdges.push(pair);
          }
        }
    ...
    if (sharpEdgeAngle !== undefined) {
      ...
      for (const pair of pairedEdges) {
        if (Array.isArray(pair) && pair.length === 2) {
          ...
          const e1 = pair[1];
          if (
            undefined !== PolyfaceQuery.computeFacetUnitNormal(visitor, e0.facetIndex, normal0) &&
  ...
  public static markAllEdgeVisibility(mesh: IndexedPolyface, value: boolean) {
    ...
    for (let i = 0; i < data.edgeVisible.length; i++) data.edgeVisible[i] = value;
  }
  /**
  ...
  public static intersectRay3d(
    ...
    if (visitor instanceof Polyface) return PolyfaceQuery.intersectRay3d(visitor.createVisitor(0), ray, options);
    ...
    while (visitor.moveToNextFacet()) {
      ...
      if (detail.isInsideOrOn) {
        ...
        if (options?.needParam && visitor.param) detail.getParam(visitor.param, vertices, options?.distanceTolerance);
        if (options?.needColor && visitor.color) detail.getColor(visitor.color, vertices, options?.distanceTolerance);
        if (options?.needBarycentricCoordinates) detail.getBarycentricCoordinates(vertices, options?.distanceTolerance);
        if (options?.acceptIntersection && !options.acceptIntersection(detail, visitor)) continue;
        return detail;
      }
    }
