...
export class OffsetMeshContext {
  ...
  private assignOffsetByAverageNormalAroundVertex(
    ...
    if (OffsetMeshContext.stringDebugFunction) {
      OffsetMeshContext.stringDebugFunction(
        `XYZ ${HalfEdge.nodeToIdXYZString(vertexSeed)} Average Normal ${JSON.stringify(data.averageNormal.toJSON())}`,
      );
      OffsetMeshContext.stringDebugFunction(
        `           angle ratio ${data.radiansSum / (2 * Math.PI)}   maxDeviation ${data.maxDeviationRadiansFromAverage}`,
      );
    }
      ...
      if (breakEdges.length <= 1) {
        ...
        for (let i = 0; i < breakEdges.length; i++) {
          ...
          if (
            ...
            if (OffsetMeshContext.stringDebugFunction !== undefined)
              OffsetMeshContext.stringDebugFunction(
                `    ChamferChamfer Fixup ${this.inspectMasks(breakEdges[i0])} ${this.inspectMasks(breakEdges[i1])} ${
            ...
            if (vectorFromOrigin !== undefined) {
              ...
              for (const iOutput of [i0, i1, i2]) {
                this.announceNodeAndSectorPropertiesInSmoothSector(breakEdges[iOutput], (node: HalfEdge, properties: SectorOffsetProperties) => {
        ...
        for (let i = 0; i < breakEdges.length; i++) {
          ...
          if (OffsetMeshContext.stringDebugFunction !== undefined)
            OffsetMeshContext.stringDebugFunction(
              `    Intersection Fixup ${this.inspectMasks(breakEdges[i0])} ${this.inspectMasks(breakEdges[i1])} ${
                this.inspectMasks(breakEdges[i2])
              } `,
            );
          const vectorFromOrigin = this.compute3SectorIntersection(breakEdges[i0], breakEdges[i1], breakEdges[i2]);
          if (vectorFromOrigin !== undefined) {
            if (vertexXYZ.distance(vectorFromOrigin) < maxVertexMove) {
              this.announceNodeAndSectorPropertiesInSmoothSector(breakEdges[i1], (node: HalfEdge, properties: SectorOffsetProperties) => {
                properties.setXYAndZ(vectorFromOrigin);
                node.setMask(this._offsetCoordinatesReassigned);
              });
            }
  ...
  private transferXYZFromNodeToSmoothSector(
    ...
    this.announceNodeAndSectorPropertiesInSmoothSector(destinationStartNode, (node: HalfEdge, properties: SectorOffsetProperties) => {
