...
export class TaggedNumericData {
  ...
  public isAlmostEqual(other: TaggedNumericData): boolean {
    ...
    if (this.tagA !== other.tagA) return false;
  ...
  public clone(result?: TaggedNumericData): TaggedNumericData {
    if (!result) result = new TaggedNumericData(this.tagA, this.tagB);
    if (this.intData) result.intData = this.intData.slice();
    if (this.doubleData) result.doubleData = this.doubleData.slice();
    return result;
  }
}
