...
export class ViewManager implements Iterable<ScreenViewport> {
  ...
  public notifySelectedViewportChanged(previous: ScreenViewport | undefined, current: ScreenViewport | undefined) {
    const handleSelectedViewportChange = async () => IModelApp.toolAdmin.onSelectedViewportChanged(previous, current);
    this._selectedViewportChange = this._selectedViewportChange.then(handleSelectedViewportChange, handleSelectedViewportChange);
