...
export class ReadonlyTileUserSet extends ReadonlySortedArray<TileUser> {
  public constructor(user?: TileUser) {
    super((lhs, rhs) => lhs.userId - rhs.userId);
    if (undefined !== user)
...
function compareTileUserSets(lhs: TileUserSet, rhs: TileUserSet): number {
  ...
  if (0 === diff) {
    for (let i = 0; i < lhs.length; i++) {
      diff = lhs.get(i)!.userId - rhs.get(i)!.userId;
      if (0 !== diff)
        break;
    }
  }
 ...
 * For example, the tile request scheduler needs to associate with each [[TileRequest]] the set of TileUsers awaiting the request's response.
 * Using UniqueTileUserSets ensures all tiles requested by the same users use the same TileUserSet object.
 * Additionally, whenever a user (e.g., a viewport) is destroyed there is no need to track down every associated tile request - the user can just be removed from the TileUserSets managed by this object.
