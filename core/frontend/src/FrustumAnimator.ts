...
export class FrustumAnimator implements Animator {
  ...
  public constructor(
    public options: ViewAnimationOptions,
    ...
    let duration =
      undefined !== options.animationTime
        ? options.animationTime
        : settings.time.normal.milliseconds;
    ...
    const axis = end.rotation
      .multiplyMatrixMatrixInverse(begin.rotation)!
      .getAxisAndAngleOfRotation(); // axis to rotate begin to get to end
    ...
    if (
      ...
      const endRange = Range3d.createTransformedArray(
        viewTransform,
        view.calculateFocusCorners()
      ); // get the view-aligned range of the focus plane at the end
      const beginRange = Range3d.createTransformedArray(
        viewTransform,
        view.applyPose(begin).calculateFocusCorners()
      ); // get the view-aligned range of the focus plane at the beginning
      ...
      if (!expand(beginRange).intersectsRangeXY(expand(endRange))) {
        view3.lookAtViewAlignedVolume(
          beginRange.union(endRange),
          viewport.viewRect.aspect
        ); // set up a view that would show both extents
        ...
        if (begin.cameraOn)
          eyeBias = zVec.scaleToLength(
            zVec.dotProduct(begin3.camera.eye.vectorTo(view3.camera.eye))
          ); // if the camera is on, the bias is the difference in height of the two eye positions
      }
    }
    ...
    this._tweens.create(timing, {
      to: {
        fraction: 1.0,
        height: zoomSettings.heights,
        position: zoomSettings.positions,
      },
      duration,
      start: true,
      ...
      onUpdate: () => {
        ...
        const rot = Matrix3d.createRotationAroundVector(
          axis.axis,
          Angle.createDegrees(fraction * axis.angle.degrees)
        )!.multiplyMatrixMatrix(begin.rotation);
        if (begin.cameraOn) {
          const newExtents = begin.extents.interpolate(fraction, end.extents);
          if (undefined !== eyeBias) {
            const eyePoint = begin3.camera.eye.interpolate(
              fraction,
              end3.camera.eye
            );
            ...
            view3.lookAt({
              eyePoint: data.eye,
              targetPoint: data.target,
              upVector: rot.getRow(1),
              newExtents,
            });
          }
        } else {
          const extents = begin.extents.interpolate(
            timing.fraction,
            end.extents
          );
  ...
  public interrupt() {
    ...
    this._tweens.update(
      this.options.cancelOnAbort ? Date.now() + this._duration * 0.06 : Infinity
    );
