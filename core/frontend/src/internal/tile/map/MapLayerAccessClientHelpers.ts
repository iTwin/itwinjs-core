...
/** Gives an access client's [[MapLayerRequestShaper.applyToRequest]] the opportunity to shape an
 ...
 * [[MapLayerRequestShaper.applyToRequest]]. Delegates to
 * [[MapLayerRequestShaper.classifyResponse]] when defined; otherwise treats HTTP 401/403 as
...
export async function isAccessClientAuthFailure(response: Response, context: MapLayerAccessTokenParams, accessClient?: MapLayerAccessClient): Promise<boolean> {
  if (accessClient?.classifyResponse)
    return "authentication" === await accessClient.classifyResponse({ response, context });

  return response.status === 401 || response.status === 403;
}

/** The redirect policy for a request shaped by [[MapLayerRequestShaper.applyToRequest]]: refused while
