...
export function getScriptDelta(prev: RenderSchedule.Script, next: RenderSchedule.Script): Set<Id64String> {
  ...
  for (const modelId of new Set([...prevModels.keys(), ...nextModels.keys()])) {
    ...
    for (const batchId of allBatchIds) {
      ...
      if (
        prevTimeline.containsTransform !== nextTimeline.containsTransform ||
        (!!prevTimeline.cuttingPlane !== !!nextTimeline.cuttingPlane) ||
        prevTimeline.requiresBatching !== nextTimeline.requiresBatching
      ) {
