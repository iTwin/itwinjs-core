...
export class EmphasizeElements implements FeatureOverrideProvider {
  ...
  protected createAppearanceFromKey(key: number): FeatureAppearance {
    ...
    if (key < 0) {
      ...
      if (0 !== color.getAlpha())
        // Fully transparent signifies to use color only...
   ...
   */
  public get defaultAppearance(): FeatureAppearance | undefined {
    return this._defaultAppearance;
  }
  public set defaultAppearance(appearance: FeatureAppearance | undefined) {
  ...
  public getEmphasizedIsolatedElements(): Id64Set | undefined {
    return undefined !== this._defaultAppearance && undefined !== this._emphasizeIsolated && 0 !== this._emphasizeIsolated.size
  ...
  public getEmphasizedElements(vp: Viewport): Id64Set | undefined {
    return undefined !== this.getEmphasizedIsolatedElements()
      ? this._emphasizeIsolated
      : undefined !== this._defaultAppearance && !vp.isAlwaysDrawnExclusive
  ...
  public clearNeverDrawnElements(vp: Viewport): boolean {
    if (undefined === this.getNeverDrawnElements(vp)) return false;
    vp.clearNeverDrawn();
    return true;
  }
  ...
  public clearAlwaysDrawnElements(vp: Viewport): boolean {
    if (undefined === this.getAlwaysDrawnElements(vp)) return false;
    vp.clearAlwaysDrawn();
    return true;
  }
  ...
  public clearIsolatedElements(vp: Viewport): boolean {
    if (undefined === this.getIsolatedElements(vp)) return false;
    if (this.clearEmphasizedIsolatedElements(vp, true)) return true;
  ...
  public clearEmphasizedElements(vp: Viewport): boolean {
    if (undefined === this.getEmphasizedElements(vp)) return false;

    if (this.clearEmphasizedIsolatedElements(vp, false)) return true;

    if (!this.clearAlwaysDrawnElements(vp)) return false;
  ...
  public clearOverriddenElements(vp: Viewport, keyOrIds?: number | Id64Arg): boolean {
    ...
    if (undefined !== keyOrIds) {
      if (typeof keyOrIds === "number") {
        if (!this._overrideAppearance.delete(keyOrIds)) return false;
      } else {
        let changed = false;

        for (const [otherKey, otherIds] of this._overrideAppearance) {
          const oldSize = otherIds.size;
          for (const id of Id64.iterable(keyOrIds)) otherIds.delete(id);

          if (oldSize !== otherIds.size) changed = true;

          if (0 === otherIds.size) this._overrideAppearance.delete(otherKey);
        }
  ...
  public setAlwaysDrawnElements(ids: Id64Arg, vp: Viewport, exclusive: boolean = true, replace: boolean = true): boolean {
    ...
    if (undefined === visibleIds) return false;
  ...
  public emphasizeElements(ids: Id64Arg, vp: Viewport, defaultAppearance?: FeatureAppearance, replace: boolean = true): boolean {
    if (undefined !== this.getIsolatedElements(vp)) {
      ...
      if (!this.setAlwaysDrawnElements(ids, vp, false, replace)) return false;
   ...
   * @see [[Viewport.isAlwaysDrawnExclusive]]
   */
  public emphasizeSelectedElements(
    vp: Viewport,
    ...
    const selection = vp.view.iModel.selectionSet;
    if (!selection.isActive || !this.emphasizeElements(selection.elements, vp, defaultAppearance, replace)) return false;
   ...
   * @see [[Viewport.addFeatureOverrideProvider]]
   */
  public overrideElements(
    ids: Id64Arg,
    vp: Viewport,
    color: ColorDef,
    override: FeatureOverrideType = FeatureOverrideType.ColorOnly,
    replace: boolean = false
  ): boolean {
    const ovrKey = this.createOverrideKey(color, override);
    if (undefined === ovrKey) return false;

    const overrideIds = new Set<string>();
    for (const id of Id64.iterable(ids)) overrideIds.add(id);
    ...
    if (undefined === this._overrideAppearance) {
      ...
      for (const [key, otherIds] of this._overrideAppearance) {
        if (key === ovrKey)
          // Make sure these ids are unique to this color/transparency key...
          continue;

        for (const id of Id64.iterable(ids)) otherIds.delete(id);

        if (0 !== otherIds.size) continue;
   ...
   * @see [[Viewport.addFeatureOverrideProvider]]
   */
  public overrideSelectedElements(
    vp: Viewport,
    color: ColorDef,
    override: FeatureOverrideType = FeatureOverrideType.ColorOnly,
    replace: boolean = false,
  ...
  public isActive(vp: Viewport): boolean {
    return (
      undefined !== this.getNeverDrawnElements(vp) || undefined !== this.getAlwaysDrawnElements(vp) || undefined !== this.getOverriddenElements()
    );
  }
  ...
  public toJSON(vp: Viewport): EmphasizeElementsProps {
    ...
    if (undefined !== neverDrawn) props.neverDrawn = [...neverDrawn];

    const alwaysDrawn = this.getAlwaysDrawnElements(vp);
    if (undefined !== alwaysDrawn) props.alwaysDrawn = [...alwaysDrawn];

    if (vp.isAlwaysDrawnExclusive) props.isAlwaysDrawnExclusive = true; // isolate

    const alwaysDrawnExclusiveEmphasized = this.getEmphasizedIsolatedElements();
    if (undefined !== alwaysDrawnExclusiveEmphasized) props.alwaysDrawnExclusiveEmphasized = [...alwaysDrawnExclusiveEmphasized];
    ...
    if (this.wantEmphasis) props.wantEmphasis = true;
  ...
  public fromJSON(props: EmphasizeElementsProps, vp: Viewport): boolean {
    let changed = false;
    if (undefined !== props.neverDrawn && this.setNeverDrawnElements(new Set<string>(props.neverDrawn), vp, true)) changed = true;

    if (
      undefined !== props.alwaysDrawn &&
      this.setAlwaysDrawnElements(new Set<string>(props.alwaysDrawn), vp, undefined !== props.isAlwaysDrawnExclusive && props.isAlwaysDrawnExclusive)
    )
      changed = true;

    if (undefined !== props.alwaysDrawnExclusiveEmphasized) this._emphasizeIsolated = new Set<string>(props.alwaysDrawnExclusiveEmphasized); // changed status determined by setAlwaysDrawnElements...

    if (undefined !== props.defaultAppearance) this.defaultAppearance = FeatureAppearance.fromJSON(props.defaultAppearance); // changed status determined by setAlwaysDrawnElements or overrideElements...
    ...
    if (undefined !== props.appearanceOverride) {
      for (const ovrApp of props.appearanceOverride) {
        if (undefined === ovrApp.ids) continue;

        if (this.overrideElements(new Set<string>(ovrApp.ids), vp, ColorDef.fromJSON(ovrApp.color), ovrApp.overrideType, true)) changed = true;
      }
    }
  ...
  public static clear(vp: Viewport, inactiveOnly: boolean = false) {
    ...
    if (undefined === provider || (inactiveOnly && provider.isActive)) return;
