...
export class EmphasizeElements implements FeatureOverrideProvider {
  ...
  private readonly _emphasizedAppearance = FeatureAppearance.fromJSON({
    emphasized: true,
  });
  ...
  public getOverrideFromKey(key: number): {
  ...
  public getEmphasizedIsolatedElements(): Id64Set | undefined {
    return undefined !== this._defaultAppearance &&
      undefined !== this._emphasizeIsolated &&
      0 !== this._emphasizeIsolated.size
  ...
  public isActive(vp: Viewport): boolean {
    return (
      undefined !== this.getNeverDrawnElements(vp) ||
      undefined !== this.getAlwaysDrawnElements(vp) ||
      undefined !== this.getOverriddenElements()
    );
  }
      ...
      for (const ovrApp of props.appearanceOverride) {
        ...
        if (
          this.overrideElements(
            new Set<string>(ovrApp.ids),
            vp,
            ColorDef.fromJSON(ovrApp.color),
            ovrApp.overrideType,
            true
          )
        )
