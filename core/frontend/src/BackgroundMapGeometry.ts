...
export class BackgroundMapGeometry {
  ...
  private static _transitionDistanceMultiplier = 0.25; // In the transition range which extends beyond the cartesian range we interpolate between cartesian and ellipsoid.
  ...
  public async cartographicToDbFromGcs(cartographic: Cartographic, result?: Point3d): Promise<Point3d> {
    ...
    return !this._iModel.noGcsDefined && this.cartesianRange.containsPoint(db)
  ...
  public getEarthEllipsoid(radiusOffset = 0): Ellipsoid {
    const equatorRadius = Constant.earthRadiusWGS84.equator + radiusOffset,
      polarRadius = Constant.earthRadiusWGS84.polar + radiusOffset;
    return Ellipsoid.createCenterMatrixRadii(
      this.globeOrigin,
      this.globeMatrix,
      equatorRadius,
      equatorRadius,
  ...
  public getPlane(offset = 0) {
    return Plane3dByOriginAndUnitNormal.create(
      Point3d.create(0, 0, this._bimElevationBias + offset),
  ...
  public getRayIntersection(ray: Ray3d, positiveOnly: boolean): Ray3d | undefined {
    ...
    if (this.globeMode === GlobeMode.Ellipsoid) {
      ...
      const count = ellipsoid.intersectRay(
        ray,
        BackgroundMapGeometry._scratchRayFractions,
        undefined,
        BackgroundMapGeometry._scratchRayAngles
      );
      let intersectDistance;
      for (let i = 0; i < count; i++) {
        const thisFraction = BackgroundMapGeometry._scratchRayFractions[i];
        if (
          (!positiveOnly || thisFraction > 0) &&
          (undefined === intersectDistance || thisFraction < intersectDistance)
        ) {
  ...
  public getFrustumIntersectionDepthRange(
    ...
    const eyePoint = frustum.getEyePoint(scratchEyePoint);
    const viewRotation = frustum.getRotation(scratchViewRotation);
    if (undefined === viewRotation) return Range1d.createNull(); // Degenerate frustum...
    const viewZ = viewRotation.getRow(2);
          ...
          if (silhouette !== undefined) {
            ...
            clipPlanes.planes.push(
              ClipPlane.createNormalAndDistance(
                ...
                scratchSilhouetteNormal.dotProduct(silhouette.center)
              )!
            );
          } else {
 ...
 * @public
 */
export async function calculateEcefToDbTransformAtLocation(
  originIn: Point3d,
  ...
  const ecefOrigin = Cartographic.fromDegrees({
    longitude: geoOrigin.x,
    latitude: geoOrigin.y,
    height: geoOrigin.z,
  }).toEcef()!;
  const ecefNorth = Cartographic.fromDegrees({
    longitude: geoNorth.x,
    latitude: geoNorth.y,
    height: geoNorth.z,
  }).toEcef()!;
  const ecefEast = Cartographic.fromDegrees({
    longitude: geoEast.x,
    latitude: geoEast.y,
    height: geoEast.z,
  }).toEcef()!;
