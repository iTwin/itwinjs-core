...
import {
  IModelConnectionProps, IModelRpcOpenProps, IModelVersion, RpcManager, RpcNotFoundResponse, RpcRequest,
...
export class CheckpointConnection extends IModelConnection {
   ...
   */
  public static async openRemote(iTwinId: GuidString, iModelId: GuidString, version = IModelVersion.latest(), iModelReadApiFactory: (key: string) => IModelReadAPI): Promise<CheckpointConnection> {
    ...
    const iModelReadApi = iModelReadApiFactory(`${iModelId}:${changeset.id}`);
    if (IpcApp.isValid) {
      connection = new this(await IpcApp.appFunctionIpc.openCheckpoint(iModelProps), iModelReadApi, true);
    } else {
      const routingContext = IModelRoutingContext.current || IModelRoutingContext.default;
      connection = new this(await this.callOpen(iModelProps, iModelReadApi), iModelReadApi, false);
  ...
  private static async callOpen(iModelToken: IModelRpcOpenProps, iModelReadApi: IModelReadAPI): Promise<IModelConnectionProps> {
    Logger.logTrace(loggerCategory, `IModelConnection.open`, iModelToken);
    return {
      ...(await iModelReadApi.getConnectionProps()),
      ...iModelToken,
  ...
  private _reopenConnectionHandler = async (request: RpcRequest<RpcNotFoundResponse>, response: any, resubmit: () => void, reject: (reason?: any) => void) => {
    ...
    try {
      const openResponse = await CheckpointConnection.callOpen(iModelRpcProps, {} as any);
