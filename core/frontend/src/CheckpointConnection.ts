...
export class CheckpointConnection extends IModelConnection {
  ...
  public static async openRemote(iTwinId: string, iModelId: string, version: IModelVersion = IModelVersion.latest()): Promise<CheckpointConnection> {
    ...
    if (undefined === IModelApp.hubAccess)
      throw new Error(
        "Missing an implementation of FrontendHubAccess on IModelApp, it is required to open a remote iModel Connection. Please provide an implementation to the IModelApp.startup using IModelAppOptions.hubAccess."
      );
  ...
  private static async callOpen(iModelToken: IModelRpcOpenProps, routingContext: IModelRoutingContext): Promise<IModelConnectionProps> {
    ...
    if (!openForReadOperation) throw new IModelError(BentleyStatus.ERROR, "IModelReadRpcInterface.getConnectionProps() is not available");
    ...
    const removeListener = RpcRequest.events.addListener((type: RpcRequestEvent, request: RpcRequest) => {
      if (type !== RpcRequestEvent.PendingUpdateReceived) return;
  ...
  private _reopenConnectionHandler = async (
    ...
    if (!response.hasOwnProperty("isIModelNotFoundResponse")) return;
