...
export class GlobeAnimator implements Animator {
  ...
  protected _moveFlightToFraction(fraction: number): boolean {
    ...
    if (!view.is3d() || !vp.iModel.isGeoLocated)
      // This animation only works for 3d views and geolocated models
      return true;

    // If we're done, set the final state directly
    if (fraction >= 1.0) {
      if (vp.view.is3d())
        // Need to reset focus as well -- setupViewFromFustum does not set this and it will remain at flight distance.
    ...
    if (fraction >= this._fixLandingFraction && fraction < 1.0) {
      if (this._fixLandingInterpolator === undefined) {
        ...
        this._fixLandingInterpolator = SmoothTransformBetweenFrusta.create(
          beforeLanding.points,
          this._afterLanding.points
        );
      }
  ...
  ): Promise<GlobeAnimator | undefined> {
    const view = viewport.view;

    if (!view.is3d() || !viewport.iModel.isGeoLocated)
      // This animation only works for 3d views and geolocated models
      return undefined;
  ...
  protected constructor(
    ...
    if (undefined === backgroundMapGeometry) return;
    ...
    if (Geometry.isSmallMetricDistance(this._flightLength)) return;
    ...
    const flightDurationInMilliseconds = metersToRange(
      this._flightLength,
      1000,
      maxFlightDuration,
      ViewGlobalLocationConstants.largestEarthArc
