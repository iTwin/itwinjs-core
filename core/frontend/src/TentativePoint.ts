...
export class TentativePoint {
  ...
  public setCurrSnap(newSnap?: SnapDetail): void {
    if (newSnap) newSnap.setSnapPoint(newSnap.snapPoint, SnapHeat.InRange); // Reset adjustedPoint from pre-located snap and set SnapHeat...
  ...
  public onButtonEvent(ev: BeButtonEvent): void {
    switch (ev.button) {
      case BeButton.Data:
        if (!ev.isDown) return; // cleared on down...
        break;
      case BeButton.Reset:
        if (ev.isDown) return; // cleared on up...
        break;
  ...
  public decorate(context: DecorateContext): void {
    ...
    if (!this.isActive || !viewport) return;
  ...
  private async getSnap(newSearch: boolean): Promise<SnapDetail | undefined> {
    ...
    const thisSnap = await AccuSnap.requestSnap(
      ...
      IModelApp.accuSnap.keypointDivisor,
      this.tpHits
    );

    if (undefined !== thisSnap) IModelApp.accuDraw.onSnap(thisSnap); // AccuDraw can adjust nearest snap to intersection of circle (polar distance lock) or line (axis lock) with snapped to curve...
  ...
  public process(ev: BeButtonEvent): void {
    if (undefined !== this._tentativePromise) return;

    const currTool = IModelApp.toolAdmin.viewTool;
    if (currTool && currTool.inDynamicUpdate) return; // trying to tentative snap while view is changing isn't useful...
    ...
    const newSearch =
      !this.isSnapped ||
      !TentativePoint.arePointsCloseEnough(
        lastPtView,
        this._viewPoint,
        this.viewport.pixelsFromInches(IModelApp.locateManager.apertureInches)
      );
    ...
    promise.then((newSnap) => {
      ...
      if (this._tentativePromise === promise) {
        ...
        if (this.isSnapped) this._point.setFrom(this.currSnap!.snapPoint);
        else if (wasActive && newSearch) this._point.setFrom(ev.rawPoint);
