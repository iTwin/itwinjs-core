...
describe("Vector3d functions", () => {
  it("fromNormalizedCrossProduct", () => {
    ...
    assert.isTrue(
      fromNormalizedCrossProduct(vec0, vec1)!.isAlmostEqual(expectedResult),
      "fromNormalizedCrossProduct works as expected"
    );
  });
  it("normalizedDifference", () => {
    ...
    assert.isTrue(
      normalizedDifference(target, origin)!.isAlmostEqual(expectedResult),
      "normalizedDifference works as expected"
    );
  });
});
