...
describe("EdgeSettings", () => {
  ...
  it("initializes correctly based on HiddenLine.Settings", () => {
    ...
    function makeSettings(
      visible?: HiddenLine.StyleProps,
      hidden?: HiddenLine.StyleProps
    ): HiddenLine.Settings {
    ...
    function makeExpected(
      ...
      const visCode =
        undefined !== visPix ? LineCode.valueFromLinePixels(visPix) : undefined;
      const hidCode =
        undefined !== hidPix ? LineCode.valueFromLinePixels(hidPix) : undefined;
    ...
    const testCases = [
      {
        settings: makeSettings(
          makeStyle(ColorDef.red, 4, LinePixels.Code2),
          makeStyle(ColorDef.green, 2, LinePixels.Code3)
        ),
      ...
      },
      {
        ...
        settings: makeSettings(
          makeStyle(undefined, 0, LinePixels.Invalid),
          makeStyle(undefined, 4, LinePixels.Invalid)
        ),
      ...
      },
      {
        ...
        settings: makeSettings(
          makeStyle(undefined, undefined, 1234321 as LinePixels),
          makeStyle(undefined, undefined, 4321234 as LinePixels)
        ),
        expected: makeExpected(
          undefined,
          undefined,
      ...
      {
        settings: makeSettings(
          makeStyle(undefined, 40, undefined),
          makeStyle(undefined, -10, undefined)
        ),
        expected: makeExpected(
          undefined,
          32,
  ...
  it("overrides defaults for certain render modes", () => {
    ...
    for (const renderMode of [RenderMode.HiddenLine, RenderMode.SolidFill])
      for (const pass of [RenderPass.OpaqueLinear, RenderPass.HiddenEdge]) {
        ...
        expect(es.wantContrastingColor(renderMode)).to.equal(
          RenderMode.SolidFill === renderMode
        );
  ...
  it("clamps and inverts transparency threshold", () => {
    ...
    for (const inputOutput of inputsOutputs) {
      const es = EdgeSettings.create(
        HiddenLine.Settings.fromJSON({ transThreshold: inputOutput[0] })
      );
  ...
  it("uses expected defaults for hidden edges if hidden edges unspecified", () => {
    ...
    const es = EdgeSettings.create(
      HiddenLine.Settings.fromJSON({
        visible: { width: 3, pattern: LinePixels.Code4 },
      })
    );
    expect(es.getWeight(RenderPass.HiddenEdge, vf)).to.equal(
      es.getWeight(RenderPass.OpaqueLinear, vf)
    );
    expect(es.getLineCode(RenderPass.HiddenEdge, vf)).to.equal(hiddenPattern);

    es.init(
      HiddenLine.Settings.fromJSON({
        visible: { width: 4, pattern: LinePixels.Code3 },
        hidden: {},
      })
    );
    expect(es.getWeight(RenderPass.HiddenEdge, vf)).to.equal(
  ...
  it("does not permit hidden edges to be wider than visible edges", () => {
    ...
    expect(
      es.getWeight(
        RenderPass.HiddenEdge,
        ViewFlags.fromJSON({ renderMode: RenderMode.SolidFill })
      )
    ).to.equal(2);
  });
});
