...
describe("RenderState", () => {
  ...
  it("should apply state", () => {
    ...
    newState.apply(prevState);
    assert.isTrue(
      gl.getParameter(GL.Capability.FrontFace) === GL.FrontFace.Clockwise,
      "frontFace should now be Clockwise"
    );
    ...
    newState.apply(prevState);
    assert.isTrue(
      gl.getParameter(GL.Capability.DepthFunc) === GL.DepthFunc.GreaterOrEqual,
      "depthFunc should now be GreaterOrEqual"
    );
