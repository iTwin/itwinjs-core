...
describe("Surface transparency", () => {
  ...
  beforeAll(async () => {
    ...
    expect(opaqueTexture).toBeDefined();
    ...
    expect(translucentTexture).toBeDefined();
  ...
  function createMaterial(alpha?: number, texture?: RenderTexture, textureWeight?: number): RenderMaterial {
	...
	it("ignores material and texture transparency if both view flags are disabled", () => {
	  ...
	  const materials = [createMaterial(1, opaqueTexture), createMaterial(0.5, opaqueTexture), createMaterial(0.5, translucentTexture), createMaterial(1, translucentTexture)];
	...
	it("always applies to glyph text unless reading pixels", () => {
	  ...
	  expect((viewport.target as Target).isReadPixelsInProgress).toBe(true);
