...
describe("GraphicPrimitive", () => {
  class Builder extends GraphicBuilder {
    ...
    public override addLineString2d(points: Point2d[], zDepth: number) {
      this.primitive = { type: "linestring2d", points, zDepth };
    }
    public override addPointString(points: Point3d[]) {
      this.primitive = { type: "pointstring", points };
    }
    public override addPointString2d(points: Point2d[], zDepth: number) {
      this.primitive = { type: "pointstring2d", points, zDepth };
    }
    public override addShape(points: Point3d[]) {
      this.primitive = { type: "shape", points };
    }
    public override addShape2d(points: Point2d[], zDepth: number) {
      this.primitive = { type: "shape2d", points, zDepth };
    }
    public override addArc(arc: Arc3d, isEllipse: boolean, filled: boolean) {
      this.primitive = { type: "arc", arc, isEllipse, filled };
    }
    public override addArc2d(arc: Arc3d, isEllipse: boolean, filled: boolean, zDepth: number) {
      this.primitive = { type: "arc2d", arc, isEllipse, filled, zDepth };
    }
    public override addPath(path: Path) {
      this.primitive = { type: "path", path };
    }
    public override addLoop(loop: Loop) {
      this.primitive = { type: "loop", loop };
    }
    public override addPolyface(polyface: Polyface, filled: boolean) {
      this.primitive = { type: "polyface", polyface, filled };
    }
    public override addSolidPrimitive(solidPrimitive: SolidPrimitive) {
      this.primitive = { type: "solidPrimitive", solidPrimitive };
    }
  ...
  it("forwards to appropriate method with appropriate arguments", () => {
    ...
    const arc = Arc3d.createXYZXYZXYZ(1, 2, 3, 4, 5, 6, 7, 8, 9);
		...
		const tests: Test[] = [
		  ...
		  [
		    { type: "arc", arc },
		    { type: "arc", arc, isEllipse: false, filled: false },
		  ],
