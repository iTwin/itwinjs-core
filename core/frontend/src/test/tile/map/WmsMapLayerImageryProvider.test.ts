...
describe("WmsMapLayerImageryProvider", () => {
  ...
  it("construct proper tile url", async () => {
    ...
    const refUrl =
      "https://sub.service.com/service?SERVICE=WMS&VERSION=undefined&REQUEST=GetMap&FORMAT=image%2Fpng&TRANSPARENT=TRUE&LAYERS=sublayer&WIDTH=256&HEIGHT=256&SRS=EPSG%3A3857&STYLES=&BBOX=1,2,3,4";
  ...
  it("construct proper tooltip url", async () => {
    ...
    await provider.initialize();
    await provider.getToolTip(
      [],
      new QuadId(0, 0, 0),
      Cartographic.createZero(),
      ({ getTileRectangle: () => MapCartoRectangle.createZero() } as unknown) as ImageryMapTileTree,
    );
    const refUrl =
      "https://sub.service.com/service?SERVICE=WMS&VERSION=undefined&REQUEST=GetFeatureInfo&LAYERS=sublayer&WIDTH=256&HEIGHT=256&SRS=EPSG%3A3857&BBOX=1,2,3,4&QUERY_LAYERS=sublayer&x=0&y=256&info_format=text/html";
  ...
  it("initialize() should handle 401 error from WmsCapabilities", async () => {
    ...
    const createSub = sandbox.stub(WmsCapabilities, "create").callsFake(
      async function _(_url: string, _credentials?: RequestBasicCredentials, _ignoreCache?: boolean) {
  ...
  it("initialize() should handle 401 error from WmtsCapabilities", async () => {
    ...
    const createSub = sandbox.stub(WmtsCapabilities, "create").callsFake(
      async function _(_url: string, _credentials?: RequestBasicCredentials, _ignoreCache?: boolean) {
        // eslint-disable-next-line no-throw-literal
        throw { status: 401 };
      },
  ...
  it("initialize() should handle unknown exception from WmtsCapabilities", async () => {
    ...
    sandbox.stub(WmtsCapabilities, "create").callsFake(
      async function _(_url: string, _credentials?: RequestBasicCredentials, _ignoreCache?: boolean) {
        throw { someError: "error" }; // eslint-disable-line no-throw-literal
      },
    );
