...
describe("WmtsCapabilities", () => {
  ...
  it("should parse USGS WMTS capabilities", async () => {
    ...
    if (capabilities?.contents?.tileMatrixSets[0].tileMatrix[0].scaleDenominator)
      expect(
        Math.abs(capabilities.contents.tileMatrixSets[0].tileMatrix[0].scaleDenominator - 5.590822640285016e8)
    ...
    expect(capabilities?.contents?.tileMatrixSets[1].wellKnownScaleSet).to.contains(
  ...
  it("should parse sample OGC WMTS capabilities", async () => {
    ...
    if (capabilities?.operationsMetadata?.getCapabilities?.postDcpHttp) {
      ...
      expect(capabilities.operationsMetadata.getCapabilities.postDcpHttp[0].url).to.equals(
        "http://www.maps.bob/maps.cgi?"
      );
    }
    ...
    if (capabilities?.operationsMetadata?.getFeatureInfo?.getDcpHttp) {
      ...
      expect(capabilities.operationsMetadata.getFeatureInfo.getDcpHttp[0].url).to.equals(
        "http://www.maps.bob/maps.cgi?"
      );
    }
