...
describe("BackgroundMapGeometry", () => {
  ...
  it("provide cartographics from iModel coordinates", async () => {
    const wgs84CartographicFromSpatialFake = sandbox.stub(IModelConnection.prototype, "wgs84CartographicFromSpatial").callsFake(
      async function _(spatial: XYAndZ[]): Promise<Cartographic[]> {
        return Promise.resolve(spatial.map((value) => Cartographic.fromRadians({ longitude: value.x, latitude: value.y, height: value.z })));
      },
    );

    const dbToCartographicFake = sandbox.stub(BackgroundMapGeometry.prototype, "dbToCartographic").callsFake(
      function _(db: XYAndZ, _result?: Cartographic): any {
