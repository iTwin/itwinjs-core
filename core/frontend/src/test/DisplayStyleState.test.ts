  ...
  describe("schedule script state", () => {
    ...
    class Style extends DisplayStyle3dState {
      ...
      public constructor() {
        ...
        this.eventPayloads = [];
        this.onScheduleScriptChanged.addListener((ref) => this.eventPayloads.push(new RenderSchedule.ScriptReference(ref!)));
      }
    ...
    it("raises onScheduleScriptReferenceChanged", async () => {
      ...
      function pushExpected(expectNonNull = true) {
        ...
        expected.push(new RenderSchedule.ScriptReference(style.scheduleScript!));
        expectPayloads();
      }
