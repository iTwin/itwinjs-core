...
describe("RealityDataSourceG3DTImpl", () => {
  it("handle content type of relative urls", async () => {
    const rdSource = await RealityDataSourceG3DTImpl.createFromKey({ format: "", id: "", provider : RealityDataProvider.G3DT }, undefined);
  ...
  describe("getTileUrl", () => {
    class TestG3DTImpl extends RealityDataSourceG3DTImpl {
      ...
      public static override async createFromKey(sourceKey: RealityDataSourceKey): Promise<TestG3DTImpl | undefined> {
        const rdSource = await RealityDataSourceG3DTImpl.createFromKey(sourceKey , undefined) as TestG3DTImpl;
