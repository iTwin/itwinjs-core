      ...
      iModel[_requestSnap] = async (props) =>
		...
		async function requestSnap(vp: ScreenViewport, hit: HitDetailProps, snapModes: SnapMode[] = []): Promise<SnapResponse> {
		  ...
		  if (detail) {
		    ...
		    expect(response.snapStatus).not.toEqual(SnapStatus.Success);
		...
		it("pre-multiplies display transform if specified", async () => {
		  await testSnap(
		    ...
		    (vp) => {
		      vp.view.getModelElevation = () => 10;
		      vp.view.modelDisplayTransformProvider = new Transformer(Transform.createRefs(undefined, Matrix3d.createRotationAroundAxisIndex(AxisIndex.X, Angle.createDegrees(-90))), true);
		    },
		  );
		});
  });
