...
export abstract class RealityTileLoader {
  ...
  public get maximumScreenSpaceError(): number | undefined {
    return undefined;
  }
  ...
  public async loadGeometryFromStream(
    tile: RealityTile,
    ...
    return {
      geometry: reader?.readGltfAndCreateGeometry(tile.tree.iModelTransform),
    };
  }
          ...
          if (!tile.transformToRoot && rtcCenter)
            ...
            if (rtcCenter)
              xform = Transform.createOriginAndMatrix(
                rtcCenter.plus(tile.transformToRoot!.origin),
                tile.transformToRoot!.matrix
              );
            else xform = tile.transformToRoot!;
          }
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            focusPointCandidate.x < 1 &&
            focusPointCandidate.y > 0 &&
            focusPointCandidate.y < 1
          )
