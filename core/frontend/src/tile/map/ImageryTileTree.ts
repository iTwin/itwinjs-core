...
export class ImageryMapTile extends RealityTile {
  ...
  public selectCartoDrapeTiles(drapeTiles: ImageryMapTile[], replacementLeafTiles: ImageryMapTile[], rectangleToDrape: MapCartoRectangle, drapePixelSize: number, args: TileDrawArgs): TileTreeLoadStatus {
    ...
    if (  (this.isLeaf )           // Include leaves so tiles get stretched past max LOD levels. (Only for base imagery layer)
      ...
      if (this.isOutOfLodRange ) {
        ...
        replacementLeafTiles.push(this);
      } else {
        drapeTiles.push(this);
      }
      ...
      if (undefined !== this.children) {
        for (const child of this.children) {
          ...
          if (mapChild.rectangle.intersectsRange(rectangleToDrape))
            status = mapChild.selectCartoDrapeTiles(drapeTiles, replacementLeafTiles, rectangleToDrape, drapePixelSize, args);
