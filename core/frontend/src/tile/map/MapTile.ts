...
export class MapTile extends RealityTile {
    ...
    const resolvePlanarChildren = (childCorners: Point3d[][]) => {
      ...
      for (let j = 0; j < rowCount; j++) {
        for (let i = 0; i < columnCount; i++) {
          ...
          children.push(this.mapTree.createPlanarChild({ contentId: quadId.contentId, maximumSize: 512, range, parent: this, isLeaf: childrenAreLeaves, rangeCorners }, quadId, corners, normal, rectangle, chordHeight, heightRange));
        }
      }
  ...
  public override produceGraphics(): RenderGraphic | undefined {
    ...
    const { baseColor, baseTransparent, modelLayers: layerClassifiers } = this.mapTree;
    ...
    if (this.imageryIsReady && 0 === this.mapTree.modelLayers.size)
