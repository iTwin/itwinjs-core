...
export class WmsMapLayerImageryProvider extends MapLayerImageryProvider {
          ...
          this._settings.subLayers.forEach((subLayer) => {
            if (
              ...
              if (subLayerRange)
                ...
                else this.cartoRange = subLayerRange.clone();
            }
          });
        }

        if (!this.cartoRange) this.cartoRange = this._allLayersRange;
  ...
  public getCrsSupport(): WmsCrsSupport {
    ...
    return {
      support3857: support3857 ?? false,
      support4326: support4326 ?? false,
    };
  }
  ...
  public override async getToolTip(
    ...
    if (!doToolTips || undefined === infoFormats) return;
