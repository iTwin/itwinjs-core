...
export class ArcGISMapLayerImageryProvider extends MapLayerImageryProvider {
  ...
  public override async initialize(): Promise<void> {
    ...
    if (json === undefined) throw new ServerError(IModelStatus.ValidationFailed, "");
    ...
    if (json !== undefined) {
      ...
      if (false !== (this._usesCachedTiles = json.tileInfo !== undefined && this.isEpsg3857Compatible(json.tileInfo))) {
        if (json.maxScale !== undefined && json.maxScale !== 0 && Array.isArray(json.tileInfo.lods)) {
          for (
            ;
            this._maxDepthFromLod < json.tileInfo.lods.length && json.tileInfo.lods[this._maxDepthFromLod].scale > json.maxScale;
  ...
  private async getIdentifyData(quadId: QuadId, carto: Cartographic, tolerance: number): Promise<any> {
    ...
    const tmpUrl = `${this._settings.url}/identify?f=json&tolerance=${tolerance}&returnGeometry=false&sr=3857&imageDisplay=${this.tileSize},${
      this.tileSize
    },96&layers=${this.getLayerString("visible")}&geometry=${x},${y}&geometryType=esriGeometryPoint&mapExtent=${bboxString}`;
  ...
  public async constructUrl(row: number, column: number, zoomLevel: number): Promise<string> {
    ...
    if (this._usesCachedTiles) {
      ...
      tmpUrl = `${this._settings.url}/export?bbox=${bboxString}&size=${this.tileSize},${
        this.tileSize
      }&layers=${this.getLayerString()}&format=png&transparent=${this.transparentBackgroundString}&f=image&sr=3857&imagesr=3857`;
    }
    const urlObj = new URL(tmpUrl);
    try {
      if (this._accessClient) {
