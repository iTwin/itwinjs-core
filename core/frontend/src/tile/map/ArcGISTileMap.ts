      ...
      if (Array.isArray(json.data)) {
        ...
        for (let j = 0; j < tileMapWidth; j++) {
          for (let i = 0; i < tileMapHeight; i++) {
            ...
            if (
              curColumn >= queryTiles[0].column &&
              curColumn <= queryTiles[queryTiles.length - 1].column &&
              curRow >= queryTiles[0].row &&
              curRow <= queryTiles[queryTiles.length - 1].row
            ) {
              available[k++] = avail;
            }
          }
    ...
    try {
      ...
      if (available.length !== childIds.length) {
        if (this.tileMapRequestSize > this.fallbackTileMapRequestSize) {
          ...
          available = await this.fetchAndReadTilemap(
            childIds,
            this.fallbackTileMapRequestSize,
            this.fallbackTileMapRequestSize
          );
        }
