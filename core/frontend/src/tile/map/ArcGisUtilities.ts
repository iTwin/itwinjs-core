  ...
  public static async getSourcesFromQuery(
    ...
    for (let start = 1; start > 0;) {
      const response = await fetch(
        `${url}?f=json&q=(group:9d1199a521334e77a7d15abbc29f8144) AND (type:"Map Service")&bbox=${
          ArcGisUtilities.getBBoxString(range)
    ...
    if (json === undefined) {
      ...
      if (json.error.code === ArcGisErrorCode.TokenRequired || json.error.code === ArcGisErrorCode.MissingPermissions) {
        return (source.userName || source.password)
          ? { status: MapLayerSourceStatus.InvalidCredentials }
  ...
  public static getZoomLevelsScales(
    ...
    if (zoomScales.length > 0) {
      if (minScale) {
        ...
        for (
          ;
          minLod < zoomScales.length && (zoomScales[minLod].scale > minScale && Math.abs(zoomScales[minLod].scale - minScale) > tolerance);
          minLod++
        );
      }
