...
export abstract class MapLayerImageryProvider {
  ...
  public async initialize(): Promise<void> {
    this.loadTile(0, 0, 22).then((tileData: ImageSource | undefined) => {
  ...
  public getPotentialChildIds(tile: ImageryMapTile): QuadId[] {
    ...
    return tile.quadId.getChildIds(
      this.tilingScheme.getNumberOfXChildrenAtLevel(childLevel),
      this.tilingScheme.getNumberOfYChildrenAtLevel(childLevel)
    );
  }
  ...
  protected getImageFromTileResponse(tileResponse: Response, zoomLevel: number) {
    ...
    if (this.matchesMissingTile(byteArray) && zoomLevel > 8) return undefined;
  ...
  public matchesMissingTile(tileData: Uint8Array): boolean {
    if (!this._missingTileData) return false;
    if (tileData.length !== this._missingTileData.length) return false;
  ...
  public getEPSG4326Extent(
    row: number,
    column: number,
    zoomLevel: number
  ): { longitudeLeft: number; longitudeRight: number; latitudeTop: number; latitudeBottom: number } {
