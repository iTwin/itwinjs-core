...
export class GltfHeader extends TileHeader {
  ...
  public constructor(stream: ByteStream) {
    ...
    if (this.version === GltfVersions.Version1) {
      ...
      if (
        GltfV2ChunkTypes.JSON !== sceneChunkType ||
        GltfV2ChunkTypes.Binary !== binaryChunkType ||
        0 === binaryLength
      ) {
        this.invalidate();
        return;
 ...
 * @see [[AuxChannelTable]].
 */
type ImdlAuxChannelTable = Omit<AuxChannelTableProps, "data"> & {
  bufferView: string;
};
...
export class ImdlReader {
  ...
  private get _isVolumeClassifier(): boolean {
  ...
  public static create(args: ImdlReaderCreateArgs): ImdlReader | undefined {
    ...
    if (!imdlHeader.isValid || !imdlHeader.isReadableVersion) return undefined;

    // The feature table follows the iMdl header
    if (!this.skipFeatureTable(args.stream)) return undefined;

    // A glTF header follows the feature table
    const gltfHeader = new GltfHeader(args.stream);
    if (!gltfHeader.isValid) return undefined;
    ...
    try {
      ...
      return undefined !== imdl.meshes
        ? new ImdlReader(
            imdl,
            gltfHeader.binaryPosition,
            args,
            0 !== (imdlHeader.flags & ImdlFlags.MultiModelFeatureTable)
          )
        : undefined;
    } catch (_) {
      return undefined;
  ...
  private constructor(
    ...
    args: ImdlReaderCreateArgs,
    hasMultiModelFeatureTable: boolean
  ) {
  ...
  public async read(): Promise<ImdlReaderResult> {
    ...
    return this.finishRead(
      ...
      content.emptySubRangeMask,
      content.sizeMultiplier
    );
  }
          ...
          if (undefined !== texture) {
            ...
            textureMapping = new TextureMapping(
              ...
              new TextureMapping.Params({
                textureMat2x3: new TextureMapping.Trans2x3(0, 1, 0, 1, 0, 0),
              })
            );
          }
        }
  ...
  protected materialFromJson(key: string): RenderMaterial | undefined {
    ...
    if (!this._renderMaterials) return undefined;

    const materialJson = this._renderMaterials[key];
    if (!materialJson) return undefined;
  ...
  private constantLodParamPropsFromJson(
    propsJson:
      | {
          repetitions?: number;
          offset?: number[];
    ...
    if (undefined === propsJson) return undefined;
  ...
  private async readNamedTexture(namedTex: ImdlNamedTexture, name: string): Promise<RenderTexture | undefined> {
    ...
    if (isGlyph) textureType = RenderTexture.Type.Glyph;
    else if (isTileSection) textureType = RenderTexture.Type.TileSection;
    ...
    if (undefined !== bufferViewJson) {
      ...
      return this._system.createTextureFromSource({
        ...
        ownership,
        type: textureType,
        transparency: namedTex.transparency,
      });
    }
  ...
  protected readFeatureTable(startPos: number): RenderFeatureTable | undefined {
    ...
    if (undefined === header || 0 !== header.length % 4) return undefined;
    ...
    if (this._hasMultiModelFeatureTable) {
      featureTable = MultiModelPackedFeatureTable.create(
        packedFeatureArray,
        this._modelId,
        header.count,
        this._type,
        header.numSubCategories
      );
    } else {
      ...
      featureTable = new PackedFeatureTable(
        packedFeatureArray,
        this._modelId,
        header.count,
        this._type,
  ...
  private static skipFeatureTable(stream: ByteStream): boolean {
    ...
    if (undefined !== header) stream.curPos = startPos + header.length;
  ...
  private readVertexTable(primitive: AnyImdlPrimitive): VertexTable | undefined {
    ...
    if (undefined === rangeMin || undefined === rangeMax) return undefined;

    const qparams = QParams3d.fromRange(
      Range3d.create(
        Point3d.create(rangeMin[0], rangeMin[1], rangeMin[2]),
        Point3d.create(rangeMax[0], rangeMax[1], rangeMax[2])
      )
    );
  ...
  private readInstances(primitive: ImdlPrimitive): InstancedGraphicParams | undefined {
    ...
    if (undefined === transformBytes) return undefined;
  ...
  private readTesselatedPolyline(json: ImdlPolyline): TesselatedPolyline | undefined {
    ...
    if (!indices || !prevIndices || !nextIndicesAndParams) return undefined;
  ...
  private readSegmentEdges(json: ImdlSegmentEdges): SegmentEdgeParams | undefined {
    ...
    return undefined !== indices && undefined !== endPointAndQuadIndices
      ? { indices, endPointAndQuadIndices }
      : undefined;
  }
  ...
  private readSilhouettes(json: ImdlSilhouetteEdges): SilhouetteParams | undefined {
    ...
    return undefined !== segments && undefined !== normalPairs
      ? {
          ...
          endPointAndQuadIndices: segments.endPointAndQuadIndices,
        }
      : undefined;
  }
  ...
  private readPrimitiveParams(primitive: AnyImdlPrimitive): PrimitiveParams | undefined {
    ...
    switch (primitive.type) {
      ...
      case Mesh.PrimitiveType.Polyline: {
        ...
        return {
          params: new PolylineParams(
            vertices,
            polyline,
            displayParams.width,
            displayParams.linePixels,
            isPlanar,
            flags
          ),
          type: "polyline",
  ...
  private readAnimationBranches(output: RenderGraphic[], mesh: ImdlMesh, featureTable: PackedFeatureTable): void {
    ...
    for (const primitive of primitives) {
      if (primitive.type === "areaPattern") {
        ...
        if (gf) getBranch(AnimationNodeId.Untransformed).add(gf);
      } else {
        const prim = this.readPrimitiveParams(primitive);
        if (!prim) continue;
        ...
        switch (prim.type) {
          case "mesh": {
            ...
            for (const [nodeId, params] of split) {
              const geometry = this.createPrimitiveGeometry({
                params,
                viOrigin,
                type: "mesh",
              });
          ...
          case "polyline": {
            const split = splitPolylineParams({
              ...splitArgs,
              params: prim.params,
            });
            for (const [nodeId, params] of split) {
  ...
  private readBranch(
    ...
    primitives: Array<AnyImdlPrimitive | ImdlAreaPattern>,
    nodeId: number,
  ...
  private finishRead(
    ...
    if (undefined === this._nodes.Node_Root) {
      ...
      for (const meshKey of Object.keys(this._meshes)) {
        ...
        for (const primitive of primitives) {
          const graphic = this.readMeshGraphic(primitive);
          if (undefined !== graphic) graphics.push(graphic);
        }
      }
    } else {
      for (const nodeKey of Object.keys(this._nodes)) {
        ...
        if ("Node_Root" === nodeKey) {
          if (this._timeline) {
            ...
            assert(
              featureTable instanceof PackedFeatureTable,
              "multi-model feature tables never include animation branches"
            );
          ...
          for (const primitive of primitives) {
            const graphic = this.readMeshGraphic(primitive);
            if (undefined !== graphic) layerGraphics.push(graphic);
          }
