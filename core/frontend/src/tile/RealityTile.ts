...
export class RealityTile extends Tile {
  ...
  public constructor(props: RealityTileParams, tree: RealityTileTree) {
    ...
    if (undefined === this.transformToRoot) return;
  ...
  /** @internal */
  public get realityChildren(): RealityTile[] | undefined {
    return this.children as RealityTile[] | undefined;
  }
  /** @internal */
  public get realityParent(): RealityTile {
    return this.parent as RealityTile;
  }
  /** @internal */
  public get realityRoot(): RealityTileTree {
    return this.tree as RealityTileTree;
  }
  /** @internal */
  public get graphicType(): TileGraphicType | undefined {
    return undefined;
  } // If undefined, use tree type.
  /** @internal */
  public get maxDepth(): number {
    return this.realityRoot.loader.maxDepth;
  }
  /** @internal */
  public get isPointCloud() {
    return this.realityRoot.loader.containsPointClouds;
  }
  /** @internal */
  public get isLoaded() {
    return this.loadStatus === TileLoadStatus.Ready;
  } // Reality tiles may depend on secondary tiles (maps) so can ge loaded but not ready.
  /** @internal */
  public get geometry(): RealityTileGeometry | undefined {
    return this._geometry;
  }

  /** @internal */
  public override get isDisplayable(): boolean {
    if (this.noContentButTerminateOnSelection) return false;
    else return super.isDisplayable;
  }
  ...
  private useAdditiveRefinementStepchildren() {
    ...
    return (
      ...
      this.realityRoot.doReprojectChildren(this)
    );
  }
      ...
      .then((children: Tile[] | undefined) => {
        ...
        if (this.useAdditiveRefinementStepchildren())
          this.loadAdditiveRefinementChildren((stepChildren: Tile[]) => {
            children = children ? children?.concat(stepChildren) : stepChildren;
          });

        if (children) this.realityRoot.reprojectAndResolveChildren(this, children, resolve); /* Potentially reprojecect and resolve these children */
      })
      .catch((err) => {
        reject(err);
      });
  ...
  protected selectRealityChildren(context: TraversalSelectionContext, args: TileDrawArgs, traversalDetails: TraversalDetails) {
    ...
    if (undefined !== this.realityChildren) {
      ...
      for (let i = 0; i < this.children!.length; i++) this.realityChildren[i].selectRealityTiles(context, args, traversalChildren.getChildDetail(i));
  ...
  protected getLoadedRealityChildren(args: TileDrawArgs): boolean {
    ...
    for (const child of this.realityChildren) {
      ...
      } else if (!child.getLoadedRealityChildren(args)) return false;
    }
    return true;
  }
    ...
    if (this.isDisplayable && (visibility >= 1 || this._anyChildNotFound || this.forceSelectRealityTile() || context.selectionCountExceeded)) {
      if (!this.isOccluded(args.viewingSpace)) {
        ...
        if (!this.isReady) {
          ...
          if (this.getLoadedRealityChildren(args)) context.select(scratchLoadedChildren, args);
      ...
      if (this.isReady && (traversalDetails.childrenLoading || 0 !== traversalDetails.queuedChildren.length)) {
        const minimumVisibleFactor = 0.25; // If the tile has not yet been displayed in this viewport -- display only if it is within 25% of visible. Avoid overly tiles popping into view unexpectedly (terrain)

        if (visibility > minimumVisibleFactor || this._everDisplayed) context.selectOrQueue(this, args, traversalDetails);
      }
    }
  }
  ...
  public purgeContents(olderThan: BeTimePoint): void {
    ...
    if (this.usageMarker.isExpired(olderThan)) this.disposeContents();

    const children = this.realityChildren;
    if (children) for (const child of children) child.purgeContents(olderThan);
  }
  ...
  public preloadTilesInFrustum(args: TileDrawArgs, context: TraversalSelectionContext, preloadSizeModifier: number) {
    ...
    if (visibility * preloadSizeModifier > 1) {
      if (this.isDisplayable) context.preload(this, args);
    } else {
      const childrenLoadStatus = this.loadChildren(); // NB: asynchronous
      if (TileTreeLoadStatus.Loading === childrenLoadStatus) {
        args.markChildrenLoading();
      } else if (undefined !== this.realityChildren) {
        for (const child of this.realityChildren) child.preloadTilesInFrustum(args, context, preloadSizeModifier);
      }
    }
  }
  ...
  public override getSizeProjectionCorners(): Point3d[] | undefined {
    if (!this.tree.isContentUnbounded) return undefined; // For a non-global tree use the standard size algorithm.
    ...
    if (!this.rangeCorners) return this.range.corners(scratchCorners);

    return this.region ? this.rangeCorners.slice(4) : this.rangeCorners;
  }

  /** @internal */
  public get isStepChild() {
    return false;
  }
    ...
    for (let i = 0, minLongitude = region.minLongitude, step = 0; i < 2; i++, minLongitude += longitudeDelta, step++) {
      for (let j = 0, minLatitude = region.minLatitude; j < 2; j++, minLatitude += latitudeDelta) {
        const childRegion = new RealityTileRegion({
          ...
          minLongitude,
          maxLongitude: minLongitude + longitudeDelta,
          minHeight,
          maxHeight,
        });
        ...
        const childParams: RealityTileParams = {
          rangeCorners: childRange.corners,
          contentId,
          range: childRange.range,
          maximumSize,
          parent: this,
          additiveRefinement: false,
          isLeaf,
          region: childRegion,
        };
...
class AdditiveRefinementStepChild extends RealityTile {
  public override get isStepChild() {
    return true;
  }
  ...
  public override get isLoading(): boolean {
    return this._loadableTile.isLoading;
  }
  public override get isQueued(): boolean {
    return this._loadableTile.isQueued;
  }
  public override get isNotFound(): boolean {
    return this._loadableTile.isNotFound;
  }
  public override get isReady(): boolean {
    return this._loadableTile.isReady;
  }
  public override get isLoaded(): boolean {
    return this._loadableTile.isLoaded;
  }
  ...
  public override produceGraphics(): RenderGraphic | undefined {
    if (undefined === this._graphic) {
      ...
      if (!parentGraphics || !this._reprojectionTransform) return undefined;
