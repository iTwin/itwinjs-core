...
export class RealityTile extends Tile {
      ...
      .then((children: Tile[] | undefined) => {
        ...
        if (children)
          this.realityRoot.reprojectAndResolveChildren(
            ...
            children,
            resolve
          ); /* Potentially reproject and resolve these children */
      })
      .catch((err) => {
        reject(err);
      });
  ...
  protected minimumVisibleFactor(): number {
    if (this.additiveRefinement) return 0.25;
    else return 0;
  }
  ...
  public selectRealityTiles(
    ...
    if (shouldSelectThisTile && this.isDisplayable) {
      ...
      if (this.isOccluded(args.viewingSpace)) return;
  ...
  /** @internal */
  protected selectRealityChildrenAsFallback(
    context: TraversalSelectionContext,
    args: TileDrawArgs,
    traversalDetails: TraversalDetails
  ) {
  ...
  /** @internal */
  protected selectRealityChildren(
    context: TraversalSelectionContext,
    args: TileDrawArgs,
    traversalDetails: TraversalDetails
  ) {
  ...
  public purgeContents(olderThan: BeTimePoint, useProtectedTiles: boolean): void {
    ...
    for (const tile of tilesToPurge) tile.disposeContents();
  }
  ...
  private getTilesToPurgeWithoutProtection(olderThan: BeTimePoint, tilesToPurge: Set<RealityTile>): void {
    ...
    if (this.usageMarker.isExpired(olderThan)) tilesToPurge.add(this);
  }
