...
class OrbitGtTreeSupplier implements TileTreeSupplier {
  ...
  public compareTileTreeIds(lhs: OrbitGtTreeId, rhs: OrbitGtTreeId): number {
    ...
    if (0 === cmp) cmp = compareStringsOrUndefined(lhs.rdSourceKey.format, rhs.rdSourceKey.format);
    if (0 === cmp) cmp = compareStringsOrUndefined(lhs.rdSourceKey.iTwinId, rhs.rdSourceKey.iTwinId);
    if (0 === cmp) cmp = compareStringsOrUndefined(lhs.modelId, rhs.modelId);
...
function transformFromOrbitGt(ogtTransform: OrbitGtTransform, result?: Transform): Transform {
  ...
  result.matrix.setRowValues(
    ogtTransform.getElement(0, 0),
    ogtTransform.getElement(0, 1),
    ogtTransform.getElement(0, 2),
    ogtTransform.getElement(1, 0),
    ogtTransform.getElement(1, 1),
    ogtTransform.getElement(1, 2),
    ogtTransform.getElement(2, 0),
    ogtTransform.getElement(2, 1),
    ogtTransform.getElement(2, 2)
  );
...
function rangeFromOrbitGt(ogtBounds: OrbitGtBounds, result?: Range3d) {
  if (undefined === result) result = Range3d.create();
...
class OrbitGtRootTile extends Tile {
  ...
  public get channel() {
    return IModelApp.tileAdmin.channels.getForHttp("itwinjs-orbitgit");
  }
...
export class OrbitGtTileTree extends TileTree {
  ...
  public constructor(
    treeParams: TileTreeParams,
    private _dataManager: OrbitGtDataManager,
    cloudRange: Range3d,
    private _centerOffset: Vector3d,
    private _ecefTransform: Transform
  ) {
    super(treeParams);
  ...
  public override dispose(): void {
    ...
    for (const graphic of this._tileGraphics.values()) graphic.dispose();
  ...
  public draw(args: TileDrawArgs) {
    ...
    IModelApp.tileAdmin.addExternalTilesForUser(args.context.viewport, {
      requested: frameData.tilesToLoad.size(),
      selected: tileCount,
      ready: tileCount,
    });

    if (debugBuilder) debugBuilder.setSymbology(ColorDef.red, ColorDef.red, 1);
    ...
    if (debugBuilder) args.graphics.add(debugBuilder.finish());
    ...
    if (frameData.hasMissingData()) {
      this._dataManager
        .loadData(frameData)
        .then(() => IModelApp.tileAdmin.onTileLoad.raiseEvent(this.rootTile))
...
export namespace OrbitGtTileTree {
  ...
  export async function createOrbitGtTileTree(
    rdSourceKey: RealityDataSourceKey,
    ...
    if (isContextShare) {
      ...
      if (rdSource === undefined || realityData === undefined) return undefined;
      const docRootName = realityData.rootDocument;
      if (!docRootName) return undefined;
      ...
      if (accountName.length > 0) blobStringUrl = UrlFS.getAzureBlobSasUrl(accountName, containerName, blobFileName, sasToken);
    }
    ...
    if (pointCloudCRS.length > 0) {
      ...
      const pointCloudToEcef = transformFromOrbitGt(
        CRSManager.createTransform(pointCloudCRS, new OrbitGtCoordinate(pointCloudCenter.x, pointCloudCenter.y, pointCloudCenter.z), wgs84CRS)
      );
      ...
      if (realityOriginToProjectDistance < maxProjectDistance) {
        ...
        if (cartographicOrigin !== undefined && geoConverter !== undefined) {
          ...
          if (response.iModelCoords[0].s === GeoCoordStatus.Success) {
            ...
            if (ecefToDbOrigin) ecefToDb = ecefToDbOrigin;
          }
        }
      }
...
class OrbitGtTreeReference extends RealityModelTileTree.Reference {
  ...
  public override async getToolTip(hit: HitDetail): Promise<HTMLElement | string | undefined> {
    ...
    if (this._name) strings.push(`${IModelApp.localization.getLocalizedString("iModelJs:TooltipInfo.Name")} ${this._name}`);
