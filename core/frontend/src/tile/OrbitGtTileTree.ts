...
function compareSourceKeys(lhs: RealityDataSourceKey, rhs: RealityDataSourceKey): number {
  return (
    ...
    compareStringsOrUndefined(lhs.iTwinId, rhs.iTwinId)
  );
}
...
class OrbitGtViewRequest extends OrbitGtIViewRequest {
  ...
  public shouldSplit(level: OrbitGtLevel, tile: OrbitGtTileIndex) {
    ...
    const worldCenter2: Point3d = this._tileDrawArgs.location.multiplyXYZ(
      tileCenter.x,
      tileCenter.y,
      tileCenter.z + level.getTileGrid().size.z
    );
...
class TileSortProjector implements OrbitGtIProjectToViewForSort {
  ...
  constructor(iModelTransform: Transform, viewingSpace: ViewingSpace, centerOffset: Vector3d) {
    ...
    if (undefined === viewingSpace.eyePoint) {
      ...
      const eyeDelta = Vector3d.createFrom({
        x: viewDelta.x / 2,
        y: viewDelta.y / 2,
        z: viewDelta.z * 10,
      });
...
export class OrbitGtTileTree extends TileTree {
  ...
  public draw(args: TileDrawArgs) {
    ...
    this._dataManager.filterLoadList(
      ...
      blocksInView,
      tilesInView,
      frameData.levelsToLoad,
      frameData.blocksToLoad,
      frameData.tilesToLoad
    );
    tilesInView.sort(
    ...
    if (doLogging) {
      ...
      console.log(
        `Total OrbitGtTiles: ${tileCount} MinLevel: ${minLevel} MaxLevel: ${maxLevel} Total Points: ${totalPointCount}`
      );
    }
...
export namespace OrbitGtTileTree {
  ...
  export async function createOrbitGtTileTree(
    ...
    const cachedBlobFile = new PageCachedFile(
      urlFS,
      blobStringUrl,
      blobFileSize,
