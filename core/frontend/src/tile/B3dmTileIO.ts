...
import { Transform, Point3d, Vector3d } from "@bentley/geometry-core";
...
export namespace B3dmTileIO {
  ...
  export class Reader extends GltfTileIO.Reader {
    public static create(stream: TileIO.StreamBuffer, iModel: IModelConnection, modelId: Id64String, is3d: boolean, range: ElementAlignedBox3d, system: RenderSystem, yAxisUp: boolean, isLeaf: boolean, tileCenter: Point3d, transformToRoot?: Transform, isCanceled?: GltfTileIO.IsCanceled, idMap?: BatchedTileIdMap): Reader | undefined {
      ...
      if (header.featureTableJson && Array.isArray(header.featureTableJson.RTC_CENTER)) {
         ...
         * capacity of 32 bit integers. It is essentially an ad hoc RTC applied at read time.
         ...
         */
        const minForceCenterMagnitude = 1.0E5;
        if (tileCenter.magnitude() > minForceCenterMagnitude) {
          returnToCenterTransform = Transform.createTranslation(tileCenter);
