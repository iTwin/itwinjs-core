...
export class TileUserIdSets extends SortedArray<TileUserIdSet> {
  ...
  /** Remove the specified user Id from all sets and remove empty and duplicate sets. */
  public drop(userId: number): void {
...
export interface LRUTileListNode {
  ...
  bytesUsed: number;
  /** For a tile, the Ids of all of the TileUsers for which the tile is currently selected for display. The TileUserIdSet is owned by the LRUTileList's TileUserIdSets member.
   * Undefined if the tile is not in use by any TileUser.
   */
  userIds?: TileUserIdSet | undefined;
}
...
/** An intrusive doubly-linked list of LRUTileListNodes, containing Tiles partitioned by a singleton sentinel node into two partitions and ordered from least-recently- to most-recently-selected for display in any TileUser.
 ...
 * - Just before a Viewport selects tiles for display, it is removed from each tile's set of viewports in which they are selected. If a tile's set of TileUsers becomes empty as a result, it is moved to the end of the "not selected" partition.
 * - When a tile becomes selected for display in a Viewport, the viewport's TileUser is added to its TileUser set and the tile is moved to the end of the "selected" partition.
 *
...
export class LRUTileList {
  ...
  public drop(tile: Tile): void {
    ...
    this.unlink(tile);
    tile.userIds = undefined;
    tile.bytesUsed = 0;

    this.assertList();
  }

  /** Mark the tiles as selected for display in the specified TileUser. They are moved to the end of the "selected" partition. */
  public markSelectedForUser(userId: number, tiles: Iterable<Tile>): void {
    for (const tile of tiles) {
      ...
      if (isLinked(tile)) {
        tile.userIds = this._userIdSets.plus(userId, tile.userIds);
        this.moveToEnd(tile);
      }
    }
  }

  /** Mark the tiles as no longer selected for display in the specified TileUser.
   * If this results in a tile being no longer in use by any TileUser, it is moved to the end of the "not selected" partition.
   */
  public clearSelectedForUser(userId: number): void {
    ...
    while (prev && prev.next) {
      ...
      tile.userIds = this._userIdSets.minus(userId, tile.userIds);
      if (undefined === tile.userIds)
        this.moveBeforeSentinel(tile);
      else
        prev = tile;
    }
