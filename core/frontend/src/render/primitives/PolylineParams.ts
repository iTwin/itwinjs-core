...
class PolylineVertex {
  ...
  public computeParam(
    negatePerp: boolean,
    adjacentToJoint: boolean = false,
    joint: boolean = false,
    noDisplacement: boolean = false
  ): number {
  ...
  public static fromMesh(args: MeshArgs): PolylineTesselator | undefined {
    if (undefined !== args.edges?.polylines.lines && undefined !== args.points)
      return new PolylineTesselator(
        args.edges.polylines.lines,
        args.points,
        wantJointTriangles(args.edges.width, true === args.is2d)
      );
...
export function tesselatePolyline(
