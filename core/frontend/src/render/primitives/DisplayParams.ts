...
function compareMaterials(lhs?: RenderMaterial, rhs?: RenderMaterial): number {
  return comparePossiblyUndefined(
    (lhMat: RenderMaterial, rhMat: RenderMaterial) =>
      lhMat === rhMat ? 0 : compareStringsOrUndefined(lhMat.key, rhMat.key),
    lhs,
    rhs
  );
}
...
export class DisplayParams {
  ...
  public get regionEdgeType(): DisplayParams.RegionEdgeType {
    ...
    return !this.fillColor.equals(this.lineColor)
      ? DisplayParams.RegionEdgeType.Outline
