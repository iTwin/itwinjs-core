...
class UpsampleIndexMap extends Map<number, number> {
  ...
  public addTriangle(indices: number[]) {
    for (const index of indices) {
      ...
      if (undefined === mapIndex) this.set(index, (mapIndex = this._next++));
...
export function upsampleRealityMeshParams(
  ...
  const addedPoints = new Array<QPoint3d>(),
  ...
  for (const entry of indexMap.entries()) {
    ...
    if (parentIndex < parentPointCount) {
      ...
      pos.setFromScalars(
        parentPoints.points[pointIndex],
        parentPoints.points[pointIndex + 1],
      ...
      if (addedNormals.length > 0) normal = addedNormals[addedIndex];
    }
...
function interpolateOctEncodedNormal(normal0: number, normal1: number, fraction: number): number {
  ...
  if (undefined !== n0 && undefined !== n1) {
    const n = Vector3d.create(
      interpolate(n0.x, n1.x, fraction),
      interpolate(n0.y, n1.y, fraction),
      interpolate(n0.z, n1.z, fraction)
    );
    n.normalizeInPlace();
...
function addClipped(
  ...
  const scratchQPoint3d = new QPoint3d(),
    scratchQPoint3d1 = new QPoint3d();
  const scratchQPoint2d = new QPoint2d(),
    scratchQPoint2d1 = new QPoint2d();
  ...
  for (let i = 0; i < 3; i++) {
    const index = triangleIndices[i];
    const next = (i + 1) % 3;
    if (inside[i]) clipOutput.push(index);
  ...
  if (clipOutput.length > 2) {
    ...
    if (clipOutput.length > 3)
      addClipped(
        params,
        [clipOutput[0], clipOutput[2], clipOutput[3]],
        indexMap,
        clipAxes,
        clipIndex,
        addedPoints,
