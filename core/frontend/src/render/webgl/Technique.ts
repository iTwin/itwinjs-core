...
import {
  createEDLCalcBasicProgram,
  createEDLCalcFullProgram,
  createEDLFilterProgram,
  createEDLMixProgram,
} from "./glsl/EDL";
...
import {
  createVolClassBlendProgram,
  createVolClassColorUsingStencilProgram,
  createVolClassCopyZProgram,
...
import {
  ...
  addFeatureSymbology,
  addRenderOrder,
  addUniformFeatureSymbology,
  FeatureSymbologyOptions,
  mixFeatureColor,
} from "./glsl/FeatureSymbology";
import {
...
export class SingularTechnique implements Technique {
  ...
  public getShader(_flags: TechniqueFlags) {
    return this.program;
  }
  public getShaderByIndex(_index: number) {
    return this.program;
  }
  ...
  public compileShaders(): boolean {
    return this.program.compile() === CompileStatus.Success;
  }

  public get isDisposed(): boolean {
    return this.program.isDisposed;
  }
...
export abstract class VariedTechnique implements Technique {
  ...
  public compileShaders(): boolean {
    ...
    for (const program of this._basicPrograms) {
      if (program.compile() !== CompileStatus.Success) allCompiled = false;
    }

    for (const clipper of this._clippingPrograms)
      if (!clipper.compile()) allCompiled = false;
  ...
  protected finishConstruction(): void {
    ...
    assert(
      -1 ===
        (emptyShaderIndex = this._basicPrograms.findIndex(
          (prog) => undefined === prog
        )),
      `Shader index ${emptyShaderIndex} is undefined in ${this.constructor.name}`
    );
  }
  ...
  protected addShader(
    ...
    if (System.instance.supportsLogZBuffer) {
      ...
      if (System.instance.fragDepthDoesNotDisableEarlyZ)
        builder.frag.requiresEarlyZWorkaround =
          -1 !== this._earlyZFlags.findIndex((x) => x.equals(flags));
    }
  ...
  protected addHiliteShader(
    gl: WebGL2RenderingContext,
    instanced: IsInstanced,
    classified: IsClassified,
    posType: PositionType,
    create: CreateHiliter
  ): void {
  ...
  protected addTranslucentShader(
    builder: ProgramBuilder,
    flags: TechniqueFlags,
    gl: WebGL2RenderingContext
  ): void {
  ...
  private getShaderIndex(flags: TechniqueFlags) {
    assert(
      !flags.isHilite ||
        (!flags.isTranslucent &&
          (flags.isClassified === IsClassified.Yes || flags.hasFeatures)),
  ...
  public getShader(flags: TechniqueFlags): ShaderProgram {
    ...
    if (program === undefined) program = this._basicPrograms[index];
  ...
  public forEachProgram(func: (program: ShaderProgram) => void): void {
    for (const basic of this._basicPrograms) func(basic);
...
class SurfaceTechnique extends VariedTechnique {
  ...
  private static readonly _kHilite =
    SurfaceTechnique._kEdgeTestNeeded + SurfaceTechnique._kFeature * 2;
  ...
  private static readonly _kClassified =
    SurfaceTechnique._kHilite + numHiliteVariants;
  ...
  public constructor(gl: WebGL2RenderingContext) {
    ...
    this._earlyZFlags = [
      ...
      TechniqueFlags.fromDescription(
        "Unquantized-Opaque-Instanced-Hilite-Overrides"
      ),
    ...
    for (const posType of positionTypes) {
      for (
        let instanced = IsInstanced.No;
        instanced <= IsInstanced.Yes;
        instanced++
      ) {
        this.addHiliteShader(
          gl,
          instanced,
          IsClassified.No,
          posType,
          createSurfaceHiliter
        );
        for (
          let iAnimate = IsAnimated.No;
          iAnimate <= IsAnimated.Yes;
          iAnimate++
        ) {
          for (
            let shadowable = IsShadowable.No;
            shadowable <= IsShadowable.Yes;
            shadowable++
          ) {
            for (
              let wiremesh = IsWiremesh.No;
              wiremesh <= IsWiremesh.Yes;
              wiremesh++
            ) {
              for (
                let thematic = IsThematic.No;
                thematic <= IsThematic.Yes;
                thematic++
              ) {
                for (
                  let edgeTestNeeded = IsEdgeTestNeeded.No;
                  edgeTestNeeded <= IsEdgeTestNeeded.Yes;
                  edgeTestNeeded++
                ) {
                  for (const featureMode of featureModes) {
                    for (
                      ...
                      if (
                        FeatureMode.None !== featureMode ||
                        IsEdgeTestNeeded.No === edgeTestNeeded
                      ) {
                        if (
                          IsThematic.Yes === thematic &&
                          IsShadowable.Yes === shadowable
                        )
        ...
        for (
          ...
          for (
            ...
            for (const featureMode of featureModes) {
              ...
              const builder = createSurfaceBuilder(flags);
              if (flags.isTranslucent) addTranslucency(builder);
  ...
  protected get _debugDescription() {
    return "Surface";
  }
  ...
  public computeShaderIndex(flags: TechniqueFlags): number {
    ...
    const idxOffset =
      flags.positionType === "unquantized" ? SurfaceTechnique._kUnquantized : 0;
    if (flags.isClassified) {
      ...
      if (flags.isHilite) return SurfaceTechnique._kClassified + idxOffset;
      ...
      if (flags.isTranslucent) baseIndex += 1;
      if (flags.isShadowable) baseIndex += 2;
      if (flags.isThematic) baseIndex += 4;
    ...
    let index = flags.isTranslucent
      ? SurfaceTechnique._kTranslucent
      : SurfaceTechnique._kOpaque;
    ...
    if (flags.isEdgeTestNeeded)
      index +=
        SurfaceTechnique._kEdgeTestNeeded +
        (flags.featureMode - 1) * SurfaceTechnique._kFeature;
    else index += SurfaceTechnique._kFeature * flags.featureMode;
...
class PolylineTechnique extends VariedTechnique {
  ...
  private static readonly _kHilite = numFeatureVariants(
    PolylineTechnique._kFeature
  );
  private static readonly _kUnquantized =
    PolylineTechnique._kHilite + numHiliteVariants;
    ...
    for (const posType of positionTypes) {
      for (
        ...
        this.addHiliteShader(
          ...
          IsClassified.No,
          posType,
          (inst, _class, pos) => createPolylineHiliter(inst, pos)
        );
        for (const featureMode of featureModes) {
          flags.reset(
            featureMode,
          ...
          if (FeatureMode.Overrides === featureMode) {
            addFeatureSymbology(
              builderTrans,
              featureMode,
              FeatureSymbologyOptions.Linear
            );
            addFeatureSymbology(
              builder,
              featureMode,
  ...
  public computeShaderIndex(flags: TechniqueFlags): number {
    const idxOffset =
      flags.positionType === "unquantized"
        ? PolylineTechnique._kUnquantized
        : 0;
    if (flags.isHilite) {
      assert(flags.hasFeatures);
    ...
    let index = flags.isTranslucent
      ? PolylineTechnique._kTranslucent
      : PolylineTechnique._kOpaque;
...
class EdgeTechnique extends VariedTechnique {
  ...
  private static readonly _kUnquantized = numFeatureVariants(
    EdgeTechnique._kFeature
  );
      ...
      for (
        ...
        for (
          ...
          for (const featureMode of featureModes) {
            ...
            // The translucent shaders do not need the element IDs.
            const builderTrans = createEdgeBuilder(
              type,
              flags.isInstanced,
              flags.isAnimated,
              posType
  ...
  public computeShaderIndex(flags: TechniqueFlags): number {
    let index = flags.isTranslucent
      ? EdgeTechnique._kTranslucent
      : EdgeTechnique._kOpaque;
    index += EdgeTechnique._kFeature * flags.featureMode;
    if (flags.isAnimated) index += EdgeTechnique._kAnimated;

    if (flags.isInstanced) index += EdgeTechnique._kInstanced;
...
class PointStringTechnique extends VariedTechnique {
  ...
  private static readonly _kFeature = 4;
  private static readonly _kHilite = numFeatureVariants(
    PointStringTechnique._kFeature
  );
  private static readonly _kUnquantized =
    PointStringTechnique._kHilite + numHiliteVariants;
    ...
    for (const posType of positionTypes) {
      for (
        ...
        this.addHiliteShader(
          ...
          IsClassified.No,
          posType,
          (inst, _class, pos) => createPointStringHiliter(inst, pos)
        );
        for (const featureMode of featureModes) {
          flags.reset(
            featureMode,
  ...
  public computeShaderIndex(flags: TechniqueFlags): number {
    const idxOffset =
      "quantized" === flags.positionType
        ? 0
        : PointStringTechnique._kUnquantized;
    if (flags.isHilite) {
      assert(flags.hasFeatures);
    ...
    let index = flags.isTranslucent
      ? PointStringTechnique._kTranslucent
      : PointStringTechnique._kOpaque;
...
class PointCloudTechnique extends VariedTechnique {
  ...
  public constructor(gl: WebGL2RenderingContext) {
    ...
    for (
      let iClassified = IsClassified.No;
      iClassified <= IsClassified.Yes;
      iClassified++
    ) {
      this.addHiliteShader(
        gl,
        ...
        iClassified,
        "quantized",
        (_inst, classified) => createPointCloudHiliter(classified)
      );
      const flags = scratchTechniqueFlags;
      for (
        let thematic = IsThematic.No;
...
class RealityMeshTechnique extends VariedTechnique {
        ...
        for (
          ...
          for (
            ...
            for (
              ...
              for (
                let enableAtmosphere = EnableAtmosphere.No;
                enableAtmosphere <= EnableAtmosphere.Yes;
                enableAtmosphere++
              ) {
                const flags = scratchTechniqueFlags;
                for (const featureMode of featureModes) {
                  ...
                  if (flags.isTranslucent) {
                    ...
                    addTranslucency(builder);
                  } else this.addFeatureId(builder, featureMode);
  ...
  public computeShaderIndex(flags: TechniqueFlags): number {
    assert(
      "quantized" === flags.positionType,
      "Unquantized reality mesh positions not currently supported."
    );

    if (flags.isHilite) return flags.isClassified ? 1 : 0;
    let ndx = 2;
    if (flags.isClassified) ndx++;
    if (flags.isShadowable) ndx += 2;
    if (flags.isTranslucent) ndx += 4;
    ndx += 8 * flags.featureMode;
    if (flags.isThematic) ndx += 24;
    if (flags.isWiremesh) ndx += 48;
    if (flags.enableAtmosphere) ndx += 96;
    return ndx;
  }
}
...
abstract class MultipleTechnique implements Technique {
  ...
  public getShader(flags: TechniqueFlags): ShaderProgram {
    ...
    if (program === undefined) program = this._programs[index];
  ...
  protected finishConstruction(): void {
    ...
    assert(
      -1 ===
        (emptyShaderIndex = this._programs.findIndex(
          (prog) => undefined === prog
        )),
      `Shader index ${emptyShaderIndex} is undefined in ${this.constructor.name}`
    );
...
class SkySphereTechnique extends MultipleTechnique {
  ...
  protected get _debugDescription() {
    return `SkySphere-${this._isGradient ? "Gradient" : "Texture"}`;
  }

  public computeShaderIndex(flags: TechniqueFlags): number {
    let index = 0;
    if (flags.enableAtmosphere) index += 1 << 0;
...
const techniquesByPriority: PrioritizedTechniqueOrShader[] = [
  // Compile these specific shader variations first because they seem most likely to be used immediately upon opening a file.
  {
    ...
    specificShader: {
      featureMode: FeatureMode.None,
      isInstanced: IsInstanced.No,
      isShadowable: IsShadowable.No,
      isEdgeTestedNeeded: IsEdgeTestNeeded.No,
      isTranslucent: false,
    },
  },
  {
  ...
  },
  {
    ...
    specificShader: {
      ...
      isInstanced: IsInstanced.No,
      isShadowable: IsShadowable.No,
      isEdgeTestedNeeded: IsEdgeTestNeeded.Yes,
      isTranslucent: false,
    },
  },
  {
    techniqueId: TechniqueId.Surface,
    specificShader: {
      featureMode: FeatureMode.Overrides,
      isInstanced: IsInstanced.No,
      isShadowable: IsShadowable.No,
      isEdgeTestedNeeded: IsEdgeTestNeeded.No,
      isTranslucent: false,
...
export class Techniques implements WebGLDisposable {
  ...
  public execute(
    ...
    using(
      new ShaderProgramExecutor(target, renderPass),
      (executor: ShaderProgramExecutor) => {
        for (const command of commands) command.execute(executor);
      }
    );
  ...
  public executeForIndexedClassifier(
    target: Target,
    cmdsByIndex: DrawCommands,
    renderPass: RenderPass
  ) {
  ...
  public draw(params: DrawParams): void {
    ...
    using(
      new ShaderProgramExecutor(params.target, params.renderPass, program),
      (executor: ShaderProgramExecutor) => {
  ...
  public dispose(): void {
    for (const tech of this._list) dispose(tech);
    this._list.length = 0;
  }
  ...
  public compileShaders(): boolean {
    ...
    for (const tech of this._list) {
      if (!tech.compileShaders()) allCompiled = false;
    }
  ...
  public idleCompileNextShader(): boolean {
    ...
    do {
      ...
      if (pTech.specificShader !== undefined) {
        // if this entry consists of a specific shader, just compile that
        const flags = scratchTechniqueFlags;
        flags.reset(
          pTech.specificShader.featureMode,
          pTech.specificShader.isInstanced,
          pTech.specificShader.isShadowable,
          IsThematic.No,
          "quantized"
        );
        ...
        // if this entry only contains a techniqueId, then compile all uncompiled shaders for that technique
        shader = tech.getShaderByIndex(this._shaderIndex);
        this._shaderIndex++;
        numShaders = tech.getShaderCount();
      }

      if (shader.isCompiled) wasPreviouslyCompiled = true;
      else {
  ...
  public forEachVariedProgram(func: (program: ShaderProgram) => void): void {
    for (const technique of this._list)
      if (technique instanceof VariedTechnique) technique.forEachProgram(func);
  }

  private constructor() {}

  private initializeBuiltIns(gl: WebGL2RenderingContext): void {
    this._list[TechniqueId.OITClearTranslucent] = new SingularTechnique(
      createClearTranslucentProgram(gl)
    );
    this._list[TechniqueId.ClearPickAndColor] = new SingularTechnique(
      createClearPickAndColorProgram(gl)
    );
    this._list[TechniqueId.CopyColor] = new SingularTechnique(
      createCopyColorProgram(gl)
    );
    this._list[TechniqueId.CopyColorNoAlpha] = new SingularTechnique(
      createCopyColorProgram(gl, false)
    );
    this._list[TechniqueId.CopyPickBuffers] = new SingularTechnique(
      createCopyPickBuffersProgram(gl)
    );
    this._list[TechniqueId.EVSMFromDepth] = new SingularTechnique(
      createEVSMProgram(gl)
    );
    this._list[TechniqueId.SkyBox] = new SingularTechnique(
      createSkyBoxProgram(gl)
    );
    this._list[TechniqueId.SkySphereGradient] = new SkySphereTechnique(
      gl,
      true
    );
    this._list[TechniqueId.SkySphereTexture] = new SkySphereTechnique(
      gl,
      false
    );
    this._list[TechniqueId.AmbientOcclusion] = new SingularTechnique(
      createAmbientOcclusionProgram(gl)
    );
    this._list[TechniqueId.Blur] = new SingularTechnique(
      createBlurProgram(gl, BlurType.NoTest)
    );
    this._list[TechniqueId.BlurTestOrder] = new SingularTechnique(
      createBlurProgram(gl, BlurType.TestOrder)
    );
    this._list[TechniqueId.CombineTextures] = new SingularTechnique(
      createCombineTexturesProgram(gl)
    );
    this._list[TechniqueId.Combine3Textures] = new SingularTechnique(
      createCombine3TexturesProgram(gl)
    );
    this._list[TechniqueId.Surface] = new SurfaceTechnique(gl);
    this._list[TechniqueId.Edge] = new EdgeTechnique(gl, "SegmentEdge");
    this._list[TechniqueId.SilhouetteEdge] = new EdgeTechnique(
      gl,
      "Silhouette"
    );
    ...
    this._list[TechniqueId.PlanarGrid] = new SingularTechnique(
      createPlanarGridProgram(gl)
    );
    this._list[TechniqueId.EDLCalcBasic] = new SingularTechnique(
      createEDLCalcBasicProgram(gl)
    );
    this._list[TechniqueId.EDLCalcFull] = new SingularTechnique(
      createEDLCalcFullProgram(gl)
    );
    this._list[TechniqueId.EDLFilter] = new SingularTechnique(
      createEDLFilterProgram(gl)
    );
    this._list[TechniqueId.EDLMix] = new SingularTechnique(
      createEDLMixProgram(gl)
    );

    this._list[TechniqueId.IndexedEdge] = new EdgeTechnique(gl, "IndexedEdge");

    this._list[TechniqueId.VolClassCopyZ] = new SingularTechnique(
      createVolClassCopyZProgram(gl)
    );
    this._list[TechniqueId.VolClassSetBlend] = new SingularTechnique(
      createVolClassSetBlendProgram(gl)
    );
    this._list[TechniqueId.VolClassBlend] = new SingularTechnique(
      createVolClassBlendProgram(gl)
    );
    this._list[TechniqueId.VolClassColorUsingStencil] = new SingularTechnique(
      createVolClassColorUsingStencilProgram(gl)
    );

    for (let compositeFlags = 1; compositeFlags <= 7; compositeFlags++) {
      const techId = computeCompositeTechniqueId(compositeFlags);
      this._list[techId] = new SingularTechnique(
        createCompositeProgram(compositeFlags, gl)
      );
    }

    assert(
      this._list.length === TechniqueId.NumBuiltIn,
      "unexpected number of built-in techniques"
    );
  }
}
