...
import { GraphicsCollectorDrawArgs, MapTileTreeReference, MapTileTreeReference2, TileTreeReference } from "../../tile/internal";
...
export class BackgroundMapDrape2 extends TextureDrape {
  private _fbo?: FrameBuffer;
  private readonly _graphics: RenderGraphic[] = [];
  private _frustum?: Frustum;
  private _width = 0;
  private _height = 0;
  private _mapTree: MapTileTreeReference2;
  private _drapedTree: TileTreeReference;
  private static _postProjectionMatrix = Matrix4d.createRowValues(
    0, 1, 0, 0,
    0, 0, -1, 0,
  ...
  private constructor(drapedTree: TileTreeReference, mapTree: MapTileTreeReference2) {
    super();
    this._drapedTree = drapedTree;
    this._mapTree = mapTree;
  }
  ...
  public static create(draped: TileTreeReference, map: MapTileTreeReference2) {
    return new BackgroundMapDrape2(draped, map);
  }
  ...
  public draw(target: Target) {
    ...
    const prevProjMatrix = target.uniforms.frustum.projectionMatrix;
    target.uniforms.frustum.changeProjectionMatrix(BackgroundMapDrape2._postProjectionMatrix.multiplyMatrixMatrix(prevProjMatrix));
