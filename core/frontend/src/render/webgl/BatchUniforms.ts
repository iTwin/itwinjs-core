...
export class BatchUniforms {
  ...
  public setCurrentBatch(batch: Batch | undefined): void {
    ...
    if (undefined !== batch) this.state.push(batch, false);
    else this.state.pop();
  ...
  public bindNumThematicSensors(uniform: UniformHandle): void {
    if (undefined !== this._sensors) this._sensors.bindNumSensors(uniform);
  }

  public bindThematicSensors(uniform: UniformHandle): void {
    if (undefined !== this._sensors) this._sensors.bindTexture(uniform);
  }
  ...
  public bindLUT(uniform: UniformHandle): void {
    ...
    if (undefined !== this._overrides) this._overrides.bindLUT(uniform);
  }
  ...
  public bindBatchId(uniform: UniformHandle): void {
    if (!sync(this, uniform)) uniform.setUniform4fv(this._batchId);
  }
