...
export class FrameBuffer implements WebGLDisposable {
  ...
  public get isBound(): boolean {
    return (
      FrameBufferBindState.Bound <= this._bindState &&
      FrameBufferBindState.BoundWithAttachmentsMultisampled >= this._bindState
    );
  }

  public get isBoundMultisampled(): boolean {
    return (
      FrameBufferBindState.BoundMultisampled === this._bindState ||
      FrameBufferBindState.BoundWithAttachmentsMultisampled === this._bindState
    );
  }
  ...
  public getColorTargets(
    useMSBuffers: boolean,
    ndx: number
  ): { tex: TextureHandle; msBuf: RenderBufferMultiSample | undefined } {
    let msBuf;
    if (useMSBuffers) {
  ...
  private constructor(
    ...
    if (
      undefined !== colorMsBuffers &&
      colorMsBuffers.length === colorTextures.length &&
      undefined !== msFilters &&
  ...
  public bind(
    ...
    if (undefined === this._fbo) return false;
  ...
  public blitMsBuffersToTextures(blitDepth: boolean, ndx?: number) {
    ...
    for (let i = 0; i < max; ++i) {
      ...
      if (this._colorMsBuffers[i].isDirty) {
        ...
        if (undefined !== ndx && i === ndx) break;
      }
    }
    if (
      blitDepth &&
      undefined !== this.depthBuffer &&
      undefined !== this.depthBufferMs &&
      (this.depthBufferMs as RenderBufferMultiSample).isDirty
    ) {
  ...
  public invalidate(
    ...
    const attachments: number[] = invDepth
      ? invStencil
        ? [gl.DEPTH_STENCIL_ATTACHMENT]
        : [System.instance.context.DEPTH_ATTACHMENT]
      : invDepth
      ? [gl.STENCIL_ATTACHMENT]
      : [];
    if (undefined !== indices) {
      if (indices.length > 0) {
        for (const i of indices) attachments.push(gl.COLOR_ATTACHMENT0 + i);
      }
    } else {
...
export class FrameBufferStack {
  ...
  public markTargetsDirty(): void {
    const top = this._top;
    if (undefined !== top) top.fbo.markTargetsDirty();
  }
