...
export class AtmosphereUniforms implements WebGLDisposable, SyncTarget {
  ...
  private readonly _earthScaleMatrix = new Matrix3d(
    new Float64Array([1, 0, 0, 0, 1, 0, 0, 0, 1])
  );
  private readonly _inverseEllipsoidRotationMatrix = new Matrix3d(
    new Float64Array([1, 0, 0, 0, 1, 0, 0, 0, 1])
  );
  private readonly _atmosphereScaleMatrix = new Matrix3d(
    new Float64Array([1, 0, 0, 0, 1, 0, 0, 0, 1])
  );
  ...
  public update(target: Target): void {
    const atmosphereHasNotChanged =
      this._atmosphere &&
      target.plan.atmosphere &&
      this._atmosphere.equals(target.plan.atmosphere);
    const ellipsoidHasNotChanged =
      this._ellipsoid &&
      target.plan.ellipsoid &&
      this._ellipsoid.equals(target.plan.ellipsoid);
    ...
    this._updateAtmosphereScaleMatrix(
      this._atmosphere.atmosphereHeightAboveEarth
    );
    this._updateExposure(this._atmosphere.exposure);
    this._updateDensityFalloff(this._atmosphere.densityFalloff);
    this._updateEarthCenter(
      this._ellipsoid.ellipsoidCenter,
      target.uniforms.frustum.viewMatrix
    );
    this._updateEarthScaleMatrix(this._ellipsoid.ellipsoidRadii);
    this._updateInverseEllipsoidRotationMatrix(
      this._ellipsoid.ellipsoidRotation,
      target.uniforms.frustum.viewMatrix.matrix
    );
    this._updateAtmosphereRadiusScaleFactor(
      this._atmosphere.atmosphereHeightAboveEarth
    );
    this._updateAtmosphereMaxDensityThresholdScaleFactor(
      this._atmosphere.depthBelowEarthForMaxDensity
    );
    ...
    this._updateScatteringCoefficients(
      this._atmosphere.scatteringStrength,
      this._atmosphere.wavelengths
    );
  }
  ...
  private _updateInverseEllipsoidRotationMatrix(
    ...
    ellipsoidRotation.multiplyMatrixInverseMatrix(
      this._scratchMatrix3d,
      this._inverseEllipsoidRotationMatrix
    );
  }
  ...
  private _updateAtmosphereScaleMatrix(heightAboveSurface: number) {
    ...
    const scaleFactor =
      earthPolarRadius === 0
        ? 1.0
        : (earthPolarRadius + heightAboveSurface) / earthPolarRadius;
  ...
  private _updateAtmosphereRadiusScaleFactor(
    ...
    const earthPolarRadius = this._earthScaleMatrix.at(2, 2);
    const minDensityThresholdRadius =
      earthPolarRadius + atmosphereHeightAboveEarth;
    const atmosphereRadiusScaleFactor =
      earthPolarRadius === 0 ? 1 : minDensityThresholdRadius / earthPolarRadius;
    this.atmosphereData.data[0] = atmosphereRadiusScaleFactor;
  }

  private _updateAtmosphereMaxDensityThresholdScaleFactor(
    maxDensityDepthBelowEarth: number
  ) {
    const earthPolarRadius = this._earthScaleMatrix.at(2, 2);
    const maxDensityThresholdRadius =
      earthPolarRadius - maxDensityDepthBelowEarth;
    const atmosphereMaxDensityThresholdScaleFactor =
      earthPolarRadius === 0 ? 1 : maxDensityThresholdRadius / earthPolarRadius;
  ...
  private _updateScatteringCoefficients(
    ...
    this.atmosphereData.data[12] =
      (violetLightWavelength / wavelengths.r) ** 4.0 * scatteringStrength;
    this.atmosphereData.data[13] =
      (violetLightWavelength / wavelengths.g) ** 4.0 * scatteringStrength;
    this.atmosphereData.data[14] =
      (violetLightWavelength / wavelengths.b) ** 4.0 * scatteringStrength;
  }
  ...
  private _updateNumViewRaySamples(_numViewRaySamples: number) {
    const numViewRaySamples = Math.max(
      0,
      Math.min(MAX_SAMPLE_POINTS, _numViewRaySamples)
    );
    this.atmosphereData.data[4] = numViewRaySamples;
  }

  private _updateNumSunRaySamples(_numSunRaySamples: number) {
    const numSunRaySamples = Math.max(
      0,
      Math.min(MAX_SAMPLE_POINTS, _numSunRaySamples)
    );
    this.atmosphereData.data[5] = numSunRaySamples;
  }

  public bindInverseRotationInverseEarthScaleMatrix(
    uniform: UniformHandle
  ): void {
    if (!sync(this, uniform)) {
      this._earthScaleMatrix.multiplyMatrixInverseMatrix(
        this._inverseEllipsoidRotationMatrix,
        this._scratchMatrix3d
      );
  ...
  public bindInverseRotationInverseAtmosphereScaleMatrix(
    uniform: UniformHandle
  ): void {
    if (!sync(this, uniform)) {
      this._atmosphereScaleMatrix.multiplyMatrixInverseMatrix(
        this._inverseEllipsoidRotationMatrix,
        this._scratchMatrix3d
      );
      uniform.setMatrix3(Matrix3.fromMatrix3d(this._scratchMatrix3d));
  ...
  public bindInverseEarthScaleMatrix(uniform: UniformHandle): void {
    if (!sync(this, uniform))
      uniform.setMatrix3(
        Matrix3.fromMatrix3d(this._earthScaleMatrix.inverse()!)
      );
  }
  ...
  public bindInverseAtmosphereScaleMatrix(uniform: UniformHandle): void {
    if (!sync(this, uniform))
      uniform.setMatrix3(
        Matrix3.fromMatrix3d(this._atmosphereScaleMatrix.inverse()!)
      );
  }
