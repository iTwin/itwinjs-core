...
export class AtmosphereUniforms implements WebGLDisposable, SyncTarget {
  ...
  public update(target: Target): void {
    const atmosphereHasNotChanged =
      this._atmosphere && target.plan.atmosphere && this._atmosphere.equals(target.plan.atmosphere);
    const ellipsoidHasNotChanged =
      this._ellipsoid && target.plan.ellipsoid && this._ellipsoid.equals(target.plan.ellipsoid);
  ...
  private _updateAtmosphereRadiusScaleFactor(atmosphereHeightAboveEarth: number) {
    ...
    const atmosphereRadiusScaleFactor = earthPolarRadius === 0 ? 1 : minDensityThresholdRadius / earthPolarRadius;
  ...
  public bindEarthScaleMatrix(uniform: UniformHandle): void {
    if (!sync(this, uniform)) uniform.setMatrix3(Matrix3.fromMatrix3d(this._earthScaleMatrix));
  }

  public bindAtmosphereScaleMatrix(uniform: UniformHandle): void {
    if (!sync(this, uniform)) uniform.setMatrix3(Matrix3.fromMatrix3d(this._atmosphereScaleMatrix));
  }

  public bindInverseEarthScaleMatrix(uniform: UniformHandle): void {
    if (!sync(this, uniform)) uniform.setMatrix3(Matrix3.fromMatrix3d(this._earthScaleMatrix.inverse()!));
  }

  public bindInverseAtmosphereScaleMatrix(uniform: UniformHandle): void {
    if (!sync(this, uniform)) uniform.setMatrix3(Matrix3.fromMatrix3d(this._atmosphereScaleMatrix.inverse()!));
  }
