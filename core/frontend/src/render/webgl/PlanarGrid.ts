...
class PlanarGridGeometryParams extends IndexedGeometryParams {
  ...
  public constructor(
    ...
    const attrParams = AttributeMap.findAttribute(
      "a_uvParam",
      TechniqueId.PlanarGrid,
      false
    );
  ...
  public static create(
    ...
    if (qPoints.params.origin.magnitude() > 1.0e4) {
      transform = Transform.createTranslationXYZ(
        qPoints.params.origin.x,
        qPoints.params.origin.y,
        qPoints.params.origin.z
      );
    ...
    const pointBuffer = QBufferHandle3d.create(
      qPoints.params,
      qPoints.toTypedArray()
    );
    const paramBuffer = QBufferHandle2d.create(
      qParams.params,
      qParams.toTypedArray()
    );
    const indBuffer = BufferHandle.createBuffer(
      GL.Buffer.Target.ElementArrayBuffer,
      indices
    );
    if (!pointBuffer || !paramBuffer || !indBuffer) return undefined;

    const geomParams = new PlanarGridGeometryParams(
      pointBuffer,
      paramBuffer,
      indBuffer,
      indices.length,
