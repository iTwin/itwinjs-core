...
class RealityTextureParam {
  constructor(public texture: RenderTexture | undefined, public matrix: Matrix4, private _classifier: PlanarClassifier | undefined) { }
  public get isProjected() { return undefined  !== this._classifier; }

  public getParams(result: Matrix4): Matrix4 {
    if (this._classifier === undefined) {
      result.data[0] = 0;
    } else {
      result.data[0] = 1;    // Denotes a classifier texture.  UV mapping generated by multiplying XYZ by matrix (projection)
      result.data[1] = this._classifier.textureImageCount;
      scratchBatchBaseId[0] = this._classifier.baseBatchId;
  ...
  public static create(textures: TerrainTextureOrClassifier[]) {
    ...
    for (const texture of textures) {
      if (texture instanceof TerrainTexture) {
        ...
        textureParams.push(new RealityTextureParam(classifier.texture, Matrix4.fromMatrix4d(classifier.projectionMatrix), classifier));
      }
    }
