...
class RealityTextureParam {
  constructor(public texture: RenderTexture | undefined, public params: Matrix4, public matrix: Matrix4, public isProjected: boolean) {}
}
/** @internal */
export class RealityTextureParams {
  ...
  public static create(textures: TerrainTextureOrClassifier[]) {
    ...
    for (const texture of textures) {
      const params = new Matrix4();

      if (texture instanceof TerrainTexture) {
        params.data[0] = 0;    // Denotes a terrain texture. UV mapping generated by cartographic rectangle.
        ...
        textureParams.push(new RealityTextureParam(terrainTexture.texture, params, matrix, false));
      } else {
        params.data[0] = 1;    // Denotes a classifier texture.  UV mapping generated by multiplying XYZ by matrix (projection)
        const classifier = texture as PlanarClassifier;
        params.data[1] = classifier.textureImageCount;
        scratchBatchBaseId[0] = classifier.baseBatchId;
        params.data[4] = scratchBytes[0];
        params.data[5] = scratchBytes[1];
        params.data[6] = scratchBytes[2];
        params.data[7] = scratchBytes[3];
        textureParams.push(new RealityTextureParam(classifier.texture, params, Matrix4.fromMatrix4d(classifier.projectionMatrix), true));
      }
    }

    for (let i = textures.length; i < maxTexturesPerMesh; i++) {
      const params = new Matrix4(), matrix = new Matrix4();
      params.data[0] = 0;
      ...
      textureParams.push(new RealityTextureParam(undefined, params, matrix, false));
    }
