  ...
  implements RenderTargetDebugControl, WebGLDisposable
{
  ...
  protected allocateFbo(): FrameBuffer | undefined {
    ...
    const color = TextureHandle.createForAttachment(
      rect.width,
      rect.height,
  ...
  private changeDrapesOrClassifiers<T extends IDisposable>(
    oldMap: Map<String, T> | undefined,
  ...
  private createOrReuseReadPixelResources(
    ...
    if (this._readPixelReusableResources !== undefined) {
      ...
      if (
        this._readPixelReusableResources.texture.width >= rect.width &&
        this._readPixelReusableResources.texture.height >= rect.height
      ) {
    ...
    const texture = TextureHandle.createForAttachment(
      rect.width,
      rect.height,
      GL.Texture.Format.Rgba,
      GL.Texture.DataType.UnsignedByte
  ...
  private disposeOrReuseReadPixelResources({
    ...
    if (reuseResources && this._readPixelReusableResources !== undefined) {
      ...
      if (
        this._readPixelReusableResources.texture.width > texture.width &&
        this._readPixelReusableResources.texture.height > texture.height
      ) {
   ...
   */
  public override readImage(
    wantRectIn: ViewRect,
    targetSizeIn: Point2d,
    ...
    if (targetSize.x === wantRect.width && targetSize.y === wantRect.height) {
      ...
      const adjustedTargetSize = Target._applyAspectRatioCorrection(
        new Point2d(wantRect.width, wantRect.height),
        targetSize
      );
  ...
  public override readImageBuffer(
    ...
    if (
      ...
      const adjustedSize = Target._applyAspectRatioCorrection(
        { x: captureRect.width, y: captureRect.height },
        args.size
      );
  ...
  public getAnimationTransformNodeId(
    ...
    if (
      ...
      undefined === animationNodeId
    )
      return undefined;

    return this.animationBranches.transformNodeIds.has(animationNodeId)
      ? animationNodeId
...
export class OnScreenTarget extends Target {
  ...
  protected _endPaint(): void {
    ...
    const onscreenContext = this._2dCanvas.canvas.getContext("2d", {
      alpha: true,
    });
...
function interpolateFrustumPoint(
  destFrust: Frustum,
  srcFrust: Frustum,
  destPoint: Npc,
  scale: number,
