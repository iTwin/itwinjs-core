...
export class SpriteLocation implements CanvasDecoration {
    ...
    sprite.loadPromise
      .then(() => {
        if (this._viewport === viewport)
          // was this deactivated while we were loading?
  ...
  public drawDecoration(ctx: CanvasRenderingContext2D): void {
    ...
    if (undefined === sprite.image) return;

    if (undefined !== this._alpha) ctx.globalAlpha = this._alpha;
  ...
  public decorate(context: DecorateContext) {
    if (context.viewport === this._viewport) context.addCanvasDecoration(this);
  }
}
