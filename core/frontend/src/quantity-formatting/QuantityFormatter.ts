...
class StandardQuantityTypeDefinition implements QuantityTypeDefinition {
  ...
  constructor(
    ...
    public persistenceUnit: UnitProps,
...
export class QuantityFormatter implements UnitsProvider {
  ...
  constructor(showMetricOrUnitSystem?: boolean | UnitSystemKey) {
    if (undefined !== showMetricOrUnitSystem) {
      ...
      else this._activeUnitSystem = showMetricOrUnitSystem;
    }
  }
  ...
  private async setOverrideFormatsByQuantityTypeKey(typeKey: QuantityTypeKey, overrideEntry: OverrideFormatEntry) {
    ...
    await this._unitFormattingSettingsProvider?.storeFormatOverrides({
      typeKey,
      overrideEntry,
    });
  ...
  public async setUnitsProvider(unitsProvider: UnitsProvider) {
    ...
    if (IModelApp.toolAdmin) await IModelApp.toolAdmin.startDefaultTool();
    this.onUnitsProviderChanged.emit();
  }
  ...
  public async resetToUseInternalUnitsProvider() {
    if (this._unitsProvider instanceof BasicUnitsProvider) return;
  ...
  public async setActiveUnitSystem(
    isImperialOrUnitSystem: UnitSystemKey | boolean,
    ...
    await this._unitFormattingSettingsProvider?.storeUnitSystemSetting({
      system: systemType,
    });
    // fire current event
    ...
    if (this._overrideFormatPropsByUnitSystem.has(this.activeUnitSystem)) {
      ...
      if (overrides && overrides.size) {
        ...
        overrides.forEach((_props, typeKey) => {
          ...
          promises.push(
            this._unitFormattingSettingsProvider?.storeFormatOverrides({
              typeKey,
      ...
      if (typesRemoved.length) {
        ...
        await Promise.all(promises);
        // trigger a message to let callers know the format has changed.
        this.onQuantityFormatsChanged.emit({
          quantityType: typesRemoved.join("|"),
        });
      }
    }
  }
   ...
   */
  public async getFormatterSpecByQuantityTypeAndSystem(
    ...
    if (requestedSystem === this.activeUnitSystem) {
      ...
      if (formatterSpec) return formatterSpec;
    }
   ...
   * @return A FormatterSpec Promise.
   */
  public async getFormatterSpecByQuantityType(
    type: QuantityTypeArg,
    isImperial?: boolean
  ): Promise<FormatterSpec | undefined> {
  ...
  public async getParserSpecByQuantityTypeAndSystem(
    type: QuantityTypeArg,
    system?: UnitSystemKey
    ...
    if (requestedSystem === this.activeUnitSystem) {
      ...
      if (parserSpec) return parserSpec;
    }
   ...
   * @return A FormatterSpec Promise.
   */
  public async getParserSpecByQuantityType(
    type: QuantityTypeArg,
    isImperial?: boolean
  ): Promise<ParserSpec | undefined> {
    let requestedSystem = this.activeUnitSystem;
  ...
  public getUnitSystemFromString(inputSystem: string, fallback?: UnitSystemKey): UnitSystemKey {
    switch (inputSystem.toLowerCase()) {
      ...
      case "survey":
        return "usSurvey";
      default:
        if (undefined !== fallback) return fallback;
        break;
    }
    return "imperial";
  }
  ...
  public hasActiveOverride(type: QuantityTypeArg, checkOnlyActiveUnitSystem?: boolean): boolean {
    ...
    for (const [_key, overrideMap] of this._overrideFormatPropsByUnitSystem) {
      if (overrideMap.has(quantityTypeKey)) return true;
    }
    return false;
  }
   ...
   */
  public getFormatPropsByQuantityType(
    quantityType: QuantityTypeArg,
    requestedSystem?: UnitSystemKey,
    ignoreOverrides?: boolean
  ) {
    ...
    if (quantityEntry)
      return this.getFormatPropsByQuantityTypeEntryAndSystem(
        quantityEntry,
        requestedSystem ?? this.activeUnitSystem,
        ignoreOverrides
      );
    return undefined;
  }
...
const DEFAULT_FORMATKEY_BY_UNIT_SYSTEM = [
  {
    ...
    entries: [
      {
        type: getQuantityTypeKey(QuantityType.Length),
        formatKey: "[units:length]meter4",
      },
      {
        type: getQuantityTypeKey(QuantityType.Angle),
        formatKey: "[units:angle]degree2",
      },
      {
        type: getQuantityTypeKey(QuantityType.Area),
        formatKey: "[units:area]mSquared4",
      },
      {
        type: getQuantityTypeKey(QuantityType.Volume),
        formatKey: "[units:volume]mCubed4",
      },
      {
        type: getQuantityTypeKey(QuantityType.LatLong),
        formatKey: "[units:angle]dms",
      },
      {
        type: getQuantityTypeKey(QuantityType.Coordinate),
        formatKey: "[units:length]meter2",
      },
      {
        type: getQuantityTypeKey(QuantityType.Stationing),
        formatKey: "[units:length]m-sta2",
      },
      {
        type: getQuantityTypeKey(QuantityType.LengthSurvey),
        formatKey: "[units:length]meter4",
      },
      {
        type: getQuantityTypeKey(QuantityType.LengthEngineering),
        formatKey: "[units:length]meter4",
      },
    ],
  },
  {
    ...
    entries: [
      {
        type: getQuantityTypeKey(QuantityType.Length),
        formatKey: "[units:length]fi8",
      },
      {
        type: getQuantityTypeKey(QuantityType.Angle),
        formatKey: "[units:angle]dms2",
      },
      {
        type: getQuantityTypeKey(QuantityType.Area),
        formatKey: "[units:area]fSquared4",
      },
      {
        type: getQuantityTypeKey(QuantityType.Volume),
        formatKey: "[units:volume]fCubed4",
      },
      {
        type: getQuantityTypeKey(QuantityType.LatLong),
        formatKey: "[units:angle]dms",
      ...
      {
        type: getQuantityTypeKey(QuantityType.Coordinate),
        formatKey: "[units:length]feet2",
      },
      {
        type: getQuantityTypeKey(QuantityType.Stationing),
        formatKey: "[units:length]f-sta2",
      },
      {
        type: getQuantityTypeKey(QuantityType.LengthSurvey),
        formatKey: "[units:length]f-survey-4-labeled",
      },
      {
        type: getQuantityTypeKey(QuantityType.LengthEngineering),
        formatKey: "[units:length]feet4",
      },
    ],
  },
  {
    ...
    entries: [
      ...
      {
        type: getQuantityTypeKey(QuantityType.LengthSurvey),
        formatKey: "[units:length]f-survey-4",
      },
      {
        type: getQuantityTypeKey(QuantityType.LengthEngineering),
        formatKey: "[units:length]feet4",
  ...
  },
  {
    ...
    entries: [
      ...
      {
        type: getQuantityTypeKey(QuantityType.Area),
        formatKey: "[units:area]usSurveyFtSquared4",
      },
      {
        type: getQuantityTypeKey(QuantityType.Volume),
        formatKey: "[units:volume]usSurveyFtCubed4",
      },
      {
        type: getQuantityTypeKey(QuantityType.LatLong),
        formatKey: "[units:angle]dms",
      ...
      {
        type: getQuantityTypeKey(QuantityType.Coordinate),
        formatKey: "[units:length]f-survey-2",
      },
      {
        type: getQuantityTypeKey(QuantityType.Stationing),
        formatKey: "[units:length]f-survey-sta2",
      },
      {
        type: getQuantityTypeKey(QuantityType.LengthSurvey),
        formatKey: "[units:length]f-survey-4",
