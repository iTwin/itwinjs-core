...
export class FormatsProviderManager implements FormatsProvider {
	...
	public get formatsProvider(): FormatsProvider {
		return this;
	}
...
export class QuantityFormatter implements UnitsProvider {
	...
	constructor(showMetricOrUnitSystem?: boolean | UnitSystemKey) {
		if (undefined !== showMetricOrUnitSystem) {
			if (typeof showMetricOrUnitSystem === "boolean") this._activeUnitSystem = showMetricOrUnitSystem ? "metric" : "imperial";
	...
	public async onInitialized() {
		...
		const initialKoQs = [
			...
			["DefaultToolsUnits.ANGLE", "Units.RAD"],
			["DefaultToolsUnits.AREA", "Units.SQ_M"],
			["DefaultToolsUnits.VOLUME", "Units.CUB_M"],
			["DefaultToolsUnits.LENGTH_COORDINATE", "Units.M"],
			["CivilUnits.STATION", "Units.M"],
			["CivilUnits.LENGTH", "Units.M"],
			["AecUnits.LENGTH", "Units.M"],
		];
	 ...
	 * @public
	 */
	public async reinitializeFormatAndParsingsMaps(overrideFormatPropsByUnitSystem: Map<UnitSystemKey, Map<QuantityTypeKey, FormatProps>>, unitSystemKey?: UnitSystemKey, fireUnitSystemChanged?: boolean, startDefaultTool?: boolean): Promise<void> {
	 ...
	 */
	public formatQuantity(props: { value: number; valueUnitName: string; kindOfQuantityName: string }): Promise<string>;
	...
	public formatQuantity(args: number | object, spec?: FormatterSpec): string | Promise<string> {
		if (typeof args === "number") {
			...
			if (spec) return spec.applyFormatting(magnitude);
			return magnitude.toString();
		}

		return this.formatQuantityAsync(
			args as {
				value: number;
	...
	private async formatQuantityAsync(args: { value: number; valueUnitName: string; kindOfQuantityName: string }): Promise<string> {
	 ...
	 */
	public parseToQuantityValue(props: { value: string; valueUnitName: string; kindOfQuantityName: string }): Promise<QuantityParseResult>;
	...
	public parseToQuantityValue(args: string | object, parserSpec?: ParserSpec): QuantityParseResult | Promise<QuantityParseResult> {
		...
		return this.parseToQuantityValueAsync(
			args as {
				value: string;
				valueUnitName: string;
				kindOfQuantityName: string;
	...
	private async parseToQuantityValueAsync(args: { value: string; valueUnitName: string; kindOfQuantityName: string }): Promise<QuantityParseResult> {
		const { value, valueUnitName, kindOfQuantityName } = args;
		const formatProps = await IModelApp.formatsProvider.getFormat(kindOfQuantityName);
	...
	public getFormatPropsByQuantityType(quantityType: QuantityTypeArg, requestedSystem?: UnitSystemKey, ignoreOverrides?: boolean) {
		...
		if (quantityEntry) return this.getFormatPropsByQuantityTypeEntryAndSystem(quantityEntry, requestedSystem ?? this.activeUnitSystem, ignoreOverrides);
		return undefined;
	}
