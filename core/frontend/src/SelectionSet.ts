...
class HilitedElementIds extends HilitedIds {
  ...
  public set wantSyncWithSelectionSet(want: boolean) {
    if (want === this.wantSyncWithSelectionSet) return;
...
export class HiliteSet {
  ...
  public setHilite(arg: Id64Arg, onOff: boolean): void {
    for (const id of Id64.iterable(arg)) {
      if (onOff) this.elements.addId(id);
...
export class SelectionSet {
  ...
  private _add(elem: Id64Arg, sendEvent = true): boolean {
    ...
    for (const id of Id64.iterable(elem)) this.elements.add(id);

    const changed = oldSize !== this.elements.size;
    if (sendEvent && changed) this.sendChangedEvent({ type: SelectionSetEventType.Add, set: this, added: elem });
  ...
  private _remove(elem: Id64Arg, sendEvent = true): boolean {
    const oldSize = this.elements.size;
    for (const id of Id64.iterable(elem)) this.elements.delete(id);

    const changed = oldSize !== this.elements.size;
    if (sendEvent && changed) this.sendChangedEvent({ type: SelectionSetEventType.Remove, set: this, removed: elem });
  ...
  public addAndRemove(adds: Id64Arg, removes: Id64Arg): boolean {
    ...
    if (added && removed) this.sendChangedEvent({ type: SelectionSetEventType.Replace, set: this, added: adds, removed: removes });
    else if (added) this.sendChangedEvent({ type: SelectionSetEventType.Add, set: this, added: adds });
    else if (removed) this.sendChangedEvent({ type: SelectionSetEventType.Remove, set: this, removed: removes });
  ...
  public invert(elem: Id64Arg): boolean {
    ...
    for (const id of Id64.iterable(elem)) {
      if (this.elements.has(id)) elementsToRemove.add(id);
      else elementsToAdd.add(id);
    }
  ...
  public replace(elem: Id64Arg): void {
    if (areEqual(this.elements, elem)) return;
