...
export class ElementAgenda {
  ...
  public getSource() {
    return this.groupMarks.length === 0
      ? ModifyElementSource.Unknown
      : this.groupMarks[this.groupMarks.length - 1].source;
  }
  ...
  public add(arg: Id64Arg) {
    ...
    this.groupMarks.push({
      start: groupStart,
      source: ModifyElementSource.Unknown,
    });
...
export abstract class ElementSetTool extends PrimitiveTool {
   ...
   */
  protected async buildDragSelectAgenda(
    vp: Viewport,
    origin: Point3d,
    corner: Point3d,
    method: SelectionMethod,
