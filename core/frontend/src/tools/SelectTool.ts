...
export class SelectionTool extends PrimitiveTool {
        ...
        params: [
          {
            ...
            buttons: [{ iconSpec: "icon-select-single" }, { iconSpec: "icon-select-line" }, { iconSpec: "icon-select-box" }],
  ...
  public updateSelection(elementId: Id64Arg, process: SelectionProcessing): boolean {
    ...
    if (returnValue) this.syncSelectionMode();
    return returnValue;
  }
  ...
  private selectByPointsDecorate(context: DecorateContext): void {
    if (!this._isSelectByPoints) return;
    ...
    const crossingLine =
      SelectionMethod.Line === this.selectionMethod || (SelectionMethod.Pick === this.selectionMethod && BeButton.Reset === ev.button);
      ...
      (pixels) => {
        ...
        if (SelectionMethod.Box === method) {
          ...
          if (undefined !== outline && 0 !== outline.size) {
            ...
            contents.forEach((id) => {
              if (!outline.has(id)) inside.add(id);
            });
  ...
  protected selectByPointsEnd(ev: BeButtonEvent): boolean {
    if (!this._isSelectByPoints) return false;
  ...
  public async selectDecoration(ev: BeButtonEvent, currHit?: HitDetail): Promise<EventHandled> {
    if (undefined === currHit) currHit = await IModelApp.locateManager.doLocate(new LocateResponse(), true, ev.point, ev.viewport, ev.inputSource);
  ...
  public override async onResetButtonUp(ev: BeButtonEvent): Promise<EventHandled> {
    ...
    if (lastHit && this.iModel.selectionSet.has(lastHit.sourceId)) {
      ...
      if (undefined === autoHit || autoHit.isSameHit(lastHit)) {
        ...
        if (undefined !== nextHit) await this.processSelection(nextHit.sourceId, SelectionProcessing.AddElementToSelection);
  ...
  public override async onSuspend() {
    ...
    if (this.wantEditManipulators()) IModelApp.toolAdmin.manipulatorToolEvent.raiseEvent(this, ManipulatorToolEvent.Suspend);
  }

  public override async onUnsuspend() {
    this._isSuspended = false;
    if (this.wantEditManipulators()) IModelApp.toolAdmin.manipulatorToolEvent.raiseEvent(this, ManipulatorToolEvent.Unsuspend);

    this.showPrompt(this.selectionMode, this.selectionMethod);
  }

  public override async onTouchMoveStart(ev: BeTouchEvent, startEv: BeTouchEvent): Promise<EventHandled> {
    if (startEv.isSingleTouch && !this._isSelectByPoints) await IModelApp.toolAdmin.convertTouchMoveStartToButtonDownAndMotion(startEv, ev);
  ...
  public override async onCleanup() {
    if (this.wantEditManipulators()) IModelApp.toolAdmin.manipulatorToolEvent.raiseEvent(this, ManipulatorToolEvent.Stop);
  }

  public override async onPostInstall() {
    await super.onPostInstall();
    if (!this.targetView) return;
    if (this.wantEditManipulators()) IModelApp.toolAdmin.manipulatorToolEvent.raiseEvent(this, ManipulatorToolEvent.Start);
    this.initSelectTool();
  }
  ...
  public override supplyToolSettingsProperties(): DialogItem[] | undefined {
    if (!this.wantToolSettings()) return undefined;
  ...
  public override async applyToolSettingPropertyChange(updatedValue: DialogPropertySyncItem): Promise<boolean> {
    ...
    if (changed) this.initSelectTool();
