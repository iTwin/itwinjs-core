...
export class ViewClipTool extends PrimitiveTool {
  ...
  protected static _getEnumAsOrientationDescription = (): PropertyDescription => {
    return {
      ...
      enum: {
        choices: [
          {
            label: ViewClipTool.enumAsOrientationMessage("Top"),
            value: ContextRotationId.Top,
          },
          {
            label: ViewClipTool.enumAsOrientationMessage("Front"),
            value: ContextRotationId.Front,
          },
          {
            label: ViewClipTool.enumAsOrientationMessage("Left"),
            value: ContextRotationId.Left,
          },
          {
            label: ViewClipTool.enumAsOrientationMessage("Bottom"),
            value: ContextRotationId.Bottom,
          },
          {
            label: ViewClipTool.enumAsOrientationMessage("Back"),
            value: ContextRotationId.Back,
          },
          {
            label: ViewClipTool.enumAsOrientationMessage("Right"),
            value: ContextRotationId.Right,
          },
          {
            label: ViewClipTool.enumAsOrientationMessage("View"),
            value: ContextRotationId.View,
          },
          {
            label: ViewClipTool.enumAsOrientationMessage("Face"),
  ...
  public static enableClipVolume(viewport: Viewport): boolean {
    if (viewport.viewFlags.clipVolume) return false;
  ...
  public static doClipToPlane(
    viewport: Viewport,
    origin: Point3d,
    normal: Vector3d,
    clearExistingPlanes: boolean
  ): boolean {
    const plane = Plane3dByOriginAndUnitNormal.create(origin, normal);
    if (undefined === plane) return false;
  ...
  public static doClipToRange(viewport: Viewport, range: Range3d, transform?: Transform): boolean {
    if (range.isNull || range.isAlmostZeroX || range.isAlmostZeroY) return false;
    const clip = ClipVector.createEmpty();
    const block = ClipShape.createBlock(
      range,
      range.isAlmostZeroZ ? ClipMaskXYZRangePlanes.XAndY : ClipMaskXYZRangePlanes.All,
      false,
      false,
      transform
    );
  ...
  public static doClipClear(viewport: Viewport): boolean {
    if (!ViewClipTool.hasClip(viewport)) return false;
  ...
  public static getOffsetValueTransformed(offset: number, transform?: Transform) {
    if (undefined === transform) return offset;
  ...
  public static addClipPlanesLoops(builder: GraphicBuilder, loops: GeometryQuery[], outline: boolean): void {
    for (const geom of loops) {
      if (!(geom instanceof Loop)) continue;
      if (outline) builder.addPath(Path.createArray(geom.children));
      else builder.addLoop(geom);
    }
  }
  ...
  public static drawClip(
    ...
    if (undefined === clipShape && undefined === clipPlanes) return;

    const viewRange = viewExtents ? viewExtents : context.viewport.computeViewRange();
    const clipPlanesLoops =
      undefined !== clipPlanes
        ? ClipUtilities.loopsOfConvexClipPlaneIntersectionWithRange(clipPlanes, viewRange)
        : undefined;
    if (undefined === clipShape && (undefined === clipPlanesLoops || 0 === clipPlanesLoops.length)) return;

    const color =
      options && options.color
        ? options.color
        : EditManipulator.HandleUtils.adjustForBackgroundColor(ColorDef.white, context.viewport);
    const builderVis = context.createGraphicBuilder(
      GraphicType.WorldDecoration,
      clipShape ? clipShape.transformFromClip : undefined,
    ...
    builderHid.setSymbology(
      color,
      ColorDef.black,
      options && options.hiddenWidth ? options.hiddenWidth : 1,
      options && options.hiddenStyle ? options.hiddenStyle : LinePixels.Code2
    );
    ...
    if (undefined !== clipPlanesLoops) {
      ...
      if (options && options.fillClipPlanes) {
        const fill = options.fill
          ? options.fill
          : EditManipulator.HandleUtils.adjustForBackgroundColor(ColorDef.from(0, 255, 255, 225), context.viewport);
        const builderFill = context.createGraphicBuilder(GraphicType.WorldDecoration);
        builderFill.setSymbology(fill, fill, 0);
        ViewClipTool.addClipPlanesLoops(
          builderFill,
          options.hasPrimaryPlane ? [clipPlanesLoops[0]] : clipPlanesLoops,
          false
        );
  ...
  public static drawClipShape(
    ...
    if (!this.isHilited(context.viewport, id)) {
      ...
      builderHid.setSymbology(
        this.isFlashed(context.viewport, id) ? context.viewport.hilite.color : color,
        ColorDef.black,
        1,
        LinePixels.Code2
      );
  ...
  public static getClipShapePoints(shape: ClipShape, z: number): Point3d[] {
    ...
    for (const pt of shape.polygon) points.push(Point3d.create(pt.x, pt.y, z));
    return points;
  }
  ...
  public static getClipShapeExtents(shape: ClipShape, viewRange: Range3d): Range1d {
    ...
    if (undefined === zLow || undefined === zHigh) {
      ...
      if (undefined !== shape.transformToClip) shape.transformToClip.multiplyPoint3dArrayInPlace(corners);
      for (const corner of corners) {
        const delta = Vector3d.createStartEnd(origin, corner);
        const projection = delta.dotProduct(zVec);
        if (undefined === shape.zLow && (undefined === zLow || projection < zLow)) zLow = projection;
        if (undefined === shape.zHigh && (undefined === zHigh || projection > zHigh)) zHigh = projection;
      }
    }
  ...
  public static isSingleClipShape(clip: ClipVector): ClipShape | undefined {
    if (1 !== clip.clips.length) return undefined;
    const prim = clip.clips[0];
    if (!(prim instanceof ClipShape)) return undefined;
    if (!prim.isValidPolygon) return undefined;
    return prim;
  }

  public static drawClipPlanesLoops(
    context: DecorateContext,
    loops: GeometryQuery[],
    color: ColorDef,
    weight: number,
  ...
  public static isSingleClipPlane(clip: ClipVector): ClipPlane | undefined {
    ...
    if (undefined === clipPlanes || 1 !== clipPlanes.planes.length) return undefined;
  ...
  public static areClipsEqual(clipA: ClipVector, clipB: ClipVector): boolean {
    ...
    if (clipA.clips.length !== clipB.clips.length) return false;
    for (let iPrim = 0; iPrim < clipA.clips.length; iPrim++) {
      ...
      if (undefined !== planesA && undefined !== planesB) {
        if (planesA.convexSets.length !== planesB.convexSets.length) return false;
        for (let iPlane = 0; iPlane < planesA.convexSets.length; iPlane++) {
          ...
          if (planeSetA.planes.length !== planeSetB.planes.length) return false;
          for (let iClipPlane = 0; iClipPlane < planeSetA.planes.length; iClipPlane++) {
            ...
            if (!planeA.isAlmostEqual(planeB)) return false;
          }
        }
...
export class ViewClipClearTool extends ViewClipTool {
  ...
  /** @internal */
  public override isCompatibleViewport(vp: Viewport | undefined, isSelectedViewChange: boolean): boolean {
    return super.isCompatibleViewport(vp, isSelectedViewChange) && undefined !== vp && ViewClipTool.hasClip(vp);
  }
  /** @internal */
  protected override showPrompt(): void {
    const mainInstruction = ToolAssistance.createInstruction(
      this.iconSpec,
      CoreTools.translate("ViewClip.Clear.Prompts.FirstPoint")
    );
  ...
  protected async doClipClear(viewport: Viewport): Promise<boolean> {
    if (!ViewClipTool.doClipClear(viewport)) return false;
    if (undefined !== this._clipEventHandler) this._clipEventHandler.onClearClip(viewport);
    await this.onReinitialize();
    return true;
  }
  ...
  public override async onPostInstall(): Promise<void> {
    await super.onPostInstall();
    if (undefined !== this.targetView) await this.doClipClear(this.targetView);
  }
...
export class ViewClipByPlaneTool extends ViewClipTool {
  ...
  /** @internal */
  private _orientationValue: DialogItemValue = {
    value: ContextRotationId.Face,
  };

  constructor(clipEventHandler?: ViewClipEventHandler, protected _clearExistingPlanes: boolean = false) {
  ...
  public override supplyToolSettingsProperties(): DialogItem[] | undefined {
    const initialValue = IModelApp.toolAdmin.toolSettingsState.getInitialToolSettingValue(
      this.toolId,
      ViewClipTool._orientationName
    );
    initialValue && (this._orientationValue = initialValue);
    const toolSettings = new Array<DialogItem>();
    const settingsItem: DialogItem = {
      value: this._orientationValue,
      property: ViewClipTool._getEnumAsOrientationDescription(),
  ...
  protected override showPrompt(): void {
    const mainInstruction = ToolAssistance.createInstruction(
      this.iconSpec,
      CoreTools.translate("ViewClip.ByPlane.Prompts.FirstPoint")
    );
  ...
  public override async onDataButtonDown(ev: BeButtonEvent): Promise<EventHandled> {
    ...
    if (undefined === normal) return EventHandled.No;
    ...
    if (!ViewClipTool.doClipToPlane(this.targetView, ev.point, normal, this._clearExistingPlanes))
      return EventHandled.No;
    if (undefined !== this._clipEventHandler) this._clipEventHandler.onNewClipPlane(this.targetView);
    await this.onReinitialize();
...
export class ViewClipByShapeTool extends ViewClipTool {
  ...
  protected override setupAndPromptForNextAction(): void {
    ...
    if (undefined === this._matrix) return;
  ...
  protected getClipPoints(ev: BeButtonEvent): Point3d[] {
    ...
    if (undefined === this._matrix) return points;
    ...
    if (undefined === currentPt) currentPt = ev.point.clone();
  ...
  public override async onMouseMotion(ev: BeButtonEvent): Promise<void> {
    if (this._points.length > 0 && undefined !== ev.viewport) ev.viewport.invalidateDecorations();
  }
  ...
  public override async onDataButtonDown(ev: BeButtonEvent): Promise<EventHandled> {
    ...
    if (this._points.length > 1 && !ev.isControlKey) {
      ...
      if (!ViewClipTool.doClipToShape(this.targetView, points, transform, this._zLow, this._zHigh))
        return EventHandled.No;
      if (undefined !== this._clipEventHandler) this._clipEventHandler.onNewClip(this.targetView);
      await this.onReinitialize();
      return EventHandled.Yes;
    }
...
export class ViewClipByRangeTool extends ViewClipTool {
  ...
  protected override showPrompt(): void {
    const mainInstruction = ToolAssistance.createInstruction(
      this.iconSpec,
      CoreTools.translate(
        undefined === this._corner ? "ViewClip.ByRange.Prompts.FirstPoint" : "ViewClip.ByRange.Prompts.NextPoint"
      )
    );
    const mouseInstructions: ToolAssistanceInstruction[] = [];
    const touchInstructions: ToolAssistanceInstruction[] = [];
  ...
  protected getClipRange(range: Range3d, transform: Transform, ev: BeButtonEvent): boolean {
    if (undefined === this.targetView || undefined === this._corner) return false;
    ...
    if (undefined === pt1 || undefined === pt2) return false;
  ...
  /** @internal */
  public override decorate(context: DecorateContext): void {
    if (context.viewport !== this.targetView || undefined === this._corner) return;
    ...
    if (!this.getClipRange(range, transform, ev)) return;
  ...
  /** @internal */
  public override async onMouseMotion(ev: BeButtonEvent): Promise<void> {
    if (undefined !== this._corner && undefined !== ev.viewport) ev.viewport.invalidateDecorations();
  }
  ...
  public override async onDataButtonDown(ev: BeButtonEvent): Promise<EventHandled> {
    ...
    if (undefined !== this._corner) {
      const range = Range3d.create();
      const transform = Transform.createIdentity();
      if (!this.getClipRange(range, transform, ev)) return EventHandled.No;
      ViewClipTool.enableClipVolume(this.targetView);
      if (!ViewClipTool.doClipToRange(this.targetView, range, transform)) return EventHandled.No;
      if (undefined !== this._clipEventHandler) this._clipEventHandler.onNewClip(this.targetView);
      await this.onReinitialize();
      return EventHandled.Yes;
    }
  ...
  /** @internal */
  public override async onUndoPreviousStep(): Promise<boolean> {
    if (undefined === this._corner) return false;
...
export class ViewClipByElementTool extends ViewClipTool {
  ...
  constructor(clipEventHandler?: ViewClipEventHandler, protected _alwaysUseRange: boolean = false) {
    super(clipEventHandler);
  }
  ...
  protected override showPrompt(): void {
    const mainInstruction = ToolAssistance.createInstruction(
      this.iconSpec,
      CoreTools.translate("ViewClip.ByElement.Prompts.FirstPoint")
    );
    const mouseInstructions: ToolAssistanceInstruction[] = [];
    const touchInstructions: ToolAssistanceInstruction[] = [];
  ...
  public override async onPostInstall() {
    ...
    if (undefined !== this.targetView && this.targetView.iModel.selectionSet.isActive) {
      ...
      this.targetView.iModel.selectionSet.elements.forEach((val) => {
        if (Id64.isInvalid(val) || Id64.isTransient(val)) useSelection = false;
      });
  ...
  public async doClipToSelectedElements(viewport: Viewport): Promise<boolean> {
    if (await this.doClipToElements(viewport, viewport.iModel.selectionSet.elements, this._alwaysUseRange)) return true;
    await this.exitTool();
    return false;
  }

  protected async doClipToElements(
    viewport: Viewport,
    ids: Id64Arg,
    alwaysUseRange: boolean = false,
    modelId?: Id64String
  ): Promise<boolean> {
    try {
      const placements = await viewport.iModel.elements.getPlacements(ids, {
        type: viewport.view.is3d() ? "3d" : "2d",
      });
      if (0 === placements.length) return false;

      const displayTransform =
        modelId && 1 === placements.length
          ? viewport.view.computeDisplayTransform({
              modelId,
      ...
      if (!alwaysUseRange && 1 === placements.length) {
        ...
        for (const placement of placements) range.extendRange(placement.calculateRange());

        if (displayTransform) transform.setFrom(displayTransform);
      }
      ...
      if (range.isAlmostZeroX || range.isAlmostZeroY) {
        if (range.isAlmostZeroZ) return false;
        ...
        if (!canUseXZ && !canUseYZ) return false;
        ...
        if (!ViewClipTool.doClipToShape(viewport, points, transform)) return false;

        if (undefined !== this._clipEventHandler) this._clipEventHandler.onNewClip(viewport);

        await this.onReinitialize();
        return true;
      }
      ViewClipTool.enableClipVolume(viewport);
      if (!ViewClipTool.doClipToRange(viewport, range, transform)) return false;
  ...
  public override async onDataButtonDown(ev: BeButtonEvent): Promise<EventHandled> {
    ...
    return (await this.doClipToElements(this.targetView, hit.sourceId, this._alwaysUseRange, hit.modelId))
      ? EventHandled.Yes
      : EventHandled.No;
  }
}
...
export abstract class ViewClipModifyTool extends EditManipulator.HandleTool {
  ...
  public constructor(
    manipulator: EditManipulator.HandleProvider,
    clip: ClipVector,
    vp: Viewport,
    hitId: string,
    ids: string[],
    controls: ViewClipControlArrow[]
  ) {
    super(manipulator);
  ...
  protected getOffsetValue(ev: BeButtonEvent, transformFromClip?: Transform): number | undefined {
    if (-1 === this._anchorIndex || undefined === ev.viewport || ev.viewport !== this._clipView) return undefined;
    ...
    const anchorRay = ViewClipTool.getClipRayTransformed(
      this._controls[this._anchorIndex].origin,
      this._controls[this._anchorIndex].direction,
      transformFromClip
    );
    const projectedPt = AccuDrawHintBuilder.projectPointToLineInView(
      ev.point,
      anchorRay.origin,
      anchorRay.direction,
      ev.viewport,
    ...
    if (undefined === projectedPt) return undefined;
    ...
    if (offsetVec.dotProduct(anchorRay.direction) < 0.0) offset *= -1.0;

    this._currentDistance = offset;
    return offset;
  }

  protected drawAnchorOffset(
    context: DecorateContext,
    color: ColorDef,
    weight: number,
    transformFromClip?: Transform
  ): void {
    if (-1 === this._anchorIndex || Math.abs(this._currentDistance) < Geometry.smallMetricDistance) return;
  ...
  public override decorate(context: DecorateContext): void {
    if (-1 === this._anchorIndex || context.viewport !== this._clipView) return;
    this.drawViewClip(context);
  }

  public override async onMouseMotion(ev: BeButtonEvent): Promise<void> {
    if (!this.updateViewClip(ev, false)) return;
    this._clipView.invalidateDecorations();
  }

  protected accept(ev: BeButtonEvent): boolean {
    if (!this.updateViewClip(ev, true)) return false;
    this._restoreClip = false;
    return true;
  }

  public override async onCleanup() {
    if (this._restoreClip && ViewClipTool.hasClip(this._clipView)) ViewClipTool.setViewClip(this._clipView, this._clip);
...
export class ViewClipShapeModifyTool extends ViewClipModifyTool {
  protected updateViewClip(ev: BeButtonEvent, _isAccept: boolean): boolean {
    ...
    if (undefined === offset) return false;
    ...
    for (let i = 0; i < shapePts.length; i++) {
      ...
      const prevSelected =
        offsetAll || prevFace === this._anchorIndex || this.manipulator.iModel.selectionSet.has(this._ids[prevFace]);
      const nextSelected =
        offsetAll || nextFace === this._anchorIndex || this.manipulator.iModel.selectionSet.has(this._ids[nextFace]);
      if (prevSelected && nextSelected) {
        ...
        const prevRay = Ray3d.create(
          prevPt,
          Vector3d.createStartEnd(shapePts[i === 0 ? shapePts.length - 2 : i - 1], shapePts[i])
        );
        const nextPlane = Plane3dByOriginAndUnitNormal.create(nextPt, this._controls[nextFace].direction);
        if (undefined === nextPlane || undefined === prevRay.intersectionWithPlane(nextPlane, prevPt)) return false;
    ...
    const zLowSelected =
      offsetAll || zLowIndex === this._anchorIndex || this.manipulator.iModel.selectionSet.has(this._ids[zLowIndex]);
    const zHighSelected =
      offsetAll || zHighIndex === this._anchorIndex || this.manipulator.iModel.selectionSet.has(this._ids[zHighIndex]);

    if (zLowSelected || zHighSelected) {
      const clipExtents = ViewClipTool.getClipShapeExtents(clipShape, this._viewRange);
      if (zLowSelected) zLow = clipExtents.low - localOffset;
      if (zHighSelected) zHigh = clipExtents.high + localOffset;
  ...
  protected drawViewClip(context: DecorateContext): void {
    ...
    if (undefined === clip) return;
    const clipShape = ViewClipTool.isSingleClipShape(clip);
    if (undefined === clipShape) return;
    const clipExtents = ViewClipTool.getClipShapeExtents(clipShape, this._viewRange);
...
export class ViewClipPlanesModifyTool extends ViewClipModifyTool {
  protected updateViewClip(ev: BeButtonEvent, _isAccept: boolean): boolean {
    ...
    for (let i: number = 0; i < this._controls.length; i++) {
      ...
      if (selected) origin.plusScaled(direction, offset, origin);
  ...
  protected drawViewClip(context: DecorateContext): void {
    ...
    if (undefined === clip) return;
    const clipPlanes = ViewClipTool.isSingleConvexClipPlaneSet(clip);
    if (undefined === clipPlanes) return;
    const clipPlanesLoops = ClipUtilities.loopsOfConvexClipPlaneIntersectionWithRange(
      clipPlanes,
      this._viewRange,
      true,
      false,
    ...
    if (undefined === clipPlanesLoops) return;
    const color = EditManipulator.HandleUtils.adjustForBackgroundColor(ColorDef.white, context.viewport);
...
export class ViewClipDecoration extends EditManipulator.HandleProvider {
  ...
  public get clipId(): string | undefined {
    return this._clipId;
  }
  public get clipShape(): ClipShape | undefined {
    return this._clipShape;
  }
  public get clipPlaneSet(): ConvexClipPlaneSet | undefined {
    return this._clipPlanes;
  }
  public getControlIndex(id: string): number {
    return this._controlIds.indexOf(id);
  }

  protected override stop(): void {
    const selectedId =
      undefined !== this._clipId && this.iModel.selectionSet.has(this._clipId) ? this._clipId : undefined;
  ...
  public onViewClose(vp: ScreenViewport): void {
    if (this._clipView === vp) ViewClipDecoration.clear();
  }
  ...
  private getClipData(): boolean {
    this._clip =
      ...
      this._clipPlanes =
      this._clipPlanesLoops =
      this._clipPlanesLoopsNoncontributing =
        undefined;
    ...
    if (undefined !== clipShape) {
      ...
      if (undefined === clipPlanes || clipPlanes.planes.length > 12) {
        ...
        for (const primitive of clip.clips) {
          ...
          if (undefined === union) continue;

          for (const convexSet of union.convexSets) {
            const convexSetLoops = ClipUtilities.loopsOfConvexClipPlaneIntersectionWithRange(
              convexSet,
              viewRange,
              true,
              false,
            ...
            if (undefined === convexSetLoops) continue;

            if (undefined === this._clipPlanesLoops) this._clipPlanesLoops = convexSetLoops;
            else this._clipPlanesLoops.push(...convexSetLoops);
          }
        }

        if (undefined === this._clipPlanesLoops) return false;
      ...
      const clipPlanesLoops = ClipUtilities.loopsOfConvexClipPlaneIntersectionWithRange(
        clipPlanes,
        this._clipView.computeViewRange(),
        true,
        false,
        true
      );
      if (undefined !== clipPlanesLoops && clipPlanesLoops.length > clipPlanes.planes.length) return false;
  ...
  private ensureNumControls(numReqControls: number): void {
    ...
    if (numCurrent < numReqControls) {
      ...
      for (let i: number = numCurrent; i < numReqControls; i++) this._controlIds[i] = transientIds.getNext();
  ...
  private createClipShapeControls(): boolean {
    if (undefined === this._clipShape || undefined === this._clipShapeExtents) return false;
    ...
    this._controls[numControls - 2] = new ViewClipControlArrow(
      shapeArea.origin,
      Vector3d.unitZ(-1.0),
      0.75,
      zFillColor,
    ...
    this._controls[numControls - 1] = new ViewClipControlArrow(
  ...
  private getLoopCentroidAreaNormal(geom: GeometryQuery): Ray3d | undefined {
    if (!(geom instanceof Loop) || geom.children.length > 1) return undefined;
    const child = geom.getChild(0);
    if (!(child instanceof LineString3d)) return undefined;
    return PolygonOps.centroidAreaNormal(child.points);
  }

  private createClipPlanesControls(): boolean {
    if (undefined === this._clipPlanes) return false;
    ...
    if (undefined !== this._clipPlanesLoops) {
      for (const geom of this._clipPlanesLoops) {
        ...
        if (undefined !== loopArea) loopData.push(loopArea);
      }
    }
    ...
    for (let i: number = 0; i < this._clipPlanes.planes.length; i++) {
      ...
      if (iLoop < loopData.length) {
        if (
          Math.abs(loopData[iLoop].direction.dotProduct(plane.getNormalRef())) > 0.9999 &&
      ...
      if (undefined === viewRange) viewRange = this._clipView.computeViewRange();
      ...
      const nonContribLoops = ClipUtilities.loopsOfConvexClipPlaneIntersectionWithRange(
        ConvexClipPlaneSet.createPlanes([this._clipPlanes.planes[i]]),
        expandedRange,
        true,
        false,
        true
      ...
      if (undefined !== nonContribLoops && nonContribLoops.length > 0) {
        if (undefined === this._clipPlanesLoopsNoncontributing) this._clipPlanesLoopsNoncontributing = nonContribLoops;
        else this._clipPlanesLoopsNoncontributing = this._clipPlanesLoopsNoncontributing.concat(nonContribLoops);
    ...
    if (this.iModel.selectionSet.size <= this._controlIds.length + 1 && this.iModel.selectionSet.has(this._clipId)) {
      ...
      if (this.iModel.selectionSet.size > 1) {
        this.iModel.selectionSet.elements.forEach((val) => {
          if (this._clipId !== val && !this._controlIds.includes(val)) showControls = false;
        });
      }
    }

    if (!showControls) {
      if (undefined !== this._clipEventHandler && this._clipEventHandler.clearOnDeselect()) ViewClipDecoration.clear();
      return false;
    }

    if (undefined !== this._clipShape) return this.createClipShapeControls();
    else if (undefined !== this._clipPlanes) return this.createClipPlanesControls();
  ...
  protected async modifyControls(hit: HitDetail, _ev: BeButtonEvent): Promise<boolean> {
    ...
    if (undefined !== this._clipShape) {
      ...
      const clipPlanesModifyTool = new ViewClipPlanesModifyTool(
        this,
        this._clip,
        this._clipView,
        hit.sourceId,
    ...
    if (this._suspendDecorator) IModelApp.toolAdmin.currentInputState.qualifiers = saveQualifiers; // onInstallTool cleared qualifiers, preserve for "modify all" behavior when shift was held and drag started...
  ...
  public doClipPlaneNegate(index: number): boolean {
    ...
    if (index < 0 || index >= this._clipPlanes.planes.length) return false;
    ...
    if (!ViewClipTool.doClipToConvexClipPlaneSet(this._clipView, planeSet)) return false;
  ...
  public doClipPlaneClear(index: number): boolean {
    ...
    if (1 === this._clipPlanes.planes.length) {
      if (!ViewClipTool.doClipClear(this._clipView)) return false;
      if (undefined !== this._clipEventHandler) this._clipEventHandler.onClearClip(this._clipView);
  ...
  private getClipShapeFaceLoops(): GeometryQuery[] | undefined {
    if (undefined === this._clipShape || undefined === this._clipShapeExtents) return undefined;
    ...
    for (let i: number = 0; i < shapePtsLo.length; i++) {
      ...
      const side = Loop.createPolygon([
        shapePtsLo[i].clone(),
        shapePtsLo[next].clone(),
        shapePtsHi[next].clone(),
        shapePtsHi[i].clone(),
      ]);
      faces.push(side);
    }
  ...
  private getMatchingLoop(loops: GeometryQuery[], ray: Ray3d): GeometryQuery | undefined {
    for (const geom of loops) {
      const loopArea = this.getLoopCentroidAreaNormal(geom);
      if (undefined === loopArea) continue;

      if (!loopArea.direction.isParallelTo(ray.direction, true))
        // don't assume outward normal for clip plane loops...
        continue;

      const plane = Plane3dByOriginAndUnitNormal.create(loopArea.origin, loopArea.direction);
      if (undefined === plane || !plane.isPointInPlane(ray.origin)) continue;
  ...
  private getLoopPreferredX(loop: GeometryQuery, outwardNormal: Vector3d): Vector3d | undefined {
    ...
    if (dirZ.isParallelTo(unitZ, true)) {
      ...
      if (vectorX.dotProduct(unitX) < 0.0) vectorX.negate(vectorX); // prefer positive x...
    } else {
      ...
      if (vectorY && vectorY.dotProduct(unitZ) < 0.0) vectorX.negate(vectorX); // prefer positive z...
    }
  ...
  public doClipPlaneOrientView(index: number): boolean {
    if (index < 0 || index >= this._controlIds.length) return false;

    const vp = this._clipView;
    const anchorRay = ViewClipTool.getClipRayTransformed(
      this._controls[index].origin,
      this._controls[index].direction,
    ...
    if (
      undefined === vectorX ||
      undefined === Matrix3d.createRigidFromColumns(anchorRay.direction, vectorX, AxisOrder.ZXY, matrix)
    )
  ...
  private getWorldUpPlane(viewport: Viewport): Plane3dByOriginAndUnitNormal | undefined {
    ...
    const planePt = viewport.isContextRotationRequired
      ? viewport.getAuxCoordOrigin()
      : viewport.view.isSpatialView()
      ? viewport.view.iModel.globalOrigin
      : Point3d.createZero();
  ...
  private isAlignedToWorldUpPlane(plane: Plane3dByOriginAndUnitNormal, transformFromClip?: Transform): boolean {
    const normal =
      undefined !== transformFromClip ? transformFromClip.multiplyVector(Vector3d.unitZ()) : Vector3d.unitZ();
  ...
  public isClipShapeAlignedWithWorldUp(extents?: Range1d): boolean {
    ...
    if (undefined === plane || !this.isAlignedToWorldUpPlane(plane, this._clipShape.transformFromClip)) return false;

    if (undefined === extents) return true;
    ...
    if (lowDir.dotProduct(plane.getNormalRef()) < 0.0) zLowWorld = -zLowWorld;
    if (highDir.dotProduct(plane.getNormalRef()) < 0.0) zHighWorld = -zHighWorld;
  ...
  public doClipShapeSetZExtents(extents: Range1d): boolean {
    if (extents.low > extents.high) return false;
    if (undefined === this._clipShape) return false;
    const plane = this.getWorldUpPlane(this._clipView);
    if (undefined === plane || !this.isAlignedToWorldUpPlane(plane, this._clipShape.transformFromClip)) return false;
    ...
    shape.initSecondaryProps(
      shape.isMask,
      reversed ? zHigh.z : zLow.z,
    ...
    if (!ViewClipTool.setViewClip(this._clipView, clip)) return false;
  ...
  protected override async onRightClick(hit: HitDetail, ev: BeButtonEvent): Promise<EventHandled> {
    if (undefined === this._clipEventHandler) return EventHandled.No;
  ...
  public async getDecorationToolTip(hit: HitDetail): Promise<HTMLElement | string> {
    if (hit.sourceId === this._clipId) return CoreTools.translate("ViewClip.Message.Clip");
  ...
  public override decorate(context: DecorateContext): void {
    ...
    if (undefined === this._clipId || undefined === this._clip) return;

    const vp = context.viewport;
    if (this._clipView !== vp) return;
    ...
    if (undefined !== this._clipShape) {
      ViewClipTool.drawClipShape(
        ...
        this._clipShape,
        this._clipShapeExtents!,
        EditManipulator.HandleUtils.adjustForBackgroundColor(ColorDef.white, vp),
        3,
        this._clipId
      );
    } else if (undefined !== this._clipPlanes) {
      if (undefined !== this._clipPlanesLoops)
        ViewClipTool.drawClipPlanesLoops(
          context,
          this._clipPlanesLoops,
          EditManipulator.HandleUtils.adjustForBackgroundColor(ColorDef.white, vp),
          3,
          false,
          EditManipulator.HandleUtils.adjustForBackgroundColor(ColorDef.from(0, 255, 255, 225), vp),
          this._clipId
        );
      if (undefined !== this._clipPlanesLoopsNoncontributing)
        ViewClipTool.drawClipPlanesLoops(
          context,
          this._clipPlanesLoopsNoncontributing,
          EditManipulator.HandleUtils.adjustForBackgroundColor(ColorDef.red, vp),
          1,
          true
        );
    ...
    for (let iFace = 0; iFace < this._controlIds.length; iFace++) {
      ...
      if (
        1 === this._controls.length &&
        undefined !== this._clipPlanes &&
        undefined !== this._clipPlanesLoops &&
        this._clipPlanesLoops.length > 0
      ) {
        if (!vp.isPointVisibleXY(this._controls[iFace].origin, CoordSystem.World, 0.05)) {
          ...
          undefined !== this._controls[iFace].floatingOrigin &&
          vp.isPointVisibleXY(this._controls[iFace].floatingOrigin!, CoordSystem.World, 0.1)
        ) {
  ...
  public static get(vp: ScreenViewport): ViewClipDecoration | undefined {
    if (undefined === ViewClipDecoration._decorator || vp !== ViewClipDecoration._decorator._clipView) return undefined;
  ...
  public static create(vp: ScreenViewport, clipEventHandler?: ViewClipEventHandler): string | undefined {
    if (undefined !== ViewClipDecoration._decorator) ViewClipDecoration.clear();
    if (!ViewClipTool.hasClip(vp)) return undefined;
    ViewClipDecoration._decorator = new ViewClipDecoration(vp, clipEventHandler);
    return ViewClipDecoration._decorator.clipId;
  }

  public static clear(): void {
    if (undefined === ViewClipDecoration._decorator) return;
  ...
  public static async toggle(vp: ScreenViewport, clipEventHandler?: ViewClipEventHandler): Promise<string | undefined> {
    ...
    if (undefined === ViewClipDecoration._decorator) clipId = ViewClipDecoration.create(vp, clipEventHandler);
    else ViewClipDecoration.clear();
...
export class ViewClipDecorationProvider implements ViewClipEventHandler {
  ...
  public readonly onActiveClipChanged = new BeEvent<
    (viewport: Viewport, eventType: ClipEventType, provider: ViewClipDecorationProvider) => void
  >();
   ...
   */
  public readonly onActiveClipRightClick = new BeEvent<
    (hit: HitDetail, ev: BeButtonEvent, provider: ViewClipDecorationProvider) => void
  >();

  public selectOnCreate(): boolean {
    return this.selectDecorationOnCreate;
  }
  public clearOnDeselect(): boolean {
    return this.clearDecorationOnDeselect;
  }
  ...
  public onRightClick(hit: HitDetail, ev: BeButtonEvent): boolean {
    ...
    if (undefined === decoration) return false;
  ...
  public showDecoration(vp: ScreenViewport): void {
    ViewClipDecoration.create(vp, this);
  }
  public hideDecoration(): void {
    ViewClipDecoration.clear();
  }
  public async toggleDecoration(vp: ScreenViewport) {
    return ViewClipDecoration.toggle(vp, this);
  }
  public isDecorationActive(vp: ScreenViewport): boolean {
    return undefined !== ViewClipDecoration.get(vp);
  }
  ...
  public static clear(): void {
    if (undefined === ViewClipDecorationProvider._provider) return;
    ViewClipDecoration.clear();
