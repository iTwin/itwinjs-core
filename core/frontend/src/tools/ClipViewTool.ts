...
export class ViewClipTool extends PrimitiveTool {
    ...
    if (!clearExistingPlanes) {
      ...
      if (undefined !== existingClip && 1 === existingClip.clips.length) {
        ...
        if (!(existingPrim instanceof ClipShape)) {
          ...
          if (undefined !== existingPlaneSets && 1 === existingPlaneSets.convexSets.length) planeSet = existingPlaneSets.convexSets[0];
        }
      }
    }
  ...
  private static addClipShape(builder: GraphicBuilder, shape: ClipShape, extents: Range1d): void {
    ...
    for (let i: number = 0; i < shapePtsLo.length; i++) builder.addLineString([shapePtsLo[i].clone(), shapePtsHi[i].clone()]);
...
export class ViewClipByPlaneTool extends ViewClipTool {
  ...
  public override async onDataButtonDown(ev: BeButtonEvent): Promise<EventHandled> {
    ...
    if (!ViewClipTool.doClipToPlane(this.targetView, ev.point, normal, this._clearExistingPlanes)) return EventHandled.No;
...
export class ViewClipByShapeTool extends ViewClipTool {
  ...
  public override async onDataButtonDown(ev: BeButtonEvent): Promise<EventHandled> {
    ...
    if (this._points.length > 1 && !ev.isControlKey) {
      ...
      ViewClipTool.enableClipVolume(this.targetView);
      if (!ViewClipTool.doClipToShape(this.targetView, points, transform, this._zLow, this._zHigh)) return EventHandled.No;
...
export class ViewClipByRangeTool extends ViewClipTool {
  ...
  protected override showPrompt(): void {
    const mainInstruction = ToolAssistance.createInstruction(
      this.iconSpec,
      CoreTools.translate(undefined === this._corner ? "ViewClip.ByRange.Prompts.FirstPoint" : "ViewClip.ByRange.Prompts.NextPoint")
    );
    const mouseInstructions: ToolAssistanceInstruction[] = [];
    const touchInstructions: ToolAssistanceInstruction[] = [];
  ...
  protected async doClipToElements(viewport: Viewport, ids: Id64Arg, alwaysUseRange: boolean = false, modelId?: Id64String): Promise<boolean> {
    try {
      ...
      const displayTransform =
        modelId && 1 === placements.length ? viewport.view.computeDisplayTransform({ modelId, elementId: placements[0].elementId }) : undefined;
...
export class ViewClipDecoration extends EditManipulator.HandleProvider {
  ...
  public constructor(protected _clipView: ScreenViewport, protected _clipEventHandler?: ViewClipEventHandler) {
    ...
    if (undefined !== this._clipEventHandler && this._clipEventHandler.selectOnCreate()) this.iModel.selectionSet.replace(this._clipId);
  }
  ...
  private ensureNumControls(numReqControls: number): void {
    ...
    if (numCurrent < numReqControls) {
      ...
      for (let i: number = numCurrent; i < numReqControls; i++) this._controlIds[i] = transientIds.next;
  ...
  public override onManipulatorEvent(eventType: EditManipulator.EventType): void {
    ...
    if (EditManipulator.EventType.Accept === eventType && undefined !== this._clipEventHandler) this._clipEventHandler.onModifyClip(this._clipView);
  }
  ...
  public async getDecorationToolTip(hit: HitDetail): Promise<HTMLElement | string> {
    return hit.sourceId === this._clipId ? "View Clip" : "Modify View Clip";
  }
          ...
          if (geom instanceof Loop && geom.children.length > 0) {
            ...
            if (child instanceof LineString3d) {
              ...
              if (finalPoints.length > 0) {
                ...
                if (undefined !== loopArea) {
                  if (undefined === this._controls[iFace].floatingOrigin) this._controls[iFace].floatingOrigin = this._controls[iFace].origin.clone();
...
export class ViewClipDecorationProvider implements ViewClipEventHandler {
  ...
  public onRightClick(hit: HitDetail, ev: BeButtonEvent): boolean {
    ...
    if (0 === this.onActiveClipRightClick.numberOfListeners) return decoration.doClipPlaneNegate(decoration.getControlIndex(hit.sourceId));
