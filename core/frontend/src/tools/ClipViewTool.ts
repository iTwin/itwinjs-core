...
export class ViewClipByElementTool extends ViewClipTool {
  ...
  protected async doClipToElements(viewport: Viewport, ids: Id64Arg, alwaysUseRange: boolean = false, modelId?: Id64String): Promise<boolean> {
    try {
      ...
      const displayTransform = modelId && 1 === placements.length
        ? viewport.view.computeDisplayTransform({ modelId, elementId: placements[0].elementId })
        : undefined;
...
export class ViewClipDecoration extends EditManipulator.HandleProvider {
  ...
  private getWorldUpPlane(viewport: Viewport): Plane3dByOriginAndUnitNormal | undefined {
    ...
    const planePt = viewport.isContextRotationRequired
      ...
      : (viewport.view.isSpatialView() ? viewport.view.iModel.globalOrigin : Point3d.createZero());
    ...
    for (let iFace = 0; iFace < this._controlIds.length; iFace++) {
      ...
      if (1 === this._controls.length && undefined !== this._clipPlanes && undefined !== this._clipPlanesLoops && this._clipPlanesLoops.length > 0) {
        if (!vp.isPointVisibleXY(this._controls[iFace].origin, CoordSystem.World, 0.05)) {
          ...
          undefined !== this._controls[iFace].floatingOrigin && vp.isPointVisibleXY(this._controls[iFace].floatingOrigin!, CoordSystem.World, 0.1)
        ) {
