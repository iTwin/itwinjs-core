...
export class ViewClipTool extends PrimitiveTool {
  ...
  /** @internal */
  public static getPlaneInwardNormal(
    orientation: ContextRotationId,
    viewport: Viewport
  ): Vector3d | undefined {
    const matrix = AccuDrawHintBuilder.getContextRotation(
      orientation,
      viewport
    );
  ...
  public static doClipToConvexClipPlaneSet(
    viewport: Viewport,
    ...
    if (!clearExistingPlanes) {
      ...
      if (undefined !== existingClip && 1 === existingClip.clips.length) {
        ...
        if (!(existingPrim instanceof ClipShape)) {
          ...
          if (
            undefined !== existingPlaneSets &&
            1 === existingPlaneSets.convexSets.length
          )
  ...
  /** @internal */
  public static getClipRayTransformed(
    origin: Point3d,
    direction: Vector3d,
    transform?: Transform
  ): Ray3d {
  ...
  /** @internal */
  public static getOffsetValueTransformed(
    offset: number,
    transform?: Transform
  ) {
  ...
  /** @internal */
  public static addClipPlanesLoops(
  ...
  public static drawClip(
    ...
    const viewRange = viewExtents
      ? viewExtents
      : context.viewport.computeViewRange();
    ...
    if (
      undefined === clipShape &&
      (undefined === clipPlanesLoops || 0 === clipPlanesLoops.length)
    )
      return;
    ...
    builderVis.setSymbology(
      color,
      ColorDef.black,
      options && options.visibleWidth ? options.visibleWidth : 3
    );
    ...
    if (undefined !== clipPlanesLoops) {
      ...
      if (options && options.fillClipPlanes) {
        const fill = options.fill
          ? options.fill
          : EditManipulator.HandleUtils.adjustForBackgroundColor(
              ColorDef.from(0, 255, 255, 225),
              context.viewport
            );
        const builderFill = context.createGraphicBuilder(
          GraphicType.WorldDecoration
        );
      ...
      const clipExtents = ViewClipTool.getClipShapeExtents(
        clipShape,
        viewRange
      );
  ...
  private static isHilited(vp: Viewport, id?: string): boolean {
    return undefined !== id
      ? vp.iModel.hilited.elements.has(
  ...
  public static drawClipShape(
    ...
    const builder = context.createGraphicBuilder(
      GraphicType.WorldDecoration,
      shape.transformFromClip,
      id
    ); // Use WorldDecoration not WorldOverlay to make sure handles have priority...
    ...
    if (!this.isHilited(context.viewport, id)) {
      ...
      builderHid.setSymbology(
        this.isFlashed(context.viewport, id)
  ...
  /** @internal */
  public static getClipShapeExtents(
    shape: ClipShape,
    viewRange: Range3d
  ): Range1d {
    let zLow = shape.zLow;
    ...
    if (undefined === zLow || undefined === zHigh) {
      ...
      for (const corner of corners) {
        ...
        if (
          undefined === shape.zLow &&
          (undefined === zLow || projection < zLow)
        )
          zLow = projection;
        if (
          undefined === shape.zHigh &&
          (undefined === zHigh || projection > zHigh)
        )
          zHigh = projection;
      }
    }
  ...
  public static drawClipPlanesLoops(
    ...
    const builderEdge = context.createGraphicBuilder(
      GraphicType.WorldDecoration,
      undefined,
      id
    ); // Use WorldDecoration not WorldOverlay to make sure handles have priority...
    builderEdge.setSymbology(
      color,
      ColorDef.black,
      weight,
      dashed ? LinePixels.Code2 : undefined
    );
    ...
    // If the clip decoration is being flashed, draw using the hilite color to match the pickable world decoration display.
    if (!this.isHilited(context.viewport, id)) {
      const builderEdgeHid = context.createGraphicBuilder(
        GraphicType.WorldOverlay
      );
    ...
    const builderFace = context.createGraphicBuilder(
      GraphicType.WorldDecoration,
      undefined
    );
  ...
  /** @internal */
  public static isSingleConvexClipPlaneSet(
    clip: ClipVector
    ...
    return undefined !== planeSets && 1 === planeSets.convexSets.length
    ...
    for (let iPrim = 0; iPrim < clipA.clips.length; iPrim++) {
      ...
      if (undefined !== planesA && undefined !== planesB) {
        ...
        for (let iPlane = 0; iPlane < planesA.convexSets.length; iPlane++) {
          ...
          for (
            let iClipPlane = 0;
            iClipPlane < planeSetA.planes.length;
            iClipPlane++
          ) {
...
export class ViewClipClearTool extends ViewClipTool {
  ...
  protected override showPrompt(): void {
    ...
    IModelApp.notifications.setToolAssistance(
      ToolAssistance.createInstructions(mainInstruction)
    );
  }
  ...
  public override async onDataButtonDown(
    ...
    return (await this.doClipClear(this.targetView))
      ? EventHandled.Yes
      : EventHandled.No;
  }
}
...
export class ViewClipByPlaneTool extends ViewClipTool {
  ...
  constructor(
    clipEventHandler?: ViewClipEventHandler,
    protected _clearExistingPlanes: boolean = false
  ) {
  ...
  public override async onDataButtonDown(
    ...
    if (undefined === this.targetView) return EventHandled.No;
    const normal = ViewClipTool.getPlaneInwardNormal(
      this.orientation,
      this.targetView
    );
    ...
    if (
      !ViewClipTool.doClipToPlane(
        this.targetView,
        ev.point,
        normal,
        this._clearExistingPlanes
      )
    )
      return EventHandled.No;
    if (undefined !== this._clipEventHandler)
...
export class ViewClipByShapeTool extends ViewClipTool {
  ...
  public override isValidLocation(
    ...
    return this._points.length > 0
      ? true
      : super.isValidLocation(ev, isButtonEvent);
  }
  ...
  public override decorate(context: DecorateContext): void {
    ...
    const colorAccVis = EditManipulator.HandleUtils.adjustForBackgroundColor(
      ColorDef.white,
      context.viewport
    );
...
export class ViewClipByRangeTool extends ViewClipTool {
  ...
  protected override showPrompt(): void {
    const mainInstruction = ToolAssistance.createInstruction(
      this.iconSpec,
      CoreTools.translate(
        undefined === this._corner
          ? "ViewClip.ByRange.Prompts.FirstPoint"
          : "ViewClip.ByRange.Prompts.NextPoint"
      )
    );
    const mouseInstructions: ToolAssistanceInstruction[] = [];
    const touchInstructions: ToolAssistanceInstruction[] = [];
...
export class ViewClipByElementTool extends ViewClipTool {
  ...
  constructor(
    clipEventHandler?: ViewClipEventHandler,
    protected _alwaysUseRange: boolean = false
  ) {
    super(clipEventHandler);
  }
  ...
  protected async doClipToElements(
    ...
    try {
      ...
      if (!alwaysUseRange && 1 === placements.length) {
        ...
        range.setFrom(
          placement instanceof Placement2d
            ? Range3d.createRange2d(placement.bbox, 0)
            : placement.bbox
        );
      ...
      if (range.isAlmostZeroX || range.isAlmostZeroY) {
        ...
        transform.setFrom(
          Transform.createOriginAndMatrix(
            points[0],
            Matrix3d.createRigidHeadsUp(zDir)
          )
        );
        transform.multiplyInversePoint3dArrayInPlace(points);
...
export abstract class ViewClipModifyTool extends EditManipulator.HandleTool {
  ...
  protected drawAnchorOffset(
    ...
    if (
      -1 === this._anchorIndex ||
      Math.abs(this._currentDistance) < Geometry.smallMetricDistance
    )
      return;
    ...
    anchorRay.direction.scaleToLength(
...
export class ViewClipShapeModifyTool extends ViewClipModifyTool {
  protected updateViewClip(ev: BeButtonEvent, _isAccept: boolean): boolean {
    ...
    const localOffset = ViewClipTool.getOffsetValueTransformed(
      offset,
      clipShape.transformToClip
    );
    ...
    for (let i = 0; i < shapePts.length; i++) {
      ...
      const prevSelected =
        offsetAll ||
        prevFace === this._anchorIndex ||
        this.manipulator.iModel.selectionSet.has(this._ids[prevFace]);
      const nextSelected =
        offsetAll ||
        nextFace === this._anchorIndex ||
        this.manipulator.iModel.selectionSet.has(this._ids[nextFace]);
      if (prevSelected && nextSelected) {
        const prevPt = shapePts[i].plusScaled(
          this._controls[prevFace].direction,
          localOffset
        );
        const nextPt = shapePts[i].plusScaled(
          this._controls[nextFace].direction,
          localOffset
        );
        const prevRay = Ray3d.create(
          prevPt,
          Vector3d.createStartEnd(
            shapePts[i === 0 ? shapePts.length - 2 : i - 1],
            shapePts[i]
          )
        );
        const nextPlane = Plane3dByOriginAndUnitNormal.create(
          nextPt,
          this._controls[nextFace].direction
        );
        if (
          undefined === nextPlane ||
          undefined === prevRay.intersectionWithPlane(nextPlane, prevPt)
        )
          return false;
        adjustedPts[i] = prevPt;
      } else if (prevSelected) {
        adjustedPts[i] = shapePts[i].plusScaled(
          this._controls[prevFace].direction,
          localOffset
        );
    ...
    const zLowSelected =
      offsetAll ||
      zLowIndex === this._anchorIndex ||
      this.manipulator.iModel.selectionSet.has(this._ids[zLowIndex]);
    const zHighSelected =
      offsetAll ||
      zHighIndex === this._anchorIndex ||
      this.manipulator.iModel.selectionSet.has(this._ids[zHighIndex]);
    ...
    return ViewClipTool.doClipToShape(
      this._clipView,
      adjustedPts,
      clipShape.transformFromClip,
      zLow,
      zHigh
    );
  }
  ...
  protected updateViewClip(ev: BeButtonEvent, _isAccept: boolean): boolean {
    ...
    for (let i: number = 0; i < this._controls.length; i++) {
      const selected =
        offsetAll ||
        i === this._anchorIndex ||
        this.manipulator.iModel.selectionSet.has(this._ids[i]);
      ...
      planeSet.addPlaneToConvexSet(
        ClipPlane.createNormalAndPoint(direction.negate(), origin)
      );
    }
...
export class ViewClipDecoration extends EditManipulator.HandleProvider {
  ...
  public constructor(
    ...
    protected _clipEventHandler?: ViewClipEventHandler
  ) {
    ...
    this._removeViewCloseListener =
      IModelApp.viewManager.onViewClose.addListener(this.onViewClose, this); // eslint-disable-line @typescript-eslint/unbound-method
    if (
      undefined !== this._clipEventHandler &&
      this._clipEventHandler.selectOnCreate()
    )
  ...
  private getClipData(): boolean {
    ...
    if (undefined !== clipShape) {
      if (clipShape.polygon.length > 5) {
        const compressed = PolylineOps.compressByChordError(
          clipShape.polygon,
          1.0e-5
        );
      ...
      this._clipShapeExtents = ViewClipTool.getClipShapeExtents(
        clipShape,
      ...
      if (
        undefined !== clipPlanesLoops &&
        clipPlanesLoops.length > clipPlanes.planes.length
      )
        return false;
  ...
  private createClipShapeControls(): boolean {
    ...
    for (let i: number = 0; i < numControls - 2; i++) {
      ...
      this._controls[i] = new ViewClipControlArrow(
        faceCenter,
        faceNormal,
        shapePtsLo.length > 5 ? 0.5 : 0.75
      );
    }
    ...
    for (let i: number = 0; i < this._clipPlanes.planes.length; i++) {
      ...
      if (undefined !== nonContribLoops && nonContribLoops.length > 0) {
        if (undefined === this._clipPlanesLoopsNoncontributing)
          ...
          this._clipPlanesLoopsNoncontributing =
            this._clipPlanesLoopsNoncontributing.concat(nonContribLoops);
      ...
      this._controls[i] = new ViewClipControlArrow(
        ...
        0.5,
        nonContribColor
      ); // Just show arrow for right-click menu options...
    }
  ...
  protected async createControls(): Promise<boolean> {
    ...
    if (
      this.iModel.selectionSet.size <= this._controlIds.length + 1 &&
      this.iModel.selectionSet.has(this._clipId)
    ) {
      showControls = true;
    ...
    if (!showControls) {
      if (
        undefined !== this._clipEventHandler &&
        this._clipEventHandler.clearOnDeselect()
      )
  ...
  public doClipPlaneNegate(index: number): boolean {
    ...
    for (let i: number = 0; i < this._clipPlanes.planes.length; i++) {
      const plane =
        i === index
          ? this._clipPlanes.planes[i].cloneNegated()
          : this._clipPlanes.planes[i];
  ...
  private getLoopPreferredX(
    ...
    if (dirZ.isParallelTo(unitZ, true)) {
      ...
      vectorX =
        Math.abs(dirX.dotProduct(unitX)) > Math.abs(dirY.dotProduct(unitX))
          ? dirX
          : dirY;
      ...
      if (crossX && crossY) {
        if (
          Math.abs(crossY.dotProduct(unitZ)) >
          Math.abs(crossX.dotProduct(unitZ))
        ) {
          vectorX = dirX;
          vectorY = crossY;
        } else {
  ...
  public doClipPlaneOrientView(index: number): boolean {
    ...
    const loops =
      undefined !== this._clipPlanesLoops
        ? this._clipPlanesLoops
        : this.getClipShapeFaceLoops();
    const loop = loops
      ? 1 === loops.length
        ? loops[0]
        : this.getMatchingLoop(loops, anchorRay)
      : undefined;
    const vectorX = loop
      ? this.getLoopPreferredX(loop, anchorRay.direction)
      : undefined;
  ...
  private isAlignedToWorldUpPlane(
    ...
    const normal =
      undefined !== transformFromClip
        ? transformFromClip.multiplyVector(Vector3d.unitZ())
        : Vector3d.unitZ();
  ...
  public isClipShapeAlignedWithWorldUp(extents?: Range1d): boolean {
    ...
    if (
      undefined === plane ||
      !this.isAlignedToWorldUpPlane(plane, this._clipShape.transformFromClip)
    )
      return false;
    ...
    const lowDir = Vector3d.createStartEnd(
      plane.projectPointToPlane(zLow),
      zLow
    );
    const highDir = Vector3d.createStartEnd(
      plane.projectPointToPlane(zHigh),
      zHigh
    );
  ...
  public doClipShapeSetZExtents(extents: Range1d): boolean {
    ...
    const zLow = plane
      .getOriginRef()
      .plusScaled(plane.getNormalRef(), extents.low);
    const zHigh = plane
      .getOriginRef()
      .plusScaled(plane.getNormalRef(), extents.high);
  ...
  protected override async onRightClick(
    ...
    return this._clipEventHandler.onRightClick(hit, ev)
      ? EventHandled.Yes
      : EventHandled.No;
  }
  ...
  public override onManipulatorEvent(
    ...
    if (
      EditManipulator.EventType.Accept === eventType &&
      undefined !== this._clipEventHandler
    )
  ...
  public override decorate(context: DecorateContext): void {
    ...
    const outlineColor = EditManipulator.HandleUtils.adjustForBackgroundColor(
      ColorDef.from(0, 0, 0, 50),
      vp
    );
    const fillVisColor = EditManipulator.HandleUtils.adjustForBackgroundColor(
      ColorDef.from(150, 250, 200, 175),
      vp
    );
    const fillHidColor = fillVisColor.withAlpha(225);
    const fillSelColor = fillVisColor.inverse().withAlpha(75);
    const shapePts = EditManipulator.HandleUtils.getArrowShape(
      0.0,
      0.15,
      0.55,
      1.0,
    ...
    for (let iFace = 0; iFace < this._controlIds.length; iFace++) {
      ...
      if (
        ...
        if (
          ...
          if (geom instanceof Loop && geom.children.length > 0) {
            ...
            if (child instanceof LineString3d) {
              ...
              if (finalPoints.length > 0) {
                const loopArea = PolygonOps.centroidAreaNormal(
                  finalPoints.getPoint3dArray()
                );
                if (undefined !== loopArea) {
                  if (undefined === this._controls[iFace].floatingOrigin)
                    this._controls[iFace].floatingOrigin =
                      this._controls[iFace].origin.clone();
          ...
          this._controls[iFace].origin.setFrom(
            this._controls[iFace].floatingOrigin
          );
      ...
      if (undefined !== outlineColorOvr) {
        outlineColorOvr = EditManipulator.HandleUtils.adjustForBackgroundColor(
          outlineColorOvr,
          vp
        );
      ...
      if (undefined !== fillVisColorOvr) {
        fillVisColorOvr = EditManipulator.HandleUtils.adjustForBackgroundColor(
          fillVisColorOvr,
          vp
        );
        ...
        fillSelColorOvr = fillVisColorOvr
          .inverse()
          .withAlpha(fillSelColor.getAlpha());
      } else {
      ...
      arrowVisBuilder.setBlankingFill(
        isSelected ? fillSelColorOvr : fillVisColorOvr
      );
  ...
  public static get(vp: ScreenViewport): ViewClipDecoration | undefined {
    if (
      undefined === ViewClipDecoration._decorator ||
      vp !== ViewClipDecoration._decorator._clipView
    )
      return undefined;
  ...
  public static create(
    ...
    ViewClipDecoration._decorator = new ViewClipDecoration(
      vp,
      clipEventHandler
    );
...
export class ViewClipDecorationProvider implements ViewClipEventHandler {
  ...
  public readonly onActiveClipChanged = new BeEvent<
    (
      ...
      provider: ViewClipDecorationProvider
    ) => void
  >();
  ...
  public onRightClick(hit: HitDetail, ev: BeButtonEvent): boolean {
    const decoration =
      undefined !== ev.viewport
        ? ViewClipDecoration.get(ev.viewport)
        : undefined;
    if (undefined === decoration) return false;
    if (0 === this.onActiveClipRightClick.numberOfListeners)
      return decoration.doClipPlaneNegate(
        decoration.getControlIndex(hit.sourceId)
      );
