...
export class ToolAssistance {
  ...
  private static translateKey(key: string) { return IModelApp.localizationProvider.getLocalizedString(`${CoreTools.namespace}:toolAssistance.${key}`); }
  private static translateTouch(cursor: string) { return IModelApp.localizationProvider.getLocalizedString(`${CoreTools.namespace}:touchCursor.${cursor}`); }
