...
export class ToolAssistance {
  ...
  private static translateKey(key: string) { return IModelApp.localizationClient.getLocalizedString(`${CoreTools.namespace}:toolAssistance.${key}`); }
  private static translateTouch(cursor: string) { return IModelApp.localizationClient.getLocalizedString(`${CoreTools.namespace}:touchCursor.${cursor}`); }
