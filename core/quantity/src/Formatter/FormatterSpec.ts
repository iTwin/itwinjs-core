...
export class FormatterSpec {
   ...
   *  @note For Bearing/Azimuth formats, `format.azimuthBaseUnit`/`format.revolutionUnit` must share the same phenomenon as `inputUnit`
   *  (e.g. use a `Units.HORIZONTAL_DIRECTION` revolution unit like `Units.HORIZONTAL_DIR_REVOLUTION` when `inputUnit` is `Units.HORIZONTAL_DIRECTION`).
