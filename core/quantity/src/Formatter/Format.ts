...
export class BaseFormat {
  ...
  public parseFormatTraits(formatTraitsFromJson: string | string[]) {
    const formatTraits = Array.isArray(formatTraitsFromJson)
      ? formatTraitsFromJson
      : formatTraitsFromJson.split(/,|;|\|/);
    formatTraits.forEach((formatTraitsString: string) => {
  ...
  public loadFormatProperties(formatProps: FormatProps) {
    ...
    if (this.type === FormatType.Scientific) {
      if (undefined === formatProps.scientificType)
        // if format type is scientific and scientific type is undefined, throw
        throw new QuantityError(
          QuantityStatus.InvalidJson,
    ...
    if (undefined !== formatProps.roundFactor) {
      ...
      if (formatProps.roundFactor !== this.roundFactor)
        // if roundFactor isn't default value of 0.0, reassign roundFactor variable
    ...
    if (undefined !== formatProps.decimalSeparator) {
      ...
      if (typeof formatProps.decimalSeparator !== "string")
        // not a string or not a one character string
        throw new QuantityError(
          QuantityStatus.InvalidJson,
...
export class Format extends BaseFormat {
  ...
  private async createUnit(
    ...
    if (
      ...
      throw new QuantityError(
        QuantityStatus.InvalidJson,
        `This Composite has a unit with an invalid 'name' or 'label' attribute.`
      );
  ...
  public clone(options?: CloneOptions): Format {
    ...
    if (newFormat._units) {
      if (options?.showOnlyPrimaryUnit) {
        if (newFormat._units.length > 1) newFormat._units.length = 1;
      }
    }
    ...
    if (undefined !== jsonObj.composite) {
      ...
      if (jsonObj.composite.includeZero !== undefined) {
        if (typeof jsonObj.composite.includeZero !== "boolean")
          // includeZero must be a boolean IF it is defined
          throw new QuantityError(
            QuantityStatus.InvalidJson,
      ...
      if (jsonObj.composite.spacer !== undefined) {
        // spacer must be a string IF it is defined
      ...
      if (jsonObj.composite.units !== undefined) {
        // if composite is defined, it must be an array with 1-4 units
      ...
      if (undefined === this.units || this.units.length === 0)
        throw new QuantityError(
          QuantityStatus.InvalidJson,
          `The Format ${this.name} has a Composite with no valid 'units'`
        );
    }
  }
