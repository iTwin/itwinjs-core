...
describe("Synchronous Parsing tests:", async () => {
  ...
  it("Parse into length values using custom parse labels", () => {
    ...
    for (const testEntry of testData) {
      ...
      if (logTestOutput) {
        if (Parser.isParsedQuantity(parseResult)) console.log(`input=${testEntry.value} output=${parseResult.value}`);
        // eslint-disable-line no-console
        else if (Parser.isParseError(parseResult)) console.log(`input=${testEntry.value} error=${parseResult.error}`); // eslint-disable-line no-console
      }
      assert.isTrue(Parser.isParsedQuantity(parseResult));
      if (Parser.isParsedQuantity(parseResult)) expect(parseResult.value).closeTo(testEntry.magnitude, 0.0001);
    }
  });
