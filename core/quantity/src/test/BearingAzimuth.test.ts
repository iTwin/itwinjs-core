...
describe("Bearing format tests:", () => {
  it("Roundtrip persisted radian to and from bearing", async () => {
    ...
    for (const entry of testData) {
      ...
      expect(parseBearingDMSWithLabelResult.value, `Parsed result for ${normalizedAngle} from formatted ${resultBearingDMSWithLabel}`).closeTo(
        normalizedAngle,
        0.0001,
      );
      ...
      expect(parseBearingDecimalResult.value, `Parsed result for ${normalizedAngle} from formatted ${resultBearingDecimal}`).closeTo(
        normalizedAngle,
        0.0001,
      );
    }
...
describe("Azimuth format tests:", () => {
  ...
  it("^Roundtrip degrees with various bases", async () => {
    ...
    for (const entry of testData) {
      ...
      expect(parseResult.value, `Parsed result for ${entry.input} with base ${entry.base} ccw: ${entry.counterClockwise}`).closeTo(
        entry.input,
        0.0001,
      );
    }
...
describe("Azimuth and Revolution formatting that throws error:", () => {
  ...
  it("should throw an error if revolutionUnit is required but not provided", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        e.message,
        "The Format testAzimuthFormat is 'Azimuth' or 'Bearing' type therefore the attribute 'revolutionUnit' is required.",
      );
      assert.instanceOf(e, QuantityError);
    }
  });
  ...
  it("should throw an error if _azimuthBase is defined and _azimuthBaseUnit is undefined when revolutionUnit is defined", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        e.message,
        "The Format testAzimuthFormat has an 'azimuthBase' attribute therefore the attribute 'azimuthBaseUnit' is required.",
      );
      assert.instanceOf(e, QuantityError);
    }
  });
