...
export class Parser {
  ...
  private static checkForScientificNotation(
    ...
    for (; i < stringToParse.length; i++) {
      ...
      if (
        ...
        ((charCode === QuantityConstants.CHAR_MINUS || charCode === QuantityConstants.CHAR_PLUS) && i === index + 1)
      ) {
  ...
  private static async lookupUnitByLabel(
    ...
    if (format.units && format.units.length > 0) {
      const formatUnit = format.units.find(([unit, label]) => {
        ...
        if (alternateLabels && alternateLabels.find((lbl) => lbl.toLowerCase() === labelToFind)) return true;
      ...
      for (let i = 0; i < tokens.length; i = i + 2) {
        if (tokens[i].isNumber && tokens[i + 1].isString) {
          ...
          if (0 === i) {
            ...
            if (defaultUnit) {
              const conversion = await unitsProvider.getConversion(unit, defaultUnit);
              if (mag < 0.0) mag = mag - value * conversion.factor + conversion.offset;
      ...
      if (preferredUnit) {
        ...
        for (const conversion of preferredSystemConversions) {
          if (conversion.parseLabels) {
            if (-1 !== conversion.parseLabels.findIndex((lbl) => lbl === label)) return conversion.conversion;
          }
        }
      }
  ...
  private static getQuantityValueFromParseTokens(
    ...
    if (tokens.length === 1) {
      if (tokens[0].isNumber) {
        ...
        if (undefined !== conversion) return { ok: true, value: conversion.factor + conversion.offset };
  ...
  public static parseToQuantityValue(
    ...
    const tokens: ParseToken[] = Parser.parseQuantitySpecification(inString, format);
    if (tokens.length === 0) return { ok: false, error: ParseError.UnableToGenerateParseTokens };
