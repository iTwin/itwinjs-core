...
export class Parser {
  ...
  public static isParsedQuantity(
    item: QuantityParseResult
  ): item is ParsedQuantity {
    return item.ok;
  }

  public static isParseError(
    item: QuantityParseResult
  ): item is ParseQuantityError {
    return !item.ok;
  }
  ...
  private static checkForScientificNotation(
    ...
    for (; i < stringToParse.length; i++) {
      ...
      if (
        Parser.isDigit(charCode) ||
        ((charCode === QuantityConstants.CHAR_MINUS ||
          charCode === QuantityConstants.CHAR_PLUS) &&
          i === index + 1)
      ) {
    ...
    if (
      exponentString.length > 1 ||
      (exponentString.length === 1 &&
        exponentString.charCodeAt(0) !== QuantityConstants.CHAR_MINUS &&
        exponentString.charCodeAt(0) !== QuantityConstants.CHAR_PLUS)
    )
  ...
  private static checkForFractions(
    ...
    for (; i < stringToParse.length; i++) {
      ...
      if (Parser.isDigit(charCode)) {
        ...
        if (
          processingNumerator &&
          (charCode === QuantityConstants.CHAR_SLASH ||
            charCode === QuantityConstants.CHAR_DIVISION_SLASH ||
            charCode === QuantityConstants.CHAR_DIVISION_SLASH)
        ) {
          processingNumerator = false;
        } else {
    ...
    if (numeratorToken.length > 0 && denominatorToken.length > 0) {
      ...
      if (denominator > 0) return new FractionToken(i, numerator / denominator);
  ...
  private static isDigit(charCode: number): boolean {
    return (
      charCode >= QuantityConstants.CHAR_DIGIT_ZERO &&
      charCode <= QuantityConstants.CHAR_DIGIT_NINE
    );
  }
  ...
  private static isDigitOrDecimalSeparator(
    ...
    return (
      charCode === format.decimalSeparator.charCodeAt(0) ||
      Parser.isDigit(charCode)
    );
  }
   ...
   */
  public static parseQuantitySpecification(
    ...
    if (
      format.type === FormatType.Station &&
      format.stationSeparator &&
      format.stationSeparator.length === 1
    )
      skipCodes.push(format.stationSeparator.charCodeAt(0));

    if (
      format.type === FormatType.Fractional &&
      format.hasFormatTraitSet(FormatTraits.FractionDash)
    ) {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      if (Parser.isDigitOrDecimalSeparator(charCode, format)) {
        ...
        if (processingNumber) {
          if (
            charCode === QuantityConstants.CHAR_SLASH ||
            charCode === QuantityConstants.CHAR_FRACTION_SLASH ||
            charCode === QuantityConstants.CHAR_DIVISION_SLASH
          ) {
            const fractSymbol = Parser.checkForFractions(
              i + 1,
              str,
              uomSeparatorToIgnore,
              wipToken
            );
            ...
            if (
              charCode === QuantityConstants.CHAR_SPACE ||
              charCode === fractionDashCode
            ) {
              const fractSymbol = Parser.checkForFractions(
                i + 1,
                str,
              ...
              if (
                charCode === QuantityConstants.CHAR_UPPER_E ||
                charCode === QuantityConstants.CHAR_LOWER_E
              ) {
                const exponentSymbol = Parser.checkForScientificNotation(
                  i,
                  str,
                  uomSeparatorToIgnore
                );
                ...
                if (
                  exponentSymbol.exponent &&
                  exponentSymbol.exponent.length > 0
                ) {
                  if (signToken.length > 0) {
                    wipToken = signToken + wipToken;
                    signToken = "";
          ...
          if (
            ...
            if (0 === tokens.length)
              // sign token only needed for left most value
              signToken = str[i];
            continue;
          }
  ...
  private static async lookupUnitByLabel(
    unitLabel: string,
    format: Format,
    ...
    const defaultUnit =
      format.units && format.units.length > 0 ? format.units[0][0] : undefined;
    ...
    if (format.units && format.units.length > 0) {
      const formatUnit = format.units.find(([unit, label]) => {
        if (label && label.toLowerCase() === labelToFind) return true;
        const alternateLabels =
          altUnitLabelsProvider?.getAlternateUnitLabels(unit);
        // check any alternate labels that may be defined for the Unit
        if (
          alternateLabels &&
          alternateLabels.find((lbl) => lbl.toLowerCase() === labelToFind)
        )
          return true;

        return false;
      });
      if (formatUnit) return formatUnit[0];
    }

    // now try to find a unit from the same family and system
    let foundUnit = await unitsProvider.findUnit(
      unitLabel,
      defaultUnit ? defaultUnit.phenomenon : undefined,
      defaultUnit ? defaultUnit.system : undefined
    );

    // if nothing found yet just limit to family
    if (!foundUnit.isValid && defaultUnit)
      foundUnit = await unitsProvider.findUnit(
        unitLabel,
        defaultUnit ? defaultUnit.phenomenon : undefined
      );
    return foundUnit;
  ...
  private static async createQuantityFromParseTokens(
    tokens: ParseToken[],
    format: Format,
    unitsProvider: UnitsProvider,
    altUnitLabelsProvider?: AlternateUnitLabelsProvider
  ): Promise<QuantityProps> {
    let defaultUnit =
      format.units && format.units.length > 0 ? format.units[0][0] : undefined;
    ...
    if (tokens.length === 1) {
      if (tokens[0].isNumber) {
        ...
        const unit = await this.lookupUnitByLabel(
          tokens[0].value as string,
          format,
          unitsProvider,
    ...
    if (tokens.length === 2) {
      if (tokens[0].isNumber && tokens[1].isString) {
        ...
        if (undefined === defaultUnit) defaultUnit = unit;
        if (defaultUnit && defaultUnit.name === unit.name) {
          return new Quantity(defaultUnit, tokens[0].value as number);
        } else if (defaultUnit) {
          const conversion = await unitsProvider.getConversion(
            unit,
            defaultUnit
          );
          const mag =
            (tokens[0].value as number) * conversion.factor + conversion.offset;
        ...
        if (tokens[1].isNumber && tokens[0].isString) {
          ...
          if (defaultUnit && defaultUnit.name === unit.name) {
            ...
            const mag =
              (tokens[1].value as number) * conversion.factor +
              conversion.offset;
            return new Quantity(defaultUnit, mag);
          }
        }
    ...
    if (tokens.length % 2 === 0) {
      ...
      for (let i = 0; i < tokens.length; i = i + 2) {
        if (tokens[i].isNumber && tokens[i + 1].isString) {
          ...
          if (undefined === defaultUnit) defaultUnit = unit;
          if (0 === i) {
            if (defaultUnit.name === unit.name) mag = value;
            else {
              const conversion = await unitsProvider.getConversion(
                unit,
                defaultUnit
            ...
            if (defaultUnit) {
              ...
              else mag = mag + value * conversion.factor + conversion.offset;
            }
          }
        }
      }
   ...
   */
  public static async parseIntoQuantity(
    inString: string,
    format: Format,
    unitsProvider: UnitsProvider,
    altUnitLabelsProvider?: AlternateUnitLabelsProvider
  ): Promise<QuantityProps> {
    const tokens: ParseToken[] = Parser.parseQuantitySpecification(
      inString,
      format
    );
    if (tokens.length === 0) return new Quantity();

    return Parser.createQuantityFromParseTokens(
      tokens,
      format,
      unitsProvider,
      altUnitLabelsProvider
  ...
  private static tryFindUnitConversion(
    unitLabel: string,
    unitsConversions: UnitConversionSpec[],
    preferredUnit?: UnitProps
  ): UnitConversionProps | undefined {
    if (unitsConversions.length > 0) {
      ...
      if (preferredUnit) {
        ...
        const preferredConversion = unitsConversions.find(
          (conversion) => conversion.name === preferredUnit.name
        );
        if (preferredConversion && preferredConversion.parseLabels) {
          if (
            -1 !==
            preferredConversion.parseLabels.findIndex((lbl) => lbl === label)
          )
        ...
        const preferredSystemConversions = unitsConversions.filter(
          (conversion) => conversion.system === preferredUnit.system
        );
      ...
      for (const conversion of unitsConversions) {
        if (conversion.parseLabels) {
          ...
          console.log(
            "ERROR: Parser expects to find parseLabels array populate with all possible unit labels for the unit."
          );
        }
      }
    }
  ...
  private static getQuantityValueFromParseTokens(
    ...
    if (tokens.length === 1) {
      if (tokens[0].isNumber) {
        if (defaultUnit) {
          const conversion = Parser.tryFindUnitConversion(
            defaultUnit.label,
            unitsConversions,
            defaultUnit
          );
          if (conversion) {
            const value =
              (tokens[0].value as number) * conversion.factor +
              conversion.offset;
        ...
        else return { ok: false, error: ParseError.NoValueOrUnitFoundInString };
      }
    }
    ...
    return {
      ok: false,
      error: ParseError.UnableToConvertParseTokensToQuantity,
    };
  }
   ...
   */
  public static parseQuantityString(
    inString: string,
    parserSpec: ParserSpec
  ): QuantityParseResult {
    return Parser.parseToQuantityValue(
      inString,
      parserSpec.format,
   ...
   */
  public static parseToQuantityValue(
    inString: string,
    format: Format,
    unitsConversions: UnitConversionSpec[]
  ): QuantityParseResult {
    ...
    if (format.units) {
      format.units.forEach(([unit, label]) => {
        if (label) {
          if (unit.label !== label) {
            // if default unit label does not match composite label ensure the label is in the list of parse labels for the conversion
            const unitConversion = unitsConversions.find(
              (conversion) => conversion.name === unit.name
            );
            if (
              unitConversion &&
              unitConversion.parseLabels &&
              !unitConversion.parseLabels.find((entry) => entry === label)
            )
    ...
    if (Parser._log) {
      ...
      for (const token of tokens) {
        // eslint-disable-next-line no-console
        console.log(
          `  [${i++}] isNumber=${token.isNumber} isString=${
            token.isString
          } token=${token.value}`
        );
      }
    ...
    return Parser.getQuantityValueFromParseTokens(
      tokens,
      format,
      unitsConversions
    );
  ...
  public static async createUnitConversionSpecsForUnit(
    unitsProvider: UnitsProvider,
    outUnit: UnitProps,
    altUnitLabelsProvider?: AlternateUnitLabelsProvider
  ...
  public static async createUnitConversionSpecs(
    unitsProvider: UnitsProvider,
    outUnitName: string,
    potentialParseUnits: PotentialParseUnit[],
    altUnitLabelsProvider?: AlternateUnitLabelsProvider
  ): Promise<UnitConversionSpec[]> {
    const unitConversionSpecs: UnitConversionSpec[] = [];
    ...
    if (!outUnit || !outUnit.name || 0 === outUnit.name.length) {
      // eslint-disable-next-line no-console
      console.log(
        `[Parser.createUnitConversionSpecs] ERROR: Unable to locate out unit ${outUnitName}.`
      );
      return unitConversionSpecs;
    }

    for (const potentialParseUnit of potentialParseUnits) {
      const unit = await unitsProvider.findUnitByName(
        potentialParseUnit.unitName
      );
      if (!unit || !unit.name || 0 === unit.name.length) {
        // eslint-disable-next-line no-console
        console.log(
          `[Parser.createUnitConversionSpecs] ERROR: Unable to locate potential unit ${potentialParseUnit.unitName}.`
        );
        continue;
      }
