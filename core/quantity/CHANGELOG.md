...
- Add `FormatDefinition`, `FormatsProvider` and `MutableFormatsProvider` interfaces.
- Add limitations to quantity parsing logic
- minor fix in parseToken
- minor fixes in bearing parser
...
- deprecate FormatEnums toString() methods, convert int enums to string enums
- deprecated Parser.parseToQuantityValue()
- Ratio format type support and inverted units
- Change default azimuth/bearing base from east to north
...
- Bearing and Azimuth format support
...
- Add optional property `allowMathematicOperations` to `BaseFormat` to calculate mathematical operations like addition and subtraction.
...
- Add parsing support for different decimal and thousand separators
