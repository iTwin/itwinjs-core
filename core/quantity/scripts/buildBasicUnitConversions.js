...
function buildDefaultPersistenceUnitEntries(sourceSchema, assertUniqueGeneratedKeys) {
  ...
  const unitsByPhenomenon = new Map();
  ...
  for (const [name, item] of Object.entries(sourceSchema.items)) {
    ...
    let phenomenon;
    if (item.schemaItemType === "Unit") {
      phenomenon = qualifyItemName(item.phenomenon, sourceSchema.name);
    } else {
      const invertsUnitName = qualifyItemName(item.invertsUnit, sourceSchema.name);
      const invertedSource = resolvedUnits.get(stripAliasPrefix(item.invertsUnit)) ?? resolvedUnits.get(invertsUnitName.slice(`${sourceSchema.name}.`.length));
      if (!invertedSource)
    ...
    const units = unitsByPhenomenon.get(phenomenon) ?? [];
    units.push({
      name: qualifiedSchemaItemName(name),
      unitSystem: qualifyItemName(item.unitSystem, sourceSchema.name),
    });
    unitsByPhenomenon.set(phenomenon, units);
  }

  const entries = [];
  const unresolved = [];
  for (const [name, item] of Object.entries(sourceSchema.items).sort(([a], [b]) => a.localeCompare(b))) {
...
function buildGeneratedDefaultPersistenceModule(sourceSchema, assertUniqueGeneratedKeys) {
  const entries = buildDefaultPersistenceUnitEntries(sourceSchema, assertUniqueGeneratedKeys);
  const stripSchemaPrefix = (fullName) => fullName.slice(`${sourceSchema.name}.`.length);
  const lines = [
    "/*---------------------------------------------------------------------------------------------",
    "* Copyright (c) Bentley Systems, Incorporated. All rights reserved.",
    "* See LICENSE.md in the project root for license terms and full copyright notice.",
  ...
  for (const entry of entries) {
    const phenomenonName = stripSchemaPrefix(entry.key);
    const unitName = stripSchemaPrefix(entry.value);
    lines.push(`  [Phenomena.${phenomenonName}]: Units.${phenomenonName}.${unitName},`);
  }

  lines.push(
    "  // Phenomena.LENGTH_RATIO is intentionally omitted until the built-in default length-ratio unit is settled.",
...
module.exports = {
  ...
  buildDefaultPersistenceUnitEntries,
