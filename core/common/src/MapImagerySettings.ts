...
export class MapImagerySettings {
  ...
  private constructor(
    ...
    backgroundLayerProps?: MapLayerProps[],
    overlayLayersProps?: MapLayerProps[]
  ) {
    ...
    if (backgroundLayerProps) {
      for (const layerProps of backgroundLayerProps) {
        ...
        if (layer) this._backgroundLayers.push(layer);
      }
    }
    if (overlayLayersProps) {
      for (const overlayLayerProps of overlayLayersProps) {
        ...
        if (overlayLayer) this._overlayLayers.push(overlayLayer);
      }
    }
  }
  ...
  public get baseTransparency(): number {
    return this._backgroundBase instanceof ColorDef
      ? this._backgroundBase.getTransparency() / 255
      : this._backgroundBase.transparency;
  }
