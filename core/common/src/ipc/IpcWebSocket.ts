...
export namespace IpcWebSocketMessage {
  export function internal(): IpcWebSocketMessage {
    return {
      type: IpcWebSocketMessageType.Internal,
      channel: "",
  ...
  export function duplicate(): IpcWebSocketMessage {
    return {
      type: IpcWebSocketMessageType.Duplicate,
      channel: "",
      sequence: Number.MIN_SAFE_INTEGER,
    };
  }
...
export abstract class IpcWebSocket implements IpcSocket {
  ...
  public addListener(channel: string, listener: IpcListener): RemoveFunction {
    ...
    if (!listeners.has(listener)) listeners.add(listener);
  ...
  private async broadcast(evt: Event, message: IpcWebSocketMessage) {
    ...
    for (const handler of handlers) handler(evt, ...arg);
  }
}
  ...
  implements IpcSocketFrontend
{
  ...
  public send(channel: string, ...data: any[]): void {
    IpcWebSocket.transport.send({
      type: IpcWebSocketMessageType.Send,
      channel,
      data,
      sequence: -1,
    });
  ...
  implements IpcSocketBackend
{
  ...
  public send(channel: string, ...data: any[]): void {
    IpcWebSocket.transport.send({
      type: IpcWebSocketMessageType.Push,
      channel,
      data,
      sequence: -1,
    });
  ...
  public handle(
    channel: string,
    handler: (event: Event, methodName: string, ...args: any[]) => Promise<any>
  ): RemoveFunction {
  ...
  private async dispatch(_evt: Event, message: IpcWebSocketMessage) {
    if (message.type !== IpcWebSocketMessageType.Invoke) return;
