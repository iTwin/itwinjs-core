...
export namespace Gradient {
  ...
  export class Symb {
    ...
    public static createThematic(settings: ThematicGradientSettings) {
      ...
      if (settings.colorScheme < ThematicGradientColorScheme.Custom) {
        ...
        if (settings.customKeys.length > 1) {
          ...
          // if custom color keys are not specified properly, revert to some basic key scheme and assert
    ...
    public static compareSymb(lhs: Gradient.Symb, rhs: Gradient.Symb): number {
      if (lhs === rhs) return 0; // Same pointer
      if (lhs.mode !== rhs.mode) return lhs.mode - rhs.mode;
      if (lhs.flags !== rhs.flags)
        if (lhs.flags === undefined) return -1;
        else if (rhs.flags === undefined) return 1;
        else return lhs.flags - rhs.flags;
      if (lhs.tint !== rhs.tint)
        if (lhs.tint === undefined) return -1;
        else if (rhs.tint === undefined) return 1;
        else return lhs.tint - rhs.tint;
      if (lhs.shift !== rhs.shift)
        if (lhs.shift === undefined) return -1;
        else if (rhs.shift === undefined) return 1;
        else return lhs.shift - rhs.shift;
    ...
    private roundToByte(num: number): number {
      return Math.min(num + 0.5, 255.0) & 0xff;
    }
    ...
    public mapColor(value: number) {
      ...
      if ((this.flags & Flags.Invert) !== 0) value = 1 - value;
      ...
      return ColorDef.from(
        this.roundToByte(red),
        this.roundToByte(green),
        this.roundToByte(blue),
        this.roundToByte(transparency)
      );
    }

    public get hasTranslucency(): boolean {
      for (const key of this.keys) {
        if (!key.color.isOpaque) return true;
      }
      ...
      switch (settings.mode) {
        ...
        case ThematicGradientMode.Stepped: {
          assert(
            ...
            "Step count must be at least two to generate renderer gradient for thematic display"
          );
          for (let j = 0; j < dimension; j++) {
    ...
    public getImage(width: number, height: number): ImageBuffer {
      if (this.mode === Mode.Thematic) width = 1;
          ...
          for (let j = 0; j < height; j++) {
            ...
            for (let i = 0; i < width; i++) {
              ...
              if (this.mode === Mode.Linear) {
                ...
                if (d > 0) f = Math.sin((Math.PI / 2) * (1.0 - d / dMax));
                else f = Math.sin((Math.PI / 2) * (1.0 - d / dMin));
              }
