...
import { AngleSweep, AnyCurvePrimitive, Arc3d, LineString3d, Point3d, Range2d, Range2dProps, Range3d, Transform, TransformProps } from "@itwin/core-geometry";

// I don't love where this is.

export namespace FrameGeometry {
  // Rectangle
  export const computeRectangleFrame = (rangeProps: Range2dProps, transformProps: TransformProps): AnyCurvePrimitive[] => {
  ...
  // RoundedRectangle
  export const computeRoundedRectangleFrame0 = (rangeProps: Range2dProps, transformProps: TransformProps, radiusFactor: number = 0.5): AnyCurvePrimitive[] => {
    const range = Range2d.fromJSON(rangeProps);
  ...
  export const computeRoundedRectangleFrame = (rangeProps: Range2dProps, transformProps: TransformProps, radiusFactor: number = 0.5): AnyCurvePrimitive[] => {
    const range = Range2d.fromJSON(rangeProps);
    const radius = range.yLength() * radiusFactor * (Math.sqrt(2) / 2);
    // We're going to circumscribe the range with our rounded edges. The corners of the range will fall on 45 degree angles.
    const radiusOffsetFactor = radius / Math.sqrt(2);
  ...
  // Circle
  export const computeCircleFrame = (rangeProps: Range2dProps, transformProps: TransformProps): AnyCurvePrimitive[] => {
    const range = Range2d.fromJSON(rangeProps);
  ...
  // Capsule
  export const computeCapsuleFrame = (rangeProps: Range2dProps, transformProps: TransformProps): AnyCurvePrimitive[] => {
    const range = Range2d.fromJSON(rangeProps);
