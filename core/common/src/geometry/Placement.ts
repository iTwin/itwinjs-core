...
export class Placement3d implements Placement3dProps {
  ...
  public get isValid(): boolean {
    return !this.bbox.isNull && Math.max(this.origin.maxAbs(), this.bbox.maxAbs()) < Constant.circumferenceOfEarth;
  }
  ...
  public multiplyTransform(other: Transform): void {
    ...
    if (undefined === angles) throw new IModelError(IModelStatus.BadRequest, "Invalid Transform");
...
export class Placement2d implements Placement2dProps {
  ...
  /** Get the transform from local coordinates of this placement to world coordinates. */
  public get transform(): Transform {
    return Transform.createOriginAndMatrix(Point3d.createFrom(this.origin), this.rotation);
  }
