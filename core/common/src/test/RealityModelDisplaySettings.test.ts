...
describe("PointCloudDisplaySettings", () => {
  it("round-trips through JSON", () => {
    const roundTrip = (
      props: PointCloudDisplayProps | undefined,
  ...
  it("clones", () => {
    const test = (
      ...
      if (expected === "input") expected = baseProps;
    ...
    test(
      ...
      { voxelScale: 0.5, maxPixelsPerVoxel: 12 },
      { voxelScale: 0.5, minPixelsPerVoxel: 3, maxPixelsPerVoxel: 12 }
    );
    test(
      {
        voxelScale: 2,
    ...
    test(
      { voxelScale: 2, pixelSize: 3, minPixelsPerVoxel: 4 },
      { pixelSize: undefined, minPixelsPerVoxel: 5 },
      { voxelScale: 2, minPixelsPerVoxel: 5 }
    );
  });
});
...
describe("RealityModelDisplaySettings", () => {
  it("round-trips through JSON", () => {
    const roundTrip = (
      props: RealityModelDisplayProps | undefined,
  ...
  it("clones", () => {
    ...
    test(
      ...
      { overrideColorRatio: 2 },
      { pointCloud: { sizeMode: "pixel" }, overrideColorRatio: 2 }
    );
    test(
      { pointCloud: { sizeMode: "pixel" } },
      { pointCloud: { pixelSize: 2 } },
      { pointCloud: { sizeMode: "pixel", pixelSize: 2 } }
    );
    test(
      { pointCloud: { sizeMode: "pixel" }, overrideColorRatio: 2 },
      { pointCloud: { sizeMode: undefined } },
      { overrideColorRatio: 2 }
    );
    test(
      { pointCloud: { sizeMode: "pixel" }, overrideColorRatio: 2 },
      { pointCloud: {} },
      { overrideColorRatio: 2, pointCloud: { sizeMode: "pixel" } }
    );
  });
});
