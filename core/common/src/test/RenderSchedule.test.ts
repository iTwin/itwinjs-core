...
describe("RenderSchedule", () => {
  it("interpolates transforms", () => {
    ...
    expectTransform(script.duration.low, [
      [1, 0, 0, 7860.699219],
      [0, 1, 0, 8590.857422],
      [0, 0, 1, -39.539063],
    ]);
  ...
  describe("TransformEntry", () => {
    it("compares for equality", () => {
      ...
      expectUnequal(
        new RS.TransformEntry({ time: 1, interpolation: 1 }),
        new RS.TransformEntry({ time: 1, interpolation: 2 })
      );

      const tf = makeTransform;
      expectEqual(
        new RS.TransformEntry({ time: 1, value: tf(1, 2, 3, 4) }),
        new RS.TransformEntry({ time: 1, value: tf(1, 2, 3, 4) })
      );
      expectUnequal(
        new RS.TransformEntry({ time: 1, value: tf(1, 2, 3, 4) }),
        new RS.TransformEntry({ time: 1, value: tf(5, 6, 7, 8) })
      );

      const cmp = makeTransformComponents;
      expectEqual(
        new RS.TransformEntry({ time: 1, value: cmp(1, 2, 3) }),
        new RS.TransformEntry({ time: 1, value: cmp(1, 2, 3) })
      );
      expectUnequal(
        new RS.TransformEntry({ time: 1, value: cmp(1, 2, 3) }),
        new RS.TransformEntry({ time: 1, value: cmp(3, 2, 1) })
      );
  ...
  describe("CuttingPlaneEntry", () => {
    it("compares for equality", () => {
      ...
      expectUnequal(
        new RS.CuttingPlaneEntry({ time: 1, interpolation: 1 }),
        new RS.CuttingPlaneEntry({ time: 1, interpolation: 2 })
      );

      const cp = makeCuttingPlane;
      expectEqual(
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2, "vis") }),
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2, "vis") })
      );
      expectEqual(
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2) }),
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2) })
      );
      expectEqual(
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2, "hid") }),
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2, "hid") })
      );

      expectUnequal(
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2, "vis") }),
        new RS.CuttingPlaneEntry({ time: 1, value: cp(3, 2, "vis") })
      );
      expectUnequal(
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2, "hid") }),
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 3, "hid") })
      );
      expectUnequal(
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2, "vis") }),
        new RS.CuttingPlaneEntry({ time: 1, value: cp(1, 2, "hid") })
  ...
  describe("ElementTimeline", () => {
    it("compares for equality", () => {
      ...
      expectUnequal(
        RS.ElementTimeline.fromJSON({ batchId: 4, elementIds }),
        RS.ElementTimeline.fromJSON({ batchId: 5, elementIds })
      );

      expectUnequal(
        RS.ElementTimeline.fromJSON({ batchId: 4, elementIds }),
        RS.ElementTimeline.fromJSON({ batchId: 4, elementIds: ["0xabc"] })
      );
      expectUnequal(
        RS.ElementTimeline.fromJSON({ batchId: 4, elementIds }),
        RS.ElementTimeline.fromJSON({ batchId: 4, elementIds: ["0x1", "0x5"] })
      );
      expectUnequal(
        RS.ElementTimeline.fromJSON({ batchId: 4, elementIds }),
        RS.ElementTimeline.fromJSON({
          batchId: 4,
          elementIds: ["0x1", "0x5", "0xdef"],
        })
      );
      ...
      const c2 = {
        time: 4,
        interpolation: 2,
        value: { red: 255, green: 254, blue: 253 },
      };
      ...
      const p2 = {
        time: 4,
        interpolation: 1,
        value: makeCuttingPlane(-1, -2, "hid"),
      };
    ...
    it("considers the same element Ids equal only if in the same order", () => {
      ...
      const sorted = RS.ElementTimeline.fromJSON({
        batchId: 0,
        elementIds: sortedIds,
      });
      const unsorted = RS.ElementTimeline.fromJSON({
        batchId: 0,
        elementIds: unsortedIds,
      });
      const compressed = RS.ElementTimeline.fromJSON({
        batchId: 0,
        elementIds: compressedIds,
      });
  ...
  describe("ModelTimeline", () => {
    it("compares for equality", () => {
      ...
      expectEqual(
        RS.ModelTimeline.fromJSON({
          modelId: "0x1",
          realityModelUrl: "blah",
          elementTimelines,
        }),
        RS.ModelTimeline.fromJSON({
          modelId: "0x1",
      ...
      expectEqual(
        RS.ModelTimeline.fromJSON({ modelId: "0x2", elementTimelines }),
        RS.ModelTimeline.fromJSON({ modelId: "0x2", elementTimelines })
      );
      ...
      expectUnequal(
        ...
        RS.ModelTimeline.fromJSON({
          modelId: "0x1",
          elementTimelines: [{ batchId: 0, elementIds: ["0x2"] }],
        })
      );
    });
  });
