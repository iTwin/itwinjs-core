...
describe("ThematicDisplay", () => {
  it("Ensures ThematicDisplay derives values properly from JSON, including handling defaults and incorrect values", () => {
    function verifyDefaults(thematicDisplay: ThematicDisplay) {
      expect(thematicDisplay.axis).to.deep.equal(
        Vector3d.fromJSON({ x: 0.0, y: 0.0, z: 0.0 })
      );
      expect(thematicDisplay.displayMode).to.equal(ThematicDisplayMode.Height);
      expect(thematicDisplay.gradientSettings.mode).to.equal(
        ThematicGradientMode.Smooth
      );
      expect(thematicDisplay.gradientSettings.stepCount).to.equal(10);
      expect(thematicDisplay.gradientSettings.colorScheme).to.equal(
      ...
      expect(thematicDisplay.sensorSettings).to.deep.equal(
        ThematicDisplaySensorSettings.fromJSON()
      );
    ...
    expect(td.sensorSettings.sensors[0].position).to.deep.equal(
      Point3d.fromJSON(sensorSettingsProps.sensors[0].position)
    );
    expect(td.sensorSettings.sensors[0].value).to.equal(
      sensorSettingsProps.sensors[0].value
    );
    expect(td.sensorSettings.sensors[1].position).to.deep.equal(
      Point3d.fromJSON(sensorSettingsProps.sensors[1].position)
    );
    expect(td.sensorSettings.sensors[1].value).to.equal(
      sensorSettingsProps.sensors[1].value
    );
    expect(td.sensorSettings.sensors[2].position).to.deep.equal(
      Point3d.fromJSON(sensorSettingsProps.sensors[2].position)
    );
    expect(td.sensorSettings.sensors[2].value).to.equal(
      sensorSettingsProps.sensors[2].value
    );
    expect(td.sensorSettings.sensors[3].position).to.deep.equal(
      Point3d.fromJSON(sensorSettingsProps.sensors[3].position)
    );
    expect(td.sensorSettings.sensors[3].value).to.equal(0); // verify that the 'bad' value of -1 gets clamped to 0
    expect(td.sensorSettings.sensors[4].position).to.deep.equal(
      Point3d.fromJSON(sensorSettingsProps.sensors[4].position)
    );
    expect(td.sensorSettings.sensors[4].value).to.equal(1); // verify that the 'bad' value of 2 gets clamped to 1
    expect(td.sensorSettings.distanceCutoff).to.equal(
      sensorSettingsProps.distanceCutoff
    );
    ...
    expect(td.gradientSettings.customKeys[0].color).to.deep.equal(
      ColorDef.from(255, 255, 255, 0)
    ); // first should be white
    expect(td.gradientSettings.customKeys[0].value).to.equal(0.0); // value for black should be 0.0
    expect(td.gradientSettings.customKeys[1].color).to.deep.equal(
      ColorDef.from(0, 0, 0, 0)
