...
describe("LightSettings", () => {
  it("round-trips through JSON", () => {
    const roundTrip = (
      input: LightSettingsProps | undefined,
      expected: LightSettingsProps | "input" | undefined
    ) => {
    ...
    roundTrip(
      { portrait: { intensity: 5.1 } },
      { portrait: { intensity: 5.0 } }
    );
    roundTrip(
      { portrait: { intensity: -0.1 } },
      { portrait: { intensity: 0.0 } }
    );

    roundTrip(
      {
        solar: {
          direction: Vector3d.create(0.272166, 0.680414, 0.680414).toJSON(),
          intensity: 1,
          alwaysEnabled: false,
        },
      },
    ...
    roundTrip(
      { solar: { direction: Vector3d.create(-1, -1, -1).toJSON() } },
      "input"
    );
    ...
    roundTrip(
      {
        solar: { direction: new Vector3d(-1, 0, 1).toJSON(), timePoint: 12345 },
      },
      "input"
    );

    roundTrip(
      { ambient: { color: new RgbColor(0, 0, 0).toJSON(), intensity: 0.2 } },
      undefined
    );
    roundTrip(
      { ambient: { color: new RgbColor(1, 127, 255).toJSON() } },
      "input"
    );
    ...
    roundTrip(
      {
        hemisphere: {
          lowerColor: new RgbColor(120, 143, 125).toJSON(),
          upperColor: new RgbColor(143, 205, 255),
          intensity: 0,
        },
      },
      undefined
    ...
    roundTrip(
      { hemisphere: { lowerColor: new RgbColor(0, 1, 2).toJSON() } },
      "input"
    );
    roundTrip(
      { hemisphere: { upperColor: new RgbColor(254, 254, 255).toJSON() } },
      "input"
    );
    roundTrip({ hemisphere: { intensity: 2.5 } }, "input");
    roundTrip({ hemisphere: { intensity: -0.1 } }, undefined);
    roundTrip(
      { hemisphere: { intensity: 5.1 } },
      { hemisphere: { intensity: 5.0 } }
    );

    roundTrip({ fresnel: {} }, undefined);
    roundTrip(
      { fresnel: { intensity: 0.8, invert: false } },
      { fresnel: { intensity: 0.8 } }
    );
    roundTrip(
      { fresnel: { intensity: 0, invert: true } },
      { fresnel: { invert: true } }
    );
    roundTrip(
      { fresnel: { intensity: -1, invert: true } },
      { fresnel: { invert: true } }
    );
  ...
  it("clone preserves solar time point unless new direction differs from previous direction and new time point was not supplied", () => {
    const test = (
      srcProps: SolarLightProps,
      changedProps: SolarLightProps,
      expectedProps: SolarLightProps
    ) => {
    ...
    test(
      { direction: dir1 },
      { timePoint: 123 },
      { direction: dir1, timePoint: 123 }
    );
    test(
      { direction: dir1, timePoint: 123 },
      { timePoint: 456 },
      { direction: dir1, timePoint: 456 }
    );
    test(
      { direction: dir1, timePoint: 123 },
      { direction: dir2 },
