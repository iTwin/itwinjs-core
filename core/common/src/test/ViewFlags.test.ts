...
function invertDefaults(): ViewFlags {
  ...
  for (const propname of Object.keys(ViewFlags.defaults)) {
    ...
    if (typeof value === "boolean") (invertedProperties as any)[key] = !value;
  }
  ...
  it("should compute whether edges are required", () => {
    ...
    for (const renderMode of [
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (undefined !== renderMode) ovrs.renderMode = renderMode;

        let visibleEdges;
        if (i > 0) visibleEdges = ovrs.visibleEdges = i > 1;
