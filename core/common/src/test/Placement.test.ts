...
describe("Placement", () => {
  it("Placement3d", () => {
    const invalidPlacement: Placement3d = new Placement3d(
      new Point3d(0, 0, 0),
      YawPitchRollAngles.createDegrees(0, 0, 0),
      new Range3d()
    );
    assert.isFalse(
      invalidPlacement.isValid,
      "Expect isValid to be false since no bbox provided"
    );
    const placement: Placement3d = new Placement3d(
      new Point3d(0, 1, 0),
      YawPitchRollAngles.createDegrees(0, 0, 0),
      new Range3d(-5, -5, -2, 5, 5, 2)
    );
    assert.isTrue(placement.isValid, "Expect isValid to be true");
    assert.deepEqual(
      placement.calculateRange(),
      new Range3d(-5, -4, -2, 5, 6, 2)
    );
  ...
  it("Placement2d", () => {
    const invalidPlacement: Placement2d = new Placement2d(
      new Point2d(0, 0),
    ...
    const placement: Placement2d = new Placement2d(
      new Point2d(0, 1),
      Angle.createDegrees(0),
      new Range2d(-5, -5, 5, 5)
    );
    ...
    assert.exists(invalidTransform);
    assert.throws(() => placement.multiplyTransform(invalidTransform));
    const invalidRotation: Transform = Transform.createOriginAndMatrix(
      undefined,
