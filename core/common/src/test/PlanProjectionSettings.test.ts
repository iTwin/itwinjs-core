...
describe("PlanProjectionSettings", () => {
  it("round-trips through JSON", () => {
    const roundTrip = (
      input: PlanProjectionSettingsProps | undefined,
      expected: PlanProjectionSettingsProps | undefined | "input"
    ) => {
  ...
  it("clones", () => {
    const clone = (
      input: PlanProjectionSettingsProps,
      changed: PlanProjectionSettingsProps | undefined,
      expected: PlanProjectionSettingsProps
    ) => {
    ...
    clone({ overlay: true }, undefined, {
      overlay: true,
      enforceDisplayPriority: false,
    });
    clone({ overlay: false }, undefined, {
      overlay: false,
      enforceDisplayPriority: false,
    });
    clone(
    ...
    clone(
      { transparency: 0.5 },
      { transparency: 0.75 },
      { transparency: 0.75, overlay: false, enforceDisplayPriority: false }
    );
    clone(
      { transparency: 0.5 },
      { transparency: 1.25 },
      { transparency: 1.0, overlay: false, enforceDisplayPriority: false }
    );

    clone(
      {},
      { elevation: 1, transparency: 0.2 },
      {
        elevation: 1,
        transparency: 0.2,
        overlay: false,
    ...
    clone({ enforceDisplayPriority: true }, undefined, {
      enforceDisplayPriority: true,
      overlay: false,
    });
    clone({ enforceDisplayPriority: false }, undefined, {
      enforceDisplayPriority: false,
      overlay: false,
    });
    clone(
    ...
    clone(
      { enforceDisplayPriority: true },
      { enforceDisplayPriority: false },
      { enforceDisplayPriority: false, overlay: false }
    );
  });
});
