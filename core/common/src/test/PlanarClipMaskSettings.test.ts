...
describe("PlanarClipMaskSettings", () => {
  it("uses defaults", () => {
    ...
    expect(
      PlanarClipMaskSettings.fromJSON({
        mode: undefined,
      } as unknown as PlanarClipMaskProps)
    ).to.equal(PlanarClipMaskSettings.defaults);
  });
  ...
  it("creates for models", () => {
    expectProps(PlanarClipMaskSettings.create({}), {
      mode: PlanarClipMaskMode.Models,
    });
    expectProps(PlanarClipMaskSettings.create({ modelIds: undefined }), {
      mode: PlanarClipMaskMode.Models,
    });
    expectProps(PlanarClipMaskSettings.create({ modelIds: ["0x3", "0x1"] }), {
      mode: PlanarClipMaskMode.Models,
      modelIds: "+1+2",
    });
    expectProps(
  ...
  it("creates for elements or subcategories", () => {
    expectProps(PlanarClipMaskSettings.create({ elementIds: "0x5" }), {
      ...
      subCategoryOrElementIds: "+5",
    });

    expectProps(
      PlanarClipMaskSettings.create({
        elementIds: ["0x1", "0x5"],
        modelIds: "0x2",
        exclude: true,
      }),
  ...
  it("creates by priority", () => {
    expectProps(PlanarClipMaskSettings.create({ priority: 1234 }), {
  ...
  it("clones", () => {
    ...
    expectClone(
      { mode: PlanarClipMaskMode.Models, subCategoryOrElementIds: undefined },
      {
        mode: PlanarClipMaskMode.Models,
        priority: 12,
        modelIds: "+1+5",
