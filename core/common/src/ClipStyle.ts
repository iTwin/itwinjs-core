...
export class CutStyle {
  ...
  private constructor(
    viewflags?: Readonly<ViewFlagOverrides>,
    hiddenLine?: HiddenLine.Settings,
    appearance?: FeatureAppearance
  ) {
    this.viewflags = viewflags ?? {};
    if (hiddenLine && !hiddenLine.matchesDefaults) this.hiddenLine = hiddenLine;

    if (appearance && !appearance.matchesDefaults) this.appearance = appearance;
  }
  ...
  public static create(
    ...
    if (
      (viewflags && JsonUtils.isNonEmptyObject(viewflags)) ||
      (hiddenLine && !hiddenLine.matchesDefaults) ||
  ...
  public get matchesDefaults(): boolean {
    if (this === CutStyle.defaults) return true;

    return (
      !JsonUtils.isNonEmptyObject(this.viewflags) &&
      (!this.hiddenLine || this.hiddenLine.matchesDefaults) &&
...
export class ClipStyle {
  ...
  public toJSON(): ClipStyleProps | undefined {
    ...
    if (this.produceCutGeometry) props.produceCutGeometry = true;
    ...
    if (this.insideColor) props.insideColor = this.insideColor.toJSON();

    if (this.outsideColor) props.outsideColor = this.outsideColor.toJSON();
