...
export class SubCategoryOverride {
  ...
  public get anyOverridden(): boolean {
    return (
      undefined !== this.invisible ||
      undefined !== this.color ||
      undefined !== this.weight ||
      undefined !== this.style ||
      undefined !== this.priority ||
      undefined !== this.material ||
      undefined !== this.transparency
    );
  }
  ...
  public override(appearance: SubCategoryAppearance): SubCategoryAppearance {
    if (!this.anyOverridden) return appearance;
  ...
  public toJSON(): SubCategoryAppearance.Props {
    ...
    if (undefined !== this.invisible) props.invisible = this.invisible;

    if (undefined !== this.weight) props.weight = this.weight;

    if (undefined !== this.style) props.style = this.style;

    if (undefined !== this.material) props.material = this.material;

    if (undefined !== this.priority) props.priority = this.priority;

    if (undefined !== this.transparency) props.transp = this.transparency;

    if (undefined !== this.color) props.color = this.color.toJSON();
