...
export class PolylineFlags {
  ...
  public setIsNormalEdge(): void {
    this.type = PolylineTypeFlags.Edge;
  }
  public setIsOutlineEdge(): void {
    this.type = PolylineTypeFlags.Outline;
  }

  /** Convert these flags to a numeric representation for serialization. */
  public pack(): number {
    let val: number = 0;
    if (this.isDisjoint) val += 1;
    if (this.isPlanar) val += 1 << 1;
    if (this.is2d) val += 1 << 2;
...
export class MeshEdge {
  ...
  public compareTo(other: MeshEdge): number {
    ...
    if (0 === diff) diff = this.indices[1] - other.indices[1];
...
export class PolylineEdgeArgs {
  ...
  public constructor(lines?: PolylineData[]) {
    this.init(lines);
  }
