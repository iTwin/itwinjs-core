...
describe("iModelHub BriefcaseHandler", () => {
  ...
  before(async () => {
    ...
    if (!TestConfig.enableMocks) {
      const briefcases = await utils.getClient(iModelId).Briefcases().get(actx, accessToken, iModelId);
      let briefcasesCount = briefcases.length;
      if (briefcasesCount > 19) {
        // Ensure that tests can still acquire briefcases
        for (const briefcase of briefcases) {
          await utils.getClient(iModelId).Briefcases().delete(actx, accessToken, iModelId, briefcase.briefcaseId!);
        }
        briefcasesCount = 0;
      }
      ...
      if (briefcasesCount === 0) {
        const briefcase = await utils.getClient(iModelId).Briefcases().create(actx, accessToken, iModelId);
  ...
  it("should delete a briefcase", async () => {
    ...
    const originalBriefcaseCount = (await utils.getClient(iModelId).Briefcases().get(actx, accessToken, iModelId)).length;

    mockDeleteBriefcase(iModelId, acquiredBriefcaseId);
    await utils.getClient(iModelId).Briefcases().delete(actx, accessToken, iModelId, acquiredBriefcaseId);

    utils.mockGetBriefcase(iModelId, utils.generateBriefcase(2));
    const briefcaseCount = (await utils.getClient(iModelId).Briefcases().get(actx, accessToken, iModelId)).length;
  ...
  it("should fail getting an invalid briefcase", async () => {
    let error: any;
    try {
      await utils.getClient(iModelId).Briefcases().get(actx, accessToken, iModelId, new BriefcaseQuery().byId(-1));
    } catch (err) {
      error = err;
    }
    chai.assert(error);
  ...
  it("should get information on a briefcase by id", async () => {
    ...
    const briefcase: Briefcase = (await utils.getClient(iModelId).Briefcases().get(actx, accessToken, iModelId, new BriefcaseQuery().byId(briefcaseId)))[0];
  ...
  it("should fail deleting an invalid briefcase", async () => {
    let error: any;
    try {
      await utils.getClient(iModelId).Briefcases().delete(actx, accessToken, iModelId, -1);
    } catch (err) {
      error = err;
    }
    chai.assert(error);
  ...
  it("should get the download URL for a Briefcase", async () => {
    ...
    const briefcase: Briefcase = (await utils.getClient(iModelId).Briefcases().get(actx, accessToken, iModelId, new BriefcaseQuery().byId(briefcaseId).selectDownloadUrl()))[0];
    chai.expect(briefcase.briefcaseId).to.be.equal(briefcaseId);
  ...
  it("should download a Briefcase", async () => {
    ...
    await utils.getClient(iModelId).Briefcases().download(actx, briefcase, downloadToPathname, progressTracker.track());
  ...
  it("should get error 409 and fail to get briefcase", async function (this: Mocha.ITestCallbackContext) {
    ...
    let error;
    try {
      (await utils.getClient(iModelId).Briefcases().get(actx, accessToken, iModelId));
    } catch (err) {
      error = err;
    }
    chai.assert(error);
  ...
  it("should fail downloading briefcase with no file url", async () => {
    let error: IModelHubClientError | undefined;
    try {
      await utils.getClient(iModelId).Briefcases().download(actx, new Briefcase(), utils.workDir);
    } catch (err) {
      if (err instanceof IModelHubClientError)
        error = err;
    }
