...
describe("iModelHub iModelHandler", () => {
  ...
  it("should be able to delete iModels", async function (this: Mocha.ITestCallbackContext) {
    ...
    for (const name of names) {
      ...
      await imodelProject.deleteIModel(alctx, accessToken, projectId, imodel.id!);
    }
  });
  ...
  it("should fail getting an invalid iModel", async () => {
    ...
    try {
      await imodelProject.queryIModels(alctx, accessToken, projectId, new IModelQuery().byId(mockGuid));
    } catch (err) {
