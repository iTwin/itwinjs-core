...
describe("iModelHub VersionHandler", () => {
  ...
  before(async () => {
    ...
    if (!TestConfig.enableMocks) {
      ...
      const versionsCount = (await utils.getClient(iModelId).Versions().get(accessToken, iModelId)).length;
      if (versionsCount === 0) {
        ...
        if (changeSetCount === 0 || changeSetCount > 9) {
          ...
          changeSet = (await utils.getClient(iModelId).ChangeSets().get(accessToken, iModelId))[0];
        }
        const version: Version = await utils.getClient(iModelId).Versions().create(accessToken, iModelId, changeSet.id!, "Version 1");

        if (utils.getIModelProjectAbstraction().isIModelHub) {
          // Wait for large thumbnail.
          for (let i = 0; i < 5; i++) {
            const largeThumbnails = (await utils.getClient(iModelId).Thumbnails().get(accessToken, iModelId, "Large", new ThumbnailQuery().byVersionId(version.wsgId)));
  ...
  it("should create named version", async function (this: Mocha.ITestCallbackContext) {
    ...
    const changeSetsCount = (await utils.getClient(iModelId).ChangeSets().get(accessToken, iModelId)).length;
    ...
    const version: Version = await utils.getClient(iModelId).Versions().create(accessToken, iModelId, changeSet.id!, versionName);
  ...
  it("should get named versions", async function (this: Mocha.ITestCallbackContext) {
    ...
    for (const expectedVersion of versions) {
      ...
      const actualVersion: Version = (await utils.getClient(iModelId).Versions().get(accessToken, iModelId, new VersionQuery().byId(expectedVersion.wsgId)))[0];
  ...
  it("should query named versions by ChangeSet id", async function (this: Mocha.ITestCallbackContext) {
    ...
    const expectedVersion: Version = (await utils.getClient(iModelId).Versions().get(accessToken, iModelId))[0];
    chai.assert(expectedVersion);

    const version: Version[] = await utils.getClient(iModelId).Versions().get(accessToken, iModelId, new VersionQuery().byChangeSet(expectedVersion.changeSetId!));
    chai.assert(version);
  ...
  it("should get named versions with thumbnail id", async () => {
    ...
    utils.mockGetVersions(iModelId, undefined, ...mockedVersions);
    let versions: Version[] = await utils.getClient(iModelId).Versions().get(accessToken, iModelId, new VersionQuery());
    ...
    const smallThumbnail: Thumbnail = (await utils.getClient(iModelId).Thumbnails().get(accessToken, iModelId, "Small", new ThumbnailQuery().byVersionId(firstVersion.wsgId)))[0];
    ...
    const largeThumbnail: Thumbnail = (await utils.getClient(iModelId).Thumbnails().get(accessToken, iModelId, "Large", new ThumbnailQuery().byVersionId(firstVersion.wsgId)))[0];
    ...
    versions = await utils.getClient(iModelId).Versions().get(accessToken, iModelId, new VersionQuery().byId(firstVersion.wsgId).selectThumbnailId("Small", "Large"));
    chai.expect(versions.length === 1);
    ...
    versions = await utils.getClient(iModelId).Versions().get(accessToken, iModelId, new VersionQuery().byName(firstVersion.name!).selectThumbnailId("Large"));
    chai.expect(versions.length === 1);
  ...
  it("should update named version", async function (this: Mocha.ITestCallbackContext) {
    ...
    let version: Version = (await utils.getClient(iModelId).Versions().get(accessToken, iModelId))[0];
    chai.assert(!!version);
    chai.expect(version.wsgId).to.have.length.above(0);
    ...
    version = await utils.getClient(iModelId).Versions().update(accessToken, iModelId, version);
