...
* `fdt attach model maplayer [name] [globe|reality|imodel] [modelId0,modelId1,...modelIdN]` - Attach a model map layer for each unique model of the currently selected elements to either the globe, attached reality models, or iModel. If `globe`, `reality`, or `imodel` is not specified, the attachment will default to `globe`. If `name` is specified, use that. If `name` is specified as `0`, that is considered `undefined` or unspecified. `modelIds` is a list of models within an iModel to drape onto, given that `imodel` is specified as the drape target; the list is separated by commas and must have no spaces.
