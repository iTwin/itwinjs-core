...
* `fdt attach model maplayer [name] [globe|reality|imodel]` - Attach a model map layer for each unique model of the currently selected elements to either the globe, attached reality models, or iModel. If `globe`, `reality`, or `imodel` is not specified, the attachment will default to `globe`. If `name` is specified, use that. If `name` is specified as `0`, that is considered `undefined` or unspecified.
