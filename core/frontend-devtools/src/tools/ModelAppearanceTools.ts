...
function changeModelAppearanceOverrides(
    ...
    if (changed)
      IModelApp.notifications.outputMessage(
        new NotifyMessageDetails(
          ...
          `${modelChangedString(name)} set to transparency: ${transparency}`
        )
      );
  ...
  public override async run(weight: number, name: string): Promise<boolean> {
    ...
    if (changed)
      IModelApp.notifications.outputMessage(
        new NotifyMessageDetails(
          OutputMessagePriority.Info,
          `${modelChangedString(name)} set to line weight: ${weight}`
        )
      );
  ...
  public override async run(lineCode: number, name: string): Promise<boolean> {
    ...
    if (changed)
      IModelApp.notifications.outputMessage(
        new NotifyMessageDetails(
          OutputMessagePriority.Info,
          `${modelChangedString(name)} set to line code: ${lineCode}`
        )
      );
  ...
  public override async run(ignoresMaterial: true | undefined, name: string): Promise<boolean> {
    ...
    if (changed)
      IModelApp.notifications.outputMessage(
        new NotifyMessageDetails(
          OutputMessagePriority.Info,
          `Mode: ${name} set to ignore Materials: ${ignoresMaterial}`
        )
      );
