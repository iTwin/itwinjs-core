...
class AttachMapLayerBaseTool extends Tool {
  ...
  protected doAttach(source?: MapLayerSource) {
    ...
    if (vp === undefined || source === undefined) return;

    source
      .validateSource()
      .then((validation) => {
        if (
          validation.status === MapLayerSourceStatus.Valid ||
          ...
          if (this._isBase) {
            ...
            if (settings) {
              ...
              vp.displayStyle.attachMapLayer({
                settings,
                mapLayerIndex: { isOverlay: !this._isBackground, index: -1 },
              });
            }
          }
...
export class AttachMapLayerTool extends AttachMapLayerBaseTool {
  ...
  public override async run(name: string): Promise<boolean> {
    ...
    MapLayerSources.create()
      .then((sources) => {
        if (sources !== undefined) {
          const source = sources.findByName(name, this._isBase);
          if (source !== undefined) this.doAttach(source);
        }
      })
      .catch((_err) => {});
...
export class MapLayerVisibilityTool extends Tool {
  ...
  public override async run(
    ...
    const mapLayer = vp.displayStyle.mapLayerAtIndex({
      index: layerIndex,
      isOverlay: false,
    });
    if (undefined === mapLayer) return false;
  ...
  public override async parseAndRun(...args: string[]): Promise<boolean> {
    ...
    if (typeof enable !== "string") await this.run(layerIndex, enable);
...
export class MapLayerSubLayerVisibilityTool extends Tool {
  ...
  public override async run(
    ...
    vp.displayStyle.changeMapSubLayerProps({ visible }, -1, {
      index: layerIndex,
      isOverlay: false,
    });
    vp.invalidateRenderPlan();
...
export class MapLayerZoomTool extends Tool {
  ...
  public override async run(layerIndex: number): Promise<boolean> {
    ...
    vp.viewMapLayerRange({ index: layerIndex, isOverlay: false }, vp)
      .then(() => {})
      .catch(() => {});
...
export class MapBaseColorTool extends Tool {
   ...
   */
  public override async parseAndRun(...args: string[]): Promise<boolean> {
    const red = parseFloat(args[0]),
      green = parseFloat(args[1]),
      blue = parseFloat(args[2]);
