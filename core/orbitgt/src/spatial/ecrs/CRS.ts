...
export class CRS {
  ...
  public toGeoCentric(local: Coordinate): Coordinate {
    ...
    if (this._type == CRS.PROJECTED) {
      ...
      ASystem.assertNot(
        ...
        "Projected CRS '" + this._code + "' needs a geographic base CRS '" + this._baseCRS.getCode() + "', not '" + this._baseCRS.getTypeLabel() +
          "'",
      );
  ...
  public toWGSi(source: Coordinate, wgsTransformationIndex: int32): Coordinate {
    ...
    /* Get the transformation from the local datum to the WGS datum */
    let localToWGS: Operation = (wgsTransformationIndex < 0)
  ...
  public toString(): string {
    return "[CRS:code=" + this._code + ",name='" + this._name + "',area=" + this._area + ",type='" + CRS.labelCRSType(this._type) + "',datum=" +
      this._datum + ",baseCRS=" + this._baseCRS + ",wgs-transform=" + (this._transformationToWGS != null) + "]";
  }
