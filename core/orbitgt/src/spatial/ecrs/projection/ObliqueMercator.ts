...
export class ObliqueMercator extends OperationMethod {
    ...
    public initialize(operation: Operation): void {
        /* Get the ellipsoid */
        let sourceCRS: CRS = operation.getSourceCRS();
    ...
    public forward(sourceCRS: CRS, source: Coordinate, targetCRS: CRS, target: Coordinate): void {
        ...
        let t: float64 = Math.tan(ObliqueMercator.PI / 4.0 - lat / 2.0) / Math.pow((1.0 - esinLat) / (1.0 + esinLat), this._e / 2.0);
        ...
        let U: float64 = (-V * Math.cos(this._gamO) + S * Math.sin(this._gamO)) / T;
        ...
        let E: float64 = v * cosGamC + u * sinGamC + this._eC;
        let N: float64 = u * cosGamC - v * sinGamC + this._nC;
        /* Save the position */
        target.setX(E);
        target.setY(N);
    ...
    public reverse(sourceCRS: CRS, source: Coordinate, targetCRS: CRS, target: Coordinate): void {
        ...
        let sinGamC: float64 = Math.sin(this._gamC);
        let cosGamC: float64 = Math.cos(this._gamC);
        let v: float64 = (E - this._eC) * cosGamC - (N - this._nC) * sinGamC;
        let u: float64 = (N - this._nC) * cosGamC + (E - this._eC) * sinGamC + (Math.abs(this._uC) * ObliqueMercator.sign(this._latC));
        ...
        let chi: float64 = ObliqueMercator.PI / 2.0 - 2.0 * Math.atan(t);
        ...
        let lon: float64 = this._lonO - Math.atan2((S * Math.cos(this._gamO) - V * Math.sin(this._gamO)), Math.cos(this._B * u / this._A)) / this._B; // replaced atan(y/x) by atan2(y,x). LER, 23/05/2017
        /* Save the position */
        source.setX(lon);
        source.setY(lat);
