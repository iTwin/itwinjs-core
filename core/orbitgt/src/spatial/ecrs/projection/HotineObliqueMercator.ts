...
export class HotineObliqueMercator extends OperationMethod {
    ...
    public initializeProjection(ellipsoid: Ellipsoid): HotineObliqueMercator {
        ...
        let esinLatC: float64 = this._e * Math.sin(this._latC);
        ...
        let D2: float64 = (this._D < 1.0) ? (1.0) : (this._D * this._D);
        ...
        let e4: float64 = this._e2 * this._e2;
        let e6: float64 = e4 * this._e2;
        let e8: float64 = e4 * e4;
    ...
    public forward(sourceCRS: CRS, source: Coordinate, targetCRS: CRS, target: Coordinate): void {
        ...
        let esinLat: float64 = this._e * Math.sin(lat);
        let t: float64 = Math.tan(Math.PI / 4.0 - lat / 2.0) / Math.pow((1.0 - esinLat) / (1.0 + esinLat), this._e / 2.0);
        let Q: float64 = this._H / Math.pow(t, this._B);
        let S: float64 = (Q - 1.0 / Q) / 2.0;
        let T: float64 = (Q + 1.0 / Q) / 2.0;
        let V: float64 = Math.sin(this._B * (lon - this._lonO));
        let U: float64 = (-V * Math.cos(this._latO) + S * Math.sin(this._latO)) / T;
        let v: float64 = this._A * Math.log((1.0 - U) / (1.0 + U)) / (2.0 * this._B);
        let u: float64 = (this._A * Math.atan((S * Math.cos(this._latO) + V * Math.sin(this._latO)) / Math.cos(this._B * (lon - this._lonO))) / this._B); // possibly related to method 9815 atan2 problem? (LER 15/06/2018)
        let sinGamC: float64 = Math.sin(this._gamC);
        let cosGamC: float64 = Math.cos(this._gamC);
        let E: float64 = v * cosGamC + u * sinGamC + this._fe;
        let N: float64 = u * cosGamC - v * sinGamC + this._fn;
        /* Save the position */
        target.setX(E);
        target.setY(N);
    ...
    public reverse(sourceCRS: CRS, source: Coordinate, targetCRS: CRS, target: Coordinate): void {
        ...
        let sinGamC: float64 = Math.sin(this._gamC);
        let cosGamC: float64 = Math.cos(this._gamC);
        let v: float64 = (E - this._fe) * cosGamC - (N - this._fn) * sinGamC;
        let u: float64 = (N - this._fn) * cosGamC + (E - this._fe) * sinGamC;
        let Q: float64 = Math.exp(-this._B * v / this._A);
        let S: float64 = (Q - 1.0 / Q) / 2.0;
        let T: float64 = (Q + 1.0 / Q) / 2.0;
        let V: float64 = Math.sin(this._B * u / this._A);
        let U: float64 = (V * Math.cos(this._latO) + S * Math.sin(this._latO)) / T;
        let t: float64 = Math.pow(this._H / Math.sqrt((1.0 + U) / (1.0 - U)), 1.0 / this._B);
        let chi: float64 = Math.PI / 2.0 - 2.0 * Math.atan(t);
        let lat: float64 = chi + Math.sin(2.0 * chi) * this._rev1 + Math.sin(4.0 * chi) * this._rev2 + Math.sin(6.0 * chi) * this._rev3 + Math.sin(8.0 * chi) * this._rev4;
        let lon: float64 = this._lonO - Math.atan((S * cosGamC - V * sinGamC) / Math.cos(this._B * u / this._A)) / this._B; // possibly related to method 9815 atan2 problem? (LER 15/06/2018)
        /* Save the position */
        source.setX(lon);
        source.setY(lat);
