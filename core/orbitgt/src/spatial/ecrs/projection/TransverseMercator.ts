...
export class TransverseMercator extends OperationMethod {
  ...
  private calcM(lat: float64): float64 {
    const M: float64 = lat * (1.0 - this._e2 / 4 - 3 * this._e4 / 64 - 5 * this._e6 / 256) -
      Math.sin(2.0 * lat) * (3 * this._e2 / 8 + 3 * this._e4 / 32 + 45 * this._e6 / 1024) +
      Math.sin(4.0 * lat) * (15 * this._e4 / 256 + 45 * this._e6 / 1024) - Math.sin(6.0 * lat) * (35 * this._e6 / 3072);
    return this._a * M;
  }
  ...
  public toProjection(lon: float64, lat: float64, projected: Coordinate): void {
    ...
    const temp1: float64 = A + (1.0 - T + C) * TransverseMercator.pow(A, 3) / 6 +
      (5 - 18 * T + T * T + 72 * C - 58 * this._e_2) * TransverseMercator.pow(A, 5) / 120.0;
    const E: float64 = this._fE + this._k0 * v * temp1;
    const temp2: float64 = m - this._m0 +
      v * tan *
        (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * TransverseMercator.pow(A, 4) / 24.0 +
  ...
  public toGeoGraphic(x: float64, y: float64, geographic: Coordinate): void {
    ...
    /* Make the calculation */
    const temp1: float64 = D_2 / 2 - (5 + 3 * T1 + 10 * C1 - 4 * C1_2 - 9 * this._e_2) * TransverseMercator.pow(D, 4) / 24.0 +
      (61 + 90 * T1 + 298 * C1 + 45 * T1_2 - 252 * this._e_2 - 3 * C1_2) * TransverseMercator.pow(D, 6) / 720.0;
    const lat: float64 = lat1 - (v1 * tan / rho1) * temp1;
    const temp2: float64 = D - (1 + 2 * T1 + C1) * TransverseMercator.pow(D, 3) / 6 +
