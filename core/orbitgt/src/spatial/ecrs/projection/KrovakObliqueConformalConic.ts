...
export class KrovakObliqueConformalConic extends OperationMethod {
  ...
  public override initialize(operation: Operation): void {
    ...
    this._tO = Math.tan(Math.PI / 4 + this._gO / 2) * Math.pow((1 + e * Math.sin(this._latC)) / (1 - e * Math.sin(this._latC)), e * this._B / 2) /
  ...
  public forward(sourceCRS: CRS, source: Coordinate, targetCRS: CRS, target: Coordinate): void {
    ...
    const U: float64 = 2 *
      (Math.atan(
        this._tO * Math.pow(Math.tan(lat / 2 + Math.PI / 4), this._B) / Math.pow((1 + e * Math.sin(lat)) / (1 - e * Math.sin(lat)), e * this._B / 2),
      ) - Math.PI / 4);
  ...
  public reverse(sourceCRS: CRS, source: Coordinate, targetCRS: CRS, target: Coordinate): void {
    ...
    for (let i: number = 1; i < 5; i++) {
      lat = 2 *
        (Math.atan(
          Math.pow(this._tO, -1 / this._B) * Math.pow(Math.tan(U / 2 + Math.PI / 4), 1 / this._B) *
