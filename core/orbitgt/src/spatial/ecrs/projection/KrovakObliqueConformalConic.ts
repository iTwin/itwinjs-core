...
export class KrovakObliqueConformalConic extends OperationMethod {
  ...
  public forward(
    ...
    const r: float64 =
      (this._rO * Math.pow(Math.tan(Math.PI / 4 + this._latP / 2), this._n)) /
      Math.pow(Math.tan(S / 2 + Math.PI / 4), this._n);
  ...
  public reverse(
    ...
    for (let i: number = 1; i < 5; i++) {
      lat =
        2 *
        (Math.atan(
          Math.pow(this._tO, -1 / this._B) *
            Math.pow(Math.tan(U / 2 + Math.PI / 4), 1 / this._B) *
