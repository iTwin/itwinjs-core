...
export class WellKnownText {
  ...
  public static parseToWGS84(node: WellKnownTextNode, dialect: string): OperationMethod {
    ...
    return PositionVector.create(
      ...
      dy,
      dz,
      rx * WellKnownText._ARC_SEC_TO_RAD,
      ry * WellKnownText._ARC_SEC_TO_RAD,
      rz * WellKnownText._ARC_SEC_TO_RAD,
      ppm / 1.0e6
    );
  }
   ...
   */
  private static parseProjectionMethod(
    projection: WellKnownTextNode,
    parameters: AList<WellKnownTextNode>,
    dialect: string
  ): OperationMethod {
    ...
    for (let i: number = 0; i < parameters.size(); i++) {
      ...
      if (
        Strings.equalsIgnoreCase(projectionName, "Oblique_Stereographic") ||
        Strings.equalsIgnoreCase(projectionName, "Transverse_Mercator")
      ) {
        if (Strings.equalsIgnoreCase(parameterName, "latitude_of_origin"))
          parameterList.add(new ParameterValue(8801, parameterValue, DEG));
        if (Strings.equalsIgnoreCase(parameterName, "central_meridian"))
  ...
  private static parseProjection(crsCode: int32, node: WellKnownTextNode, dialect: string): CRS {
    ...
    let code: int32 = epsgCode == 0 ? crsCode : epsgCode;
    /* Return the CRS */
    let projectionOperation: Operation = new Operation(
      0 /*code*/,
      "" /*name*/,
