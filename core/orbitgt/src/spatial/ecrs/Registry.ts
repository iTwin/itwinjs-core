...
export class Registry {
  ...
  private static readAllUnits(): AList<Unit> {
    /* Make the list */
    const units: AList<Unit> = new AList<Unit>();
    /* Open the table */
    const tableReader: AList<string> = Registry.openTable(Registry._TABLE_UNIT);
    const headerLine: string = tableReader.get(0);
    /* Process the table */
    for (let i: number = 1; i < tableReader.size(); i++) {
      /* Read the next line */
      const line: string = tableReader.get(i);
      /* Get the columns */
      const parts: AList<string> = Strings.splitAsList(line, Strings.TAB);
      if (parts.size() != Registry._COL_COUNT_UNIT) continue;
      /* Get the parameters */
      const code: int32 = Numbers.getInteger(parts.get(Registry._COL_UNIT_CODE), 0);
      const name: string = parts.get(Registry._COL_UNIT_NAME);
      const type: string = parts.get(Registry._COL_UNIT_TYPE);
      const targetUnitCode: int32 = Numbers.getInteger(parts.get(Registry._COL_UNIT_TARGET_CODE), 0);
      const b: float64 = Numbers.getDouble(parts.get(Registry._COL_UNIT_FACTOR_B), 0.0);
      const c: float64 = Numbers.getDouble(parts.get(Registry._COL_UNIT_FACTOR_C), 0.0);
      const abbreviation: string = parts.get(Registry._COL_UNIT_ABBREVIATION);
      /* Add a new unit */
      const unit: Unit = new Unit(code, name, abbreviation, type, targetUnitCode, b, c);
      units.add(unit);
    }
    /* Return the units */
    return units;
  ...
  public static findUnit(unitCode: int32): Unit {
    const unitList: AList<Unit> = Registry.listUnits();
    for (const unit of unitList) if (unit.getCode() == unitCode) return unit;
    return null;
  }
  ...
  public static getUnit(unitCode: int32): Unit {
    const unit: Unit = Registry.findUnit(unitCode);
  ...
  public static findUnitName(unitName: string): Unit {
    ...
    const unitList: AList<Unit> = Registry.listUnits();
    for (const unit of unitList) if (Strings.equalsIgnoreCase(unitName, unit.getName())) return unit;
    for (const unit of unitList) if (Strings.equalsIgnoreCase(unitName, unit.getAbbreviation())) return unit;
    for (const unit of unitList) if (Strings.equals(unitName, `${  unit.getCode()}`)) return unit;
    return null;
  }
  ...
  public static setCRS(crsCode: int32, crs: CRS): void {
    ...
    const baseCRS: CRS = crs.getBaseCRS();
