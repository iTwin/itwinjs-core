...
export class DownloaderXhr extends Downloader {
	...
	private static readResponseHeaders(request: XMLHttpRequest, responseHeaders: StringMap<string>): void {
		...
		let headerLines: Array<string> = request.getAllResponseHeaders().trim().split(/[\r\n]+/);
		for (let headerLine of headerLines) {
			let parts: Array<string> = headerLine.split(': ');
			let name: string = parts[0];
			let value: string = request.getResponseHeader(name);
