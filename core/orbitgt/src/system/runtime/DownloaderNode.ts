...
export class DownloaderNode extends Downloader {
  ...
  private download0(
    ...
    if (url.protocol === "http:")
      request = http.request(options, (response) => {
        ...
        response.on("end", () => {
          callback(DownloaderNode.joinChunks(chunks, responseType));
        });
        response.on("error", () => {
          errorCallback("error when downloading '" + requestURL + "'");
        });
      });
  ...
  //
  public override async downloadBytes(
    method: string,
    requestURL: string,
    requestHeaders: StringMap<string>,
    postText: string,
    ...
    return new Promise<ABuffer>((resolve, reject) => {
      this.download0(method, requestURL, "arraybuffer", /*responseType*/ requestHeaders, postText, postData, responseHeaders, resolve, reject);
    });
  }

  // Downloader base class method override
  //
  public override async downloadText(
    method: string,
    requestURL: string,
    requestHeaders: StringMap<string>,
    postText: string,
    ...
    return new Promise<string>((resolve, reject) => {
      this.download0(method, requestURL, "text", /*responseType*/ requestHeaders, postText, postData, responseHeaders, resolve, reject);
    });
  }
