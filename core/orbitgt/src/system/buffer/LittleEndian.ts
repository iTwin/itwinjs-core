...
export class LittleEndian {
    ...
    public static readBufferString(buffer: ABuffer, offset: int32): string {
        ...
        offset += 4;
        let value: string = "";
        for (let i: number = 0; i < valueLength; i++) value = Strings.appendChar(value, LittleEndian.readBufferShort(buffer, offset + 2 * i));
        return value;
    }
    ...
    public static readStreamString(stream: InStream): string {
        ...
        if (valueLength < 0) return null;
        let value: string = "";
        for (let i: number = 0; i < valueLength; i++) value = Strings.appendChar(value, LittleEndian.readStreamShort(stream));
        return value;
    }
