        ...
        for (let x: number = 0; x < 2; x++) {
          ...
          if (block == null) {
            ...
            Message.printWarning(
              ViewTree.MODULE,
              "Unable to find tile block " + childIndex + " in child level " + childLevel.getIndex() + " (" + childLevel.getBlockCount() + " blocks)",
            );
            complete = false;
            continue;
          }
  ...
  private addTilesToView(
    ...
    for (let i: number = 0; i < tiles.length; i++) {
      ...
      if (ViewTree.DEBUG)
        Message.print(
          ViewTree.MODULE,
          "Checking tile L" + tile.level + " (" + tile.gridIndex.x + "," + tile.gridIndex.y + "," + tile.gridIndex.z + ")",
        );
      /* Visible? */
  ...
  public renderView3D(
    ...
    for (let i: number = 0; i < this._rootBlocks.size(); i++) { // start from the root tiles
      ...
      if (ViewTree.DEBUG)
        Message.print(
          ViewTree.MODULE,
          "Checking top-level block L" + blockIndex.level + " (" + blockIndex.gridIndex.x + "," + blockIndex.gridIndex.y + "," +
