...
export class FileReader {
  ...
  private async open(lazyLoading: boolean): Promise<FileReader> {
    ...
    Message.print(
      FileReader.MODULE,
      "Opening OPC with " + this._fileRecord.getLevelCount() + " levels (crs " + this._fileRecord.getCRS() + ", lazy? " + lazyLoading + ")",
    );
    ...
    Message.print(
      FileReader.MODULE,
      "Loading " + this._fileRecord.getLevelCount() + " levels and " + this._fileRecord.getAttributeCount() + " attributes",
    );
    ...
    for (let i: number = 0; i < this._fileRecord.getLevelCount(); i++) {
      ...
      Message.print(
        FileReader.MODULE,
        "Level " + i + " has " + directoryRecord.getPointCount() + " points, " + directoryRecord.getTileCount() + " tiles, " +
          directoryRecord.getBlockCount() + " blocks",
      );
