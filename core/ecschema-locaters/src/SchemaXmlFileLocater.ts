...
export class SchemaXmlFileLocater extends SchemaFileLocater implements ISchemaLocater {
  ...
  public async getSchema<T extends Schema>(
    ...
    // Load the file
    if (undefined === (await this.fileExists(schemaPath))) return undefined;

    const schemaText = await this.readUtf8FileToString(schemaPath);
    if (undefined === schemaText) return undefined;
  ...
  public getSchemaSync<T extends Schema>(
    ...
    // Load the file
    if (!this.fileExistsSync(schemaPath)) return undefined;
