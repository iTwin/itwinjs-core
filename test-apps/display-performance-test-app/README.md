...
If you wish to specify multiple saved views in the same model, you can use a asterisk wildcard instead of a specific view name to test multiple views matching the given wildcard. For example, to test all views, set viewName to "*". To test all views with the word 'edge' in them, set viewName to "*edge" or "edge*". Additionally, you can use the savedViewType setting to "external", "internal", or "both" to specify if you only want to test all of the external saved views, local saved views, or both the local and external saved views. If nothing is specified, the savedViewType will default to using both sets of saved views.
