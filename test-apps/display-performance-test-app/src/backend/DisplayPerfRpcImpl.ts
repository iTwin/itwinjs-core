...
export default class DisplayPerfRpcImpl extends DisplayPerfRpcInterface {
  ...
  public override async getDefaultConfigs(): Promise<string> {
    ...
    process.argv.forEach((arg, index) => {
      if (
        ...
        while (arg.endsWith("\\") || arg.endsWith("/")) arg = arg.slice(0, -1);
  ...
  public override async writeExternalFile(
    ...
    if (!append && fs.existsSync(fileName)) fs.unlinkSync(fileName);

    if (append) fs.appendFileSync(fileName, content);
