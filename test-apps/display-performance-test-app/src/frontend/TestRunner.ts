...
class Timings {
  ...
  public constructor(numFramesToCollect: number) {
    this.callback = (result: GLTimerResult) => {
      if (this.gpuFramesCollected >= numFramesToCollect) return;
...
class OverrideProvider {
  ...
  private constructor(ovrs: ElementOverrideProps[]) {
    for (const ovr of ovrs) {
      ...
      if (ovr.id === "-default-") this._defaultOvrs = app;
      else this._elementOvrs.set(ovr.id, app);
    }
  }
...
export class TestRunner {
  ...
  public async run(): Promise<void> {
    ...
    // Run all the tests
    for (const set of this._testSets) await this.runTestSet(set);
  ...
  private async runTestSet(set: TestSetProps): Promise<void> {
    ...
    for (const testProps of set.tests) {
      if (this._config.push(testProps)) needRestart = true;
  ...
  private async runTest(context: TestContext): Promise<TestResult | undefined> {
    ...
    document.title = "Display Performance Test App:  ".concat(
      testConfig.iModelName ?? "",
      "  [",
      testConfig.viewName ?? "",
      "]"
    );
  ...
  private async setupTest(context: TestContext): Promise<TestCase | undefined> {
    ...
    if (config.hilite) viewport.hilite = config.hilite;

    if (config.emphasis) viewport.emphasisSettings = config.emphasis;

    // Apply display style.
    if (config.displayStyle) {
      const styleProps = await imodel.elements.queryProps({
        from: DisplayStyleState.classFullName,
  ...
  private async loadView(
    ...
    const ids = await context.iModel.elements.queryIds({
      from: ViewState.classFullName,
      where: `CodeValue='${config.viewName}'`,
    });
      ...
      if (esv) {
        ...
        externalSavedViews = json.map((x) => {
          return {
            ...
            elementOverrides: x._overrideElements
              ? (JSON.parse(x._overrideElements) as ElementOverrideProps[])
              : undefined,
            selectedElements: x._selectedElements
              ? (JSON.parse(x._selectedElements) as Id64String | Id64Array)
              : undefined,
          };
        });
      }
  ...
  private async getViewNames(context: TestContext): Promise<string[]> {
    ...
    if (this.curConfig.savedViewType !== "external") {
      const specs = await context.iModel.views.getViewList({
        wantPrivate: true,
      });
  ...
  private async finish(): Promise<void> {
    ...
    await DisplayPerfRpcInterface.getClient().finishCsv(
      renderData,
      this.curConfig.outputPath,
      this.curConfig.outputName,
  ...
  private async logToFile(
    ...
    return DisplayPerfRpcInterface.getClient().writeExternalFile(
      this.curConfig.outputPath,
      this._logFileName,
      append,
      message
  ...
  private getTestName(
    ...
    if (renderMode) testName += `_${renderMode}`;
  ...
  private getImageName(test: TestCase, prefix?: string): string {
    ...
    if (ProcessDetector.isMobileAppFrontend) return filename; // on mobile we use device's Documents path as determined by mobile backend
  ...
  private getRowData(
    ...
    const setGpuData = (name: string) => {
      if (name === "CPU Total Time") name = "Total";

      const gpuDataArray = timings.gpu.get(name);
      if (gpuDataArray) {
        let gpuSum = 0;
        for (const gpuData of gpuDataArray) gpuSum += gpuData;
    ...
    const cpuBound = disjointTimerUsed
      ? totalRenderTime - totalGpuTime > gpuTolerance && totalRenderTime > 2
      : !gpuBound;
    let boundBy = "";
    if (totalRenderTime < 2 && !gpuBound)
      // ie total cpu time < 2ms && !gpuBound
      boundBy = "unmeasurable";
    else if (!gpuBound && !cpuBound) boundBy = "unknown";
    else if (gpuBound) boundBy = "gpu";
    else boundBy = "CPU";
    if (1000.0 / totalTime > 59)
      // ie actual fps > 60fps - 1fps tolerance
      boundBy += " (vsync)";
...
function removeOptsFromString(
  ...
  let output = input;
  if (!(ignore instanceof Array)) ignore = ignore.split(" ");
...
function hiliteSettingsStr(settings: Hilite.Settings): string {
  let hsStr = (
    ...
    .padStart(5, "0");
  hsStr += (
    settings.silhouette * 256 * 256 +
    Math.round(settings.visibleRatio * 255) * 256 +
    Math.round(settings.hiddenRatio * 255)
  )
    .toString(36)
    .padStart(4, "0");
...
function getViewFlagsString(test: TestCase): string {
  ...
  for (const propName of Object.keys(vf)) {
    ...
    if (vf[key] === includeIf) vfString += abbrev;
  }

  if (undefined !== test.view.elementOverrides) vfString += "+ovrEl";

  if (undefined !== test.view.selectedElements) vfString += "+selEl";
