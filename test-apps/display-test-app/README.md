...
* IMJS_AUTH_SERVER_ALLOWLIST
  * Electron only. If defined, a comma-separated list of servers for which Chromium is allowed to perform integrated authentication (Kerberos/NTLM SSO), e.g. `*.example.com`. Without this, Chromium silently ignores `Negotiate`/`NTLM` challenges from servers outside the intranet zone and map layer SSO fails with HTTP 401.
...
* IMJS_MAP_LAYER_AUTH_HEADER
  * If defined, a `Header-Name=value` pair (e.g. `Authorization=Bearer my-test-token`, split on the first `=`). display-test-app registers a map-layer fetch handler (`MapLayerFormatRegistry.addMapLayerFetchHandler`) for IMJS_MAP_LAYER_AUTH_FORMATS. The header is injected when the target shares the layer URL's HTTP(S) origin, or its origin is listed in IMJS_MAP_LAYER_AUTH_ORIGINS, for testing request-shaping authentication (e.g. services behind an authenticating proxy).
* IMJS_MAP_LAYER_AUTH_QUERY_PARAMS
  * If defined, a comma-separated list of `name=value` pairs (e.g. `token=my-test-token,client=dta`). Injected as query parameters using the same format and origin rules as IMJS_MAP_LAYER_AUTH_HEADER; either variable may be used alone or both together.
* IMJS_MAP_LAYER_AUTH_FORMATS
  * A comma-separated list of map-layer format ids (e.g. `WMS,WMTS,ArcGIS`) to which the IMJS_MAP_LAYER_AUTH_HEADER/IMJS_MAP_LAYER_AUTH_QUERY_PARAMS fetch handler applies. Required: without it, no handler is registered.
* IMJS_MAP_LAYER_AUTH_ORIGINS
  * An optional comma-separated list of additional exact HTTP(S) destination origins (e.g. `https://tiles.corp.example.com:8443`) allowed to receive the injected header/query parameters. Same-origin requests are always allowed without an entry. When omitted or empty, only same-origin requests receive the injected values. Invalid entries are ignored; wildcards, credentials, paths other than `/`, queries, and fragments are not accepted. Relative target and layer URLs are resolved against the document's base URL; origin comparison includes scheme, host, and port.
  * WMTS resource templates and OGC collection links can advertise other hosts. Cross-origin destinations not in this list are left to other handlers or the default request path **without these injected values**. The same configured values are sent to every attached layer of a selected format on its own origin, so attach only layers intended to receive those values. This list does not grant SSO/basic-auth trust. To also prevent injected values from following redirects, enable the existing restriction using IMJS_MAP_LAYER_TRUSTED_CREDENTIALS_ORIGINS; the handler's direct-destination checks apply even when that restriction is disabled.
