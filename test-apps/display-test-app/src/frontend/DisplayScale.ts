...
export class ApplyModelDisplayScaleTool extends Tool {
  ...
  public override async run(scale: Point3d): Promise<boolean> {
    ...
    if (
      ...
      const scaleFrustumInvTf = Transform.createRefs(
        Point3d.createZero(),
        Matrix3d.createScale(inverseMax, inverseMax, inverseMax)
      );
    ...
    const scaleFrustumTf = Transform.createRefs(
      Point3d.createZero(),
      Matrix3d.createScale(maxScale, maxScale, maxScale)
    );
