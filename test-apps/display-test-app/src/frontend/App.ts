...
function createHubAccess(configuration: DtaConfiguration) {
  if (configuration.urlPrefix) {
    return new FrontendIModelsAccess(
      new IModelsClient({
        api: {
          baseUrl: `https://${configuration.urlPrefix}api.bentley.com/imodels`,
        },
      })
    );
  } else {
...
export class DisplayTestApp {
  ...
  public static async startup(
    ...
    if (realityApiKey) registerRealityDataSourceProvider(realityApiKey);
