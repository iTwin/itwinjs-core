...
export async function signIn(): Promise<boolean> {
  ...
  if (undefined !== auth && (auth instanceof BrowserAuthorizationClient || auth instanceof ElectronAuthorizationRenderer)) {
...
export async function signOut(): Promise<void> {
  const auth = IModelApp.authorizationClient;
  if (auth instanceof ElectronAuthorizationRenderer || auth instanceof BrowserAuthorizationClient)
    return auth.signOut();
}
