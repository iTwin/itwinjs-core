...
import { SectionDrawingApi } from "./SectionDrawingApi";
  ...
  export class ViewSetupApi {
    ...
    public static setViewState(viewport: Viewport, settings: Partial<ViewStateSettings>) {
      ...
      else (viewport.displayStyle.settings as DisplayStyle3dSettings).hiddenLineSettings = HiddenLine.Settings.defaults;
  
      if (settings.backgroundColor) viewport.view.displayStyle.backgroundColor = settings.backgroundColor;
    ...
    public static async changeViewportView2d(iModel: IModelConnection, newModelId: string, fitView?: boolean) {
      ...
      if (sectionDrawings.length > 0) {
        ...
        if (vp && targetView) {
          ...
          if (fitView) void IModelApp.tools.run(FitViewTool.toolId, vp, true, false);
        } else {
    ...
    public static getAspectRatio(): number | undefined {
      ...
      if (null === viewDiv) return undefined;
