...
export class EditingScopeTool extends Tool {
  ...
  private async _run(): Promise<void> {
    ...
    if (!imodel || !imodel.isBriefcaseConnection()) return;

    const scope = imodel.editingScope;
    if (scope) await scope.exit();
    else await imodel.enterEditingScope();
...
export class PlaceLineStringTool extends CreateElementTool {
  ...
  protected override setupAndPromptForNextAction(): void {
    ...
    if (0 !== nPts) {
      ...
      if (
        nPts > 1 &&
        !this._points[nPts - 1].isAlmostEqual(this._points[nPts - 2])
      )
        hints.setXAxis(
  ...
  public override getDecorationGeometry(
    ...
    if (this._points.length < 2) return undefined;

    const geom = IModelJson.Writer.toIModelJson(
      LineString3d.create(this._points)
    );
  ...
  public override onDynamicFrame(
    ...
    builder.addLineString([
      this._points[this._points.length - 1].clone(),
      ev.point.clone(),
    ]);
  ...
  protected async createElement(): Promise<void> {
    ...
    try {
      ...
      if (this._testGeomJson) {
        ...
        if (!builder.appendGeometry(primitive)) return;

        if (this._testGeomParts) {
          const partBuilder = new GeometryStreamBuilder();
          const sphere = Sphere.createCenterRadius(
            Point3d.createZero(),
            this._points[0].distance(this._points[1]) * 0.05
          );

          if (!partBuilder.appendGeometry(sphere)) return;
          ...
          const partId = await basicManipulationIpc.insertGeometryPart(
            partProps
          );

          for (const pt of this._points) {
            if (!builder.appendGeometryPart3d(partId, pt)) return;
          }
        }
        ...
        builder.setLocalToWorld3d(origin, angles); // Establish world to local transform...
        if (!builder.appendGeometryQuery(primitive)) return;
        ...
        if (this._testGeomParts) {
          ...
          if (!partBuilder.appendGeometryQuery(sphere)) return;
          ...
          const partId = await basicManipulationIpc.insertGeometryPart(
            partProps,
            { entryArray: partBuilder.entries }
          );
      ...
      IModelApp.notifications.outputMessage(
        new NotifyMessageDetails(OutputMessagePriority.Error, err.toString())
      );
    }
  }
  ...
  public override async onResetButtonUp(
    ...
    if (this._points.length >= 2) await this.createElement();
  ...
  public override async onUndoPreviousStep(): Promise<boolean> {
    if (0 === this._points.length) return false;

    this._points.pop();
    if (0 === this._points.length) await this.onReinitialize();
