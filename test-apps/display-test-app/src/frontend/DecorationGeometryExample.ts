...
class GeometryDecorator {
  ...
  public decorate(context: DecorateContext): void {
    if (context.viewport.iModel !== this._iModel) return;
  ...
  private addCone(cx: number, cy: number = 0): void {
    const cone = Cone.createAxisPoints(
      new Point3d(cx + 0.5, cy + 0.5, 0),
      new Point3d(cx + 0.5, cy + 0.5, 1),
      0.5,
      0.25,
      true
    );
    if (cone) this.addDecorator((builder) => builder.addSolidPrimitive(cone));
  }
...
export async function openDecorationGeometryExample(viewer: Viewer) {
  ...
  const texture = IModelApp.renderSystem.createTexture({
    image: { source: txrEl, transparency: TextureTransparency.Opaque },
    ownership: "external",
  });
