...
/**
 * Key-in that demonstrates the new backend-to-frontend Ipc invoke support. There are two hops here, only the
 * second of which is new:
 *  1. Frontend-to-backend RPC (pre-existing mechanism) - the only way this frontend-triggered key-in can ask the
 *     backend to do anything, since the backend cannot initiate work on its own.
 *  2. Inside `DisplayTestAppRpc.invokeFrontendIpc` on the backend: backend-to-frontend Ipc invoke (the new
 *     feature) - `dtaFrontendIpc.getFrontendInfo()`, a proxy created via `IpcHost.makeIpcProxy`, calls back into
 *     this frontend's `DtaFrontendIpcHandler` and awaits its return value.
 * The value displayed below is the frontend-computed data that made the full round trip: frontend --RPC--> backend
 * --IpcHost.invoke--> frontend --(RPC response)--> frontend.
 */
export class InvokeFrontendIpcTool extends Tool {
  public static override toolId = "InvokeFrontendIpc";

  public override async run(): Promise<boolean> {
    // Hop 1: ordinary frontend-to-backend RPC, just the trigger for the demo below.
