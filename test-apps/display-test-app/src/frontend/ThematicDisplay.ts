...
const defaultSettings: Required<ThematicDisplayProps> = {
  ...
  axis: [0.0, 0.0, 1.0],
  range: [0, 1],
  sunDirection: calculateSolarDirectionFromAngles({
    azimuth: 315.0,
    elevation: 45.0,
  }).toJSON(),
  sensorSettings: {
...
export class ThematicDisplayEditor {
  ...
  private _createSensorGrid(sensors: ThematicDisplaySensorProps[]) {
    ...
    for (let y = 0; y < sensorGridYLength; y++) {
      ...
      for (let x = 0; x < sensorGridXLength; x++) {
        ...
        this._pushNewSensor(sensors, {
          position: sensorPos,
          value: sensorValues[sensorValueIndex],
        });

        sensorValueIndex++;
        if (sensorValueIndex >= sensorValues.length) sensorValueIndex = 0;
      }
    }
  }
  ...
  private static _gradientModeEntriesForHeight = [
    ...
    {
      name: "SteppedWithDelimiter",
      value: ThematicGradientMode.SteppedWithDelimiter,
    },
  ...
  public constructor(vp: Viewport, parent: HTMLElement) {
    ...
    const displayModeEntries = [
      ...
      {
        name: "InverseDistanceWeightedSensors",
        value: ThematicDisplayMode.InverseDistanceWeightedSensors,
      },
  ...
  private updateThematicDisplayUI(view: ViewState) {
    ...
    if (ThematicDisplayMode.Height === displayMode) {
      ThematicDisplayEditor._setComboBoxEntries(
        ...
        ThematicDisplayEditor._gradientModeEntriesForHeight
      );
    } else {
      ThematicDisplayEditor._setComboBoxEntries(
        this._thematicGradientMode,
        ThematicDisplayEditor._gradientModeEntriesForOthers
      );
    }
