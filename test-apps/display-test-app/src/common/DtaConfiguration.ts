...
export interface DtaStringConfiguration {
  ...
  mapLayerAuthHeader?: string; // default is undefined; a "Header-Name=value" pair injected for mapLayerAuthFormats at the layer's own origin or an additional mapLayerAuthOrigins destination. See README.md.
  mapLayerAuthQueryParams?: string; // default is undefined; comma-separated "name=value" pairs injected for mapLayerAuthFormats at the layer's own origin or an additional mapLayerAuthOrigins destination. See README.md.
  mapLayerAuthFormats?: string; // default is undefined; comma-separated list of map-layer format ids the IMJS_MAP_LAYER_AUTH_HEADER/IMJS_MAP_LAYER_AUTH_QUERY_PARAMS fetch handler applies to. Required for the handler to be registered.
  mapLayerAuthOrigins?: string; // optional comma-separated exact HTTP(S) origins allowed in addition to the layer's own origin; independent of SSO trust.
}
...
export const getConfig = (): DtaConfiguration => {
  ...
  configuration.mapLayerAuthOrigins = process.env.IMJS_MAP_LAYER_AUTH_ORIGINS;
