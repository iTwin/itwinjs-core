...
import { AbstractStatusBarItemUtilities, CommonStatusBarItem, CommonToolbarItem, StageUsage, StatusBarSection, ToolbarItemUtilities, ToolbarOrientation, ToolbarUsage, UiItemsProvider } from "@bentley/ui-abstract";
...
export class SampleUiItemsProvider implements UiItemsProvider {
  ...
  public provideToolbarButtonItems(_stageId: string, stageUsage: string, toolbarUsage: ToolbarUsage, toolbarOrientation: ToolbarOrientation): CommonToolbarItem[] {
    if (stageUsage === StageUsage.General && toolbarUsage === ToolbarUsage.ContentManipulation && toolbarOrientation === ToolbarOrientation.Horizontal) {
      const simpleActionSpec = ToolbarItemUtilities.createActionButton("uiTestExtension:sampleTool", 1000, `svg:${sampleToolButtonSvg}`, "Sample Tool", this.startSampleTool);
  ...
  public provideStatusBarItems(_stageId: string, stageUsage: string): CommonStatusBarItem[] {
    ...
    if (stageUsage === StageUsage.General) {
      statusBarItems.push(
        AbstractStatusBarItemUtilities.createActionItem("UiTestExtension:UnitsStatusBarItem", StatusBarSection.Center, 100, `svg:${statusBarButtonSvg}`, SampleTool.i18n.translate("uiTestExtension:StatusBar.UnitsFlyover"),
          () => {
