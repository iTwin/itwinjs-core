...
export class RealityModelContextIModelCreator {
  ...
  public async create(): Promise<void> {
    ...
    requestPromise(this.url, { json: true }).then((json: any) => {
      ...
      if (undefined === rootTransform || undefined === range)
        return;
      const ecefRange = rootTransform.multiplyRange(range);
      ...
      const worldRange = AxisAlignedBox3d.fromJSON(ecefToWorld.multiplyRange(ecefRange));

      this.insertSpatialView("Test View", worldRange, { tilesetUrl: this.url, name: this.url });
  ...
  protected insertSpatialView(viewName: string, range: AxisAlignedBox3d, realityModel: ContextRealityModelProps): Id64String {
