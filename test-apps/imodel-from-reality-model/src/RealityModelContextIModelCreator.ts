...
export class RealityModelContextIModelCreator {
  ...
  private realityModelFromJson(
    json: any,
    worldRange: AxisAlignedBox3d
  ): {
    realityModel: ContextRealityModelProps | undefined;
    geoLocated: boolean;
  } {
    let geoLocated = true;
    ...
    return {
      realityModel: {
        tilesetUrl: this.url,
        name: this._name ? this._name : this.url,
      },
      geoLocated,
    };
  }
  ...
  public async create(): Promise<void> {
    ...
    if (this.url.endsWith("_AppData.json")) {
      ...
      for (const modelValue of Object.values(json.models)) {
        ...
        if (model.tilesetUrl !== undefined && model.type === "spatial") {
      ...
      if (result.realityModel) {
        ...
        if (result.geoLocated) geoLocated = true;
