 ...
 * ----------------------------------------------- */
  ...
  public static messageHandler(event: MessageEvent) {
    if (!event.data.hasOwnProperty("msgId")) {
      throw new Error("Improperly formatted message: msgId property needed");
    }
    if (!event.data.hasOwnProperty("operation")) {
      throw new Error("Improperly formatted message: operation property needed");
    }
    if (!event.data.hasOwnProperty("operands")) {
      throw new Error("Improperly formatted message: operands array property needed");
    }
    const operation = event.data["operation"];
    const operands = event.data["operands"];
    const result = (TestWebWorker as any)[operation].apply(TestWebWorker, operands);
