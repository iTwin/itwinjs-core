...
class Tool1UiProvider extends ToolUiProvider {
  ...
  private getHorizontalToolSettings(): ToolSettingsEntry[] | undefined {
    return [
      ...
      { labelNode: IModelApp.localizationProvider.getLocalizedString("SampleApp:tool1.date"), editorNode: <input type="date" /> },
      { labelNode: IModelApp.localizationProvider.getLocalizedString("SampleApp:tool1.number"), editorNode: <input type="number" min="10" max="20" /> },
      { labelNode: IModelApp.localizationProvider.getLocalizedString("SampleApp:tool1.password"), editorNode: <input type="password" /> },
      { labelNode: <label htmlFor="radio">{IModelApp.localizationProvider.getLocalizedString("SampleApp:tool1.radio")}</label>, editorNode: <input name="radio" type="radio" /> },
      { labelNode: <label htmlFor="range">{IModelApp.localizationProvider.getLocalizedString("SampleApp:tool1.range")}</label>, editorNode: <input name="range" type="range" min="1" max="100" step="5" /> },
      { labelNode: <label htmlFor="month">{IModelApp.localizationProvider.getLocalizedString("SampleApp:tool1.month")}</label>, editorNode: <input name="month" type="month" /> },
    ];
  }
}
