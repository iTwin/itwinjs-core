...
5. To control the Cesium camera you can use the following keyboard shortcuts:
  a. Press `ctrl` + `w`, `a`, `s`, `d` to move in and out and rotate
  b. Press `ctrl` + arrow keys to pan
6. To control the camera using the default iTwin.js mouse controls, you need to make some small code changes:
  a. Replace the following line in core/frontend/src/tools/EventController.ts which adds listeners for DOM events:
...
```
  ...
  b. To the following if statement in core/frontend/src/tools/ToolAdmin.ts, in the `ToolAdmin.tryReplace()` method:
...
```
  ...
  c. Also, add the following cases to the switch statement in `ToolAdmin.processNextEvent()` to call `ToolAdmin` methods for pointer events:
