...
import { GeoPhotos, GeoPhotoExtension, GeoPhotoSettings } from "./geoPhoto";
...
class GeoPhotoMarker extends Marker {
  ...
  private hotSpotSetStyle(distance: number, hs: PannellumHotSpot) {
    let maxDistance = this._manager.extension.settings.maxDistance;
  ...
  private saveCorrections(correctedFile: PhotoFile, correctionMatrix: Matrix3d | undefined) {
    ...
    if (result) {
      ...
      correctedFile.correctionDir = GeoPhotoMarker.getCameraDirection(correctedFile, this._manager.extension);
      correctedFile.saveFileInfo().then(() => { }).catch((_err) => { console.log("Error saving correction "); });
    }
  }

  // gets camera direction from marker settings.
  public static getCameraDirection(file: PhotoFile, extension: GeoPhotoExtension): number {
    let direction = 0;
    if (extension.settings.directionFromPath && file.pathOrientation) {
      direction = extension.settings.reversed ? file.pathOrientation + 180.0 : file.pathOrientation;
    } else if (file.gpsTrack) {
      direction = extension.settings.reversed ? file.gpsTrack + 180.0 : file.gpsTrack;
    }
    if (direction > 360.0)
      direction -= 360.0;
    return direction;
  ...
  private closeEnoughToCameraPath(centerPoint: Point3d, nextPoint: Point3d | undefined, previousPoint: Point3d | undefined, thisPoint: Point3d): boolean {
    ...
    const settings = this._manager.extension.settings;
    ...
    return crossDistance < this._manager.extension.settings.maxCrossDistance;
  }
  ...
  private calculateHotSpotsFromMatrix(closeNeighbors: Neighbor[], cameraDirection: number, correctionMatrix: Matrix3d, hotSpots: PannellumHotSpot[] | undefined): PannellumHotSpot[] {
    ...
    const eyeHeight = this._manager.extension.settings.eyeHeight;
  ...
  private calculateHotSpotChanges(panoFile: PhotoFile, closeNeighbors: Neighbor[], baseRotation: Matrix3d, existingHotSpots: PannellumHotSpot[], _centerYaw: number, yawChange: number, pitchChange: number): Matrix3d | undefined {
    const cameraDirection = GeoPhotoMarker.getCameraDirection(panoFile, this._manager.extension);
  ...
  private getMarkerDisplaySettings(_photoFile: PhotoFile): MarkerDisplaySettings {
    const settings: GeoPhotoSettings = this._manager.extension.settings!;
  ...
  private setMarkerDisplaySettings(_photoFile: PhotoFile, whichButton: number): boolean {
    ...
    switch (whichButton) {
      case 1:
        ...
        if (this._manager.extension.uiProvider)
          this._manager.extension.uiProvider.syncShowMarkers();
        break;
      case 2:
        // fromTrack button;
  ...
  private async getViewerData(photoFile: PhotoFile): Promise<ViewerData | undefined> {
    const maxDistance = this._manager.extension.settings.maxDistance;
    ...
    try {
      ...
      if ((baseRotation !== undefined) && closeFiles.length > 0) {
        const cameraDirection = GeoPhotoMarker.getCameraDirection(photoFile, this._manager.extension);
        ...
        if (this.photoFile.isPanorama) {
          ...
          if (0 !== (ev.keyModifiers & (BeModifierKeys.Shift | BeModifierKeys.Control))) {
            ...
            try {
              ...
              if (viewerData) {
                await PannellumModalFrontstage.open(viewerData.panoBlob, viewerData.photoFile, viewerData.config, this._manager.extension);
              }
            } catch (error) {
...
export class GeoPhotoMarkerManager implements Decorator {
  ...
  constructor(public extension: GeoPhotoExtension, private _geoPhotos: GeoPhotos) {
    this._jpgSvg = imageElementFromUrl(this.extension.resolveResourceUrl("jpeg_pin.svg"));
    this._panoSvg = imageElementFromUrl(this.extension.resolveResourceUrl("pano_pin.svg"));
    this.visitedSvg = imageElementFromUrl(this.extension.resolveResourceUrl("visited_pin.svg"));
