...
export class EsriSFS implements EsriSymbol {
  ...
  constructor(json: EsriSFSProps) {
    this.props = json;
    if (json.outline) this._outline = EsriSLS.fromJSON(json.outline);
  }
...
export class ArcGisSymbologyRenderer {
  ...
  public applyStrokeStyle(context: CanvasRenderingContext2D) {
    ...
    if (sls) {
      if (sls.color) context.strokeStyle = sls.color.toRgbaString();
  ...
  public drawPoint(
    ...
    if (this._symbol?.type === "esriPMS") {
      ...
      let xOffset = 0,
        yOffset = 0;
      if (pms.xoffset) xOffset = pms.xoffset;
      else if (pms.width) xOffset = pms.width * -0.5; // if no offset center in the middle

      if (pms.yoffset) yOffset = pms.yoffset;
