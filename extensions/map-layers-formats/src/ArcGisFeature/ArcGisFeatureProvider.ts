...
import {
  ...
  ArcGisFeatureQuery,
  ArcGisGeometry,
  FeatureQueryQuantizationParams,
...
export class ArcGisFeatureProvider extends ArcGISImageryProvider {
  ...
  public override get minimumZoomLevel(): number {
    return this._minDepthFromLod;
  }
  public override get maximumZoomLevel(): number {
    return this._maxDepthFromLod;
  }
  ...
  public override async initialize(): Promise<void> {
    ...
    if (!foundVisibleSubLayer && json !== undefined) {
      ...
      if (
        Array.isArray(this.serviceJson.layers) &&
        this.serviceJson.layers.length >= 1
      ) {
        const hasDefaultVisibility = Object.keys(
          this.serviceJson.layers[0]
        ).includes("defaultVisibility");
    ...
    if (
      majorVersion &&
      majorVersion >= 11 &&
      this._layerMetadata.supportsCoordinatesQuantization
    ) {
    ...
    if (this._layerMetadata.supportedQueryFormats) {
      const formats: string[] =
        this._layerMetadata.supportedQueryFormats.split(", ");
    ...
    if (!this._format) {
      Logger.logError(
        loggerCategory,
        "Could not get request format from service JSON"
      );
      throw new ServerError(IModelStatus.ValidationFailed, "");
    }
    ...
    const scales = ArcGisUtilities.getZoomLevelsScales(
      this.defaultMaximumZoomLevel,
      this.tileSize,
      minScale,
      maxScale,
      1.0
    );
    if (scales.minLod) this._minDepthFromLod = scales.minLod;
    ...
    this._symbologyRenderer = new ArcGisSymbologyRenderer(
      this._layerMetadata?.geometryType,
      this._layerMetadata?.drawingInfo?.renderer
    );
  }
  ...
  public get format(): ArcGisFeatureFormat | undefined {
    return this._format;
  }
  ...
  public constructFeatureUrl(
    row: number,
    column: number,
    zoomLevel: number,
    format: ArcGisFeatureFormat,
    geomOverride?: ArcGisGeometry,
    outFields?: string,
    tolerance?: number,
    returnGeometry?: boolean
  ): ArcGisFeatureUrl | undefined {
    const tileExtent = this.getEPSG3857Extent(row, column, zoomLevel);
    const tileEnvelope = {
      xmin: tileExtent.left,
      ymin: tileExtent.bottom,
      xmax: tileExtent.right,
      ymax: tileExtent.top,
  ...
  public override async getFeatureInfo(
    ...
    if (!this._querySupported || this.format === undefined) return;
    ...
    const doFeatureInfoQuery = async (
      format: ArcGisFeatureFormat,
      outFields?: string,
      returnGeometry?: boolean
    ) => {
      const infoUrl = this.constructFeatureUrl(
        quadId.row,
        quadId.column,
        quadId.level,
        format,
        { geom: cartoPoint, type: "esriGeometryPoint" },
        outFields,
        3 /* tolerance in pixel*/,
        returnGeometry
      );
      ...
      if (!infoUrl || infoUrl.url.length === 0) {
        Logger.logError(
          loggerCategory,
          `Could not construct feature info query URL`
        );
        return undefined;
      }
    ...
    if (this._debugFeatureGeom) {
      try {
        ...
        if (responseData) {
          Logger.logInfo(
            loggerCategory,
            JSON.stringify(responseData.data.toObject())
          );
        }
        ...
        Logger.logInfo(
          loggerCategory,
          `Error occured with debug FeatureInfo: ${e}`
        );
      }
    }
    ...
    try {
      ...
      if (responseData.exceedTransferLimit) {
        Logger.logError(
          loggerCategory,
          `Could not get feature info : transfert limit exeeded.`
        );
        return;
      }
      const featureReader: ArcGisFeatureReader =
        this.format === "PBF"
          ? new ArcGisFeaturePBF(this._settings, this._layerMetadata)
          : new ArcGisFeatureJSON(this._settings, this._layerMetadata);
      featureReader.readFeatureInfo(responseData, featureInfos);
    } catch (e) {
      Logger.logError(
        loggerCategory,
        `Exception occured while loading feature info data : ${e}`
      );
      return;
    }
  ...
  private async fetchTile(
    ...
    column: number,
    zoomLevel: number,
    refineEnvelope?: ArcGisExtent
  ): Promise<ArcGisFeatureResponse | undefined> {
    if (!this.format) {
      return undefined;
    }

    const geomOverride: ArcGisGeometry | undefined = refineEnvelope
      ? { geom: refineEnvelope, type: "esriGeometryEnvelope" }
      : undefined;
    const tileUrl = this.constructFeatureUrl(
      row,
      column,
      zoomLevel,
      this.format,
    ...
    if (!tileUrl || tileUrl.url.length === 0) {
      Logger.logError(
        loggerCategory,
        `Could not construct feature query URL for tile ${zoomLevel}/${row}/${column}`
      );
      return undefined;
    }
  ...
  protected computeTileWorld2CanvasTransform(
    row: number,
    column: number,
    zoomLevel: number
  ) {
    ...
    const worldTileOrigin = Point3d.create(
      tileExtentWorld3857.left,
      tileExtentWorld3857.bottom
    );
    const worldTileExtent = Point3d.create(
      tileExtentWorld3857.right,
      tileExtentWorld3857.top
    );
    ...
    const matrix = Matrix4d.createTranslationAndScaleXYZ(
      xTranslate,
      yTranslate,
      0,
      world2CanvasRatio,
  ...
  public override async loadTile(
    ...
    if (ctx == null) {
      Logger.logError(
        loggerCategory,
        "No canvas context available for loading tile."
      );
      return undefined;
    }
    ...
    if (!this._symbologyRenderer) {
      Logger.logError(
        loggerCategory,
        "No symbology renderer available for loading tile."
      );
      return undefined;
    }
    try {
      ...
      if (!this._supportsCoordinatesQuantization) {
        ...
        if (!transfo) {
          Logger.logError(
            loggerCategory,
            `Could not compute data transformation for tile (${zoomLevel}/${row}/${column})`
          );
        }
      }

      const renderer = new ArcGisFeatureRenderer(
        ctx,
      ...
      const getSubEnvelopes = (envelope: ArcGisExtent): ArcGisExtent[] => {
        ...
        for (let posX = 0; posX < ArcGisFeatureProvider._nbSubTiles; posX++) {
          for (let posY = 0; posY < ArcGisFeatureProvider._nbSubTiles; posY++) {
            subEnvelopes.push({
              xmin: envelope.xmin + dx * posX,
              ymin: envelope.ymin + dy * posY,
              xmax: envelope.xmin + dx * (posX + 1),
              ymax: envelope.ymin + dy * (posY + 1),
              spatialReference: { wkid: 102100, latestWkid: 3857 },
            });
          }
        }
      ...
      const renderData = async (envelope?: ArcGisExtent) => {
        ...
        try {
          ...
          if (!response) {
            Logger.logError(
              loggerCategory,
              `Error occurred while fetching tile (${zoomLevel}/${row}/${column})`
            );
            return;
          }
          ...
          if (!responseData) {
            Logger.logError(
              loggerCategory,
              `Could not get response data for tile (${zoomLevel}/${row}/${column})`
            );
            return;
          }
        } catch (e) {
          Logger.logError(
            loggerCategory,
            `Exception occurred while loading tile (${zoomLevel}/${row}/${column}) : ${e}`
          );
          return;
        }
        ...
        if (responseData.exceedTransferLimit) {
          if (response.envelope) {
            ...
            Logger.logError(
              loggerCategory,
              `Request exceeded transfer limit, could not refine request`
            );
          }
        } else {
    ...
    try {
      ...
      return new ImageSource(
        base64StringToUint8Array(dataUrl2),
        ImageSourceFormat.Png
      );
    } catch (e) {
      Logger.logError(
        loggerCategory,
        `Exception occurred while rendering tile (${zoomLevel}/${row}/${column}) : ${e}.`
      );
    }
