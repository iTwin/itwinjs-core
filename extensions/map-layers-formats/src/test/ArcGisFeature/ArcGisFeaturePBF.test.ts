...
describe("ArcGisFeaturePBF", () => {
  ...
  it("should convert pbf geometry type to Esri types", async () => {
    expect(
      ArcGisFeaturePBF.getArcGisFeatureGeometryType(
        esriPBuffer.FeatureCollectionPBuffer.GeometryType
    ...
    expect(
      ArcGisFeaturePBF.getArcGisFeatureGeometryType(
        esriPBuffer.FeatureCollectionPBuffer.GeometryType.esriGeometryTypePoint
      )
    ...
    expect(
      ArcGisFeaturePBF.getArcGisFeatureGeometryType(
        "" as unknown as esriPBuffer.FeatureCollectionPBuffer.GeometryType
      )
  ...
  it("should read FeatureInfo in PBF", async () => {
    const byteArray = Base64EncodedString.toUint8Array(
      PhillyLandmarksDataset.phillyAirportGetFeatureInfoQueryPbf
    );
    const featureCollection =
      esriPBuffer.FeatureCollectionPBuffer.deserialize(byteArray);
    ...
    featurePbf.readFeatureInfo(
      { data: featureCollection, exceedTransferLimit: false },
      results
    );

    // Make deep comparison by using JSON.stringify
    expect(JSON.stringify(results)).equals(
      JSON.stringify(
        PhillyLandmarksDataset.phillyAirportGetFeatureInfoResultRef
      )
    );
  });
  ...
  it("should read FeatureInfo from PBF (philly transportation)", async () => {
    ...
    expect(JSON.stringify(results)).equals(
      JSON.stringify(
        PhillyLandmarksDataset.phillyTansportationGetFeatureInfoResultRef
      )
    );
  });
  ...
  it("should read all data types from FeatureInfo ", async () => {
    ...
    const featureCollection = esriPBuffer.FeatureCollectionPBuffer.fromObject(
      PhillyLandmarksDataset.fieldsCoveragePbufferCollection
    );
    ...
    expect(JSON.stringify(results)).equals(
      JSON.stringify(
        PhillyLandmarksDataset.fieldsCoverageGetFeatureInfoResultRef
      )
    );
  });
  ...
  it("should readAndRender single ring polygon feature", async () => {
    ...
    const data = esriPBuffer.FeatureCollectionPBuffer.fromObject(
      PhillyLandmarksDataset.phillySimplePolyQueryPbf
    );
    const geomType = ArcGisFeaturePBF.getArcGisFeatureGeometryType(
      PhillyLandmarksDataset.phillySimplePolyQueryPbf.queryResult.featureResult
        .geometryType
    );
    ...
    const featureRenderer = new ArcGisFeatureRenderer(
      fakeContext,
      symbolRenderer
    );
    const renderPathSpy = sinon.spy(featureRenderer, "renderPath");
    featurePbf.readAndRender(
      { data, exceedTransferLimit: false },
      featureRenderer
    );
    ...
    expect(firstCall.args[0]).to.eql(
      PhillyLandmarksDataset.phillySimplePolyQueryPbf.queryResult.featureResult
        .features[0].geometry.lengths
    ); // geometryLengths
    expect(firstCall.args[1]).to.eql(
      PhillyLandmarksDataset.phillySimplePolyQueryPbf.queryResult.featureResult
        .features[0].geometry.coords
    ); // geometryCoords
  ...
  it("should readAndRender multiple ring polygon feature", async () => {
    ...
    const geomType = ArcGisFeaturePBF.getArcGisFeatureGeometryType(
      PhillyLandmarksDataset.phillyDoubleRingPolyQueryPbf.queryResult
        .featureResult.geometryType
    );
    const data = esriPBuffer.FeatureCollectionPBuffer.fromObject(
      PhillyLandmarksDataset.phillyDoubleRingPolyQueryPbf
    );
    ...
    expect(firstCall.args[0]).to.eql(
      PhillyLandmarksDataset.phillyDoubleRingPolyQueryPbf.queryResult
        .featureResult.features[0].geometry.lengths
    ); // geometryLengths
    expect(firstCall.args[1]).to.eql(
      PhillyLandmarksDataset.phillyDoubleRingPolyQueryPbf.queryResult
        .featureResult.features[0].geometry.coords
    ); // geometryCoords
    expect(firstCall.args[2]).to.eql(true); // fill
    expect(firstCall.args[3]).to.eql(2); // stride
  });
  ...
  it("should readAndRender simple path", async () => {
    const featurePbf = createFeaturePBF();
    const data = esriPBuffer.FeatureCollectionPBuffer.fromObject(
      PhillyLandmarksDataset.phillySimplePathQueryPbf
    );
    const symbolRenderer = new ArcGisSymbologyRenderer(
      ArcGisFeaturePBF.getArcGisFeatureGeometryType(
        PhillyLandmarksDataset.phillySimplePathQueryPbf.queryResult
          .featureResult.geometryType
      ),
    ...
    expect(firstCall.args[0]).to.eql(
      PhillyLandmarksDataset.phillySimplePathQueryPbf.queryResult.featureResult
        .features[0].geometry.lengths
    ); // geometryLengths
    expect(firstCall.args[1]).to.eql(
      PhillyLandmarksDataset.phillySimplePathQueryPbf.queryResult.featureResult
  ...
  it("should readAndRender multi path", async () => {
    const featurePbf = createFeaturePBF();
    const data = esriPBuffer.FeatureCollectionPBuffer.fromObject(
      PhillyLandmarksDataset.phillyMultiPathQueryPbf
    );
    const geomType = ArcGisFeaturePBF.getArcGisFeatureGeometryType(
      PhillyLandmarksDataset.phillyMultiPathQueryPbf.queryResult.featureResult
        .geometryType
    );
    const symbolRenderer = new ArcGisSymbologyRenderer(
      geomType,
  ...
  it("should readAndRender simple point", async () => {
    const featurePbf = createFeaturePBF();
    const data = esriPBuffer.FeatureCollectionPBuffer.fromObject(
      PhillyLandmarksDataset.phillySimplePointQueryPbf
    );
    const geomType = ArcGisFeaturePBF.getArcGisFeatureGeometryType(
      PhillyLandmarksDataset.phillyMultiPathQueryPbf.queryResult.featureResult
        .geometryType
    ...
    const geometryCoords =
      PhillyLandmarksDataset.phillySimplePointQueryPbf.queryResult.featureResult
        .features[0].geometry.coords;
  ...
  it("should log error when readAndRender /  readFeatureInfo is called invalid response Data", async () => {
    ...
    featurePbf.readAndRender(
      { data: { test: "test" }, exceedTransferLimit: false },
      featureRenderer
    );
