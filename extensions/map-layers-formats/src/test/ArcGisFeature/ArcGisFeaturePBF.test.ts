...
describe("ArcGisFeaturePBF", () => {
  ...
  it("should convert pbf geometry type to Esri types", async () => {
    ...
    expect(
      ArcGisFeaturePBF.getArcGisFeatureGeometryType(
        esriPBuffer.FeatureCollectionPBuffer.GeometryType.esriGeometryTypeMultipoint
      )
    ...
    expect(
      ArcGisFeaturePBF.getArcGisFeatureGeometryType(
        esriPBuffer.FeatureCollectionPBuffer.GeometryType.esriGeometryTypePolygon
      )
  ...
  it("should read FeatureInfo from PBF (philly transportation)", async () => {
    ...
    expect(JSON.stringify(results)).equals(
      JSON.stringify(PhillyLandmarksDataset.phillyTansportationGetFeatureInfoResultRef)
    );
  });
  ...
  it("should read all data types from FeatureInfo ", async () => {
    ...
    // Make deep comparison by using JSON.stringify
    expect(JSON.stringify(results)).equals(
      JSON.stringify(PhillyLandmarksDataset.fieldsCoverageGetFeatureInfoResultRef)
    );
  });
  ...
  it("should readAndRender single ring polygon feature", async () => {
    ...
    expect(firstCall.args[0]).to.eql(
      PhillyLandmarksDataset.phillySimplePolyQueryPbf.queryResult.featureResult.features[0].geometry.lengths
    ); // geometryLengths
    expect(firstCall.args[1]).to.eql(
      PhillyLandmarksDataset.phillySimplePolyQueryPbf.queryResult.featureResult.features[0].geometry.coords
    ); // geometryCoords
  ...
  it("should readAndRender multiple ring polygon feature", async () => {
    ...
    const firstCall = renderPathSpy.getCalls()[0];
    expect(firstCall.args[0]).to.eql(
      PhillyLandmarksDataset.phillyDoubleRingPolyQueryPbf.queryResult.featureResult.features[0].geometry.lengths
    ); // geometryLengths
    expect(firstCall.args[1]).to.eql(
      PhillyLandmarksDataset.phillyDoubleRingPolyQueryPbf.queryResult.featureResult.features[0].geometry.coords
    ); // geometryCoords
    expect(firstCall.args[2]).to.eql(true); // fill
    expect(firstCall.args[3]).to.eql(2); // stride
  });
  ...
  it("should readAndRender simple path", async () => {
    ...
    const symbolRenderer = new ArcGisSymbologyRenderer(
      ArcGisFeaturePBF.getArcGisFeatureGeometryType(
        PhillyLandmarksDataset.phillySimplePathQueryPbf.queryResult.featureResult.geometryType
      ),
    ...
    expect(firstCall.args[0]).to.eql(
      PhillyLandmarksDataset.phillySimplePathQueryPbf.queryResult.featureResult.features[0].geometry.lengths
    ); // geometryLengths
    expect(firstCall.args[1]).to.eql(
      PhillyLandmarksDataset.phillySimplePathQueryPbf.queryResult.featureResult.features[0].geometry.coords
    ); // geometryCoords
  ...
  it("should readAndRender multi path", async () => {
    ...
    const firstCall = renderPathSpy.getCalls()[0];
    expect(firstCall.args[0]).to.eql(
      PhillyLandmarksDataset.phillyMultiPathQueryPbf.queryResult.featureResult.features[0].geometry.lengths
    ); // geometryLengths
    expect(firstCall.args[1]).to.eql(
      PhillyLandmarksDataset.phillyMultiPathQueryPbf.queryResult.featureResult.features[0].geometry.coords
    ); // geometryCoords
    expect(firstCall.args[2]).to.eql(false); // fill
    expect(firstCall.args[3]).to.eql(2); // stride
  });
  ...
  it("should readAndRender simple point", async () => {
    ...
    const geometryCoords =
      PhillyLandmarksDataset.phillySimplePointQueryPbf.queryResult.featureResult.features[0].geometry.coords;
    const firstCall = spy.getCalls()[0];
    expect(firstCall.args[0]).to.eql(
      PhillyLandmarksDataset.phillySimplePointQueryPbf.queryResult.featureResult.features[0].geometry.lengths
    ); // geometryLengths
