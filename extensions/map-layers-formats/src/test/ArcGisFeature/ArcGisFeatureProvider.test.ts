...
describe("ArcGisFeatureProvider", () => {
  ...
  it("should initialize with valid data", async () => {
    sandbox
      ...
      .callsFake(async function _(
        ...
        _requireToken?: boolean
      ) {
        return {
          accessTokenRequired: false,
          content: NewYorkDataset.serviceCapabilities,
        };
      });

    sandbox
      .stub(ArcGisFeatureProvider.prototype, "getLayerMetadata" as any)
      .callsFake(async function _(_layerId: unknown) {
  ...
  it("should pick sub-layers from service metadata if none provided on layer settings", async () => {
    ...
    expect((provider as any)._layerMetadata).to.eql({
  ...
  it("should throw error if no layers in capabilities", async () => {
    sandbox
      ...
      .callsFake(async function _(
        ...
        return {
          accessTokenRequired: false,
          content: { capabilities: "Query", layers: [] },
        };
      });
  ...
  it("should read supported supported format", async () => {
    ...
    let getLayerMetadataStub = sandbox
      .stub(ArcGisFeatureProvider.prototype as any, "getLayerMetadata")
      .callsFake(async function (_id: unknown) {
        return { defaultVisibility: true, supportedQueryFormats: "PBF, JSON" };
      });

    const getServiceJsonStub = sandbox
      .stub(ArcGisUtilities, "getServiceJson")
      .callsFake(async function _(
        _url: string,
        _formatId: string,
  ...
  it("should process data in getFeatureInfo", async () => {
    ...
    sandbox
      .stub(ArcGISImageryProvider.prototype as any, "fetch")
      .callsFake(async function _(_url: unknown, _options?: unknown) {
        const test = {
  ...
  it("should compute computeTileWorld2CanvasTransform", async () => {
    ...
    const provider = new ArcGisFeatureProvider(settings);
    const getEPSG3857ExtentStub = sandbox
      .stub(ArcGisFeatureProvider.prototype, "getEPSG3857Extent")
      .callsFake(function (_row: number, _column: number, _zoomLevel: number) {
