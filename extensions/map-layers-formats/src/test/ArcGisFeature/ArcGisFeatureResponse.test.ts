...
describe("ArcGisFeatureResponse", () => {
  ...
  it("should create FeatureCollectionPBuffer from PBF data", async () => {
    ...
    expect(data?.data instanceof esriPBuffer.FeatureCollectionPBuffer).to.be
      .true;
  });

  it("should report exceededTransferLimit from PBF object", async () => {
    const collection = esriPBuffer.FeatureCollectionPBuffer.fromObject(
      PhillyLandmarksDataset.phillyExceededTransferLimitPbf
    );
