...
describe("ArcGisSymbologyRenderer", () => {
  ...
  it("should construct renderer from incomplete drawing info", async () => {
    let provider = new ArcGisSymbologyRenderer(
      "esriGeometryPoint",
  ...
  it("should construct renderer from unique value drawing info", async () => {
    const provider = new ArcGisSymbologyRenderer(
      "esriGeometryPoint",
  ...
  it("should construct renderer from point symbology drawing info", async () => {
    let provider = new ArcGisSymbologyRenderer(
      "esriGeometryPoint",
      PhillyLandmarksDataset.pointDrawingInfo.drawingInfo.renderer
    );
    comparePointSymbol(
      (provider as any)._symbol,
      PhillyLandmarksDataset.pointDrawingInfo.drawingInfo.renderer.symbol as any
    );

    provider = new ArcGisSymbologyRenderer(
      "esriGeometryPolyline",
      PhillyLandmarksDataset.lineDrawingInfo.drawingInfo.renderer
    );
    comparePointSymbol(
      (provider as any)._symbol,
      PhillyLandmarksDataset.lineDrawingInfo.drawingInfo.renderer.symbol as any
    );
