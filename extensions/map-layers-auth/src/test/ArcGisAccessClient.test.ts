...
describe("ArcGisUtilities tests", () => {
  ...
  const sampleOnlineAuthorize1 =
    "https://www.arcgis.com/sharing/rest/oauth2/authorize";

  it("should find the matching enterprise client id ", async () => {
    let clientId = (fakeAccessClient as any)!.getMatchingEnterpriseClientId(
      sampleOnPremiseHostName
    );
    expect(clientId).to.not.undefined;
    expect(
      clientId === fakeAccessClient?.arcGisEnterpriseClientIds![0].clientId
    );

    // Check it returns undefined if no match (and not default value set)
    clientId = (fakeAccessClient as any).getMatchingEnterpriseClientId(
      "https://yyz.com"
    );
  ...
  it("should construct the proper login Url from a enterprise token url", async () => {
    const loginURl = (fakeAccessClient as any).constructLoginUrl(
      sampleOnPremiseMapServerAuthorizeUrl,
      false
    );

    const refLoginUrl = new URL(sampleOnPremiseMapServerAuthorizeUrl);
    refLoginUrl.searchParams.append(
      "client_id",
      fakeAccessClient!.getMatchingEnterpriseClientId(
        "https://dtlgeoarcgis.adtl.com"
      )!
    );
    refLoginUrl.searchParams.append("response_type", "token");
    refLoginUrl.searchParams.append(
      "redirect_uri",
      fakeAccessClient!.redirectUri!
    );
  ...
  it("should construct the proper login Url from arcgis online token url", async () => {
    ...
    refLoginUrl.searchParams.append(
      "client_id",
      fakeAccessClient!.arcGisOnlineClientId!
    );
    refLoginUrl.searchParams.append("response_type", "token");
    refLoginUrl.searchParams.append(
      "redirect_uri",
  ...
  it("should build proper OAuth2 enterprise endpoint URL using generateTokenUrl", async () => {
    sandbox
      .stub(ArcGisUrl, "getRestUrlFromGenerateTokenUrl")
      .callsFake(async function _(_url: URL) {
        return Promise.resolve(new URL(sampleOnPremiseMapServerRestUrl));
      });

    const onPremiseEndpoint = await fakeAccessClient?.getTokenServiceEndPoint(
      sampleOnPremiseMapServer
    );
    ...
    expect(acOnPremiseEndpoint?.getUrl()).to.equals(
      sampleOnPremiseMapServerAuthorizeUrl
    );
  });
  ...
  it("should build proper OAuth2 enterprise endpoint URL if no generateTokenUrl response", async () => {
    ...
    expect(acOnPremiseEndpoint?.isArcgisOnline).to.false;
    expect(acOnPremiseEndpoint?.getUrl()).to.equals(
      sampleOnPremiseMapServerFallbackAuthorizeUrl
    );
  });
});
