...
function AttachLayerPanel({ isOverlay, onLayerAttached }: AttachLayerPanelProps) {
  ...
  const [placeholderLabel] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.SearchPlaceholder"));
  const [addCustomLayerLabel] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.Custom"));
  const [addCustomLayerToolTip] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.AttachCustomLayer"));
  const [loadingMapSources] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.LoadingMapSources"));
  const [removeLayerDefButtonTitle] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefButtonTitle"));
  const [editLayerDefButtonTitle] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.EditLayerDefButtonTitle"));
  const [removeLayerDefDialogTitle] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefDialogTitle"));
      ...
      if (layerName && activeViewport) {
        ...
        if (undefined === backgroundLayers?.find((layer) => layerName === layer.name) && undefined === overlayLayers?.find((layer) => layerName === layer.name)) {
          ...
          try {
            ...
            if (status === MapLayerSourceStatus.Valid || status === MapLayerSourceStatus.RequireAuth) {
              ...
              if (status === MapLayerSourceStatus.Valid) {
                ...
                if (layerSettings) {
                  ...
                  const msg = IModelApp.localizationClient.getLocalizedString("mapLayers:Messages.MapLayerAttached", { sourceName: layerSettings.name, sourceUrl: layerSettings.url });
              ...
              const msg = IModelApp.localizationClient.getLocalizedString("mapLayers:Messages.MapLayerValidationFailed", { sourceUrl: mapLayerSettings.url });
            ...
            const msg = IModelApp.localizationClient.getLocalizedString("mapLayers:Messages.MapLayerAttachError", { error: err, sourceUrl: mapLayerSettings.url });
            IModelApp.notifications.outputMessage(new NotifyMessageDetails(OutputMessagePriority.Error, msg));
          }
        }
  ...
  const handleYesConfirmation = React.useCallback(async (source: MapLayerSource) => {
    ...
    if (!!iTwinId && !!iModelId) {
      if (await MapLayerSettingsService.deleteSharedSettings(source, iTwinId, iModelId)) {
        const msg = MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefSuccess", { layerName });
        IModelApp.notifications.outputMessage(new NotifyMessageDetails(OutputMessagePriority.Info, msg));
      } else {
        const msg = MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefError", { layerName });
        IModelApp.notifications.outputMessage(new NotifyMessageDetails(OutputMessagePriority.Error, msg));
      }
    }
  ...
  const onItemRemoveButtonClicked = React.useCallback((source, event) => {
    ...
    const msg = MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefDialogMessage", { layerName });
    ModalDialogManager.openDialog(
...
export function AttachLayerPopupButton(props: AttachLayerPopupButtonProps) {
  const [showAttachLayerLabel] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:AttachLayerPopup.Attach"));
  const [hideAttachLayerLabel] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:AttachLayerPopup.Close"));
  const [addCustomLayerButtonLabel] = React.useState(MapLayersUiItemsProvider.localizationClient.getLocalizedString("mapLayers:CustomAttach.AddCustomLayerButtonLabel"));
