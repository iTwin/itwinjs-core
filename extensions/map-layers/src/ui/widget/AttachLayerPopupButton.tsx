...
function AttachLayerPanel({ isOverlay, onLayerAttached }: AttachLayerPanelProps) {
  ...
  const [placeholderLabel] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.SearchPlaceholder"));
  const [addCustomLayerLabel] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.Custom"));
  const [addCustomLayerToolTip] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.AttachCustomLayer"));
  const [loadingMapSources] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.LoadingMapSources"));
  const [removeLayerDefButtonTitle] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefButtonTitle"));
  const [editLayerDefButtonTitle] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.EditLayerDefButtonTitle"));
  const [removeLayerDefDialogTitle] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefDialogTitle"));
      ...
      if (layerName && activeViewport) {
        ...
        if (undefined === backgroundLayers?.find((layer) => layerName === layer.name) && undefined === overlayLayers?.find((layer) => layerName === layer.name)) {
          ...
          try {
            ...
            if (status === MapLayerSourceStatus.Valid || status === MapLayerSourceStatus.RequireAuth) {
              ...
              if (status === MapLayerSourceStatus.Valid) {
                ...
                if (layerSettings) {
                  ...
                  const msg = IModelApp.localizationProvider.getLocalizedString("mapLayers:Messages.MapLayerAttached", { sourceName: layerSettings.name, sourceUrl: layerSettings.url });
              ...
              const msg = IModelApp.localizationProvider.getLocalizedString("mapLayers:Messages.MapLayerValidationFailed", { sourceUrl: mapLayerSettings.url });
            ...
            const msg = IModelApp.localizationProvider.getLocalizedString("mapLayers:Messages.MapLayerAttachError", { error: err, sourceUrl: mapLayerSettings.url });
            IModelApp.notifications.outputMessage(new NotifyMessageDetails(OutputMessagePriority.Error, msg));
          }
        }
  ...
  const handleYesConfirmation = React.useCallback(async (source: MapLayerSource) => {
    ...
    if (!!contextId && !!iModelId) {
      if (await MapLayerSettingsService.deleteSharedSettings(source, contextId, iModelId)) {
        const msg = MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefSuccess", { layerName });
        IModelApp.notifications.outputMessage(new NotifyMessageDetails(OutputMessagePriority.Info, msg));
      } else {
        const msg = MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefError", { layerName });
        IModelApp.notifications.outputMessage(new NotifyMessageDetails(OutputMessagePriority.Error, msg));
      }
    }
  ...
  const onItemRemoveButtonClicked = React.useCallback((source, event) => {
    ...
    const msg = MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.RemoveLayerDefDialogMessage", { layerName });
    ModalDialogManager.openDialog(
...
export function AttachLayerPopupButton(props: AttachLayerPopupButtonProps) {
  const [showAttachLayerLabel] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:AttachLayerPopup.Attach"));
  const [hideAttachLayerLabel] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:AttachLayerPopup.Close"));
  const [addCustomLayerButtonLabel] = React.useState(MapLayersUiItemsProvider.localizationProvider.getLocalizedString("mapLayers:CustomAttach.AddCustomLayerButtonLabel"));
