...
import { ColorDef, Frustum, Npc } from "@itwin/core-common";
import { CollectTileStatus, DecorateContext, Decorator, DisclosedTileTreeSet, GeometryTileTreeReference, GraphicPrimitive, GraphicType, IModelApp, MapTileTree, MapTileTreeReference, RenderGraphic, Tile, TileGeometryCollector, TileTreeReference, TileUser, Viewport } from "@itwin/core-frontend";
...
export class MapFeatureInfoDecorator implements Decorator {
  ...
  protected renderGraphics(context: DecorateContext) {
    ...
    if (this._draper && this._state.graphics) {
      ...
      const tolerance = drapeRange.diagonal().magnitude() / 10000;
