...
if (ProcessDetector.isElectronAppFrontend) {
  describe("RobotWorldRpc", () => {
    it("should run robotWorld through Ipc as a client", async () => {
      ...
      const iModel = await BriefcaseConnection.openFile({
        fileName: `${KnownTestLocations.outputDir}/` + `RobotWorldRpc.bim`,
      });
      assert.isTrue(iModel !== undefined);
      const iToken = iModel.getRpcProps();

      let modelId!: Id64String;
      for (const modelStr of await iModel.queryEntityIds({
        from: "bis:element",
        where: "CodeValue='test'",
      }))
      ...
      const barrier1Id = await roWrite.insertBarrier(
        iToken,
        modelId,
        Point3d.create(0, 5, 0).toJSON(),
        Angle.createDegrees(0).toJSON(),
        5
      );
      const barrier2Id = await roWrite.insertBarrier(
        iToken,
        modelId,
        Point3d.create(5, 0, 0).toJSON(),
        Angle.createDegrees(90).toJSON(),
        5
      );
