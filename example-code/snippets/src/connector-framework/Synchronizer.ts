...
export class Synchronizer {
  ...
  public get unchangedSources() {
    return this._unchangedSources;
  }

  public set jobSubjectId(id: Id64String) {
  ...
  private getOrCreateExternalSource(repositoryLinkId: Id64String, modelId: Id64String): Id64String {
    ...
    const xseProps: ExternalSourceProps = {
      ...
      repository: {
        id: repositoryLinkId,
        relClassName: RepositoryLink.classFullName,
      },
  ...
  public recordDocument(sourceDocument: SourceDocument): RecordDocumentResults {
    ...
    const sourceItem: ItemWithScopeAndKind = {
      ...
      checksum: () => {
        return sourceDocument.checksum?.();
      },
    };
    ...
    if (undefined === repositoryLink) {
      throw new IModelError(
        ...
        `Failed to create repositoryLink for ${JSON.stringify(sourceDocument)}`
      );
    }
    ...
    const aspectNoVersion: ExternalSourceAspectProps = {
      ...
      version: undefined,
      checksum: undefined,
    }; // at the start, erase the version and checksum values
  ...
  public detectChanges(item: SourceItem): ChangeResults {
    ...
    if (undefined === aspect) return results;
   ...
   * @beta
   */
  public insertResultsIntoIModel(
    ...
    this.imodel.elements.insertAspect({
      ...aspectProps,
      element: { id: elid },
    }); // throws on error
    ...
    for (const child of results.childElements) {
      const parent = new RelatedElement({
        id: elid,
  ...
  public updateResultsInIModel(results: SynchronizationResults, aspectProps: ExternalSourceAspectProps): IModelStatus {
    ...
    this.imodel.elements.updateAspect({
      ...aspectProps,
      element: { id: results.elementProps.id! },
    }); // throws on error
  ...
  private updateResultsInIModelForChildren(
    results: SynchronizationResults,
    ...
    if (
      undefined !== results.childElements[0].elementProps &&
      results.childElements[0].elementProps.id !== undefined &&
      Id64.isValidId64(results.childElements[0].elementProps.id)
    ) {
    ...
    if (undefined === repositoryLink) {
      ...
      if (docProps !== undefined) {
        ...
        elementProps.jsonProperties = {
          DocumentProperties: {
            desktopURN: docProps.desktopURN,
            webURN: docProps.webURN,
            attributes: docProps.attributesJson,
          },
        };
      }
  ...
  public static parseDocGuidFromPwUri(pwUri: string): GuidString {
    ...
    if (-1 === startDguid) return emptyGuid;
    const endDguid = pwUri.indexOf("}", startDguid);
    if (-1 === endDguid) return emptyGuid;
  ...
  private makeExternalSourceAspectPropsFromSourceItem(
    sourceItem: ItemWithScopeAndKind,
