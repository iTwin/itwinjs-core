...
describe("ChangesetReader Examples — SuppressedError handling", () => {
  let db: BriefcaseDb;
  let txn: EditTxn;
  let txnId: string;
  const adminToken = "super manager token";
  ...
  before(async () => {
    HubMock.startup("ChangesetReaderSuppressedError", KnownTestLocations.outputDir);
    const iTwinId = HubMock.iTwinId;
    const iModelId = await HubMock.createNewIModel({ iTwinId, iModelName: "suppressedErrorDemo", accessToken: adminToken });
    db = await HubWrappers.downloadAndOpenBriefcase({ iTwinId, iModelId, accessToken: adminToken });
    txn = startTestTxn(db, "suppressed error demo");
    db.channels.addAllowedChannel(ChannelControl.sharedChannelName);
    await db.locks.acquireLocks({ shared: IModel.dictionaryId });
    BackendTestUtils.createAndInsertDrawingPartitionAndModel(txn, Code.createEmpty(), true);
    txn.saveChanges("setup");
    txnId = db.txns.getLastSavedTxnProps()!.id;
  });
  ...
  it("SuppressedError — handling when body and disposal both throw", () => {
    // __PUBLISH_EXTRACT_START__ ChangesetReader.SuppressedError
    function processChanges(id: string): void {
      try {
        using reader = ChangesetReader.openTxn({ db, txnId: id });
        using pcu = new PartialChangeUnifier(ChangeUnifierCache.createInMemoryCache());
        while (reader.step())
          pcu.appendFrom(reader);
        for (const instance of pcu.instances) {
        ...
        // `using` raises SuppressedError when both the operation body *and* a
        // [Symbol.dispose]() call throw.  Guard with typeof before instanceof because
        // the global SuppressedError is absent in older Node.js runtimes.
        //  Without the guard the check itself throws a ReferenceError at runtime.
        if (typeof SuppressedError !== "undefined" && err instanceof SuppressedError) {
          throw new Error(
            `Change processing failed: ${err.suppressed}. ` +
            `Additionally, resource disposal failed: ${err.error}.`,
          );
        }
        throw err;
      }
